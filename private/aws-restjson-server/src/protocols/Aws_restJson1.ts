...
export const deserializeMalformedListRequest = async (
  ...
  const data: Record<string, any> = __expectNonNull(__expectObject(await parseBody(output.body, context)), "body");
  const doc = take(data, {
    bodyList: [, (_) => deserializeAws_restJson1SimpleList(_, context)],
  });
  Object.assign(contents, doc);
  return contents;
};
...
export const deserializeMalformedMapRequest = async (
  ...
  const data: Record<string, any> = __expectNonNull(__expectObject(await parseBody(output.body, context)), "body");
  const doc = take(data, {
    bodyMap: [, (_) => deserializeAws_restJson1SimpleMap(_, context)],
  });
  Object.assign(contents, doc);
  return contents;
};
...
export const deserializeMalformedUnionRequest = async (
  ...
  const data: Record<string, any> = __expectNonNull(__expectObject(await parseBody(output.body, context)), "body");
  const doc = take(data, {
    union: [, (_) => deserializeAws_restJson1SimpleUnion(__expectUnion(_), context)],
  });
  Object.assign(contents, doc);
  return contents;
};
...
export const serializeComplexErrorError = async (
  ...
  body = JSON.stringify(
    take(input, {
      Nested: [, (_) => serializeAws_restJson1ComplexNestedErrorData(_, context)],
      TopLevel: [],
    })
  );
  return new __HttpResponse({
