...
export type MalformedTimestampPathEpoch<Context> = __Operation<MalformedTimestampPathEpochServerInput, MalformedTimestampPathEpochServerOutput, Context>
...
export namespace MalformedTimestampPathEpochServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof MalformedTimestampPathEpochInput.validate>[0]) => __ValidationFailure[] = MalformedTimestampPathEpochInput.validate;
}
export interface MalformedTimestampPathEpochServerOutput {}

export type MalformedTimestampPathEpochErrors = never;

export class MalformedTimestampPathEpochSerializer implements __OperationSerializer<RestJsonService<any>, "MalformedTimestampPathEpoch", MalformedTimestampPathEpochErrors> {
...
export const getMalformedTimestampPathEpochHandler = <Context>(operation: __Operation<MalformedTimestampPathEpochServerInput, MalformedTimestampPathEpochServerOutput, Context>, customizer: __ValidationCustomizer<"MalformedTimestampPathEpoch">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new MalformedTimestampPathEpochHandler(operation, mux, new MalformedTimestampPathEpochSerializer(), serializeFrameworkException, customizer);
}
...
export class MalformedTimestampPathEpochHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly operation: __Operation<MalformedTimestampPathEpochServerInput, MalformedTimestampPathEpochServerOutput, Context>;
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "MalformedTimestampPathEpoch", MalformedTimestampPathEpochErrors>;
  ...
  constructor(
    ...
    serializer: __OperationSerializer<RestJsonService<Context>, "MalformedTimestampPathEpoch", MalformedTimestampPathEpochErrors>,
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedTimestampPathEpoch" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedTimestampPathEpochServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedTimestampPathEpoch" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedTimestampPathEpochServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedTimestampPathEpochServerOutput, serdeContextBase),
