...
export type NullAndEmptyHeadersClient<Context> = __Operation<NullAndEmptyHeadersClientServerInput, NullAndEmptyHeadersClientServerOutput, Context>
...
export class NullAndEmptyHeadersClientSerializer implements __OperationSerializer<RestJsonService<any>, "NullAndEmptyHeadersClient", NullAndEmptyHeadersClientErrors> {
...
export const getNullAndEmptyHeadersClientHandler = <Context>(operation: __Operation<NullAndEmptyHeadersClientServerInput, NullAndEmptyHeadersClientServerOutput, Context>, customizer: __ValidationCustomizer<"NullAndEmptyHeadersClient">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new NullAndEmptyHeadersClientHandler(operation, mux, new NullAndEmptyHeadersClientSerializer(), serializeFrameworkException, customizer);
}
...
export class NullAndEmptyHeadersClientHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly operation: __Operation<NullAndEmptyHeadersClientServerInput, NullAndEmptyHeadersClientServerOutput, Context>;
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "NullAndEmptyHeadersClient", NullAndEmptyHeadersClientErrors>;
  ...
  constructor(
    ...
    serializer: __OperationSerializer<RestJsonService<Context>, "NullAndEmptyHeadersClient", NullAndEmptyHeadersClientErrors>,
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "NullAndEmptyHeadersClient" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (NullAndEmptyHeadersClientServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "NullAndEmptyHeadersClient" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as NullAndEmptyHeadersClientServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as NullAndEmptyHeadersClientServerOutput, serdeContextBase),
