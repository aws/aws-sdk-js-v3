...
export type MalformedTimestampPathHttpDate<Context> = __Operation<MalformedTimestampPathHttpDateServerInput, MalformedTimestampPathHttpDateServerOutput, Context>
...
export namespace MalformedTimestampPathHttpDateServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof MalformedTimestampPathHttpDateInput.validate>[0]) => __ValidationFailure[] = MalformedTimestampPathHttpDateInput.validate;
}
export interface MalformedTimestampPathHttpDateServerOutput {}

export type MalformedTimestampPathHttpDateErrors = never;

export class MalformedTimestampPathHttpDateSerializer implements __OperationSerializer<RestJsonService<any>, "MalformedTimestampPathHttpDate", MalformedTimestampPathHttpDateErrors> {
...
export const getMalformedTimestampPathHttpDateHandler = <Context>(operation: __Operation<MalformedTimestampPathHttpDateServerInput, MalformedTimestampPathHttpDateServerOutput, Context>, customizer: __ValidationCustomizer<"MalformedTimestampPathHttpDate">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new MalformedTimestampPathHttpDateHandler(operation, mux, new MalformedTimestampPathHttpDateSerializer(), serializeFrameworkException, customizer);
}
...
export class MalformedTimestampPathHttpDateHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly operation: __Operation<MalformedTimestampPathHttpDateServerInput, MalformedTimestampPathHttpDateServerOutput, Context>;
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "MalformedTimestampPathHttpDate", MalformedTimestampPathHttpDateErrors>;
   ...
   */
  constructor(
    operation: __Operation<MalformedTimestampPathHttpDateServerInput, MalformedTimestampPathHttpDateServerOutput, Context>,
    mux: __Mux<"RestJson", "MalformedTimestampPathHttpDate">,
    serializer: __OperationSerializer<RestJsonService<Context>, "MalformedTimestampPathHttpDate", MalformedTimestampPathHttpDateErrors>,
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedTimestampPathHttpDate" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedTimestampPathHttpDateServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedTimestampPathHttpDate" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedTimestampPathHttpDateServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedTimestampPathHttpDateServerOutput, serdeContextBase),
