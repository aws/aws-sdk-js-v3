...
export type TimestampFormatHeaders<Context> = __Operation<TimestampFormatHeadersServerInput, TimestampFormatHeadersServerOutput, Context>
...
export namespace TimestampFormatHeadersServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof TimestampFormatHeadersIO.validate>[0]) => __ValidationFailure[] = TimestampFormatHeadersIO.validate;
}
...
export class TimestampFormatHeadersSerializer implements __OperationSerializer<RestJsonService<any>, "TimestampFormatHeaders", TimestampFormatHeadersErrors> {
...
export const getTimestampFormatHeadersHandler = <Context>(operation: __Operation<TimestampFormatHeadersServerInput, TimestampFormatHeadersServerOutput, Context>, customizer: __ValidationCustomizer<"TimestampFormatHeaders">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new TimestampFormatHeadersHandler(operation, mux, new TimestampFormatHeadersSerializer(), serializeFrameworkException, customizer);
}
...
export class TimestampFormatHeadersHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly operation: __Operation<TimestampFormatHeadersServerInput, TimestampFormatHeadersServerOutput, Context>;
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "TimestampFormatHeaders", TimestampFormatHeadersErrors>;
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "TimestampFormatHeaders" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (TimestampFormatHeadersServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "TimestampFormatHeaders" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as TimestampFormatHeadersServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as TimestampFormatHeadersServerOutput, serdeContextBase),
