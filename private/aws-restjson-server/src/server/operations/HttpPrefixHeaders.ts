...
export type HttpPrefixHeaders<Context> = __Operation<HttpPrefixHeadersServerInput, HttpPrefixHeadersServerOutput, Context>
...
export namespace HttpPrefixHeadersServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof HttpPrefixHeadersInput.validate>[0]) => __ValidationFailure[] = HttpPrefixHeadersInput.validate;
}
export interface HttpPrefixHeadersServerOutput extends HttpPrefixHeadersOutput {}

export type HttpPrefixHeadersErrors = never;

export class HttpPrefixHeadersSerializer implements __OperationSerializer<RestJsonService<any>, "HttpPrefixHeaders", HttpPrefixHeadersErrors> {
...
export const getHttpPrefixHeadersHandler = <Context>(operation: __Operation<HttpPrefixHeadersServerInput, HttpPrefixHeadersServerOutput, Context>, customizer: __ValidationCustomizer<"HttpPrefixHeaders">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new HttpPrefixHeadersHandler(operation, mux, new HttpPrefixHeadersSerializer(), serializeFrameworkException, customizer);
}
...
export class HttpPrefixHeadersHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "HttpPrefixHeaders", HttpPrefixHeadersErrors>;
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "HttpPrefixHeaders" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (HttpPrefixHeadersServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "HttpPrefixHeaders" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as HttpPrefixHeadersServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as HttpPrefixHeadersServerOutput, serdeContextBase),
