...
export class OutputStreamSerializer implements __OperationSerializer<RestJsonService<any>, "OutputStream", OutputStreamErrors> {
...
export const getOutputStreamHandler = <Context>(operation: __Operation<OutputStreamServerInput, OutputStreamServerOutput, Context>, customizer: __ValidationCustomizer<"OutputStream">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
...
export class OutputStreamHandler<Context> implements __ServiceHandler<Context> {
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "OutputStream" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (OutputStreamServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "OutputStream" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as OutputStreamServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as OutputStreamServerOutput, serdeContextBase),
