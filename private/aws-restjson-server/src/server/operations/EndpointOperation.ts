...
export class EndpointOperationHandler<Context> implements __ServiceHandler<Context> {
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "EndpointOperation" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (EndpointOperationServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "EndpointOperation" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as EndpointOperationServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as EndpointOperationServerOutput, serdeContextBase),
