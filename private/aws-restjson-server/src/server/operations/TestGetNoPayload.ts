...
export class TestGetNoPayloadSerializer implements __OperationSerializer<RestJsonService<any>, "TestGetNoPayload", TestGetNoPayloadErrors> {
...
export const getTestGetNoPayloadHandler = <Context>(operation: __Operation<TestGetNoPayloadServerInput, TestGetNoPayloadServerOutput, Context>, customizer: __ValidationCustomizer<"TestGetNoPayload">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new TestGetNoPayloadHandler(operation, mux, new TestGetNoPayloadSerializer(), serializeFrameworkException, customizer);
}
...
export class TestGetNoPayloadHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "TestGetNoPayload", TestGetNoPayloadErrors>;
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "TestGetNoPayload" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (TestGetNoPayloadServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "TestGetNoPayload" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as TestGetNoPayloadServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as TestGetNoPayloadServerOutput, serdeContextBase),
