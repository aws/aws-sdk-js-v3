...
export namespace JsonMapsServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof JsonMapsInputOutput.validate>[0]) => __ValidationFailure[] = JsonMapsInputOutput.validate;
}
...
export const getJsonMapsHandler = <Context>(operation: __Operation<JsonMapsServerInput, JsonMapsServerOutput, Context>, customizer: __ValidationCustomizer<"JsonMaps">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
...
export class JsonMapsHandler<Context> implements __ServiceHandler<Context> {
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "JsonMaps" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (JsonMapsServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "JsonMaps" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as JsonMapsServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as JsonMapsServerOutput, serdeContextBase),
