...
export namespace PostPlayerActionServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof PostPlayerActionInput.validate>[0]) => __ValidationFailure[] = PostPlayerActionInput.validate;
}
export interface PostPlayerActionServerOutput extends PostPlayerActionOutput {}

export type PostPlayerActionErrors = never;

export class PostPlayerActionSerializer implements __OperationSerializer<RestJsonService<any>, "PostPlayerAction", PostPlayerActionErrors> {
...
export const getPostPlayerActionHandler = <Context>(operation: __Operation<PostPlayerActionServerInput, PostPlayerActionServerOutput, Context>, customizer: __ValidationCustomizer<"PostPlayerAction">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new PostPlayerActionHandler(operation, mux, new PostPlayerActionSerializer(), serializeFrameworkException, customizer);
}
...
export class PostPlayerActionHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "PostPlayerAction", PostPlayerActionErrors>;
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "PostPlayerAction" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (PostPlayerActionServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "PostPlayerAction" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as PostPlayerActionServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as PostPlayerActionServerOutput, serdeContextBase),
