...
import {
  deserializeDatetimeOffsetsRequest,
  serializeDatetimeOffsetsResponse,
...
export class DatetimeOffsetsSerializer implements __OperationSerializer<RestJsonService<any>, "DatetimeOffsets", DatetimeOffsetsErrors> {
...
export const getDatetimeOffsetsHandler = <Context>(operation: __Operation<DatetimeOffsetsServerInput, DatetimeOffsetsServerOutput, Context>, customizer: __ValidationCustomizer<"DatetimeOffsets">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new DatetimeOffsetsHandler(operation, mux, new DatetimeOffsetsSerializer(), serializeFrameworkException, customizer);
}
...
export class DatetimeOffsetsHandler<Context> implements __ServiceHandler<Context> {
  private readonly mux: __Mux<"RestJson", "DatetimeOffsets">;
  private readonly operation: __Operation<DatetimeOffsetsServerInput, DatetimeOffsetsServerOutput, Context>;
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "DatetimeOffsets", DatetimeOffsetsErrors>;
  ...
  /**
   * Construct a DatetimeOffsets handler.
   * @param operation The {@link __Operation} implementation that supplies the business logic for DatetimeOffsets
   * @param mux The {@link __Mux} that verifies which service and operation are being invoked by a given {@link __HttpRequest}
   * @param serializer An {@link __OperationSerializer} for DatetimeOffsets that
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "DatetimeOffsets" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (DatetimeOffsetsServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "DatetimeOffsets" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as DatetimeOffsetsServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as DatetimeOffsetsServerOutput, serdeContextBase),
