...
export namespace JsonIntEnumsServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof JsonIntEnumsInputOutput.validate>[0]) => __ValidationFailure[] = JsonIntEnumsInputOutput.validate;
}
export interface JsonIntEnumsServerOutput extends JsonIntEnumsInputOutput {}

export type JsonIntEnumsErrors = never;

export class JsonIntEnumsSerializer implements __OperationSerializer<RestJsonService<any>, "JsonIntEnums", JsonIntEnumsErrors> {
...
export const getJsonIntEnumsHandler = <Context>(operation: __Operation<JsonIntEnumsServerInput, JsonIntEnumsServerOutput, Context>, customizer: __ValidationCustomizer<"JsonIntEnums">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
...
export class JsonIntEnumsHandler<Context> implements __ServiceHandler<Context> {
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "JsonIntEnums" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (JsonIntEnumsServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "JsonIntEnums" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as JsonIntEnumsServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as JsonIntEnumsServerOutput, serdeContextBase),
