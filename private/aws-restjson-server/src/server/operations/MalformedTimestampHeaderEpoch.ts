...
export type MalformedTimestampHeaderEpoch<Context> = __Operation<MalformedTimestampHeaderEpochServerInput, MalformedTimestampHeaderEpochServerOutput, Context>
...
export namespace MalformedTimestampHeaderEpochServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof MalformedTimestampHeaderEpochInput.validate>[0]) => __ValidationFailure[] = MalformedTimestampHeaderEpochInput.validate;
}
export interface MalformedTimestampHeaderEpochServerOutput {}

export type MalformedTimestampHeaderEpochErrors = never;

export class MalformedTimestampHeaderEpochSerializer implements __OperationSerializer<RestJsonService<any>, "MalformedTimestampHeaderEpoch", MalformedTimestampHeaderEpochErrors> {
...
export const getMalformedTimestampHeaderEpochHandler = <Context>(operation: __Operation<MalformedTimestampHeaderEpochServerInput, MalformedTimestampHeaderEpochServerOutput, Context>, customizer: __ValidationCustomizer<"MalformedTimestampHeaderEpoch">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new MalformedTimestampHeaderEpochHandler(operation, mux, new MalformedTimestampHeaderEpochSerializer(), serializeFrameworkException, customizer);
}
...
export class MalformedTimestampHeaderEpochHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly operation: __Operation<MalformedTimestampHeaderEpochServerInput, MalformedTimestampHeaderEpochServerOutput, Context>;
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "MalformedTimestampHeaderEpoch", MalformedTimestampHeaderEpochErrors>;
  ...
  constructor(
    ...
    serializer: __OperationSerializer<RestJsonService<Context>, "MalformedTimestampHeaderEpoch", MalformedTimestampHeaderEpochErrors>,
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedTimestampHeaderEpoch" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedTimestampHeaderEpochServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedTimestampHeaderEpoch" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedTimestampHeaderEpochServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedTimestampHeaderEpochServerOutput, serdeContextBase),
