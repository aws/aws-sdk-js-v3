...
export namespace InputStreamServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof InputStreamInput.validate>[0]) => __ValidationFailure[] = InputStreamInput.validate;
}
export interface InputStreamServerOutput {}

export type InputStreamErrors = never;

export class InputStreamSerializer implements __OperationSerializer<RestJsonService<any>, "InputStream", InputStreamErrors> {
...
export const getInputStreamHandler = <Context>(operation: __Operation<InputStreamServerInput, InputStreamServerOutput, Context>, customizer: __ValidationCustomizer<"InputStream">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
...
export class InputStreamHandler<Context> implements __ServiceHandler<Context> {
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "InputStream" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (InputStreamServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "InputStream" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as InputStreamServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as InputStreamServerOutput, serdeContextBase),
