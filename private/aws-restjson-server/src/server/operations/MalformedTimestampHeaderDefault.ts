...
export type MalformedTimestampHeaderDefault<Context> = __Operation<MalformedTimestampHeaderDefaultServerInput, MalformedTimestampHeaderDefaultServerOutput, Context>
...
export namespace MalformedTimestampHeaderDefaultServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof MalformedTimestampHeaderDefaultInput.validate>[0]) => __ValidationFailure[] = MalformedTimestampHeaderDefaultInput.validate;
}
export interface MalformedTimestampHeaderDefaultServerOutput {}

export type MalformedTimestampHeaderDefaultErrors = never;

export class MalformedTimestampHeaderDefaultSerializer implements __OperationSerializer<RestJsonService<any>, "MalformedTimestampHeaderDefault", MalformedTimestampHeaderDefaultErrors> {
...
export const getMalformedTimestampHeaderDefaultHandler = <Context>(operation: __Operation<MalformedTimestampHeaderDefaultServerInput, MalformedTimestampHeaderDefaultServerOutput, Context>, customizer: __ValidationCustomizer<"MalformedTimestampHeaderDefault">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  const mux = new httpbinding.HttpBindingMux<"RestJson", "MalformedTimestampHeaderDefault">([
  ...
  return new MalformedTimestampHeaderDefaultHandler(operation, mux, new MalformedTimestampHeaderDefaultSerializer(), serializeFrameworkException, customizer);
}
...
export class MalformedTimestampHeaderDefaultHandler<Context> implements __ServiceHandler<Context> {
  private readonly mux: __Mux<"RestJson", "MalformedTimestampHeaderDefault">;
  private readonly operation: __Operation<MalformedTimestampHeaderDefaultServerInput, MalformedTimestampHeaderDefaultServerOutput, Context>;
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "MalformedTimestampHeaderDefault", MalformedTimestampHeaderDefaultErrors>;
  private readonly serializeFrameworkException: (e: __SmithyFrameworkException, ctx: __ServerSerdeContext) => Promise<__HttpResponse>;
  private readonly validationCustomizer: __ValidationCustomizer<"MalformedTimestampHeaderDefault">;
  private readonly interceptors: __ServerInterceptor<Context>[] = [];
  private readonly authSchemes: __AuthScheme<Context>[] = [];
  /**
   * Construct a MalformedTimestampHeaderDefault handler.
   * @param operation The {@link __Operation} implementation that supplies the business logic for MalformedTimestampHeaderDefault
   * @param mux The {@link __Mux} that verifies which service and operation are being invoked by a given {@link __HttpRequest}
   * @param serializer An {@link __OperationSerializer} for MalformedTimestampHeaderDefault that
   *                   handles deserialization of requests and serialization of responses
   * @param serializeFrameworkException A function that can serialize {@link __SmithyFrameworkException}s
   * @param validationCustomizer A {@link __ValidationCustomizer} for turning validation failures into {@link __SmithyFrameworkException}s
   */
  constructor(
    operation: __Operation<MalformedTimestampHeaderDefaultServerInput, MalformedTimestampHeaderDefaultServerOutput, Context>,
    mux: __Mux<"RestJson", "MalformedTimestampHeaderDefault">,
    serializer: __OperationSerializer<RestJsonService<Context>, "MalformedTimestampHeaderDefault", MalformedTimestampHeaderDefaultErrors>,
    serializeFrameworkException: (e: __SmithyFrameworkException, ctx: __ServerSerdeContext) => Promise<__HttpResponse>,
    validationCustomizer: __ValidationCustomizer<"MalformedTimestampHeaderDefault">
  ) {
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedTimestampHeaderDefault" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedTimestampHeaderDefaultServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedTimestampHeaderDefault" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedTimestampHeaderDefaultServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedTimestampHeaderDefaultServerOutput, serdeContextBase),
