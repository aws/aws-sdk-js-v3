...
export type NullAndEmptyHeadersServer<Context> = __Operation<NullAndEmptyHeadersServerServerInput, NullAndEmptyHeadersServerServerOutput, Context>
...
export namespace NullAndEmptyHeadersServerServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof NullAndEmptyHeadersIO.validate>[0]) => __ValidationFailure[] = NullAndEmptyHeadersIO.validate;
}
...
export class NullAndEmptyHeadersServerSerializer implements __OperationSerializer<RestJsonService<any>, "NullAndEmptyHeadersServer", NullAndEmptyHeadersServerErrors> {
...
export const getNullAndEmptyHeadersServerHandler = <Context>(operation: __Operation<NullAndEmptyHeadersServerServerInput, NullAndEmptyHeadersServerServerOutput, Context>, customizer: __ValidationCustomizer<"NullAndEmptyHeadersServer">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new NullAndEmptyHeadersServerHandler(operation, mux, new NullAndEmptyHeadersServerSerializer(), serializeFrameworkException, customizer);
}
...
export class NullAndEmptyHeadersServerHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly operation: __Operation<NullAndEmptyHeadersServerServerInput, NullAndEmptyHeadersServerServerOutput, Context>;
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "NullAndEmptyHeadersServer", NullAndEmptyHeadersServerErrors>;
  ...
  constructor(
    ...
    serializer: __OperationSerializer<RestJsonService<Context>, "NullAndEmptyHeadersServer", NullAndEmptyHeadersServerErrors>,
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "NullAndEmptyHeadersServer" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (NullAndEmptyHeadersServerServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "NullAndEmptyHeadersServer" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as NullAndEmptyHeadersServerServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as NullAndEmptyHeadersServerServerOutput, serdeContextBase),
