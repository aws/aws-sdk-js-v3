...
export type HttpRequestWithLabels<Context> = __Operation<HttpRequestWithLabelsServerInput, HttpRequestWithLabelsServerOutput, Context>
...
export namespace HttpRequestWithLabelsServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof HttpRequestWithLabelsInput.validate>[0]) => __ValidationFailure[] = HttpRequestWithLabelsInput.validate;
}
export interface HttpRequestWithLabelsServerOutput {}

export type HttpRequestWithLabelsErrors = never;

export class HttpRequestWithLabelsSerializer implements __OperationSerializer<RestJsonService<any>, "HttpRequestWithLabels", HttpRequestWithLabelsErrors> {
...
export const getHttpRequestWithLabelsHandler = <Context>(operation: __Operation<HttpRequestWithLabelsServerInput, HttpRequestWithLabelsServerOutput, Context>, customizer: __ValidationCustomizer<"HttpRequestWithLabels">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new HttpRequestWithLabelsHandler(operation, mux, new HttpRequestWithLabelsSerializer(), serializeFrameworkException, customizer);
}
...
export class HttpRequestWithLabelsHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "HttpRequestWithLabels", HttpRequestWithLabelsErrors>;
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "HttpRequestWithLabels" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (HttpRequestWithLabelsServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "HttpRequestWithLabels" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as HttpRequestWithLabelsServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as HttpRequestWithLabelsServerOutput, serdeContextBase),
