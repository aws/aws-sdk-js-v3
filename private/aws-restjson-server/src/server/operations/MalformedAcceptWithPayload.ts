...
export type MalformedAcceptWithPayload<Context> = __Operation<MalformedAcceptWithPayloadServerInput, MalformedAcceptWithPayloadServerOutput, Context>
...
export class MalformedAcceptWithPayloadSerializer implements __OperationSerializer<RestJsonService<any>, "MalformedAcceptWithPayload", MalformedAcceptWithPayloadErrors> {
...
export const getMalformedAcceptWithPayloadHandler = <Context>(operation: __Operation<MalformedAcceptWithPayloadServerInput, MalformedAcceptWithPayloadServerOutput, Context>, customizer: __ValidationCustomizer<"MalformedAcceptWithPayload">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new MalformedAcceptWithPayloadHandler(operation, mux, new MalformedAcceptWithPayloadSerializer(), serializeFrameworkException, customizer);
}
...
export class MalformedAcceptWithPayloadHandler<Context> implements __ServiceHandler<Context> {
  private readonly mux: __Mux<"RestJson", "MalformedAcceptWithPayload">;
  private readonly operation: __Operation<MalformedAcceptWithPayloadServerInput, MalformedAcceptWithPayloadServerOutput, Context>;
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "MalformedAcceptWithPayload", MalformedAcceptWithPayloadErrors>;
  ...
  /**
   * Construct a MalformedAcceptWithPayload handler.
   * @param operation The {@link __Operation} implementation that supplies the business logic for MalformedAcceptWithPayload
   * @param mux The {@link __Mux} that verifies which service and operation are being invoked by a given {@link __HttpRequest}
   * @param serializer An {@link __OperationSerializer} for MalformedAcceptWithPayload that
  ...
  constructor(
    ...
    mux: __Mux<"RestJson", "MalformedAcceptWithPayload">,
    serializer: __OperationSerializer<RestJsonService<Context>, "MalformedAcceptWithPayload", MalformedAcceptWithPayloadErrors>,
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedAcceptWithPayload" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedAcceptWithPayloadServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedAcceptWithPayload" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedAcceptWithPayloadServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedAcceptWithPayloadServerOutput, serdeContextBase),
