...
import {
  deserializeMalformedIntegerRequest,
  serializeFrameworkException,
  serializeMalformedIntegerResponse,
...
export class MalformedIntegerHandler<Context> implements __ServiceHandler<Context> {
   ...
   * @param operation The {@link __Operation} implementation that supplies the business logic for MalformedInteger
   * @param mux The {@link __Mux} that verifies which service and operation are being invoked by a given {@link __HttpRequest}
   * @param serializer An {@link __OperationSerializer} for MalformedInteger that
  ...
  constructor(
    ...
    mux: __Mux<"RestJson", "MalformedInteger">,
    serializer: __OperationSerializer<RestJsonService<Context>, "MalformedInteger", MalformedIntegerErrors>,
    serializeFrameworkException: (e: __SmithyFrameworkException, ctx: __ServerSerdeContext) => Promise<__HttpResponse>,
    validationCustomizer: __ValidationCustomizer<"MalformedInteger">
  ) {
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedInteger" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedIntegerServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedInteger" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedIntegerServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedIntegerServerOutput, serdeContextBase),
