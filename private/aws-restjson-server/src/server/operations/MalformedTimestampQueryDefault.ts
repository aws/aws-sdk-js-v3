...
export type MalformedTimestampQueryDefault<Context> = __Operation<MalformedTimestampQueryDefaultServerInput, MalformedTimestampQueryDefaultServerOutput, Context>
...
export namespace MalformedTimestampQueryDefaultServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof MalformedTimestampQueryDefaultInput.validate>[0]) => __ValidationFailure[] = MalformedTimestampQueryDefaultInput.validate;
}
export interface MalformedTimestampQueryDefaultServerOutput {}

export type MalformedTimestampQueryDefaultErrors = never;

export class MalformedTimestampQueryDefaultSerializer implements __OperationSerializer<RestJsonService<any>, "MalformedTimestampQueryDefault", MalformedTimestampQueryDefaultErrors> {
...
export const getMalformedTimestampQueryDefaultHandler = <Context>(operation: __Operation<MalformedTimestampQueryDefaultServerInput, MalformedTimestampQueryDefaultServerOutput, Context>, customizer: __ValidationCustomizer<"MalformedTimestampQueryDefault">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new MalformedTimestampQueryDefaultHandler(operation, mux, new MalformedTimestampQueryDefaultSerializer(), serializeFrameworkException, customizer);
}
...
export class MalformedTimestampQueryDefaultHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly operation: __Operation<MalformedTimestampQueryDefaultServerInput, MalformedTimestampQueryDefaultServerOutput, Context>;
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "MalformedTimestampQueryDefault", MalformedTimestampQueryDefaultErrors>;
   ...
   */
  constructor(
    operation: __Operation<MalformedTimestampQueryDefaultServerInput, MalformedTimestampQueryDefaultServerOutput, Context>,
    mux: __Mux<"RestJson", "MalformedTimestampQueryDefault">,
    serializer: __OperationSerializer<RestJsonService<Context>, "MalformedTimestampQueryDefault", MalformedTimestampQueryDefaultErrors>,
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedTimestampQueryDefault" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedTimestampQueryDefaultServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedTimestampQueryDefault" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedTimestampQueryDefaultServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedTimestampQueryDefaultServerOutput, serdeContextBase),
