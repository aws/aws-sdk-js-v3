...
export class HttpResponseCodeSerializer implements __OperationSerializer<RestJsonService<any>, "HttpResponseCode", HttpResponseCodeErrors> {
...
export const getHttpResponseCodeHandler = <Context>(operation: __Operation<HttpResponseCodeServerInput, HttpResponseCodeServerOutput, Context>, customizer: __ValidationCustomizer<"HttpResponseCode">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new HttpResponseCodeHandler(operation, mux, new HttpResponseCodeSerializer(), serializeFrameworkException, customizer);
}
...
export class HttpResponseCodeHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "HttpResponseCode", HttpResponseCodeErrors>;
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "HttpResponseCode" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (HttpResponseCodeServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "HttpResponseCode" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as HttpResponseCodeServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as HttpResponseCodeServerOutput, serdeContextBase),
