...
export namespace DocumentTypeServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof DocumentTypeInputOutput.validate>[0]) => __ValidationFailure[] = DocumentTypeInputOutput.validate;
}
export interface DocumentTypeServerOutput extends DocumentTypeInputOutput {}

export type DocumentTypeErrors = never;

export class DocumentTypeSerializer implements __OperationSerializer<RestJsonService<any>, "DocumentType", DocumentTypeErrors> {
...
export const getDocumentTypeHandler = <Context>(operation: __Operation<DocumentTypeServerInput, DocumentTypeServerOutput, Context>, customizer: __ValidationCustomizer<"DocumentType">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
...
export class DocumentTypeHandler<Context> implements __ServiceHandler<Context> {
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "DocumentType" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (DocumentTypeServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "DocumentType" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as DocumentTypeServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as DocumentTypeServerOutput, serdeContextBase),
