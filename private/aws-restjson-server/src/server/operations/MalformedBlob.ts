...
export namespace MalformedBlobServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof MalformedBlobInput.validate>[0]) => __ValidationFailure[] = MalformedBlobInput.validate;
}
export interface MalformedBlobServerOutput {}

export type MalformedBlobErrors = never;

export class MalformedBlobSerializer implements __OperationSerializer<RestJsonService<any>, "MalformedBlob", MalformedBlobErrors> {
...
export const getMalformedBlobHandler = <Context>(operation: __Operation<MalformedBlobServerInput, MalformedBlobServerOutput, Context>, customizer: __ValidationCustomizer<"MalformedBlob">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new MalformedBlobHandler(operation, mux, new MalformedBlobSerializer(), serializeFrameworkException, customizer);
}
...
export class MalformedBlobHandler<Context> implements __ServiceHandler<Context> {
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedBlob" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedBlobServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedBlob" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedBlobServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedBlobServerOutput, serdeContextBase),
