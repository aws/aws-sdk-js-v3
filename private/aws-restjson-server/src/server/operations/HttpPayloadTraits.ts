...
export type HttpPayloadTraits<Context> = __Operation<HttpPayloadTraitsServerInput, HttpPayloadTraitsServerOutput, Context>
...
export namespace HttpPayloadTraitsServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof HttpPayloadTraitsInputOutput.validate>[0]) => __ValidationFailure[] = HttpPayloadTraitsInputOutput.validate;
}
...
export class HttpPayloadTraitsSerializer implements __OperationSerializer<RestJsonService<any>, "HttpPayloadTraits", HttpPayloadTraitsErrors> {
...
export const getHttpPayloadTraitsHandler = <Context>(operation: __Operation<HttpPayloadTraitsServerInput, HttpPayloadTraitsServerOutput, Context>, customizer: __ValidationCustomizer<"HttpPayloadTraits">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new HttpPayloadTraitsHandler(operation, mux, new HttpPayloadTraitsSerializer(), serializeFrameworkException, customizer);
}
...
export class HttpPayloadTraitsHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "HttpPayloadTraits", HttpPayloadTraitsErrors>;
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "HttpPayloadTraits" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (HttpPayloadTraitsServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "HttpPayloadTraits" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as HttpPayloadTraitsServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as HttpPayloadTraitsServerOutput, serdeContextBase),
