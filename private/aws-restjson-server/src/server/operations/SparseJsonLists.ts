...
export namespace SparseJsonListsServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof SparseJsonListsInputOutput.validate>[0]) => __ValidationFailure[] = SparseJsonListsInputOutput.validate;
}
export interface SparseJsonListsServerOutput extends SparseJsonListsInputOutput {}

export type SparseJsonListsErrors = never;

export class SparseJsonListsSerializer implements __OperationSerializer<RestJsonService<any>, "SparseJsonLists", SparseJsonListsErrors> {
...
export const getSparseJsonListsHandler = <Context>(operation: __Operation<SparseJsonListsServerInput, SparseJsonListsServerOutput, Context>, customizer: __ValidationCustomizer<"SparseJsonLists">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new SparseJsonListsHandler(operation, mux, new SparseJsonListsSerializer(), serializeFrameworkException, customizer);
}
...
export class SparseJsonListsHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "SparseJsonLists", SparseJsonListsErrors>;
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "SparseJsonLists" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (SparseJsonListsServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "SparseJsonLists" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as SparseJsonListsServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as SparseJsonListsServerOutput, serdeContextBase),
