...
export class OperationWithNestedStructureHandler<Context> implements __ServiceHandler<Context> {
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "OperationWithNestedStructure" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (OperationWithNestedStructureServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "OperationWithNestedStructure" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as OperationWithNestedStructureServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as OperationWithNestedStructureServerOutput, serdeContextBase),
