...
export class DuplexStreamWithInitialMessagesHandler<Context> implements __ServiceHandler<Context> {
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "DuplexStreamWithInitialMessages" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (DuplexStreamWithInitialMessagesServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "DuplexStreamWithInitialMessages" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as DuplexStreamWithInitialMessagesServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as DuplexStreamWithInitialMessagesServerOutput, serdeContextBase),
