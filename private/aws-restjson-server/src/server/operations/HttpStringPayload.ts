...
export type HttpStringPayload<Context> = __Operation<HttpStringPayloadServerInput, HttpStringPayloadServerOutput, Context>
...
export namespace HttpStringPayloadServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof StringPayloadInput.validate>[0]) => __ValidationFailure[] = StringPayloadInput.validate;
}
export interface HttpStringPayloadServerOutput extends StringPayloadInput {}

export type HttpStringPayloadErrors = never;

export class HttpStringPayloadSerializer implements __OperationSerializer<RestJsonService<any>, "HttpStringPayload", HttpStringPayloadErrors> {
...
export const getHttpStringPayloadHandler = <Context>(operation: __Operation<HttpStringPayloadServerInput, HttpStringPayloadServerOutput, Context>, customizer: __ValidationCustomizer<"HttpStringPayload">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new HttpStringPayloadHandler(operation, mux, new HttpStringPayloadSerializer(), serializeFrameworkException, customizer);
}
...
export class HttpStringPayloadHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "HttpStringPayload", HttpStringPayloadErrors>;
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "HttpStringPayload" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (HttpStringPayloadServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "HttpStringPayload" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as HttpStringPayloadServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as HttpStringPayloadServerOutput, serdeContextBase),
