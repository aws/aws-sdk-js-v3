...
export class NoInputAndOutputSerializer implements __OperationSerializer<RestJsonService<any>, "NoInputAndOutput", NoInputAndOutputErrors> {
...
export const getNoInputAndOutputHandler = <Context>(operation: __Operation<NoInputAndOutputServerInput, NoInputAndOutputServerOutput, Context>, customizer: __ValidationCustomizer<"NoInputAndOutput">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new NoInputAndOutputHandler(operation, mux, new NoInputAndOutputSerializer(), serializeFrameworkException, customizer);
}
...
export class NoInputAndOutputHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "NoInputAndOutput", NoInputAndOutputErrors>;
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "NoInputAndOutput" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (NoInputAndOutputServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "NoInputAndOutput" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as NoInputAndOutputServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as NoInputAndOutputServerOutput, serdeContextBase),
