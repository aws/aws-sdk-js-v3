...
export namespace MalformedStringServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof MalformedStringInput.validate>[0]) => __ValidationFailure[] = MalformedStringInput.validate;
}
export interface MalformedStringServerOutput {}

export type MalformedStringErrors = never;

export class MalformedStringSerializer implements __OperationSerializer<RestJsonService<any>, "MalformedString", MalformedStringErrors> {
...
export const getMalformedStringHandler = <Context>(operation: __Operation<MalformedStringServerInput, MalformedStringServerOutput, Context>, customizer: __ValidationCustomizer<"MalformedString">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new MalformedStringHandler(operation, mux, new MalformedStringSerializer(), serializeFrameworkException, customizer);
}
...
export class MalformedStringHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "MalformedString", MalformedStringErrors>;
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedString" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedStringServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedString" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedStringServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedStringServerOutput, serdeContextBase),
