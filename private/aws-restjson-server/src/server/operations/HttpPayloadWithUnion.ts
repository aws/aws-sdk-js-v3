...
export type HttpPayloadWithUnion<Context> = __Operation<HttpPayloadWithUnionServerInput, HttpPayloadWithUnionServerOutput, Context>
...
export namespace HttpPayloadWithUnionServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof HttpPayloadWithUnionInputOutput.validate>[0]) => __ValidationFailure[] = HttpPayloadWithUnionInputOutput.validate;
}
...
export class HttpPayloadWithUnionSerializer implements __OperationSerializer<RestJsonService<any>, "HttpPayloadWithUnion", HttpPayloadWithUnionErrors> {
...
export const getHttpPayloadWithUnionHandler = <Context>(operation: __Operation<HttpPayloadWithUnionServerInput, HttpPayloadWithUnionServerOutput, Context>, customizer: __ValidationCustomizer<"HttpPayloadWithUnion">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new HttpPayloadWithUnionHandler(operation, mux, new HttpPayloadWithUnionSerializer(), serializeFrameworkException, customizer);
}
...
export class HttpPayloadWithUnionHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "HttpPayloadWithUnion", HttpPayloadWithUnionErrors>;
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "HttpPayloadWithUnion" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (HttpPayloadWithUnionServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "HttpPayloadWithUnion" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as HttpPayloadWithUnionServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as HttpPayloadWithUnionServerOutput, serdeContextBase),
