...
export namespace QueryPrecedenceServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof QueryPrecedenceInput.validate>[0]) => __ValidationFailure[] = QueryPrecedenceInput.validate;
}
export interface QueryPrecedenceServerOutput {}

export type QueryPrecedenceErrors = never;

export class QueryPrecedenceSerializer implements __OperationSerializer<RestJsonService<any>, "QueryPrecedence", QueryPrecedenceErrors> {
...
export const getQueryPrecedenceHandler = <Context>(operation: __Operation<QueryPrecedenceServerInput, QueryPrecedenceServerOutput, Context>, customizer: __ValidationCustomizer<"QueryPrecedence">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new QueryPrecedenceHandler(operation, mux, new QueryPrecedenceSerializer(), serializeFrameworkException, customizer);
}
...
export class QueryPrecedenceHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "QueryPrecedence", QueryPrecedenceErrors>;
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "QueryPrecedence" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (QueryPrecedenceServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "QueryPrecedence" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as QueryPrecedenceServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as QueryPrecedenceServerOutput, serdeContextBase),
