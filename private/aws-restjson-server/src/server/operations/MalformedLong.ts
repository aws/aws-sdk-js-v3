...
export namespace MalformedLongServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof MalformedLongInput.validate>[0]) => __ValidationFailure[] = MalformedLongInput.validate;
}
export interface MalformedLongServerOutput {}

export type MalformedLongErrors = never;

export class MalformedLongSerializer implements __OperationSerializer<RestJsonService<any>, "MalformedLong", MalformedLongErrors> {
...
export const getMalformedLongHandler = <Context>(operation: __Operation<MalformedLongServerInput, MalformedLongServerOutput, Context>, customizer: __ValidationCustomizer<"MalformedLong">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new MalformedLongHandler(operation, mux, new MalformedLongSerializer(), serializeFrameworkException, customizer);
}
...
export class MalformedLongHandler<Context> implements __ServiceHandler<Context> {
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedLong" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedLongServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedLong" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedLongServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedLongServerOutput, serdeContextBase),
