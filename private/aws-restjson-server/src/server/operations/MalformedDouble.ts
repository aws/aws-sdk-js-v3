...
export namespace MalformedDoubleServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof MalformedDoubleInput.validate>[0]) => __ValidationFailure[] = MalformedDoubleInput.validate;
}
export interface MalformedDoubleServerOutput {}

export type MalformedDoubleErrors = never;

export class MalformedDoubleSerializer implements __OperationSerializer<RestJsonService<any>, "MalformedDouble", MalformedDoubleErrors> {
...
export const getMalformedDoubleHandler = <Context>(operation: __Operation<MalformedDoubleServerInput, MalformedDoubleServerOutput, Context>, customizer: __ValidationCustomizer<"MalformedDouble">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new MalformedDoubleHandler(operation, mux, new MalformedDoubleSerializer(), serializeFrameworkException, customizer);
}
...
export class MalformedDoubleHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "MalformedDouble", MalformedDoubleErrors>;
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedDouble" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedDoubleServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedDouble" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedDoubleServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedDoubleServerOutput, serdeContextBase),
