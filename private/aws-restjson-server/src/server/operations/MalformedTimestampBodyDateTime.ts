...
export type MalformedTimestampBodyDateTime<Context> = __Operation<MalformedTimestampBodyDateTimeServerInput, MalformedTimestampBodyDateTimeServerOutput, Context>
...
export namespace MalformedTimestampBodyDateTimeServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof MalformedTimestampBodyDateTimeInput.validate>[0]) => __ValidationFailure[] = MalformedTimestampBodyDateTimeInput.validate;
}
export interface MalformedTimestampBodyDateTimeServerOutput {}

export type MalformedTimestampBodyDateTimeErrors = never;

export class MalformedTimestampBodyDateTimeSerializer implements __OperationSerializer<RestJsonService<any>, "MalformedTimestampBodyDateTime", MalformedTimestampBodyDateTimeErrors> {
...
export const getMalformedTimestampBodyDateTimeHandler = <Context>(operation: __Operation<MalformedTimestampBodyDateTimeServerInput, MalformedTimestampBodyDateTimeServerOutput, Context>, customizer: __ValidationCustomizer<"MalformedTimestampBodyDateTime">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new MalformedTimestampBodyDateTimeHandler(operation, mux, new MalformedTimestampBodyDateTimeSerializer(), serializeFrameworkException, customizer);
}
...
export class MalformedTimestampBodyDateTimeHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly operation: __Operation<MalformedTimestampBodyDateTimeServerInput, MalformedTimestampBodyDateTimeServerOutput, Context>;
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "MalformedTimestampBodyDateTime", MalformedTimestampBodyDateTimeErrors>;
   ...
   */
  constructor(
    operation: __Operation<MalformedTimestampBodyDateTimeServerInput, MalformedTimestampBodyDateTimeServerOutput, Context>,
    mux: __Mux<"RestJson", "MalformedTimestampBodyDateTime">,
    serializer: __OperationSerializer<RestJsonService<Context>, "MalformedTimestampBodyDateTime", MalformedTimestampBodyDateTimeErrors>,
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedTimestampBodyDateTime" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedTimestampBodyDateTimeServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedTimestampBodyDateTime" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedTimestampBodyDateTimeServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedTimestampBodyDateTimeServerOutput, serdeContextBase),
