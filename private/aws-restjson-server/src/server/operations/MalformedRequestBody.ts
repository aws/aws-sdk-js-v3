...
export type MalformedRequestBody<Context> = __Operation<MalformedRequestBodyServerInput, MalformedRequestBodyServerOutput, Context>
...
export namespace MalformedRequestBodyServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof MalformedRequestBodyInput.validate>[0]) => __ValidationFailure[] = MalformedRequestBodyInput.validate;
}
export interface MalformedRequestBodyServerOutput {}

export type MalformedRequestBodyErrors = never;

export class MalformedRequestBodySerializer implements __OperationSerializer<RestJsonService<any>, "MalformedRequestBody", MalformedRequestBodyErrors> {
...
export const getMalformedRequestBodyHandler = <Context>(operation: __Operation<MalformedRequestBodyServerInput, MalformedRequestBodyServerOutput, Context>, customizer: __ValidationCustomizer<"MalformedRequestBody">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new MalformedRequestBodyHandler(operation, mux, new MalformedRequestBodySerializer(), serializeFrameworkException, customizer);
}
...
export class MalformedRequestBodyHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "MalformedRequestBody", MalformedRequestBodyErrors>;
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedRequestBody" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedRequestBodyServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedRequestBody" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedRequestBodyServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedRequestBodyServerOutput, serdeContextBase),
