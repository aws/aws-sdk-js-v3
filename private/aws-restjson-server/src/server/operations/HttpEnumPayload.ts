...
export namespace HttpEnumPayloadServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof EnumPayloadInput.validate>[0]) => __ValidationFailure[] = EnumPayloadInput.validate;
}
export interface HttpEnumPayloadServerOutput extends EnumPayloadInput {}

export type HttpEnumPayloadErrors = never;

export class HttpEnumPayloadSerializer implements __OperationSerializer<RestJsonService<any>, "HttpEnumPayload", HttpEnumPayloadErrors> {
...
export const getHttpEnumPayloadHandler = <Context>(operation: __Operation<HttpEnumPayloadServerInput, HttpEnumPayloadServerOutput, Context>, customizer: __ValidationCustomizer<"HttpEnumPayload">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new HttpEnumPayloadHandler(operation, mux, new HttpEnumPayloadSerializer(), serializeFrameworkException, customizer);
}
...
export class HttpEnumPayloadHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "HttpEnumPayload", HttpEnumPayloadErrors>;
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "HttpEnumPayload" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (HttpEnumPayloadServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "HttpEnumPayload" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as HttpEnumPayloadServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as HttpEnumPayloadServerOutput, serdeContextBase),
