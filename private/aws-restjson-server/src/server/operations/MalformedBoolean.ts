...
export namespace MalformedBooleanServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof MalformedBooleanInput.validate>[0]) => __ValidationFailure[] = MalformedBooleanInput.validate;
}
export interface MalformedBooleanServerOutput {}

export type MalformedBooleanErrors = never;

export class MalformedBooleanSerializer implements __OperationSerializer<RestJsonService<any>, "MalformedBoolean", MalformedBooleanErrors> {
...
export const getMalformedBooleanHandler = <Context>(operation: __Operation<MalformedBooleanServerInput, MalformedBooleanServerOutput, Context>, customizer: __ValidationCustomizer<"MalformedBoolean">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new MalformedBooleanHandler(operation, mux, new MalformedBooleanSerializer(), serializeFrameworkException, customizer);
}
...
export class MalformedBooleanHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "MalformedBoolean", MalformedBooleanErrors>;
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedBoolean" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedBooleanServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedBoolean" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedBooleanServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedBooleanServerOutput, serdeContextBase),
