...
export namespace JsonEnumsServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof JsonEnumsInputOutput.validate>[0]) => __ValidationFailure[] = JsonEnumsInputOutput.validate;
}
...
export const getJsonEnumsHandler = <Context>(operation: __Operation<JsonEnumsServerInput, JsonEnumsServerOutput, Context>, customizer: __ValidationCustomizer<"JsonEnums">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
...
export class JsonEnumsHandler<Context> implements __ServiceHandler<Context> {
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "JsonEnums" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (JsonEnumsServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "JsonEnums" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as JsonEnumsServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as JsonEnumsServerOutput, serdeContextBase),
