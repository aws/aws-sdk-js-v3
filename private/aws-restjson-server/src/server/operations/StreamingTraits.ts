...
export namespace StreamingTraitsServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof StreamingTraitsInputOutput.validate>[0]) => __ValidationFailure[] = StreamingTraitsInputOutput.validate;
}
export interface StreamingTraitsServerOutput extends StreamingTraitsInputOutput {}

export type StreamingTraitsErrors = never;

export class StreamingTraitsSerializer implements __OperationSerializer<RestJsonService<any>, "StreamingTraits", StreamingTraitsErrors> {
...
export const getStreamingTraitsHandler = <Context>(operation: __Operation<StreamingTraitsServerInput, StreamingTraitsServerOutput, Context>, customizer: __ValidationCustomizer<"StreamingTraits">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new StreamingTraitsHandler(operation, mux, new StreamingTraitsSerializer(), serializeFrameworkException, customizer);
}
...
export class StreamingTraitsHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "StreamingTraits", StreamingTraitsErrors>;
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "StreamingTraits" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (StreamingTraitsServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "StreamingTraits" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as StreamingTraitsServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as StreamingTraitsServerOutput, serdeContextBase),
