...
export type NoInputAndNoOutput<Context> = __Operation<NoInputAndNoOutputServerInput, NoInputAndNoOutputServerOutput, Context>
...
export class NoInputAndNoOutputSerializer implements __OperationSerializer<RestJsonService<any>, "NoInputAndNoOutput", NoInputAndNoOutputErrors> {
...
export const getNoInputAndNoOutputHandler = <Context>(operation: __Operation<NoInputAndNoOutputServerInput, NoInputAndNoOutputServerOutput, Context>, customizer: __ValidationCustomizer<"NoInputAndNoOutput">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new NoInputAndNoOutputHandler(operation, mux, new NoInputAndNoOutputSerializer(), serializeFrameworkException, customizer);
}
...
export class NoInputAndNoOutputHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "NoInputAndNoOutput", NoInputAndNoOutputErrors>;
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "NoInputAndNoOutput" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (NoInputAndNoOutputServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "NoInputAndNoOutput" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as NoInputAndNoOutputServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as NoInputAndNoOutputServerOutput, serdeContextBase),
