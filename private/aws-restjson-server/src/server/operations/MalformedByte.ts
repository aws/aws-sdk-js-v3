...
export namespace MalformedByteServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof MalformedByteInput.validate>[0]) => __ValidationFailure[] = MalformedByteInput.validate;
}
export interface MalformedByteServerOutput {}

export type MalformedByteErrors = never;

export class MalformedByteSerializer implements __OperationSerializer<RestJsonService<any>, "MalformedByte", MalformedByteErrors> {
...
export const getMalformedByteHandler = <Context>(operation: __Operation<MalformedByteServerInput, MalformedByteServerOutput, Context>, customizer: __ValidationCustomizer<"MalformedByte">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new MalformedByteHandler(operation, mux, new MalformedByteSerializer(), serializeFrameworkException, customizer);
}
...
export class MalformedByteHandler<Context> implements __ServiceHandler<Context> {
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedByte" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedByteServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedByte" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedByteServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedByteServerOutput, serdeContextBase),
