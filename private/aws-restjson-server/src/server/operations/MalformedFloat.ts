...
export namespace MalformedFloatServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof MalformedFloatInput.validate>[0]) => __ValidationFailure[] = MalformedFloatInput.validate;
}
export interface MalformedFloatServerOutput {}

export type MalformedFloatErrors = never;

export class MalformedFloatSerializer implements __OperationSerializer<RestJsonService<any>, "MalformedFloat", MalformedFloatErrors> {
...
export const getMalformedFloatHandler = <Context>(operation: __Operation<MalformedFloatServerInput, MalformedFloatServerOutput, Context>, customizer: __ValidationCustomizer<"MalformedFloat">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new MalformedFloatHandler(operation, mux, new MalformedFloatSerializer(), serializeFrameworkException, customizer);
}
...
export class MalformedFloatHandler<Context> implements __ServiceHandler<Context> {
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedFloat" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedFloatServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedFloat" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedFloatServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedFloatServerOutput, serdeContextBase),
