...
export type PostUnionWithJsonName<Context> = __Operation<PostUnionWithJsonNameServerInput, PostUnionWithJsonNameServerOutput, Context>
...
export namespace PostUnionWithJsonNameServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof PostUnionWithJsonNameInput.validate>[0]) => __ValidationFailure[] = PostUnionWithJsonNameInput.validate;
}
...
export class PostUnionWithJsonNameSerializer implements __OperationSerializer<RestJsonService<any>, "PostUnionWithJsonName", PostUnionWithJsonNameErrors> {
...
export const getPostUnionWithJsonNameHandler = <Context>(operation: __Operation<PostUnionWithJsonNameServerInput, PostUnionWithJsonNameServerOutput, Context>, customizer: __ValidationCustomizer<"PostUnionWithJsonName">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new PostUnionWithJsonNameHandler(operation, mux, new PostUnionWithJsonNameSerializer(), serializeFrameworkException, customizer);
}
...
export class PostUnionWithJsonNameHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "PostUnionWithJsonName", PostUnionWithJsonNameErrors>;
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "PostUnionWithJsonName" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (PostUnionWithJsonNameServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "PostUnionWithJsonName" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as PostUnionWithJsonNameServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as PostUnionWithJsonNameServerOutput, serdeContextBase),
