...
export namespace JsonTimestampsServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof JsonTimestampsInputOutput.validate>[0]) => __ValidationFailure[] = JsonTimestampsInputOutput.validate;
}
export interface JsonTimestampsServerOutput extends JsonTimestampsInputOutput {}

export type JsonTimestampsErrors = never;

export class JsonTimestampsSerializer implements __OperationSerializer<RestJsonService<any>, "JsonTimestamps", JsonTimestampsErrors> {
...
export const getJsonTimestampsHandler = <Context>(operation: __Operation<JsonTimestampsServerInput, JsonTimestampsServerOutput, Context>, customizer: __ValidationCustomizer<"JsonTimestamps">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new JsonTimestampsHandler(operation, mux, new JsonTimestampsSerializer(), serializeFrameworkException, customizer);
}
...
export class JsonTimestampsHandler<Context> implements __ServiceHandler<Context> {
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "JsonTimestamps" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (JsonTimestampsServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "JsonTimestamps" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as JsonTimestampsServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as JsonTimestampsServerOutput, serdeContextBase),
