...
export class MalformedContentTypeWithoutBodyEmptyInputHandler<Context> implements __ServiceHandler<Context> {
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    ...
    if (target === undefined) {
      console.log('Received a request that did not match aws.protocoltests.restjson#RestJson.MalformedContentTypeWithoutBodyEmptyInput. This indicates a misconfiguration.');
      return this.serializeFrameworkException(new __InternalFailureException(), serdeContextBase);
    }
    return handle(request, context, "MalformedContentTypeWithoutBodyEmptyInput", this.serializer, this.operation, this.serializeFrameworkException, MalformedContentTypeWithoutBodyEmptyInputServerInput.validate, this.validationCustomizer);
  }
}
