...
export namespace MalformedMapServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof MalformedMapInput.validate>[0]) => __ValidationFailure[] = MalformedMapInput.validate;
}
export interface MalformedMapServerOutput {}

export type MalformedMapErrors = never;

export class MalformedMapSerializer implements __OperationSerializer<RestJsonService<any>, "MalformedMap", MalformedMapErrors> {
...
export const getMalformedMapHandler = <Context>(operation: __Operation<MalformedMapServerInput, MalformedMapServerOutput, Context>, customizer: __ValidationCustomizer<"MalformedMap">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
...
export class MalformedMapHandler<Context> implements __ServiceHandler<Context> {
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedMap" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedMapServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedMap" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedMapServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedMapServerOutput, serdeContextBase),
