...
export type EndpointWithHostLabelOperation<Context> = __Operation<EndpointWithHostLabelOperationServerInput, EndpointWithHostLabelOperationServerOutput, Context>
...
export namespace EndpointWithHostLabelOperationServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof HostLabelInput.validate>[0]) => __ValidationFailure[] = HostLabelInput.validate;
}
export interface EndpointWithHostLabelOperationServerOutput {}

export type EndpointWithHostLabelOperationErrors = never;

export class EndpointWithHostLabelOperationSerializer implements __OperationSerializer<RestJsonService<any>, "EndpointWithHostLabelOperation", EndpointWithHostLabelOperationErrors> {
...
export const getEndpointWithHostLabelOperationHandler = <Context>(operation: __Operation<EndpointWithHostLabelOperationServerInput, EndpointWithHostLabelOperationServerOutput, Context>, customizer: __ValidationCustomizer<"EndpointWithHostLabelOperation">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new EndpointWithHostLabelOperationHandler(operation, mux, new EndpointWithHostLabelOperationSerializer(), serializeFrameworkException, customizer);
}
...
export class EndpointWithHostLabelOperationHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly operation: __Operation<EndpointWithHostLabelOperationServerInput, EndpointWithHostLabelOperationServerOutput, Context>;
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "EndpointWithHostLabelOperation", EndpointWithHostLabelOperationErrors>;
   ...
   */
  constructor(
    operation: __Operation<EndpointWithHostLabelOperationServerInput, EndpointWithHostLabelOperationServerOutput, Context>,
    mux: __Mux<"RestJson", "EndpointWithHostLabelOperation">,
    serializer: __OperationSerializer<RestJsonService<Context>, "EndpointWithHostLabelOperation", EndpointWithHostLabelOperationErrors>,
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "EndpointWithHostLabelOperation" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (EndpointWithHostLabelOperationServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "EndpointWithHostLabelOperation" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as EndpointWithHostLabelOperationServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as EndpointWithHostLabelOperationServerOutput, serdeContextBase),
