...
export namespace JsonBlobsServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof JsonBlobsInputOutput.validate>[0]) => __ValidationFailure[] = JsonBlobsInputOutput.validate;
}
...
export const getJsonBlobsHandler = <Context>(operation: __Operation<JsonBlobsServerInput, JsonBlobsServerOutput, Context>, customizer: __ValidationCustomizer<"JsonBlobs">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
...
export class JsonBlobsHandler<Context> implements __ServiceHandler<Context> {
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "JsonBlobs" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (JsonBlobsServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "JsonBlobs" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as JsonBlobsServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as JsonBlobsServerOutput, serdeContextBase),
