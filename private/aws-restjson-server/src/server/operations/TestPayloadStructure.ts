...
export type TestPayloadStructure<Context> = __Operation<TestPayloadStructureServerInput, TestPayloadStructureServerOutput, Context>
...
export namespace TestPayloadStructureServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof TestPayloadStructureInputOutput.validate>[0]) => __ValidationFailure[] = TestPayloadStructureInputOutput.validate;
}
...
export class TestPayloadStructureSerializer implements __OperationSerializer<RestJsonService<any>, "TestPayloadStructure", TestPayloadStructureErrors> {
...
export const getTestPayloadStructureHandler = <Context>(operation: __Operation<TestPayloadStructureServerInput, TestPayloadStructureServerOutput, Context>, customizer: __ValidationCustomizer<"TestPayloadStructure">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new TestPayloadStructureHandler(operation, mux, new TestPayloadStructureSerializer(), serializeFrameworkException, customizer);
}
...
export class TestPayloadStructureHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "TestPayloadStructure", TestPayloadStructureErrors>;
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "TestPayloadStructure" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (TestPayloadStructureServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "TestPayloadStructure" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as TestPayloadStructureServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as TestPayloadStructureServerOutput, serdeContextBase),
