...
export namespace MalformedListServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof MalformedListInput.validate>[0]) => __ValidationFailure[] = MalformedListInput.validate;
}
export interface MalformedListServerOutput {}

export type MalformedListErrors = never;

export class MalformedListSerializer implements __OperationSerializer<RestJsonService<any>, "MalformedList", MalformedListErrors> {
...
export const getMalformedListHandler = <Context>(operation: __Operation<MalformedListServerInput, MalformedListServerOutput, Context>, customizer: __ValidationCustomizer<"MalformedList">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new MalformedListHandler(operation, mux, new MalformedListSerializer(), serializeFrameworkException, customizer);
}
...
export class MalformedListHandler<Context> implements __ServiceHandler<Context> {
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    ...
    if (target === undefined) {
      console.log('Received a request that did not match aws.protocoltests.restjson#RestJson.MalformedList. This indicates a misconfiguration.');
      return this.serializeFrameworkException(new __InternalFailureException(), serdeContextBase);
    }
    return handle(request, context, "MalformedList", this.serializer, this.operation, this.serializeFrameworkException, MalformedListServerInput.validate, this.validationCustomizer);
  }
}
