...
export type QueryParamsAsStringListMap<Context> = __Operation<QueryParamsAsStringListMapServerInput, QueryParamsAsStringListMapServerOutput, Context>
...
export namespace QueryParamsAsStringListMapServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof QueryParamsAsStringListMapInput.validate>[0]) => __ValidationFailure[] = QueryParamsAsStringListMapInput.validate;
}
export interface QueryParamsAsStringListMapServerOutput {}

export type QueryParamsAsStringListMapErrors = never;

export class QueryParamsAsStringListMapSerializer implements __OperationSerializer<RestJsonService<any>, "QueryParamsAsStringListMap", QueryParamsAsStringListMapErrors> {
...
export const getQueryParamsAsStringListMapHandler = <Context>(operation: __Operation<QueryParamsAsStringListMapServerInput, QueryParamsAsStringListMapServerOutput, Context>, customizer: __ValidationCustomizer<"QueryParamsAsStringListMap">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new QueryParamsAsStringListMapHandler(operation, mux, new QueryParamsAsStringListMapSerializer(), serializeFrameworkException, customizer);
}
...
export class QueryParamsAsStringListMapHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly operation: __Operation<QueryParamsAsStringListMapServerInput, QueryParamsAsStringListMapServerOutput, Context>;
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "QueryParamsAsStringListMap", QueryParamsAsStringListMapErrors>;
  ...
  constructor(
    ...
    serializer: __OperationSerializer<RestJsonService<Context>, "QueryParamsAsStringListMap", QueryParamsAsStringListMapErrors>,
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "QueryParamsAsStringListMap" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (QueryParamsAsStringListMapServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "QueryParamsAsStringListMap" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as QueryParamsAsStringListMapServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as QueryParamsAsStringListMapServerOutput, serdeContextBase),
