...
export type MalformedTimestampQueryHttpDate<Context> = __Operation<MalformedTimestampQueryHttpDateServerInput, MalformedTimestampQueryHttpDateServerOutput, Context>
...
export namespace MalformedTimestampQueryHttpDateServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof MalformedTimestampQueryHttpDateInput.validate>[0]) => __ValidationFailure[] = MalformedTimestampQueryHttpDateInput.validate;
}
export interface MalformedTimestampQueryHttpDateServerOutput {}

export type MalformedTimestampQueryHttpDateErrors = never;

export class MalformedTimestampQueryHttpDateSerializer implements __OperationSerializer<RestJsonService<any>, "MalformedTimestampQueryHttpDate", MalformedTimestampQueryHttpDateErrors> {
...
export const getMalformedTimestampQueryHttpDateHandler = <Context>(operation: __Operation<MalformedTimestampQueryHttpDateServerInput, MalformedTimestampQueryHttpDateServerOutput, Context>, customizer: __ValidationCustomizer<"MalformedTimestampQueryHttpDate">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new MalformedTimestampQueryHttpDateHandler(operation, mux, new MalformedTimestampQueryHttpDateSerializer(), serializeFrameworkException, customizer);
}
...
export class MalformedTimestampQueryHttpDateHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly operation: __Operation<MalformedTimestampQueryHttpDateServerInput, MalformedTimestampQueryHttpDateServerOutput, Context>;
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "MalformedTimestampQueryHttpDate", MalformedTimestampQueryHttpDateErrors>;
   ...
   */
  constructor(
    operation: __Operation<MalformedTimestampQueryHttpDateServerInput, MalformedTimestampQueryHttpDateServerOutput, Context>,
    mux: __Mux<"RestJson", "MalformedTimestampQueryHttpDate">,
    serializer: __OperationSerializer<RestJsonService<Context>, "MalformedTimestampQueryHttpDate", MalformedTimestampQueryHttpDateErrors>,
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedTimestampQueryHttpDate" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedTimestampQueryHttpDateServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedTimestampQueryHttpDate" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedTimestampQueryHttpDateServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedTimestampQueryHttpDateServerOutput, serdeContextBase),
