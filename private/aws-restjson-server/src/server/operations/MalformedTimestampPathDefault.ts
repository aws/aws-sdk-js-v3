...
export type MalformedTimestampPathDefault<Context> = __Operation<MalformedTimestampPathDefaultServerInput, MalformedTimestampPathDefaultServerOutput, Context>
...
export namespace MalformedTimestampPathDefaultServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof MalformedTimestampPathDefaultInput.validate>[0]) => __ValidationFailure[] = MalformedTimestampPathDefaultInput.validate;
}
export interface MalformedTimestampPathDefaultServerOutput {}

export type MalformedTimestampPathDefaultErrors = never;

export class MalformedTimestampPathDefaultSerializer implements __OperationSerializer<RestJsonService<any>, "MalformedTimestampPathDefault", MalformedTimestampPathDefaultErrors> {
...
export const getMalformedTimestampPathDefaultHandler = <Context>(operation: __Operation<MalformedTimestampPathDefaultServerInput, MalformedTimestampPathDefaultServerOutput, Context>, customizer: __ValidationCustomizer<"MalformedTimestampPathDefault">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new MalformedTimestampPathDefaultHandler(operation, mux, new MalformedTimestampPathDefaultSerializer(), serializeFrameworkException, customizer);
}
...
export class MalformedTimestampPathDefaultHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly operation: __Operation<MalformedTimestampPathDefaultServerInput, MalformedTimestampPathDefaultServerOutput, Context>;
  private readonly serializer: __OperationSerializer<RestJsonService<Context>, "MalformedTimestampPathDefault", MalformedTimestampPathDefaultErrors>;
  ...
  constructor(
    ...
    serializer: __OperationSerializer<RestJsonService<Context>, "MalformedTimestampPathDefault", MalformedTimestampPathDefaultErrors>,
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedTimestampPathDefault" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedTimestampPathDefaultServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedTimestampPathDefault" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedTimestampPathDefaultServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedTimestampPathDefaultServerOutput, serdeContextBase),
