...
export namespace JsonListsServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof JsonListsInputOutput.validate>[0]) => __ValidationFailure[] = JsonListsInputOutput.validate;
}
...
export const getJsonListsHandler = <Context>(operation: __Operation<JsonListsServerInput, JsonListsServerOutput, Context>, customizer: __ValidationCustomizer<"JsonLists">): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
...
export class JsonListsHandler<Context> implements __ServiceHandler<Context> {
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    ...
    if (target === undefined) {
      console.log('Received a request that did not match aws.protocoltests.restjson#RestJson.JsonLists. This indicates a misconfiguration.');
      return this.serializeFrameworkException(new __InternalFailureException(), serdeContextBase);
    }
    return handle(request, context, "JsonLists", this.serializer, this.operation, this.serializeFrameworkException, JsonListsServerInput.validate, this.validationCustomizer);
  }
}
