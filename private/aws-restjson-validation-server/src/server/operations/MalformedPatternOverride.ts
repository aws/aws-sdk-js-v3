...
import { MalformedPatternOverrideInput } from "../../models/models_0";
import {
...
export type MalformedPatternOverride<Context> = __Operation<MalformedPatternOverrideServerInput, MalformedPatternOverrideServerOutput, Context>
...
export namespace MalformedPatternOverrideServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof MalformedPatternOverrideInput.validate>[0]) => __ValidationFailure[] = MalformedPatternOverrideInput.validate;
}
export interface MalformedPatternOverrideServerOutput {}

export type MalformedPatternOverrideErrors = ValidationException

export class MalformedPatternOverrideSerializer implements __OperationSerializer<RestJsonValidationService<any>, "MalformedPatternOverride", MalformedPatternOverrideErrors> {
...
export const getMalformedPatternOverrideHandler = <Context>(operation: __Operation<MalformedPatternOverrideServerInput, MalformedPatternOverrideServerOutput, Context>): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new MalformedPatternOverrideHandler(operation, mux, new MalformedPatternOverrideSerializer(), serializeFrameworkException, customizer);
}
...
export class MalformedPatternOverrideHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly operation: __Operation<MalformedPatternOverrideServerInput, MalformedPatternOverrideServerOutput, Context>;
  private readonly serializer: __OperationSerializer<RestJsonValidationService<Context>, "MalformedPatternOverride", MalformedPatternOverrideErrors>;
  ...
  constructor(
    ...
    serializer: __OperationSerializer<RestJsonValidationService<Context>, "MalformedPatternOverride", MalformedPatternOverrideErrors>,
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedPatternOverride" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedPatternOverrideServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedPatternOverride" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedPatternOverrideServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedPatternOverrideServerOutput, serdeContextBase),
