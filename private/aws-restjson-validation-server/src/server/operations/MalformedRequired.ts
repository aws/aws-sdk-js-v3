...
import { MalformedRequiredInput } from "../../models/models_0";
import {
...
export type MalformedRequired<Context> = __Operation<MalformedRequiredServerInput, MalformedRequiredServerOutput, Context>
...
export namespace MalformedRequiredServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof MalformedRequiredInput.validate>[0]) => __ValidationFailure[] = MalformedRequiredInput.validate;
}
export interface MalformedRequiredServerOutput {}

export type MalformedRequiredErrors = ValidationException

export class MalformedRequiredSerializer implements __OperationSerializer<RestJsonValidationService<any>, "MalformedRequired", MalformedRequiredErrors> {
...
export const getMalformedRequiredHandler = <Context>(operation: __Operation<MalformedRequiredServerInput, MalformedRequiredServerOutput, Context>): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new MalformedRequiredHandler(operation, mux, new MalformedRequiredSerializer(), serializeFrameworkException, customizer);
}
...
export class MalformedRequiredHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonValidationService<Context>, "MalformedRequired", MalformedRequiredErrors>;
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedRequired" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedRequiredServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedRequired" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedRequiredServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedRequiredServerOutput, serdeContextBase),
