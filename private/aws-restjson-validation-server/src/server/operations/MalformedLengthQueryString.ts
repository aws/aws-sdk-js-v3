...
import { MalformedLengthQueryStringInput } from "../../models/models_0";
import {
...
export class MalformedLengthQueryStringHandler<Context> implements __ServiceHandler<Context> {
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedLengthQueryString" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedLengthQueryStringServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedLengthQueryString" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedLengthQueryStringServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedLengthQueryStringServerOutput, serdeContextBase),
