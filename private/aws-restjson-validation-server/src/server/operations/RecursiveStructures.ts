...
import { RecursiveStructuresInput } from "../../models/models_0";
import {
...
export type RecursiveStructures<Context> = __Operation<RecursiveStructuresServerInput, RecursiveStructuresServerOutput, Context>
...
export namespace RecursiveStructuresServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof RecursiveStructuresInput.validate>[0]) => __ValidationFailure[] = RecursiveStructuresInput.validate;
}
export interface RecursiveStructuresServerOutput {}

export type RecursiveStructuresErrors = ValidationException

export class RecursiveStructuresSerializer implements __OperationSerializer<RestJsonValidationService<any>, "RecursiveStructures", RecursiveStructuresErrors> {
...
export const getRecursiveStructuresHandler = <Context>(operation: __Operation<RecursiveStructuresServerInput, RecursiveStructuresServerOutput, Context>): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new RecursiveStructuresHandler(operation, mux, new RecursiveStructuresSerializer(), serializeFrameworkException, customizer);
}
...
export class RecursiveStructuresHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonValidationService<Context>, "RecursiveStructures", RecursiveStructuresErrors>;
  ...
  constructor(
    ...
    serializer: __OperationSerializer<RestJsonValidationService<Context>, "RecursiveStructures", RecursiveStructuresErrors>,
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "RecursiveStructures" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (RecursiveStructuresServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "RecursiveStructures" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as RecursiveStructuresServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as RecursiveStructuresServerOutput, serdeContextBase),
