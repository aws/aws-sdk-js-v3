...
export type MalformedUniqueItems<Context> = __Operation<MalformedUniqueItemsServerInput, MalformedUniqueItemsServerOutput, Context>
...
export namespace MalformedUniqueItemsServerInput {
   ...
   * @internal
   */
  export const validate: (obj: Parameters<typeof MalformedUniqueItemsInput.validate>[0]) => __ValidationFailure[] = MalformedUniqueItemsInput.validate;
}
export interface MalformedUniqueItemsServerOutput {}

export type MalformedUniqueItemsErrors = ValidationException

export class MalformedUniqueItemsSerializer implements __OperationSerializer<RestJsonValidationService<any>, "MalformedUniqueItems", MalformedUniqueItemsErrors> {
...
export const getMalformedUniqueItemsHandler = <Context>(operation: __Operation<MalformedUniqueItemsServerInput, MalformedUniqueItemsServerOutput, Context>): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  ...
  return new MalformedUniqueItemsHandler(operation, mux, new MalformedUniqueItemsSerializer(), serializeFrameworkException, customizer);
}
...
export class MalformedUniqueItemsHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly serializer: __OperationSerializer<RestJsonValidationService<Context>, "MalformedUniqueItems", MalformedUniqueItemsErrors>;
  ...
  constructor(
    ...
    serializer: __OperationSerializer<RestJsonValidationService<Context>, "MalformedUniqueItems", MalformedUniqueItemsErrors>,
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    const steps: __FrameworkSteps<Context> = {
      route: (request) => this.mux.match(request) !== undefined ? "MalformedUniqueItems" : undefined,
      ...
      validate: (_op, input) => {
        const validationFailures = (MalformedUniqueItemsServerInput.validate as (input: any) => __ValidationFailure[])(input);
        if (validationFailures && validationFailures.length > 0) {
          const validationException = this.validationCustomizer({ operation: "MalformedUniqueItems" }, validationFailures);
          if (validationException) {
            throw validationException;
          }
        }
      },
      invoke: (_op, input, context) => this.operation(input as MalformedUniqueItemsServerInput, context),
      serialize: (_op, output) => this.serializer.serialize(output as MalformedUniqueItemsServerOutput, serdeContextBase),
