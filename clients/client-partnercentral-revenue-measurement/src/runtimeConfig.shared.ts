...
import { defaultPartnerCentralRevenueMeasurementHttpAuthSchemeProvider } from "./auth/httpAuthSchemeProvider";
...
export const getRuntimeConfig = (config: PartnerCentralRevenueMeasurementClientConfig) => {
  return {
    ...
    httpAuthSchemeProvider: config?.httpAuthSchemeProvider ?? defaultPartnerCentralRevenueMeasurementHttpAuthSchemeProvider,
    ...
    protocolSettings: config?.protocolSettings ?? {
      defaultNamespace: "com.amazonaws.partnercentralrevenuemeasurement",
      errorTypeRegistries,
      version: "2022-07-26",
      serviceTarget: "PartnerCentralRevenueMeasurement",
    },
    serviceId: config?.serviceId ?? "PartnerCentral Revenue Measurement",
