...
`GeoPlaces` extends `GeoPlacesClient` and additionally supports all operations, waiters, and paginators as methods.
This style may be familiar to you from the AWS SDK for JavaScript v2.

If you are bundling the AWS SDK, we recommend using only the bare-bones client (`GeoPlacesClient`).
More details are in the blog post on
[modular packages in AWS SDK for JavaScript](https://aws.amazon.com/blogs/developer/modular-packages-in-aws-sdk-for-javascript/).

```ts
import { GeoPlaces } from "@aws-sdk/client-geo-places";

const client = new GeoPlaces({ region: "REGION" });
