...
> {
   ...
   * @public
   */
  constructor(readonly input: ListAnomaliesCommandInput) {
    super();
  }
  ...
  ): Handler<ListAnomaliesCommandInput, ListAnomaliesCommandOutput> {
    this.middlewareStack.use(getSerdePlugin(configuration, this.serialize, this.deserialize));
    this.middlewareStack.use(getEndpointPlugin(configuration, ListAnomaliesCommand.getEndpointParameterInstructions()));

    const stack = clientStack.concat(this.middlewareStack);

    const { logger } = configuration;
    const clientName = "CloudWatchLogsClient";
    const commandName = "ListAnomaliesCommand";
   ...
   * @internal
   */
  private serialize(input: ListAnomaliesCommandInput, context: __SerdeContext): Promise<__HttpRequest> {
    return se_ListAnomaliesCommand(input, context);
  }

  /**
   * @internal
   */
  private deserialize(output: __HttpResponse, context: __SerdeContext): Promise<ListAnomaliesCommandOutput> {
    return de_ListAnomaliesCommand(output, context);
  }
}
