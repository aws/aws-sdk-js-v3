...
const serializeAws_queryBinaryList = (
  ...
  let counter = 1;
  (input || []).map(entry => {
    const loc: string = "BinaryListValue." + counter;
    entries[loc] = context.base64Encoder(entry);
    counter++;
  });
  return entries;
};
...
const serializeAws_queryMessageBodySystemAttributeMap = (
  ...
  Object.keys(input).forEach(key => {
    ...
    entries[keyLoc] = key;
    const valueLoc: string = "entry.Value." + counter;
    entries[valueLoc] = serializeAws_queryMessageSystemAttributeValue(
      input[key],
      context
    );
    counter++;
...
const serializeAws_querySendMessageBatchRequestEntry = (
  ...
  if (input.MessageSystemAttributes !== undefined) {
    ...
    Object.keys(memberEntries).forEach(key => {
      const loc =
        "MessageSystemAttribute." + key.substring(key.indexOf(".") + 1);
      entries[loc] = memberEntries[key];
    });
  }
  return entries;
...
const serializeAws_queryStringList = (
  ...
  let counter = 1;
  (input || []).map(entry => {
    const loc: string = "StringListValue." + counter;
    entries[loc] = entry;
    counter++;
  });
  return entries;
