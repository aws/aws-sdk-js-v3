...
> {
   ...
   * @public
   */
  constructor(readonly input: GetCapabilityCommandInput) {
    super();
  }
  ...
  resolveMiddleware(
    ...
    this.middlewareStack.use(getEndpointPlugin(configuration, GetCapabilityCommand.getEndpointParameterInstructions()));

    const stack = clientStack.concat(this.middlewareStack);

    const { logger } = configuration;
    const clientName = "B2biClient";
    const commandName = "GetCapabilityCommand";
   ...
   * @internal
   */
  private serialize(input: GetCapabilityCommandInput, context: __SerdeContext): Promise<__HttpRequest> {
    return se_GetCapabilityCommand(input, context);
  }

  /**
   * @internal
   */
  private deserialize(output: __HttpResponse, context: __SerdeContext): Promise<GetCapabilityCommandOutput> {
    return de_GetCapabilityCommand(output, context);
  }
}
