...
> {
   ...
   * @public
   */
  constructor(readonly input: GetExecutionCommandInput) {
    super();
  }
  ...
  resolveMiddleware(
    ...
    this.middlewareStack.use(getEndpointPlugin(configuration, GetExecutionCommand.getEndpointParameterInstructions()));
   ...
   * @internal
   */
  private serialize(input: GetExecutionCommandInput, context: __SerdeContext): Promise<__HttpRequest> {
    return se_GetExecutionCommand(input, context);
  }

  /**
   * @internal
   */
  private deserialize(output: __HttpResponse, context: __SerdeContext): Promise<GetExecutionCommandOutput> {
    return de_GetExecutionCommand(output, context);
  }
}
