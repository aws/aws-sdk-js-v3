const _A = "Action";
const _AC = "AttackCounters";
const _ACt = "AttackCount";
const _AD = "AttackDetail";
const _ADE = "AccessDeniedException";
const _ADFDE = "AccessDeniedForDependencyException";
const _ADRTLB = "AssociateDRTLogBucket";
const _ADRTLBR = "AssociateDRTLogBucketRequest";
const _ADRTLBRs = "AssociateDRTLogBucketResponse";
const _ADRTR = "AssociateDRTRole";
const _ADRTRR = "AssociateDRTRoleRequest";
const _ADRTRRs = "AssociateDRTRoleResponse";
const _AHC = "AssociateHealthCheck";
const _AHCR = "AssociateHealthCheckRequest";
const _AHCRs = "AssociateHealthCheckResponse";
const _AI = "AttackId";
const _AL = "AttackLayer";
const _ALARC = "ApplicationLayerAutomaticResponseConfiguration";
const _AP = "AttackProperties";
const _APED = "AssociateProactiveEngagementDetails";
const _APEDR = "AssociateProactiveEngagementDetailsRequest";
const _APEDRs = "AssociateProactiveEngagementDetailsResponse";
const _API = "AttackPropertyIdentifier";
const _APL = "ArbitraryPatternLimits";
const _APt = "AttackProperty";
const _AR = "AutoRenew";
const _AS = "AttackSummary";
const _ASDI = "AttackStatisticsDataItem";
const _ASDL = "AttackStatisticsDataList";
const _ASt = "AttackSummaries";
const _AV = "AttackVolume";
const _AVD = "AttackVectorDescription";
const _AVDL = "AttackVectorDescriptionList";
const _AVS = "AttackVolumeStatistics";
const _AVt = "AttackVectors";
const _Ag = "Aggregation";
const _Agg = "Aggregations";
const _At = "Attack";
const _Av = "Average";
const _B = "Block";
const _BA = "BlockAction";
const _BPS = "BitsPerSecond";
const _C = "Contributor";
const _CA = "CountAction";
const _CN = "ContactNotes";
const _CP = "CreateProtection";
const _CPG = "CreateProtectionGroup";
const _CPGR = "CreateProtectionGroupRequest";
const _CPGRr = "CreateProtectionGroupResponse";
const _CPR = "CreateProtectionRequest";
const _CPRr = "CreateProtectionResponse";
const _CS = "CreateSubscription";
const _CSR = "CreateSubscriptionRequest";
const _CSRr = "CreateSubscriptionResponse";
const _Co = "Count";
const _Cou = "Counters";
const _DA = "DescribeAttack";
const _DALAR = "DisableApplicationLayerAutomaticResponse";
const _DALARR = "DisableApplicationLayerAutomaticResponseRequest";
const _DALARRi = "DisableApplicationLayerAutomaticResponseResponse";
const _DAR = "DescribeAttackRequest";
const _DARe = "DescribeAttackResponse";
const _DAS = "DescribeAttackStatistics";
const _DASR = "DescribeAttackStatisticsRequest";
const _DASRe = "DescribeAttackStatisticsResponse";
const _DDRTA = "DescribeDRTAccess";
const _DDRTAR = "DescribeDRTAccessRequest";
const _DDRTARe = "DescribeDRTAccessResponse";
const _DDRTLB = "DisassociateDRTLogBucket";
const _DDRTLBR = "DisassociateDRTLogBucketRequest";
const _DDRTLBRi = "DisassociateDRTLogBucketResponse";
const _DDRTR = "DisassociateDRTRole";
const _DDRTRR = "DisassociateDRTRoleRequest";
const _DDRTRRi = "DisassociateDRTRoleResponse";
const _DECS = "DescribeEmergencyContactSettings";
const _DECSR = "DescribeEmergencyContactSettingsRequest";
const _DECSRe = "DescribeEmergencyContactSettingsResponse";
const _DHC = "DisassociateHealthCheck";
const _DHCR = "DisassociateHealthCheckRequest";
const _DHCRi = "DisassociateHealthCheckResponse";
const _DI = "DataItems";
const _DP = "DeleteProtection";
const _DPE = "DisableProactiveEngagement";
const _DPER = "DisableProactiveEngagementRequest";
const _DPERi = "DisableProactiveEngagementResponse";
const _DPG = "DeleteProtectionGroup";
const _DPGR = "DeleteProtectionGroupRequest";
const _DPGRe = "DeleteProtectionGroupResponse";
const _DPGRes = "DescribeProtectionGroupRequest";
const _DPGResc = "DescribeProtectionGroupResponse";
const _DPGe = "DescribeProtectionGroup";
const _DPR = "DeleteProtectionRequest";
const _DPRe = "DeleteProtectionResponse";
const _DPRes = "DescribeProtectionRequest";
const _DPResc = "DescribeProtectionResponse";
const _DPe = "DescribeProtection";
const _DS = "DeleteSubscription";
const _DSR = "DeleteSubscriptionRequest";
const _DSRe = "DeleteSubscriptionResponse";
const _DSRes = "DescribeSubscriptionRequest";
const _DSResc = "DescribeSubscriptionResponse";
const _DSe = "DescribeSubscription";
const _EA = "EmailAddress";
const _EALAR = "EnableApplicationLayerAutomaticResponse";
const _EALARR = "EnableApplicationLayerAutomaticResponseRequest";
const _EALARRn = "EnableApplicationLayerAutomaticResponseResponse";
const _EC = "EmergencyContact";
const _ECL = "EmergencyContactList";
const _EPE = "EnableProactiveEngagement";
const _EPER = "EnableProactiveEngagementRequest";
const _EPERn = "EnableProactiveEngagementResponse";
const _ET = "EndTime";
const _FI = "FromInclusive";
const _GSS = "GetSubscriptionState";
const _GSSR = "GetSubscriptionStateRequest";
const _GSSRe = "GetSubscriptionStateResponse";
const _HCA = "HealthCheckArn";
const _HCI = "HealthCheckIds";
const _I = "Id";
const _IEE = "InternalErrorException";
const _IF = "InclusionFilters";
const _IOE = "InvalidOperationException";
const _IPE = "InvalidParameterException";
const _IPF = "InclusionProtectionFilters";
const _IPGF = "InclusionProtectionGroupFilters";
const _IPTE = "InvalidPaginationTokenException";
const _IRE = "InvalidResourceException";
const _K = "Key";
const _L = "Limit";
const _LA = "ListAttacks";
const _LAR = "ListAttacksRequest";
const _LARi = "ListAttacksResponse";
const _LB = "LogBucket";
const _LBL = "LogBucketList";
const _LEE = "LimitsExceededException";
const _LP = "ListProtections";
const _LPG = "ListProtectionGroups";
const _LPGR = "ListProtectionGroupsRequest";
const _LPGRi = "ListProtectionGroupsResponse";
const _LPR = "ListProtectionsRequest";
const _LPRi = "ListProtectionsResponse";
const _LRIPG = "ListResourcesInProtectionGroup";
const _LRIPGR = "ListResourcesInProtectionGroupRequest";
const _LRIPGRi = "ListResourcesInProtectionGroupResponse";
const _LSE = "LockedSubscriptionException";
const _LTFR = "ListTagsForResource";
const _LTFRR = "ListTagsForResourceRequest";
const _LTFRRi = "ListTagsForResourceResponse";
const _Li = "Limits";
const _M = "Mitigations";
const _ML = "MitigationList";
const _MM = "MaxMembers";
const _MN = "MitigationName";
const _MPG = "MaxProtectionGroups";
const _MR = "MaxResults";
const _Ma = "Max";
const _Me = "Members";
const _Mi = "Mitigation";
const _N = "Name";
const _NARE = "NoAssociatedRoleException";
const _NT = "NextToken";
const _N_ = "N";
const _OLE = "OptimisticLockException";
const _P = "Pattern";
const _PA = "ProtectionArn";
const _PES = "ProactiveEngagementStatus";
const _PG = "ProtectionGroup";
const _PGA = "ProtectionGroupArn";
const _PGAPL = "ProtectionGroupArbitraryPatternLimits";
const _PGI = "ProtectionGroupId";
const _PGIr = "ProtectionGroupIds";
const _PGL = "ProtectionGroupLimits";
const _PGPTL = "ProtectionGroupPatternTypeLimits";
const _PGr = "ProtectionGroups";
const _PI = "ProtectionId";
const _PL = "ProtectionLimits";
const _PN = "PhoneNumber";
const _PNr = "ProtectionNames";
const _PPS = "PacketsPerSecond";
const _PRTL = "ProtectedResourceTypeLimits";
const _PTL = "PatternTypeLimits";
const _Pa = "Patterns";
const _Pr = "Protection";
const _Pro = "Protections";
const _RA = "RoleArn";
const _RAEE = "ResourceAlreadyExistsException";
const _RARN = "ResourceARN";
const _RAe = "ResourceArn";
const _RAes = "ResourceArns";
const _RAesp = "ResponseAction";
const _RNFE = "ResourceNotFoundException";
const _RPS = "RequestsPerSecond";
const _RT = "ResourceType";
const _RTe = "ResourceTypes";
const _S = "Status";
const _SA = "SubscriptionArn";
const _SAV = "SummarizedAttackVector";
const _SAVL = "SummarizedAttackVectorList";
const _SC = "SummarizedCounter";
const _SCL = "SummarizedCounterList";
const _SL = "SubscriptionLimits";
const _SR = "SubResources";
const _SRS = "SubResourceSummary";
const _SRSL = "SubResourceSummaryList";
const _SS = "SubscriptionState";
const _ST = "StartTime";
const _Su = "Subscription";
const _Sum = "Sum";
const _T = "Total";
const _TC = "TopContributors";
const _TCIS = "TimeCommitmentInSeconds";
const _TE = "ToExclusive";
const _TK = "TagKeys";
const _TL = "TagList";
const _TR = "TimeRange";
const _TRR = "TagResourceRequest";
const _TRRa = "TagResourceResponse";
const _TRa = "TagResource";
const _Ta = "Tags";
const _Tag = "Tag";
const _Ty = "Type";
const _U = "Unit";
const _UALAR = "UpdateApplicationLayerAutomaticResponse";
const _UALARR = "UpdateApplicationLayerAutomaticResponseRequest";
const _UALARRp = "UpdateApplicationLayerAutomaticResponseResponse";
const _UECS = "UpdateEmergencyContactSettings";
const _UECSR = "UpdateEmergencyContactSettingsRequest";
const _UECSRp = "UpdateEmergencyContactSettingsResponse";
const _UPG = "UpdateProtectionGroup";
const _UPGR = "UpdateProtectionGroupRequest";
const _UPGRp = "UpdateProtectionGroupResponse";
const _UR = "UntagResource";
const _URR = "UntagResourceRequest";
const _URRn = "UntagResourceResponse";
const _US = "UpdateSubscription";
const _USR = "UpdateSubscriptionRequest";
const _USRp = "UpdateSubscriptionResponse";
const _V = "Value";
const _VC = "VectorCounters";
const _VEF = "ValidationExceptionField";
const _VEFL = "ValidationExceptionFieldList";
const _VT = "VectorType";
const _c = "client";
const _e = "error";
const _f = "fields";
const _m = "message";
const _n = "name";
const _r = "reason";
const _rT = "resourceType";
const _s = "server";
const _sm = "smithy.ts.sdk.synthetic.com.amazonaws.shield";
const n0 = "com.amazonaws.shield";

// smithy-typescript generated code
import { TypeRegistry } from "@smithy/core/schema";
import type { StaticErrorSchema, StaticListSchema, StaticOperationSchema, StaticStructureSchema } from "@smithy/types";

import {
  AccessDeniedException,
  AccessDeniedForDependencyException,
  InternalErrorException,
  InvalidOperationException,
  InvalidPaginationTokenException,
  InvalidParameterException,
  InvalidResourceException,
  LimitsExceededException,
  LockedSubscriptionException,
  NoAssociatedRoleException,
  OptimisticLockException,
  ResourceAlreadyExistsException,
  ResourceNotFoundException,
} from "../models/errors";
import { ShieldServiceException } from "../models/ShieldServiceException";

/* eslint no-var: 0 */
export var AccessDeniedException$: StaticErrorSchema = [-3, n0, _ADE,
  { [_e]: _c },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(AccessDeniedException$, AccessDeniedException);
export var AccessDeniedForDependencyException$: StaticErrorSchema = [-3, n0, _ADFDE,
  { [_e]: _c },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(AccessDeniedForDependencyException$, AccessDeniedForDependencyException);
export var ApplicationLayerAutomaticResponseConfiguration$: StaticStructureSchema = [3, n0, _ALARC,
  0,
  [_S, _A],
  [0, () => ResponseAction$]
];
export var AssociateDRTLogBucketRequest$: StaticStructureSchema = [3, n0, _ADRTLBR,
  0,
  [_LB],
  [0]
];
export var AssociateDRTLogBucketResponse$: StaticStructureSchema = [3, n0, _ADRTLBRs,
  0,
  [],
  []
];
export var AssociateDRTRoleRequest$: StaticStructureSchema = [3, n0, _ADRTRR,
  0,
  [_RA],
  [0]
];
export var AssociateDRTRoleResponse$: StaticStructureSchema = [3, n0, _ADRTRRs,
  0,
  [],
  []
];
export var AssociateHealthCheckRequest$: StaticStructureSchema = [3, n0, _AHCR,
  0,
  [_PI, _HCA],
  [0, 0]
];
export var AssociateHealthCheckResponse$: StaticStructureSchema = [3, n0, _AHCRs,
  0,
  [],
  []
];
export var AssociateProactiveEngagementDetailsRequest$: StaticStructureSchema = [3, n0, _APEDR,
  0,
  [_ECL],
  [() => EmergencyContactList]
];
export var AssociateProactiveEngagementDetailsResponse$: StaticStructureSchema = [3, n0, _APEDRs,
  0,
  [],
  []
];
export var AttackDetail$: StaticStructureSchema = [3, n0, _AD,
  0,
  [_AI, _RAe, _SR, _ST, _ET, _AC, _AP, _M],
  [0, 0, () => SubResourceSummaryList, 4, 4, () => SummarizedCounterList, () => AttackProperties, () => MitigationList]
];
export var AttackProperty$: StaticStructureSchema = [3, n0, _APt,
  0,
  [_AL, _API, _TC, _U, _T],
  [0, 0, () => TopContributors, 0, 1]
];
export var AttackStatisticsDataItem$: StaticStructureSchema = [3, n0, _ASDI,
  0,
  [_AV, _ACt],
  [() => AttackVolume$, 1]
];
export var AttackSummary$: StaticStructureSchema = [3, n0, _AS,
  0,
  [_AI, _RAe, _ST, _ET, _AVt],
  [0, 0, 4, 4, () => AttackVectorDescriptionList]
];
export var AttackVectorDescription$: StaticStructureSchema = [3, n0, _AVD,
  0,
  [_VT],
  [0]
];
export var AttackVolume$: StaticStructureSchema = [3, n0, _AV,
  0,
  [_BPS, _PPS, _RPS],
  [() => AttackVolumeStatistics$, () => AttackVolumeStatistics$, () => AttackVolumeStatistics$]
];
export var AttackVolumeStatistics$: StaticStructureSchema = [3, n0, _AVS,
  0,
  [_Ma],
  [1]
];
export var BlockAction$: StaticStructureSchema = [3, n0, _BA,
  0,
  [],
  []
];
export var Contributor$: StaticStructureSchema = [3, n0, _C,
  0,
  [_N, _V],
  [0, 1]
];
export var CountAction$: StaticStructureSchema = [3, n0, _CA,
  0,
  [],
  []
];
export var CreateProtectionGroupRequest$: StaticStructureSchema = [3, n0, _CPGR,
  0,
  [_PGI, _Ag, _P, _RT, _Me, _Ta],
  [0, 0, 0, 0, 64 | 0, () => TagList]
];
export var CreateProtectionGroupResponse$: StaticStructureSchema = [3, n0, _CPGRr,
  0,
  [],
  []
];
export var CreateProtectionRequest$: StaticStructureSchema = [3, n0, _CPR,
  0,
  [_N, _RAe, _Ta],
  [0, 0, () => TagList]
];
export var CreateProtectionResponse$: StaticStructureSchema = [3, n0, _CPRr,
  0,
  [_PI],
  [0]
];
export var CreateSubscriptionRequest$: StaticStructureSchema = [3, n0, _CSR,
  0,
  [],
  []
];
export var CreateSubscriptionResponse$: StaticStructureSchema = [3, n0, _CSRr,
  0,
  [],
  []
];
export var DeleteProtectionGroupRequest$: StaticStructureSchema = [3, n0, _DPGR,
  0,
  [_PGI],
  [0]
];
export var DeleteProtectionGroupResponse$: StaticStructureSchema = [3, n0, _DPGRe,
  0,
  [],
  []
];
export var DeleteProtectionRequest$: StaticStructureSchema = [3, n0, _DPR,
  0,
  [_PI],
  [0]
];
export var DeleteProtectionResponse$: StaticStructureSchema = [3, n0, _DPRe,
  0,
  [],
  []
];
export var DeleteSubscriptionRequest$: StaticStructureSchema = [3, n0, _DSR,
  0,
  [],
  []
];
export var DeleteSubscriptionResponse$: StaticStructureSchema = [3, n0, _DSRe,
  0,
  [],
  []
];
export var DescribeAttackRequest$: StaticStructureSchema = [3, n0, _DAR,
  0,
  [_AI],
  [0]
];
export var DescribeAttackResponse$: StaticStructureSchema = [3, n0, _DARe,
  0,
  [_At],
  [() => AttackDetail$]
];
export var DescribeAttackStatisticsRequest$: StaticStructureSchema = [3, n0, _DASR,
  0,
  [],
  []
];
export var DescribeAttackStatisticsResponse$: StaticStructureSchema = [3, n0, _DASRe,
  0,
  [_TR, _DI],
  [() => TimeRange$, () => AttackStatisticsDataList]
];
export var DescribeDRTAccessRequest$: StaticStructureSchema = [3, n0, _DDRTAR,
  0,
  [],
  []
];
export var DescribeDRTAccessResponse$: StaticStructureSchema = [3, n0, _DDRTARe,
  0,
  [_RA, _LBL],
  [0, 64 | 0]
];
export var DescribeEmergencyContactSettingsRequest$: StaticStructureSchema = [3, n0, _DECSR,
  0,
  [],
  []
];
export var DescribeEmergencyContactSettingsResponse$: StaticStructureSchema = [3, n0, _DECSRe,
  0,
  [_ECL],
  [() => EmergencyContactList]
];
export var DescribeProtectionGroupRequest$: StaticStructureSchema = [3, n0, _DPGRes,
  0,
  [_PGI],
  [0]
];
export var DescribeProtectionGroupResponse$: StaticStructureSchema = [3, n0, _DPGResc,
  0,
  [_PG],
  [() => ProtectionGroup$]
];
export var DescribeProtectionRequest$: StaticStructureSchema = [3, n0, _DPRes,
  0,
  [_PI, _RAe],
  [0, 0]
];
export var DescribeProtectionResponse$: StaticStructureSchema = [3, n0, _DPResc,
  0,
  [_Pr],
  [() => Protection$]
];
export var DescribeSubscriptionRequest$: StaticStructureSchema = [3, n0, _DSRes,
  0,
  [],
  []
];
export var DescribeSubscriptionResponse$: StaticStructureSchema = [3, n0, _DSResc,
  0,
  [_Su],
  [() => Subscription$]
];
export var DisableApplicationLayerAutomaticResponseRequest$: StaticStructureSchema = [3, n0, _DALARR,
  0,
  [_RAe],
  [0]
];
export var DisableApplicationLayerAutomaticResponseResponse$: StaticStructureSchema = [3, n0, _DALARRi,
  0,
  [],
  []
];
export var DisableProactiveEngagementRequest$: StaticStructureSchema = [3, n0, _DPER,
  0,
  [],
  []
];
export var DisableProactiveEngagementResponse$: StaticStructureSchema = [3, n0, _DPERi,
  0,
  [],
  []
];
export var DisassociateDRTLogBucketRequest$: StaticStructureSchema = [3, n0, _DDRTLBR,
  0,
  [_LB],
  [0]
];
export var DisassociateDRTLogBucketResponse$: StaticStructureSchema = [3, n0, _DDRTLBRi,
  0,
  [],
  []
];
export var DisassociateDRTRoleRequest$: StaticStructureSchema = [3, n0, _DDRTRR,
  0,
  [],
  []
];
export var DisassociateDRTRoleResponse$: StaticStructureSchema = [3, n0, _DDRTRRi,
  0,
  [],
  []
];
export var DisassociateHealthCheckRequest$: StaticStructureSchema = [3, n0, _DHCR,
  0,
  [_PI, _HCA],
  [0, 0]
];
export var DisassociateHealthCheckResponse$: StaticStructureSchema = [3, n0, _DHCRi,
  0,
  [],
  []
];
export var EmergencyContact$: StaticStructureSchema = [3, n0, _EC,
  0,
  [_EA, _PN, _CN],
  [0, 0, 0]
];
export var EnableApplicationLayerAutomaticResponseRequest$: StaticStructureSchema = [3, n0, _EALARR,
  0,
  [_RAe, _A],
  [0, () => ResponseAction$]
];
export var EnableApplicationLayerAutomaticResponseResponse$: StaticStructureSchema = [3, n0, _EALARRn,
  0,
  [],
  []
];
export var EnableProactiveEngagementRequest$: StaticStructureSchema = [3, n0, _EPER,
  0,
  [],
  []
];
export var EnableProactiveEngagementResponse$: StaticStructureSchema = [3, n0, _EPERn,
  0,
  [],
  []
];
export var GetSubscriptionStateRequest$: StaticStructureSchema = [3, n0, _GSSR,
  0,
  [],
  []
];
export var GetSubscriptionStateResponse$: StaticStructureSchema = [3, n0, _GSSRe,
  0,
  [_SS],
  [0]
];
export var InclusionProtectionFilters$: StaticStructureSchema = [3, n0, _IPF,
  0,
  [_RAes, _PNr, _RTe],
  [64 | 0, 64 | 0, 64 | 0]
];
export var InclusionProtectionGroupFilters$: StaticStructureSchema = [3, n0, _IPGF,
  0,
  [_PGIr, _Pa, _RTe, _Agg],
  [64 | 0, 64 | 0, 64 | 0, 64 | 0]
];
export var InternalErrorException$: StaticErrorSchema = [-3, n0, _IEE,
  { [_e]: _s },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(InternalErrorException$, InternalErrorException);
export var InvalidOperationException$: StaticErrorSchema = [-3, n0, _IOE,
  { [_e]: _c },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidOperationException$, InvalidOperationException);
export var InvalidPaginationTokenException$: StaticErrorSchema = [-3, n0, _IPTE,
  { [_e]: _c },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidPaginationTokenException$, InvalidPaginationTokenException);
export var InvalidParameterException$: StaticErrorSchema = [-3, n0, _IPE,
  { [_e]: _c },
  [_m, _r, _f],
  [0, 0, () => ValidationExceptionFieldList]
];
TypeRegistry.for(n0).registerError(InvalidParameterException$, InvalidParameterException);
export var InvalidResourceException$: StaticErrorSchema = [-3, n0, _IRE,
  { [_e]: _c },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidResourceException$, InvalidResourceException);
export var Limit$: StaticStructureSchema = [3, n0, _L,
  0,
  [_Ty, _Ma],
  [0, 1]
];
export var LimitsExceededException$: StaticErrorSchema = [-3, n0, _LEE,
  { [_e]: _c },
  [_m, _Ty, _L],
  [0, 0, 1]
];
TypeRegistry.for(n0).registerError(LimitsExceededException$, LimitsExceededException);
export var ListAttacksRequest$: StaticStructureSchema = [3, n0, _LAR,
  0,
  [_RAes, _ST, _ET, _NT, _MR],
  [64 | 0, () => TimeRange$, () => TimeRange$, 0, 1]
];
export var ListAttacksResponse$: StaticStructureSchema = [3, n0, _LARi,
  0,
  [_ASt, _NT],
  [() => AttackSummaries, 0]
];
export var ListProtectionGroupsRequest$: StaticStructureSchema = [3, n0, _LPGR,
  0,
  [_NT, _MR, _IF],
  [0, 1, () => InclusionProtectionGroupFilters$]
];
export var ListProtectionGroupsResponse$: StaticStructureSchema = [3, n0, _LPGRi,
  0,
  [_PGr, _NT],
  [() => ProtectionGroups, 0]
];
export var ListProtectionsRequest$: StaticStructureSchema = [3, n0, _LPR,
  0,
  [_NT, _MR, _IF],
  [0, 1, () => InclusionProtectionFilters$]
];
export var ListProtectionsResponse$: StaticStructureSchema = [3, n0, _LPRi,
  0,
  [_Pro, _NT],
  [() => Protections, 0]
];
export var ListResourcesInProtectionGroupRequest$: StaticStructureSchema = [3, n0, _LRIPGR,
  0,
  [_PGI, _NT, _MR],
  [0, 0, 1]
];
export var ListResourcesInProtectionGroupResponse$: StaticStructureSchema = [3, n0, _LRIPGRi,
  0,
  [_RAes, _NT],
  [64 | 0, 0]
];
export var ListTagsForResourceRequest$: StaticStructureSchema = [3, n0, _LTFRR,
  0,
  [_RARN],
  [0]
];
export var ListTagsForResourceResponse$: StaticStructureSchema = [3, n0, _LTFRRi,
  0,
  [_Ta],
  [() => TagList]
];
export var LockedSubscriptionException$: StaticErrorSchema = [-3, n0, _LSE,
  { [_e]: _c },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(LockedSubscriptionException$, LockedSubscriptionException);
export var Mitigation$: StaticStructureSchema = [3, n0, _Mi,
  0,
  [_MN],
  [0]
];
export var NoAssociatedRoleException$: StaticErrorSchema = [-3, n0, _NARE,
  { [_e]: _c },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(NoAssociatedRoleException$, NoAssociatedRoleException);
export var OptimisticLockException$: StaticErrorSchema = [-3, n0, _OLE,
  { [_e]: _c },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(OptimisticLockException$, OptimisticLockException);
export var Protection$: StaticStructureSchema = [3, n0, _Pr,
  0,
  [_I, _N, _RAe, _HCI, _PA, _ALARC],
  [0, 0, 0, 64 | 0, 0, () => ApplicationLayerAutomaticResponseConfiguration$]
];
export var ProtectionGroup$: StaticStructureSchema = [3, n0, _PG,
  0,
  [_PGI, _Ag, _P, _RT, _Me, _PGA],
  [0, 0, 0, 0, 64 | 0, 0]
];
export var ProtectionGroupArbitraryPatternLimits$: StaticStructureSchema = [3, n0, _PGAPL,
  0,
  [_MM],
  [1]
];
export var ProtectionGroupLimits$: StaticStructureSchema = [3, n0, _PGL,
  0,
  [_MPG, _PTL],
  [1, () => ProtectionGroupPatternTypeLimits$]
];
export var ProtectionGroupPatternTypeLimits$: StaticStructureSchema = [3, n0, _PGPTL,
  0,
  [_APL],
  [() => ProtectionGroupArbitraryPatternLimits$]
];
export var ProtectionLimits$: StaticStructureSchema = [3, n0, _PL,
  0,
  [_PRTL],
  [() => Limits]
];
export var ResourceAlreadyExistsException$: StaticErrorSchema = [-3, n0, _RAEE,
  { [_e]: _c },
  [_m, _rT],
  [0, 0]
];
TypeRegistry.for(n0).registerError(ResourceAlreadyExistsException$, ResourceAlreadyExistsException);
export var ResourceNotFoundException$: StaticErrorSchema = [-3, n0, _RNFE,
  { [_e]: _c },
  [_m, _rT],
  [0, 0]
];
TypeRegistry.for(n0).registerError(ResourceNotFoundException$, ResourceNotFoundException);
export var ResponseAction$: StaticStructureSchema = [3, n0, _RAesp,
  0,
  [_B, _Co],
  [() => BlockAction$, () => CountAction$]
];
export var SubResourceSummary$: StaticStructureSchema = [3, n0, _SRS,
  0,
  [_Ty, _I, _AVt, _Cou],
  [0, 0, () => SummarizedAttackVectorList, () => SummarizedCounterList]
];
export var Subscription$: StaticStructureSchema = [3, n0, _Su,
  0,
  [_ST, _ET, _TCIS, _AR, _Li, _PES, _SL, _SA],
  [4, 4, 1, 0, () => Limits, 0, () => SubscriptionLimits$, 0]
];
export var SubscriptionLimits$: StaticStructureSchema = [3, n0, _SL,
  0,
  [_PL, _PGL],
  [() => ProtectionLimits$, () => ProtectionGroupLimits$]
];
export var SummarizedAttackVector$: StaticStructureSchema = [3, n0, _SAV,
  0,
  [_VT, _VC],
  [0, () => SummarizedCounterList]
];
export var SummarizedCounter$: StaticStructureSchema = [3, n0, _SC,
  0,
  [_N, _Ma, _Av, _Sum, _N_, _U],
  [0, 1, 1, 1, 1, 0]
];
export var Tag$: StaticStructureSchema = [3, n0, _Tag,
  0,
  [_K, _V],
  [0, 0]
];
export var TagResourceRequest$: StaticStructureSchema = [3, n0, _TRR,
  0,
  [_RARN, _Ta],
  [0, () => TagList]
];
export var TagResourceResponse$: StaticStructureSchema = [3, n0, _TRRa,
  0,
  [],
  []
];
export var TimeRange$: StaticStructureSchema = [3, n0, _TR,
  0,
  [_FI, _TE],
  [4, 4]
];
export var UntagResourceRequest$: StaticStructureSchema = [3, n0, _URR,
  0,
  [_RARN, _TK],
  [0, 64 | 0]
];
export var UntagResourceResponse$: StaticStructureSchema = [3, n0, _URRn,
  0,
  [],
  []
];
export var UpdateApplicationLayerAutomaticResponseRequest$: StaticStructureSchema = [3, n0, _UALARR,
  0,
  [_RAe, _A],
  [0, () => ResponseAction$]
];
export var UpdateApplicationLayerAutomaticResponseResponse$: StaticStructureSchema = [3, n0, _UALARRp,
  0,
  [],
  []
];
export var UpdateEmergencyContactSettingsRequest$: StaticStructureSchema = [3, n0, _UECSR,
  0,
  [_ECL],
  [() => EmergencyContactList]
];
export var UpdateEmergencyContactSettingsResponse$: StaticStructureSchema = [3, n0, _UECSRp,
  0,
  [],
  []
];
export var UpdateProtectionGroupRequest$: StaticStructureSchema = [3, n0, _UPGR,
  0,
  [_PGI, _Ag, _P, _RT, _Me],
  [0, 0, 0, 0, 64 | 0]
];
export var UpdateProtectionGroupResponse$: StaticStructureSchema = [3, n0, _UPGRp,
  0,
  [],
  []
];
export var UpdateSubscriptionRequest$: StaticStructureSchema = [3, n0, _USR,
  0,
  [_AR],
  [0]
];
export var UpdateSubscriptionResponse$: StaticStructureSchema = [3, n0, _USRp,
  0,
  [],
  []
];
export var ValidationExceptionField$: StaticStructureSchema = [3, n0, _VEF,
  0,
  [_n, _m],
  [0, 0]
];
export var ShieldServiceException$: StaticErrorSchema = [-3, _sm, "ShieldServiceException", 0, [], []];
TypeRegistry.for(_sm).registerError(ShieldServiceException$, ShieldServiceException);
var AttackProperties: StaticListSchema = [1, n0, _AP,
  0, () => AttackProperty$
];
var AttackStatisticsDataList: StaticListSchema = [1, n0, _ASDL,
  0, () => AttackStatisticsDataItem$
];
var AttackSummaries: StaticListSchema = [1, n0, _ASt,
  0, () => AttackSummary$
];
var AttackVectorDescriptionList: StaticListSchema = [1, n0, _AVDL,
  0, () => AttackVectorDescription$
];
var EmergencyContactList: StaticListSchema = [1, n0, _ECL,
  0, () => EmergencyContact$
];
var HealthCheckIds = 64 | 0;
var Limits: StaticListSchema = [1, n0, _Li,
  0, () => Limit$
];
var LogBucketList = 64 | 0;
var MitigationList: StaticListSchema = [1, n0, _ML,
  0, () => Mitigation$
];
var ProtectedResourceTypeFilters = 64 | 0;
var ProtectionGroupAggregationFilters = 64 | 0;
var ProtectionGroupIdFilters = 64 | 0;
var ProtectionGroupMembers = 64 | 0;
var ProtectionGroupPatternFilters = 64 | 0;
var ProtectionGroups: StaticListSchema = [1, n0, _PGr,
  0, () => ProtectionGroup$
];
var ProtectionNameFilters = 64 | 0;
var Protections: StaticListSchema = [1, n0, _Pro,
  0, () => Protection$
];
var ResourceArnFilterList = 64 | 0;
var ResourceArnFilters = 64 | 0;
var ResourceArnList = 64 | 0;
var SubResourceSummaryList: StaticListSchema = [1, n0, _SRSL,
  0, () => SubResourceSummary$
];
var SummarizedAttackVectorList: StaticListSchema = [1, n0, _SAVL,
  0, () => SummarizedAttackVector$
];
var SummarizedCounterList: StaticListSchema = [1, n0, _SCL,
  0, () => SummarizedCounter$
];
var TagKeyList = 64 | 0;
var TagList: StaticListSchema = [1, n0, _TL,
  0, () => Tag$
];
var TopContributors: StaticListSchema = [1, n0, _TC,
  0, () => Contributor$
];
var ValidationExceptionFieldList: StaticListSchema = [1, n0, _VEFL,
  0, () => ValidationExceptionField$
];
export var AssociateDRTLogBucket$: StaticOperationSchema = [9, n0, _ADRTLB,
  0, () => AssociateDRTLogBucketRequest$, () => AssociateDRTLogBucketResponse$
];
export var AssociateDRTRole$: StaticOperationSchema = [9, n0, _ADRTR,
  0, () => AssociateDRTRoleRequest$, () => AssociateDRTRoleResponse$
];
export var AssociateHealthCheck$: StaticOperationSchema = [9, n0, _AHC,
  0, () => AssociateHealthCheckRequest$, () => AssociateHealthCheckResponse$
];
export var AssociateProactiveEngagementDetails$: StaticOperationSchema = [9, n0, _APED,
  0, () => AssociateProactiveEngagementDetailsRequest$, () => AssociateProactiveEngagementDetailsResponse$
];
export var CreateProtection$: StaticOperationSchema = [9, n0, _CP,
  0, () => CreateProtectionRequest$, () => CreateProtectionResponse$
];
export var CreateProtectionGroup$: StaticOperationSchema = [9, n0, _CPG,
  0, () => CreateProtectionGroupRequest$, () => CreateProtectionGroupResponse$
];
export var CreateSubscription$: StaticOperationSchema = [9, n0, _CS,
  0, () => CreateSubscriptionRequest$, () => CreateSubscriptionResponse$
];
export var DeleteProtection$: StaticOperationSchema = [9, n0, _DP,
  0, () => DeleteProtectionRequest$, () => DeleteProtectionResponse$
];
export var DeleteProtectionGroup$: StaticOperationSchema = [9, n0, _DPG,
  0, () => DeleteProtectionGroupRequest$, () => DeleteProtectionGroupResponse$
];
export var DeleteSubscription$: StaticOperationSchema = [9, n0, _DS,
  0, () => DeleteSubscriptionRequest$, () => DeleteSubscriptionResponse$
];
export var DescribeAttack$: StaticOperationSchema = [9, n0, _DA,
  0, () => DescribeAttackRequest$, () => DescribeAttackResponse$
];
export var DescribeAttackStatistics$: StaticOperationSchema = [9, n0, _DAS,
  0, () => DescribeAttackStatisticsRequest$, () => DescribeAttackStatisticsResponse$
];
export var DescribeDRTAccess$: StaticOperationSchema = [9, n0, _DDRTA,
  0, () => DescribeDRTAccessRequest$, () => DescribeDRTAccessResponse$
];
export var DescribeEmergencyContactSettings$: StaticOperationSchema = [9, n0, _DECS,
  0, () => DescribeEmergencyContactSettingsRequest$, () => DescribeEmergencyContactSettingsResponse$
];
export var DescribeProtection$: StaticOperationSchema = [9, n0, _DPe,
  0, () => DescribeProtectionRequest$, () => DescribeProtectionResponse$
];
export var DescribeProtectionGroup$: StaticOperationSchema = [9, n0, _DPGe,
  0, () => DescribeProtectionGroupRequest$, () => DescribeProtectionGroupResponse$
];
export var DescribeSubscription$: StaticOperationSchema = [9, n0, _DSe,
  0, () => DescribeSubscriptionRequest$, () => DescribeSubscriptionResponse$
];
export var DisableApplicationLayerAutomaticResponse$: StaticOperationSchema = [9, n0, _DALAR,
  0, () => DisableApplicationLayerAutomaticResponseRequest$, () => DisableApplicationLayerAutomaticResponseResponse$
];
export var DisableProactiveEngagement$: StaticOperationSchema = [9, n0, _DPE,
  0, () => DisableProactiveEngagementRequest$, () => DisableProactiveEngagementResponse$
];
export var DisassociateDRTLogBucket$: StaticOperationSchema = [9, n0, _DDRTLB,
  0, () => DisassociateDRTLogBucketRequest$, () => DisassociateDRTLogBucketResponse$
];
export var DisassociateDRTRole$: StaticOperationSchema = [9, n0, _DDRTR,
  0, () => DisassociateDRTRoleRequest$, () => DisassociateDRTRoleResponse$
];
export var DisassociateHealthCheck$: StaticOperationSchema = [9, n0, _DHC,
  0, () => DisassociateHealthCheckRequest$, () => DisassociateHealthCheckResponse$
];
export var EnableApplicationLayerAutomaticResponse$: StaticOperationSchema = [9, n0, _EALAR,
  0, () => EnableApplicationLayerAutomaticResponseRequest$, () => EnableApplicationLayerAutomaticResponseResponse$
];
export var EnableProactiveEngagement$: StaticOperationSchema = [9, n0, _EPE,
  0, () => EnableProactiveEngagementRequest$, () => EnableProactiveEngagementResponse$
];
export var GetSubscriptionState$: StaticOperationSchema = [9, n0, _GSS,
  0, () => GetSubscriptionStateRequest$, () => GetSubscriptionStateResponse$
];
export var ListAttacks$: StaticOperationSchema = [9, n0, _LA,
  0, () => ListAttacksRequest$, () => ListAttacksResponse$
];
export var ListProtectionGroups$: StaticOperationSchema = [9, n0, _LPG,
  0, () => ListProtectionGroupsRequest$, () => ListProtectionGroupsResponse$
];
export var ListProtections$: StaticOperationSchema = [9, n0, _LP,
  0, () => ListProtectionsRequest$, () => ListProtectionsResponse$
];
export var ListResourcesInProtectionGroup$: StaticOperationSchema = [9, n0, _LRIPG,
  0, () => ListResourcesInProtectionGroupRequest$, () => ListResourcesInProtectionGroupResponse$
];
export var ListTagsForResource$: StaticOperationSchema = [9, n0, _LTFR,
  0, () => ListTagsForResourceRequest$, () => ListTagsForResourceResponse$
];
export var TagResource$: StaticOperationSchema = [9, n0, _TRa,
  0, () => TagResourceRequest$, () => TagResourceResponse$
];
export var UntagResource$: StaticOperationSchema = [9, n0, _UR,
  0, () => UntagResourceRequest$, () => UntagResourceResponse$
];
export var UpdateApplicationLayerAutomaticResponse$: StaticOperationSchema = [9, n0, _UALAR,
  0, () => UpdateApplicationLayerAutomaticResponseRequest$, () => UpdateApplicationLayerAutomaticResponseResponse$
];
export var UpdateEmergencyContactSettings$: StaticOperationSchema = [9, n0, _UECS,
  0, () => UpdateEmergencyContactSettingsRequest$, () => UpdateEmergencyContactSettingsResponse$
];
export var UpdateProtectionGroup$: StaticOperationSchema = [9, n0, _UPG,
  0, () => UpdateProtectionGroupRequest$, () => UpdateProtectionGroupResponse$
];
export var UpdateSubscription$: StaticOperationSchema = [9, n0, _US,
  0, () => UpdateSubscriptionRequest$, () => UpdateSubscriptionResponse$
];
