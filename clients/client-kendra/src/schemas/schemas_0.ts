const _A = "Attributes";
const _AA = "AdditionalAttributes";
const _AAF = "AndAllFilters";
const _AC = "AclConfiguration";
const _ACC = "AccessControlConfigurations";
const _ACCI = "AccessControlConfigurationId";
const _ACCS = "AccessControlConfigurationSummary";
const _ACCSL = "AccessControlConfigurationSummaryList";
const _ACL = "AccessControlList";
const _ACLC = "AccessControlListConfiguration";
const _ACl = "AlfrescoConfiguration";
const _ACt = "AttachmentConfiguration";
const _ACu = "AuthenticationConfiguration";
const _ADE = "AccessDeniedException";
const _AEL = "AssociateEntityList";
const _AETE = "AssociateEntitiesToExperience";
const _AETEFEL = "AssociateEntitiesToExperienceFailedEntityList";
const _AETER = "AssociateEntitiesToExperienceRequest";
const _AETERs = "AssociateEntitiesToExperienceResponse";
const _AF = "AttributeFilter";
const _AFL = "AttributeFilterList";
const _AFM = "AttachmentFieldMappings";
const _AGCN = "AllowedGroupsColumnName";
const _AN = "AttributeName";
const _APTE = "AssociatePersonasToEntities";
const _APTER = "AssociatePersonasToEntitiesRequest";
const _APTERs = "AssociatePersonasToEntitiesResponse";
const _ARA = "AdditionalResultAttribute";
const _ARAL = "AdditionalResultAttributeList";
const _ARAV = "AdditionalResultAttributeValue";
const _ARAd = "AdditionalResponseAttributes";
const _ASC = "AttributeSuggestionsConfig";
const _ASDC = "AttributeSuggestionsDescribeConfig";
const _ASGC = "AttributeSuggestionsGetConfig";
const _ASM = "AttributeSuggestionsMode";
const _ASUC = "AttributeSuggestionsUpdateConfig";
const _AT = "AuthenticationType";
const _Ac = "Access";
const _B = "Blob";
const _BA = "BasicAuthentication";
const _BAC = "BasicAuthenticationConfiguration";
const _BACL = "BasicAuthenticationConfigurationList";
const _BC = "BoxConfiguration";
const _BCl = "BlogConfiguration";
const _BDD = "BatchDeleteDocument";
const _BDDR = "BatchDeleteDocumentRequest";
const _BDDRFD = "BatchDeleteDocumentResponseFailedDocument";
const _BDDRFDa = "BatchDeleteDocumentResponseFailedDocuments";
const _BDDRa = "BatchDeleteDocumentResponse";
const _BDFRS = "BatchDeleteFeaturedResultsSet";
const _BDFRSE = "BatchDeleteFeaturedResultsSetError";
const _BDFRSEa = "BatchDeleteFeaturedResultsSetErrors";
const _BDFRSR = "BatchDeleteFeaturedResultsSetRequest";
const _BDFRSRa = "BatchDeleteFeaturedResultsSetResponse";
const _BFM = "BlogFieldMappings";
const _BGDS = "BatchGetDocumentStatus";
const _BGDSR = "BatchGetDocumentStatusRequest";
const _BGDSRE = "BatchGetDocumentStatusResponseError";
const _BGDSREa = "BatchGetDocumentStatusResponseErrors";
const _BGDSRa = "BatchGetDocumentStatusResponse";
const _BLSI = "BlockListSummaryItems";
const _BN = "BucketName";
const _BO = "BeginOffset";
const _BPD = "BatchPutDocument";
const _BPDR = "BatchPutDocumentRequest";
const _BPDRFD = "BatchPutDocumentResponseFailedDocument";
const _BPDRFDa = "BatchPutDocumentResponseFailedDocuments";
const _BPDRa = "BatchPutDocumentResponse";
const _Bu = "Bucket";
const _C = "Credentials";
const _CA = "ContainsAll";
const _CAC = "ConfluenceAttachmentConfiguration";
const _CACC = "CreateAccessControlConfiguration";
const _CACCR = "CreateAccessControlConfigurationRequest";
const _CACCRr = "CreateAccessControlConfigurationResponse";
const _CAFML = "ConfluenceAttachmentFieldMappingsList";
const _CAS = "CrawlArchivedSpaces";
const _CATIFM = "ConfluenceAttachmentToIndexFieldMapping";
const _CAo = "ContainsAny";
const _CAr = "CrawlAttachments";
const _CAre = "CreatedAt";
const _CBC = "ConfluenceBlogConfiguration";
const _CBFML = "ConfluenceBlogFieldMappingsList";
const _CBM = "CrawlBotMessage";
const _CBTIFM = "ConfluenceBlogToIndexFieldMapping";
const _CC = "CrawlComments";
const _CCR = "CrawlChatRooms";
const _CCo = "CollapseConfiguration";
const _CCol = "ColumnConfiguration";
const _CCon = "ConfluenceConfiguration";
const _CConn = "ConnectionConfiguration";
const _CD = "CrawlDepth";
const _CDAK = "ConditionDocumentAttributeKey";
const _CDC = "ChangeDetectingColumns";
const _CDEC = "CustomDocumentEnrichmentConfiguration";
const _CDS = "CreateDataSource";
const _CDSR = "CreateDataSourceRequest";
const _CDSRr = "CreateDataSourceResponse";
const _CE = "ConflictException";
const _CER = "CreateExperienceRequest";
const _CERr = "CreateExperienceResponse";
const _CEr = "CreateExperience";
const _CF = "ClickFeedback";
const _CFC = "CrawlFileComments";
const _CFCh = "ChatterFeedConfiguration";
const _CFI = "ClickFeedbackItems";
const _CFL = "ClickFeedbackList";
const _CFM = "CommentFieldMappings";
const _CFR = "CreateFaqRequest";
const _CFRS = "CreateFeaturedResultsSet";
const _CFRSR = "CreateFeaturedResultsSetRequest";
const _CFRSRr = "CreateFeaturedResultsSetResponse";
const _CFRr = "CreateFaqResponse";
const _CFr = "CreateFaq";
const _CI = "ConflictingItem";
const _CIC = "CrawlIssueComment";
const _CICA = "CrawlIssueCommentAttachment";
const _CIR = "CreateIndexRequest";
const _CIRr = "CreateIndexResponse";
const _CIo = "ConflictingItems";
const _CIr = "CrawlIssue";
const _CIre = "CreateIndex";
const _CKATC = "CustomKnowledgeArticleTypeConfigurations";
const _CL = "CorrectionList";
const _COV = "ConditionOnValue";
const _CPC = "ConfluencePageConfiguration";
const _CPFML = "ConfluencePageFieldMappingsList";
const _CPR = "CrawlPullRequest";
const _CPRC = "CrawlPullRequestComment";
const _CPRCA = "CrawlPullRequestCommentAttachment";
const _CPS = "CrawlPersonalSpaces";
const _CPTIFM = "ConfluencePageToIndexFieldMapping";
const _CQS = "ClearQuerySuggestions";
const _CQSBL = "CreateQuerySuggestionsBlockList";
const _CQSBLR = "CreateQuerySuggestionsBlockListRequest";
const _CQSBLRr = "CreateQuerySuggestionsBlockListResponse";
const _CQSR = "ClearQuerySuggestionsRequest";
const _CR = "ClaimRegex";
const _CRD = "CollapsedResultDetail";
const _CRDr = "CrawlRepositoryDocuments";
const _CSC = "ConfluenceSpaceConfiguration";
const _CSCo = "ContentSourceConfiguration";
const _CSF = "CrawlSystemFolders";
const _CSFML = "ConfluenceSpaceFieldMappingsList";
const _CSTIFM = "ConfluenceSpaceToIndexFieldMapping";
const _CT = "CrawlTasks";
const _CTR = "CreateThesaurusRequest";
const _CTRr = "CreateThesaurusResponse";
const _CTl = "ClickTime";
const _CTli = "ClientToken";
const _CTo = "CorrectedTerm";
const _CTon = "ContentType";
const _CTr = "CreationTimestamp";
const _CTre = "CreateThesaurus";
const _CU = "CapacityUnits";
const _CUC = "CapacityUnitsConfiguration";
const _CWL = "CrawlWebLinks";
const _Ce = "Cells";
const _Co = "Correction";
const _Cod = "Code";
const _Con = "Configuration";
const _Cond = "Condition";
const _Cont = "Content";
const _Cor = "Corrections";
const _Cou = "Count";
const _D = "Documents";
const _DA = "DocumentAttribute";
const _DAC = "DocumentAttributeCondition";
const _DACC = "DeleteAccessControlConfiguration";
const _DACCR = "DeleteAccessControlConfigurationRequest";
const _DACCRe = "DeleteAccessControlConfigurationResponse";
const _DACCRes = "DescribeAccessControlConfigurationRequest";
const _DACCResc = "DescribeAccessControlConfigurationResponse";
const _DACCe = "DescribeAccessControlConfiguration";
const _DAK = "DocumentAttributeKey";
const _DAL = "DocumentAttributeList";
const _DAT = "DocumentAttributeTarget";
const _DAV = "DocumentAttributeValue";
const _DAVCP = "DocumentAttributeValueCountPair";
const _DAVCPL = "DocumentAttributeValueCountPairList";
const _DAVCPo = "DocumentAttributeValueCountPairs";
const _DAVT = "DocumentAttributeValueType";
const _DAo = "DocumentsAdded";
const _DAoc = "DocumentAttributes";
const _DC = "DatabaseConfiguration";
const _DCD = "DocumentContentDeletion";
const _DD = "DocumentsDeleted";
const _DDCN = "DocumentDataColumnName";
const _DDFN = "DocumentDataFieldName";
const _DDS = "DeleteDataSource";
const _DDSR = "DeleteDataSourceRequest";
const _DDSRe = "DescribeDataSourceRequest";
const _DDSRes = "DescribeDataSourceResponse";
const _DDSe = "DescribeDataSource";
const _DDi = "DisplayData";
const _DE = "DocumentExcerpt";
const _DEFE = "DisassociateEntitiesFromExperience";
const _DEFER = "DisassociateEntitiesFromExperienceRequest";
const _DEFERi = "DisassociateEntitiesFromExperienceResponse";
const _DEL = "DisassociateEntityList";
const _DER = "DeleteExperienceRequest";
const _DERe = "DeleteExperienceResponse";
const _DERes = "DescribeExperienceRequest";
const _DEResc = "DescribeExperienceResponse";
const _DET = "DatabaseEngineType";
const _DEe = "DeleteExperience";
const _DEes = "DescribeExperience";
const _DF = "DocumentsFailed";
const _DFF = "DateFieldFormat";
const _DFR = "DeleteFaqRequest";
const _DFRS = "DescribeFeaturedResultsSet";
const _DFRSR = "DescribeFeaturedResultsSetRequest";
const _DFRSRe = "DescribeFeaturedResultsSetResponse";
const _DFRe = "DescribeFaqRequest";
const _DFRes = "DescribeFaqResponse";
const _DFe = "DeleteFaq";
const _DFes = "DescribeFaq";
const _DH = "DatabaseHost";
const _DI = "DocumentId";
const _DICN = "DocumentIdColumnName";
const _DIL = "DocumentIdList";
const _DILo = "DocumentInfoList";
const _DIR = "DeleteIndexRequest";
const _DIRe = "DescribeIndexRequest";
const _DIRes = "DescribeIndexResponse";
const _DIe = "DeleteIndex";
const _DIes = "DescribeIndex";
const _DIo = "DocumentInfo";
const _DL = "DocumentList";
const _DLFM = "DocumentLibraryFieldMappings";
const _DLG = "DisableLocalGroups";
const _DM = "DocumentsModified";
const _DMC = "DocumentMetadataConfigurations";
const _DMCL = "DocumentMetadataConfigurationList";
const _DMCU = "DocumentMetadataConfigurationUpdates";
const _DMCo = "DocumentMetadataConfiguration";
const _DMCoc = "DocumentsMetadataConfiguration";
const _DN = "DatabaseName";
const _DP = "DatabasePort";
const _DPC = "DirectPutContent";
const _DPFE = "DisassociatePersonasFromEntities";
const _DPFER = "DisassociatePersonasFromEntitiesRequest";
const _DPFERi = "DisassociatePersonasFromEntitiesResponse";
const _DPM = "DeletePrincipalMapping";
const _DPMR = "DeletePrincipalMappingRequest";
const _DPMRe = "DescribePrincipalMappingRequest";
const _DPMRes = "DescribePrincipalMappingResponse";
const _DPMe = "DescribePrincipalMapping";
const _DQSBL = "DeleteQuerySuggestionsBlockList";
const _DQSBLR = "DeleteQuerySuggestionsBlockListRequest";
const _DQSBLRe = "DescribeQuerySuggestionsBlockListRequest";
const _DQSBLRes = "DescribeQuerySuggestionsBlockListResponse";
const _DQSBLe = "DescribeQuerySuggestionsBlockList";
const _DQSC = "DescribeQuerySuggestionsConfig";
const _DQSCR = "DescribeQuerySuggestionsConfigRequest";
const _DQSCRe = "DescribeQuerySuggestionsConfigResponse";
const _DRC = "DocumentRelevanceConfiguration";
const _DROC = "DocumentRelevanceOverrideConfigurations";
const _DROCL = "DocumentRelevanceOverrideConfigurationList";
const _DS = "DocumentsScanned";
const _DSC = "DataSourceConfiguration";
const _DSEC = "DataSourceErrorCode";
const _DSFN = "DataSourceFieldName";
const _DSG = "DataSourceGroup";
const _DSGa = "DataSourceGroups";
const _DSI = "DataSourceId";
const _DSIa = "DataSourceIds";
const _DSL = "DocumentStatusList";
const _DSS = "DataSourceSummary";
const _DSSJ = "DataSourceSyncJob";
const _DSSJHL = "DataSourceSyncJobHistoryList";
const _DSSJI = "DataSourceSyncJobId";
const _DSSJM = "DataSourceSyncJobMetrics";
const _DSSJMT = "DataSourceSyncJobMetricTarget";
const _DSSL = "DataSourceSummaryList";
const _DSTIFM = "DataSourceToIndexFieldMapping";
const _DSTIFML = "DataSourceToIndexFieldMappingList";
const _DSVC = "DataSourceVpcConfiguration";
const _DSo = "DocumentStatus";
const _DT = "DocumentTitle";
const _DTCN = "DocumentTitleColumnName";
const _DTFN = "DocumentTitleFieldName";
const _DTR = "DeleteThesaurusRequest";
const _DTRe = "DescribeThesaurusRequest";
const _DTRes = "DescribeThesaurusResponse";
const _DTe = "DeleteThesaurus";
const _DTes = "DescribeThesaurus";
const _DURI = "DocumentURI";
const _DV = "DateValue";
const _De = "Description";
const _Di = "Displayable";
const _Do = "Document";
const _Dom = "Domain";
const _Du = "Duration";
const _E = "Errors";
const _EA = "ExcludeArchived";
const _EAFP = "ExcludeAttachmentFilePatterns";
const _EC = "ErrorCode";
const _ECn = "EntityConfiguration";
const _ECx = "ExpandConfiguration";
const _ECxp = "ExperienceConfiguration";
const _EDD = "EntityDisplayData";
const _EE = "ExperienceEndpoint";
const _EES = "ExperienceEntitiesSummary";
const _EESL = "ExperienceEntitiesSummaryList";
const _EEx = "ExperienceEndpoints";
const _EF = "EntityFilter";
const _EFNP = "ExclusionFolderNamePatterns";
const _EFNPx = "ExclusionFileNamePatterns";
const _EFTP = "ExclusionFileTypePatterns";
const _EI = "EnterpriseId";
const _EIn = "EntityIds";
const _EInt = "EntityId";
const _EIx = "ExecutionId";
const _EL = "EntityList";
const _EM = "ErrorMessage";
const _EMT = "ExcludeMimeTypes";
const _EO = "EndOffset";
const _EP = "ExclusionPatterns";
const _EPC = "EntityPersonaConfiguration";
const _EPCL = "EntityPersonaConfigurationList";
const _ER = "ExpandedResults";
const _ERI = "ExpandedResultItem";
const _ERL = "ExpandedResultList";
const _ES = "ExcludeSpaces";
const _ESD = "ExcludeSharedDrives";
const _ESL = "ExperiencesSummaryList";
const _ESx = "ExperiencesSummary";
const _ET = "EqualsTo";
const _ETn = "EndTime";
const _ETnd = "EndpointType";
const _ETnt = "EntityType";
const _EUA = "ExcludeUserAccounts";
const _Ed = "Edition";
const _En = "Endpoints";
const _End = "Endpoint";
const _Ex = "Expand";
const _F = "Facet";
const _FC = "FsxConfiguration";
const _FCa = "FailureCode";
const _FD = "FailedDocuments";
const _FDL = "FeaturedDocumentList";
const _FDM = "FeaturedDocumentsMissing";
const _FDML = "FeaturedDocumentMissingList";
const _FDMe = "FeaturedDocumentMissing";
const _FDWM = "FeaturedDocumentsWithMetadata";
const _FDWML = "FeaturedDocumentWithMetadataList";
const _FDWMe = "FeaturedDocumentWithMetadata";
const _FDe = "FeaturedDocuments";
const _FDea = "FeaturedDocument";
const _FE = "FailedEntity";
const _FEL = "FailedEntityList";
const _FF = "FileFormat";
const _FFM = "FileFieldMappings";
const _FI = "FaqIds";
const _FIo = "FolderIds";
const _FL = "FacetList";
const _FM = "FieldMappings";
const _FN = "FirstName";
const _FQ = "FilterQuery";
const _FR = "FacetResults";
const _FRCE = "FeaturedResultsConflictException";
const _FRI = "FeaturedResultsItem";
const _FRIL = "FeaturedResultsItemList";
const _FRIe = "FeaturedResultsItems";
const _FRL = "FacetResultList";
const _FRS = "FeaturedResultsSet";
const _FRSI = "FeaturedResultsSetIds";
const _FRSIe = "FeaturedResultsSetId";
const _FRSN = "FeaturedResultsSetName";
const _FRSS = "FeaturedResultsSetSummary";
const _FRSSI = "FeaturedResultsSetSummaryItems";
const _FRa = "FacetResult";
const _FRai = "FailureReason";
const _FS = "FaqStatistics";
const _FSB = "FileSizeBytes";
const _FSI = "FileSystemId";
const _FSIa = "FaqSummaryItems";
const _FST = "FileSystemType";
const _FSa = "FaqSummary";
const _FT = "FeedbackToken";
const _Fa = "Facets";
const _Fac = "Facetable";
const _Fo = "Format";
const _Fr = "Freshness";
const _G = "Groups";
const _GAF = "GroupAttributeField";
const _GDC = "GoogleDriveConfiguration";
const _GHC = "GitHubConfiguration";
const _GHCCFM = "GitHubCommitConfigurationFieldMappings";
const _GHDCP = "GitHubDocumentCrawlProperties";
const _GHIACFM = "GitHubIssueAttachmentConfigurationFieldMappings";
const _GHICCFM = "GitHubIssueCommentConfigurationFieldMappings";
const _GHIDCFM = "GitHubIssueDocumentConfigurationFieldMappings";
const _GHPRCCFM = "GitHubPullRequestCommentConfigurationFieldMappings";
const _GHPRDACFM = "GitHubPullRequestDocumentAttachmentConfigurationFieldMappings";
const _GHPRDCFM = "GitHubPullRequestDocumentConfigurationFieldMappings";
const _GHRCFM = "GitHubRepositoryConfigurationFieldMappings";
const _GI = "GroupId";
const _GM = "GroupMembers";
const _GN = "GroupName";
const _GOIS = "GroupOrderingIdSummaries";
const _GOISr = "GroupOrderingIdSummary";
const _GQS = "GetQuerySuggestions";
const _GQSR = "GetQuerySuggestionsRequest";
const _GQSRe = "GetQuerySuggestionsResponse";
const _GS = "GroupSummary";
const _GSR = "GetSnapshotsRequest";
const _GSRe = "GetSnapshotsResponse";
const _GSe = "GetSnapshots";
const _GSr = "GroupsSummaries";
const _GT = "GreaterThan";
const _GTOE = "GreaterThanOrEquals";
const _H = "Host";
const _HACL = "HierarchicalAccessControlList";
const _HC = "HookConfiguration";
const _HL = "HighlightList";
const _HP = "HierarchicalPrincipal";
const _HPL = "HierarchicalPrincipalList";
const _HU = "HostUrl";
const _He = "Header";
const _Hi = "Highlight";
const _Hig = "Highlights";
const _High = "Highlighted";
const _His = "History";
const _I = "Id";
const _IAFP = "IncludeAttachmentFilePatterns";
const _IAN = "IdentityAttributeName";
const _IC = "InlineConfigurations";
const _ICDEC = "InlineCustomDocumentEnrichmentConfiguration";
const _ICDECL = "InlineCustomDocumentEnrichmentConfigurationList";
const _ICS = "IndexConfigurationSummary";
const _ICSI = "IndexConfigurationSummaryItems";
const _ICSL = "IndexConfigurationSummaryList";
const _ICn = "InvocationCondition";
const _ICt = "ItemCount";
const _IFM = "IssueFieldMappings";
const _IFN = "IndexFieldName";
const _IFNP = "InclusionFolderNamePatterns";
const _IFNPn = "InclusionFileNamePatterns";
const _IFT = "IncludeFilterTypes";
const _IFTP = "InclusionFileTypePatterns";
const _II = "IndexId";
const _IP = "InclusionPatterns";
const _IPn = "InclusionPrefixes";
const _IQAC = "IndexedQuestionAnswersCount";
const _IQSCS = "IncludeQuerySpellCheckSuggestions";
const _IQWUI = "IncludeQueriesWithoutUserInformation";
const _IRE = "InvalidRequestException";
const _IS = "IncludeSpaces";
const _ISE = "InternalServerException";
const _ISEF = "IssueSubEntityFilter";
const _ISn = "IndexStatistics";
const _ISnc = "IncludedStates";
const _IT = "IssueType";
const _ITB = "IndexedTextBytes";
const _ITDC = "IndexedTextDocumentsCount";
const _IUN = "IdentifiedUserName";
const _Im = "Importance";
const _In = "Interval";
const _Is = "Issuer";
const _JAU = "JiraAccountUrl";
const _JC = "JiraConfiguration";
const _JTTC = "JsonTokenTypeConfiguration";
const _JTTCw = "JwtTokenTypeConfiguration";
const _K = "Key";
const _KAC = "KnowledgeArticleConfiguration";
const _KKI = "KmsKeyId";
const _KL = "KeyLocation";
const _KP = "KeyPath";
const _LA = "LambdaArn";
const _LACC = "ListAccessControlConfigurations";
const _LACCR = "ListAccessControlConfigurationsRequest";
const _LACCRi = "ListAccessControlConfigurationsResponse";
const _LBP = "LookBackPeriod";
const _LC = "LanguageCode";
const _LCT = "LastClearTime";
const _LDS = "ListDataSources";
const _LDSR = "ListDataSourcesRequest";
const _LDSRi = "ListDataSourcesResponse";
const _LDSSJ = "ListDataSourceSyncJobs";
const _LDSSJR = "ListDataSourceSyncJobsRequest";
const _LDSSJRi = "ListDataSourceSyncJobsResponse";
const _LE = "ListExperiences";
const _LEE = "ListExperienceEntities";
const _LEER = "ListExperienceEntitiesRequest";
const _LEERi = "ListExperienceEntitiesResponse";
const _LEP = "ListEntityPersonas";
const _LEPR = "ListEntityPersonasRequest";
const _LEPRi = "ListEntityPersonasResponse";
const _LER = "ListExperiencesRequest";
const _LERi = "ListExperiencesResponse";
const _LF = "ListFaqs";
const _LFR = "ListFaqsRequest";
const _LFRS = "ListFeaturedResultsSets";
const _LFRSR = "ListFeaturedResultsSetsRequest";
const _LFRSRi = "ListFeaturedResultsSetsResponse";
const _LFRi = "ListFaqsResponse";
const _LGOTOI = "ListGroupsOlderThanOrderingId";
const _LGOTOIR = "ListGroupsOlderThanOrderingIdRequest";
const _LGOTOIRi = "ListGroupsOlderThanOrderingIdResponse";
const _LI = "ListIndices";
const _LIR = "ListIndicesRequest";
const _LIRi = "ListIndicesResponse";
const _LN = "LastName";
const _LOGS = "ListOfGroupSummaries";
const _LQSBL = "ListQuerySuggestionsBlockLists";
const _LQSBLR = "ListQuerySuggestionsBlockListsRequest";
const _LQSBLRi = "ListQuerySuggestionsBlockListsResponse";
const _LSBT = "LastSuggestionsBuildTime";
const _LT = "LessThan";
const _LTFR = "ListTagsForResource";
const _LTFRR = "ListTagsForResourceRequest";
const _LTFRRi = "ListTagsForResourceResponse";
const _LTOE = "LessThanOrEquals";
const _LTR = "ListThesauriRequest";
const _LTRi = "ListThesauriResponse";
const _LTi = "ListThesauri";
const _LUA = "LastUpdatedAt";
const _LUT = "LastUpdatedTimestamp";
const _LV = "LongValue";
const _M = "Message";
const _MAKS = "MissingAttributeKeyStrategy";
const _MCSPPIMB = "MaxContentSizePerPageInMegaBytes";
const _MERPI = "MaxExpandedResultsPerItem";
const _MFM = "MessageFieldMappings";
const _MG = "MemberGroups";
const _MGe = "MemberGroup";
const _MLPP = "MaxLinksPerPage";
const _MNOQU = "MinimumNumberOfQueryingUsers";
const _MQC = "MinimumQueryCount";
const _MR = "MaxResults";
const _MRITE = "MaxResultItemsToExpand";
const _MSC = "MaxSuggestionsCount";
const _MT = "MetricType";
const _MU = "MemberUsers";
const _MUPMCR = "MaxUrlsPerMinuteCrawlRate";
const _MUe = "MemberUser";
const _Me = "Metrics";
const _Mo = "Mode";
const _N = "Name";
const _NF = "NotFilter";
const _NT = "NameType";
const _NTe = "NextToken";
const _O = "Operator";
const _OAF = "OrAllFilters";
const _ODC = "OneDriveConfiguration";
const _ODU = "OneDriveUsers";
const _ODUL = "OneDriveUserList";
const _ODUSP = "OneDriveUserS3Path";
const _OI = "OrderingId";
const _OIr = "OrganizationId";
const _ON = "OrganizationName";
const _OPC = "OnPremiseConfiguration";
const _P = "Personas";
const _PC = "PageConfiguration";
const _PCF = "PrivateChannelFilter";
const _PCFu = "PublicChannelFilter";
const _PCr = "ProxyConfiguration";
const _PEHC = "PreExtractionHookConfiguration";
const _PEHCo = "PostExtractionHookConfiguration";
const _PFM = "PageFieldMappings";
const _PFMr = "ProjectFieldMappings";
const _PL = "PrincipalList";
const _PN = "PageNumber";
const _PPM = "PutPrincipalMapping";
const _PPMR = "PutPrincipalMappingRequest";
const _PS = "PersonasSummary";
const _PSL = "PersonasSummaryList";
const _PSa = "PageSize";
const _Pe = "Persona";
const _Po = "Port";
const _Pr = "Project";
const _Pri = "Principal";
const _Q = "Query";
const _QC = "QuipConfiguration";
const _QCU = "QueryCapacityUnits";
const _QI = "QueryId";
const _QIEO = "QueryIdentifiersEnclosingOption";
const _QLLBWID = "QueryLogLookBackWindowInDays";
const _QR = "QueryRequest";
const _QRI = "QueryResultItem";
const _QRIL = "QueryResultItemList";
const _QRTF = "QueryResultTypeFilter";
const _QRu = "QueryResult";
const _QSBLS = "QuerySuggestionsBlockListSummary";
const _QSBLSI = "QuerySuggestionsBlockListSummaryItems";
const _QSI = "QuerySuggestionsId";
const _QT = "QueryText";
const _QTu = "QueryTexts";
const _R = "Relevance";
const _RA = "RoleArn";
const _RAEE = "ResourceAlreadyExistException";
const _RARN = "ResourceARN";
const _RAe = "ReceivedAt";
const _RDA = "RequestedDocumentAttributes";
const _RF = "RepositoryFilter";
const _RFI = "RelevanceFeedbackItems";
const _RFL = "RelevanceFeedbackList";
const _RFe = "RelevanceFeedback";
const _RI = "ResultId";
const _RIUE = "ResourceInUseException";
const _RIe = "ResultItems";
const _RNFE = "ResourceNotFoundException";
const _RO = "RankOrder";
const _RR = "RetrieveRequest";
const _RRI = "RetrieveResultItem";
const _RRIL = "RetrieveResultItemList";
const _RRe = "RetrieveResult";
const _RUE = "ResourceUnavailableException";
const _RV = "RelevanceValue";
const _Re = "Retrieve";
const _Ro = "Rows";
const _S = "Schedule";
const _SA = "SecretArn";
const _SAc = "ScoreAttributes";
const _SAu = "SuggestionAttributes";
const _SB = "S3Bucket";
const _SC = "SortingConfigurations";
const _SCC = "SpellCorrectionConfiguration";
const _SCCe = "ServiceCatalogConfiguration";
const _SCD = "SinceCrawlDate";
const _SCFC = "SalesforceChatterFeedConfiguration";
const _SCKATC = "SalesforceCustomKnowledgeArticleTypeConfiguration";
const _SCKATCL = "SalesforceCustomKnowledgeArticleTypeConfigurationList";
const _SCL = "SuggestableConfigList";
const _SCLo = "SortingConfigurationList";
const _SCQ = "SpellCorrectedQueries";
const _SCQL = "SpellCorrectedQueryList";
const _SCQp = "SpellCorrectedQuery";
const _SCSP = "SslCertificateS3Path";
const _SCU = "StorageCapacityUnits";
const _SCa = "SalesforceConfiguration";
const _SCc = "ScoreConfidence";
const _SCl = "SlackConfiguration";
const _SCo = "S3Configuration";
const _SCor = "SortingConfiguration";
const _SCp = "SpaceConfiguration";
const _SCq = "SqlConfiguration";
const _SCu = "SuggestableConfig";
const _SD = "SnapshotsData";
const _SDH = "SnapshotsDataHeader";
const _SDR = "SnapshotsDataRecords";
const _SDSC = "S3DataSourceConfiguration";
const _SDSSJ = "StartDataSourceSyncJob";
const _SDSSJR = "StartDataSourceSyncJobRequest";
const _SDSSJRt = "StartDataSourceSyncJobResponse";
const _SDSSJRto = "StopDataSourceSyncJobRequest";
const _SDSSJt = "StopDataSourceSyncJob";
const _SDo = "SourceDocument";
const _SDou = "SourceDocuments";
const _SEL = "SlackEntityList";
const _SF = "StatusFilter";
const _SFM = "SpaceFieldMappings";
const _SFR = "SubmitFeedbackRequest";
const _SFu = "SubmitFeedback";
const _SGI = "SecurityGroupIds";
const _SH = "SuggestionHighlight";
const _SHL = "SuggestionHighlightList";
const _SI = "SiteId";
const _SIe = "SetId";
const _SIu = "SubnetIds";
const _SIum = "SummaryItems";
const _SKAC = "SalesforceKnowledgeArticleConfiguration";
const _SKATC = "StandardKnowledgeArticleTypeConfiguration";
const _SL = "SuggestionList";
const _SLV = "StringListValue";
const _SM = "SiteMaps";
const _SMA = "SecretManagerArn";
const _SMC = "SiteMapsConfiguration";
const _SN = "SetName";
const _SNBV = "ServiceNowBuildVersion";
const _SNC = "ServiceNowConfiguration";
const _SNKAC = "ServiceNowKnowledgeArticleConfiguration";
const _SNSCC = "ServiceNowServiceCatalogConfiguration";
const _SO = "SortOrder";
const _SOAC = "StandardObjectAttachmentConfiguration";
const _SOC = "StandardObjectConfigurations";
const _SP = "S3Path";
const _SPC = "SharePointConfiguration";
const _SPGM = "S3PathforGroupMembers";
const _SPV = "SharePointVersion";
const _SPr = "S3Prefix";
const _SQEE = "ServiceQuotaExceededException";
const _SQT = "SuggestedQueryText";
const _SRC = "SynonymRuleCount";
const _SSC = "SaaSConfiguration";
const _SSEC = "ServerSideEncryptionConfiguration";
const _SSKATC = "SalesforceStandardKnowledgeArticleTypeConfiguration";
const _SSOAC = "SalesforceStandardObjectAttachmentConfiguration";
const _SSOC = "SalesforceStandardObjectConfiguration";
const _SSOCL = "SalesforceStandardObjectConfigurationList";
const _SSP = "SourceS3Path";
const _SSTF = "SnapShotTimeFilter";
const _ST = "StartTime";
const _STF = "StartTimeFilter";
const _STWH = "SuggestionTextWithHighlights";
const _STu = "SuggestionTypes";
const _SU = "SiteUrl";
const _SUC = "SeedUrlConfiguration";
const _SUe = "ServerUrl";
const _SUee = "SeedUrls";
const _SV = "StringValue";
const _SVu = "SuggestionValue";
const _Se = "Search";
const _Sea = "Searchable";
const _So = "Sortable";
const _St = "Status";
const _Su = "Suggestions";
const _Sug = "Suggestable";
const _Sugg = "Suggestion";
const _T = "Term";
const _TA = "TopAnswer";
const _TC = "TemplateConfiguration";
const _TCL = "TableCellList";
const _TCa = "TableCell";
const _TCe = "TermCount";
const _TD = "TenantDomain";
const _TDAK = "TargetDocumentAttributeKey";
const _TDAV = "TargetDocumentAttributeValue";
const _TDAVD = "TargetDocumentAttributeValueDeletion";
const _TDS = "TextDocumentStatistics";
const _TE = "TableExcerpt";
const _TEh = "ThrottlingException";
const _TFM = "TaskFieldMappings";
const _TFMh = "ThreadFieldMappings";
const _TI = "TeamId";
const _TK = "TagKeys";
const _TL = "TagList";
const _TN = "TableName";
const _TNOR = "TotalNumberOfResults";
const _TNORo = "TotalNumberOfRows";
const _TR = "TableRow";
const _TRL = "TableRowList";
const _TRR = "TagResourceRequest";
const _TRRa = "TagResourceResponse";
const _TRa = "TagResource";
const _TRi = "TimeRange";
const _TS = "ThesaurusSummary";
const _TSC = "TotalSuggestionsCount";
const _TSI = "ThesaurusSummaryItems";
const _TWH = "TextWithHighlights";
const _TWHV = "TextWithHighlightsValue";
const _Ta = "Tags";
const _Tag = "Tag";
const _Tar = "Target";
const _Te = "Text";
const _Tem = "Template";
const _Ti = "Title";
const _To = "Token";
const _Ty = "Type";
const _U = "Urls";
const _UA = "UpdatedAt";
const _UACC = "UpdateAccessControlConfiguration";
const _UACCR = "UpdateAccessControlConfigurationRequest";
const _UACCRp = "UpdateAccessControlConfigurationResponse";
const _UC = "UserContext";
const _UCL = "UseChangeLog";
const _UCP = "UserContextPolicy";
const _UDS = "UpdateDataSource";
const _UDSR = "UpdateDataSourceRequest";
const _UE = "UpdateExperience";
const _UEP = "UrlExclusionPatterns";
const _UER = "UpdateExperienceRequest";
const _UFRS = "UpdateFeaturedResultsSet";
const _UFRSR = "UpdateFeaturedResultsSetRequest";
const _UFRSRp = "UpdateFeaturedResultsSetResponse";
const _UGRC = "UserGroupResolutionConfiguration";
const _UGRM = "UserGroupResolutionMode";
const _UI = "UserId";
const _UIC = "UserIdentityConfiguration";
const _UIP = "UrlInclusionPatterns";
const _UIR = "UpdateIndexRequest";
const _UIp = "UpdateIndex";
const _UN = "UserName";
const _UNAF = "UserNameAttributeField";
const _UQSBL = "UpdateQuerySuggestionsBlockList";
const _UQSBLR = "UpdateQuerySuggestionsBlockListRequest";
const _UQSC = "UpdateQuerySuggestionsConfig";
const _UQSCR = "UpdateQuerySuggestionsConfigRequest";
const _UR = "UntagResource";
const _URI = "URI";
const _URL = "URL";
const _URR = "UntagResourceRequest";
const _URRn = "UntagResourceResponse";
const _UT = "UpdateThesaurus";
const _UTC = "UserTokenConfigurations";
const _UTCL = "UserTokenConfigurationList";
const _UTCs = "UserTokenConfiguration";
const _UTR = "UpdateThesaurusRequest";
const _V = "Value";
const _VC = "VpcConfiguration";
const _VE = "ValidationException";
const _VI = "VisitorId";
const _VIM = "ValueImportanceMap";
const _VT = "ValueType";
const _Ve = "Version";
const _W = "Warnings";
const _WCC = "WebCrawlerConfiguration";
const _WCM = "WebCrawlerMode";
const _WDC = "WorkDocsConfiguration";
const _WFM = "WikiFieldMappings";
const _WL = "WarningList";
const _WLFM = "WebLinkFieldMappings";
const _WLFMo = "WorkLogFieldMappings";
const _Wa = "Warning";
const _c = "client";
const _e = "error";
const _hE = "httpError";
const _s = "server";
const _sm = "smithy.ts.sdk.synthetic.com.amazonaws.kendra";
const n0 = "com.amazonaws.kendra";

// smithy-typescript generated code
import { TypeRegistry } from "@smithy/core/schema";
import type {
  StaticErrorSchema,
  StaticListSchema,
  StaticMapSchema,
  StaticOperationSchema,
  StaticSimpleSchema,
  StaticStructureSchema,
} from "@smithy/types";

import {
  AccessDeniedException,
  ConflictException,
  FeaturedResultsConflictException,
  InternalServerException,
  InvalidRequestException,
  ResourceAlreadyExistException,
  ResourceInUseException,
  ResourceNotFoundException,
  ResourceUnavailableException,
  ServiceQuotaExceededException,
  ThrottlingException,
  ValidationException,
} from "../models/errors";
import { KendraServiceException } from "../models/KendraServiceException";

/* eslint no-var: 0 */
var KmsKeyId: StaticSimpleSchema = [0, n0, _KKI, 8, 0];
var NameType: StaticSimpleSchema = [0, n0, _NT, 8, 0];
export var AccessControlConfigurationSummary$: StaticStructureSchema = [3, n0, _ACCS,
  0,
  [_I],
  [0]
];
export var AccessControlListConfiguration$: StaticStructureSchema = [3, n0, _ACLC,
  0,
  [_KP],
  [0]
];
export var AccessDeniedException$: StaticErrorSchema = [-3, n0, _ADE,
  { [_e]: _c, [_hE]: 403 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(AccessDeniedException$, AccessDeniedException);
export var AclConfiguration$: StaticStructureSchema = [3, n0, _AC,
  0,
  [_AGCN],
  [0]
];
export var AdditionalResultAttribute$: StaticStructureSchema = [3, n0, _ARA,
  0,
  [_K, _VT, _V],
  [0, 0, () => AdditionalResultAttributeValue$]
];
export var AdditionalResultAttributeValue$: StaticStructureSchema = [3, n0, _ARAV,
  0,
  [_TWHV],
  [() => TextWithHighlights$]
];
export var AlfrescoConfiguration$: StaticStructureSchema = [3, n0, _ACl,
  0,
  [_SU, _SI, _SA, _SCSP, _CSF, _CC, _EF, _DLFM, _BFM, _WFM, _IP, _EP, _VC],
  [0, 0, 0, () => S3Path$, 2, 2, 64 | 0, () => DataSourceToIndexFieldMappingList, () => DataSourceToIndexFieldMappingList, () => DataSourceToIndexFieldMappingList, 64 | 0, 64 | 0, () => DataSourceVpcConfiguration$]
];
export var AssociateEntitiesToExperienceRequest$: StaticStructureSchema = [3, n0, _AETER,
  0,
  [_I, _II, _EL],
  [0, 0, () => AssociateEntityList]
];
export var AssociateEntitiesToExperienceResponse$: StaticStructureSchema = [3, n0, _AETERs,
  0,
  [_FEL],
  [() => AssociateEntitiesToExperienceFailedEntityList]
];
export var AssociatePersonasToEntitiesRequest$: StaticStructureSchema = [3, n0, _APTER,
  0,
  [_I, _II, _P],
  [0, 0, () => EntityPersonaConfigurationList]
];
export var AssociatePersonasToEntitiesResponse$: StaticStructureSchema = [3, n0, _APTERs,
  0,
  [_FEL],
  [() => FailedEntityList]
];
export var AttributeFilter$: StaticStructureSchema = [3, n0, _AF,
  0,
  [_AAF, _OAF, _NF, _ET, _CA, _CAo, _GT, _GTOE, _LT, _LTOE],
  [() => AttributeFilterList, () => AttributeFilterList, () => AttributeFilter$, () => DocumentAttribute$, () => DocumentAttribute$, () => DocumentAttribute$, () => DocumentAttribute$, () => DocumentAttribute$, () => DocumentAttribute$, () => DocumentAttribute$]
];
export var AttributeSuggestionsDescribeConfig$: StaticStructureSchema = [3, n0, _ASDC,
  0,
  [_SCL, _ASM],
  [() => SuggestableConfigList, 0]
];
export var AttributeSuggestionsGetConfig$: StaticStructureSchema = [3, n0, _ASGC,
  0,
  [_SAu, _ARAd, _AF, _UC],
  [64 | 0, 64 | 0, () => AttributeFilter$, () => UserContext$]
];
export var AttributeSuggestionsUpdateConfig$: StaticStructureSchema = [3, n0, _ASUC,
  0,
  [_SCL, _ASM],
  [() => SuggestableConfigList, 0]
];
export var AuthenticationConfiguration$: StaticStructureSchema = [3, n0, _ACu,
  0,
  [_BA],
  [() => BasicAuthenticationConfigurationList]
];
export var BasicAuthenticationConfiguration$: StaticStructureSchema = [3, n0, _BAC,
  0,
  [_H, _Po, _C],
  [0, 1, 0]
];
export var BatchDeleteDocumentRequest$: StaticStructureSchema = [3, n0, _BDDR,
  0,
  [_II, _DIL, _DSSJMT],
  [0, 64 | 0, () => DataSourceSyncJobMetricTarget$]
];
export var BatchDeleteDocumentResponse$: StaticStructureSchema = [3, n0, _BDDRa,
  0,
  [_FD],
  [() => BatchDeleteDocumentResponseFailedDocuments]
];
export var BatchDeleteDocumentResponseFailedDocument$: StaticStructureSchema = [3, n0, _BDDRFD,
  0,
  [_I, _DSI, _EC, _EM],
  [0, 0, 0, 0]
];
export var BatchDeleteFeaturedResultsSetError$: StaticStructureSchema = [3, n0, _BDFRSE,
  0,
  [_I, _EC, _EM],
  [0, 0, 0]
];
export var BatchDeleteFeaturedResultsSetRequest$: StaticStructureSchema = [3, n0, _BDFRSR,
  0,
  [_II, _FRSI],
  [0, 64 | 0]
];
export var BatchDeleteFeaturedResultsSetResponse$: StaticStructureSchema = [3, n0, _BDFRSRa,
  0,
  [_E],
  [() => BatchDeleteFeaturedResultsSetErrors]
];
export var BatchGetDocumentStatusRequest$: StaticStructureSchema = [3, n0, _BGDSR,
  0,
  [_II, _DILo],
  [0, () => DocumentInfoList]
];
export var BatchGetDocumentStatusResponse$: StaticStructureSchema = [3, n0, _BGDSRa,
  0,
  [_E, _DSL],
  [() => BatchGetDocumentStatusResponseErrors, () => DocumentStatusList]
];
export var BatchGetDocumentStatusResponseError$: StaticStructureSchema = [3, n0, _BGDSRE,
  0,
  [_DI, _DSI, _EC, _EM],
  [0, 0, 0, 0]
];
export var BatchPutDocumentRequest$: StaticStructureSchema = [3, n0, _BPDR,
  0,
  [_II, _RA, _D, _CDEC],
  [0, 0, () => DocumentList, () => CustomDocumentEnrichmentConfiguration$]
];
export var BatchPutDocumentResponse$: StaticStructureSchema = [3, n0, _BPDRa,
  0,
  [_FD],
  [() => BatchPutDocumentResponseFailedDocuments]
];
export var BatchPutDocumentResponseFailedDocument$: StaticStructureSchema = [3, n0, _BPDRFD,
  0,
  [_I, _DSI, _EC, _EM],
  [0, 0, 0, 0]
];
export var BoxConfiguration$: StaticStructureSchema = [3, n0, _BC,
  0,
  [_EI, _SA, _UCL, _CC, _CT, _CWL, _FFM, _TFM, _CFM, _WLFM, _IP, _EP, _VC],
  [0, 0, 2, 2, 2, 2, () => DataSourceToIndexFieldMappingList, () => DataSourceToIndexFieldMappingList, () => DataSourceToIndexFieldMappingList, () => DataSourceToIndexFieldMappingList, 64 | 0, 64 | 0, () => DataSourceVpcConfiguration$]
];
export var CapacityUnitsConfiguration$: StaticStructureSchema = [3, n0, _CUC,
  0,
  [_SCU, _QCU],
  [1, 1]
];
export var ClearQuerySuggestionsRequest$: StaticStructureSchema = [3, n0, _CQSR,
  0,
  [_II],
  [0]
];
export var ClickFeedback$: StaticStructureSchema = [3, n0, _CF,
  0,
  [_RI, _CTl],
  [0, 4]
];
export var CollapseConfiguration$: StaticStructureSchema = [3, n0, _CCo,
  0,
  [_DAK, _SC, _MAKS, _Ex, _ECx],
  [0, () => SortingConfigurationList, 0, 2, () => ExpandConfiguration$]
];
export var CollapsedResultDetail$: StaticStructureSchema = [3, n0, _CRD,
  0,
  [_DA, _ER],
  [() => DocumentAttribute$, () => ExpandedResultList]
];
export var ColumnConfiguration$: StaticStructureSchema = [3, n0, _CCol,
  0,
  [_DICN, _DDCN, _DTCN, _FM, _CDC],
  [0, 0, 0, () => DataSourceToIndexFieldMappingList, 64 | 0]
];
export var ConflictException$: StaticErrorSchema = [-3, n0, _CE,
  { [_e]: _c, [_hE]: 409 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ConflictException$, ConflictException);
export var ConflictingItem$: StaticStructureSchema = [3, n0, _CI,
  0,
  [_QT, _SN, _SIe],
  [0, 0, 0]
];
export var ConfluenceAttachmentConfiguration$: StaticStructureSchema = [3, n0, _CAC,
  0,
  [_CAr, _AFM],
  [2, () => ConfluenceAttachmentFieldMappingsList]
];
export var ConfluenceAttachmentToIndexFieldMapping$: StaticStructureSchema = [3, n0, _CATIFM,
  0,
  [_DSFN, _DFF, _IFN],
  [0, 0, 0]
];
export var ConfluenceBlogConfiguration$: StaticStructureSchema = [3, n0, _CBC,
  0,
  [_BFM],
  [() => ConfluenceBlogFieldMappingsList]
];
export var ConfluenceBlogToIndexFieldMapping$: StaticStructureSchema = [3, n0, _CBTIFM,
  0,
  [_DSFN, _DFF, _IFN],
  [0, 0, 0]
];
export var ConfluenceConfiguration$: StaticStructureSchema = [3, n0, _CCon,
  0,
  [_SUe, _SA, _Ve, _SCp, _PC, _BCl, _ACt, _VC, _IP, _EP, _PCr, _AT],
  [0, 0, 0, () => ConfluenceSpaceConfiguration$, () => ConfluencePageConfiguration$, () => ConfluenceBlogConfiguration$, () => ConfluenceAttachmentConfiguration$, () => DataSourceVpcConfiguration$, 64 | 0, 64 | 0, () => ProxyConfiguration$, 0]
];
export var ConfluencePageConfiguration$: StaticStructureSchema = [3, n0, _CPC,
  0,
  [_PFM],
  [() => ConfluencePageFieldMappingsList]
];
export var ConfluencePageToIndexFieldMapping$: StaticStructureSchema = [3, n0, _CPTIFM,
  0,
  [_DSFN, _DFF, _IFN],
  [0, 0, 0]
];
export var ConfluenceSpaceConfiguration$: StaticStructureSchema = [3, n0, _CSC,
  0,
  [_CPS, _CAS, _IS, _ES, _SFM],
  [2, 2, 64 | 0, 64 | 0, () => ConfluenceSpaceFieldMappingsList]
];
export var ConfluenceSpaceToIndexFieldMapping$: StaticStructureSchema = [3, n0, _CSTIFM,
  0,
  [_DSFN, _DFF, _IFN],
  [0, 0, 0]
];
export var ConnectionConfiguration$: StaticStructureSchema = [3, n0, _CConn,
  0,
  [_DH, _DP, _DN, _TN, _SA],
  [0, 1, 0, 0, 0]
];
export var ContentSourceConfiguration$: StaticStructureSchema = [3, n0, _CSCo,
  0,
  [_DSIa, _FI, _DPC],
  [64 | 0, 64 | 0, 2]
];
export var Correction$: StaticStructureSchema = [3, n0, _Co,
  0,
  [_BO, _EO, _T, _CTo],
  [1, 1, 0, 0]
];
export var CreateAccessControlConfigurationRequest$: StaticStructureSchema = [3, n0, _CACCR,
  0,
  [_II, _N, _De, _ACL, _HACL, _CTli],
  [0, 0, 0, () => PrincipalList, () => HierarchicalPrincipalList, [0, 4]]
];
export var CreateAccessControlConfigurationResponse$: StaticStructureSchema = [3, n0, _CACCRr,
  0,
  [_I],
  [0]
];
export var CreateDataSourceRequest$: StaticStructureSchema = [3, n0, _CDSR,
  0,
  [_N, _II, _Ty, _Con, _VC, _De, _S, _RA, _Ta, _CTli, _LC, _CDEC],
  [0, 0, 0, () => DataSourceConfiguration$, () => DataSourceVpcConfiguration$, 0, 0, 0, () => TagList, [0, 4], 0, () => CustomDocumentEnrichmentConfiguration$]
];
export var CreateDataSourceResponse$: StaticStructureSchema = [3, n0, _CDSRr,
  0,
  [_I],
  [0]
];
export var CreateExperienceRequest$: StaticStructureSchema = [3, n0, _CER,
  0,
  [_N, _II, _RA, _Con, _De, _CTli],
  [0, 0, 0, () => ExperienceConfiguration$, 0, [0, 4]]
];
export var CreateExperienceResponse$: StaticStructureSchema = [3, n0, _CERr,
  0,
  [_I],
  [0]
];
export var CreateFaqRequest$: StaticStructureSchema = [3, n0, _CFR,
  0,
  [_II, _N, _De, _SP, _RA, _Ta, _FF, _CTli, _LC],
  [0, 0, 0, () => S3Path$, 0, () => TagList, 0, [0, 4], 0]
];
export var CreateFaqResponse$: StaticStructureSchema = [3, n0, _CFRr,
  0,
  [_I],
  [0]
];
export var CreateFeaturedResultsSetRequest$: StaticStructureSchema = [3, n0, _CFRSR,
  0,
  [_II, _FRSN, _De, _CTli, _St, _QTu, _FDe, _Ta],
  [0, 0, 0, 0, 0, 64 | 0, () => FeaturedDocumentList, () => TagList]
];
export var CreateFeaturedResultsSetResponse$: StaticStructureSchema = [3, n0, _CFRSRr,
  0,
  [_FRS],
  [() => FeaturedResultsSet$]
];
export var CreateIndexRequest$: StaticStructureSchema = [3, n0, _CIR,
  0,
  [_N, _Ed, _RA, _SSEC, _De, _CTli, _Ta, _UTC, _UCP, _UGRC],
  [0, 0, 0, [() => ServerSideEncryptionConfiguration$, 0], 0, [0, 4], () => TagList, () => UserTokenConfigurationList, 0, () => UserGroupResolutionConfiguration$]
];
export var CreateIndexResponse$: StaticStructureSchema = [3, n0, _CIRr,
  0,
  [_I],
  [0]
];
export var CreateQuerySuggestionsBlockListRequest$: StaticStructureSchema = [3, n0, _CQSBLR,
  0,
  [_II, _N, _De, _SSP, _CTli, _RA, _Ta],
  [0, 0, 0, () => S3Path$, [0, 4], 0, () => TagList]
];
export var CreateQuerySuggestionsBlockListResponse$: StaticStructureSchema = [3, n0, _CQSBLRr,
  0,
  [_I],
  [0]
];
export var CreateThesaurusRequest$: StaticStructureSchema = [3, n0, _CTR,
  0,
  [_II, _N, _De, _RA, _Ta, _SSP, _CTli],
  [0, 0, 0, 0, () => TagList, () => S3Path$, [0, 4]]
];
export var CreateThesaurusResponse$: StaticStructureSchema = [3, n0, _CTRr,
  0,
  [_I],
  [0]
];
export var CustomDocumentEnrichmentConfiguration$: StaticStructureSchema = [3, n0, _CDEC,
  0,
  [_IC, _PEHC, _PEHCo, _RA],
  [() => InlineCustomDocumentEnrichmentConfigurationList, () => HookConfiguration$, () => HookConfiguration$, 0]
];
export var DatabaseConfiguration$: StaticStructureSchema = [3, n0, _DC,
  0,
  [_DET, _CConn, _VC, _CCol, _AC, _SCq],
  [0, () => ConnectionConfiguration$, () => DataSourceVpcConfiguration$, () => ColumnConfiguration$, () => AclConfiguration$, () => SqlConfiguration$]
];
export var DataSourceConfiguration$: StaticStructureSchema = [3, n0, _DSC,
  0,
  [_SCo, _SPC, _DC, _SCa, _ODC, _SNC, _CCon, _GDC, _WCC, _WDC, _FC, _SCl, _BC, _QC, _JC, _GHC, _ACl, _TC],
  [() => S3DataSourceConfiguration$, () => SharePointConfiguration$, () => DatabaseConfiguration$, () => SalesforceConfiguration$, () => OneDriveConfiguration$, () => ServiceNowConfiguration$, () => ConfluenceConfiguration$, () => GoogleDriveConfiguration$, () => WebCrawlerConfiguration$, () => WorkDocsConfiguration$, () => FsxConfiguration$, () => SlackConfiguration$, () => BoxConfiguration$, () => QuipConfiguration$, () => JiraConfiguration$, () => GitHubConfiguration$, () => AlfrescoConfiguration$, () => TemplateConfiguration$]
];
export var DataSourceGroup$: StaticStructureSchema = [3, n0, _DSG,
  0,
  [_GI, _DSI],
  [0, 0]
];
export var DataSourceSummary$: StaticStructureSchema = [3, n0, _DSS,
  0,
  [_N, _I, _Ty, _CAre, _UA, _St, _LC],
  [0, 0, 0, 4, 4, 0, 0]
];
export var DataSourceSyncJob$: StaticStructureSchema = [3, n0, _DSSJ,
  0,
  [_EIx, _ST, _ETn, _St, _EM, _EC, _DSEC, _Me],
  [0, 4, 4, 0, 0, 0, 0, () => DataSourceSyncJobMetrics$]
];
export var DataSourceSyncJobMetrics$: StaticStructureSchema = [3, n0, _DSSJM,
  0,
  [_DAo, _DM, _DD, _DF, _DS],
  [0, 0, 0, 0, 0]
];
export var DataSourceSyncJobMetricTarget$: StaticStructureSchema = [3, n0, _DSSJMT,
  0,
  [_DSI, _DSSJI],
  [0, 0]
];
export var DataSourceToIndexFieldMapping$: StaticStructureSchema = [3, n0, _DSTIFM,
  0,
  [_DSFN, _DFF, _IFN],
  [0, 0, 0]
];
export var DataSourceVpcConfiguration$: StaticStructureSchema = [3, n0, _DSVC,
  0,
  [_SIu, _SGI],
  [64 | 0, 64 | 0]
];
export var DeleteAccessControlConfigurationRequest$: StaticStructureSchema = [3, n0, _DACCR,
  0,
  [_II, _I],
  [0, 0]
];
export var DeleteAccessControlConfigurationResponse$: StaticStructureSchema = [3, n0, _DACCRe,
  0,
  [],
  []
];
export var DeleteDataSourceRequest$: StaticStructureSchema = [3, n0, _DDSR,
  0,
  [_I, _II],
  [0, 0]
];
export var DeleteExperienceRequest$: StaticStructureSchema = [3, n0, _DER,
  0,
  [_I, _II],
  [0, 0]
];
export var DeleteExperienceResponse$: StaticStructureSchema = [3, n0, _DERe,
  0,
  [],
  []
];
export var DeleteFaqRequest$: StaticStructureSchema = [3, n0, _DFR,
  0,
  [_I, _II],
  [0, 0]
];
export var DeleteIndexRequest$: StaticStructureSchema = [3, n0, _DIR,
  0,
  [_I],
  [0]
];
export var DeletePrincipalMappingRequest$: StaticStructureSchema = [3, n0, _DPMR,
  0,
  [_II, _DSI, _GI, _OI],
  [0, 0, 0, 1]
];
export var DeleteQuerySuggestionsBlockListRequest$: StaticStructureSchema = [3, n0, _DQSBLR,
  0,
  [_II, _I],
  [0, 0]
];
export var DeleteThesaurusRequest$: StaticStructureSchema = [3, n0, _DTR,
  0,
  [_I, _II],
  [0, 0]
];
export var DescribeAccessControlConfigurationRequest$: StaticStructureSchema = [3, n0, _DACCRes,
  0,
  [_II, _I],
  [0, 0]
];
export var DescribeAccessControlConfigurationResponse$: StaticStructureSchema = [3, n0, _DACCResc,
  0,
  [_N, _De, _EM, _ACL, _HACL],
  [0, 0, 0, () => PrincipalList, () => HierarchicalPrincipalList]
];
export var DescribeDataSourceRequest$: StaticStructureSchema = [3, n0, _DDSRe,
  0,
  [_I, _II],
  [0, 0]
];
export var DescribeDataSourceResponse$: StaticStructureSchema = [3, n0, _DDSRes,
  0,
  [_I, _II, _N, _Ty, _Con, _VC, _CAre, _UA, _De, _St, _S, _RA, _EM, _LC, _CDEC],
  [0, 0, 0, 0, () => DataSourceConfiguration$, () => DataSourceVpcConfiguration$, 4, 4, 0, 0, 0, 0, 0, 0, () => CustomDocumentEnrichmentConfiguration$]
];
export var DescribeExperienceRequest$: StaticStructureSchema = [3, n0, _DERes,
  0,
  [_I, _II],
  [0, 0]
];
export var DescribeExperienceResponse$: StaticStructureSchema = [3, n0, _DEResc,
  0,
  [_I, _II, _N, _En, _Con, _CAre, _UA, _De, _St, _RA, _EM],
  [0, 0, 0, () => ExperienceEndpoints, () => ExperienceConfiguration$, 4, 4, 0, 0, 0, 0]
];
export var DescribeFaqRequest$: StaticStructureSchema = [3, n0, _DFRe,
  0,
  [_I, _II],
  [0, 0]
];
export var DescribeFaqResponse$: StaticStructureSchema = [3, n0, _DFRes,
  0,
  [_I, _II, _N, _De, _CAre, _UA, _SP, _St, _RA, _EM, _FF, _LC],
  [0, 0, 0, 0, 4, 4, () => S3Path$, 0, 0, 0, 0, 0]
];
export var DescribeFeaturedResultsSetRequest$: StaticStructureSchema = [3, n0, _DFRSR,
  0,
  [_II, _FRSIe],
  [0, 0]
];
export var DescribeFeaturedResultsSetResponse$: StaticStructureSchema = [3, n0, _DFRSRe,
  0,
  [_FRSIe, _FRSN, _De, _St, _QTu, _FDWM, _FDM, _LUT, _CTr],
  [0, 0, 0, 0, 64 | 0, () => FeaturedDocumentWithMetadataList, () => FeaturedDocumentMissingList, 1, 1]
];
export var DescribeIndexRequest$: StaticStructureSchema = [3, n0, _DIRe,
  0,
  [_I],
  [0]
];
export var DescribeIndexResponse$: StaticStructureSchema = [3, n0, _DIRes,
  0,
  [_N, _I, _Ed, _RA, _SSEC, _St, _De, _CAre, _UA, _DMC, _ISn, _EM, _CU, _UTC, _UCP, _UGRC],
  [0, 0, 0, 0, [() => ServerSideEncryptionConfiguration$, 0], 0, 0, 4, 4, () => DocumentMetadataConfigurationList, () => IndexStatistics$, 0, () => CapacityUnitsConfiguration$, () => UserTokenConfigurationList, 0, () => UserGroupResolutionConfiguration$]
];
export var DescribePrincipalMappingRequest$: StaticStructureSchema = [3, n0, _DPMRe,
  0,
  [_II, _DSI, _GI],
  [0, 0, 0]
];
export var DescribePrincipalMappingResponse$: StaticStructureSchema = [3, n0, _DPMRes,
  0,
  [_II, _DSI, _GI, _GOIS],
  [0, 0, 0, () => GroupOrderingIdSummaries]
];
export var DescribeQuerySuggestionsBlockListRequest$: StaticStructureSchema = [3, n0, _DQSBLRe,
  0,
  [_II, _I],
  [0, 0]
];
export var DescribeQuerySuggestionsBlockListResponse$: StaticStructureSchema = [3, n0, _DQSBLRes,
  0,
  [_II, _I, _N, _De, _St, _EM, _CAre, _UA, _SSP, _ICt, _FSB, _RA],
  [0, 0, 0, 0, 0, 0, 4, 4, () => S3Path$, 1, 1, 0]
];
export var DescribeQuerySuggestionsConfigRequest$: StaticStructureSchema = [3, n0, _DQSCR,
  0,
  [_II],
  [0]
];
export var DescribeQuerySuggestionsConfigResponse$: StaticStructureSchema = [3, n0, _DQSCRe,
  0,
  [_Mo, _St, _QLLBWID, _IQWUI, _MNOQU, _MQC, _LSBT, _LCT, _TSC, _ASC],
  [0, 0, 1, 2, 1, 1, 4, 4, 1, () => AttributeSuggestionsDescribeConfig$]
];
export var DescribeThesaurusRequest$: StaticStructureSchema = [3, n0, _DTRe,
  0,
  [_I, _II],
  [0, 0]
];
export var DescribeThesaurusResponse$: StaticStructureSchema = [3, n0, _DTRes,
  0,
  [_I, _II, _N, _De, _St, _EM, _CAre, _UA, _RA, _SSP, _FSB, _TCe, _SRC],
  [0, 0, 0, 0, 0, 0, 4, 4, 0, () => S3Path$, 1, 1, 1]
];
export var DisassociateEntitiesFromExperienceRequest$: StaticStructureSchema = [3, n0, _DEFER,
  0,
  [_I, _II, _EL],
  [0, 0, () => DisassociateEntityList]
];
export var DisassociateEntitiesFromExperienceResponse$: StaticStructureSchema = [3, n0, _DEFERi,
  0,
  [_FEL],
  [() => FailedEntityList]
];
export var DisassociatePersonasFromEntitiesRequest$: StaticStructureSchema = [3, n0, _DPFER,
  0,
  [_I, _II, _EIn],
  [0, 0, 64 | 0]
];
export var DisassociatePersonasFromEntitiesResponse$: StaticStructureSchema = [3, n0, _DPFERi,
  0,
  [_FEL],
  [() => FailedEntityList]
];
export var Document$: StaticStructureSchema = [3, n0, _Do,
  0,
  [_I, _Ti, _B, _SP, _A, _ACL, _HACL, _CTon, _ACCI],
  [0, 0, 21, () => S3Path$, () => DocumentAttributeList, () => PrincipalList, () => HierarchicalPrincipalList, 0, 0]
];
export var DocumentAttribute$: StaticStructureSchema = [3, n0, _DA,
  0,
  [_K, _V],
  [0, () => DocumentAttributeValue$]
];
export var DocumentAttributeCondition$: StaticStructureSchema = [3, n0, _DAC,
  0,
  [_CDAK, _O, _COV],
  [0, 0, () => DocumentAttributeValue$]
];
export var DocumentAttributeTarget$: StaticStructureSchema = [3, n0, _DAT,
  0,
  [_TDAK, _TDAVD, _TDAV],
  [0, 2, () => DocumentAttributeValue$]
];
export var DocumentAttributeValue$: StaticStructureSchema = [3, n0, _DAV,
  0,
  [_SV, _SLV, _LV, _DV],
  [0, 64 | 0, 1, 4]
];
export var DocumentAttributeValueCountPair$: StaticStructureSchema = [3, n0, _DAVCP,
  0,
  [_DAV, _Cou, _FR],
  [() => DocumentAttributeValue$, 1, () => FacetResultList]
];
export var DocumentInfo$: StaticStructureSchema = [3, n0, _DIo,
  0,
  [_DI, _A],
  [0, () => DocumentAttributeList]
];
export var DocumentMetadataConfiguration$: StaticStructureSchema = [3, n0, _DMCo,
  0,
  [_N, _Ty, _R, _Se],
  [0, 0, () => Relevance$, () => Search$]
];
export var DocumentRelevanceConfiguration$: StaticStructureSchema = [3, n0, _DRC,
  0,
  [_N, _R],
  [0, () => Relevance$]
];
export var DocumentsMetadataConfiguration$: StaticStructureSchema = [3, n0, _DMCoc,
  0,
  [_SPr],
  [0]
];
export var EntityConfiguration$: StaticStructureSchema = [3, n0, _ECn,
  0,
  [_EInt, _ETnt],
  [0, 0]
];
export var EntityDisplayData$: StaticStructureSchema = [3, n0, _EDD,
  0,
  [_UN, _GN, _IUN, _FN, _LN],
  [[() => NameType, 0], [() => NameType, 0], [() => NameType, 0], [() => NameType, 0], [() => NameType, 0]]
];
export var EntityPersonaConfiguration$: StaticStructureSchema = [3, n0, _EPC,
  0,
  [_EInt, _Pe],
  [0, 0]
];
export var ExpandConfiguration$: StaticStructureSchema = [3, n0, _ECx,
  0,
  [_MRITE, _MERPI],
  [1, 1]
];
export var ExpandedResultItem$: StaticStructureSchema = [3, n0, _ERI,
  0,
  [_I, _DI, _DT, _DE, _DURI, _DAoc],
  [0, 0, () => TextWithHighlights$, () => TextWithHighlights$, 0, () => DocumentAttributeList]
];
export var ExperienceConfiguration$: StaticStructureSchema = [3, n0, _ECxp,
  0,
  [_CSCo, _UIC],
  [() => ContentSourceConfiguration$, () => UserIdentityConfiguration$]
];
export var ExperienceEndpoint$: StaticStructureSchema = [3, n0, _EE,
  0,
  [_ETnd, _End],
  [0, 0]
];
export var ExperienceEntitiesSummary$: StaticStructureSchema = [3, n0, _EES,
  0,
  [_EInt, _ETnt, _DDi],
  [0, 0, [() => EntityDisplayData$, 0]]
];
export var ExperiencesSummary$: StaticStructureSchema = [3, n0, _ESx,
  0,
  [_N, _I, _CAre, _St, _En],
  [0, 0, 4, 0, () => ExperienceEndpoints]
];
export var Facet$: StaticStructureSchema = [3, n0, _F,
  0,
  [_DAK, _Fa, _MR],
  [0, () => FacetList, 1]
];
export var FacetResult$: StaticStructureSchema = [3, n0, _FRa,
  0,
  [_DAK, _DAVT, _DAVCPo],
  [0, 0, () => DocumentAttributeValueCountPairList]
];
export var FailedEntity$: StaticStructureSchema = [3, n0, _FE,
  0,
  [_EInt, _EM],
  [0, 0]
];
export var FaqStatistics$: StaticStructureSchema = [3, n0, _FS,
  0,
  [_IQAC],
  [1]
];
export var FaqSummary$: StaticStructureSchema = [3, n0, _FSa,
  0,
  [_I, _N, _St, _CAre, _UA, _FF, _LC],
  [0, 0, 0, 4, 4, 0, 0]
];
export var FeaturedDocument$: StaticStructureSchema = [3, n0, _FDea,
  0,
  [_I],
  [0]
];
export var FeaturedDocumentMissing$: StaticStructureSchema = [3, n0, _FDMe,
  0,
  [_I],
  [0]
];
export var FeaturedDocumentWithMetadata$: StaticStructureSchema = [3, n0, _FDWMe,
  0,
  [_I, _Ti, _URI],
  [0, 0, 0]
];
export var FeaturedResultsConflictException$: StaticErrorSchema = [-3, n0, _FRCE,
  { [_e]: _c, [_hE]: 409 },
  [_M, _CIo],
  [0, () => ConflictingItems]
];
TypeRegistry.for(n0).registerError(FeaturedResultsConflictException$, FeaturedResultsConflictException);
export var FeaturedResultsItem$: StaticStructureSchema = [3, n0, _FRI,
  0,
  [_I, _Ty, _AA, _DI, _DT, _DE, _DURI, _DAoc, _FT],
  [0, 0, () => AdditionalResultAttributeList, 0, () => TextWithHighlights$, () => TextWithHighlights$, 0, () => DocumentAttributeList, 0]
];
export var FeaturedResultsSet$: StaticStructureSchema = [3, n0, _FRS,
  0,
  [_FRSIe, _FRSN, _De, _St, _QTu, _FDe, _LUT, _CTr],
  [0, 0, 0, 0, 64 | 0, () => FeaturedDocumentList, 1, 1]
];
export var FeaturedResultsSetSummary$: StaticStructureSchema = [3, n0, _FRSS,
  0,
  [_FRSIe, _FRSN, _St, _LUT, _CTr],
  [0, 0, 0, 1, 1]
];
export var FsxConfiguration$: StaticStructureSchema = [3, n0, _FC,
  0,
  [_FSI, _FST, _VC, _SA, _IP, _EP, _FM],
  [0, 0, () => DataSourceVpcConfiguration$, 0, 64 | 0, 64 | 0, () => DataSourceToIndexFieldMappingList]
];
export var GetQuerySuggestionsRequest$: StaticStructureSchema = [3, n0, _GQSR,
  0,
  [_II, _QT, _MSC, _STu, _ASC],
  [0, 0, 1, 64 | 0, () => AttributeSuggestionsGetConfig$]
];
export var GetQuerySuggestionsResponse$: StaticStructureSchema = [3, n0, _GQSRe,
  0,
  [_QSI, _Su],
  [0, () => SuggestionList]
];
export var GetSnapshotsRequest$: StaticStructureSchema = [3, n0, _GSR,
  0,
  [_II, _In, _MT, _NTe, _MR],
  [0, 0, 0, 0, 1]
];
export var GetSnapshotsResponse$: StaticStructureSchema = [3, n0, _GSRe,
  0,
  [_SSTF, _SDH, _SD, _NTe],
  [() => TimeRange$, 64 | 0, [1, n0, _SDR, 0, 64 | 0], 0]
];
export var GitHubConfiguration$: StaticStructureSchema = [3, n0, _GHC,
  0,
  [_SSC, _OPC, _Ty, _SA, _UCL, _GHDCP, _RF, _IFNP, _IFTP, _IFNPn, _EFNP, _EFTP, _EFNPx, _VC, _GHRCFM, _GHCCFM, _GHIDCFM, _GHICCFM, _GHIACFM, _GHPRCCFM, _GHPRDCFM, _GHPRDACFM],
  [() => SaaSConfiguration$, () => OnPremiseConfiguration$, 0, 0, 2, () => GitHubDocumentCrawlProperties$, 64 | 0, 64 | 0, 64 | 0, 64 | 0, 64 | 0, 64 | 0, 64 | 0, () => DataSourceVpcConfiguration$, () => DataSourceToIndexFieldMappingList, () => DataSourceToIndexFieldMappingList, () => DataSourceToIndexFieldMappingList, () => DataSourceToIndexFieldMappingList, () => DataSourceToIndexFieldMappingList, () => DataSourceToIndexFieldMappingList, () => DataSourceToIndexFieldMappingList, () => DataSourceToIndexFieldMappingList]
];
export var GitHubDocumentCrawlProperties$: StaticStructureSchema = [3, n0, _GHDCP,
  0,
  [_CRDr, _CIr, _CIC, _CICA, _CPR, _CPRC, _CPRCA],
  [2, 2, 2, 2, 2, 2, 2]
];
export var GoogleDriveConfiguration$: StaticStructureSchema = [3, n0, _GDC,
  0,
  [_SA, _IP, _EP, _FM, _EMT, _EUA, _ESD],
  [0, 64 | 0, 64 | 0, () => DataSourceToIndexFieldMappingList, 64 | 0, 64 | 0, 64 | 0]
];
export var GroupMembers$: StaticStructureSchema = [3, n0, _GM,
  0,
  [_MG, _MU, _SPGM],
  [() => MemberGroups, () => MemberUsers, () => S3Path$]
];
export var GroupOrderingIdSummary$: StaticStructureSchema = [3, n0, _GOISr,
  0,
  [_St, _LUA, _RAe, _OI, _FRai],
  [0, 4, 4, 1, 0]
];
export var GroupSummary$: StaticStructureSchema = [3, n0, _GS,
  0,
  [_GI, _OI],
  [0, 1]
];
export var HierarchicalPrincipal$: StaticStructureSchema = [3, n0, _HP,
  0,
  [_PL],
  [() => PrincipalList]
];
export var Highlight$: StaticStructureSchema = [3, n0, _Hi,
  0,
  [_BO, _EO, _TA, _Ty],
  [1, 1, 2, 0]
];
export var HookConfiguration$: StaticStructureSchema = [3, n0, _HC,
  0,
  [_ICn, _LA, _SB],
  [() => DocumentAttributeCondition$, 0, 0]
];
export var IndexConfigurationSummary$: StaticStructureSchema = [3, n0, _ICS,
  0,
  [_N, _I, _Ed, _CAre, _UA, _St],
  [0, 0, 0, 4, 4, 0]
];
export var IndexStatistics$: StaticStructureSchema = [3, n0, _ISn,
  0,
  [_FS, _TDS],
  [() => FaqStatistics$, () => TextDocumentStatistics$]
];
export var InlineCustomDocumentEnrichmentConfiguration$: StaticStructureSchema = [3, n0, _ICDEC,
  0,
  [_Cond, _Tar, _DCD],
  [() => DocumentAttributeCondition$, () => DocumentAttributeTarget$, 2]
];
export var InternalServerException$: StaticErrorSchema = [-3, n0, _ISE,
  { [_e]: _s, [_hE]: 500 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InternalServerException$, InternalServerException);
export var InvalidRequestException$: StaticErrorSchema = [-3, n0, _IRE,
  { [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidRequestException$, InvalidRequestException);
export var JiraConfiguration$: StaticStructureSchema = [3, n0, _JC,
  0,
  [_JAU, _SA, _UCL, _Pr, _IT, _St, _ISEF, _AFM, _CFM, _IFM, _PFMr, _WLFMo, _IP, _EP, _VC],
  [0, 0, 2, 64 | 0, 64 | 0, 64 | 0, 64 | 0, () => DataSourceToIndexFieldMappingList, () => DataSourceToIndexFieldMappingList, () => DataSourceToIndexFieldMappingList, () => DataSourceToIndexFieldMappingList, () => DataSourceToIndexFieldMappingList, 64 | 0, 64 | 0, () => DataSourceVpcConfiguration$]
];
export var JsonTokenTypeConfiguration$: StaticStructureSchema = [3, n0, _JTTC,
  0,
  [_UNAF, _GAF],
  [0, 0]
];
export var JwtTokenTypeConfiguration$: StaticStructureSchema = [3, n0, _JTTCw,
  0,
  [_KL, _URL, _SMA, _UNAF, _GAF, _Is, _CR],
  [0, 0, 0, 0, 0, 0, 0]
];
export var ListAccessControlConfigurationsRequest$: StaticStructureSchema = [3, n0, _LACCR,
  0,
  [_II, _NTe, _MR],
  [0, 0, 1]
];
export var ListAccessControlConfigurationsResponse$: StaticStructureSchema = [3, n0, _LACCRi,
  0,
  [_NTe, _ACC],
  [0, () => AccessControlConfigurationSummaryList]
];
export var ListDataSourcesRequest$: StaticStructureSchema = [3, n0, _LDSR,
  0,
  [_II, _NTe, _MR],
  [0, 0, 1]
];
export var ListDataSourcesResponse$: StaticStructureSchema = [3, n0, _LDSRi,
  0,
  [_SIum, _NTe],
  [() => DataSourceSummaryList, 0]
];
export var ListDataSourceSyncJobsRequest$: StaticStructureSchema = [3, n0, _LDSSJR,
  0,
  [_I, _II, _NTe, _MR, _STF, _SF],
  [0, 0, 0, 1, () => TimeRange$, 0]
];
export var ListDataSourceSyncJobsResponse$: StaticStructureSchema = [3, n0, _LDSSJRi,
  0,
  [_His, _NTe],
  [() => DataSourceSyncJobHistoryList, 0]
];
export var ListEntityPersonasRequest$: StaticStructureSchema = [3, n0, _LEPR,
  0,
  [_I, _II, _NTe, _MR],
  [0, 0, 0, 1]
];
export var ListEntityPersonasResponse$: StaticStructureSchema = [3, n0, _LEPRi,
  0,
  [_SIum, _NTe],
  [() => PersonasSummaryList, 0]
];
export var ListExperienceEntitiesRequest$: StaticStructureSchema = [3, n0, _LEER,
  0,
  [_I, _II, _NTe],
  [0, 0, 0]
];
export var ListExperienceEntitiesResponse$: StaticStructureSchema = [3, n0, _LEERi,
  0,
  [_SIum, _NTe],
  [[() => ExperienceEntitiesSummaryList, 0], 0]
];
export var ListExperiencesRequest$: StaticStructureSchema = [3, n0, _LER,
  0,
  [_II, _NTe, _MR],
  [0, 0, 1]
];
export var ListExperiencesResponse$: StaticStructureSchema = [3, n0, _LERi,
  0,
  [_SIum, _NTe],
  [() => ExperiencesSummaryList, 0]
];
export var ListFaqsRequest$: StaticStructureSchema = [3, n0, _LFR,
  0,
  [_II, _NTe, _MR],
  [0, 0, 1]
];
export var ListFaqsResponse$: StaticStructureSchema = [3, n0, _LFRi,
  0,
  [_NTe, _FSIa],
  [0, () => FaqSummaryItems]
];
export var ListFeaturedResultsSetsRequest$: StaticStructureSchema = [3, n0, _LFRSR,
  0,
  [_II, _NTe, _MR],
  [0, 0, 1]
];
export var ListFeaturedResultsSetsResponse$: StaticStructureSchema = [3, n0, _LFRSRi,
  0,
  [_FRSSI, _NTe],
  [() => FeaturedResultsSetSummaryItems, 0]
];
export var ListGroupsOlderThanOrderingIdRequest$: StaticStructureSchema = [3, n0, _LGOTOIR,
  0,
  [_II, _DSI, _OI, _NTe, _MR],
  [0, 0, 1, 0, 1]
];
export var ListGroupsOlderThanOrderingIdResponse$: StaticStructureSchema = [3, n0, _LGOTOIRi,
  0,
  [_GSr, _NTe],
  [() => ListOfGroupSummaries, 0]
];
export var ListIndicesRequest$: StaticStructureSchema = [3, n0, _LIR,
  0,
  [_NTe, _MR],
  [0, 1]
];
export var ListIndicesResponse$: StaticStructureSchema = [3, n0, _LIRi,
  0,
  [_ICSI, _NTe],
  [() => IndexConfigurationSummaryList, 0]
];
export var ListQuerySuggestionsBlockListsRequest$: StaticStructureSchema = [3, n0, _LQSBLR,
  0,
  [_II, _NTe, _MR],
  [0, 0, 1]
];
export var ListQuerySuggestionsBlockListsResponse$: StaticStructureSchema = [3, n0, _LQSBLRi,
  0,
  [_BLSI, _NTe],
  [() => QuerySuggestionsBlockListSummaryItems, 0]
];
export var ListTagsForResourceRequest$: StaticStructureSchema = [3, n0, _LTFRR,
  0,
  [_RARN],
  [0]
];
export var ListTagsForResourceResponse$: StaticStructureSchema = [3, n0, _LTFRRi,
  0,
  [_Ta],
  [() => TagList]
];
export var ListThesauriRequest$: StaticStructureSchema = [3, n0, _LTR,
  0,
  [_II, _NTe, _MR],
  [0, 0, 1]
];
export var ListThesauriResponse$: StaticStructureSchema = [3, n0, _LTRi,
  0,
  [_NTe, _TSI],
  [0, () => ThesaurusSummaryItems]
];
export var MemberGroup$: StaticStructureSchema = [3, n0, _MGe,
  0,
  [_GI, _DSI],
  [0, 0]
];
export var MemberUser$: StaticStructureSchema = [3, n0, _MUe,
  0,
  [_UI],
  [0]
];
export var OneDriveConfiguration$: StaticStructureSchema = [3, n0, _ODC,
  0,
  [_TD, _SA, _ODU, _IP, _EP, _FM, _DLG],
  [0, 0, () => OneDriveUsers$, 64 | 0, 64 | 0, () => DataSourceToIndexFieldMappingList, 2]
];
export var OneDriveUsers$: StaticStructureSchema = [3, n0, _ODU,
  0,
  [_ODUL, _ODUSP],
  [64 | 0, () => S3Path$]
];
export var OnPremiseConfiguration$: StaticStructureSchema = [3, n0, _OPC,
  0,
  [_HU, _ON, _SCSP],
  [0, 0, () => S3Path$]
];
export var PersonasSummary$: StaticStructureSchema = [3, n0, _PS,
  0,
  [_EInt, _Pe, _CAre, _UA],
  [0, 0, 4, 4]
];
export var Principal$: StaticStructureSchema = [3, n0, _Pri,
  0,
  [_N, _Ty, _Ac, _DSI],
  [0, 0, 0, 0]
];
export var ProxyConfiguration$: StaticStructureSchema = [3, n0, _PCr,
  0,
  [_H, _Po, _C],
  [0, 1, 0]
];
export var PutPrincipalMappingRequest$: StaticStructureSchema = [3, n0, _PPMR,
  0,
  [_II, _DSI, _GI, _GM, _OI, _RA],
  [0, 0, 0, () => GroupMembers$, 1, 0]
];
export var QueryRequest$: StaticStructureSchema = [3, n0, _QR,
  0,
  [_II, _QT, _AF, _Fa, _RDA, _QRTF, _DROC, _PN, _PSa, _SCor, _SC, _UC, _VI, _SCC, _CCo],
  [0, 0, () => AttributeFilter$, () => FacetList, 64 | 0, 0, () => DocumentRelevanceOverrideConfigurationList, 1, 1, () => SortingConfiguration$, () => SortingConfigurationList, () => UserContext$, 0, () => SpellCorrectionConfiguration$, () => CollapseConfiguration$]
];
export var QueryResult$: StaticStructureSchema = [3, n0, _QRu,
  0,
  [_QI, _RIe, _FR, _TNOR, _W, _SCQ, _FRIe],
  [0, () => QueryResultItemList, () => FacetResultList, 1, () => WarningList, () => SpellCorrectedQueryList, () => FeaturedResultsItemList]
];
export var QueryResultItem$: StaticStructureSchema = [3, n0, _QRI,
  0,
  [_I, _Ty, _Fo, _AA, _DI, _DT, _DE, _DURI, _DAoc, _SAc, _FT, _TE, _CRD],
  [0, 0, 0, () => AdditionalResultAttributeList, 0, () => TextWithHighlights$, () => TextWithHighlights$, 0, () => DocumentAttributeList, () => ScoreAttributes$, 0, () => TableExcerpt$, () => CollapsedResultDetail$]
];
export var QuerySuggestionsBlockListSummary$: StaticStructureSchema = [3, n0, _QSBLS,
  0,
  [_I, _N, _St, _CAre, _UA, _ICt],
  [0, 0, 0, 4, 4, 1]
];
export var QuipConfiguration$: StaticStructureSchema = [3, n0, _QC,
  0,
  [_Dom, _SA, _CFC, _CCR, _CAr, _FIo, _TFMh, _MFM, _AFM, _IP, _EP, _VC],
  [0, 0, 2, 2, 2, 64 | 0, () => DataSourceToIndexFieldMappingList, () => DataSourceToIndexFieldMappingList, () => DataSourceToIndexFieldMappingList, 64 | 0, 64 | 0, () => DataSourceVpcConfiguration$]
];
export var Relevance$: StaticStructureSchema = [3, n0, _R,
  0,
  [_Fr, _Im, _Du, _RO, _VIM],
  [2, 1, 0, 0, 128 | 1]
];
export var RelevanceFeedback$: StaticStructureSchema = [3, n0, _RFe,
  0,
  [_RI, _RV],
  [0, 0]
];
export var ResourceAlreadyExistException$: StaticErrorSchema = [-3, n0, _RAEE,
  { [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ResourceAlreadyExistException$, ResourceAlreadyExistException);
export var ResourceInUseException$: StaticErrorSchema = [-3, n0, _RIUE,
  { [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ResourceInUseException$, ResourceInUseException);
export var ResourceNotFoundException$: StaticErrorSchema = [-3, n0, _RNFE,
  { [_e]: _c, [_hE]: 404 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ResourceNotFoundException$, ResourceNotFoundException);
export var ResourceUnavailableException$: StaticErrorSchema = [-3, n0, _RUE,
  { [_e]: _c, [_hE]: 404 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ResourceUnavailableException$, ResourceUnavailableException);
export var RetrieveRequest$: StaticStructureSchema = [3, n0, _RR,
  0,
  [_II, _QT, _AF, _RDA, _DROC, _PN, _PSa, _UC],
  [0, 0, () => AttributeFilter$, 64 | 0, () => DocumentRelevanceOverrideConfigurationList, 1, 1, () => UserContext$]
];
export var RetrieveResult$: StaticStructureSchema = [3, n0, _RRe,
  0,
  [_QI, _RIe],
  [0, () => RetrieveResultItemList]
];
export var RetrieveResultItem$: StaticStructureSchema = [3, n0, _RRI,
  0,
  [_I, _DI, _DT, _Cont, _DURI, _DAoc, _SAc],
  [0, 0, 0, 0, 0, () => DocumentAttributeList, () => ScoreAttributes$]
];
export var S3DataSourceConfiguration$: StaticStructureSchema = [3, n0, _SDSC,
  0,
  [_BN, _IPn, _IP, _EP, _DMCoc, _ACLC],
  [0, 64 | 0, 64 | 0, 64 | 0, () => DocumentsMetadataConfiguration$, () => AccessControlListConfiguration$]
];
export var S3Path$: StaticStructureSchema = [3, n0, _SP,
  0,
  [_Bu, _K],
  [0, 0]
];
export var SaaSConfiguration$: StaticStructureSchema = [3, n0, _SSC,
  0,
  [_ON, _HU],
  [0, 0]
];
export var SalesforceChatterFeedConfiguration$: StaticStructureSchema = [3, n0, _SCFC,
  0,
  [_DDFN, _DTFN, _FM, _IFT],
  [0, 0, () => DataSourceToIndexFieldMappingList, 64 | 0]
];
export var SalesforceConfiguration$: StaticStructureSchema = [3, n0, _SCa,
  0,
  [_SUe, _SA, _SOC, _KAC, _CFCh, _CAr, _SOAC, _IAFP, _EAFP],
  [0, 0, () => SalesforceStandardObjectConfigurationList, () => SalesforceKnowledgeArticleConfiguration$, () => SalesforceChatterFeedConfiguration$, 2, () => SalesforceStandardObjectAttachmentConfiguration$, 64 | 0, 64 | 0]
];
export var SalesforceCustomKnowledgeArticleTypeConfiguration$: StaticStructureSchema = [3, n0, _SCKATC,
  0,
  [_N, _DDFN, _DTFN, _FM],
  [0, 0, 0, () => DataSourceToIndexFieldMappingList]
];
export var SalesforceKnowledgeArticleConfiguration$: StaticStructureSchema = [3, n0, _SKAC,
  0,
  [_ISnc, _SKATC, _CKATC],
  [64 | 0, () => SalesforceStandardKnowledgeArticleTypeConfiguration$, () => SalesforceCustomKnowledgeArticleTypeConfigurationList]
];
export var SalesforceStandardKnowledgeArticleTypeConfiguration$: StaticStructureSchema = [3, n0, _SSKATC,
  0,
  [_DDFN, _DTFN, _FM],
  [0, 0, () => DataSourceToIndexFieldMappingList]
];
export var SalesforceStandardObjectAttachmentConfiguration$: StaticStructureSchema = [3, n0, _SSOAC,
  0,
  [_DTFN, _FM],
  [0, () => DataSourceToIndexFieldMappingList]
];
export var SalesforceStandardObjectConfiguration$: StaticStructureSchema = [3, n0, _SSOC,
  0,
  [_N, _DDFN, _DTFN, _FM],
  [0, 0, 0, () => DataSourceToIndexFieldMappingList]
];
export var ScoreAttributes$: StaticStructureSchema = [3, n0, _SAc,
  0,
  [_SCc],
  [0]
];
export var Search$: StaticStructureSchema = [3, n0, _Se,
  0,
  [_Fac, _Sea, _Di, _So],
  [2, 2, 2, 2]
];
export var SeedUrlConfiguration$: StaticStructureSchema = [3, n0, _SUC,
  0,
  [_SUee, _WCM],
  [64 | 0, 0]
];
export var ServerSideEncryptionConfiguration$: StaticStructureSchema = [3, n0, _SSEC,
  0,
  [_KKI],
  [[() => KmsKeyId, 0]]
];
export var ServiceNowConfiguration$: StaticStructureSchema = [3, n0, _SNC,
  0,
  [_HU, _SA, _SNBV, _KAC, _SCCe, _AT],
  [0, 0, 0, () => ServiceNowKnowledgeArticleConfiguration$, () => ServiceNowServiceCatalogConfiguration$, 0]
];
export var ServiceNowKnowledgeArticleConfiguration$: StaticStructureSchema = [3, n0, _SNKAC,
  0,
  [_CAr, _IAFP, _EAFP, _DDFN, _DTFN, _FM, _FQ],
  [2, 64 | 0, 64 | 0, 0, 0, () => DataSourceToIndexFieldMappingList, 0]
];
export var ServiceNowServiceCatalogConfiguration$: StaticStructureSchema = [3, n0, _SNSCC,
  0,
  [_CAr, _IAFP, _EAFP, _DDFN, _DTFN, _FM],
  [2, 64 | 0, 64 | 0, 0, 0, () => DataSourceToIndexFieldMappingList]
];
export var ServiceQuotaExceededException$: StaticErrorSchema = [-3, n0, _SQEE,
  { [_e]: _c, [_hE]: 402 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ServiceQuotaExceededException$, ServiceQuotaExceededException);
export var SharePointConfiguration$: StaticStructureSchema = [3, n0, _SPC,
  0,
  [_SPV, _U, _SA, _CAr, _UCL, _IP, _EP, _VC, _FM, _DTFN, _DLG, _SCSP, _AT, _PCr],
  [0, 64 | 0, 0, 2, 2, 64 | 0, 64 | 0, () => DataSourceVpcConfiguration$, () => DataSourceToIndexFieldMappingList, 0, 2, () => S3Path$, 0, () => ProxyConfiguration$]
];
export var SiteMapsConfiguration$: StaticStructureSchema = [3, n0, _SMC,
  0,
  [_SM],
  [64 | 0]
];
export var SlackConfiguration$: StaticStructureSchema = [3, n0, _SCl,
  0,
  [_TI, _SA, _VC, _SEL, _UCL, _CBM, _EA, _SCD, _LBP, _PCF, _PCFu, _IP, _EP, _FM],
  [0, 0, () => DataSourceVpcConfiguration$, 64 | 0, 2, 2, 2, 0, 1, 64 | 0, 64 | 0, 64 | 0, 64 | 0, () => DataSourceToIndexFieldMappingList]
];
export var SortingConfiguration$: StaticStructureSchema = [3, n0, _SCor,
  0,
  [_DAK, _SO],
  [0, 0]
];
export var SourceDocument$: StaticStructureSchema = [3, n0, _SDo,
  0,
  [_DI, _SAu, _AA],
  [0, 64 | 0, () => DocumentAttributeList]
];
export var SpellCorrectedQuery$: StaticStructureSchema = [3, n0, _SCQp,
  0,
  [_SQT, _Cor],
  [0, () => CorrectionList]
];
export var SpellCorrectionConfiguration$: StaticStructureSchema = [3, n0, _SCC,
  0,
  [_IQSCS],
  [2]
];
export var SqlConfiguration$: StaticStructureSchema = [3, n0, _SCq,
  0,
  [_QIEO],
  [0]
];
export var StartDataSourceSyncJobRequest$: StaticStructureSchema = [3, n0, _SDSSJR,
  0,
  [_I, _II],
  [0, 0]
];
export var StartDataSourceSyncJobResponse$: StaticStructureSchema = [3, n0, _SDSSJRt,
  0,
  [_EIx],
  [0]
];
export var Status$: StaticStructureSchema = [3, n0, _St,
  0,
  [_DI, _DSo, _FCa, _FRai],
  [0, 0, 0, 0]
];
export var StopDataSourceSyncJobRequest$: StaticStructureSchema = [3, n0, _SDSSJRto,
  0,
  [_I, _II],
  [0, 0]
];
export var SubmitFeedbackRequest$: StaticStructureSchema = [3, n0, _SFR,
  0,
  [_II, _QI, _CFI, _RFI],
  [0, 0, () => ClickFeedbackList, () => RelevanceFeedbackList]
];
export var SuggestableConfig$: StaticStructureSchema = [3, n0, _SCu,
  0,
  [_AN, _Sug],
  [0, 2]
];
export var Suggestion$: StaticStructureSchema = [3, n0, _Sugg,
  0,
  [_I, _V, _SDou],
  [0, () => SuggestionValue$, () => SourceDocuments]
];
export var SuggestionHighlight$: StaticStructureSchema = [3, n0, _SH,
  0,
  [_BO, _EO],
  [1, 1]
];
export var SuggestionTextWithHighlights$: StaticStructureSchema = [3, n0, _STWH,
  0,
  [_Te, _Hig],
  [0, () => SuggestionHighlightList]
];
export var SuggestionValue$: StaticStructureSchema = [3, n0, _SVu,
  0,
  [_Te],
  [() => SuggestionTextWithHighlights$]
];
export var TableCell$: StaticStructureSchema = [3, n0, _TCa,
  0,
  [_V, _TA, _High, _He],
  [0, 2, 2, 2]
];
export var TableExcerpt$: StaticStructureSchema = [3, n0, _TE,
  0,
  [_Ro, _TNORo],
  [() => TableRowList, 1]
];
export var TableRow$: StaticStructureSchema = [3, n0, _TR,
  0,
  [_Ce],
  [() => TableCellList]
];
export var Tag$: StaticStructureSchema = [3, n0, _Tag,
  0,
  [_K, _V],
  [0, 0]
];
export var TagResourceRequest$: StaticStructureSchema = [3, n0, _TRR,
  0,
  [_RARN, _Ta],
  [0, () => TagList]
];
export var TagResourceResponse$: StaticStructureSchema = [3, n0, _TRRa,
  0,
  [],
  []
];
export var TemplateConfiguration$: StaticStructureSchema = [3, n0, _TC,
  0,
  [_Tem],
  [15]
];
export var TextDocumentStatistics$: StaticStructureSchema = [3, n0, _TDS,
  0,
  [_ITDC, _ITB],
  [1, 1]
];
export var TextWithHighlights$: StaticStructureSchema = [3, n0, _TWH,
  0,
  [_Te, _Hig],
  [0, () => HighlightList]
];
export var ThesaurusSummary$: StaticStructureSchema = [3, n0, _TS,
  0,
  [_I, _N, _St, _CAre, _UA],
  [0, 0, 0, 4, 4]
];
export var ThrottlingException$: StaticErrorSchema = [-3, n0, _TEh,
  { [_e]: _c, [_hE]: 429 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ThrottlingException$, ThrottlingException);
export var TimeRange$: StaticStructureSchema = [3, n0, _TRi,
  0,
  [_ST, _ETn],
  [4, 4]
];
export var UntagResourceRequest$: StaticStructureSchema = [3, n0, _URR,
  0,
  [_RARN, _TK],
  [0, 64 | 0]
];
export var UntagResourceResponse$: StaticStructureSchema = [3, n0, _URRn,
  0,
  [],
  []
];
export var UpdateAccessControlConfigurationRequest$: StaticStructureSchema = [3, n0, _UACCR,
  0,
  [_II, _I, _N, _De, _ACL, _HACL],
  [0, 0, 0, 0, () => PrincipalList, () => HierarchicalPrincipalList]
];
export var UpdateAccessControlConfigurationResponse$: StaticStructureSchema = [3, n0, _UACCRp,
  0,
  [],
  []
];
export var UpdateDataSourceRequest$: StaticStructureSchema = [3, n0, _UDSR,
  0,
  [_I, _N, _II, _Con, _VC, _De, _S, _RA, _LC, _CDEC],
  [0, 0, 0, () => DataSourceConfiguration$, () => DataSourceVpcConfiguration$, 0, 0, 0, 0, () => CustomDocumentEnrichmentConfiguration$]
];
export var UpdateExperienceRequest$: StaticStructureSchema = [3, n0, _UER,
  0,
  [_I, _N, _II, _RA, _Con, _De],
  [0, 0, 0, 0, () => ExperienceConfiguration$, 0]
];
export var UpdateFeaturedResultsSetRequest$: StaticStructureSchema = [3, n0, _UFRSR,
  0,
  [_II, _FRSIe, _FRSN, _De, _St, _QTu, _FDe],
  [0, 0, 0, 0, 0, 64 | 0, () => FeaturedDocumentList]
];
export var UpdateFeaturedResultsSetResponse$: StaticStructureSchema = [3, n0, _UFRSRp,
  0,
  [_FRS],
  [() => FeaturedResultsSet$]
];
export var UpdateIndexRequest$: StaticStructureSchema = [3, n0, _UIR,
  0,
  [_I, _N, _RA, _De, _DMCU, _CU, _UTC, _UCP, _UGRC],
  [0, 0, 0, 0, () => DocumentMetadataConfigurationList, () => CapacityUnitsConfiguration$, () => UserTokenConfigurationList, 0, () => UserGroupResolutionConfiguration$]
];
export var UpdateQuerySuggestionsBlockListRequest$: StaticStructureSchema = [3, n0, _UQSBLR,
  0,
  [_II, _I, _N, _De, _SSP, _RA],
  [0, 0, 0, 0, () => S3Path$, 0]
];
export var UpdateQuerySuggestionsConfigRequest$: StaticStructureSchema = [3, n0, _UQSCR,
  0,
  [_II, _Mo, _QLLBWID, _IQWUI, _MNOQU, _MQC, _ASC],
  [0, 0, 1, 2, 1, 1, () => AttributeSuggestionsUpdateConfig$]
];
export var UpdateThesaurusRequest$: StaticStructureSchema = [3, n0, _UTR,
  0,
  [_I, _N, _II, _De, _RA, _SSP],
  [0, 0, 0, 0, 0, () => S3Path$]
];
export var Urls$: StaticStructureSchema = [3, n0, _U,
  0,
  [_SUC, _SMC],
  [() => SeedUrlConfiguration$, () => SiteMapsConfiguration$]
];
export var UserContext$: StaticStructureSchema = [3, n0, _UC,
  0,
  [_To, _UI, _G, _DSGa],
  [0, 0, 64 | 0, () => DataSourceGroups]
];
export var UserGroupResolutionConfiguration$: StaticStructureSchema = [3, n0, _UGRC,
  0,
  [_UGRM],
  [0]
];
export var UserIdentityConfiguration$: StaticStructureSchema = [3, n0, _UIC,
  0,
  [_IAN],
  [0]
];
export var UserTokenConfiguration$: StaticStructureSchema = [3, n0, _UTCs,
  0,
  [_JTTCw, _JTTC],
  [() => JwtTokenTypeConfiguration$, () => JsonTokenTypeConfiguration$]
];
export var ValidationException$: StaticErrorSchema = [-3, n0, _VE,
  { [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ValidationException$, ValidationException);
export var Warning$: StaticStructureSchema = [3, n0, _Wa,
  0,
  [_M, _Cod],
  [0, 0]
];
export var WebCrawlerConfiguration$: StaticStructureSchema = [3, n0, _WCC,
  0,
  [_U, _CD, _MLPP, _MCSPPIMB, _MUPMCR, _UIP, _UEP, _PCr, _ACu],
  [() => Urls$, 1, 1, 1, 1, 64 | 0, 64 | 0, () => ProxyConfiguration$, () => AuthenticationConfiguration$]
];
export var WorkDocsConfiguration$: StaticStructureSchema = [3, n0, _WDC,
  0,
  [_OIr, _CC, _UCL, _IP, _EP, _FM],
  [0, 2, 2, 64 | 0, 64 | 0, () => DataSourceToIndexFieldMappingList]
];
var __Unit = "unit" as const;
export var KendraServiceException$: StaticErrorSchema = [-3, _sm, "KendraServiceException", 0, [], []];
TypeRegistry.for(_sm).registerError(KendraServiceException$, KendraServiceException);
var AccessControlConfigurationSummaryList: StaticListSchema = [1, n0, _ACCSL,
  0, () => AccessControlConfigurationSummary$
];
var AdditionalResultAttributeList: StaticListSchema = [1, n0, _ARAL,
  0, () => AdditionalResultAttribute$
];
var AssociateEntitiesToExperienceFailedEntityList: StaticListSchema = [1, n0, _AETEFEL,
  0, () => FailedEntity$
];
var AssociateEntityList: StaticListSchema = [1, n0, _AEL,
  0, () => EntityConfiguration$
];
var AttributeFilterList: StaticListSchema = [1, n0, _AFL,
  0, () => AttributeFilter$
];
var BasicAuthenticationConfigurationList: StaticListSchema = [1, n0, _BACL,
  0, () => BasicAuthenticationConfiguration$
];
var BatchDeleteDocumentResponseFailedDocuments: StaticListSchema = [1, n0, _BDDRFDa,
  0, () => BatchDeleteDocumentResponseFailedDocument$
];
var BatchDeleteFeaturedResultsSetErrors: StaticListSchema = [1, n0, _BDFRSEa,
  0, () => BatchDeleteFeaturedResultsSetError$
];
var BatchGetDocumentStatusResponseErrors: StaticListSchema = [1, n0, _BGDSREa,
  0, () => BatchGetDocumentStatusResponseError$
];
var BatchPutDocumentResponseFailedDocuments: StaticListSchema = [1, n0, _BPDRFDa,
  0, () => BatchPutDocumentResponseFailedDocument$
];
var ChangeDetectingColumns = 64 | 0;
var ClickFeedbackList: StaticListSchema = [1, n0, _CFL,
  0, () => ClickFeedback$
];
var ConflictingItems: StaticListSchema = [1, n0, _CIo,
  0, () => ConflictingItem$
];
var ConfluenceAttachmentFieldMappingsList: StaticListSchema = [1, n0, _CAFML,
  0, () => ConfluenceAttachmentToIndexFieldMapping$
];
var ConfluenceBlogFieldMappingsList: StaticListSchema = [1, n0, _CBFML,
  0, () => ConfluenceBlogToIndexFieldMapping$
];
var ConfluencePageFieldMappingsList: StaticListSchema = [1, n0, _CPFML,
  0, () => ConfluencePageToIndexFieldMapping$
];
var ConfluenceSpaceFieldMappingsList: StaticListSchema = [1, n0, _CSFML,
  0, () => ConfluenceSpaceToIndexFieldMapping$
];
var ConfluenceSpaceList = 64 | 0;
var CorrectionList: StaticListSchema = [1, n0, _CL,
  0, () => Correction$
];
var DataSourceGroups: StaticListSchema = [1, n0, _DSGa,
  0, () => DataSourceGroup$
];
var DataSourceIdList = 64 | 0;
var DataSourceInclusionsExclusionsStrings = 64 | 0;
var DataSourceSummaryList: StaticListSchema = [1, n0, _DSSL,
  0, () => DataSourceSummary$
];
var DataSourceSyncJobHistoryList: StaticListSchema = [1, n0, _DSSJHL,
  0, () => DataSourceSyncJob$
];
var DataSourceToIndexFieldMappingList: StaticListSchema = [1, n0, _DSTIFML,
  0, () => DataSourceToIndexFieldMapping$
];
var DisassociateEntityList: StaticListSchema = [1, n0, _DEL,
  0, () => EntityConfiguration$
];
var DocumentAttributeKeyList = 64 | 0;
var DocumentAttributeList: StaticListSchema = [1, n0, _DAL,
  0, () => DocumentAttribute$
];
var DocumentAttributeStringListValue = 64 | 0;
var DocumentAttributeValueCountPairList: StaticListSchema = [1, n0, _DAVCPL,
  0, () => DocumentAttributeValueCountPair$
];
var DocumentIdList = 64 | 0;
var DocumentInfoList: StaticListSchema = [1, n0, _DILo,
  0, () => DocumentInfo$
];
var DocumentList: StaticListSchema = [1, n0, _DL,
  0, () => Document$
];
var DocumentMetadataConfigurationList: StaticListSchema = [1, n0, _DMCL,
  0, () => DocumentMetadataConfiguration$
];
var DocumentRelevanceOverrideConfigurationList: StaticListSchema = [1, n0, _DROCL,
  0, () => DocumentRelevanceConfiguration$
];
var DocumentStatusList: StaticListSchema = [1, n0, _DSL,
  0, () => Status$
];
var EntityFilter = 64 | 0;
var EntityIdsList = 64 | 0;
var EntityPersonaConfigurationList: StaticListSchema = [1, n0, _EPCL,
  0, () => EntityPersonaConfiguration$
];
var ExcludeMimeTypesList = 64 | 0;
var ExcludeSharedDrivesList = 64 | 0;
var ExcludeUserAccountsList = 64 | 0;
var ExpandedResultList: StaticListSchema = [1, n0, _ERL,
  0, () => ExpandedResultItem$
];
var ExperienceEndpoints: StaticListSchema = [1, n0, _EEx,
  0, () => ExperienceEndpoint$
];
var ExperienceEntitiesSummaryList: StaticListSchema = [1, n0, _EESL,
  0, [() => ExperienceEntitiesSummary$,
    0]
];
var ExperiencesSummaryList: StaticListSchema = [1, n0, _ESL,
  0, () => ExperiencesSummary$
];
var FacetList: StaticListSchema = [1, n0, _FL,
  0, () => Facet$
];
var FacetResultList: StaticListSchema = [1, n0, _FRL,
  0, () => FacetResult$
];
var FailedEntityList: StaticListSchema = [1, n0, _FEL,
  0, () => FailedEntity$
];
var FaqIdsList = 64 | 0;
var FaqSummaryItems: StaticListSchema = [1, n0, _FSIa,
  0, () => FaqSummary$
];
var FeaturedDocumentList: StaticListSchema = [1, n0, _FDL,
  0, () => FeaturedDocument$
];
var FeaturedDocumentMissingList: StaticListSchema = [1, n0, _FDML,
  0, () => FeaturedDocumentMissing$
];
var FeaturedDocumentWithMetadataList: StaticListSchema = [1, n0, _FDWML,
  0, () => FeaturedDocumentWithMetadata$
];
var FeaturedResultsItemList: StaticListSchema = [1, n0, _FRIL,
  0, () => FeaturedResultsItem$
];
var FeaturedResultsSetIdList = 64 | 0;
var FeaturedResultsSetSummaryItems: StaticListSchema = [1, n0, _FRSSI,
  0, () => FeaturedResultsSetSummary$
];
var FolderIdList = 64 | 0;
var GroupOrderingIdSummaries: StaticListSchema = [1, n0, _GOIS,
  0, () => GroupOrderingIdSummary$
];
var Groups = 64 | 0;
var HierarchicalPrincipalList: StaticListSchema = [1, n0, _HPL,
  0, () => HierarchicalPrincipal$
];
var HighlightList: StaticListSchema = [1, n0, _HL,
  0, () => Highlight$
];
var IndexConfigurationSummaryList: StaticListSchema = [1, n0, _ICSL,
  0, () => IndexConfigurationSummary$
];
var InlineCustomDocumentEnrichmentConfigurationList: StaticListSchema = [1, n0, _ICDECL,
  0, () => InlineCustomDocumentEnrichmentConfiguration$
];
var IssueSubEntityFilter = 64 | 0;
var IssueType = 64 | 0;
var JiraStatus = 64 | 0;
var ListOfGroupSummaries: StaticListSchema = [1, n0, _LOGS,
  0, () => GroupSummary$
];
var MemberGroups: StaticListSchema = [1, n0, _MG,
  0, () => MemberGroup$
];
var MemberUsers: StaticListSchema = [1, n0, _MU,
  0, () => MemberUser$
];
var OneDriveUserList = 64 | 0;
var PersonasSummaryList: StaticListSchema = [1, n0, _PSL,
  0, () => PersonasSummary$
];
var PrincipalList: StaticListSchema = [1, n0, _PL,
  0, () => Principal$
];
var PrivateChannelFilter = 64 | 0;
var Project = 64 | 0;
var PublicChannelFilter = 64 | 0;
var QueryResultItemList: StaticListSchema = [1, n0, _QRIL,
  0, () => QueryResultItem$
];
var QuerySuggestionsBlockListSummaryItems: StaticListSchema = [1, n0, _QSBLSI,
  0, () => QuerySuggestionsBlockListSummary$
];
var QueryTextList = 64 | 0;
var RelevanceFeedbackList: StaticListSchema = [1, n0, _RFL,
  0, () => RelevanceFeedback$
];
var RepositoryNames = 64 | 0;
var RetrieveResultItemList: StaticListSchema = [1, n0, _RRIL,
  0, () => RetrieveResultItem$
];
var SalesforceChatterFeedIncludeFilterTypes = 64 | 0;
var SalesforceCustomKnowledgeArticleTypeConfigurationList: StaticListSchema = [1, n0, _SCKATCL,
  0, () => SalesforceCustomKnowledgeArticleTypeConfiguration$
];
var SalesforceKnowledgeArticleStateList = 64 | 0;
var SalesforceStandardObjectConfigurationList: StaticListSchema = [1, n0, _SSOCL,
  0, () => SalesforceStandardObjectConfiguration$
];
var SecurityGroupIdList = 64 | 0;
var SeedUrlList = 64 | 0;
var SharePointUrlList = 64 | 0;
var SiteMapsList = 64 | 0;
var SlackEntityList = 64 | 0;
var SnapshotsDataHeaderFields = 64 | 0;
var SnapshotsDataRecord = 64 | 0;
var SnapshotsDataRecords: StaticListSchema = [1, n0, _SDR,
  0, 64 | 0
];
var SortingConfigurationList: StaticListSchema = [1, n0, _SCLo,
  0, () => SortingConfiguration$
];
var SourceDocuments: StaticListSchema = [1, n0, _SDou,
  0, () => SourceDocument$
];
var SpellCorrectedQueryList: StaticListSchema = [1, n0, _SCQL,
  0, () => SpellCorrectedQuery$
];
var StringList = 64 | 0;
var SubnetIdList = 64 | 0;
var SuggestableConfigList: StaticListSchema = [1, n0, _SCL,
  0, () => SuggestableConfig$
];
var SuggestionHighlightList: StaticListSchema = [1, n0, _SHL,
  0, () => SuggestionHighlight$
];
var SuggestionList: StaticListSchema = [1, n0, _SL,
  0, () => Suggestion$
];
var SuggestionTypes = 64 | 0;
var TableCellList: StaticListSchema = [1, n0, _TCL,
  0, () => TableCell$
];
var TableRowList: StaticListSchema = [1, n0, _TRL,
  0, () => TableRow$
];
var TagKeyList = 64 | 0;
var TagList: StaticListSchema = [1, n0, _TL,
  0, () => Tag$
];
var ThesaurusSummaryItems: StaticListSchema = [1, n0, _TSI,
  0, () => ThesaurusSummary$
];
var UserTokenConfigurationList: StaticListSchema = [1, n0, _UTCL,
  0, () => UserTokenConfiguration$
];
var WarningList: StaticListSchema = [1, n0, _WL,
  0, () => Warning$
];
var ValueImportanceMap = 128 | 1;
export var AssociateEntitiesToExperience$: StaticOperationSchema = [9, n0, _AETE,
  0, () => AssociateEntitiesToExperienceRequest$, () => AssociateEntitiesToExperienceResponse$
];
export var AssociatePersonasToEntities$: StaticOperationSchema = [9, n0, _APTE,
  0, () => AssociatePersonasToEntitiesRequest$, () => AssociatePersonasToEntitiesResponse$
];
export var BatchDeleteDocument$: StaticOperationSchema = [9, n0, _BDD,
  0, () => BatchDeleteDocumentRequest$, () => BatchDeleteDocumentResponse$
];
export var BatchDeleteFeaturedResultsSet$: StaticOperationSchema = [9, n0, _BDFRS,
  0, () => BatchDeleteFeaturedResultsSetRequest$, () => BatchDeleteFeaturedResultsSetResponse$
];
export var BatchGetDocumentStatus$: StaticOperationSchema = [9, n0, _BGDS,
  0, () => BatchGetDocumentStatusRequest$, () => BatchGetDocumentStatusResponse$
];
export var BatchPutDocument$: StaticOperationSchema = [9, n0, _BPD,
  0, () => BatchPutDocumentRequest$, () => BatchPutDocumentResponse$
];
export var ClearQuerySuggestions$: StaticOperationSchema = [9, n0, _CQS,
  0, () => ClearQuerySuggestionsRequest$, () => __Unit
];
export var CreateAccessControlConfiguration$: StaticOperationSchema = [9, n0, _CACC,
  0, () => CreateAccessControlConfigurationRequest$, () => CreateAccessControlConfigurationResponse$
];
export var CreateDataSource$: StaticOperationSchema = [9, n0, _CDS,
  0, () => CreateDataSourceRequest$, () => CreateDataSourceResponse$
];
export var CreateExperience$: StaticOperationSchema = [9, n0, _CEr,
  0, () => CreateExperienceRequest$, () => CreateExperienceResponse$
];
export var CreateFaq$: StaticOperationSchema = [9, n0, _CFr,
  0, () => CreateFaqRequest$, () => CreateFaqResponse$
];
export var CreateFeaturedResultsSet$: StaticOperationSchema = [9, n0, _CFRS,
  0, () => CreateFeaturedResultsSetRequest$, () => CreateFeaturedResultsSetResponse$
];
export var CreateIndex$: StaticOperationSchema = [9, n0, _CIre,
  0, () => CreateIndexRequest$, () => CreateIndexResponse$
];
export var CreateQuerySuggestionsBlockList$: StaticOperationSchema = [9, n0, _CQSBL,
  0, () => CreateQuerySuggestionsBlockListRequest$, () => CreateQuerySuggestionsBlockListResponse$
];
export var CreateThesaurus$: StaticOperationSchema = [9, n0, _CTre,
  0, () => CreateThesaurusRequest$, () => CreateThesaurusResponse$
];
export var DeleteAccessControlConfiguration$: StaticOperationSchema = [9, n0, _DACC,
  0, () => DeleteAccessControlConfigurationRequest$, () => DeleteAccessControlConfigurationResponse$
];
export var DeleteDataSource$: StaticOperationSchema = [9, n0, _DDS,
  0, () => DeleteDataSourceRequest$, () => __Unit
];
export var DeleteExperience$: StaticOperationSchema = [9, n0, _DEe,
  0, () => DeleteExperienceRequest$, () => DeleteExperienceResponse$
];
export var DeleteFaq$: StaticOperationSchema = [9, n0, _DFe,
  0, () => DeleteFaqRequest$, () => __Unit
];
export var DeleteIndex$: StaticOperationSchema = [9, n0, _DIe,
  0, () => DeleteIndexRequest$, () => __Unit
];
export var DeletePrincipalMapping$: StaticOperationSchema = [9, n0, _DPM,
  0, () => DeletePrincipalMappingRequest$, () => __Unit
];
export var DeleteQuerySuggestionsBlockList$: StaticOperationSchema = [9, n0, _DQSBL,
  0, () => DeleteQuerySuggestionsBlockListRequest$, () => __Unit
];
export var DeleteThesaurus$: StaticOperationSchema = [9, n0, _DTe,
  0, () => DeleteThesaurusRequest$, () => __Unit
];
export var DescribeAccessControlConfiguration$: StaticOperationSchema = [9, n0, _DACCe,
  0, () => DescribeAccessControlConfigurationRequest$, () => DescribeAccessControlConfigurationResponse$
];
export var DescribeDataSource$: StaticOperationSchema = [9, n0, _DDSe,
  0, () => DescribeDataSourceRequest$, () => DescribeDataSourceResponse$
];
export var DescribeExperience$: StaticOperationSchema = [9, n0, _DEes,
  0, () => DescribeExperienceRequest$, () => DescribeExperienceResponse$
];
export var DescribeFaq$: StaticOperationSchema = [9, n0, _DFes,
  0, () => DescribeFaqRequest$, () => DescribeFaqResponse$
];
export var DescribeFeaturedResultsSet$: StaticOperationSchema = [9, n0, _DFRS,
  0, () => DescribeFeaturedResultsSetRequest$, () => DescribeFeaturedResultsSetResponse$
];
export var DescribeIndex$: StaticOperationSchema = [9, n0, _DIes,
  0, () => DescribeIndexRequest$, () => DescribeIndexResponse$
];
export var DescribePrincipalMapping$: StaticOperationSchema = [9, n0, _DPMe,
  0, () => DescribePrincipalMappingRequest$, () => DescribePrincipalMappingResponse$
];
export var DescribeQuerySuggestionsBlockList$: StaticOperationSchema = [9, n0, _DQSBLe,
  0, () => DescribeQuerySuggestionsBlockListRequest$, () => DescribeQuerySuggestionsBlockListResponse$
];
export var DescribeQuerySuggestionsConfig$: StaticOperationSchema = [9, n0, _DQSC,
  0, () => DescribeQuerySuggestionsConfigRequest$, () => DescribeQuerySuggestionsConfigResponse$
];
export var DescribeThesaurus$: StaticOperationSchema = [9, n0, _DTes,
  0, () => DescribeThesaurusRequest$, () => DescribeThesaurusResponse$
];
export var DisassociateEntitiesFromExperience$: StaticOperationSchema = [9, n0, _DEFE,
  0, () => DisassociateEntitiesFromExperienceRequest$, () => DisassociateEntitiesFromExperienceResponse$
];
export var DisassociatePersonasFromEntities$: StaticOperationSchema = [9, n0, _DPFE,
  0, () => DisassociatePersonasFromEntitiesRequest$, () => DisassociatePersonasFromEntitiesResponse$
];
export var GetQuerySuggestions$: StaticOperationSchema = [9, n0, _GQS,
  0, () => GetQuerySuggestionsRequest$, () => GetQuerySuggestionsResponse$
];
export var GetSnapshots$: StaticOperationSchema = [9, n0, _GSe,
  0, () => GetSnapshotsRequest$, () => GetSnapshotsResponse$
];
export var ListAccessControlConfigurations$: StaticOperationSchema = [9, n0, _LACC,
  0, () => ListAccessControlConfigurationsRequest$, () => ListAccessControlConfigurationsResponse$
];
export var ListDataSources$: StaticOperationSchema = [9, n0, _LDS,
  0, () => ListDataSourcesRequest$, () => ListDataSourcesResponse$
];
export var ListDataSourceSyncJobs$: StaticOperationSchema = [9, n0, _LDSSJ,
  0, () => ListDataSourceSyncJobsRequest$, () => ListDataSourceSyncJobsResponse$
];
export var ListEntityPersonas$: StaticOperationSchema = [9, n0, _LEP,
  0, () => ListEntityPersonasRequest$, () => ListEntityPersonasResponse$
];
export var ListExperienceEntities$: StaticOperationSchema = [9, n0, _LEE,
  0, () => ListExperienceEntitiesRequest$, () => ListExperienceEntitiesResponse$
];
export var ListExperiences$: StaticOperationSchema = [9, n0, _LE,
  0, () => ListExperiencesRequest$, () => ListExperiencesResponse$
];
export var ListFaqs$: StaticOperationSchema = [9, n0, _LF,
  0, () => ListFaqsRequest$, () => ListFaqsResponse$
];
export var ListFeaturedResultsSets$: StaticOperationSchema = [9, n0, _LFRS,
  0, () => ListFeaturedResultsSetsRequest$, () => ListFeaturedResultsSetsResponse$
];
export var ListGroupsOlderThanOrderingId$: StaticOperationSchema = [9, n0, _LGOTOI,
  0, () => ListGroupsOlderThanOrderingIdRequest$, () => ListGroupsOlderThanOrderingIdResponse$
];
export var ListIndices$: StaticOperationSchema = [9, n0, _LI,
  0, () => ListIndicesRequest$, () => ListIndicesResponse$
];
export var ListQuerySuggestionsBlockLists$: StaticOperationSchema = [9, n0, _LQSBL,
  0, () => ListQuerySuggestionsBlockListsRequest$, () => ListQuerySuggestionsBlockListsResponse$
];
export var ListTagsForResource$: StaticOperationSchema = [9, n0, _LTFR,
  0, () => ListTagsForResourceRequest$, () => ListTagsForResourceResponse$
];
export var ListThesauri$: StaticOperationSchema = [9, n0, _LTi,
  0, () => ListThesauriRequest$, () => ListThesauriResponse$
];
export var PutPrincipalMapping$: StaticOperationSchema = [9, n0, _PPM,
  0, () => PutPrincipalMappingRequest$, () => __Unit
];
export var Query$: StaticOperationSchema = [9, n0, _Q,
  0, () => QueryRequest$, () => QueryResult$
];
export var Retrieve$: StaticOperationSchema = [9, n0, _Re,
  0, () => RetrieveRequest$, () => RetrieveResult$
];
export var StartDataSourceSyncJob$: StaticOperationSchema = [9, n0, _SDSSJ,
  0, () => StartDataSourceSyncJobRequest$, () => StartDataSourceSyncJobResponse$
];
export var StopDataSourceSyncJob$: StaticOperationSchema = [9, n0, _SDSSJt,
  0, () => StopDataSourceSyncJobRequest$, () => __Unit
];
export var SubmitFeedback$: StaticOperationSchema = [9, n0, _SFu,
  0, () => SubmitFeedbackRequest$, () => __Unit
];
export var TagResource$: StaticOperationSchema = [9, n0, _TRa,
  0, () => TagResourceRequest$, () => TagResourceResponse$
];
export var UntagResource$: StaticOperationSchema = [9, n0, _UR,
  0, () => UntagResourceRequest$, () => UntagResourceResponse$
];
export var UpdateAccessControlConfiguration$: StaticOperationSchema = [9, n0, _UACC,
  0, () => UpdateAccessControlConfigurationRequest$, () => UpdateAccessControlConfigurationResponse$
];
export var UpdateDataSource$: StaticOperationSchema = [9, n0, _UDS,
  0, () => UpdateDataSourceRequest$, () => __Unit
];
export var UpdateExperience$: StaticOperationSchema = [9, n0, _UE,
  0, () => UpdateExperienceRequest$, () => __Unit
];
export var UpdateFeaturedResultsSet$: StaticOperationSchema = [9, n0, _UFRS,
  0, () => UpdateFeaturedResultsSetRequest$, () => UpdateFeaturedResultsSetResponse$
];
export var UpdateIndex$: StaticOperationSchema = [9, n0, _UIp,
  0, () => UpdateIndexRequest$, () => __Unit
];
export var UpdateQuerySuggestionsBlockList$: StaticOperationSchema = [9, n0, _UQSBL,
  0, () => UpdateQuerySuggestionsBlockListRequest$, () => __Unit
];
export var UpdateQuerySuggestionsConfig$: StaticOperationSchema = [9, n0, _UQSC,
  0, () => UpdateQuerySuggestionsConfigRequest$, () => __Unit
];
export var UpdateThesaurus$: StaticOperationSchema = [9, n0, _UT,
  0, () => UpdateThesaurusRequest$, () => __Unit
];
