 ...
 */
export const waitForLinkRoutingRuleDeleted = async (
 ...
 */
export const waitUntilLinkRoutingRuleDeleted = async (
  params: WaiterConfiguration<RTBFabricClient>,
  input: GetLinkRoutingRuleCommandInput
): Promise<WaiterResult<GetLinkRoutingRuleCommandOutput | ResourceNotFoundException>> => {
  const serviceDefaults = { minDelay: 5, maxDelay: 120 };
  const result = await createWaiter({ ...serviceDefaults, ...params }, input, checkState);
  return checkExceptions(result) as WaiterResult<GetLinkRoutingRuleCommandOutput | ResourceNotFoundException>;
};
