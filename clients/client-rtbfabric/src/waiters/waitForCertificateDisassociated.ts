 ...
 */
export const waitForCertificateDisassociated = async (
 ...
 */
export const waitUntilCertificateDisassociated = async (
  params: WaiterConfiguration<RTBFabricClient>,
  input: GetCertificateAssociationCommandInput
): Promise<WaiterResult<GetCertificateAssociationCommandOutput | ResourceNotFoundException>> => {
  const serviceDefaults = { minDelay: 15, maxDelay: 120 };
  const result = await createWaiter({ ...serviceDefaults, ...params }, input, checkState);
  return checkExceptions(result) as WaiterResult<GetCertificateAssociationCommandOutput | ResourceNotFoundException>;
};
