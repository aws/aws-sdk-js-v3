...
const se_FilterCriteria = (input: FilterCriteria, context: __SerdeContext): any => {
  return take(input, {
    ...
    componentId: (_) => _json(_),
    componentType: (_) => _json(_),
    ec2InstanceImageId: (_) => _json(_),
    ec2InstanceSubnetId: (_) => _json(_),
    ec2InstanceVpcId: (_) => _json(_),
    ecrImageArchitecture: (_) => _json(_),
    ecrImageHash: (_) => _json(_),
    ecrImagePushedAt: (_) => se_DateFilterList(_, context),
    ecrImageRegistry: (_) => _json(_),
    ecrImageRepositoryName: (_) => _json(_),
    ecrImageTags: (_) => _json(_),
    exploitAvailable: (_) => _json(_),
    findingArn: (_) => _json(_),
    findingStatus: (_) => _json(_),
    findingType: (_) => _json(_),
    firstObservedAt: (_) => se_DateFilterList(_, context),
    fixAvailable: (_) => _json(_),
    inspectorScore: (_) => se_NumberFilterList(_, context),
    lambdaFunctionExecutionRoleArn: (_) => _json(_),
    lambdaFunctionLastModifiedAt: (_) => se_DateFilterList(_, context),
    lambdaFunctionLayers: (_) => _json(_),
    lambdaFunctionName: (_) => _json(_),
    lambdaFunctionRuntime: (_) => _json(_),
    ...
    vendorSeverity: (_) => _json(_),
    vulnerabilityId: (_) => _json(_),
    vulnerabilitySource: (_) => _json(_),
...
const se_NumberFilter = (input: NumberFilter, context: __SerdeContext): any => {
  return take(input, {
    lowerInclusive: (_) => __serializeFloat(_),
    upperInclusive: (_) => __serializeFloat(_),
  });
};
...
const se_PackageFilter = (input: PackageFilter, context: __SerdeContext): any => {
  return take(input, {
    architecture: (_) => _json(_),
    epoch: (_) => se_NumberFilter(_, context),
    name: (_) => _json(_),
    release: (_) => _json(_),
    sourceLambdaLayerArn: (_) => _json(_),
    sourceLayerHash: (_) => _json(_),
    version: (_) => _json(_),
  });
};
