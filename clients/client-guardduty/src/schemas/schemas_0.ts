const _A = "Account";
const _AA = "AdminAccount";
const _AAC = "ActiveAccountsCount";
const _AACA = "AwsApiCallAction";
const _AAI = "AdminAccountId";
const _AAIR = "AcceptAdministratorInvitationRequest";
const _AAIRc = "AcceptAdministratorInvitationResponse";
const _AAIc = "AcceptAdministratorInvitation";
const _AASA = "AssociatedAttackSequenceArn";
const _AASG = "AutoscalingAutoScalingGroup";
const _AAd = "AdminAccounts";
const _AC = "ApiCall";
const _ACL = "AccessControlList";
const _ACd = "AdditionalConfiguration";
const _AD = "AccountDetail";
const _ADE = "AccessDeniedException";
const _ADI = "AdminDetectorId";
const _ADc = "AccountDetails";
const _ADd = "AddonDetails";
const _ADg = "AgentDetails";
const _ADs = "AssociationDetail";
const _AE = "AutoEnable";
const _AEOM = "AutoEnableOrganizationMembers";
const _AF = "AddressFamily";
const _AFR = "ArchiveFindingsRequest";
const _AFRr = "ArchiveFindingsResponse";
const _AFTI = "AccountFreeTrialInfo";
const _AFTIc = "AccountFreeTrialInfos";
const _AFr = "ArchiveFindings";
const _AI = "AdministratorId";
const _AIR = "AcceptInvitationRequest";
const _AIRc = "AcceptInvitationResponse";
const _AIc = "AccountId";
const _AIcc = "AccountIds";
const _AIcce = "AcceptInvitation";
const _AIct = "ActorIds";
const _AId = "AdditionalInfo";
const _AIs = "AssociationId";
const _AK = "AccessKey";
const _AKD = "AccessKeyDetails";
const _AKI = "AccessKeyId";
const _AL = "AuditLogs";
const _ALP = "AccountLevelPermissions";
const _AM = "AuthMethod";
const _AN = "AttributeName";
const _AO = "AnomalyObject";
const _AOs = "AsnOrg";
const _AP = "ActorProcess";
const _APA = "AccountPublicAccess";
const _APE = "AllowPrivilegeEscalation";
const _APF = "AnomalyProfileFeatures";
const _APFO = "AnomalyProfileFeatureObjects";
const _APRA = "AllowsPublicReadAccess";
const _APWA = "AllowsPublicWriteAccess";
const _APn = "AnomalyProfiles";
const _AR = "AffectedResources";
const _AS = "AccountStatistics";
const _ASC = "ActiveServicesCount";
const _ASL = "AssociationSummaryList";
const _AST = "AdditionalSequenceTypes";
const _ASd = "AddonStatus";
const _ASdm = "AdminStatus";
const _ASs = "AssociationSummary";
const _ASu = "AutonomousSystem";
const _AT = "ActionType";
const _AU = "AnomalyUnusual";
const _AUBF = "AnomalyUnusualBehaviorFeature";
const _AV = "AddonVersion";
const _AVt = "AttachedVolumes";
const _AZ = "AvailabilityZone";
const _Ac = "Action";
const _Acc = "Accounts";
const _Act = "Activity";
const _Acti = "Activate";
const _Actio = "Actions";
const _Activ = "Activities";
const _Acto = "Actor";
const _Actor = "Actors";
const _Ad = "Administrator";
const _Af = "Affiliated";
const _Al = "Allowed";
const _Am = "Amount";
const _An = "Anomaly";
const _Ap = "Api";
const _App = "Application";
const _Ar = "Arn";
const _Arc = "Archived";
const _As = "Asn";
const _B = "Behavior";
const _BG = "BedrockGuardrail";
const _BGD = "BedrockGuardrailDetails";
const _BGe = "BedrockGuardrails";
const _BLP = "BucketLevelPermissions";
const _BN = "BucketName";
const _BP = "BucketPolicy";
const _BPA = "BlockPublicAccess";
const _BPAl = "BlockPublicAcls";
const _BPAu = "BucketPublicAccess";
const _BPP = "BlockPublicPolicy";
const _BRA = "BaselineResourceArn";
const _BRE = "BadRequestException";
const _BVN = "BackupVaultName";
const _Bl = "Blocked";
const _Bu = "Bucket";
const _C = "City";
const _CA = "CreatedAt";
const _CAl = "ClusterArn";
const _CBCS = "CountByCoverageStatus";
const _CBF = "CountByFeature";
const _CBRT = "CountByResourceType";
const _CBS = "CountBySeverity";
const _CC = "CountryCode";
const _CCDRA = "CreateCustomDetectionRuleAssociation";
const _CCDRAR = "CreateCustomDetectionRuleAssociationRequest";
const _CCDRARr = "CreateCustomDetectionRuleAssociationResponse";
const _CCDROC = "CreateCustomDetectionRuleOrgConfiguration";
const _CCDROCR = "CreateCustomDetectionRuleOrgConfigurationRequest";
const _CCDROCRr = "CreateCustomDetectionRuleOrgConfigurationResponse";
const _CCI = "CoveredContainerInstances";
const _CCIo = "CompatibleContainerInstances";
const _CD = "CloudDetails";
const _CDR = "CreateDetectorRequest";
const _CDRr = "CreateDetectorResponse";
const _CDo = "ConnectionDirection";
const _CDon = "ContainerDetails";
const _CDr = "CreateDetector";
const _CE = "ConflictException";
const _CECD = "CoverageEcsClusterDetails";
const _CECDo = "CoverageEksClusterDetails";
const _CEID = "CoverageEc2InstanceDetails";
const _CF = "CreateFilter";
const _CFC = "CoverageFilterCondition";
const _CFCL = "CoverageFilterCriterionList";
const _CFCo = "CoverageFilterCriteria";
const _CFCov = "CoverageFilterCriterion";
const _CFR = "ContainerFindingResource";
const _CFRr = "CreateFilterRequest";
const _CFRre = "CreateFilterResponse";
const _CI = "CreateInvestigation";
const _CID = "ContainerInstanceDetails";
const _CIPS = "CreateIPSet";
const _CIPSR = "CreateIPSetRequest";
const _CIPSRr = "CreateIPSetResponse";
const _CIR = "CreateInvestigationRequest";
const _CIRr = "CreateInvestigationResponse";
const _CK = "CriterionKey";
const _CLE = "CommandLineExample";
const _CM = "CreateMembers";
const _CMPP = "CreateMalwareProtectionPlan";
const _CMPPR = "CreateMalwareProtectionPlanRequest";
const _CMPPRr = "CreateMalwareProtectionPlanResponse";
const _CMR = "CreateMembersRequest";
const _CMRr = "CreateMembersResponse";
const _CN = "CityName";
const _CNl = "ClusterName";
const _CNo = "CountryName";
const _CNom = "CompatibleNodes";
const _CNov = "CoveredNodes";
const _CP = "CloudPartition";
const _CPD = "CreatePublishingDestination";
const _CPDR = "CreatePublishingDestinationRequest";
const _CPDRr = "CreatePublishingDestinationResponse";
const _CPF = "ContentPolicyFilters";
const _CPFo = "ContentPolicyFilter";
const _CPR = "CreateProtectedResource";
const _CR = "ContainerRuntime";
const _CRD = "CoverageResourceDetails";
const _CRo = "CoverageResource";
const _CRov = "CoverageResources";
const _CS = "CloudformationStack";
const _CSBR = "CreateS3BucketResource";
const _CSC = "CoverageSortCriteria";
const _CSD = "ContinuousScanDetails";
const _CSF = "CreateSampleFindings";
const _CSFR = "CreateSampleFindingsRequest";
const _CSFRr = "CreateSampleFindingsResponse";
const _CSo = "CoverageStatus";
const _CSov = "CoverageStatistics";
const _CT = "CallerType";
const _CTCR = "CloudTrailConfigurationResult";
const _CTES = "CreateThreatEntitySet";
const _CTESR = "CreateThreatEntitySetRequest";
const _CTESRr = "CreateThreatEntitySetResponse";
const _CTESRre = "CreateTrustedEntitySetRequest";
const _CTESRrea = "CreateTrustedEntitySetResponse";
const _CTESr = "CreateTrustedEntitySet";
const _CTIS = "CreateThreatIntelSet";
const _CTISR = "CreateThreatIntelSetRequest";
const _CTISRr = "CreateThreatIntelSetResponse";
const _CTl = "ClientToken";
const _CTlo = "CloudTrail";
const _CTr = "CreatedTime";
const _CU = "ContainerUids";
const _CUr = "CredentialUid";
const _Cl = "Cloud";
const _Co = "Condition";
const _Cod = "Code";
const _Com = "Comments";
const _Con = "Container";
const _Conf = "Confidence";
const _Confi = "Configuration";
const _Config = "Configurations";
const _Conn = "Connection";
const _Cont = "Containers";
const _Conte = "Context";
const _Cou = "Country";
const _Coun = "Count";
const _Cr = "Criterion";
const _D = "Description";
const _DA = "DestinationArn";
const _DAC = "DetectorAdditionalConfiguration";
const _DACR = "DetectorAdditionalConfigurationResult";
const _DACRe = "DetectorAdditionalConfigurationResults";
const _DACe = "DetectorAdditionalConfigurations";
const _DAe = "DefinitionArn";
const _DCDRA = "DeleteCustomDetectionRuleAssociation";
const _DCDRAR = "DeleteCustomDetectionRuleAssociationRequest";
const _DCDRARe = "DeleteCustomDetectionRuleAssociationResponse";
const _DCDROC = "DeleteCustomDetectionRuleOrgConfiguration";
const _DCDROCR = "DeleteCustomDetectionRuleOrgConfigurationRequest";
const _DCDROCRe = "DeleteCustomDetectionRuleOrgConfigurationResponse";
const _DCI = "DbClusterIdentifier";
const _DD = "DomainDetails";
const _DDR = "DeleteDetectorRequest";
const _DDRe = "DeleteDetectorResponse";
const _DDe = "DeleteDetector";
const _DEN = "DisableEmailNotification";
const _DF = "DeleteFilter";
const _DFAA = "DisassociateFromAdministratorAccount";
const _DFAAR = "DisassociateFromAdministratorAccountRequest";
const _DFAARi = "DisassociateFromAdministratorAccountResponse";
const _DFC = "DetectorFeatureConfiguration";
const _DFCR = "DetectorFeatureConfigurationResult";
const _DFCRe = "DetectorFeatureConfigurationsResults";
const _DFCe = "DetectorFeatureConfigurations";
const _DFMA = "DisassociateFromMasterAccount";
const _DFMAR = "DisassociateFromMasterAccountRequest";
const _DFMARi = "DisassociateFromMasterAccountResponse";
const _DFR = "DeleteFilterRequest";
const _DFRe = "DeleteFilterResponse";
const _DI = "DetectorId";
const _DIA = "DbInstanceArn";
const _DII = "DbInstanceIdentifier";
const _DIPS = "DeleteIPSet";
const _DIPSR = "DeleteIPSetRequest";
const _DIPSRe = "DeleteIPSetResponse";
const _DIR = "DeclineInvitationsRequest";
const _DIRe = "DeclineInvitationsResponse";
const _DIRel = "DeleteInvitationsRequest";
const _DIRele = "DeleteInvitationsResponse";
const _DIe = "DestinationId";
const _DIec = "DeclineInvitations";
const _DIel = "DeleteInvitations";
const _DIet = "DetectorIds";
const _DL = "DnsLogs";
const _DM = "DeleteMembers";
const _DMPP = "DeleteMalwareProtectionPlan";
const _DMPPR = "DeleteMalwareProtectionPlanRequest";
const _DMR = "DeleteMembersRequest";
const _DMRe = "DeleteMembersResponse";
const _DMRi = "DisassociateMembersRequest";
const _DMRis = "DisassociateMembersResponse";
const _DMS = "DescribeMalwareScans";
const _DMSR = "DescribeMalwareScansRequest";
const _DMSRe = "DescribeMalwareScansResponse";
const _DMi = "DisassociateMembers";
const _DN = "DeviceName";
const _DNSL = "DNSLogs";
const _DNSLCR = "DNSLogsConfigurationResult";
const _DOAA = "DisableOrganizationAdminAccount";
const _DOAAR = "DisableOrganizationAdminAccountRequest";
const _DOAARi = "DisableOrganizationAdminAccountResponse";
const _DOC = "DescribeOrganizationConfiguration";
const _DOCR = "DescribeOrganizationConfigurationRequest";
const _DOCRe = "DescribeOrganizationConfigurationResponse";
const _DP = "DestinationProperties";
const _DPD = "DeletePublishingDestination";
const _DPDR = "DeletePublishingDestinationRequest";
const _DPDRe = "DeletePublishingDestinationResponse";
const _DPDRes = "DescribePublishingDestinationRequest";
const _DPDResc = "DescribePublishingDestinationResponse";
const _DPDe = "DescribePublishingDestination";
const _DRA = "DnsRequestAction";
const _DRF = "DetectionRuleFilter";
const _DRFL = "DetectionRuleFilterList";
const _DRI = "DbiResourceId";
const _DROC = "DetectionRuleOrgConfiguration";
const _DROCS = "DetectionRuleOrgConfigurationSummary";
const _DROCSL = "DetectionRuleOrgConfigurationSummaryList";
const _DS = "DataSources";
const _DSC = "DataSourceConfigurations";
const _DSCR = "DataSourceConfigurationsResult";
const _DSFT = "DataSourceFreeTrial";
const _DSFTa = "DataSourcesFreeTrial";
const _DSGA = "DbShardGroupArn";
const _DSGI = "DbShardGroupIdentifier";
const _DSGRI = "DbShardGroupResourceId";
const _DSSE = "DefaultServerSideEncryption";
const _DSa = "DateStatistics";
const _DSat = "DataSource";
const _DT = "DestinationType";
const _DTES = "DeleteThreatEntitySet";
const _DTESR = "DeleteThreatEntitySetRequest";
const _DTESRe = "DeleteThreatEntitySetResponse";
const _DTESRel = "DeleteTrustedEntitySetRequest";
const _DTESRele = "DeleteTrustedEntitySetResponse";
const _DTESe = "DeleteTrustedEntitySet";
const _DTIS = "DeleteThreatIntelSet";
const _DTISR = "DeleteThreatIntelSetRequest";
const _DTISRe = "DeleteThreatIntelSetResponse";
const _DWS = "DomainWithSuffix";
const _Da = "Date";
const _Dat = "Database";
const _Data = "Data";
const _De = "Destination";
const _Def = "Definition";
const _Des = "Destinations";
const _Det = "Detection";
const _Di = "Direction";
const _Do = "Domain";
const _E = "Email";
const _EA = "ExpiresAt";
const _EAC = "EnabledAccountsCount";
const _EAI = "ExcludeAccountIds";
const _EBO = "ExpectedBucketOwner";
const _EC = "ErrorCode";
const _ECD = "EksClusterDetails";
const _ECDc = "EcsClusterDetails";
const _ECc = "EcsCluster";
const _ECk = "EksCluster";
const _ED = "ErrorDetails";
const _EFS = "EventFirstSeen";
const _EI = "Ec2Image";
const _EID = "Ec2InstanceDetails";
const _EIDc = "Ec2ImageDetails";
const _EIU = "Ec2InstanceUids";
const _EIc = "Ec2Instance";
const _EIn = "EndpointIds";
const _EKA = "EncryptionKeyArn";
const _ELS = "EventLastSeen";
const _ELT = "Ec2LaunchTemplate";
const _ENI = "Ec2NetworkInterface";
const _ENIU = "Ec2NetworkInterfaceUids";
const _EOAA = "EnableOrganizationAdminAccount";
const _EOAAR = "EnableOrganizationAdminAccountRequest";
const _EOAARn = "EnableOrganizationAdminAccountResponse";
const _EP = "ExecutablePath";
const _EPf = "EffectivePermission";
const _ES = "EbsSnapshot";
const _ESD = "EbsSnapshotDetails";
const _ESP = "EbsSnapshotPreservation";
const _ESx = "ExecutableSha256";
const _ET = "EndTime";
const _ETD = "EcsTaskDetails";
const _ETa = "ETag";
const _ETc = "EcsTask";
const _ETn = "EncryptionType";
const _EV = "Ec2Vpc";
const _EVD = "EbsVolumeDetails";
const _EVR = "EbsVolumesResult";
const _EVSD = "EbsVolumeScanDetails";
const _EVb = "EbsVolumes";
const _EVbs = "EbsVolume";
const _EVn = "EngineVersion";
const _EVq = "EqualsValue";
const _En = "Enable";
const _End = "Endpoints";
const _Eng = "Engine";
const _Eq = "Eq";
const _Equ = "Equals";
const _Er = "Error";
const _Eu = "Euid";
const _Ev = "Evidence";
const _Ex = "Expression";
const _Exc = "Exclude";
const _F = "Features";
const _FA = "FunctionArn";
const _FC = "FilterCriterion";
const _FCL = "FilterCriterionList";
const _FCi = "FilterCondition";
const _FCil = "FilterCriteria";
const _FCile = "FileCount";
const _FCin = "FindingCriteria";
const _FD = "FargateDetails";
const _FFC = "FailedFileCount";
const _FI = "FindingIds";
const _FL = "FlowLogs";
const _FLA = "FailedLoginAttempts";
const _FLCR = "FlowLogsConfigurationResult";
const _FN = "FilterName";
const _FNe = "FeatureName";
const _FNi = "FilterNames";
const _FNil = "FileName";
const _FNu = "FunctionName";
const _FO = "FileOperation";
const _FP = "FilePath";
const _FPF = "FindingPublishingFrequency";
const _FPi = "FilePaths";
const _FR = "FailureReason";
const _FRC = "FailedResourcesCount";
const _FS = "FindingStatistics";
const _FSA = "FirstSeenAt";
const _FST = "FindingStatisticTypes";
const _FSTi = "FileSystemType";
const _FT = "FindingTypes";
const _FTDR = "FreeTrialDaysRemaining";
const _FTFCR = "FreeTrialFeatureConfigurationResult";
const _FTFCRr = "FreeTrialFeatureConfigurationsResults";
const _FTS = "FindingTypeStatistics";
const _FTi = "FindingType";
const _FV = "FunctionVersion";
const _Fe = "Feature";
const _Fee = "Feedback";
const _Fi = "Finding";
const _Fil = "Filters";
const _File = "Files";
const _Fin = "Findings";
const _Fl = "Flags";
const _Fo = "Format";
const _G = "Guardrails";
const _GA = "GuardrailArn";
const _GAA = "GetAdministratorAccount";
const _GAAR = "GetAdministratorAccountRequest";
const _GAARe = "GetAdministratorAccountResponse";
const _GAu = "GuardrailAction";
const _GB = "GroupBy";
const _GBA = "GroupedByAccount";
const _GBD = "GroupedByDate";
const _GBFT = "GroupedByFindingType";
const _GBR = "GroupedByResource";
const _GBS = "GroupedBySeverity";
const _GCDR = "GetCustomDetectionRule";
const _GCDRA = "GetCustomDetectionRuleAssociation";
const _GCDRAR = "GetCustomDetectionRuleAssociationRequest";
const _GCDRARe = "GetCustomDetectionRuleAssociationResponse";
const _GCDROC = "GetCustomDetectionRuleOrgConfiguration";
const _GCDROCR = "GetCustomDetectionRuleOrgConfigurationRequest";
const _GCDROCRe = "GetCustomDetectionRuleOrgConfigurationResponse";
const _GCDRR = "GetCustomDetectionRuleRequest";
const _GCDRRe = "GetCustomDetectionRuleResponse";
const _GCS = "GetCoverageStatistics";
const _GCSR = "GetCoverageStatisticsRequest";
const _GCSRe = "GetCoverageStatisticsResponse";
const _GD = "GetDetector";
const _GDFI = "GuardDutyFindingId";
const _GDR = "GetDetectorRequest";
const _GDRe = "GetDetectorResponse";
const _GF = "GetFilter";
const _GFR = "GetFilterRequest";
const _GFRe = "GetFilterResponse";
const _GFRet = "GetFindingsRequest";
const _GFReti = "GetFindingsResponse";
const _GFS = "GetFindingsStatistics";
const _GFSR = "GetFindingsStatisticsRequest";
const _GFSRe = "GetFindingsStatisticsResponse";
const _GFe = "GetFindings";
const _GI = "GroupId";
const _GIC = "GetInvitationsCount";
const _GICR = "GetInvitationsCountRequest";
const _GICRe = "GetInvitationsCountResponse";
const _GIPS = "GetIPSet";
const _GIPSR = "GetIPSetRequest";
const _GIPSRe = "GetIPSetResponse";
const _GIR = "GetInvestigationRequest";
const _GIRe = "GetInvestigationResponse";
const _GIe = "GetInvestigation";
const _GL = "GeoLocation";
const _GM = "GetMembers";
const _GMA = "GetMasterAccount";
const _GMAR = "GetMasterAccountRequest";
const _GMARe = "GetMasterAccountResponse";
const _GMD = "GetMemberDetectors";
const _GMDR = "GetMemberDetectorsRequest";
const _GMDRe = "GetMemberDetectorsResponse";
const _GMPP = "GetMalwareProtectionPlan";
const _GMPPR = "GetMalwareProtectionPlanRequest";
const _GMPPRe = "GetMalwareProtectionPlanResponse";
const _GMR = "GetMembersRequest";
const _GMRe = "GetMembersResponse";
const _GMS = "GetMalwareScan";
const _GMSR = "GetMalwareScanRequest";
const _GMSRD = "GetMalwareScanResultDetails";
const _GMSRe = "GetMalwareScanResponse";
const _GMSS = "GetMalwareScanSettings";
const _GMSSR = "GetMalwareScanSettingsRequest";
const _GMSSRe = "GetMalwareScanSettingsResponse";
const _GN = "GroupName";
const _GOS = "GetOrganizationStatistics";
const _GOSR = "GetOrganizationStatisticsResponse";
const _GRFTD = "GetRemainingFreeTrialDays";
const _GRFTDR = "GetRemainingFreeTrialDaysRequest";
const _GRFTDRe = "GetRemainingFreeTrialDaysResponse";
const _GS = "GuardrailSource";
const _GT = "GreaterThan";
const _GTES = "GetThreatEntitySet";
const _GTESR = "GetThreatEntitySetRequest";
const _GTESRe = "GetThreatEntitySetResponse";
const _GTESRet = "GetTrustedEntitySetRequest";
const _GTESRetr = "GetTrustedEntitySetResponse";
const _GTESe = "GetTrustedEntitySet";
const _GTIS = "GetThreatIntelSet";
const _GTISR = "GetThreatIntelSetRequest";
const _GTISRe = "GetThreatIntelSetResponse";
const _GTOE = "GreaterThanOrEqual";
const _GUS = "GetUsageStatistics";
const _GUSR = "GetUsageStatisticsRequest";
const _GUSRe = "GetUsageStatisticsResponse";
const _GV = "GuardrailVersion";
const _Gr = "Group";
const _Gro = "Groups";
const _Gt = "Gt";
const _Gte = "Gte";
const _H = "Hash";
const _HIPC = "HostIPC";
const _HN = "HostNetwork";
const _HP = "HostPath";
const _HPID = "HostPID";
const _HSTD = "HighestSeverityThreatDetails";
const _I = "Id";
const _IA = "InvitedAt";
const _IAI = "IncludeAccountIds";
const _IAV = "IpAddressV4";
const _IAVp = "IpAddressV6";
const _IAm = "ImageArn";
const _IAn = "InstanceArn";
const _IAp = "Ipv6Addresses";
const _IC = "InvitationsCount";
const _ICt = "ItemCount";
const _ID = "ImageDescription";
const _IDL = "ItemDetailsList";
const _IDn = "InstanceDetails";
const _IDt = "ItemDetails";
const _II = "InvitationId";
const _IIP = "IamInstanceProfile";
const _IIPV = "IamInstanceProfileV2";
const _IIm = "ImageId";
const _IIn = "InstanceId";
const _IInv = "InvestigationId";
const _IM = "InvestigationMetadata";
const _IMR = "InviteMembersRequest";
const _IMRn = "InviteMembersResponse";
const _IMn = "InviteMembers";
const _IP = "ImagePrefix";
const _IPA = "IgnorePublicAcls";
const _IPN = "IanaProtocolNumber";
const _IPt = "ItemPath";
const _IPte = "ItemPaths";
const _IS = "InstanceState";
const _ISC = "InvestigationSortCriteria";
const _ISD = "IncrementalScanDetails";
const _ISEE = "InternalServerErrorException";
const _ISI = "IpSetId";
const _ISIp = "IpSetIds";
const _ISn = "InvestigationSummary";
const _ISnv = "InvestigationSummaries";
const _IT = "InstanceType";
const _IU = "ImageUid";
const _IUm = "ImpersonatedUser";
const _Im = "Image";
const _In = "Investigation";
const _Inc = "Include";
const _Ind = "Indicator";
const _Indi = "Indicators";
const _Inv = "Invitation";
const _Inve = "Investigations";
const _Invi = "Invitations";
const _Ip = "Ip";
const _Is = "Issue";
const _Isp = "Isp";
const _Iss = "Issues";
const _Issu = "Issuer";
const _K = "Kubernetes";
const _KACA = "KubernetesApiCallAction";
const _KALC = "KubernetesAuditLogsConfiguration";
const _KALCR = "KubernetesAuditLogsConfigurationResult";
const _KC = "KubernetesConfiguration";
const _KCR = "KubernetesConfigurationResult";
const _KD = "KubernetesDetails";
const _KDSFT = "KubernetesDataSourceFreeTrial";
const _KKA = "KmsKeyArn";
const _KMKA = "KmsMasterKeyArn";
const _KPCD = "KubernetesPermissionCheckedDetails";
const _KRBD = "KubernetesRoleBindingDetails";
const _KRD = "KubernetesRoleDetails";
const _KRT = "KubernetesResourcesTypes";
const _KUD = "KubernetesUserDetails";
const _KW = "KubernetesWorkload";
const _KWD = "KubernetesWorkloadDetails";
const _Ke = "Key";
const _Ki = "Kind";
const _L = "Lt";
const _LA = "LoginAttribute";
const _LAo = "LoginAttributes";
const _LC = "ListCoverage";
const _LCDR = "ListCustomDetectionRules";
const _LCDRA = "ListCustomDetectionRuleAssociations";
const _LCDRAR = "ListCustomDetectionRuleAssociationsRequest";
const _LCDRARi = "ListCustomDetectionRuleAssociationsResponse";
const _LCDROC = "ListCustomDetectionRuleOrgConfigurations";
const _LCDROCR = "ListCustomDetectionRuleOrgConfigurationsRequest";
const _LCDROCRi = "ListCustomDetectionRuleOrgConfigurationsResponse";
const _LCDRR = "ListCustomDetectionRulesRequest";
const _LCDRRi = "ListCustomDetectionRulesResponse";
const _LCR = "ListCoverageRequest";
const _LCRi = "ListCoverageResponse";
const _LD = "LambdaDetails";
const _LDR = "ListDetectorsRequest";
const _LDRi = "ListDetectorsResponse";
const _LDi = "ListDetectors";
const _LF = "ListFilters";
const _LFR = "ListFiltersRequest";
const _LFRi = "ListFiltersResponse";
const _LFRis = "ListFindingsRequest";
const _LFRist = "ListFindingsResponse";
const _LFi = "ListFindings";
const _LGA = "LastGeneratedAt";
const _LI = "ListInvestigations";
const _LID = "LocalIpDetails";
const _LIPS = "ListIPSets";
const _LIPSR = "ListIPSetsRequest";
const _LIPSRi = "ListIPSetsResponse";
const _LIR = "ListInvestigationsRequest";
const _LIRi = "ListInvestigationsResponse";
const _LIRis = "ListInvitationsRequest";
const _LIRist = "ListInvitationsResponse";
const _LIi = "ListInvitations";
const _LM = "ListMembers";
const _LMA = "LastModifiedAt";
const _LMPP = "ListMalwareProtectionPlans";
const _LMPPR = "ListMalwareProtectionPlansRequest";
const _LMPPRi = "ListMalwareProtectionPlansResponse";
const _LMR = "ListMembersRequest";
const _LMRi = "ListMembersResponse";
const _LMS = "ListMalwareScans";
const _LMSCK = "ListMalwareScansCriterionKey";
const _LMSFC = "ListMalwareScansFilterCriteria";
const _LMSFCL = "ListMalwareScansFilterCriterionList";
const _LMSFCi = "ListMalwareScansFilterCriterion";
const _LMSR = "ListMalwareScansRequest";
const _LMSRi = "ListMalwareScansResponse";
const _LNI = "LocalNetworkInterface";
const _LO = "LineageObject";
const _LOAA = "ListOrganizationAdminAccounts";
const _LOAAR = "ListOrganizationAdminAccountsRequest";
const _LOAARi = "ListOrganizationAdminAccountsResponse";
const _LP = "LibraryPath";
const _LPD = "LocalPortDetails";
const _LPDR = "ListPublishingDestinationsRequest";
const _LPDRi = "ListPublishingDestinationsResponse";
const _LPDi = "ListPublishingDestinations";
const _LPV = "LdPreloadValue";
const _LSA = "LastSeenAt";
const _LT = "LessThan";
const _LTES = "ListThreatEntitySets";
const _LTESR = "ListThreatEntitySetsRequest";
const _LTESRi = "ListThreatEntitySetsResponse";
const _LTESRis = "ListTrustedEntitySetsRequest";
const _LTESRist = "ListTrustedEntitySetsResponse";
const _LTESi = "ListTrustedEntitySets";
const _LTFR = "ListTagsForResource";
const _LTFRR = "ListTagsForResourceRequest";
const _LTFRRi = "ListTagsForResourceResponse";
const _LTIS = "ListThreatIntelSets";
const _LTISR = "ListThreatIntelSetsRequest";
const _LTISRi = "ListThreatIntelSetsResponse";
const _LTOE = "LessThanOrEqual";
const _LTa = "LaunchType";
const _LTau = "LaunchTime";
const _La = "Lat";
const _Lan = "Language";
const _Lat = "Latitude";
const _Li = "Lineage";
const _Lo = "Location";
const _Lon = "Lon";
const _Long = "Longitude";
const _Lt = "Lte";
const _M = "Message";
const _MA = "ModifiedAt";
const _MAC = "MemberAdditionalConfiguration";
const _MACR = "MemberAdditionalConfigurationResult";
const _MACRe = "MemberAdditionalConfigurationResults";
const _MACe = "MemberAccountsCount";
const _MACem = "MemberAdditionalConfigurations";
const _MALR = "MemberAccountLimitReached";
const _MD = "ModelDetail";
const _MDSC = "MemberDataSourceConfigurations";
const _MDSCe = "MemberDataSourceConfiguration";
const _MDo = "ModelDetails";
const _ME = "MapEquals";
const _MFC = "MemberFeaturesConfiguration";
const _MFCR = "MemberFeaturesConfigurationResult";
const _MFCRe = "MemberFeaturesConfigurationsResults";
const _MFCe = "MemberFeaturesConfigurations";
const _MFP = "ModuleFilePath";
const _MI = "MasterId";
const _MIo = "ModelId";
const _MN = "ModuleName";
const _MP = "MalwareProtection";
const _MPC = "MalwareProtectionConfiguration";
const _MPCR = "MalwareProtectionConfigurationResult";
const _MPDSFT = "MalwareProtectionDataSourceFreeTrial";
const _MPFSC = "MalwareProtectionFindingsScanConfiguration";
const _MPP = "MalwareProtectionPlans";
const _MPPA = "MalwareProtectionPlanActions";
const _MPPI = "MalwareProtectionPlanId";
const _MPPS = "MalwareProtectionPlanSummary";
const _MPPSR = "MalwareProtectionPlanStatusReason";
const _MPPSRL = "MalwareProtectionPlanStatusReasonsList";
const _MPPSa = "MalwareProtectionPlansSummary";
const _MPPTA = "MalwareProtectionPlanTaggingAction";
const _MPo = "ModifyingProcess";
const _MPou = "MountPath";
const _MR = "MaxResults";
const _MRe = "MemoryRegions";
const _MS = "MalwareScan";
const _MSD = "MalwareScanDetails";
const _MSa = "MalwareScans";
const _MSf = "MfaStatus";
const _MSo = "MountSource";
const _MSod = "ModuleSha256";
const _MT = "ManagementType";
const _MTo = "MountTarget";
const _Ma = "Matches";
const _Mas = "Master";
const _Me = "Members";
const _Mem = "Member";
const _Met = "Metadata";
const _Mo = "Mode";
const _N = "Name";
const _NC = "NetworkConnection";
const _NCA = "NetworkConnectionAction";
const _NE = "NotEquals";
const _NEe = "NetworkEndpoint";
const _NEet = "NetworkEndpoints";
const _NGL = "NetworkGeoLocation";
const _NI = "NetworkInterfaces";
const _NII = "NetworkInterfaceId";
const _NIP = "NestedItemPath";
const _NIe = "NetworkInterface";
const _NM = "NotMatches";
const _NP = "NamespacePid";
const _NT = "NextToken";
const _Na = "Namespace";
const _Ne = "Neq";
const _Nu = "Number";
const _O = "Observations";
const _OA = "OutpostArn";
const _OAC = "OrganizationAdditionalConfiguration";
const _OACR = "OrganizationAdditionalConfigurationResult";
const _OACRr = "OrganizationAdditionalConfigurationResults";
const _OACr = "OrganizationAdditionalConfigurations";
const _OAb = "ObjectArn";
const _OAn = "OnlyAssociated";
const _OB = "OrderBy";
const _OD = "OrganizationDetails";
const _ODSC = "OrganizationDataSourceConfigurations";
const _ODSCR = "OrganizationDataSourceConfigurationsResult";
const _OEV = "OrganizationEbsVolumes";
const _OEVR = "OrganizationEbsVolumesResult";
const _OFC = "OrganizationFeatureConfiguration";
const _OFCR = "OrganizationFeatureConfigurationResult";
const _OFCRr = "OrganizationFeaturesConfigurationsResults";
const _OFCr = "OrganizationFeaturesConfigurations";
const _OFS = "OrganizationFeatureStatistics";
const _OFSAC = "OrganizationFeatureStatisticsAdditionalConfiguration";
const _OFSACr = "OrganizationFeatureStatisticsAdditionalConfigurations";
const _OFSR = "OrganizationFeatureStatisticsResults";
const _OI = "OwnerId";
const _OKALC = "OrganizationKubernetesAuditLogsConfiguration";
const _OKALCR = "OrganizationKubernetesAuditLogsConfigurationResult";
const _OKC = "OrganizationKubernetesConfiguration";
const _OKCR = "OrganizationKubernetesConfigurationResult";
const _OMPC = "OrganizationMalwareProtectionConfiguration";
const _OMPCR = "OrganizationMalwareProtectionConfigurationResult";
const _OP = "ObjectPrefixes";
const _OS = "OrganizationStatistics";
const _OSEIWF = "OrganizationScanEc2InstanceWithFindings";
const _OSEIWFR = "OrganizationScanEc2InstanceWithFindingsResult";
const _OSLC = "OrganizationS3LogsConfiguration";
const _OSLCR = "OrganizationS3LogsConfigurationResult";
const _Op = "Operation";
const _Or = "Organization";
const _Org = "Org";
const _Ow = "Owner";
const _P = "Process";
const _PA = "PublicAccess";
const _PAA = "PublicAclAccess";
const _PAC = "PublicAccessConfiguration";
const _PAIB = "PublicAclIgnoreBehavior";
const _PBRB = "PublicBucketRestrictBehavior";
const _PC = "ProductCodes";
const _PCe = "PermissionConfiguration";
const _PCr = "ProductCode";
const _PD = "ProcessDetails";
const _PDN = "PrivateDnsName";
const _PDNu = "PublicDnsName";
const _PFST = "PublishingFailureStartTimestamp";
const _PI = "PrincipalId";
const _PIA = "PrivateIpAddresses";
const _PIAD = "PrivateIpAddressDetails";
const _PIAr = "PrivateIpAddress";
const _PIu = "PublicIp";
const _PN = "PortName";
const _PPA = "PortProbeAction";
const _PPAu = "PublicPolicyAccess";
const _PPD = "PortProbeDetails";
const _PPDo = "PortProbeDetail";
const _PR = "ProtectedResource";
const _PRA = "PublicReadAccess";
const _PS = "ProfileSubtype";
const _PT = "ProfileType";
const _PTr = "ProductType";
const _PU = "ParentUuid";
const _PWA = "PublicWriteAccess";
const _Pa = "Path";
const _Par = "Partition";
const _Para = "Parameters";
const _Pi = "Pid";
const _Pl = "Platform";
const _Po = "Port";
const _Pr = "Profiles";
const _Pri = "Privileged";
const _Pro = "Provider";
const _Prod = "Product";
const _Prot = "Protocol";
const _Pw = "Pwd";
const _R = "Region";
const _RA = "RuleAssociation";
const _RAD = "RemoteAccountDetails";
const _RAP = "ReleaseAgentPath";
const _RAe = "ResourceArn";
const _RAu = "RuleAssociations";
const _RBP = "RuncBinaryPath";
const _RC = "RuntimeContext";
const _RCIC = "RegisteredContainerInstancesCount";
const _RD = "ResourceDetails";
const _RDID = "RdsDbInstanceDetails";
const _RDUD = "RdsDbUserDetails";
const _RDe = "ResourceData";
const _RDu = "RuleDefinition";
const _RDul = "RuleDetail";
const _RDun = "RuntimeDetails";
const _RFP = "RelatedFilePaths";
const _RI = "RuleId";
const _RID = "RemoteIpDetails";
const _RIe = "ResourceId";
const _RIev = "RevisionId";
const _RL = "RiskLevel";
const _RLAA = "RdsLoginAttemptAction";
const _RLDD = "RdsLimitlessDbDetails";
const _RN = "ResourceName";
const _RNFE = "ResourceNotFoundException";
const _RP = "RecoveryPoint";
const _RPA = "RecoveryPointArn";
const _RPB = "RestrictPublicBuckets";
const _RPD = "RemotePortDetails";
const _RPDe = "RecoveryPointDetails";
const _RR = "ResourceRole";
const _RRK = "RoleRefKind";
const _RRN = "RoleRefName";
const _RS = "RelationshipStatus";
const _RSL = "RuleSummaryList";
const _RSe = "ResourceStatistics";
const _RSu = "RuleSummary";
const _RT = "ResourceType";
const _RTC = "RunningTasksCount";
const _RU = "RequestUri";
const _RUe = "ResourceUids";
const _RV = "ResourceV2";
const _Ra = "Rank";
const _Re = "Reason";
const _Res = "Resource";
const _Reso = "Resources";
const _Resu = "Result";
const _Ri = "Risk";
const _Ro = "Role";
const _Ru = "Rule";
const _Rul = "Rules";
const _S = "Session";
const _SA = "SnapshotArn";
const _SAI = "ServiceAdditionalInfo";
const _SAN = "ServiceAccountName";
const _SAt = "StartedAt";
const _SB = "S3Bucket";
const _SBA = "SumByAccount";
const _SBD = "S3BucketDetails";
const _SBDS = "SumByDataSource";
const _SBDu = "S3BucketDetail";
const _SBF = "SumByFeature";
const _SBR = "SumByResource";
const _SBt = "StartedBy";
const _SC = "SecurityContext";
const _SCA = "ScanCompletedAt";
const _SCCSD = "ScanConfigurationContinuousScanDetails";
const _SCP = "ScanConditionPair";
const _SCRP = "ScanConfigurationRecoveryPoint";
const _SCc = "ScanConfiguration";
const _SCca = "ScanCategory";
const _SCcan = "ScanCondition";
const _SCcanr = "ScanCriterion";
const _SCo = "SortCriteria";
const _SCt = "StatusCode";
const _SD = "ScanDetections";
const _SEIWF = "ScanEc2InstanceWithFindings";
const _SEIWFR = "ScanEc2InstanceWithFindingsResult";
const _SET = "ScanEndTime";
const _SFC = "SkippedFileCount";
const _SFP = "ScanFilePath";
const _SG = "SecurityGroups";
const _SGe = "SecurityGroup";
const _SHFP = "ShellHistoryFilePath";
const _SI = "ScanId";
const _SIC = "ScannedItemCount";
const _SIe = "SequenceIndicators";
const _SIi = "SignalIndicators";
const _SIo = "SourceIps";
const _SIu = "SubnetId";
const _SIub = "SubnetIds";
const _SL = "S3Logs";
const _SLA = "SuccessfulLoginAttempts";
const _SLC = "S3LogsConfiguration";
const _SLCR = "S3LogsConfigurationResult";
const _SMM = "StartMonitoringMembers";
const _SMMR = "StartMonitoringMembersRequest";
const _SMMRt = "StartMonitoringMembersResponse";
const _SMMRto = "StopMonitoringMembersRequest";
const _SMMRtop = "StopMonitoringMembersResponse";
const _SMMt = "StopMonitoringMembers";
const _SMS = "StartMalwareScan";
const _SMSC = "StartMalwareScanConfiguration";
const _SMSR = "StartMalwareScanRequest";
const _SMSRt = "StartMalwareScanResponse";
const _SN = "ServiceName";
const _SNI = "SubNetId";
const _SNe = "SessionName";
const _SO = "S3Object";
const _SOD = "S3ObjectDetails";
const _SODb = "S3ObjectDetail";
const _SOFSOMS = "S3ObjectForSendObjectMalwareScan";
const _SOMS = "SendObjectMalwareScan";
const _SOMSR = "SendObjectMalwareScanRequest";
const _SOMSRe = "SendObjectMalwareScanResponse";
const _SOU = "S3ObjectUids";
const _SP = "ScriptPath";
const _SPo = "SocketPath";
const _SR = "StatusReason";
const _SRA = "ScannedResourceArn";
const _SRC = "ScannedResourcesCount";
const _SRCc = "ScanResourceCriteria";
const _SRCk = "SkippedResourcesCount";
const _SRD = "ScanResultDetails";
const _SRDc = "ScannedResourceDetails";
const _SRS = "ScanResultStatus";
const _SRSc = "ScannedResourceStatus";
const _SRT = "ScannedResourceType";
const _SRTc = "ScanResultThreat";
const _SRTca = "ScanResultThreats";
const _SRc = "ScannedResources";
const _SRca = "ScannedResource";
const _SRcan = "ScanResult";
const _SRe = "ServiceRole";
const _SRt = "StatusReasons";
const _SS = "SensitiveString";
const _SSA = "ScanStartedAt";
const _SSR = "ScanStatusReason";
const _SST = "ScanStartTime";
const _SSc = "ScanStatus";
const _SSe = "SeverityStatistics";
const _ST = "StartTime";
const _STN = "ScanThreatName";
const _STNc = "ScanThreatNames";
const _STc = "ScanType";
const _STt = "StatisticsType";
const _SV = "SchemaVersion";
const _SVD = "ScannedVolumeDetails";
const _SVDk = "SkippedVolumeDetails";
const _Sc = "Scans";
const _Sca = "Scan";
const _Sch = "Schema";
const _Se = "Service";
const _Seq = "Sequence";
const _Sev = "Severity";
const _Sh = "Sha256";
const _Sho = "Shortened";
const _Si = "Signals";
const _Sig = "Signal";
const _So = "Sources";
const _Sou = "Source";
const _Ss = "Ssl";
const _St = "Status";
const _Su = "Summary";
const _Sub = "Subresource";
const _T = "Type";
const _TABF = "TopAccountsByFeature";
const _TAC = "TotalAccountsCount";
const _TB = "TotalBytes";
const _TBr = "TriggeredBy";
const _TC = "ToolCategory";
const _TCA = "TaskCreatedAt";
const _TD = "TaskDetails";
const _TDA = "TaskDefinitionArn";
const _TDBN = "ThreatDetectedByName";
const _TDIC = "ThreatsDetectedItemCount";
const _TDr = "TriggerDetails";
const _TESI = "ThreatEntitySetId";
const _TESIh = "ThreatEntitySetIds";
const _TESIr = "TrustedEntitySetId";
const _TESIru = "TrustedEntitySetIds";
const _TF = "TotalFindings";
const _TFC = "TotalFileCount";
const _TFFC = "ThreatFoundFileCount";
const _TFI = "TriggerFindingId";
const _TFP = "ThreatFilePath";
const _TFS = "ThreatFileSha256";
const _TG = "TotalGb";
const _TID = "ThreatIntelligenceDetails";
const _TIDh = "ThreatIntelligenceDetail";
const _TISI = "ThreatIntelSetId";
const _TISIh = "ThreatIntelSetIds";
const _TK = "TagKeys";
const _TLN = "ThreatListName";
const _TN = "ThreatName";
const _TNh = "ThreatNames";
const _TNo = "ToolName";
const _TP = "TriggerPrompt";
const _TPa = "TargetProcess";
const _TR = "TopResources";
const _TRR = "TagResourceRequest";
const _TRRa = "TagResourceResponse";
const _TRa = "TagResource";
const _TT = "TriggerType";
const _Ta = "Tags";
const _Tac = "Tactic";
const _Tag = "Tagging";
const _Tag_ = "Tag";
const _Te = "Text";
const _Tec = "Technique";
const _Th = "Threats";
const _Thr = "Threat";
const _Ti = "Title";
const _To = "Total";
const _U = "Uid";
const _UA = "UserAgent";
const _UAR = "UsageAccountResult";
const _UARL = "UsageAccountResultList";
const _UAn = "UnprocessedAccounts";
const _UAnp = "UnprocessedAccount";
const _UAp = "UpdatedAt";
const _UC = "UsageCriteria";
const _UCDRA = "UpdateCustomDetectionRuleAssociation";
const _UCDRAR = "UpdateCustomDetectionRuleAssociationRequest";
const _UCDRARp = "UpdateCustomDetectionRuleAssociationResponse";
const _UCDROC = "UpdateCustomDetectionRuleOrgConfiguration";
const _UCDROCR = "UpdateCustomDetectionRuleOrgConfigurationRequest";
const _UCDROCRp = "UpdateCustomDetectionRuleOrgConfigurationResponse";
const _UD = "UpdateDetector";
const _UDR = "UpdateDetectorRequest";
const _UDRp = "UpdateDetectorResponse";
const _UDS = "UnprocessedDataSources";
const _UDSR = "UnprocessedDataSourcesResult";
const _UDSRL = "UsageDataSourceResultList";
const _UDSRs = "UsageDataSourceResult";
const _UF = "UserFeedback";
const _UFF = "UpdateFindingsFeedback";
const _UFFR = "UpdateFindingsFeedbackRequest";
const _UFFRp = "UpdateFindingsFeedbackResponse";
const _UFR = "UnarchiveFindingsRequest";
const _UFRL = "UsageFeatureResultList";
const _UFRn = "UnarchiveFindingsResponse";
const _UFRp = "UpdateFilterRequest";
const _UFRpd = "UpdateFilterResponse";
const _UFRs = "UsageFeatureResult";
const _UFn = "UnarchiveFindings";
const _UFp = "UpdateFilter";
const _UI = "UserId";
const _UIPS = "UpdateIPSet";
const _UIPSR = "UpdateIPSetRequest";
const _UIPSRp = "UpdateIPSetResponse";
const _UMD = "UpdateMemberDetectors";
const _UMDR = "UpdateMemberDetectorsRequest";
const _UMDRp = "UpdateMemberDetectorsResponse";
const _UMPP = "UpdateMalwareProtectionPlan";
const _UMPPR = "UpdateMalwareProtectionPlanRequest";
const _UMSS = "UpdateMalwareScanSettings";
const _UMSSR = "UpdateMalwareScanSettingsRequest";
const _UMSSRp = "UpdateMalwareScanSettingsResponse";
const _UN = "UserName";
const _UOC = "UpdateOrganizationConfiguration";
const _UOCR = "UpdateOrganizationConfigurationRequest";
const _UOCRp = "UpdateOrganizationConfigurationResponse";
const _UPD = "UpdatePublishingDestination";
const _UPDR = "UpdatePublishingDestinationRequest";
const _UPDRp = "UpdatePublishingDestinationResponse";
const _UPR = "UpdateProtectedResource";
const _UR = "UntagResource";
const _URR = "UntagResourceRequest";
const _URRL = "UsageResourceResultList";
const _URRn = "UntagResourceResponse";
const _URRs = "UsageResourceResult";
const _US = "UsageStatistics";
const _USBR = "UpdateS3BucketResource";
const _UST = "UsageStatisticType";
const _UT = "UserType";
const _UTABFL = "UsageTopAccountsByFeatureList";
const _UTAR = "UsageTopAccountResult";
const _UTARL = "UsageTopAccountsResultList";
const _UTARs = "UsageTopAccountsResult";
const _UTC = "UniqueThreatCount";
const _UTES = "UpdateThreatEntitySet";
const _UTESR = "UpdateThreatEntitySetRequest";
const _UTESRp = "UpdateThreatEntitySetResponse";
const _UTESRpd = "UpdateTrustedEntitySetRequest";
const _UTESRpda = "UpdateTrustedEntitySetResponse";
const _UTESp = "UpdateTrustedEntitySet";
const _UTIS = "UpdateThreatIntelSet";
const _UTISR = "UpdateThreatIntelSetRequest";
const _UTISRp = "UpdateThreatIntelSetResponse";
const _UTNC = "UniqueThreatNameCount";
const _Un = "Unusual";
const _Uni = "Unit";
const _Us = "User";
const _Use = "Username";
const _Uu = "Uuid";
const _V = "Version";
const _VA = "VolumeArn";
const _VC = "VpcConfig";
const _VD = "VolumeDetail";
const _VDo = "VolumeDetails";
const _VI = "VersionId";
const _VIp = "VpcId";
const _VM = "VolumeMounts";
const _VMo = "VolumeMount";
const _VOAI = "VpcOwnerAccountId";
const _VSIGB = "VolumeSizeInGB";
const _VT = "VolumeType";
const _Va = "Values";
const _Val = "Value";
const _Ve = "Verb";
const _Vo = "Volumes";
const _Vol = "Volume";
const _a = "account";
const _aA = "adminAccounts";
const _aAC = "activeAccountsCount";
const _aACA = "awsApiCallAction";
const _aAI = "adminAccountId";
const _aASA = "associatedAttackSequenceArn";
const _aASG = "autoscalingAutoScalingGroup";
const _aC = "additionalConfiguration";
const _aCL = "accessControlList";
const _aD = "agentDetails";
const _aDI = "adminDetectorId";
const _aDc = "accountDetails";
const _aDd = "addonDetails";
const _aE = "autoEnable";
const _aEOM = "autoEnableOrganizationMembers";
const _aF = "addressFamily";
const _aI = "administratorId";
const _aIc = "accountId";
const _aIcc = "accountIds";
const _aIct = "actorIds";
const _aId = "additionalInfo";
const _aIs = "associationId";
const _aK = "accessKey";
const _aKD = "accessKeyDetails";
const _aKI = "accessKeyId";
const _aL = "auditLogs";
const _aLP = "accountLevelPermissions";
const _aM = "authMethod";
const _aN = "attributeName";
const _aO = "asnOrg";
const _aPA = "accountPublicAccess";
const _aPE = "allowPrivilegeEscalation";
const _aPRA = "allowsPublicReadAccess";
const _aPWA = "allowsPublicWriteAccess";
const _aR = "affectedResources";
const _aS = "addonStatus";
const _aSC = "activeServicesCount";
const _aST = "additionalSequenceTypes";
const _aSd = "adminStatus";
const _aSu = "autonomousSystem";
const _aT = "actionType";
const _aV = "addonVersion";
const _aVt = "attachedVolumes";
const _aZ = "availabilityZone";
const _ac = "action";
const _acc = "accounts";
const _act = "activate";
const _acti = "actions";
const _activ = "activities";
const _acto = "actors";
const _ad = "administrator";
const _af = "affiliated";
const _al = "allowed";
const _am = "amount";
const _an = "anomaly";
const _ap = "api";
const _app = "application";
const _ar = "arn";
const _arc = "archived";
const _as = "asn";
const _b = "behavior";
const _bGD = "bedrockGuardrailDetails";
const _bLP = "bucketLevelPermissions";
const _bN = "bucketName";
const _bP = "bucketPolicy";
const _bPA = "blockPublicAccess";
const _bPAl = "blockPublicAcls";
const _bPAu = "bucketPublicAccess";
const _bPP = "blockPublicPolicy";
const _bRA = "baselineResourceArn";
const _bVN = "backupVaultName";
const _bl = "blocked";
const _bu = "bucket";
const _c = "client";
const _cA = "createdAt";
const _cAl = "clusterArn";
const _cBCS = "countByCoverageStatus";
const _cBF = "countByFeature";
const _cBRT = "countByResourceType";
const _cBS = "countBySeverity";
const _cC = "countryCode";
const _cCI = "coveredContainerInstances";
const _cCIo = "compatibleContainerInstances";
const _cD = "connectionDirection";
const _cDo = "containerDetails";
const _cID = "containerInstanceDetails";
const _cK = "criterionKey";
const _cLE = "commandLineExample";
const _cN = "cityName";
const _cNl = "clusterName";
const _cNo = "countryName";
const _cNom = "compatibleNodes";
const _cNov = "coveredNodes";
const _cP = "cloudPartition";
const _cPF = "contentPolicyFilters";
const _cR = "containerRuntime";
const _cS = "coverageStatus";
const _cSD = "continuousScanDetails";
const _cSl = "cloudformationStack";
const _cSo = "coverageStatistics";
const _cT = "callerType";
const _cTl = "clientToken";
const _cTlo = "cloudTrail";
const _cTr = "createdTime";
const _cU = "containerUids";
const _cUr = "credentialUid";
const _ci = "city";
const _cl = "cloud";
const _co = "confidence";
const _cod = "code";
const _com = "comments";
const _con = "condition";
const _conf = "configuration";
const _confi = "configurations";
const _conn = "connection";
const _cont = "containers";
const _conta = "container";
const _conte = "context";
const _cou = "count";
const _coun = "country";
const _cr = "criterion";
const _d = "description";
const _dA = "destinationArn";
const _dAe = "definitionArn";
const _dCI = "dbClusterIdentifier";
const _dD = "domainDetails";
const _dEN = "disableEmailNotification";
const _dI = "detectorId";
const _dIA = "dbInstanceArn";
const _dII = "dbInstanceIdentifier";
const _dIe = "destinationId";
const _dIet = "detectorIds";
const _dL = "dnsLogs";
const _dN = "deviceName";
const _dP = "destinationProperties";
const _dRA = "dnsRequestAction";
const _dRI = "dbiResourceId";
const _dS = "dataSources";
const _dSGA = "dbShardGroupArn";
const _dSGI = "dbShardGroupIdentifier";
const _dSGRI = "dbShardGroupResourceId";
const _dSSE = "defaultServerSideEncryption";
const _dSa = "dataSource";
const _dT = "destinationType";
const _dWS = "domainWithSuffix";
const _da = "date";
const _dat = "database";
const _data = "data";
const _de = "destinations";
const _def = "definition";
const _det = "detection";
const _di = "direction";
const _do = "domain";
const _e = "error";
const _eA = "expiresAt";
const _eAC = "enabledAccountsCount";
const _eAI = "excludeAccountIds";
const _eBO = "expectedBucketOwner";
const _eC = "errorCode";
const _eCD = "eksClusterDetails";
const _eCDc = "ecsClusterDetails";
const _eCc = "ecsCluster";
const _eCk = "eksCluster";
const _eD = "errorDetails";
const _eFS = "eventFirstSeen";
const _eI = "ec2Instance";
const _eID = "ec2InstanceDetails";
const _eIDc = "ec2ImageDetails";
const _eIU = "ec2InstanceUids";
const _eIc = "ec2Image";
const _eIn = "endpointIds";
const _eKA = "encryptionKeyArn";
const _eLS = "eventLastSeen";
const _eLT = "ec2LaunchTemplate";
const _eNI = "ec2NetworkInterface";
const _eNIU = "ec2NetworkInterfaceUids";
const _eP = "executablePath";
const _ePf = "effectivePermission";
const _eS = "executableSha256";
const _eSD = "ebsSnapshotDetails";
const _eSP = "ebsSnapshotPreservation";
const _eSb = "ebsSnapshot";
const _eT = "endTime";
const _eTa = "eTag";
const _eTc = "ecsTask";
const _eTn = "encryptionType";
const _eV = "equalsValue";
const _eVD = "ebsVolumeDetails";
const _eVSD = "ebsVolumeScanDetails";
const _eVb = "ebsVolumes";
const _eVbs = "ebsVolume";
const _eVc = "ec2Vpc";
const _eVn = "engineVersion";
const _em = "email";
const _en = "enable";
const _end = "endpoints";
const _eng = "engine";
const _eq = "eq";
const _equ = "equals";
const _eu = "euid";
const _ev = "evidence";
const _ex = "expression";
const _exc = "exclude";
const _f = "features";
const _fA = "functionArn";
const _fC = "filterCriterion";
const _fCi = "filterCondition";
const _fCil = "filterCriteria";
const _fCile = "fileCount";
const _fCin = "findingCriteria";
const _fD = "fargateDetails";
const _fFC = "failedFileCount";
const _fI = "findingIds";
const _fL = "flowLogs";
const _fLA = "failedLoginAttempts";
const _fN = "functionName";
const _fNe = "featureName";
const _fNi = "filterNames";
const _fNil = "fileName";
const _fO = "fileOperation";
const _fP = "filePath";
const _fPF = "findingPublishingFrequency";
const _fPi = "filePaths";
const _fR = "failureReason";
const _fRC = "failedResourcesCount";
const _fS = "findingStatistics";
const _fSA = "firstSeenAt";
const _fST = "findingStatisticTypes";
const _fSTi = "fileSystemType";
const _fT = "findingTypes";
const _fTDR = "freeTrialDaysRemaining";
const _fTi = "findingType";
const _fV = "functionVersion";
const _fe = "feature";
const _fee = "feedback";
const _fi = "findings";
const _fil = "filters";
const _file = "files";
const _fl = "flags";
const _fo = "format";
const _g = "guardrails";
const _gA = "guardrailArn";
const _gAu = "guardrailAction";
const _gB = "groupBy";
const _gBA = "groupedByAccount";
const _gBD = "groupedByDate";
const _gBFT = "groupedByFindingType";
const _gBR = "groupedByResource";
const _gBS = "groupedBySeverity";
const _gDFI = "guardDutyFindingId";
const _gI = "groupId";
const _gL = "geoLocation";
const _gN = "groupName";
const _gS = "guardrailSource";
const _gT = "greaterThan";
const _gTOE = "greaterThanOrEqual";
const _gV = "guardrailVersion";
const _gr = "group";
const _gro = "groups";
const _gt = "gt";
const _gte = "gte";
const _h = "hash";
const _hE = "httpError";
const _hIPC = "hostIPC";
const _hN = "hostNetwork";
const _hP = "hostPath";
const _hPID = "hostPID";
const _hQ = "httpQuery";
const _hSTD = "highestSeverityThreatDetails";
const _ht = "http";
const _i = "id";
const _iA = "invitedAt";
const _iAI = "includeAccountIds";
const _iAV = "ipAddressV4";
const _iAVp = "ipAddressV6";
const _iAm = "imageArn";
const _iAn = "instanceArn";
const _iAp = "ipv6Addresses";
const _iC = "invitationsCount";
const _iCt = "itemCount";
const _iD = "imageDescription";
const _iDn = "instanceDetails";
const _iDt = "itemDetails";
const _iI = "invitationId";
const _iIP = "iamInstanceProfile";
const _iIm = "imageId";
const _iIn = "instanceId";
const _iInv = "investigationId";
const _iP = "imagePrefix";
const _iPA = "ignorePublicAcls";
const _iPN = "ianaProtocolNumber";
const _iPt = "itemPath";
const _iPte = "itemPaths";
const _iS = "instanceState";
const _iSD = "incrementalScanDetails";
const _iSI = "ipSetId";
const _iSIp = "ipSetIds";
const _iT = "instanceType";
const _iTd = "idempotencyToken";
const _iU = "imageUid";
const _iUm = "impersonatedUser";
const _im = "image";
const _in = "investigation";
const _inc = "include";
const _inv = "investigations";
const _invi = "invitations";
const _ip = "ip";
const _is = "issue";
const _isp = "isp";
const _iss = "issues";
const _issu = "issuer";
const _jN = "jsonName";
const _k = "kubernetes";
const _kACA = "kubernetesApiCallAction";
const _kD = "kubernetesDetails";
const _kKA = "kmsKeyArn";
const _kMKA = "kmsMasterKeyArn";
const _kPCD = "kubernetesPermissionCheckedDetails";
const _kRBD = "kubernetesRoleBindingDetails";
const _kRD = "kubernetesRoleDetails";
const _kUD = "kubernetesUserDetails";
const _kW = "kubernetesWorkload";
const _kWD = "kubernetesWorkloadDetails";
const _ke = "key";
const _ki = "kind";
const _l = "lt";
const _lD = "lambdaDetails";
const _lGA = "lastGeneratedAt";
const _lID = "localIpDetails";
const _lMA = "lastModifiedAt";
const _lNI = "localNetworkInterface";
const _lP = "libraryPath";
const _lPD = "localPortDetails";
const _lPV = "ldPreloadValue";
const _lSA = "lastSeenAt";
const _lT = "lessThan";
const _lTOE = "lessThanOrEqual";
const _lTa = "launchType";
const _lTau = "launchTime";
const _la = "lat";
const _lan = "language";
const _li = "lineage";
const _lo = "location";
const _lon = "lon";
const _lt = "lte";
const _m = "message";
const _mA = "modifiedAt";
const _mAC = "memberAccountsCount";
const _mALR = "memberAccountLimitReached";
const _mD = "modelDetails";
const _mE = "mapEquals";
const _mFP = "moduleFilePath";
const _mI = "masterId";
const _mIo = "modelId";
const _mN = "moduleName";
const _mP = "malwareProtection";
const _mPP = "malwareProtectionPlans";
const _mPPI = "malwareProtectionPlanId";
const _mPo = "modifyingProcess";
const _mPou = "mountPath";
const _mR = "maxResults";
const _mRe = "memoryRegions";
const _mS = "mountSource";
const _mSD = "malwareScanDetails";
const _mSf = "mfaStatus";
const _mSo = "moduleSha256";
const _mT = "managementType";
const _mTo = "mountTarget";
const _ma = "matches";
const _mas = "master";
const _me = "members";
const _met = "metadata";
const _mo = "mode";
const _n = "name";
const _nCA = "networkConnectionAction";
const _nE = "notEquals";
const _nI = "networkInterfaces";
const _nII = "networkInterfaceId";
const _nIP = "nestedItemPath";
const _nM = "notMatches";
const _nP = "namespacePid";
const _nT = "nextToken";
const _na = "namespace";
const _ne = "neq";
const _nu = "number";
const _o = "observations";
const _oA = "outpostArn";
const _oAb = "objectArn";
const _oAn = "onlyAssociated";
const _oB = "orderBy";
const _oD = "organizationDetails";
const _oI = "ownerId";
const _oP = "objectPrefixes";
const _oS = "organizationStatistics";
const _op = "operation";
const _or = "org";
const _org = "organization";
const _ow = "owner";
const _p = "process";
const _pA = "publicAccess";
const _pAA = "publicAclAccess";
const _pAIB = "publicAclIgnoreBehavior";
const _pBRB = "publicBucketRestrictBehavior";
const _pC = "productCodes";
const _pCI = "productCodeId";
const _pCT = "productCodeType";
const _pCe = "permissionConfiguration";
const _pDN = "privateDnsName";
const _pDNu = "publicDnsName";
const _pFST = "publishingFailureStartTimestamp";
const _pI = "principalId";
const _pIA = "privateIpAddresses";
const _pIAr = "privateIpAddress";
const _pIu = "publicIp";
const _pN = "portName";
const _pPA = "portProbeAction";
const _pPAu = "publicPolicyAccess";
const _pPD = "portProbeDetails";
const _pR = "protectedResource";
const _pRA = "publicReadAccess";
const _pS = "profileSubtype";
const _pT = "profileType";
const _pU = "parentUuid";
const _pWA = "publicWriteAccess";
const _pa = "path";
const _par = "partition";
const _para = "parameters";
const _pi = "pid";
const _pl = "platform";
const _po = "port";
const _pr = "profiles";
const _pri = "privileged";
const _pro = "provider";
const _prod = "product";
const _prot = "protocol";
const _pw = "pwd";
const _r = "region";
const _rA = "ruleAssociation";
const _rAD = "remoteAccountDetails";
const _rAP = "releaseAgentPath";
const _rAe = "resourceArn";
const _rAu = "ruleAssociations";
const _rBP = "runcBinaryPath";
const _rCIC = "registeredContainerInstancesCount";
const _rD = "resourceDetails";
const _rDID = "rdsDbInstanceDetails";
const _rDUD = "rdsDbUserDetails";
const _rDu = "runtimeDetails";
const _rFP = "relatedFilePaths";
const _rI = "ruleId";
const _rID = "remoteIpDetails";
const _rIe = "resourceId";
const _rIev = "revisionId";
const _rL = "riskLevel";
const _rLAA = "rdsLoginAttemptAction";
const _rLDD = "rdsLimitlessDbDetails";
const _rN = "resourceName";
const _rP = "recoveryPoint";
const _rPA = "recoveryPointArn";
const _rPB = "restrictPublicBuckets";
const _rPD = "remotePortDetails";
const _rPDe = "recoveryPointDetails";
const _rR = "resourceRole";
const _rRK = "roleRefKind";
const _rRN = "roleRefName";
const _rS = "relationshipStatus";
const _rT = "resourceType";
const _rTC = "runningTasksCount";
const _rU = "requestUri";
const _rUe = "resourceUids";
const _ra = "rank";
const _re = "reason";
const _res = "resource";
const _reso = "resources";
const _resu = "result";
const _ri = "risk";
const _ro = "role";
const _ru = "rule";
const _rul = "rules";
const _s = "smithy.ts.sdk.synthetic.com.amazonaws.guardduty";
const _sA = "snapshotArn";
const _sAN = "serviceAccountName";
const _sAt = "startedAt";
const _sB = "s3Bucket";
const _sBA = "sumByAccount";
const _sBD = "s3BucketDetails";
const _sBDS = "sumByDataSource";
const _sBF = "sumByFeature";
const _sBR = "sumByResource";
const _sBt = "startedBy";
const _sC = "securityContext";
const _sCA = "scanCompletedAt";
const _sCc = "scanConfiguration";
const _sCca = "scanCategory";
const _sCo = "sortCriteria";
const _sCt = "statusCode";
const _sD = "scanDetections";
const _sEIWF = "scanEc2InstanceWithFindings";
const _sET = "scanEndTime";
const _sFC = "skippedFileCount";
const _sG = "securityGroups";
const _sHFP = "shellHistoryFilePath";
const _sI = "scanId";
const _sIC = "scannedItemCount";
const _sIP = "sourceIPs";
const _sIe = "sequenceIndicators";
const _sIi = "signalIndicators";
const _sIu = "subnetId";
const _sIub = "subnetIds";
const _sL = "s3Logs";
const _sLA = "successfulLoginAttempts";
const _sN = "serviceName";
const _sNI = "subNetId";
const _sNe = "sessionName";
const _sO = "s3Object";
const _sOD = "s3ObjectDetails";
const _sOU = "s3ObjectUids";
const _sP = "scriptPath";
const _sPo = "socketPath";
const _sR = "statusReason";
const _sRA = "scannedResourceArn";
const _sRC = "scannedResourcesCount";
const _sRCc = "scanResourceCriteria";
const _sRCk = "skippedResourcesCount";
const _sRD = "scanResultDetails";
const _sRS = "scanResultStatus";
const _sRSc = "scannedResourceStatus";
const _sRT = "scannedResourceType";
const _sRc = "scannedResources";
const _sRca = "scanResult";
const _sRe = "serviceRole";
const _sRt = "statusReasons";
const _sS = "scanStatus";
const _sSA = "scanStartedAt";
const _sSR = "scanStatusReason";
const _sST = "scanStartTime";
const _sT = "startTime";
const _sTc = "scanType";
const _sTt = "statisticsType";
const _sV = "schemaVersion";
const _sVD = "scannedVolumeDetails";
const _sVDk = "skippedVolumeDetails";
const _sc = "scans";
const _sch = "schema";
const _se = "server";
const _seq = "sequence";
const _ser = "service";
const _ses = "session";
const _sev = "severity";
const _sh = "sha256";
const _sho = "shortened";
const _si = "signals";
const _so = "sources";
const _sou = "source";
const _ss = "ssl";
const _st = "status";
const _su = "summary";
const _sub = "subresource";
const _t = "type";
const _tABF = "topAccountsByFeature";
const _tAC = "totalAccountsCount";
const _tB = "totalBytes";
const _tBr = "triggeredBy";
const _tC = "toolCategory";
const _tD = "taskDetails";
const _tDA = "taskDefinitionArn";
const _tDBN = "threatDetectedByName";
const _tDIC = "threatsDetectedItemCount";
const _tDr = "triggerDetails";
const _tESI = "threatEntitySetId";
const _tESIh = "threatEntitySetIds";
const _tESIr = "trustedEntitySetId";
const _tESIru = "trustedEntitySetIds";
const _tF = "totalFindings";
const _tFC = "totalFileCount";
const _tFFC = "threatFoundFileCount";
const _tFI = "triggerFindingId";
const _tFP = "threatFilePath";
const _tFS = "threatFileSha256";
const _tG = "totalGb";
const _tID = "threatIntelligenceDetails";
const _tISI = "threatIntelSetId";
const _tISIh = "threatIntelSetIds";
const _tK = "tagKeys";
const _tLN = "threatListName";
const _tN = "threatName";
const _tNh = "threatNames";
const _tNo = "toolName";
const _tP = "triggerPrompt";
const _tPa = "targetProcess";
const _tR = "topResources";
const _tT = "triggerType";
const _ta = "tags";
const _tac = "tactic";
const _tag = "tagging";
const _te = "text";
const _tec = "technique";
const _th = "threats";
const _ti = "title";
const _to = "total";
const _u = "uid";
const _uA = "userAgent";
const _uAn = "unprocessedAccounts";
const _uAp = "updatedAt";
const _uC = "usageCriteria";
const _uDS = "unprocessedDataSources";
const _uF = "userFeedback";
const _uI = "userId";
const _uN = "userName";
const _uS = "usageStatistics";
const _uST = "usageStatisticsType";
const _uT = "userType";
const _uTC = "uniqueThreatCount";
const _uTNC = "uniqueThreatNameCount";
const _un = "unusual";
const _uni = "unit";
const _us = "user";
const _use = "username";
const _uu = "uuid";
const _v = "version";
const _vA = "volumeArn";
const _vC = "vpcConfig";
const _vI = "versionId";
const _vIp = "vpcId";
const _vM = "volumeMounts";
const _vOAI = "vpcOwnerAccountId";
const _vSIGB = "volumeSizeInGB";
const _vT = "volumeType";
const _va = "values";
const _val = "value";
const _ve = "verb";
const _vo = "volumes";
const n0 = "com.amazonaws.guardduty";

// smithy-typescript generated code
import { TypeRegistry } from "@smithy/core/schema";
import type {
  StaticErrorSchema,
  StaticListSchema,
  StaticMapSchema,
  StaticOperationSchema,
  StaticSimpleSchema,
  StaticStructureSchema,
} from "@smithy/types";

import {
  AccessDeniedException,
  BadRequestException,
  ConflictException,
  InternalServerErrorException,
  ResourceNotFoundException,
} from "../models/errors";
import { GuardDutyServiceException } from "../models/GuardDutyServiceException";

/* eslint no-var: 0 */
const _s_registry = TypeRegistry.for(_s);
export var GuardDutyServiceException$: StaticErrorSchema = [-3, _s, "GuardDutyServiceException", 0, [], []];
_s_registry.registerError(GuardDutyServiceException$, GuardDutyServiceException);
const n0_registry = TypeRegistry.for(n0);
export var AccessDeniedException$: StaticErrorSchema = [-3, n0, _ADE,
  { [_e]: _c, [_hE]: 403 },
  [_M, _T],
  [[0, { [_jN]: _m }], [0, { [_jN]: _t }]]
];
n0_registry.registerError(AccessDeniedException$, AccessDeniedException);
export var BadRequestException$: StaticErrorSchema = [-3, n0, _BRE,
  { [_e]: _c, [_hE]: 400 },
  [_M, _T],
  [[0, { [_jN]: _m }], [0, { [_jN]: _t }]]
];
n0_registry.registerError(BadRequestException$, BadRequestException);
export var ConflictException$: StaticErrorSchema = [-3, n0, _CE,
  { [_e]: _c, [_hE]: 409 },
  [_M, _T],
  [[0, { [_jN]: _m }], [0, { [_jN]: _t }]]
];
n0_registry.registerError(ConflictException$, ConflictException);
export var InternalServerErrorException$: StaticErrorSchema = [-3, n0, _ISEE,
  { [_e]: _se, [_hE]: 500 },
  [_M, _T],
  [[0, { [_jN]: _m }], [0, { [_jN]: _t }]]
];
n0_registry.registerError(InternalServerErrorException$, InternalServerErrorException);
export var ResourceNotFoundException$: StaticErrorSchema = [-3, n0, _RNFE,
  { [_e]: _c, [_hE]: 404 },
  [_M, _T],
  [[0, { [_jN]: _m }], [0, { [_jN]: _t }]]
];
n0_registry.registerError(ResourceNotFoundException$, ResourceNotFoundException);
/**
 * TypeRegistry instances containing modeled errors.
 * @internal
 *
 */
export const errorTypeRegistries = [
  _s_registry,
  n0_registry,
]
var Email: StaticSimpleSchema = [0, n0, _E, 8, 0];
var SensitiveString: StaticSimpleSchema = [0, n0, _SS, 8, 0];
export var AcceptAdministratorInvitationRequest$: StaticStructureSchema = [3, n0, _AAIR,
  0,
  [_DI, _AI, _II],
  [[0, 1], [0, { [_jN]: _aI }], [0, { [_jN]: _iI }]], 3
];
export var AcceptAdministratorInvitationResponse$: StaticStructureSchema = [3, n0, _AAIRc,
  0,
  [],
  []
];
export var AcceptInvitationRequest$: StaticStructureSchema = [3, n0, _AIR,
  0,
  [_DI, _MI, _II],
  [[0, 1], [0, { [_jN]: _mI }], [0, { [_jN]: _iI }]], 3
];
export var AcceptInvitationResponse$: StaticStructureSchema = [3, n0, _AIRc,
  0,
  [],
  []
];
export var AccessControlList$: StaticStructureSchema = [3, n0, _ACL,
  0,
  [_APRA, _APWA],
  [[2, { [_jN]: _aPRA }], [2, { [_jN]: _aPWA }]]
];
export var AccessKey$: StaticStructureSchema = [3, n0, _AK,
  0,
  [_PI, _UN, _UT],
  [[0, { [_jN]: _pI }], [0, { [_jN]: _uN }], [0, { [_jN]: _uT }]]
];
export var AccessKeyDetails$: StaticStructureSchema = [3, n0, _AKD,
  0,
  [_AKI, _PI, _UN, _UT],
  [[0, { [_jN]: _aKI }], [0, { [_jN]: _pI }], [0, { [_jN]: _uN }], [0, { [_jN]: _uT }]]
];
export var Account$: StaticStructureSchema = [3, n0, _A,
  0,
  [_U, _N],
  [[0, { [_jN]: _u }], [0, { [_jN]: _a }]], 1
];
export var AccountDetail$: StaticStructureSchema = [3, n0, _AD,
  0,
  [_AIc, _E],
  [[0, { [_jN]: _aIc }], [() => Email, { [_jN]: _em }]], 2
];
export var AccountFreeTrialInfo$: StaticStructureSchema = [3, n0, _AFTI,
  0,
  [_AIc, _DS, _F],
  [[0, { [_jN]: _aIc }], [() => DataSourcesFreeTrial$, { [_jN]: _dS }], [() => FreeTrialFeatureConfigurationsResults, { [_jN]: _f }]]
];
export var AccountLevelPermissions$: StaticStructureSchema = [3, n0, _ALP,
  0,
  [_BPA],
  [[() => BlockPublicAccess$, { [_jN]: _bPA }]]
];
export var AccountStatistics$: StaticStructureSchema = [3, n0, _AS,
  0,
  [_AIc, _LGA, _TF],
  [[0, { [_jN]: _aIc }], [4, { [_jN]: _lGA }], [1, { [_jN]: _tF }]]
];
export var Action$: StaticStructureSchema = [3, n0, _Ac,
  0,
  [_AT, _AACA, _DRA, _NCA, _PPA, _KACA, _KPCD, _KRBD, _KRD, _RLAA],
  [[0, { [_jN]: _aT }], [() => AwsApiCallAction$, { [_jN]: _aACA }], [() => DnsRequestAction$, { [_jN]: _dRA }], [() => NetworkConnectionAction$, { [_jN]: _nCA }], [() => PortProbeAction$, { [_jN]: _pPA }], [() => KubernetesApiCallAction$, { [_jN]: _kACA }], [() => KubernetesPermissionCheckedDetails$, { [_jN]: _kPCD }], [() => KubernetesRoleBindingDetails$, { [_jN]: _kRBD }], [() => KubernetesRoleDetails$, { [_jN]: _kRD }], [() => RdsLoginAttemptAction$, { [_jN]: _rLAA }]]
];
export var Activity$: StaticStructureSchema = [3, n0, _Act,
  0,
  [_T, _Ap],
  [[0, { [_jN]: _t }], [() => ApiCall$, { [_jN]: _ap }]], 1
];
export var Actor$: StaticStructureSchema = [3, n0, _Acto,
  0,
  [_I, _Us, _S, _P],
  [[0, { [_jN]: _i }], [() => User$, { [_jN]: _us }], [() => Session$, { [_jN]: _ses }], [() => ActorProcess$, { [_jN]: _p }]], 1
];
export var ActorProcess$: StaticStructureSchema = [3, n0, _AP,
  0,
  [_N, _Pa, _Sh],
  [[0, { [_jN]: _n }], [0, { [_jN]: _pa }], [0, { [_jN]: _sh }]], 2
];
export var AdditionalInfo$: StaticStructureSchema = [3, n0, _AId,
  0,
  [_VI, _DN],
  [[0, { [_jN]: _vI }], [0, { [_jN]: _dN }]]
];
export var AddonDetails$: StaticStructureSchema = [3, n0, _ADd,
  0,
  [_AV, _ASd],
  [[0, { [_jN]: _aV }], [0, { [_jN]: _aS }]]
];
export var AdminAccount$: StaticStructureSchema = [3, n0, _AA,
  0,
  [_AAI, _ASdm],
  [[0, { [_jN]: _aAI }], [0, { [_jN]: _aSd }]]
];
export var Administrator$: StaticStructureSchema = [3, n0, _Ad,
  0,
  [_AIc, _II, _RS, _IA],
  [[0, { [_jN]: _aIc }], [0, { [_jN]: _iI }], [0, { [_jN]: _rS }], [0, { [_jN]: _iA }]]
];
export var AgentDetails$: StaticStructureSchema = [3, n0, _ADg,
  0,
  [_V],
  [[0, { [_jN]: _v }]]
];
export var Anomaly$: StaticStructureSchema = [3, n0, _An,
  0,
  [_Pr, _Un],
  [[() => AnomalyProfiles, { [_jN]: _pr }], [() => AnomalyUnusual$, { [_jN]: _un }]]
];
export var AnomalyObject$: StaticStructureSchema = [3, n0, _AO,
  0,
  [_PT, _PS, _O],
  [[0, { [_jN]: _pT }], [0, { [_jN]: _pS }], [() => Observations$, { [_jN]: _o }]]
];
export var AnomalyUnusual$: StaticStructureSchema = [3, n0, _AU,
  0,
  [_B],
  [[() => Behavior, { [_jN]: _b }]]
];
export var ApiCall$: StaticStructureSchema = [3, n0, _AC,
  0,
  [_Op, _Se, _Er, _UA],
  [[0, { [_jN]: _op }], [0, { [_jN]: _ser }], [0, { [_jN]: _e }], [0, { [_jN]: _uA }]]
];
export var ArchiveFindingsRequest$: StaticStructureSchema = [3, n0, _AFR,
  0,
  [_DI, _FI],
  [[0, 1], [64 | 0, { [_jN]: _fI }]], 2
];
export var ArchiveFindingsResponse$: StaticStructureSchema = [3, n0, _AFRr,
  0,
  [],
  []
];
export var AssociationDetail$: StaticStructureSchema = [3, n0, _ADs,
  0,
  [_AIs, _Ar, _RI, _AIc, _Mo, _CA, _UAp, _EA],
  [[0, { [_jN]: _aIs }], [0, { [_jN]: _ar }], [0, { [_jN]: _rI }], [0, { [_jN]: _aIc }], [0, { [_jN]: _mo }], [4, { [_jN]: _cA }], [4, { [_jN]: _uAp }], [4, { [_jN]: _eA }]], 7
];
export var AssociationSummary$: StaticStructureSchema = [3, n0, _ASs,
  0,
  [_AIs, _Ar, _RI, _Mo, _CA, _UAp, _EA],
  [[0, { [_jN]: _aIs }], [0, { [_jN]: _ar }], [0, { [_jN]: _rI }], [0, { [_jN]: _mo }], [4, { [_jN]: _cA }], [4, { [_jN]: _uAp }], [4, { [_jN]: _eA }]], 6
];
export var AutonomousSystem$: StaticStructureSchema = [3, n0, _ASu,
  0,
  [_N, _Nu],
  [[0, { [_jN]: _n }], [1, { [_jN]: _nu }]], 2
];
export var AutoscalingAutoScalingGroup$: StaticStructureSchema = [3, n0, _AASG,
  0,
  [_EIU],
  [[64 | 0, { [_jN]: _eIU }]]
];
export var AwsApiCallAction$: StaticStructureSchema = [3, n0, _AACA,
  0,
  [_Ap, _CT, _DD, _EC, _UA, _RID, _SN, _RAD, _AR],
  [[0, { [_jN]: _ap }], [0, { [_jN]: _cT }], [() => DomainDetails$, { [_jN]: _dD }], [0, { [_jN]: _eC }], [0, { [_jN]: _uA }], [() => RemoteIpDetails$, { [_jN]: _rID }], [0, { [_jN]: _sN }], [() => RemoteAccountDetails$, { [_jN]: _rAD }], [128 | 0, { [_jN]: _aR }]]
];
export var BedrockGuardrail$: StaticStructureSchema = [3, n0, _BG,
  0,
  [_Ar, _V],
  [[0, { [_jN]: _ar }], [0, { [_jN]: _v }]]
];
export var BedrockGuardrailDetails$: StaticStructureSchema = [3, n0, _BGD,
  0,
  [_GA, _GV, _G, _GAu, _GS, _CPF],
  [[0, { [_jN]: _gA }], [0, { [_jN]: _gV }], [() => BedrockGuardrails, { [_jN]: _g }], [0, { [_jN]: _gAu }], [0, { [_jN]: _gS }], [() => ContentPolicyFilters, { [_jN]: _cPF }]]
];
export var BlockPublicAccess$: StaticStructureSchema = [3, n0, _BPA,
  0,
  [_IPA, _RPB, _BPAl, _BPP],
  [[2, { [_jN]: _iPA }], [2, { [_jN]: _rPB }], [2, { [_jN]: _bPAl }], [2, { [_jN]: _bPP }]]
];
export var BucketLevelPermissions$: StaticStructureSchema = [3, n0, _BLP,
  0,
  [_ACL, _BP, _BPA],
  [[() => AccessControlList$, { [_jN]: _aCL }], [() => BucketPolicy$, { [_jN]: _bP }], [() => BlockPublicAccess$, { [_jN]: _bPA }]]
];
export var BucketPolicy$: StaticStructureSchema = [3, n0, _BP,
  0,
  [_APRA, _APWA],
  [[2, { [_jN]: _aPRA }], [2, { [_jN]: _aPWA }]]
];
export var City$: StaticStructureSchema = [3, n0, _C,
  0,
  [_CN],
  [[0, { [_jN]: _cN }]]
];
export var CloudDetails$: StaticStructureSchema = [3, n0, _CD,
  0,
  [_Pro, _R, _A],
  [[0, { [_jN]: _pro }], [0, { [_jN]: _r }], [0, { [_jN]: _a }]], 3
];
export var CloudformationStack$: StaticStructureSchema = [3, n0, _CS,
  0,
  [_EIU],
  [[64 | 0, { [_jN]: _eIU }]]
];
export var CloudTrailConfigurationResult$: StaticStructureSchema = [3, n0, _CTCR,
  0,
  [_St],
  [[0, { [_jN]: _st }]], 1
];
export var Condition$: StaticStructureSchema = [3, n0, _Co,
  0,
  [_Eq, _Ne, _Gt, _Gte, _L, _Lt, _Equ, _NE, _GT, _GTOE, _LT, _LTOE, _Ma, _NM],
  [[64 | 0, { [_jN]: _eq }], [64 | 0, { [_jN]: _ne }], [1, { [_jN]: _gt }], [1, { [_jN]: _gte }], [1, { [_jN]: _l }], [1, { [_jN]: _lt }], [64 | 0, { [_jN]: _equ }], [64 | 0, { [_jN]: _nE }], [1, { [_jN]: _gT }], [1, { [_jN]: _gTOE }], [1, { [_jN]: _lT }], [1, { [_jN]: _lTOE }], [64 | 0, { [_jN]: _ma }], [64 | 0, { [_jN]: _nM }]]
];
export var Container$: StaticStructureSchema = [3, n0, _Con,
  0,
  [_CR, _I, _N, _Im, _IP, _VM, _SC],
  [[0, { [_jN]: _cR }], [0, { [_jN]: _i }], [0, { [_jN]: _n }], [0, { [_jN]: _im }], [0, { [_jN]: _iP }], [() => VolumeMounts, { [_jN]: _vM }], [() => SecurityContext$, { [_jN]: _sC }]]
];
export var ContainerFindingResource$: StaticStructureSchema = [3, n0, _CFR,
  0,
  [_Im, _IU],
  [[0, { [_jN]: _im }], [0, { [_jN]: _iU }]], 1
];
export var ContainerInstanceDetails$: StaticStructureSchema = [3, n0, _CID,
  0,
  [_CCI, _CCIo],
  [[1, { [_jN]: _cCI }], [1, { [_jN]: _cCIo }]]
];
export var ContentPolicyFilter$: StaticStructureSchema = [3, n0, _CPFo,
  0,
  [_T, _Conf, _Ac],
  [[0, { [_jN]: _t }], [0, { [_jN]: _co }], [0, { [_jN]: _ac }]]
];
export var ContinuousScanDetails$: StaticStructureSchema = [3, n0, _CSD,
  0,
  [_ET, _ST],
  [[4, { [_jN]: _eT }], [4, { [_jN]: _sT }]], 1
];
export var Country$: StaticStructureSchema = [3, n0, _Cou,
  0,
  [_CC, _CNo],
  [[0, { [_jN]: _cC }], [0, { [_jN]: _cNo }]]
];
export var CoverageEc2InstanceDetails$: StaticStructureSchema = [3, n0, _CEID,
  0,
  [_IIn, _IT, _CAl, _ADg, _MT],
  [[0, { [_jN]: _iIn }], [0, { [_jN]: _iT }], [0, { [_jN]: _cAl }], [() => AgentDetails$, { [_jN]: _aD }], [0, { [_jN]: _mT }]]
];
export var CoverageEcsClusterDetails$: StaticStructureSchema = [3, n0, _CECD,
  0,
  [_CNl, _FD, _CID],
  [[0, { [_jN]: _cNl }], [() => FargateDetails$, { [_jN]: _fD }], [() => ContainerInstanceDetails$, { [_jN]: _cID }]]
];
export var CoverageEksClusterDetails$: StaticStructureSchema = [3, n0, _CECDo,
  0,
  [_CNl, _CNov, _CNom, _ADd, _MT],
  [[0, { [_jN]: _cNl }], [1, { [_jN]: _cNov }], [1, { [_jN]: _cNom }], [() => AddonDetails$, { [_jN]: _aDd }], [0, { [_jN]: _mT }]]
];
export var CoverageFilterCondition$: StaticStructureSchema = [3, n0, _CFC,
  0,
  [_Equ, _NE],
  [[64 | 0, { [_jN]: _equ }], [64 | 0, { [_jN]: _nE }]]
];
export var CoverageFilterCriteria$: StaticStructureSchema = [3, n0, _CFCo,
  0,
  [_FC],
  [[() => CoverageFilterCriterionList, { [_jN]: _fC }]]
];
export var CoverageFilterCriterion$: StaticStructureSchema = [3, n0, _CFCov,
  0,
  [_CK, _FCi],
  [[0, { [_jN]: _cK }], [() => CoverageFilterCondition$, { [_jN]: _fCi }]]
];
export var CoverageResource$: StaticStructureSchema = [3, n0, _CRo,
  0,
  [_RIe, _DI, _AIc, _RD, _CSo, _Is, _UAp],
  [[0, { [_jN]: _rIe }], [0, { [_jN]: _dI }], [0, { [_jN]: _aIc }], [() => CoverageResourceDetails$, { [_jN]: _rD }], [0, { [_jN]: _cS }], [0, { [_jN]: _is }], [4, { [_jN]: _uAp }]]
];
export var CoverageResourceDetails$: StaticStructureSchema = [3, n0, _CRD,
  0,
  [_ECD, _ECDc, _EID, _RT],
  [[() => CoverageEksClusterDetails$, { [_jN]: _eCD }], [() => CoverageEcsClusterDetails$, { [_jN]: _eCDc }], [() => CoverageEc2InstanceDetails$, { [_jN]: _eID }], [0, { [_jN]: _rT }]]
];
export var CoverageSortCriteria$: StaticStructureSchema = [3, n0, _CSC,
  0,
  [_AN, _OB],
  [[0, { [_jN]: _aN }], [0, { [_jN]: _oB }]]
];
export var CoverageStatistics$: StaticStructureSchema = [3, n0, _CSov,
  0,
  [_CBRT, _CBCS],
  [[128 | 1, { [_jN]: _cBRT }], [128 | 1, { [_jN]: _cBCS }]]
];
export var CreateCustomDetectionRuleAssociationRequest$: StaticStructureSchema = [3, n0, _CCDRAR,
  0,
  [_RI, _Mo, _CTl, _Ta],
  [[0, { [_jN]: _rI }], [0, { [_jN]: _mo }], [0, { [_iTd]: 1, [_jN]: _cTl }], [128 | 0, { [_jN]: _ta }]], 2
];
export var CreateCustomDetectionRuleAssociationResponse$: StaticStructureSchema = [3, n0, _CCDRARr,
  0,
  [_RA],
  [[() => AssociationDetail$, { [_jN]: _rA }]], 1
];
export var CreateCustomDetectionRuleOrgConfigurationRequest$: StaticStructureSchema = [3, n0, _CCDROCR,
  0,
  [_RI, _Mo, _IAI, _EAI, _CTl],
  [[0, { [_jN]: _rI }], [0, { [_jN]: _mo }], [64 | 0, { [_jN]: _iAI }], [64 | 0, { [_jN]: _eAI }], [0, { [_iTd]: 1, [_jN]: _cTl }]], 2
];
export var CreateCustomDetectionRuleOrgConfigurationResponse$: StaticStructureSchema = [3, n0, _CCDROCRr,
  0,
  [],
  []
];
export var CreateDetectorRequest$: StaticStructureSchema = [3, n0, _CDR,
  0,
  [_En, _CTl, _FPF, _DS, _Ta, _F],
  [[2, { [_jN]: _en }], [0, { [_iTd]: 1, [_jN]: _cTl }], [0, { [_jN]: _fPF }], [() => DataSourceConfigurations$, { [_jN]: _dS }], [128 | 0, { [_jN]: _ta }], [() => DetectorFeatureConfigurations, { [_jN]: _f }]], 1
];
export var CreateDetectorResponse$: StaticStructureSchema = [3, n0, _CDRr,
  0,
  [_DI, _UDS],
  [[0, { [_jN]: _dI }], [() => UnprocessedDataSourcesResult$, { [_jN]: _uDS }]]
];
export var CreateFilterRequest$: StaticStructureSchema = [3, n0, _CFRr,
  0,
  [_DI, _N, _FCin, _D, _Ac, _Ra, _CTl, _Ta],
  [[0, 1], [0, { [_jN]: _n }], [() => FindingCriteria$, { [_jN]: _fCin }], [0, { [_jN]: _d }], [0, { [_jN]: _ac }], [1, { [_jN]: _ra }], [0, { [_iTd]: 1, [_jN]: _cTl }], [128 | 0, { [_jN]: _ta }]], 3
];
export var CreateFilterResponse$: StaticStructureSchema = [3, n0, _CFRre,
  0,
  [_N],
  [[0, { [_jN]: _n }]], 1
];
export var CreateInvestigationRequest$: StaticStructureSchema = [3, n0, _CIR,
  0,
  [_DI, _TP, _CTl],
  [[0, 1], [0, { [_jN]: _tP }], [0, { [_iTd]: 1, [_jN]: _cTl }]], 2
];
export var CreateInvestigationResponse$: StaticStructureSchema = [3, n0, _CIRr,
  0,
  [_IInv],
  [[0, { [_jN]: _iInv }]], 1
];
export var CreateIPSetRequest$: StaticStructureSchema = [3, n0, _CIPSR,
  0,
  [_DI, _N, _Fo, _Lo, _Acti, _CTl, _Ta, _EBO],
  [[0, 1], [0, { [_jN]: _n }], [0, { [_jN]: _fo }], [0, { [_jN]: _lo }], [2, { [_jN]: _act }], [0, { [_iTd]: 1, [_jN]: _cTl }], [128 | 0, { [_jN]: _ta }], [0, { [_jN]: _eBO }]], 5
];
export var CreateIPSetResponse$: StaticStructureSchema = [3, n0, _CIPSRr,
  0,
  [_ISI],
  [[0, { [_jN]: _iSI }]], 1
];
export var CreateMalwareProtectionPlanRequest$: StaticStructureSchema = [3, n0, _CMPPR,
  0,
  [_Ro, _PR, _CTl, _Actio, _Ta],
  [[0, { [_jN]: _ro }], [() => CreateProtectedResource$, { [_jN]: _pR }], [0, { [_iTd]: 1, [_jN]: _cTl }], [() => MalwareProtectionPlanActions$, { [_jN]: _acti }], [128 | 0, { [_jN]: _ta }]], 2
];
export var CreateMalwareProtectionPlanResponse$: StaticStructureSchema = [3, n0, _CMPPRr,
  0,
  [_MPPI],
  [[0, { [_jN]: _mPPI }]]
];
export var CreateMembersRequest$: StaticStructureSchema = [3, n0, _CMR,
  0,
  [_DI, _ADc],
  [[0, 1], [() => AccountDetails, { [_jN]: _aDc }]], 2
];
export var CreateMembersResponse$: StaticStructureSchema = [3, n0, _CMRr,
  0,
  [_UAn],
  [[() => UnprocessedAccounts, { [_jN]: _uAn }]], 1
];
export var CreateProtectedResource$: StaticStructureSchema = [3, n0, _CPR,
  0,
  [_SB],
  [[() => CreateS3BucketResource$, { [_jN]: _sB }]]
];
export var CreatePublishingDestinationRequest$: StaticStructureSchema = [3, n0, _CPDR,
  0,
  [_DI, _DT, _DP, _CTl, _Ta],
  [[0, 1], [0, { [_jN]: _dT }], [() => DestinationProperties$, { [_jN]: _dP }], [0, { [_iTd]: 1, [_jN]: _cTl }], [128 | 0, { [_jN]: _ta }]], 3
];
export var CreatePublishingDestinationResponse$: StaticStructureSchema = [3, n0, _CPDRr,
  0,
  [_DIe],
  [[0, { [_jN]: _dIe }]], 1
];
export var CreateS3BucketResource$: StaticStructureSchema = [3, n0, _CSBR,
  0,
  [_BN, _OP],
  [[0, { [_jN]: _bN }], [64 | 0, { [_jN]: _oP }]]
];
export var CreateSampleFindingsRequest$: StaticStructureSchema = [3, n0, _CSFR,
  0,
  [_DI, _FT],
  [[0, 1], [64 | 0, { [_jN]: _fT }]], 1
];
export var CreateSampleFindingsResponse$: StaticStructureSchema = [3, n0, _CSFRr,
  0,
  [],
  []
];
export var CreateThreatEntitySetRequest$: StaticStructureSchema = [3, n0, _CTESR,
  0,
  [_DI, _N, _Fo, _Lo, _Acti, _EBO, _CTl, _Ta],
  [[0, 1], [0, { [_jN]: _n }], [0, { [_jN]: _fo }], [0, { [_jN]: _lo }], [2, { [_jN]: _act }], [0, { [_jN]: _eBO }], [0, { [_iTd]: 1, [_jN]: _cTl }], [128 | 0, { [_jN]: _ta }]], 5
];
export var CreateThreatEntitySetResponse$: StaticStructureSchema = [3, n0, _CTESRr,
  0,
  [_TESI],
  [[0, { [_jN]: _tESI }]], 1
];
export var CreateThreatIntelSetRequest$: StaticStructureSchema = [3, n0, _CTISR,
  0,
  [_DI, _N, _Fo, _Lo, _Acti, _CTl, _Ta, _EBO],
  [[0, 1], [0, { [_jN]: _n }], [0, { [_jN]: _fo }], [0, { [_jN]: _lo }], [2, { [_jN]: _act }], [0, { [_iTd]: 1, [_jN]: _cTl }], [128 | 0, { [_jN]: _ta }], [0, { [_jN]: _eBO }]], 5
];
export var CreateThreatIntelSetResponse$: StaticStructureSchema = [3, n0, _CTISRr,
  0,
  [_TISI],
  [[0, { [_jN]: _tISI }]], 1
];
export var CreateTrustedEntitySetRequest$: StaticStructureSchema = [3, n0, _CTESRre,
  0,
  [_DI, _N, _Fo, _Lo, _Acti, _EBO, _CTl, _Ta],
  [[0, 1], [0, { [_jN]: _n }], [0, { [_jN]: _fo }], [0, { [_jN]: _lo }], [2, { [_jN]: _act }], [0, { [_jN]: _eBO }], [0, { [_iTd]: 1, [_jN]: _cTl }], [128 | 0, { [_jN]: _ta }]], 5
];
export var CreateTrustedEntitySetResponse$: StaticStructureSchema = [3, n0, _CTESRrea,
  0,
  [_TESIr],
  [[0, { [_jN]: _tESIr }]], 1
];
export var DataSourceConfigurations$: StaticStructureSchema = [3, n0, _DSC,
  0,
  [_SL, _K, _MP],
  [[() => S3LogsConfiguration$, { [_jN]: _sL }], [() => KubernetesConfiguration$, { [_jN]: _k }], [() => MalwareProtectionConfiguration$, { [_jN]: _mP }]]
];
export var DataSourceConfigurationsResult$: StaticStructureSchema = [3, n0, _DSCR,
  0,
  [_CTlo, _DNSL, _FL, _SL, _K, _MP],
  [[() => CloudTrailConfigurationResult$, { [_jN]: _cTlo }], [() => DNSLogsConfigurationResult$, { [_jN]: _dL }], [() => FlowLogsConfigurationResult$, { [_jN]: _fL }], [() => S3LogsConfigurationResult$, { [_jN]: _sL }], [() => KubernetesConfigurationResult$, { [_jN]: _k }], [() => MalwareProtectionConfigurationResult$, { [_jN]: _mP }]], 4
];
export var DataSourceFreeTrial$: StaticStructureSchema = [3, n0, _DSFT,
  0,
  [_FTDR],
  [[1, { [_jN]: _fTDR }]]
];
export var DataSourcesFreeTrial$: StaticStructureSchema = [3, n0, _DSFTa,
  0,
  [_CTlo, _DL, _FL, _SL, _K, _MP],
  [[() => DataSourceFreeTrial$, { [_jN]: _cTlo }], [() => DataSourceFreeTrial$, { [_jN]: _dL }], [() => DataSourceFreeTrial$, { [_jN]: _fL }], [() => DataSourceFreeTrial$, { [_jN]: _sL }], [() => KubernetesDataSourceFreeTrial$, { [_jN]: _k }], [() => MalwareProtectionDataSourceFreeTrial$, { [_jN]: _mP }]]
];
export var DateStatistics$: StaticStructureSchema = [3, n0, _DSa,
  0,
  [_Da, _LGA, _Sev, _TF],
  [[4, { [_jN]: _da }], [4, { [_jN]: _lGA }], [1, { [_jN]: _sev }], [1, { [_jN]: _tF }]]
];
export var DeclineInvitationsRequest$: StaticStructureSchema = [3, n0, _DIR,
  0,
  [_AIcc],
  [[64 | 0, { [_jN]: _aIcc }]], 1
];
export var DeclineInvitationsResponse$: StaticStructureSchema = [3, n0, _DIRe,
  0,
  [_UAn],
  [[() => UnprocessedAccounts, { [_jN]: _uAn }]], 1
];
export var DefaultServerSideEncryption$: StaticStructureSchema = [3, n0, _DSSE,
  0,
  [_ETn, _KMKA],
  [[0, { [_jN]: _eTn }], [0, { [_jN]: _kMKA }]]
];
export var DeleteCustomDetectionRuleAssociationRequest$: StaticStructureSchema = [3, n0, _DCDRAR,
  0,
  [_RI, _AIs],
  [[0, 1], [0, 1]], 2
];
export var DeleteCustomDetectionRuleAssociationResponse$: StaticStructureSchema = [3, n0, _DCDRARe,
  0,
  [],
  []
];
export var DeleteCustomDetectionRuleOrgConfigurationRequest$: StaticStructureSchema = [3, n0, _DCDROCR,
  0,
  [_RI, _Mo],
  [[0, 1], [0, { [_hQ]: _mo }]], 2
];
export var DeleteCustomDetectionRuleOrgConfigurationResponse$: StaticStructureSchema = [3, n0, _DCDROCRe,
  0,
  [],
  []
];
export var DeleteDetectorRequest$: StaticStructureSchema = [3, n0, _DDR,
  0,
  [_DI],
  [[0, 1]], 1
];
export var DeleteDetectorResponse$: StaticStructureSchema = [3, n0, _DDRe,
  0,
  [],
  []
];
export var DeleteFilterRequest$: StaticStructureSchema = [3, n0, _DFR,
  0,
  [_DI, _FN],
  [[0, 1], [0, 1]], 2
];
export var DeleteFilterResponse$: StaticStructureSchema = [3, n0, _DFRe,
  0,
  [],
  []
];
export var DeleteInvitationsRequest$: StaticStructureSchema = [3, n0, _DIRel,
  0,
  [_AIcc],
  [[64 | 0, { [_jN]: _aIcc }]], 1
];
export var DeleteInvitationsResponse$: StaticStructureSchema = [3, n0, _DIRele,
  0,
  [_UAn],
  [[() => UnprocessedAccounts, { [_jN]: _uAn }]], 1
];
export var DeleteIPSetRequest$: StaticStructureSchema = [3, n0, _DIPSR,
  0,
  [_DI, _ISI],
  [[0, 1], [0, 1]], 2
];
export var DeleteIPSetResponse$: StaticStructureSchema = [3, n0, _DIPSRe,
  0,
  [],
  []
];
export var DeleteMalwareProtectionPlanRequest$: StaticStructureSchema = [3, n0, _DMPPR,
  0,
  [_MPPI],
  [[0, 1]], 1
];
export var DeleteMembersRequest$: StaticStructureSchema = [3, n0, _DMR,
  0,
  [_DI, _AIcc],
  [[0, 1], [64 | 0, { [_jN]: _aIcc }]], 2
];
export var DeleteMembersResponse$: StaticStructureSchema = [3, n0, _DMRe,
  0,
  [_UAn],
  [[() => UnprocessedAccounts, { [_jN]: _uAn }]], 1
];
export var DeletePublishingDestinationRequest$: StaticStructureSchema = [3, n0, _DPDR,
  0,
  [_DI, _DIe],
  [[0, 1], [0, 1]], 2
];
export var DeletePublishingDestinationResponse$: StaticStructureSchema = [3, n0, _DPDRe,
  0,
  [],
  []
];
export var DeleteThreatEntitySetRequest$: StaticStructureSchema = [3, n0, _DTESR,
  0,
  [_DI, _TESI],
  [[0, 1], [0, 1]], 2
];
export var DeleteThreatEntitySetResponse$: StaticStructureSchema = [3, n0, _DTESRe,
  0,
  [],
  []
];
export var DeleteThreatIntelSetRequest$: StaticStructureSchema = [3, n0, _DTISR,
  0,
  [_DI, _TISI],
  [[0, 1], [0, 1]], 2
];
export var DeleteThreatIntelSetResponse$: StaticStructureSchema = [3, n0, _DTISRe,
  0,
  [],
  []
];
export var DeleteTrustedEntitySetRequest$: StaticStructureSchema = [3, n0, _DTESRel,
  0,
  [_DI, _TESIr],
  [[0, 1], [0, 1]], 2
];
export var DeleteTrustedEntitySetResponse$: StaticStructureSchema = [3, n0, _DTESRele,
  0,
  [],
  []
];
export var DescribeMalwareScansRequest$: StaticStructureSchema = [3, n0, _DMSR,
  0,
  [_DI, _NT, _MR, _FCil, _SCo],
  [[0, 1], [0, { [_jN]: _nT }], [1, { [_jN]: _mR }], [() => FilterCriteria$, { [_jN]: _fCil }], [() => SortCriteria$, { [_jN]: _sCo }]], 1
];
export var DescribeMalwareScansResponse$: StaticStructureSchema = [3, n0, _DMSRe,
  0,
  [_Sc, _NT],
  [[() => Scans, { [_jN]: _sc }], [0, { [_jN]: _nT }]], 1
];
export var DescribeOrganizationConfigurationRequest$: StaticStructureSchema = [3, n0, _DOCR,
  0,
  [_DI, _MR, _NT],
  [[0, 1], [1, { [_hQ]: _mR }], [0, { [_hQ]: _nT }]], 1
];
export var DescribeOrganizationConfigurationResponse$: StaticStructureSchema = [3, n0, _DOCRe,
  0,
  [_MALR, _AE, _DS, _F, _NT, _AEOM],
  [[2, { [_jN]: _mALR }], [2, { [_jN]: _aE }], [() => OrganizationDataSourceConfigurationsResult$, { [_jN]: _dS }], [() => OrganizationFeaturesConfigurationsResults, { [_jN]: _f }], [0, { [_jN]: _nT }], [0, { [_jN]: _aEOM }]], 1
];
export var DescribePublishingDestinationRequest$: StaticStructureSchema = [3, n0, _DPDRes,
  0,
  [_DI, _DIe],
  [[0, 1], [0, 1]], 2
];
export var DescribePublishingDestinationResponse$: StaticStructureSchema = [3, n0, _DPDResc,
  0,
  [_DIe, _DT, _St, _PFST, _DP, _Ta],
  [[0, { [_jN]: _dIe }], [0, { [_jN]: _dT }], [0, { [_jN]: _st }], [1, { [_jN]: _pFST }], [() => DestinationProperties$, { [_jN]: _dP }], [128 | 0, { [_jN]: _ta }]], 5
];
export var Destination$: StaticStructureSchema = [3, n0, _De,
  0,
  [_DIe, _DT, _St],
  [[0, { [_jN]: _dIe }], [0, { [_jN]: _dT }], [0, { [_jN]: _st }]], 3
];
export var DestinationProperties$: StaticStructureSchema = [3, n0, _DP,
  0,
  [_DA, _KKA],
  [[0, { [_jN]: _dA }], [0, { [_jN]: _kKA }]]
];
export var Detection$: StaticStructureSchema = [3, n0, _Det,
  0,
  [_An, _Seq],
  [[() => Anomaly$, { [_jN]: _an }], [() => Sequence$, { [_jN]: _seq }]]
];
export var DetectionRuleFilter$: StaticStructureSchema = [3, n0, _DRF,
  0,
  [_N, _Va, _Co],
  [[0, { [_jN]: _n }], [64 | 0, { [_jN]: _va }], [0, { [_jN]: _con }]], 2
];
export var DetectionRuleOrgConfiguration$: StaticStructureSchema = [3, n0, _DROC,
  0,
  [_RI, _Mo, _St, _IAI, _EAI, _CA, _UAp, _SR, _EA],
  [[0, { [_jN]: _rI }], [0, { [_jN]: _mo }], [0, { [_jN]: _st }], [64 | 0, { [_jN]: _iAI }], [64 | 0, { [_jN]: _eAI }], [4, { [_jN]: _cA }], [4, { [_jN]: _uAp }], [0, { [_jN]: _sR }], [4, { [_jN]: _eA }]], 7
];
export var DetectionRuleOrgConfigurationSummary$: StaticStructureSchema = [3, n0, _DROCS,
  0,
  [_RI, _Mo, _St, _CA, _UAp, _SR, _EA],
  [[0, { [_jN]: _rI }], [0, { [_jN]: _mo }], [0, { [_jN]: _st }], [4, { [_jN]: _cA }], [4, { [_jN]: _uAp }], [0, { [_jN]: _sR }], [4, { [_jN]: _eA }]], 5
];
export var DetectorAdditionalConfiguration$: StaticStructureSchema = [3, n0, _DAC,
  0,
  [_N, _St],
  [[0, { [_jN]: _n }], [0, { [_jN]: _st }]]
];
export var DetectorAdditionalConfigurationResult$: StaticStructureSchema = [3, n0, _DACR,
  0,
  [_N, _St, _UAp],
  [[0, { [_jN]: _n }], [0, { [_jN]: _st }], [4, { [_jN]: _uAp }]]
];
export var DetectorFeatureConfiguration$: StaticStructureSchema = [3, n0, _DFC,
  0,
  [_N, _St, _ACd],
  [[0, { [_jN]: _n }], [0, { [_jN]: _st }], [() => DetectorAdditionalConfigurations, { [_jN]: _aC }]]
];
export var DetectorFeatureConfigurationResult$: StaticStructureSchema = [3, n0, _DFCR,
  0,
  [_N, _St, _UAp, _ACd],
  [[0, { [_jN]: _n }], [0, { [_jN]: _st }], [4, { [_jN]: _uAp }], [() => DetectorAdditionalConfigurationResults, { [_jN]: _aC }]]
];
export var DisableOrganizationAdminAccountRequest$: StaticStructureSchema = [3, n0, _DOAAR,
  0,
  [_AAI],
  [[0, { [_jN]: _aAI }]], 1
];
export var DisableOrganizationAdminAccountResponse$: StaticStructureSchema = [3, n0, _DOAARi,
  0,
  [],
  []
];
export var DisassociateFromAdministratorAccountRequest$: StaticStructureSchema = [3, n0, _DFAAR,
  0,
  [_DI],
  [[0, 1]], 1
];
export var DisassociateFromAdministratorAccountResponse$: StaticStructureSchema = [3, n0, _DFAARi,
  0,
  [],
  []
];
export var DisassociateFromMasterAccountRequest$: StaticStructureSchema = [3, n0, _DFMAR,
  0,
  [_DI],
  [[0, 1]], 1
];
export var DisassociateFromMasterAccountResponse$: StaticStructureSchema = [3, n0, _DFMARi,
  0,
  [],
  []
];
export var DisassociateMembersRequest$: StaticStructureSchema = [3, n0, _DMRi,
  0,
  [_DI, _AIcc],
  [[0, 1], [64 | 0, { [_jN]: _aIcc }]], 2
];
export var DisassociateMembersResponse$: StaticStructureSchema = [3, n0, _DMRis,
  0,
  [_UAn],
  [[() => UnprocessedAccounts, { [_jN]: _uAn }]], 1
];
export var DNSLogsConfigurationResult$: StaticStructureSchema = [3, n0, _DNSLCR,
  0,
  [_St],
  [[0, { [_jN]: _st }]], 1
];
export var DnsRequestAction$: StaticStructureSchema = [3, n0, _DRA,
  0,
  [_Do, _Prot, _Bl, _DWS, _VOAI],
  [[0, { [_jN]: _do }], [0, { [_jN]: _prot }], [2, { [_jN]: _bl }], [0, { [_jN]: _dWS }], [0, { [_jN]: _vOAI }]]
];
export var DomainDetails$: StaticStructureSchema = [3, n0, _DD,
  0,
  [_Do],
  [[0, { [_jN]: _do }]]
];
export var EbsSnapshot$: StaticStructureSchema = [3, n0, _ES,
  0,
  [_DN],
  [[0, { [_jN]: _dN }]]
];
export var EbsSnapshotDetails$: StaticStructureSchema = [3, n0, _ESD,
  0,
  [_SA],
  [[0, { [_jN]: _sA }]]
];
export var EbsVolumeDetails$: StaticStructureSchema = [3, n0, _EVD,
  0,
  [_SVD, _SVDk],
  [[() => VolumeDetails, { [_jN]: _sVD }], [() => VolumeDetails, { [_jN]: _sVDk }]]
];
export var EbsVolumeScanDetails$: StaticStructureSchema = [3, n0, _EVSD,
  0,
  [_SI, _SSA, _SCA, _TFI, _So, _SD, _STc],
  [[0, { [_jN]: _sI }], [4, { [_jN]: _sSA }], [4, { [_jN]: _sCA }], [0, { [_jN]: _tFI }], [64 | 0, { [_jN]: _so }], [() => ScanDetections$, { [_jN]: _sD }], [0, { [_jN]: _sTc }]]
];
export var EbsVolumesResult$: StaticStructureSchema = [3, n0, _EVR,
  0,
  [_St, _Re],
  [[0, { [_jN]: _st }], [0, { [_jN]: _re }]]
];
export var Ec2Image$: StaticStructureSchema = [3, n0, _EI,
  0,
  [_EIU],
  [[64 | 0, { [_jN]: _eIU }]]
];
export var Ec2ImageDetails$: StaticStructureSchema = [3, n0, _EIDc,
  0,
  [_IAm],
  [[0, { [_jN]: _iAm }]]
];
export var Ec2Instance$: StaticStructureSchema = [3, n0, _EIc,
  0,
  [_AZ, _ID, _IS, _IIP, _IT, _OA, _Pl, _PC, _ENIU],
  [[0, { [_jN]: _aZ }], [0, { [_jN]: _iD }], [0, { [_jN]: _iS }], [() => IamInstanceProfile$, 0], [0, { [_jN]: _iT }], [0, { [_jN]: _oA }], [0, { [_jN]: _pl }], [() => ProductCodes, { [_jN]: _pC }], [64 | 0, { [_jN]: _eNIU }]]
];
export var Ec2LaunchTemplate$: StaticStructureSchema = [3, n0, _ELT,
  0,
  [_EIU, _V],
  [[64 | 0, { [_jN]: _eIU }], [0, { [_jN]: _v }]]
];
export var Ec2NetworkInterface$: StaticStructureSchema = [3, n0, _ENI,
  0,
  [_IAp, _PIA, _PIu, _SG, _SNI, _VIp],
  [[64 | 0, { [_jN]: _iAp }], [() => PrivateIpAddresses, { [_jN]: _pIA }], [0, { [_jN]: _pIu }], [() => SecurityGroups, { [_jN]: _sG }], [0, { [_jN]: _sNI }], [0, { [_jN]: _vIp }]]
];
export var Ec2Vpc$: StaticStructureSchema = [3, n0, _EV,
  0,
  [_EIU],
  [[64 | 0, { [_jN]: _eIU }]]
];
export var EcsCluster$: StaticStructureSchema = [3, n0, _ECc,
  0,
  [_St, _EIU],
  [[0, { [_jN]: _st }], [64 | 0, { [_jN]: _eIU }]]
];
export var EcsClusterDetails$: StaticStructureSchema = [3, n0, _ECDc,
  0,
  [_N, _Ar, _St, _ASC, _RCIC, _RTC, _Ta, _TD],
  [[0, { [_jN]: _n }], [0, { [_jN]: _ar }], [0, { [_jN]: _st }], [1, { [_jN]: _aSC }], [1, { [_jN]: _rCIC }], [1, { [_jN]: _rTC }], [() => Tags, { [_jN]: _ta }], [() => EcsTaskDetails$, { [_jN]: _tD }]]
];
export var EcsTask$: StaticStructureSchema = [3, n0, _ETc,
  0,
  [_CA, _TDA, _LTa, _CU],
  [[4, { [_jN]: _cA }], [0, { [_jN]: _tDA }], [0, { [_jN]: _lTa }], [64 | 0, { [_jN]: _cU }]]
];
export var EcsTaskDetails$: StaticStructureSchema = [3, n0, _ETD,
  0,
  [_Ar, _DAe, _V, _TCA, _SAt, _SBt, _Ta, _Vo, _Cont, _Gr, _LTa],
  [[0, { [_jN]: _ar }], [0, { [_jN]: _dAe }], [0, { [_jN]: _v }], [4, { [_jN]: _cA }], [4, { [_jN]: _sAt }], [0, { [_jN]: _sBt }], [() => Tags, { [_jN]: _ta }], [() => Volumes, { [_jN]: _vo }], [() => Containers, { [_jN]: _cont }], [0, { [_jN]: _gr }], [0, { [_jN]: _lTa }]]
];
export var EksCluster$: StaticStructureSchema = [3, n0, _ECk,
  0,
  [_Ar, _CA, _St, _VIp, _EIU],
  [[0, { [_jN]: _ar }], [4, { [_jN]: _cA }], [0, { [_jN]: _st }], [0, { [_jN]: _vIp }], [64 | 0, { [_jN]: _eIU }]]
];
export var EksClusterDetails$: StaticStructureSchema = [3, n0, _ECD,
  0,
  [_N, _Ar, _VIp, _St, _Ta, _CA],
  [[0, { [_jN]: _n }], [0, { [_jN]: _ar }], [0, { [_jN]: _vIp }], [0, { [_jN]: _st }], [() => Tags, { [_jN]: _ta }], [4, { [_jN]: _cA }]]
];
export var EnableOrganizationAdminAccountRequest$: StaticStructureSchema = [3, n0, _EOAAR,
  0,
  [_AAI],
  [[0, { [_jN]: _aAI }]], 1
];
export var EnableOrganizationAdminAccountResponse$: StaticStructureSchema = [3, n0, _EOAARn,
  0,
  [],
  []
];
export var Evidence$: StaticStructureSchema = [3, n0, _Ev,
  0,
  [_TID],
  [[() => ThreatIntelligenceDetails, { [_jN]: _tID }]]
];
export var FargateDetails$: StaticStructureSchema = [3, n0, _FD,
  0,
  [_Iss, _MT],
  [[64 | 0, { [_jN]: _iss }], [0, { [_jN]: _mT }]]
];
export var FilterCondition$: StaticStructureSchema = [3, n0, _FCi,
  0,
  [_EVq, _GT, _LT],
  [[0, { [_jN]: _eV }], [1, { [_jN]: _gT }], [1, { [_jN]: _lT }]]
];
export var FilterCriteria$: StaticStructureSchema = [3, n0, _FCil,
  0,
  [_FC],
  [[() => FilterCriterionList, { [_jN]: _fC }]]
];
export var FilterCriterion$: StaticStructureSchema = [3, n0, _FC,
  0,
  [_CK, _FCi],
  [[0, { [_jN]: _cK }], [() => FilterCondition$, { [_jN]: _fCi }]]
];
export var Finding$: StaticStructureSchema = [3, n0, _Fi,
  0,
  [_AIc, _Ar, _CA, _I, _R, _Res, _SV, _Sev, _T, _UAp, _Conf, _D, _Par, _Se, _Ti, _AASA],
  [[0, { [_jN]: _aIc }], [0, { [_jN]: _ar }], [0, { [_jN]: _cA }], [0, { [_jN]: _i }], [0, { [_jN]: _r }], [() => Resource$, { [_jN]: _res }], [0, { [_jN]: _sV }], [1, { [_jN]: _sev }], [0, { [_jN]: _t }], [0, { [_jN]: _uAp }], [1, { [_jN]: _co }], [0, { [_jN]: _d }], [0, { [_jN]: _par }], [() => Service$, { [_jN]: _ser }], [0, { [_jN]: _ti }], [0, { [_jN]: _aASA }]], 10
];
export var FindingCriteria$: StaticStructureSchema = [3, n0, _FCin,
  0,
  [_Cr],
  [[() => Criterion, { [_jN]: _cr }]]
];
export var FindingStatistics$: StaticStructureSchema = [3, n0, _FS,
  0,
  [_CBS, _GBA, _GBD, _GBFT, _GBR, _GBS],
  [[128 | 1, { [_jN]: _cBS }], [() => GroupedByAccount, { [_jN]: _gBA }], [() => GroupedByDate, { [_jN]: _gBD }], [() => GroupedByFindingType, { [_jN]: _gBFT }], [() => GroupedByResource, { [_jN]: _gBR }], [() => GroupedBySeverity, { [_jN]: _gBS }]]
];
export var FindingTypeStatistics$: StaticStructureSchema = [3, n0, _FTS,
  0,
  [_FTi, _LGA, _TF],
  [[0, { [_jN]: _fTi }], [4, { [_jN]: _lGA }], [1, { [_jN]: _tF }]]
];
export var FlowLogsConfigurationResult$: StaticStructureSchema = [3, n0, _FLCR,
  0,
  [_St],
  [[0, { [_jN]: _st }]], 1
];
export var FreeTrialFeatureConfigurationResult$: StaticStructureSchema = [3, n0, _FTFCR,
  0,
  [_N, _FTDR],
  [[0, { [_jN]: _n }], [1, { [_jN]: _fTDR }]]
];
export var GeoLocation$: StaticStructureSchema = [3, n0, _GL,
  0,
  [_La, _Lon],
  [[1, { [_jN]: _la }], [1, { [_jN]: _lon }]]
];
export var GetAdministratorAccountRequest$: StaticStructureSchema = [3, n0, _GAAR,
  0,
  [_DI],
  [[0, 1]], 1
];
export var GetAdministratorAccountResponse$: StaticStructureSchema = [3, n0, _GAARe,
  0,
  [_Ad],
  [[() => Administrator$, { [_jN]: _ad }]], 1
];
export var GetCoverageStatisticsRequest$: StaticStructureSchema = [3, n0, _GCSR,
  0,
  [_DI, _STt, _FCil],
  [[0, 1], [64 | 0, { [_jN]: _sTt }], [() => CoverageFilterCriteria$, { [_jN]: _fCil }]], 2
];
export var GetCoverageStatisticsResponse$: StaticStructureSchema = [3, n0, _GCSRe,
  0,
  [_CSov],
  [[() => CoverageStatistics$, { [_jN]: _cSo }]]
];
export var GetCustomDetectionRuleAssociationRequest$: StaticStructureSchema = [3, n0, _GCDRAR,
  0,
  [_RI, _AIs],
  [[0, 1], [0, 1]], 2
];
export var GetCustomDetectionRuleAssociationResponse$: StaticStructureSchema = [3, n0, _GCDRARe,
  0,
  [_RA, _Ta],
  [[() => AssociationDetail$, { [_jN]: _rA }], [128 | 0, { [_jN]: _ta }]], 1
];
export var GetCustomDetectionRuleOrgConfigurationRequest$: StaticStructureSchema = [3, n0, _GCDROCR,
  0,
  [_RI, _Mo],
  [[0, 1], [0, { [_hQ]: _mo }]], 2
];
export var GetCustomDetectionRuleOrgConfigurationResponse$: StaticStructureSchema = [3, n0, _GCDROCRe,
  0,
  [_Confi],
  [[() => DetectionRuleOrgConfiguration$, { [_jN]: _conf }]], 1
];
export var GetCustomDetectionRuleRequest$: StaticStructureSchema = [3, n0, _GCDRR,
  0,
  [_RI],
  [[0, 1]], 1
];
export var GetCustomDetectionRuleResponse$: StaticStructureSchema = [3, n0, _GCDRRe,
  0,
  [_Ru],
  [[() => RuleDetail$, { [_jN]: _ru }]], 1
];
export var GetDetectorRequest$: StaticStructureSchema = [3, n0, _GDR,
  0,
  [_DI],
  [[0, 1]], 1
];
export var GetDetectorResponse$: StaticStructureSchema = [3, n0, _GDRe,
  0,
  [_SRe, _St, _CA, _FPF, _UAp, _DS, _Ta, _F],
  [[0, { [_jN]: _sRe }], [0, { [_jN]: _st }], [0, { [_jN]: _cA }], [0, { [_jN]: _fPF }], [0, { [_jN]: _uAp }], [() => DataSourceConfigurationsResult$, { [_jN]: _dS }], [128 | 0, { [_jN]: _ta }], [() => DetectorFeatureConfigurationsResults, { [_jN]: _f }]], 2
];
export var GetFilterRequest$: StaticStructureSchema = [3, n0, _GFR,
  0,
  [_DI, _FN],
  [[0, 1], [0, 1]], 2
];
export var GetFilterResponse$: StaticStructureSchema = [3, n0, _GFRe,
  0,
  [_N, _Ac, _FCin, _D, _Ra, _Ta, _CA, _UAp, _V],
  [[0, { [_jN]: _n }], [0, { [_jN]: _ac }], [() => FindingCriteria$, { [_jN]: _fCin }], [0, { [_jN]: _d }], [1, { [_jN]: _ra }], [128 | 0, { [_jN]: _ta }], [4, { [_jN]: _cA }], [4, { [_jN]: _uAp }], [1, { [_jN]: _v }]], 3
];
export var GetFindingsRequest$: StaticStructureSchema = [3, n0, _GFRet,
  0,
  [_DI, _FI, _SCo],
  [[0, 1], [64 | 0, { [_jN]: _fI }], [() => SortCriteria$, { [_jN]: _sCo }]], 2
];
export var GetFindingsResponse$: StaticStructureSchema = [3, n0, _GFReti,
  0,
  [_Fin],
  [[() => Findings, { [_jN]: _fi }]], 1
];
export var GetFindingsStatisticsRequest$: StaticStructureSchema = [3, n0, _GFSR,
  0,
  [_DI, _FST, _FCin, _GB, _OB, _MR],
  [[0, 1], [64 | 0, { [_jN]: _fST }], [() => FindingCriteria$, { [_jN]: _fCin }], [0, { [_jN]: _gB }], [0, { [_jN]: _oB }], [1, { [_jN]: _mR }]], 1
];
export var GetFindingsStatisticsResponse$: StaticStructureSchema = [3, n0, _GFSRe,
  0,
  [_FS, _NT],
  [[() => FindingStatistics$, { [_jN]: _fS }], [0, { [_jN]: _nT }]], 1
];
export var GetInvestigationRequest$: StaticStructureSchema = [3, n0, _GIR,
  0,
  [_DI, _IInv],
  [[0, 1], [0, 1]], 2
];
export var GetInvestigationResponse$: StaticStructureSchema = [3, n0, _GIRe,
  0,
  [_In],
  [[() => Investigation$, { [_jN]: _in }]], 1
];
export var GetInvitationsCountRequest$: StaticStructureSchema = [3, n0, _GICR,
  0,
  [],
  []
];
export var GetInvitationsCountResponse$: StaticStructureSchema = [3, n0, _GICRe,
  0,
  [_IC],
  [[1, { [_jN]: _iC }]]
];
export var GetIPSetRequest$: StaticStructureSchema = [3, n0, _GIPSR,
  0,
  [_DI, _ISI],
  [[0, 1], [0, 1]], 2
];
export var GetIPSetResponse$: StaticStructureSchema = [3, n0, _GIPSRe,
  0,
  [_N, _Fo, _Lo, _St, _Ta, _EBO],
  [[0, { [_jN]: _n }], [0, { [_jN]: _fo }], [0, { [_jN]: _lo }], [0, { [_jN]: _st }], [128 | 0, { [_jN]: _ta }], [0, { [_jN]: _eBO }]], 4
];
export var GetMalwareProtectionPlanRequest$: StaticStructureSchema = [3, n0, _GMPPR,
  0,
  [_MPPI],
  [[0, 1]], 1
];
export var GetMalwareProtectionPlanResponse$: StaticStructureSchema = [3, n0, _GMPPRe,
  0,
  [_Ar, _Ro, _PR, _Actio, _CA, _St, _SRt, _Ta],
  [[0, { [_jN]: _ar }], [0, { [_jN]: _ro }], [() => CreateProtectedResource$, { [_jN]: _pR }], [() => MalwareProtectionPlanActions$, { [_jN]: _acti }], [4, { [_jN]: _cA }], [0, { [_jN]: _st }], [() => MalwareProtectionPlanStatusReasonsList, { [_jN]: _sRt }], [128 | 0, { [_jN]: _ta }]]
];
export var GetMalwareScanRequest$: StaticStructureSchema = [3, n0, _GMSR,
  0,
  [_SI],
  [[0, 1]], 1
];
export var GetMalwareScanResponse$: StaticStructureSchema = [3, n0, _GMSRe,
  0,
  [_SI, _DI, _ADI, _RAe, _RT, _SRC, _SRCk, _FRC, _SRc, _SCc, _SCca, _SSc, _SSR, _STc, _SSA, _SCA, _SRD],
  [[0, { [_jN]: _sI }], [0, { [_jN]: _dI }], [0, { [_jN]: _aDI }], [0, { [_jN]: _rAe }], [0, { [_jN]: _rT }], [1, { [_jN]: _sRC }], [1, { [_jN]: _sRCk }], [1, { [_jN]: _fRC }], [() => ScannedResources, { [_jN]: _sRc }], [() => ScanConfiguration$, { [_jN]: _sCc }], [0, { [_jN]: _sCca }], [0, { [_jN]: _sS }], [0, { [_jN]: _sSR }], [0, { [_jN]: _sTc }], [4, { [_jN]: _sSA }], [4, { [_jN]: _sCA }], [() => GetMalwareScanResultDetails$, { [_jN]: _sRD }]]
];
export var GetMalwareScanResultDetails$: StaticStructureSchema = [3, n0, _GMSRD,
  0,
  [_SRS, _SFC, _FFC, _TFFC, _TFC, _TB, _UTC, _Th],
  [[0, { [_jN]: _sRS }], [1, { [_jN]: _sFC }], [1, { [_jN]: _fFC }], [1, { [_jN]: _tFFC }], [1, { [_jN]: _tFC }], [1, { [_jN]: _tB }], [1, { [_jN]: _uTC }], [() => ScanResultThreats, { [_jN]: _th }]]
];
export var GetMalwareScanSettingsRequest$: StaticStructureSchema = [3, n0, _GMSSR,
  0,
  [_DI],
  [[0, 1]], 1
];
export var GetMalwareScanSettingsResponse$: StaticStructureSchema = [3, n0, _GMSSRe,
  0,
  [_SRCc, _ESP],
  [[() => ScanResourceCriteria$, { [_jN]: _sRCc }], [0, { [_jN]: _eSP }]]
];
export var GetMasterAccountRequest$: StaticStructureSchema = [3, n0, _GMAR,
  0,
  [_DI],
  [[0, 1]], 1
];
export var GetMasterAccountResponse$: StaticStructureSchema = [3, n0, _GMARe,
  0,
  [_Mas],
  [[() => Master$, { [_jN]: _mas }]], 1
];
export var GetMemberDetectorsRequest$: StaticStructureSchema = [3, n0, _GMDR,
  0,
  [_DI, _AIcc],
  [[0, 1], [64 | 0, { [_jN]: _aIcc }]], 2
];
export var GetMemberDetectorsResponse$: StaticStructureSchema = [3, n0, _GMDRe,
  0,
  [_MDSC, _UAn],
  [[() => MemberDataSourceConfigurations, { [_jN]: _me }], [() => UnprocessedAccounts, { [_jN]: _uAn }]], 2
];
export var GetMembersRequest$: StaticStructureSchema = [3, n0, _GMR,
  0,
  [_DI, _AIcc],
  [[0, 1], [64 | 0, { [_jN]: _aIcc }]], 2
];
export var GetMembersResponse$: StaticStructureSchema = [3, n0, _GMRe,
  0,
  [_Me, _UAn],
  [[() => Members, { [_jN]: _me }], [() => UnprocessedAccounts, { [_jN]: _uAn }]], 2
];
export var GetOrganizationStatisticsResponse$: StaticStructureSchema = [3, n0, _GOSR,
  0,
  [_OD],
  [[() => OrganizationDetails$, { [_jN]: _oD }]]
];
export var GetRemainingFreeTrialDaysRequest$: StaticStructureSchema = [3, n0, _GRFTDR,
  0,
  [_DI, _AIcc],
  [[0, 1], [64 | 0, { [_jN]: _aIcc }]], 2
];
export var GetRemainingFreeTrialDaysResponse$: StaticStructureSchema = [3, n0, _GRFTDRe,
  0,
  [_Acc, _UAn],
  [[() => AccountFreeTrialInfos, { [_jN]: _acc }], [() => UnprocessedAccounts, { [_jN]: _uAn }]]
];
export var GetThreatEntitySetRequest$: StaticStructureSchema = [3, n0, _GTESR,
  0,
  [_DI, _TESI],
  [[0, 1], [0, 1]], 2
];
export var GetThreatEntitySetResponse$: StaticStructureSchema = [3, n0, _GTESRe,
  0,
  [_N, _Fo, _Lo, _St, _EBO, _Ta, _CA, _UAp, _ED],
  [[0, { [_jN]: _n }], [0, { [_jN]: _fo }], [0, { [_jN]: _lo }], [0, { [_jN]: _st }], [0, { [_jN]: _eBO }], [128 | 0, { [_jN]: _ta }], [4, { [_jN]: _cA }], [4, { [_jN]: _uAp }], [0, { [_jN]: _eD }]], 4
];
export var GetThreatIntelSetRequest$: StaticStructureSchema = [3, n0, _GTISR,
  0,
  [_DI, _TISI],
  [[0, 1], [0, 1]], 2
];
export var GetThreatIntelSetResponse$: StaticStructureSchema = [3, n0, _GTISRe,
  0,
  [_N, _Fo, _Lo, _St, _Ta, _EBO],
  [[0, { [_jN]: _n }], [0, { [_jN]: _fo }], [0, { [_jN]: _lo }], [0, { [_jN]: _st }], [128 | 0, { [_jN]: _ta }], [0, { [_jN]: _eBO }]], 4
];
export var GetTrustedEntitySetRequest$: StaticStructureSchema = [3, n0, _GTESRet,
  0,
  [_DI, _TESIr],
  [[0, 1], [0, 1]], 2
];
export var GetTrustedEntitySetResponse$: StaticStructureSchema = [3, n0, _GTESRetr,
  0,
  [_N, _Fo, _Lo, _St, _EBO, _Ta, _CA, _UAp, _ED],
  [[0, { [_jN]: _n }], [0, { [_jN]: _fo }], [0, { [_jN]: _lo }], [0, { [_jN]: _st }], [0, { [_jN]: _eBO }], [128 | 0, { [_jN]: _ta }], [4, { [_jN]: _cA }], [4, { [_jN]: _uAp }], [0, { [_jN]: _eD }]], 4
];
export var GetUsageStatisticsRequest$: StaticStructureSchema = [3, n0, _GUSR,
  0,
  [_DI, _UST, _UC, _Uni, _MR, _NT],
  [[0, 1], [0, { [_jN]: _uST }], [() => UsageCriteria$, { [_jN]: _uC }], [0, { [_jN]: _uni }], [1, { [_jN]: _mR }], [0, { [_jN]: _nT }]], 3
];
export var GetUsageStatisticsResponse$: StaticStructureSchema = [3, n0, _GUSRe,
  0,
  [_US, _NT],
  [[() => UsageStatistics$, { [_jN]: _uS }], [0, { [_jN]: _nT }]]
];
export var HighestSeverityThreatDetails$: StaticStructureSchema = [3, n0, _HSTD,
  0,
  [_Sev, _TN, _Coun],
  [[0, { [_jN]: _sev }], [0, { [_jN]: _tN }], [1, { [_jN]: _cou }]]
];
export var HostPath$: StaticStructureSchema = [3, n0, _HP,
  0,
  [_Pa],
  [[0, { [_jN]: _pa }]]
];
export var IamInstanceProfile$: StaticStructureSchema = [3, n0, _IIP,
  0,
  [_Ar, _I],
  [[0, { [_jN]: _ar }], [0, { [_jN]: _i }]]
];
export var IamInstanceProfileV2$: StaticStructureSchema = [3, n0, _IIPV,
  0,
  [_EIU],
  [[64 | 0, { [_jN]: _eIU }]]
];
export var ImpersonatedUser$: StaticStructureSchema = [3, n0, _IUm,
  0,
  [_Use, _Gro],
  [[0, { [_jN]: _use }], [64 | 0, { [_jN]: _gro }]]
];
export var IncrementalScanDetails$: StaticStructureSchema = [3, n0, _ISD,
  0,
  [_BRA],
  [[0, { [_jN]: _bRA }]], 1
];
export var Indicator$: StaticStructureSchema = [3, n0, _Ind,
  0,
  [_Ke, _Va, _Ti],
  [[0, { [_jN]: _ke }], [64 | 0, { [_jN]: _va }], [0, { [_jN]: _ti }]], 1
];
export var InstanceDetails$: StaticStructureSchema = [3, n0, _IDn,
  0,
  [_AZ, _IIP, _ID, _IIm, _IIn, _IS, _IT, _OA, _LTau, _NI, _Pl, _PC, _Ta],
  [[0, { [_jN]: _aZ }], [() => IamInstanceProfile$, { [_jN]: _iIP }], [0, { [_jN]: _iD }], [0, { [_jN]: _iIm }], [0, { [_jN]: _iIn }], [0, { [_jN]: _iS }], [0, { [_jN]: _iT }], [0, { [_jN]: _oA }], [0, { [_jN]: _lTau }], [() => NetworkInterfaces, { [_jN]: _nI }], [0, { [_jN]: _pl }], [() => ProductCodes, { [_jN]: _pC }], [() => Tags, { [_jN]: _ta }]]
];
export var Investigation$: StaticStructureSchema = [3, n0, _In,
  0,
  [_IInv, _St, _TP, _TBr, _Met, _Cl, _RL, _Ri, _Conf, _Su, _ST, _ET, _Er],
  [[0, { [_jN]: _iInv }], [0, { [_jN]: _st }], [0, { [_jN]: _tP }], [0, { [_jN]: _tBr }], [() => InvestigationMetadata$, { [_jN]: _met }], [() => CloudDetails$, { [_jN]: _cl }], [0, { [_jN]: _rL }], [0, { [_jN]: _ri }], [0, { [_jN]: _co }], [0, { [_jN]: _su }], [4, { [_jN]: _sT }], [4, { [_jN]: _eT }], [0, { [_jN]: _e }]], 4
];
export var InvestigationMetadata$: StaticStructureSchema = [3, n0, _IM,
  0,
  [_V, _Prod],
  [[0, { [_jN]: _v }], [() => Product$, { [_jN]: _prod }]], 2
];
export var InvestigationSortCriteria$: StaticStructureSchema = [3, n0, _ISC,
  0,
  [_AN, _OB],
  [[0, { [_jN]: _aN }], [0, { [_jN]: _oB }]]
];
export var InvestigationSummary$: StaticStructureSchema = [3, n0, _ISn,
  0,
  [_IInv, _St, _TP, _RL, _Conf, _Ti, _AIc, _ST, _ET],
  [[0, { [_jN]: _iInv }], [0, { [_jN]: _st }], [0, { [_jN]: _tP }], [0, { [_jN]: _rL }], [0, { [_jN]: _co }], [0, { [_jN]: _ti }], [0, { [_jN]: _aIc }], [4, { [_jN]: _sT }], [4, { [_jN]: _eT }]]
];
export var Invitation$: StaticStructureSchema = [3, n0, _Inv,
  0,
  [_AIc, _II, _RS, _IA],
  [[0, { [_jN]: _aIc }], [0, { [_jN]: _iI }], [0, { [_jN]: _rS }], [0, { [_jN]: _iA }]]
];
export var InviteMembersRequest$: StaticStructureSchema = [3, n0, _IMR,
  0,
  [_DI, _AIcc, _DEN, _M],
  [[0, 1], [64 | 0, { [_jN]: _aIcc }], [2, { [_jN]: _dEN }], [0, { [_jN]: _m }]], 2
];
export var InviteMembersResponse$: StaticStructureSchema = [3, n0, _IMRn,
  0,
  [_UAn],
  [[() => UnprocessedAccounts, { [_jN]: _uAn }]], 1
];
export var ItemDetails$: StaticStructureSchema = [3, n0, _IDt,
  0,
  [_RAe, _IPt, _H, _AId],
  [[0, { [_jN]: _rAe }], [0, { [_jN]: _iPt }], [0, { [_jN]: _h }], [() => AdditionalInfo$, { [_jN]: _aId }]]
];
export var ItemPath$: StaticStructureSchema = [3, n0, _IPt,
  0,
  [_NIP, _H],
  [[0, { [_jN]: _nIP }], [0, { [_jN]: _h }]]
];
export var KubernetesApiCallAction$: StaticStructureSchema = [3, n0, _KACA,
  0,
  [_RU, _Ve, _Res, _Sub, _Na, _RN, _SIo, _UA, _RID, _SCt, _Para],
  [[0, { [_jN]: _rU }], [0, { [_jN]: _ve }], [0, { [_jN]: _res }], [0, { [_jN]: _sub }], [0, { [_jN]: _na }], [0, { [_jN]: _rN }], [64 | 0, { [_jN]: _sIP }], [0, { [_jN]: _uA }], [() => RemoteIpDetails$, { [_jN]: _rID }], [1, { [_jN]: _sCt }], [0, { [_jN]: _para }]]
];
export var KubernetesAuditLogsConfiguration$: StaticStructureSchema = [3, n0, _KALC,
  0,
  [_En],
  [[2, { [_jN]: _en }]], 1
];
export var KubernetesAuditLogsConfigurationResult$: StaticStructureSchema = [3, n0, _KALCR,
  0,
  [_St],
  [[0, { [_jN]: _st }]], 1
];
export var KubernetesConfiguration$: StaticStructureSchema = [3, n0, _KC,
  0,
  [_AL],
  [[() => KubernetesAuditLogsConfiguration$, { [_jN]: _aL }]], 1
];
export var KubernetesConfigurationResult$: StaticStructureSchema = [3, n0, _KCR,
  0,
  [_AL],
  [[() => KubernetesAuditLogsConfigurationResult$, { [_jN]: _aL }]], 1
];
export var KubernetesDataSourceFreeTrial$: StaticStructureSchema = [3, n0, _KDSFT,
  0,
  [_AL],
  [[() => DataSourceFreeTrial$, { [_jN]: _aL }]]
];
export var KubernetesDetails$: StaticStructureSchema = [3, n0, _KD,
  0,
  [_KUD, _KWD],
  [[() => KubernetesUserDetails$, { [_jN]: _kUD }], [() => KubernetesWorkloadDetails$, { [_jN]: _kWD }]]
];
export var KubernetesPermissionCheckedDetails$: StaticStructureSchema = [3, n0, _KPCD,
  0,
  [_Ve, _Res, _Na, _Al],
  [[0, { [_jN]: _ve }], [0, { [_jN]: _res }], [0, { [_jN]: _na }], [2, { [_jN]: _al }]]
];
export var KubernetesRoleBindingDetails$: StaticStructureSchema = [3, n0, _KRBD,
  0,
  [_Ki, _N, _U, _RRN, _RRK],
  [[0, { [_jN]: _ki }], [0, { [_jN]: _n }], [0, { [_jN]: _u }], [0, { [_jN]: _rRN }], [0, { [_jN]: _rRK }]]
];
export var KubernetesRoleDetails$: StaticStructureSchema = [3, n0, _KRD,
  0,
  [_Ki, _N, _U],
  [[0, { [_jN]: _ki }], [0, { [_jN]: _n }], [0, { [_jN]: _u }]]
];
export var KubernetesUserDetails$: StaticStructureSchema = [3, n0, _KUD,
  0,
  [_Use, _U, _Gro, _SNe, _IUm],
  [[0, { [_jN]: _use }], [0, { [_jN]: _u }], [64 | 0, { [_jN]: _gro }], [64 | 0, { [_jN]: _sNe }], [() => ImpersonatedUser$, { [_jN]: _iUm }]]
];
export var KubernetesWorkload$: StaticStructureSchema = [3, n0, _KW,
  0,
  [_CU, _Na, _KRT],
  [[64 | 0, { [_jN]: _cU }], [0, { [_jN]: _na }], [0, { [_jN]: _t }]]
];
export var KubernetesWorkloadDetails$: StaticStructureSchema = [3, n0, _KWD,
  0,
  [_N, _T, _U, _Na, _HN, _SAN, _Cont, _Vo, _HIPC, _HPID],
  [[0, { [_jN]: _n }], [0, { [_jN]: _t }], [0, { [_jN]: _u }], [0, { [_jN]: _na }], [2, { [_jN]: _hN }], [0, { [_jN]: _sAN }], [() => Containers, { [_jN]: _cont }], [() => Volumes, { [_jN]: _vo }], [2, { [_jN]: _hIPC }], [2, { [_jN]: _hPID }]]
];
export var LambdaDetails$: StaticStructureSchema = [3, n0, _LD,
  0,
  [_FA, _FNu, _D, _LMA, _RIev, _FV, _Ro, _VC, _Ta],
  [[0, { [_jN]: _fA }], [0, { [_jN]: _fN }], [0, { [_jN]: _d }], [4, { [_jN]: _lMA }], [0, { [_jN]: _rIev }], [0, { [_jN]: _fV }], [0, { [_jN]: _ro }], [() => VpcConfig$, { [_jN]: _vC }], [() => Tags, { [_jN]: _ta }]]
];
export var LineageObject$: StaticStructureSchema = [3, n0, _LO,
  0,
  [_ST, _NP, _UI, _N, _Pi, _Uu, _EP, _Eu, _PU],
  [[4, { [_jN]: _sT }], [1, { [_jN]: _nP }], [1, { [_jN]: _uI }], [0, { [_jN]: _n }], [1, { [_jN]: _pi }], [0, { [_jN]: _uu }], [0, { [_jN]: _eP }], [1, { [_jN]: _eu }], [0, { [_jN]: _pU }]]
];
export var ListCoverageRequest$: StaticStructureSchema = [3, n0, _LCR,
  0,
  [_DI, _NT, _MR, _FCil, _SCo],
  [[0, 1], [0, { [_jN]: _nT }], [1, { [_jN]: _mR }], [() => CoverageFilterCriteria$, { [_jN]: _fCil }], [() => CoverageSortCriteria$, { [_jN]: _sCo }]], 1
];
export var ListCoverageResponse$: StaticStructureSchema = [3, n0, _LCRi,
  0,
  [_Reso, _NT],
  [[() => CoverageResources, { [_jN]: _reso }], [0, { [_jN]: _nT }]], 1
];
export var ListCustomDetectionRuleAssociationsRequest$: StaticStructureSchema = [3, n0, _LCDRAR,
  0,
  [_MR, _NT, _RI, _Mo],
  [[1, { [_hQ]: _mR }], [0, { [_hQ]: _nT }], [0, { [_hQ]: _rI }], [0, { [_hQ]: _mo }]]
];
export var ListCustomDetectionRuleAssociationsResponse$: StaticStructureSchema = [3, n0, _LCDRARi,
  0,
  [_RAu, _NT],
  [[() => AssociationSummaryList, { [_jN]: _rAu }], [0, { [_jN]: _nT }]], 1
];
export var ListCustomDetectionRuleOrgConfigurationsRequest$: StaticStructureSchema = [3, n0, _LCDROCR,
  0,
  [_MR, _NT, _St],
  [[1, { [_hQ]: _mR }], [0, { [_hQ]: _nT }], [0, { [_hQ]: _st }]]
];
export var ListCustomDetectionRuleOrgConfigurationsResponse$: StaticStructureSchema = [3, n0, _LCDROCRi,
  0,
  [_Config, _NT],
  [[() => DetectionRuleOrgConfigurationSummaryList, { [_jN]: _confi }], [0, { [_jN]: _nT }]], 1
];
export var ListCustomDetectionRulesRequest$: StaticStructureSchema = [3, n0, _LCDRR,
  0,
  [_MR, _NT, _Fil],
  [[1, { [_jN]: _mR }], [0, { [_jN]: _nT }], [() => DetectionRuleFilterList, { [_jN]: _fil }]]
];
export var ListCustomDetectionRulesResponse$: StaticStructureSchema = [3, n0, _LCDRRi,
  0,
  [_Rul, _NT],
  [[() => RuleSummaryList, { [_jN]: _rul }], [0, { [_jN]: _nT }]], 1
];
export var ListDetectorsRequest$: StaticStructureSchema = [3, n0, _LDR,
  0,
  [_MR, _NT],
  [[1, { [_hQ]: _mR }], [0, { [_hQ]: _nT }]]
];
export var ListDetectorsResponse$: StaticStructureSchema = [3, n0, _LDRi,
  0,
  [_DIet, _NT],
  [[64 | 0, { [_jN]: _dIet }], [0, { [_jN]: _nT }]], 1
];
export var ListFiltersRequest$: StaticStructureSchema = [3, n0, _LFR,
  0,
  [_DI, _MR, _NT],
  [[0, 1], [1, { [_hQ]: _mR }], [0, { [_hQ]: _nT }]], 1
];
export var ListFiltersResponse$: StaticStructureSchema = [3, n0, _LFRi,
  0,
  [_FNi, _NT],
  [[64 | 0, { [_jN]: _fNi }], [0, { [_jN]: _nT }]], 1
];
export var ListFindingsRequest$: StaticStructureSchema = [3, n0, _LFRis,
  0,
  [_DI, _FCin, _SCo, _MR, _NT],
  [[0, 1], [() => FindingCriteria$, { [_jN]: _fCin }], [() => SortCriteria$, { [_jN]: _sCo }], [1, { [_jN]: _mR }], [0, { [_jN]: _nT }]], 1
];
export var ListFindingsResponse$: StaticStructureSchema = [3, n0, _LFRist,
  0,
  [_FI, _NT],
  [[64 | 0, { [_jN]: _fI }], [0, { [_jN]: _nT }]], 1
];
export var ListInvestigationsRequest$: StaticStructureSchema = [3, n0, _LIR,
  0,
  [_DI, _SCo, _MR, _NT],
  [[0, 1], [() => InvestigationSortCriteria$, { [_jN]: _sCo }], [1, { [_jN]: _mR }], [0, { [_jN]: _nT }]], 1
];
export var ListInvestigationsResponse$: StaticStructureSchema = [3, n0, _LIRi,
  0,
  [_Inve, _NT],
  [[() => InvestigationSummaries, { [_jN]: _inv }], [0, { [_jN]: _nT }]], 1
];
export var ListInvitationsRequest$: StaticStructureSchema = [3, n0, _LIRis,
  0,
  [_MR, _NT],
  [[1, { [_hQ]: _mR }], [0, { [_hQ]: _nT }]]
];
export var ListInvitationsResponse$: StaticStructureSchema = [3, n0, _LIRist,
  0,
  [_Invi, _NT],
  [[() => Invitations, { [_jN]: _invi }], [0, { [_jN]: _nT }]]
];
export var ListIPSetsRequest$: StaticStructureSchema = [3, n0, _LIPSR,
  0,
  [_DI, _MR, _NT],
  [[0, 1], [1, { [_hQ]: _mR }], [0, { [_hQ]: _nT }]], 1
];
export var ListIPSetsResponse$: StaticStructureSchema = [3, n0, _LIPSRi,
  0,
  [_ISIp, _NT],
  [[64 | 0, { [_jN]: _iSIp }], [0, { [_jN]: _nT }]], 1
];
export var ListMalwareProtectionPlansRequest$: StaticStructureSchema = [3, n0, _LMPPR,
  0,
  [_NT],
  [[0, { [_hQ]: _nT }]]
];
export var ListMalwareProtectionPlansResponse$: StaticStructureSchema = [3, n0, _LMPPRi,
  0,
  [_MPP, _NT],
  [[() => MalwareProtectionPlansSummary, { [_jN]: _mPP }], [0, { [_jN]: _nT }]]
];
export var ListMalwareScansFilterCriteria$: StaticStructureSchema = [3, n0, _LMSFC,
  0,
  [_LMSFCi],
  [[() => ListMalwareScansFilterCriterionList, { [_jN]: _fC }]]
];
export var ListMalwareScansFilterCriterion$: StaticStructureSchema = [3, n0, _LMSFCi,
  0,
  [_LMSCK, _FCi],
  [[0, { [_jN]: _cK }], [() => FilterCondition$, { [_jN]: _fCi }]]
];
export var ListMalwareScansRequest$: StaticStructureSchema = [3, n0, _LMSR,
  0,
  [_MR, _NT, _FCil, _SCo],
  [[1, { [_hQ]: _mR }], [0, { [_hQ]: _nT }], [() => ListMalwareScansFilterCriteria$, { [_jN]: _fCil }], [() => SortCriteria$, { [_jN]: _sCo }]]
];
export var ListMalwareScansResponse$: StaticStructureSchema = [3, n0, _LMSRi,
  0,
  [_Sc, _NT],
  [[() => MalwareScans, { [_jN]: _sc }], [0, { [_jN]: _nT }]], 1
];
export var ListMembersRequest$: StaticStructureSchema = [3, n0, _LMR,
  0,
  [_DI, _MR, _NT, _OAn],
  [[0, 1], [1, { [_hQ]: _mR }], [0, { [_hQ]: _nT }], [0, { [_hQ]: _oAn }]], 1
];
export var ListMembersResponse$: StaticStructureSchema = [3, n0, _LMRi,
  0,
  [_Me, _NT],
  [[() => Members, { [_jN]: _me }], [0, { [_jN]: _nT }]]
];
export var ListOrganizationAdminAccountsRequest$: StaticStructureSchema = [3, n0, _LOAAR,
  0,
  [_MR, _NT],
  [[1, { [_hQ]: _mR }], [0, { [_hQ]: _nT }]]
];
export var ListOrganizationAdminAccountsResponse$: StaticStructureSchema = [3, n0, _LOAARi,
  0,
  [_AAd, _NT],
  [[() => AdminAccounts, { [_jN]: _aA }], [0, { [_jN]: _nT }]]
];
export var ListPublishingDestinationsRequest$: StaticStructureSchema = [3, n0, _LPDR,
  0,
  [_DI, _MR, _NT],
  [[0, 1], [1, { [_hQ]: _mR }], [0, { [_hQ]: _nT }]], 1
];
export var ListPublishingDestinationsResponse$: StaticStructureSchema = [3, n0, _LPDRi,
  0,
  [_Des, _NT],
  [[() => Destinations, { [_jN]: _de }], [0, { [_jN]: _nT }]], 1
];
export var ListTagsForResourceRequest$: StaticStructureSchema = [3, n0, _LTFRR,
  0,
  [_RAe],
  [[0, 1]], 1
];
export var ListTagsForResourceResponse$: StaticStructureSchema = [3, n0, _LTFRRi,
  0,
  [_Ta],
  [[128 | 0, { [_jN]: _ta }]]
];
export var ListThreatEntitySetsRequest$: StaticStructureSchema = [3, n0, _LTESR,
  0,
  [_DI, _MR, _NT],
  [[0, 1], [1, { [_hQ]: _mR }], [0, { [_hQ]: _nT }]], 1
];
export var ListThreatEntitySetsResponse$: StaticStructureSchema = [3, n0, _LTESRi,
  0,
  [_TESIh, _NT],
  [[64 | 0, { [_jN]: _tESIh }], [0, { [_jN]: _nT }]], 1
];
export var ListThreatIntelSetsRequest$: StaticStructureSchema = [3, n0, _LTISR,
  0,
  [_DI, _MR, _NT],
  [[0, 1], [1, { [_hQ]: _mR }], [0, { [_hQ]: _nT }]], 1
];
export var ListThreatIntelSetsResponse$: StaticStructureSchema = [3, n0, _LTISRi,
  0,
  [_TISIh, _NT],
  [[64 | 0, { [_jN]: _tISIh }], [0, { [_jN]: _nT }]], 1
];
export var ListTrustedEntitySetsRequest$: StaticStructureSchema = [3, n0, _LTESRis,
  0,
  [_DI, _MR, _NT],
  [[0, 1], [1, { [_hQ]: _mR }], [0, { [_hQ]: _nT }]], 1
];
export var ListTrustedEntitySetsResponse$: StaticStructureSchema = [3, n0, _LTESRist,
  0,
  [_TESIru, _NT],
  [[64 | 0, { [_jN]: _tESIru }], [0, { [_jN]: _nT }]], 1
];
export var LocalIpDetails$: StaticStructureSchema = [3, n0, _LID,
  0,
  [_IAV, _IAVp],
  [[() => SensitiveString, { [_jN]: _iAV }], [() => SensitiveString, { [_jN]: _iAVp }]]
];
export var LocalPortDetails$: StaticStructureSchema = [3, n0, _LPD,
  0,
  [_Po, _PN],
  [[1, { [_jN]: _po }], [0, { [_jN]: _pN }]]
];
export var LoginAttribute$: StaticStructureSchema = [3, n0, _LA,
  0,
  [_Us, _App, _FLA, _SLA],
  [[0, { [_jN]: _us }], [0, { [_jN]: _app }], [1, { [_jN]: _fLA }], [1, { [_jN]: _sLA }]]
];
export var MalwareProtectionConfiguration$: StaticStructureSchema = [3, n0, _MPC,
  0,
  [_SEIWF],
  [[() => ScanEc2InstanceWithFindings$, { [_jN]: _sEIWF }]]
];
export var MalwareProtectionConfigurationResult$: StaticStructureSchema = [3, n0, _MPCR,
  0,
  [_SEIWF, _SRe],
  [[() => ScanEc2InstanceWithFindingsResult$, { [_jN]: _sEIWF }], [0, { [_jN]: _sRe }]]
];
export var MalwareProtectionDataSourceFreeTrial$: StaticStructureSchema = [3, n0, _MPDSFT,
  0,
  [_SEIWF],
  [[() => DataSourceFreeTrial$, { [_jN]: _sEIWF }]]
];
export var MalwareProtectionFindingsScanConfiguration$: StaticStructureSchema = [3, n0, _MPFSC,
  0,
  [_TT, _ISD],
  [[0, { [_jN]: _tT }], [() => IncrementalScanDetails$, { [_jN]: _iSD }]]
];
export var MalwareProtectionPlanActions$: StaticStructureSchema = [3, n0, _MPPA,
  0,
  [_Tag],
  [[() => MalwareProtectionPlanTaggingAction$, { [_jN]: _tag }]]
];
export var MalwareProtectionPlanStatusReason$: StaticStructureSchema = [3, n0, _MPPSR,
  0,
  [_Cod, _M],
  [[0, { [_jN]: _cod }], [0, { [_jN]: _m }]]
];
export var MalwareProtectionPlanSummary$: StaticStructureSchema = [3, n0, _MPPS,
  0,
  [_MPPI],
  [[0, { [_jN]: _mPPI }]]
];
export var MalwareProtectionPlanTaggingAction$: StaticStructureSchema = [3, n0, _MPPTA,
  0,
  [_St],
  [[0, { [_jN]: _st }]]
];
export var MalwareScan$: StaticStructureSchema = [3, n0, _MS,
  0,
  [_RAe, _RT, _SI, _SSc, _SRS, _STc, _SSA, _SCA],
  [[0, { [_jN]: _rAe }], [0, { [_jN]: _rT }], [0, { [_jN]: _sI }], [0, { [_jN]: _sS }], [0, { [_jN]: _sRS }], [0, { [_jN]: _sTc }], [4, { [_jN]: _sSA }], [4, { [_jN]: _sCA }]]
];
export var MalwareScanDetails$: StaticStructureSchema = [3, n0, _MSD,
  0,
  [_Th, _SI, _STc, _SCca, _SCc, _UTC],
  [[() => Threats, { [_jN]: _th }], [0, { [_jN]: _sI }], [0, { [_jN]: _sTc }], [0, { [_jN]: _sCca }], [() => MalwareProtectionFindingsScanConfiguration$, { [_jN]: _sCc }], [1, { [_jN]: _uTC }]]
];
export var Master$: StaticStructureSchema = [3, n0, _Mas,
  0,
  [_AIc, _II, _RS, _IA],
  [[0, { [_jN]: _aIc }], [0, { [_jN]: _iI }], [0, { [_jN]: _rS }], [0, { [_jN]: _iA }]]
];
export var Member$: StaticStructureSchema = [3, n0, _Mem,
  0,
  [_AIc, _MI, _E, _RS, _UAp, _DI, _IA, _AI],
  [[0, { [_jN]: _aIc }], [0, { [_jN]: _mI }], [() => Email, { [_jN]: _em }], [0, { [_jN]: _rS }], [0, { [_jN]: _uAp }], [0, { [_jN]: _dI }], [0, { [_jN]: _iA }], [0, { [_jN]: _aI }]], 5
];
export var MemberAdditionalConfiguration$: StaticStructureSchema = [3, n0, _MAC,
  0,
  [_N, _St],
  [[0, { [_jN]: _n }], [0, { [_jN]: _st }]]
];
export var MemberAdditionalConfigurationResult$: StaticStructureSchema = [3, n0, _MACR,
  0,
  [_N, _St, _UAp],
  [[0, { [_jN]: _n }], [0, { [_jN]: _st }], [4, { [_jN]: _uAp }]]
];
export var MemberDataSourceConfiguration$: StaticStructureSchema = [3, n0, _MDSCe,
  0,
  [_AIc, _DS, _F],
  [[0, { [_jN]: _aIc }], [() => DataSourceConfigurationsResult$, { [_jN]: _dS }], [() => MemberFeaturesConfigurationsResults, { [_jN]: _f }]], 2
];
export var MemberFeaturesConfiguration$: StaticStructureSchema = [3, n0, _MFC,
  0,
  [_N, _St, _ACd],
  [[0, { [_jN]: _n }], [0, { [_jN]: _st }], [() => MemberAdditionalConfigurations, { [_jN]: _aC }]]
];
export var MemberFeaturesConfigurationResult$: StaticStructureSchema = [3, n0, _MFCR,
  0,
  [_N, _St, _UAp, _ACd],
  [[0, { [_jN]: _n }], [0, { [_jN]: _st }], [4, { [_jN]: _uAp }], [() => MemberAdditionalConfigurationResults, { [_jN]: _aC }]]
];
export var ModelDetail$: StaticStructureSchema = [3, n0, _MD,
  0,
  [_MIo],
  [[0, { [_jN]: _mIo }]]
];
export var NetworkConnection$: StaticStructureSchema = [3, n0, _NC,
  0,
  [_Di],
  [[0, { [_jN]: _di }]], 1
];
export var NetworkConnectionAction$: StaticStructureSchema = [3, n0, _NCA,
  0,
  [_Bl, _CDo, _LPD, _Prot, _LID, _LNI, _RID, _RPD],
  [[2, { [_jN]: _bl }], [0, { [_jN]: _cD }], [() => LocalPortDetails$, { [_jN]: _lPD }], [0, { [_jN]: _prot }], [() => LocalIpDetails$, { [_jN]: _lID }], [0, { [_jN]: _lNI }], [() => RemoteIpDetails$, { [_jN]: _rID }], [() => RemotePortDetails$, { [_jN]: _rPD }]]
];
export var NetworkEndpoint$: StaticStructureSchema = [3, n0, _NEe,
  0,
  [_I, _Ip, _Do, _Po, _Lo, _ASu, _Conn],
  [[0, { [_jN]: _i }], [0, { [_jN]: _ip }], [0, { [_jN]: _do }], [1, { [_jN]: _po }], [() => NetworkGeoLocation$, { [_jN]: _lo }], [() => AutonomousSystem$, { [_jN]: _aSu }], [() => NetworkConnection$, { [_jN]: _conn }]], 1
];
export var NetworkGeoLocation$: StaticStructureSchema = [3, n0, _NGL,
  0,
  [_C, _Cou, _Lat, _Long],
  [[0, { [_jN]: _ci }], [0, { [_jN]: _coun }], [1, { [_jN]: _la }], [1, { [_jN]: _lon }]], 4
];
export var NetworkInterface$: StaticStructureSchema = [3, n0, _NIe,
  0,
  [_IAp, _NII, _PDN, _PIAr, _PIA, _PDNu, _PIu, _SG, _SIu, _VIp],
  [[64 | 0, { [_jN]: _iAp }], [0, { [_jN]: _nII }], [0, { [_jN]: _pDN }], [() => SensitiveString, { [_jN]: _pIAr }], [() => PrivateIpAddresses, { [_jN]: _pIA }], [0, { [_jN]: _pDNu }], [0, { [_jN]: _pIu }], [() => SecurityGroups, { [_jN]: _sG }], [0, { [_jN]: _sIu }], [0, { [_jN]: _vIp }]]
];
export var Observations$: StaticStructureSchema = [3, n0, _O,
  0,
  [_Te, _Nu],
  [[64 | 0, { [_jN]: _te }], [64 | 1, { [_jN]: _nu }]]
];
export var Organization$: StaticStructureSchema = [3, n0, _Or,
  0,
  [_As, _AOs, _Isp, _Org],
  [[0, { [_jN]: _as }], [0, { [_jN]: _aO }], [0, { [_jN]: _isp }], [0, { [_jN]: _or }]]
];
export var OrganizationAdditionalConfiguration$: StaticStructureSchema = [3, n0, _OAC,
  0,
  [_N, _AE],
  [[0, { [_jN]: _n }], [0, { [_jN]: _aE }]]
];
export var OrganizationAdditionalConfigurationResult$: StaticStructureSchema = [3, n0, _OACR,
  0,
  [_N, _AE],
  [[0, { [_jN]: _n }], [0, { [_jN]: _aE }]]
];
export var OrganizationDataSourceConfigurations$: StaticStructureSchema = [3, n0, _ODSC,
  0,
  [_SL, _K, _MP],
  [[() => OrganizationS3LogsConfiguration$, { [_jN]: _sL }], [() => OrganizationKubernetesConfiguration$, { [_jN]: _k }], [() => OrganizationMalwareProtectionConfiguration$, { [_jN]: _mP }]]
];
export var OrganizationDataSourceConfigurationsResult$: StaticStructureSchema = [3, n0, _ODSCR,
  0,
  [_SL, _K, _MP],
  [[() => OrganizationS3LogsConfigurationResult$, { [_jN]: _sL }], [() => OrganizationKubernetesConfigurationResult$, { [_jN]: _k }], [() => OrganizationMalwareProtectionConfigurationResult$, { [_jN]: _mP }]], 1
];
export var OrganizationDetails$: StaticStructureSchema = [3, n0, _OD,
  0,
  [_UAp, _OS],
  [[4, { [_jN]: _uAp }], [() => OrganizationStatistics$, { [_jN]: _oS }]]
];
export var OrganizationEbsVolumes$: StaticStructureSchema = [3, n0, _OEV,
  0,
  [_AE],
  [[2, { [_jN]: _aE }]]
];
export var OrganizationEbsVolumesResult$: StaticStructureSchema = [3, n0, _OEVR,
  0,
  [_AE],
  [[2, { [_jN]: _aE }]]
];
export var OrganizationFeatureConfiguration$: StaticStructureSchema = [3, n0, _OFC,
  0,
  [_N, _AE, _ACd],
  [[0, { [_jN]: _n }], [0, { [_jN]: _aE }], [() => OrganizationAdditionalConfigurations, { [_jN]: _aC }]]
];
export var OrganizationFeatureConfigurationResult$: StaticStructureSchema = [3, n0, _OFCR,
  0,
  [_N, _AE, _ACd],
  [[0, { [_jN]: _n }], [0, { [_jN]: _aE }], [() => OrganizationAdditionalConfigurationResults, { [_jN]: _aC }]]
];
export var OrganizationFeatureStatistics$: StaticStructureSchema = [3, n0, _OFS,
  0,
  [_N, _EAC, _ACd],
  [[0, { [_jN]: _n }], [1, { [_jN]: _eAC }], [() => OrganizationFeatureStatisticsAdditionalConfigurations, { [_jN]: _aC }]]
];
export var OrganizationFeatureStatisticsAdditionalConfiguration$: StaticStructureSchema = [3, n0, _OFSAC,
  0,
  [_N, _EAC],
  [[0, { [_jN]: _n }], [1, { [_jN]: _eAC }]]
];
export var OrganizationKubernetesAuditLogsConfiguration$: StaticStructureSchema = [3, n0, _OKALC,
  0,
  [_AE],
  [[2, { [_jN]: _aE }]], 1
];
export var OrganizationKubernetesAuditLogsConfigurationResult$: StaticStructureSchema = [3, n0, _OKALCR,
  0,
  [_AE],
  [[2, { [_jN]: _aE }]], 1
];
export var OrganizationKubernetesConfiguration$: StaticStructureSchema = [3, n0, _OKC,
  0,
  [_AL],
  [[() => OrganizationKubernetesAuditLogsConfiguration$, { [_jN]: _aL }]], 1
];
export var OrganizationKubernetesConfigurationResult$: StaticStructureSchema = [3, n0, _OKCR,
  0,
  [_AL],
  [[() => OrganizationKubernetesAuditLogsConfigurationResult$, { [_jN]: _aL }]], 1
];
export var OrganizationMalwareProtectionConfiguration$: StaticStructureSchema = [3, n0, _OMPC,
  0,
  [_SEIWF],
  [[() => OrganizationScanEc2InstanceWithFindings$, { [_jN]: _sEIWF }]]
];
export var OrganizationMalwareProtectionConfigurationResult$: StaticStructureSchema = [3, n0, _OMPCR,
  0,
  [_SEIWF],
  [[() => OrganizationScanEc2InstanceWithFindingsResult$, { [_jN]: _sEIWF }]]
];
export var OrganizationS3LogsConfiguration$: StaticStructureSchema = [3, n0, _OSLC,
  0,
  [_AE],
  [[2, { [_jN]: _aE }]], 1
];
export var OrganizationS3LogsConfigurationResult$: StaticStructureSchema = [3, n0, _OSLCR,
  0,
  [_AE],
  [[2, { [_jN]: _aE }]], 1
];
export var OrganizationScanEc2InstanceWithFindings$: StaticStructureSchema = [3, n0, _OSEIWF,
  0,
  [_EVb],
  [[() => OrganizationEbsVolumes$, { [_jN]: _eVb }]]
];
export var OrganizationScanEc2InstanceWithFindingsResult$: StaticStructureSchema = [3, n0, _OSEIWFR,
  0,
  [_EVb],
  [[() => OrganizationEbsVolumesResult$, { [_jN]: _eVb }]]
];
export var OrganizationStatistics$: StaticStructureSchema = [3, n0, _OS,
  0,
  [_TAC, _MACe, _AAC, _EAC, _CBF],
  [[1, { [_jN]: _tAC }], [1, { [_jN]: _mAC }], [1, { [_jN]: _aAC }], [1, { [_jN]: _eAC }], [() => OrganizationFeatureStatisticsResults, { [_jN]: _cBF }]]
];
export var Owner$: StaticStructureSchema = [3, n0, _Ow,
  0,
  [_I],
  [[0, { [_jN]: _i }]]
];
export var PermissionConfiguration$: StaticStructureSchema = [3, n0, _PCe,
  0,
  [_BLP, _ALP],
  [[() => BucketLevelPermissions$, { [_jN]: _bLP }], [() => AccountLevelPermissions$, { [_jN]: _aLP }]]
];
export var PortProbeAction$: StaticStructureSchema = [3, n0, _PPA,
  0,
  [_Bl, _PPD],
  [[2, { [_jN]: _bl }], [() => PortProbeDetails, { [_jN]: _pPD }]]
];
export var PortProbeDetail$: StaticStructureSchema = [3, n0, _PPDo,
  0,
  [_LPD, _LID, _RID],
  [[() => LocalPortDetails$, { [_jN]: _lPD }], [() => LocalIpDetails$, { [_jN]: _lID }], [() => RemoteIpDetails$, { [_jN]: _rID }]]
];
export var PrivateIpAddressDetails$: StaticStructureSchema = [3, n0, _PIAD,
  0,
  [_PDN, _PIAr],
  [[0, { [_jN]: _pDN }], [() => SensitiveString, { [_jN]: _pIAr }]]
];
export var ProcessDetails$: StaticStructureSchema = [3, n0, _PD,
  0,
  [_N, _EP, _ESx, _NP, _Pw, _Pi, _ST, _Uu, _PU, _Us, _UI, _Eu, _Li],
  [[0, { [_jN]: _n }], [0, { [_jN]: _eP }], [0, { [_jN]: _eS }], [1, { [_jN]: _nP }], [0, { [_jN]: _pw }], [1, { [_jN]: _pi }], [4, { [_jN]: _sT }], [0, { [_jN]: _uu }], [0, { [_jN]: _pU }], [0, { [_jN]: _us }], [1, { [_jN]: _uI }], [1, { [_jN]: _eu }], [() => Lineage, { [_jN]: _li }]]
];
export var Product$: StaticStructureSchema = [3, n0, _Prod,
  0,
  [_N, _Fe],
  [[0, { [_jN]: _n }], [0, { [_jN]: _fe }]], 1
];
export var ProductCode$: StaticStructureSchema = [3, n0, _PCr,
  0,
  [_Cod, _PTr],
  [[0, { [_jN]: _pCI }], [0, { [_jN]: _pCT }]]
];
export var PublicAccess$: StaticStructureSchema = [3, n0, _PA,
  0,
  [_PCe, _EPf],
  [[() => PermissionConfiguration$, { [_jN]: _pCe }], [0, { [_jN]: _ePf }]]
];
export var PublicAccessConfiguration$: StaticStructureSchema = [3, n0, _PAC,
  0,
  [_PAA, _PPAu, _PAIB, _PBRB],
  [[0, { [_jN]: _pAA }], [0, { [_jN]: _pPAu }], [0, { [_jN]: _pAIB }], [0, { [_jN]: _pBRB }]]
];
export var RdsDbInstanceDetails$: StaticStructureSchema = [3, n0, _RDID,
  0,
  [_DII, _Eng, _EVn, _DCI, _DIA, _DRI, _Ta],
  [[0, { [_jN]: _dII }], [0, { [_jN]: _eng }], [0, { [_jN]: _eVn }], [0, { [_jN]: _dCI }], [0, { [_jN]: _dIA }], [0, { [_jN]: _dRI }], [() => Tags, { [_jN]: _ta }]]
];
export var RdsDbUserDetails$: StaticStructureSchema = [3, n0, _RDUD,
  0,
  [_Us, _App, _Dat, _Ss, _AM],
  [[0, { [_jN]: _us }], [0, { [_jN]: _app }], [0, { [_jN]: _dat }], [0, { [_jN]: _ss }], [0, { [_jN]: _aM }]]
];
export var RdsLimitlessDbDetails$: StaticStructureSchema = [3, n0, _RLDD,
  0,
  [_DSGI, _DSGRI, _DSGA, _Eng, _EVn, _DCI, _Ta],
  [[0, { [_jN]: _dSGI }], [0, { [_jN]: _dSGRI }], [0, { [_jN]: _dSGA }], [0, { [_jN]: _eng }], [0, { [_jN]: _eVn }], [0, { [_jN]: _dCI }], [() => Tags, { [_jN]: _ta }]]
];
export var RdsLoginAttemptAction$: StaticStructureSchema = [3, n0, _RLAA,
  0,
  [_RID, _LAo],
  [[() => RemoteIpDetails$, { [_jN]: _rID }], [() => LoginAttributes, 0]]
];
export var RecoveryPoint$: StaticStructureSchema = [3, n0, _RP,
  0,
  [_BVN, _CSD],
  [[0, { [_jN]: _bVN }], [() => ContinuousScanDetails$, { [_jN]: _cSD }]], 1
];
export var RecoveryPointDetails$: StaticStructureSchema = [3, n0, _RPDe,
  0,
  [_RPA, _BVN, _CSD],
  [[0, { [_jN]: _rPA }], [0, { [_jN]: _bVN }], [() => ScanConfigurationContinuousScanDetails$, { [_jN]: _cSD }]]
];
export var RemoteAccountDetails$: StaticStructureSchema = [3, n0, _RAD,
  0,
  [_AIc, _Af],
  [[0, { [_jN]: _aIc }], [2, { [_jN]: _af }]]
];
export var RemoteIpDetails$: StaticStructureSchema = [3, n0, _RID,
  0,
  [_C, _Cou, _GL, _IAV, _IAVp, _Or],
  [[() => City$, { [_jN]: _ci }], [() => Country$, { [_jN]: _coun }], [() => GeoLocation$, { [_jN]: _gL }], [() => SensitiveString, { [_jN]: _iAV }], [() => SensitiveString, { [_jN]: _iAVp }], [() => Organization$, { [_jN]: _org }]]
];
export var RemotePortDetails$: StaticStructureSchema = [3, n0, _RPD,
  0,
  [_Po, _PN],
  [[1, { [_jN]: _po }], [0, { [_jN]: _pN }]]
];
export var Resource$: StaticStructureSchema = [3, n0, _Res,
  0,
  [_AKD, _SBD, _IDn, _ECD, _KD, _RT, _EVD, _ECDc, _CDon, _LD, _RDID, _RLDD, _RDUD, _ESD, _EIDc, _RPDe, _BGD, _MDo],
  [[() => AccessKeyDetails$, { [_jN]: _aKD }], [() => S3BucketDetails, { [_jN]: _sBD }], [() => InstanceDetails$, { [_jN]: _iDn }], [() => EksClusterDetails$, { [_jN]: _eCD }], [() => KubernetesDetails$, { [_jN]: _kD }], [0, { [_jN]: _rT }], [() => EbsVolumeDetails$, { [_jN]: _eVD }], [() => EcsClusterDetails$, { [_jN]: _eCDc }], [() => Container$, { [_jN]: _cDo }], [() => LambdaDetails$, { [_jN]: _lD }], [() => RdsDbInstanceDetails$, { [_jN]: _rDID }], [() => RdsLimitlessDbDetails$, { [_jN]: _rLDD }], [() => RdsDbUserDetails$, { [_jN]: _rDUD }], [() => EbsSnapshotDetails$, { [_jN]: _eSD }], [() => Ec2ImageDetails$, { [_jN]: _eIDc }], [() => RecoveryPointDetails$, { [_jN]: _rPDe }], [() => BedrockGuardrailDetails$, { [_jN]: _bGD }], [() => ModelDetails, { [_jN]: _mD }]]
];
export var ResourceData$: StaticStructureSchema = [3, n0, _RDe,
  0,
  [_SB, _EIc, _AK, _ENI, _SO, _ECk, _KW, _Con, _ECc, _ETc, _IIP, _AASG, _ELT, _EV, _EI, _CS],
  [[() => S3Bucket$, { [_jN]: _sB }], [() => Ec2Instance$, { [_jN]: _eI }], [() => AccessKey$, { [_jN]: _aK }], [() => Ec2NetworkInterface$, { [_jN]: _eNI }], [() => S3Object$, { [_jN]: _sO }], [() => EksCluster$, { [_jN]: _eCk }], [() => KubernetesWorkload$, { [_jN]: _kW }], [() => ContainerFindingResource$, { [_jN]: _conta }], [() => EcsCluster$, { [_jN]: _eCc }], [() => EcsTask$, { [_jN]: _eTc }], [() => IamInstanceProfileV2$, { [_jN]: _iIP }], [() => AutoscalingAutoScalingGroup$, { [_jN]: _aASG }], [() => Ec2LaunchTemplate$, { [_jN]: _eLT }], [() => Ec2Vpc$, { [_jN]: _eVc }], [() => Ec2Image$, { [_jN]: _eIc }], [() => CloudformationStack$, { [_jN]: _cSl }]]
];
export var ResourceDetails$: StaticStructureSchema = [3, n0, _RD,
  0,
  [_IAn],
  [[0, { [_jN]: _iAn }]]
];
export var ResourceStatistics$: StaticStructureSchema = [3, n0, _RSe,
  0,
  [_AIc, _LGA, _RIe, _RT, _TF],
  [[0, { [_jN]: _aIc }], [4, { [_jN]: _lGA }], [0, { [_jN]: _rIe }], [0, { [_jN]: _rT }], [1, { [_jN]: _tF }]]
];
export var ResourceV2$: StaticStructureSchema = [3, n0, _RV,
  0,
  [_U, _RT, _N, _AIc, _R, _Se, _CP, _Ta, _Data],
  [[0, { [_jN]: _u }], [0, { [_jN]: _rT }], [0, { [_jN]: _n }], [0, { [_jN]: _aIc }], [0, { [_jN]: _r }], [0, { [_jN]: _ser }], [0, { [_jN]: _cP }], [() => Tags, { [_jN]: _ta }], [() => ResourceData$, { [_jN]: _data }]], 2
];
export var RuleDefinition$: StaticStructureSchema = [3, n0, _RDu,
  0,
  [_Ex],
  [[0, { [_jN]: _ex }]], 1
];
export var RuleDetail$: StaticStructureSchema = [3, n0, _RDul,
  0,
  [_RI, _Ar, _N, _D, _Sev, _DSat, _Tac, _Tec, _Se, _Def, _CA, _Lan, _Sch, _UAp],
  [[0, { [_jN]: _rI }], [0, { [_jN]: _ar }], [0, { [_jN]: _n }], [0, { [_jN]: _d }], [0, { [_jN]: _sev }], [0, { [_jN]: _dSa }], [0, { [_jN]: _tac }], [0, { [_jN]: _tec }], [0, { [_jN]: _ser }], [() => RuleDefinition$, { [_jN]: _def }], [4, { [_jN]: _cA }], [0, { [_jN]: _lan }], [0, { [_jN]: _sch }], [4, { [_jN]: _uAp }]], 11
];
export var RuleSummary$: StaticStructureSchema = [3, n0, _RSu,
  0,
  [_RI, _Ar, _N, _D, _Sev, _DSat, _Tac, _Tec, _Se, _CA, _Lan, _Sch, _UAp],
  [[0, { [_jN]: _rI }], [0, { [_jN]: _ar }], [0, { [_jN]: _n }], [0, { [_jN]: _d }], [0, { [_jN]: _sev }], [0, { [_jN]: _dSa }], [0, { [_jN]: _tac }], [0, { [_jN]: _tec }], [0, { [_jN]: _ser }], [4, { [_jN]: _cA }], [0, { [_jN]: _lan }], [0, { [_jN]: _sch }], [4, { [_jN]: _uAp }]], 10
];
export var RuntimeContext$: StaticStructureSchema = [3, n0, _RC,
  0,
  [_MPo, _MA, _SP, _LP, _LPV, _SPo, _RBP, _RAP, _MSo, _MTo, _FSTi, _Fl, _MN, _MFP, _MSod, _SHFP, _TPa, _AF, _IPN, _MRe, _TNo, _TC, _SN, _CLE, _TFP, _FO, _FP, _RFP],
  [[() => ProcessDetails$, { [_jN]: _mPo }], [4, { [_jN]: _mA }], [0, { [_jN]: _sP }], [0, { [_jN]: _lP }], [0, { [_jN]: _lPV }], [0, { [_jN]: _sPo }], [0, { [_jN]: _rBP }], [0, { [_jN]: _rAP }], [0, { [_jN]: _mS }], [0, { [_jN]: _mTo }], [0, { [_jN]: _fSTi }], [64 | 0, { [_jN]: _fl }], [0, { [_jN]: _mN }], [0, { [_jN]: _mFP }], [0, { [_jN]: _mSo }], [0, { [_jN]: _sHFP }], [() => ProcessDetails$, { [_jN]: _tPa }], [0, { [_jN]: _aF }], [1, { [_jN]: _iPN }], [64 | 0, { [_jN]: _mRe }], [0, { [_jN]: _tNo }], [0, { [_jN]: _tC }], [0, { [_jN]: _sN }], [0, { [_jN]: _cLE }], [0, { [_jN]: _tFP }], [0, { [_jN]: _fO }], [0, { [_jN]: _fP }], [64 | 0, { [_jN]: _rFP }]]
];
export var RuntimeDetails$: StaticStructureSchema = [3, n0, _RDun,
  0,
  [_P, _Conte],
  [[() => ProcessDetails$, { [_jN]: _p }], [() => RuntimeContext$, { [_jN]: _conte }]]
];
export var S3Bucket$: StaticStructureSchema = [3, n0, _SB,
  0,
  [_OI, _CA, _ETn, _EKA, _EPf, _PRA, _PWA, _APA, _BPAu, _SOU],
  [[0, { [_jN]: _oI }], [4, { [_jN]: _cA }], [0, { [_jN]: _eTn }], [0, { [_jN]: _eKA }], [0, { [_jN]: _ePf }], [0, { [_jN]: _pRA }], [0, { [_jN]: _pWA }], [() => PublicAccessConfiguration$, { [_jN]: _aPA }], [() => PublicAccessConfiguration$, { [_jN]: _bPAu }], [64 | 0, { [_jN]: _sOU }]]
];
export var S3BucketDetail$: StaticStructureSchema = [3, n0, _SBDu,
  0,
  [_Ar, _N, _T, _CA, _Ow, _Ta, _DSSE, _PA, _SOD],
  [[0, { [_jN]: _ar }], [0, { [_jN]: _n }], [0, { [_jN]: _t }], [4, { [_jN]: _cA }], [() => Owner$, { [_jN]: _ow }], [() => Tags, { [_jN]: _ta }], [() => DefaultServerSideEncryption$, { [_jN]: _dSSE }], [() => PublicAccess$, { [_jN]: _pA }], [() => S3ObjectDetails, { [_jN]: _sOD }]]
];
export var S3LogsConfiguration$: StaticStructureSchema = [3, n0, _SLC,
  0,
  [_En],
  [[2, { [_jN]: _en }]], 1
];
export var S3LogsConfigurationResult$: StaticStructureSchema = [3, n0, _SLCR,
  0,
  [_St],
  [[0, { [_jN]: _st }]], 1
];
export var S3Object$: StaticStructureSchema = [3, n0, _SO,
  0,
  [_ETa, _Ke, _VI],
  [[0, { [_jN]: _eTa }], [0, { [_jN]: _ke }], [0, { [_jN]: _vI }]]
];
export var S3ObjectDetail$: StaticStructureSchema = [3, n0, _SODb,
  0,
  [_OAb, _Ke, _ETa, _H, _VI],
  [[0, { [_jN]: _oAb }], [0, { [_jN]: _ke }], [0, { [_jN]: _eTa }], [0, { [_jN]: _h }], [0, { [_jN]: _vI }]]
];
export var S3ObjectForSendObjectMalwareScan$: StaticStructureSchema = [3, n0, _SOFSOMS,
  0,
  [_Bu, _Ke, _VI],
  [[0, { [_jN]: _bu }], [0, { [_jN]: _ke }], [0, { [_jN]: _vI }]]
];
export var Scan$: StaticStructureSchema = [3, n0, _Sca,
  0,
  [_DI, _ADI, _SI, _SSc, _FR, _SST, _SET, _TDr, _RD, _SRD, _AIc, _TB, _FCile, _AVt, _STc],
  [[0, { [_jN]: _dI }], [0, { [_jN]: _aDI }], [0, { [_jN]: _sI }], [0, { [_jN]: _sS }], [0, { [_jN]: _fR }], [4, { [_jN]: _sST }], [4, { [_jN]: _sET }], [() => TriggerDetails$, { [_jN]: _tDr }], [() => ResourceDetails$, { [_jN]: _rD }], [() => ScanResultDetails$, { [_jN]: _sRD }], [0, { [_jN]: _aIc }], [1, { [_jN]: _tB }], [1, { [_jN]: _fCile }], [() => VolumeDetails, { [_jN]: _aVt }], [0, { [_jN]: _sTc }]]
];
export var ScanCondition$: StaticStructureSchema = [3, n0, _SCcan,
  0,
  [_ME],
  [[() => MapEquals, { [_jN]: _mE }]], 1
];
export var ScanConditionPair$: StaticStructureSchema = [3, n0, _SCP,
  0,
  [_Ke, _Val],
  [[0, { [_jN]: _ke }], [0, { [_jN]: _val }]], 1
];
export var ScanConfiguration$: StaticStructureSchema = [3, n0, _SCc,
  0,
  [_Ro, _TDr, _ISD, _RP],
  [[0, { [_jN]: _ro }], [() => TriggerDetails$, { [_jN]: _tDr }], [() => IncrementalScanDetails$, { [_jN]: _iSD }], [() => ScanConfigurationRecoveryPoint$, { [_jN]: _rP }]]
];
export var ScanConfigurationContinuousScanDetails$: StaticStructureSchema = [3, n0, _SCCSD,
  0,
  [_ET, _ST],
  [[4, { [_jN]: _eT }], [4, { [_jN]: _sT }]], 1
];
export var ScanConfigurationRecoveryPoint$: StaticStructureSchema = [3, n0, _SCRP,
  0,
  [_BVN, _CSD],
  [[0, { [_jN]: _bVN }], [() => ScanConfigurationContinuousScanDetails$, { [_jN]: _cSD }]]
];
export var ScanDetections$: StaticStructureSchema = [3, n0, _SD,
  0,
  [_SIC, _TDIC, _HSTD, _TDBN],
  [[() => ScannedItemCount$, { [_jN]: _sIC }], [() => ThreatsDetectedItemCount$, { [_jN]: _tDIC }], [() => HighestSeverityThreatDetails$, { [_jN]: _hSTD }], [() => ThreatDetectedByName$, { [_jN]: _tDBN }]]
];
export var ScanEc2InstanceWithFindings$: StaticStructureSchema = [3, n0, _SEIWF,
  0,
  [_EVb],
  [[2, { [_jN]: _eVb }]]
];
export var ScanEc2InstanceWithFindingsResult$: StaticStructureSchema = [3, n0, _SEIWFR,
  0,
  [_EVb],
  [[() => EbsVolumesResult$, { [_jN]: _eVb }]]
];
export var ScanFilePath$: StaticStructureSchema = [3, n0, _SFP,
  0,
  [_FP, _VA, _H, _FNil],
  [[0, { [_jN]: _fP }], [0, { [_jN]: _vA }], [0, { [_jN]: _h }], [0, { [_jN]: _fNil }]]
];
export var ScannedItemCount$: StaticStructureSchema = [3, n0, _SIC,
  0,
  [_TG, _File, _Vo],
  [[1, { [_jN]: _tG }], [1, { [_jN]: _file }], [1, { [_jN]: _vo }]]
];
export var ScannedResource$: StaticStructureSchema = [3, n0, _SRca,
  0,
  [_SRA, _SRT, _SRSc, _SSR, _RD],
  [[0, { [_jN]: _sRA }], [0, { [_jN]: _sRT }], [0, { [_jN]: _sRSc }], [0, { [_jN]: _sSR }], [() => ScannedResourceDetails$, { [_jN]: _rD }]]
];
export var ScannedResourceDetails$: StaticStructureSchema = [3, n0, _SRDc,
  0,
  [_EVbs, _ES],
  [[() => VolumeDetail$, { [_jN]: _eVbs }], [() => EbsSnapshot$, { [_jN]: _eSb }]]
];
export var ScanResourceCriteria$: StaticStructureSchema = [3, n0, _SRCc,
  0,
  [_Inc, _Exc],
  [[() => ScanCriterion, { [_jN]: _inc }], [() => ScanCriterion, { [_jN]: _exc }]]
];
export var ScanResultDetails$: StaticStructureSchema = [3, n0, _SRD,
  0,
  [_SRcan],
  [[0, { [_jN]: _sRca }]]
];
export var ScanResultThreat$: StaticStructureSchema = [3, n0, _SRTc,
  0,
  [_N, _Sou, _Coun, _H, _IDt],
  [[0, { [_jN]: _n }], [0, { [_jN]: _sou }], [1, { [_jN]: _cou }], [0, { [_jN]: _h }], [() => ItemDetailsList, { [_jN]: _iDt }]]
];
export var ScanThreatName$: StaticStructureSchema = [3, n0, _STN,
  0,
  [_N, _Sev, _ICt, _FPi],
  [[0, { [_jN]: _n }], [0, { [_jN]: _sev }], [1, { [_jN]: _iCt }], [() => FilePaths, { [_jN]: _fPi }]]
];
export var SecurityContext$: StaticStructureSchema = [3, n0, _SC,
  0,
  [_Pri, _APE],
  [[2, { [_jN]: _pri }], [2, { [_jN]: _aPE }]]
];
export var SecurityGroup$: StaticStructureSchema = [3, n0, _SGe,
  0,
  [_GI, _GN],
  [[0, { [_jN]: _gI }], [0, { [_jN]: _gN }]]
];
export var SendObjectMalwareScanRequest$: StaticStructureSchema = [3, n0, _SOMSR,
  0,
  [_SO],
  [[() => S3ObjectForSendObjectMalwareScan$, { [_jN]: _sO }]]
];
export var SendObjectMalwareScanResponse$: StaticStructureSchema = [3, n0, _SOMSRe,
  0,
  [],
  []
];
export var Sequence$: StaticStructureSchema = [3, n0, _Seq,
  0,
  [_U, _D, _Si, _Actor, _Reso, _End, _SIe, _AST],
  [[0, { [_jN]: _u }], [0, { [_jN]: _d }], [() => Signals, { [_jN]: _si }], [() => Actors, { [_jN]: _acto }], [() => Resources, { [_jN]: _reso }], [() => NetworkEndpoints, { [_jN]: _end }], [() => Indicators, { [_jN]: _sIe }], [64 | 0, { [_jN]: _aST }]], 3
];
export var Service$: StaticStructureSchema = [3, n0, _Se,
  0,
  [_Ac, _Ev, _Arc, _Coun, _DI, _EFS, _ELS, _RR, _SN, _UF, _AId, _FNe, _EVSD, _RDun, _Det, _MSD],
  [[() => Action$, { [_jN]: _ac }], [() => Evidence$, { [_jN]: _ev }], [2, { [_jN]: _arc }], [1, { [_jN]: _cou }], [0, { [_jN]: _dI }], [0, { [_jN]: _eFS }], [0, { [_jN]: _eLS }], [0, { [_jN]: _rR }], [0, { [_jN]: _sN }], [0, { [_jN]: _uF }], [() => ServiceAdditionalInfo$, { [_jN]: _aId }], [0, { [_jN]: _fNe }], [() => EbsVolumeScanDetails$, { [_jN]: _eVSD }], [() => RuntimeDetails$, { [_jN]: _rDu }], [() => Detection$, { [_jN]: _det }], [() => MalwareScanDetails$, { [_jN]: _mSD }]]
];
export var ServiceAdditionalInfo$: StaticStructureSchema = [3, n0, _SAI,
  0,
  [_Val, _T],
  [[0, { [_jN]: _val }], [0, { [_jN]: _t }]]
];
export var Session$: StaticStructureSchema = [3, n0, _S,
  0,
  [_U, _MSf, _CTr, _Issu],
  [[0, { [_jN]: _u }], [0, { [_jN]: _mSf }], [4, { [_jN]: _cTr }], [0, { [_jN]: _issu }]]
];
export var SeverityStatistics$: StaticStructureSchema = [3, n0, _SSe,
  0,
  [_LGA, _Sev, _TF],
  [[4, { [_jN]: _lGA }], [1, { [_jN]: _sev }], [1, { [_jN]: _tF }]]
];
export var Signal$: StaticStructureSchema = [3, n0, _Sig,
  0,
  [_U, _T, _N, _CA, _UAp, _FSA, _LSA, _Coun, _D, _Sev, _RUe, _AIct, _EIn, _SIi, _Activ],
  [[0, { [_jN]: _u }], [0, { [_jN]: _t }], [0, { [_jN]: _n }], [4, { [_jN]: _cA }], [4, { [_jN]: _uAp }], [4, { [_jN]: _fSA }], [4, { [_jN]: _lSA }], [1, { [_jN]: _cou }], [0, { [_jN]: _d }], [1, { [_jN]: _sev }], [64 | 0, { [_jN]: _rUe }], [64 | 0, { [_jN]: _aIct }], [64 | 0, { [_jN]: _eIn }], [() => Indicators, { [_jN]: _sIi }], [() => Activities, { [_jN]: _activ }]], 8
];
export var SortCriteria$: StaticStructureSchema = [3, n0, _SCo,
  0,
  [_AN, _OB],
  [[0, { [_jN]: _aN }], [0, { [_jN]: _oB }]]
];
export var StartMalwareScanConfiguration$: StaticStructureSchema = [3, n0, _SMSC,
  0,
  [_Ro, _ISD, _RP],
  [[0, { [_jN]: _ro }], [() => IncrementalScanDetails$, { [_jN]: _iSD }], [() => RecoveryPoint$, { [_jN]: _rP }]], 1
];
export var StartMalwareScanRequest$: StaticStructureSchema = [3, n0, _SMSR,
  0,
  [_RAe, _CTl, _SCc],
  [[0, { [_jN]: _rAe }], [0, { [_iTd]: 1, [_jN]: _cTl }], [() => StartMalwareScanConfiguration$, { [_jN]: _sCc }]], 1
];
export var StartMalwareScanResponse$: StaticStructureSchema = [3, n0, _SMSRt,
  0,
  [_SI],
  [[0, { [_jN]: _sI }]]
];
export var StartMonitoringMembersRequest$: StaticStructureSchema = [3, n0, _SMMR,
  0,
  [_DI, _AIcc],
  [[0, 1], [64 | 0, { [_jN]: _aIcc }]], 2
];
export var StartMonitoringMembersResponse$: StaticStructureSchema = [3, n0, _SMMRt,
  0,
  [_UAn],
  [[() => UnprocessedAccounts, { [_jN]: _uAn }]], 1
];
export var StopMonitoringMembersRequest$: StaticStructureSchema = [3, n0, _SMMRto,
  0,
  [_DI, _AIcc],
  [[0, 1], [64 | 0, { [_jN]: _aIcc }]], 2
];
export var StopMonitoringMembersResponse$: StaticStructureSchema = [3, n0, _SMMRtop,
  0,
  [_UAn],
  [[() => UnprocessedAccounts, { [_jN]: _uAn }]], 1
];
export var Tag$: StaticStructureSchema = [3, n0, _Tag_,
  0,
  [_Ke, _Val],
  [[0, { [_jN]: _ke }], [0, { [_jN]: _val }]]
];
export var TagResourceRequest$: StaticStructureSchema = [3, n0, _TRR,
  0,
  [_RAe, _Ta],
  [[0, 1], [128 | 0, { [_jN]: _ta }]], 2
];
export var TagResourceResponse$: StaticStructureSchema = [3, n0, _TRRa,
  0,
  [],
  []
];
export var Threat$: StaticStructureSchema = [3, n0, _Thr,
  0,
  [_N, _Sou, _IPte, _Coun, _H, _IDt],
  [[0, { [_jN]: _n }], [0, { [_jN]: _sou }], [() => ItemPaths, { [_jN]: _iPte }], [1, { [_jN]: _cou }], [0, { [_jN]: _h }], [() => ItemDetailsList, { [_jN]: _iDt }]]
];
export var ThreatDetectedByName$: StaticStructureSchema = [3, n0, _TDBN,
  0,
  [_ICt, _UTNC, _Sho, _TNh],
  [[1, { [_jN]: _iCt }], [1, { [_jN]: _uTNC }], [2, { [_jN]: _sho }], [() => ScanThreatNames, { [_jN]: _tNh }]]
];
export var ThreatIntelligenceDetail$: StaticStructureSchema = [3, n0, _TIDh,
  0,
  [_TLN, _TNh, _TFS],
  [[0, { [_jN]: _tLN }], [64 | 0, { [_jN]: _tNh }], [0, { [_jN]: _tFS }]]
];
export var ThreatsDetectedItemCount$: StaticStructureSchema = [3, n0, _TDIC,
  0,
  [_File],
  [[1, { [_jN]: _file }]]
];
export var Total$: StaticStructureSchema = [3, n0, _To,
  0,
  [_Am, _Uni],
  [[0, { [_jN]: _am }], [0, { [_jN]: _uni }]]
];
export var TriggerDetails$: StaticStructureSchema = [3, n0, _TDr,
  0,
  [_GDFI, _D, _TT],
  [[0, { [_jN]: _gDFI }], [0, { [_jN]: _d }], [0, { [_jN]: _tT }]]
];
export var UnarchiveFindingsRequest$: StaticStructureSchema = [3, n0, _UFR,
  0,
  [_DI, _FI],
  [[0, 1], [64 | 0, { [_jN]: _fI }]], 2
];
export var UnarchiveFindingsResponse$: StaticStructureSchema = [3, n0, _UFRn,
  0,
  [],
  []
];
export var UnprocessedAccount$: StaticStructureSchema = [3, n0, _UAnp,
  0,
  [_AIc, _Resu],
  [[0, { [_jN]: _aIc }], [0, { [_jN]: _resu }]], 2
];
export var UnprocessedDataSourcesResult$: StaticStructureSchema = [3, n0, _UDSR,
  0,
  [_MP],
  [[() => MalwareProtectionConfigurationResult$, { [_jN]: _mP }]]
];
export var UntagResourceRequest$: StaticStructureSchema = [3, n0, _URR,
  0,
  [_RAe, _TK],
  [[0, 1], [64 | 0, { [_hQ]: _tK }]], 2
];
export var UntagResourceResponse$: StaticStructureSchema = [3, n0, _URRn,
  0,
  [],
  []
];
export var UpdateCustomDetectionRuleAssociationRequest$: StaticStructureSchema = [3, n0, _UCDRAR,
  0,
  [_RI, _AIs, _Mo],
  [[0, 1], [0, 1], [0, { [_jN]: _mo }]], 3
];
export var UpdateCustomDetectionRuleAssociationResponse$: StaticStructureSchema = [3, n0, _UCDRARp,
  0,
  [],
  []
];
export var UpdateCustomDetectionRuleOrgConfigurationRequest$: StaticStructureSchema = [3, n0, _UCDROCR,
  0,
  [_RI, _Mo, _IAI, _EAI],
  [[0, 1], [0, { [_jN]: _mo }], [64 | 0, { [_jN]: _iAI }], [64 | 0, { [_jN]: _eAI }]], 2
];
export var UpdateCustomDetectionRuleOrgConfigurationResponse$: StaticStructureSchema = [3, n0, _UCDROCRp,
  0,
  [],
  []
];
export var UpdateDetectorRequest$: StaticStructureSchema = [3, n0, _UDR,
  0,
  [_DI, _En, _FPF, _DS, _F],
  [[0, 1], [2, { [_jN]: _en }], [0, { [_jN]: _fPF }], [() => DataSourceConfigurations$, { [_jN]: _dS }], [() => DetectorFeatureConfigurations, { [_jN]: _f }]], 1
];
export var UpdateDetectorResponse$: StaticStructureSchema = [3, n0, _UDRp,
  0,
  [],
  []
];
export var UpdateFilterRequest$: StaticStructureSchema = [3, n0, _UFRp,
  0,
  [_DI, _FN, _D, _Ac, _Ra, _FCin],
  [[0, 1], [0, 1], [0, { [_jN]: _d }], [0, { [_jN]: _ac }], [1, { [_jN]: _ra }], [() => FindingCriteria$, { [_jN]: _fCin }]], 2
];
export var UpdateFilterResponse$: StaticStructureSchema = [3, n0, _UFRpd,
  0,
  [_N],
  [[0, { [_jN]: _n }]], 1
];
export var UpdateFindingsFeedbackRequest$: StaticStructureSchema = [3, n0, _UFFR,
  0,
  [_DI, _FI, _Fee, _Com],
  [[0, 1], [64 | 0, { [_jN]: _fI }], [0, { [_jN]: _fee }], [() => SensitiveString, { [_jN]: _com }]], 3
];
export var UpdateFindingsFeedbackResponse$: StaticStructureSchema = [3, n0, _UFFRp,
  0,
  [],
  []
];
export var UpdateIPSetRequest$: StaticStructureSchema = [3, n0, _UIPSR,
  0,
  [_DI, _ISI, _N, _Lo, _Acti, _EBO],
  [[0, 1], [0, 1], [0, { [_jN]: _n }], [0, { [_jN]: _lo }], [2, { [_jN]: _act }], [0, { [_jN]: _eBO }]], 2
];
export var UpdateIPSetResponse$: StaticStructureSchema = [3, n0, _UIPSRp,
  0,
  [],
  []
];
export var UpdateMalwareProtectionPlanRequest$: StaticStructureSchema = [3, n0, _UMPPR,
  0,
  [_MPPI, _Ro, _Actio, _PR],
  [[0, 1], [0, { [_jN]: _ro }], [() => MalwareProtectionPlanActions$, { [_jN]: _acti }], [() => UpdateProtectedResource$, { [_jN]: _pR }]], 1
];
export var UpdateMalwareScanSettingsRequest$: StaticStructureSchema = [3, n0, _UMSSR,
  0,
  [_DI, _SRCc, _ESP],
  [[0, 1], [() => ScanResourceCriteria$, { [_jN]: _sRCc }], [0, { [_jN]: _eSP }]], 1
];
export var UpdateMalwareScanSettingsResponse$: StaticStructureSchema = [3, n0, _UMSSRp,
  0,
  [],
  []
];
export var UpdateMemberDetectorsRequest$: StaticStructureSchema = [3, n0, _UMDR,
  0,
  [_DI, _AIcc, _DS, _F],
  [[0, 1], [64 | 0, { [_jN]: _aIcc }], [() => DataSourceConfigurations$, { [_jN]: _dS }], [() => MemberFeaturesConfigurations, { [_jN]: _f }]], 2
];
export var UpdateMemberDetectorsResponse$: StaticStructureSchema = [3, n0, _UMDRp,
  0,
  [_UAn],
  [[() => UnprocessedAccounts, { [_jN]: _uAn }]], 1
];
export var UpdateOrganizationConfigurationRequest$: StaticStructureSchema = [3, n0, _UOCR,
  0,
  [_DI, _AE, _DS, _F, _AEOM],
  [[0, 1], [2, { [_jN]: _aE }], [() => OrganizationDataSourceConfigurations$, { [_jN]: _dS }], [() => OrganizationFeaturesConfigurations, { [_jN]: _f }], [0, { [_jN]: _aEOM }]], 1
];
export var UpdateOrganizationConfigurationResponse$: StaticStructureSchema = [3, n0, _UOCRp,
  0,
  [],
  []
];
export var UpdateProtectedResource$: StaticStructureSchema = [3, n0, _UPR,
  0,
  [_SB],
  [[() => UpdateS3BucketResource$, { [_jN]: _sB }]]
];
export var UpdatePublishingDestinationRequest$: StaticStructureSchema = [3, n0, _UPDR,
  0,
  [_DI, _DIe, _DP],
  [[0, 1], [0, 1], [() => DestinationProperties$, { [_jN]: _dP }]], 2
];
export var UpdatePublishingDestinationResponse$: StaticStructureSchema = [3, n0, _UPDRp,
  0,
  [],
  []
];
export var UpdateS3BucketResource$: StaticStructureSchema = [3, n0, _USBR,
  0,
  [_OP],
  [[64 | 0, { [_jN]: _oP }]]
];
export var UpdateThreatEntitySetRequest$: StaticStructureSchema = [3, n0, _UTESR,
  0,
  [_DI, _TESI, _N, _Lo, _EBO, _Acti],
  [[0, 1], [0, 1], [0, { [_jN]: _n }], [0, { [_jN]: _lo }], [0, { [_jN]: _eBO }], [2, { [_jN]: _act }]], 2
];
export var UpdateThreatEntitySetResponse$: StaticStructureSchema = [3, n0, _UTESRp,
  0,
  [],
  []
];
export var UpdateThreatIntelSetRequest$: StaticStructureSchema = [3, n0, _UTISR,
  0,
  [_DI, _TISI, _N, _Lo, _Acti, _EBO],
  [[0, 1], [0, 1], [0, { [_jN]: _n }], [0, { [_jN]: _lo }], [2, { [_jN]: _act }], [0, { [_jN]: _eBO }]], 2
];
export var UpdateThreatIntelSetResponse$: StaticStructureSchema = [3, n0, _UTISRp,
  0,
  [],
  []
];
export var UpdateTrustedEntitySetRequest$: StaticStructureSchema = [3, n0, _UTESRpd,
  0,
  [_DI, _TESIr, _N, _Lo, _EBO, _Acti],
  [[0, 1], [0, 1], [0, { [_jN]: _n }], [0, { [_jN]: _lo }], [0, { [_jN]: _eBO }], [2, { [_jN]: _act }]], 2
];
export var UpdateTrustedEntitySetResponse$: StaticStructureSchema = [3, n0, _UTESRpda,
  0,
  [],
  []
];
export var UsageAccountResult$: StaticStructureSchema = [3, n0, _UAR,
  0,
  [_AIc, _To],
  [[0, { [_jN]: _aIc }], [() => Total$, { [_jN]: _to }]]
];
export var UsageCriteria$: StaticStructureSchema = [3, n0, _UC,
  0,
  [_AIcc, _DS, _Reso, _F],
  [[64 | 0, { [_jN]: _aIcc }], [64 | 0, { [_jN]: _dS }], [64 | 0, { [_jN]: _reso }], [64 | 0, { [_jN]: _f }]]
];
export var UsageDataSourceResult$: StaticStructureSchema = [3, n0, _UDSRs,
  0,
  [_DSat, _To],
  [[0, { [_jN]: _dSa }], [() => Total$, { [_jN]: _to }]]
];
export var UsageFeatureResult$: StaticStructureSchema = [3, n0, _UFRs,
  0,
  [_Fe, _To],
  [[0, { [_jN]: _fe }], [() => Total$, { [_jN]: _to }]]
];
export var UsageResourceResult$: StaticStructureSchema = [3, n0, _URRs,
  0,
  [_Res, _To],
  [[0, { [_jN]: _res }], [() => Total$, { [_jN]: _to }]]
];
export var UsageStatistics$: StaticStructureSchema = [3, n0, _US,
  0,
  [_SBA, _TABF, _SBDS, _SBR, _TR, _SBF],
  [[() => UsageAccountResultList, { [_jN]: _sBA }], [() => UsageTopAccountsResultList, { [_jN]: _tABF }], [() => UsageDataSourceResultList, { [_jN]: _sBDS }], [() => UsageResourceResultList, { [_jN]: _sBR }], [() => UsageResourceResultList, { [_jN]: _tR }], [() => UsageFeatureResultList, { [_jN]: _sBF }]]
];
export var UsageTopAccountResult$: StaticStructureSchema = [3, n0, _UTAR,
  0,
  [_AIc, _To],
  [[0, { [_jN]: _aIc }], [() => Total$, { [_jN]: _to }]]
];
export var UsageTopAccountsResult$: StaticStructureSchema = [3, n0, _UTARs,
  0,
  [_Fe, _Acc],
  [[0, { [_jN]: _fe }], [() => UsageTopAccountsByFeatureList, { [_jN]: _acc }]]
];
export var User$: StaticStructureSchema = [3, n0, _Us,
  0,
  [_N, _U, _T, _CUr, _A],
  [[0, { [_jN]: _n }], [0, { [_jN]: _u }], [0, { [_jN]: _t }], [0, { [_jN]: _cUr }], [() => Account$, { [_jN]: _a }]], 3
];
export var Volume$: StaticStructureSchema = [3, n0, _Vol,
  0,
  [_N, _HP],
  [[0, { [_jN]: _n }], [() => HostPath$, { [_jN]: _hP }]]
];
export var VolumeDetail$: StaticStructureSchema = [3, n0, _VD,
  0,
  [_VA, _VT, _DN, _VSIGB, _ETn, _SA, _KKA],
  [[0, { [_jN]: _vA }], [0, { [_jN]: _vT }], [0, { [_jN]: _dN }], [1, { [_jN]: _vSIGB }], [0, { [_jN]: _eTn }], [0, { [_jN]: _sA }], [0, { [_jN]: _kKA }]]
];
export var VolumeMount$: StaticStructureSchema = [3, n0, _VMo,
  0,
  [_N, _MPou],
  [[0, { [_jN]: _n }], [0, { [_jN]: _mPou }]]
];
export var VpcConfig$: StaticStructureSchema = [3, n0, _VC,
  0,
  [_SIub, _VIp, _SG],
  [[64 | 0, { [_jN]: _sIub }], [0, { [_jN]: _vIp }], [() => SecurityGroups, { [_jN]: _sG }]]
];
var __Unit = "unit" as const;
var AccountDetails: StaticListSchema = [1, n0, _ADc,
  0, [() => AccountDetail$,
    0]
];
var AccountFreeTrialInfos: StaticListSchema = [1, n0, _AFTIc,
  0, [() => AccountFreeTrialInfo$,
    0]
];
var AccountIds = 64 | 0;
var Activities: StaticListSchema = [1, n0, _Activ,
  0, [() => Activity$,
    0]
];
var ActorIds = 64 | 0;
var Actors: StaticListSchema = [1, n0, _Actor,
  0, [() => Actor$,
    0]
];
var AdditionalSequenceTypes = 64 | 0;
var AdminAccounts: StaticListSchema = [1, n0, _AAd,
  0, [() => AdminAccount$,
    0]
];
var AnomalyProfileFeatureObjects: StaticListSchema = [1, n0, _APFO,
  0, [() => AnomalyObject$,
    0]
];
var AssociationSummaryList: StaticListSchema = [1, n0, _ASL,
  0, [() => AssociationSummary$,
    0]
];
var BedrockGuardrails: StaticListSchema = [1, n0, _BGe,
  0, [() => BedrockGuardrail$,
    0]
];
var Containers: StaticListSchema = [1, n0, _Cont,
  0, [() => Container$,
    0]
];
var ContainerUids = 64 | 0;
var ContentPolicyFilters: StaticListSchema = [1, n0, _CPF,
  0, [() => ContentPolicyFilter$,
    0]
];
var CoverageFilterCriterionList: StaticListSchema = [1, n0, _CFCL,
  0, [() => CoverageFilterCriterion$,
    0]
];
var CoverageResources: StaticListSchema = [1, n0, _CRov,
  0, [() => CoverageResource$,
    0]
];
var CoverageStatisticsTypeList = 64 | 0;
var DataSourceList = 64 | 0;
var Destinations: StaticListSchema = [1, n0, _Des,
  0, [() => Destination$,
    0]
];
var DetectionRuleAccountIds = 64 | 0;
var DetectionRuleFilterList: StaticListSchema = [1, n0, _DRFL,
  0, [() => DetectionRuleFilter$,
    0]
];
var DetectionRuleFilterValues = 64 | 0;
var DetectionRuleOrgConfigurationSummaryList: StaticListSchema = [1, n0, _DROCSL,
  0, [() => DetectionRuleOrgConfigurationSummary$,
    0]
];
var DetectorAdditionalConfigurationResults: StaticListSchema = [1, n0, _DACRe,
  0, [() => DetectorAdditionalConfigurationResult$,
    0]
];
var DetectorAdditionalConfigurations: StaticListSchema = [1, n0, _DACe,
  0, [() => DetectorAdditionalConfiguration$,
    0]
];
var DetectorFeatureConfigurations: StaticListSchema = [1, n0, _DFCe,
  0, [() => DetectorFeatureConfiguration$,
    0]
];
var DetectorFeatureConfigurationsResults: StaticListSchema = [1, n0, _DFCRe,
  0, [() => DetectorFeatureConfigurationResult$,
    0]
];
var DetectorIds = 64 | 0;
var Ec2InstanceUids = 64 | 0;
var Ec2NetworkInterfaceUids = 64 | 0;
var EndpointIds = 64 | 0;
var Eq = 64 | 0;
var Equals = 64 | 0;
var FilePaths: StaticListSchema = [1, n0, _FPi,
  0, [() => ScanFilePath$,
    0]
];
var FilterCriterionList: StaticListSchema = [1, n0, _FCL,
  0, [() => FilterCriterion$,
    0]
];
var FilterNames = 64 | 0;
var FindingIds = 64 | 0;
var Findings: StaticListSchema = [1, n0, _Fin,
  0, [() => Finding$,
    0]
];
var FindingStatisticTypes = 64 | 0;
var FindingTypes = 64 | 0;
var FlagsList = 64 | 0;
var FreeTrialFeatureConfigurationsResults: StaticListSchema = [1, n0, _FTFCRr,
  0, [() => FreeTrialFeatureConfigurationResult$,
    0]
];
var GroupedByAccount: StaticListSchema = [1, n0, _GBA,
  0, [() => AccountStatistics$,
    0]
];
var GroupedByDate: StaticListSchema = [1, n0, _GBD,
  0, [() => DateStatistics$,
    0]
];
var GroupedByFindingType: StaticListSchema = [1, n0, _GBFT,
  0, [() => FindingTypeStatistics$,
    0]
];
var GroupedByResource: StaticListSchema = [1, n0, _GBR,
  0, [() => ResourceStatistics$,
    0]
];
var GroupedBySeverity: StaticListSchema = [1, n0, _GBS,
  0, [() => SeverityStatistics$,
    0]
];
var Groups = 64 | 0;
var Indicators: StaticListSchema = [1, n0, _Indi,
  0, [() => Indicator$,
    0]
];
var IndicatorValues = 64 | 0;
var InvestigationSummaries: StaticListSchema = [1, n0, _ISnv,
  0, [() => InvestigationSummary$,
    0]
];
var Invitations: StaticListSchema = [1, n0, _Invi,
  0, [() => Invitation$,
    0]
];
var IpSetIds = 64 | 0;
var Ipv6Addresses = 64 | 0;
var Issues = 64 | 0;
var ItemDetailsList: StaticListSchema = [1, n0, _IDL,
  0, [() => ItemDetails$,
    0]
];
var ItemPaths: StaticListSchema = [1, n0, _IPte,
  0, [() => ItemPath$,
    0]
];
var Lineage: StaticListSchema = [1, n0, _Li,
  0, [() => LineageObject$,
    0]
];
var ListMalwareScansFilterCriterionList: StaticListSchema = [1, n0, _LMSFCL,
  0, [() => ListMalwareScansFilterCriterion$,
    0]
];
var LoginAttributes: StaticListSchema = [1, n0, _LAo,
  0, [() => LoginAttribute$,
    0]
];
var MalwareProtectionPlanObjectPrefixesList = 64 | 0;
var MalwareProtectionPlansSummary: StaticListSchema = [1, n0, _MPPSa,
  0, [() => MalwareProtectionPlanSummary$,
    0]
];
var MalwareProtectionPlanStatusReasonsList: StaticListSchema = [1, n0, _MPPSRL,
  0, [() => MalwareProtectionPlanStatusReason$,
    0]
];
var MalwareScans: StaticListSchema = [1, n0, _MSa,
  0, [() => MalwareScan$,
    0]
];
var MapEquals: StaticListSchema = [1, n0, _ME,
  0, [() => ScanConditionPair$,
    0]
];
var Matches = 64 | 0;
var MemberAdditionalConfigurationResults: StaticListSchema = [1, n0, _MACRe,
  0, [() => MemberAdditionalConfigurationResult$,
    0]
];
var MemberAdditionalConfigurations: StaticListSchema = [1, n0, _MACem,
  0, [() => MemberAdditionalConfiguration$,
    0]
];
var MemberDataSourceConfigurations: StaticListSchema = [1, n0, _MDSC,
  0, [() => MemberDataSourceConfiguration$,
    0]
];
var MemberFeaturesConfigurations: StaticListSchema = [1, n0, _MFCe,
  0, [() => MemberFeaturesConfiguration$,
    0]
];
var MemberFeaturesConfigurationsResults: StaticListSchema = [1, n0, _MFCRe,
  0, [() => MemberFeaturesConfigurationResult$,
    0]
];
var Members: StaticListSchema = [1, n0, _Me,
  0, [() => Member$,
    0]
];
var MemoryRegionsList = 64 | 0;
var ModelDetails: StaticListSchema = [1, n0, _MDo,
  0, [() => ModelDetail$,
    0]
];
var Neq = 64 | 0;
var NetworkEndpoints: StaticListSchema = [1, n0, _NEet,
  0, [() => NetworkEndpoint$,
    0]
];
var NetworkInterfaces: StaticListSchema = [1, n0, _NI,
  0, [() => NetworkInterface$,
    0]
];
var NotEquals = 64 | 0;
var NotMatches = 64 | 0;
var ObservationNumbers = 64 | 1;
var ObservationTexts = 64 | 0;
var OrganizationAdditionalConfigurationResults: StaticListSchema = [1, n0, _OACRr,
  0, [() => OrganizationAdditionalConfigurationResult$,
    0]
];
var OrganizationAdditionalConfigurations: StaticListSchema = [1, n0, _OACr,
  0, [() => OrganizationAdditionalConfiguration$,
    0]
];
var OrganizationFeaturesConfigurations: StaticListSchema = [1, n0, _OFCr,
  0, [() => OrganizationFeatureConfiguration$,
    0]
];
var OrganizationFeaturesConfigurationsResults: StaticListSchema = [1, n0, _OFCRr,
  0, [() => OrganizationFeatureConfigurationResult$,
    0]
];
var OrganizationFeatureStatisticsAdditionalConfigurations: StaticListSchema = [1, n0, _OFSACr,
  0, [() => OrganizationFeatureStatisticsAdditionalConfiguration$,
    0]
];
var OrganizationFeatureStatisticsResults: StaticListSchema = [1, n0, _OFSR,
  0, [() => OrganizationFeatureStatistics$,
    0]
];
var PortProbeDetails: StaticListSchema = [1, n0, _PPD,
  0, [() => PortProbeDetail$,
    0]
];
var PrivateIpAddresses: StaticListSchema = [1, n0, _PIA,
  0, [() => PrivateIpAddressDetails$,
    0]
];
var ProductCodes: StaticListSchema = [1, n0, _PC,
  0, [() => ProductCode$,
    0]
];
var RelatedFilePathsList = 64 | 0;
var ResourceList = 64 | 0;
var Resources: StaticListSchema = [1, n0, _Reso,
  0, [() => ResourceV2$,
    0]
];
var ResourceUids = 64 | 0;
var RuleSummaryList: StaticListSchema = [1, n0, _RSL,
  0, [() => RuleSummary$,
    0]
];
var S3BucketDetails: StaticListSchema = [1, n0, _SBD,
  0, [() => S3BucketDetail$,
    0]
];
var S3ObjectDetails: StaticListSchema = [1, n0, _SOD,
  0, [() => S3ObjectDetail$,
    0]
];
var S3ObjectUids = 64 | 0;
var ScannedResources: StaticListSchema = [1, n0, _SRc,
  0, [() => ScannedResource$,
    0]
];
var ScanResultThreats: StaticListSchema = [1, n0, _SRTca,
  0, [() => ScanResultThreat$,
    0]
];
var Scans: StaticListSchema = [1, n0, _Sc,
  0, [() => Scan$,
    0]
];
var ScanThreatNames: StaticListSchema = [1, n0, _STNc,
  0, [() => ScanThreatName$,
    0]
];
var SecurityGroups: StaticListSchema = [1, n0, _SG,
  0, [() => SecurityGroup$,
    0]
];
var SessionNameList = 64 | 0;
var Signals: StaticListSchema = [1, n0, _Si,
  0, [() => Signal$,
    0]
];
var SourceIps = 64 | 0;
var Sources = 64 | 0;
var SubnetIds = 64 | 0;
var TagKeyList = 64 | 0;
var Tags: StaticListSchema = [1, n0, _Ta,
  0, [() => Tag$,
    0]
];
var ThreatEntitySetIds = 64 | 0;
var ThreatIntelligenceDetails: StaticListSchema = [1, n0, _TID,
  0, [() => ThreatIntelligenceDetail$,
    0]
];
var ThreatIntelSetIds = 64 | 0;
var ThreatNames = 64 | 0;
var Threats: StaticListSchema = [1, n0, _Th,
  0, [() => Threat$,
    0]
];
var TrustedEntitySetIds = 64 | 0;
var UnprocessedAccounts: StaticListSchema = [1, n0, _UAn,
  0, [() => UnprocessedAccount$,
    0]
];
var UsageAccountResultList: StaticListSchema = [1, n0, _UARL,
  0, [() => UsageAccountResult$,
    0]
];
var UsageDataSourceResultList: StaticListSchema = [1, n0, _UDSRL,
  0, [() => UsageDataSourceResult$,
    0]
];
var UsageFeatureList = 64 | 0;
var UsageFeatureResultList: StaticListSchema = [1, n0, _UFRL,
  0, [() => UsageFeatureResult$,
    0]
];
var UsageResourceResultList: StaticListSchema = [1, n0, _URRL,
  0, [() => UsageResourceResult$,
    0]
];
var UsageTopAccountsByFeatureList: StaticListSchema = [1, n0, _UTABFL,
  0, [() => UsageTopAccountResult$,
    0]
];
var UsageTopAccountsResultList: StaticListSchema = [1, n0, _UTARL,
  0, [() => UsageTopAccountsResult$,
    0]
];
var VolumeDetails: StaticListSchema = [1, n0, _VDo,
  0, [() => VolumeDetail$,
    0]
];
var VolumeMounts: StaticListSchema = [1, n0, _VM,
  0, [() => VolumeMount$,
    0]
];
var Volumes: StaticListSchema = [1, n0, _Vo,
  0, [() => Volume$,
    0]
];
var AffectedResources = 128 | 0;
var AnomalyProfileFeatures: StaticMapSchema = [2, n0, _APF,
  0, [0,
    0]
  , [() => AnomalyProfileFeatureObjects,
    0]
];
var AnomalyProfiles: StaticMapSchema = [2, n0, _APn,
  0, [0,
    0]
  , [() => AnomalyProfileFeatures,
    0]
];
var AnomalyUnusualBehaviorFeature: StaticMapSchema = [2, n0, _AUBF,
  0, [0,
    0]
  , [() => AnomalyObject$,
    0]
];
var Behavior: StaticMapSchema = [2, n0, _B,
  0, [0,
    0]
  , [() => AnomalyUnusualBehaviorFeature,
    0]
];
var CountByCoverageStatus = 128 | 1;
var CountByResourceType = 128 | 1;
var CountBySeverity = 128 | 1;
var Criterion: StaticMapSchema = [2, n0, _Cr,
  0, [0,
    0]
  , [() => Condition$,
    0]
];
var ScanCriterion: StaticMapSchema = [2, n0, _SCcanr,
  0, [0,
    0]
  , [() => ScanCondition$,
    0]
];
var TagMap = 128 | 0;
export var AcceptAdministratorInvitation$: StaticOperationSchema = [9, n0, _AAIc,
  { [_ht]: ["POST", "/detector/{DetectorId}/administrator", 200] }, () => AcceptAdministratorInvitationRequest$, () => AcceptAdministratorInvitationResponse$
];
export var AcceptInvitation$: StaticOperationSchema = [9, n0, _AIcce,
  { [_ht]: ["POST", "/detector/{DetectorId}/master", 200] }, () => AcceptInvitationRequest$, () => AcceptInvitationResponse$
];
export var ArchiveFindings$: StaticOperationSchema = [9, n0, _AFr,
  { [_ht]: ["POST", "/detector/{DetectorId}/findings/archive", 200] }, () => ArchiveFindingsRequest$, () => ArchiveFindingsResponse$
];
export var CreateCustomDetectionRuleAssociation$: StaticOperationSchema = [9, n0, _CCDRA,
  { [_ht]: ["POST", "/custom-detection-rule/association", 200] }, () => CreateCustomDetectionRuleAssociationRequest$, () => CreateCustomDetectionRuleAssociationResponse$
];
export var CreateCustomDetectionRuleOrgConfiguration$: StaticOperationSchema = [9, n0, _CCDROC,
  { [_ht]: ["POST", "/custom-detection-rule/org-configuration", 200] }, () => CreateCustomDetectionRuleOrgConfigurationRequest$, () => CreateCustomDetectionRuleOrgConfigurationResponse$
];
export var CreateDetector$: StaticOperationSchema = [9, n0, _CDr,
  { [_ht]: ["POST", "/detector", 200] }, () => CreateDetectorRequest$, () => CreateDetectorResponse$
];
export var CreateFilter$: StaticOperationSchema = [9, n0, _CF,
  { [_ht]: ["POST", "/detector/{DetectorId}/filter", 200] }, () => CreateFilterRequest$, () => CreateFilterResponse$
];
export var CreateInvestigation$: StaticOperationSchema = [9, n0, _CI,
  { [_ht]: ["POST", "/detector/{DetectorId}/investigation", 202] }, () => CreateInvestigationRequest$, () => CreateInvestigationResponse$
];
export var CreateIPSet$: StaticOperationSchema = [9, n0, _CIPS,
  { [_ht]: ["POST", "/detector/{DetectorId}/ipset", 200] }, () => CreateIPSetRequest$, () => CreateIPSetResponse$
];
export var CreateMalwareProtectionPlan$: StaticOperationSchema = [9, n0, _CMPP,
  { [_ht]: ["POST", "/malware-protection-plan", 200] }, () => CreateMalwareProtectionPlanRequest$, () => CreateMalwareProtectionPlanResponse$
];
export var CreateMembers$: StaticOperationSchema = [9, n0, _CM,
  { [_ht]: ["POST", "/detector/{DetectorId}/member", 200] }, () => CreateMembersRequest$, () => CreateMembersResponse$
];
export var CreatePublishingDestination$: StaticOperationSchema = [9, n0, _CPD,
  { [_ht]: ["POST", "/detector/{DetectorId}/publishingDestination", 200] }, () => CreatePublishingDestinationRequest$, () => CreatePublishingDestinationResponse$
];
export var CreateSampleFindings$: StaticOperationSchema = [9, n0, _CSF,
  { [_ht]: ["POST", "/detector/{DetectorId}/findings/create", 200] }, () => CreateSampleFindingsRequest$, () => CreateSampleFindingsResponse$
];
export var CreateThreatEntitySet$: StaticOperationSchema = [9, n0, _CTES,
  { [_ht]: ["POST", "/detector/{DetectorId}/threatentityset", 200] }, () => CreateThreatEntitySetRequest$, () => CreateThreatEntitySetResponse$
];
export var CreateThreatIntelSet$: StaticOperationSchema = [9, n0, _CTIS,
  { [_ht]: ["POST", "/detector/{DetectorId}/threatintelset", 200] }, () => CreateThreatIntelSetRequest$, () => CreateThreatIntelSetResponse$
];
export var CreateTrustedEntitySet$: StaticOperationSchema = [9, n0, _CTESr,
  { [_ht]: ["POST", "/detector/{DetectorId}/trustedentityset", 200] }, () => CreateTrustedEntitySetRequest$, () => CreateTrustedEntitySetResponse$
];
export var DeclineInvitations$: StaticOperationSchema = [9, n0, _DIec,
  { [_ht]: ["POST", "/invitation/decline", 200] }, () => DeclineInvitationsRequest$, () => DeclineInvitationsResponse$
];
export var DeleteCustomDetectionRuleAssociation$: StaticOperationSchema = [9, n0, _DCDRA,
  { [_ht]: ["DELETE", "/custom-detection-rule/rule/{RuleId}/association/{AssociationId}", 200] }, () => DeleteCustomDetectionRuleAssociationRequest$, () => DeleteCustomDetectionRuleAssociationResponse$
];
export var DeleteCustomDetectionRuleOrgConfiguration$: StaticOperationSchema = [9, n0, _DCDROC,
  { [_ht]: ["DELETE", "/custom-detection-rule/org-configuration/{RuleId}", 200] }, () => DeleteCustomDetectionRuleOrgConfigurationRequest$, () => DeleteCustomDetectionRuleOrgConfigurationResponse$
];
export var DeleteDetector$: StaticOperationSchema = [9, n0, _DDe,
  { [_ht]: ["DELETE", "/detector/{DetectorId}", 200] }, () => DeleteDetectorRequest$, () => DeleteDetectorResponse$
];
export var DeleteFilter$: StaticOperationSchema = [9, n0, _DF,
  { [_ht]: ["DELETE", "/detector/{DetectorId}/filter/{FilterName}", 200] }, () => DeleteFilterRequest$, () => DeleteFilterResponse$
];
export var DeleteInvitations$: StaticOperationSchema = [9, n0, _DIel,
  { [_ht]: ["POST", "/invitation/delete", 200] }, () => DeleteInvitationsRequest$, () => DeleteInvitationsResponse$
];
export var DeleteIPSet$: StaticOperationSchema = [9, n0, _DIPS,
  { [_ht]: ["DELETE", "/detector/{DetectorId}/ipset/{IpSetId}", 200] }, () => DeleteIPSetRequest$, () => DeleteIPSetResponse$
];
export var DeleteMalwareProtectionPlan$: StaticOperationSchema = [9, n0, _DMPP,
  { [_ht]: ["DELETE", "/malware-protection-plan/{MalwareProtectionPlanId}", 200] }, () => DeleteMalwareProtectionPlanRequest$, () => __Unit
];
export var DeleteMembers$: StaticOperationSchema = [9, n0, _DM,
  { [_ht]: ["POST", "/detector/{DetectorId}/member/delete", 200] }, () => DeleteMembersRequest$, () => DeleteMembersResponse$
];
export var DeletePublishingDestination$: StaticOperationSchema = [9, n0, _DPD,
  { [_ht]: ["DELETE", "/detector/{DetectorId}/publishingDestination/{DestinationId}", 200] }, () => DeletePublishingDestinationRequest$, () => DeletePublishingDestinationResponse$
];
export var DeleteThreatEntitySet$: StaticOperationSchema = [9, n0, _DTES,
  { [_ht]: ["DELETE", "/detector/{DetectorId}/threatentityset/{ThreatEntitySetId}", 200] }, () => DeleteThreatEntitySetRequest$, () => DeleteThreatEntitySetResponse$
];
export var DeleteThreatIntelSet$: StaticOperationSchema = [9, n0, _DTIS,
  { [_ht]: ["DELETE", "/detector/{DetectorId}/threatintelset/{ThreatIntelSetId}", 200] }, () => DeleteThreatIntelSetRequest$, () => DeleteThreatIntelSetResponse$
];
export var DeleteTrustedEntitySet$: StaticOperationSchema = [9, n0, _DTESe,
  { [_ht]: ["DELETE", "/detector/{DetectorId}/trustedentityset/{TrustedEntitySetId}", 200] }, () => DeleteTrustedEntitySetRequest$, () => DeleteTrustedEntitySetResponse$
];
export var DescribeMalwareScans$: StaticOperationSchema = [9, n0, _DMS,
  { [_ht]: ["POST", "/detector/{DetectorId}/malware-scans", 200] }, () => DescribeMalwareScansRequest$, () => DescribeMalwareScansResponse$
];
export var DescribeOrganizationConfiguration$: StaticOperationSchema = [9, n0, _DOC,
  { [_ht]: ["GET", "/detector/{DetectorId}/admin", 200] }, () => DescribeOrganizationConfigurationRequest$, () => DescribeOrganizationConfigurationResponse$
];
export var DescribePublishingDestination$: StaticOperationSchema = [9, n0, _DPDe,
  { [_ht]: ["GET", "/detector/{DetectorId}/publishingDestination/{DestinationId}", 200] }, () => DescribePublishingDestinationRequest$, () => DescribePublishingDestinationResponse$
];
export var DisableOrganizationAdminAccount$: StaticOperationSchema = [9, n0, _DOAA,
  { [_ht]: ["POST", "/admin/disable", 200] }, () => DisableOrganizationAdminAccountRequest$, () => DisableOrganizationAdminAccountResponse$
];
export var DisassociateFromAdministratorAccount$: StaticOperationSchema = [9, n0, _DFAA,
  { [_ht]: ["POST", "/detector/{DetectorId}/administrator/disassociate", 200] }, () => DisassociateFromAdministratorAccountRequest$, () => DisassociateFromAdministratorAccountResponse$
];
export var DisassociateFromMasterAccount$: StaticOperationSchema = [9, n0, _DFMA,
  { [_ht]: ["POST", "/detector/{DetectorId}/master/disassociate", 200] }, () => DisassociateFromMasterAccountRequest$, () => DisassociateFromMasterAccountResponse$
];
export var DisassociateMembers$: StaticOperationSchema = [9, n0, _DMi,
  { [_ht]: ["POST", "/detector/{DetectorId}/member/disassociate", 200] }, () => DisassociateMembersRequest$, () => DisassociateMembersResponse$
];
export var EnableOrganizationAdminAccount$: StaticOperationSchema = [9, n0, _EOAA,
  { [_ht]: ["POST", "/admin/enable", 200] }, () => EnableOrganizationAdminAccountRequest$, () => EnableOrganizationAdminAccountResponse$
];
export var GetAdministratorAccount$: StaticOperationSchema = [9, n0, _GAA,
  { [_ht]: ["GET", "/detector/{DetectorId}/administrator", 200] }, () => GetAdministratorAccountRequest$, () => GetAdministratorAccountResponse$
];
export var GetCoverageStatistics$: StaticOperationSchema = [9, n0, _GCS,
  { [_ht]: ["POST", "/detector/{DetectorId}/coverage/statistics", 200] }, () => GetCoverageStatisticsRequest$, () => GetCoverageStatisticsResponse$
];
export var GetCustomDetectionRule$: StaticOperationSchema = [9, n0, _GCDR,
  { [_ht]: ["GET", "/custom-detection-rule/rule/{RuleId}", 200] }, () => GetCustomDetectionRuleRequest$, () => GetCustomDetectionRuleResponse$
];
export var GetCustomDetectionRuleAssociation$: StaticOperationSchema = [9, n0, _GCDRA,
  { [_ht]: ["GET", "/custom-detection-rule/rule/{RuleId}/association/{AssociationId}", 200] }, () => GetCustomDetectionRuleAssociationRequest$, () => GetCustomDetectionRuleAssociationResponse$
];
export var GetCustomDetectionRuleOrgConfiguration$: StaticOperationSchema = [9, n0, _GCDROC,
  { [_ht]: ["GET", "/custom-detection-rule/org-configuration/{RuleId}", 200] }, () => GetCustomDetectionRuleOrgConfigurationRequest$, () => GetCustomDetectionRuleOrgConfigurationResponse$
];
export var GetDetector$: StaticOperationSchema = [9, n0, _GD,
  { [_ht]: ["GET", "/detector/{DetectorId}", 200] }, () => GetDetectorRequest$, () => GetDetectorResponse$
];
export var GetFilter$: StaticOperationSchema = [9, n0, _GF,
  { [_ht]: ["GET", "/detector/{DetectorId}/filter/{FilterName}", 200] }, () => GetFilterRequest$, () => GetFilterResponse$
];
export var GetFindings$: StaticOperationSchema = [9, n0, _GFe,
  { [_ht]: ["POST", "/detector/{DetectorId}/findings/get", 200] }, () => GetFindingsRequest$, () => GetFindingsResponse$
];
export var GetFindingsStatistics$: StaticOperationSchema = [9, n0, _GFS,
  { [_ht]: ["POST", "/detector/{DetectorId}/findings/statistics", 200] }, () => GetFindingsStatisticsRequest$, () => GetFindingsStatisticsResponse$
];
export var GetInvestigation$: StaticOperationSchema = [9, n0, _GIe,
  { [_ht]: ["GET", "/detector/{DetectorId}/investigation/{InvestigationId}", 200] }, () => GetInvestigationRequest$, () => GetInvestigationResponse$
];
export var GetInvitationsCount$: StaticOperationSchema = [9, n0, _GIC,
  { [_ht]: ["GET", "/invitation/count", 200] }, () => GetInvitationsCountRequest$, () => GetInvitationsCountResponse$
];
export var GetIPSet$: StaticOperationSchema = [9, n0, _GIPS,
  { [_ht]: ["GET", "/detector/{DetectorId}/ipset/{IpSetId}", 200] }, () => GetIPSetRequest$, () => GetIPSetResponse$
];
export var GetMalwareProtectionPlan$: StaticOperationSchema = [9, n0, _GMPP,
  { [_ht]: ["GET", "/malware-protection-plan/{MalwareProtectionPlanId}", 200] }, () => GetMalwareProtectionPlanRequest$, () => GetMalwareProtectionPlanResponse$
];
export var GetMalwareScan$: StaticOperationSchema = [9, n0, _GMS,
  { [_ht]: ["GET", "/malware-scan/{ScanId}", 200] }, () => GetMalwareScanRequest$, () => GetMalwareScanResponse$
];
export var GetMalwareScanSettings$: StaticOperationSchema = [9, n0, _GMSS,
  { [_ht]: ["GET", "/detector/{DetectorId}/malware-scan-settings", 200] }, () => GetMalwareScanSettingsRequest$, () => GetMalwareScanSettingsResponse$
];
export var GetMasterAccount$: StaticOperationSchema = [9, n0, _GMA,
  { [_ht]: ["GET", "/detector/{DetectorId}/master", 200] }, () => GetMasterAccountRequest$, () => GetMasterAccountResponse$
];
export var GetMemberDetectors$: StaticOperationSchema = [9, n0, _GMD,
  { [_ht]: ["POST", "/detector/{DetectorId}/member/detector/get", 200] }, () => GetMemberDetectorsRequest$, () => GetMemberDetectorsResponse$
];
export var GetMembers$: StaticOperationSchema = [9, n0, _GM,
  { [_ht]: ["POST", "/detector/{DetectorId}/member/get", 200] }, () => GetMembersRequest$, () => GetMembersResponse$
];
export var GetOrganizationStatistics$: StaticOperationSchema = [9, n0, _GOS,
  { [_ht]: ["GET", "/organization/statistics", 200] }, () => __Unit, () => GetOrganizationStatisticsResponse$
];
export var GetRemainingFreeTrialDays$: StaticOperationSchema = [9, n0, _GRFTD,
  { [_ht]: ["POST", "/detector/{DetectorId}/freeTrial/daysRemaining", 200] }, () => GetRemainingFreeTrialDaysRequest$, () => GetRemainingFreeTrialDaysResponse$
];
export var GetThreatEntitySet$: StaticOperationSchema = [9, n0, _GTES,
  { [_ht]: ["GET", "/detector/{DetectorId}/threatentityset/{ThreatEntitySetId}", 200] }, () => GetThreatEntitySetRequest$, () => GetThreatEntitySetResponse$
];
export var GetThreatIntelSet$: StaticOperationSchema = [9, n0, _GTIS,
  { [_ht]: ["GET", "/detector/{DetectorId}/threatintelset/{ThreatIntelSetId}", 200] }, () => GetThreatIntelSetRequest$, () => GetThreatIntelSetResponse$
];
export var GetTrustedEntitySet$: StaticOperationSchema = [9, n0, _GTESe,
  { [_ht]: ["GET", "/detector/{DetectorId}/trustedentityset/{TrustedEntitySetId}", 200] }, () => GetTrustedEntitySetRequest$, () => GetTrustedEntitySetResponse$
];
export var GetUsageStatistics$: StaticOperationSchema = [9, n0, _GUS,
  { [_ht]: ["POST", "/detector/{DetectorId}/usage/statistics", 200] }, () => GetUsageStatisticsRequest$, () => GetUsageStatisticsResponse$
];
export var InviteMembers$: StaticOperationSchema = [9, n0, _IMn,
  { [_ht]: ["POST", "/detector/{DetectorId}/member/invite", 200] }, () => InviteMembersRequest$, () => InviteMembersResponse$
];
export var ListCoverage$: StaticOperationSchema = [9, n0, _LC,
  { [_ht]: ["POST", "/detector/{DetectorId}/coverage", 200] }, () => ListCoverageRequest$, () => ListCoverageResponse$
];
export var ListCustomDetectionRuleAssociations$: StaticOperationSchema = [9, n0, _LCDRA,
  { [_ht]: ["GET", "/custom-detection-rule/association", 200] }, () => ListCustomDetectionRuleAssociationsRequest$, () => ListCustomDetectionRuleAssociationsResponse$
];
export var ListCustomDetectionRuleOrgConfigurations$: StaticOperationSchema = [9, n0, _LCDROC,
  { [_ht]: ["GET", "/custom-detection-rule/org-configuration", 200] }, () => ListCustomDetectionRuleOrgConfigurationsRequest$, () => ListCustomDetectionRuleOrgConfigurationsResponse$
];
export var ListCustomDetectionRules$: StaticOperationSchema = [9, n0, _LCDR,
  { [_ht]: ["POST", "/custom-detection-rule/rule", 200] }, () => ListCustomDetectionRulesRequest$, () => ListCustomDetectionRulesResponse$
];
export var ListDetectors$: StaticOperationSchema = [9, n0, _LDi,
  { [_ht]: ["GET", "/detector", 200] }, () => ListDetectorsRequest$, () => ListDetectorsResponse$
];
export var ListFilters$: StaticOperationSchema = [9, n0, _LF,
  { [_ht]: ["GET", "/detector/{DetectorId}/filter", 200] }, () => ListFiltersRequest$, () => ListFiltersResponse$
];
export var ListFindings$: StaticOperationSchema = [9, n0, _LFi,
  { [_ht]: ["POST", "/detector/{DetectorId}/findings", 200] }, () => ListFindingsRequest$, () => ListFindingsResponse$
];
export var ListInvestigations$: StaticOperationSchema = [9, n0, _LI,
  { [_ht]: ["POST", "/detector/{DetectorId}/investigation/list", 200] }, () => ListInvestigationsRequest$, () => ListInvestigationsResponse$
];
export var ListInvitations$: StaticOperationSchema = [9, n0, _LIi,
  { [_ht]: ["GET", "/invitation", 200] }, () => ListInvitationsRequest$, () => ListInvitationsResponse$
];
export var ListIPSets$: StaticOperationSchema = [9, n0, _LIPS,
  { [_ht]: ["GET", "/detector/{DetectorId}/ipset", 200] }, () => ListIPSetsRequest$, () => ListIPSetsResponse$
];
export var ListMalwareProtectionPlans$: StaticOperationSchema = [9, n0, _LMPP,
  { [_ht]: ["GET", "/malware-protection-plan", 200] }, () => ListMalwareProtectionPlansRequest$, () => ListMalwareProtectionPlansResponse$
];
export var ListMalwareScans$: StaticOperationSchema = [9, n0, _LMS,
  { [_ht]: ["POST", "/malware-scan", 200] }, () => ListMalwareScansRequest$, () => ListMalwareScansResponse$
];
export var ListMembers$: StaticOperationSchema = [9, n0, _LM,
  { [_ht]: ["GET", "/detector/{DetectorId}/member", 200] }, () => ListMembersRequest$, () => ListMembersResponse$
];
export var ListOrganizationAdminAccounts$: StaticOperationSchema = [9, n0, _LOAA,
  { [_ht]: ["GET", "/admin", 200] }, () => ListOrganizationAdminAccountsRequest$, () => ListOrganizationAdminAccountsResponse$
];
export var ListPublishingDestinations$: StaticOperationSchema = [9, n0, _LPDi,
  { [_ht]: ["GET", "/detector/{DetectorId}/publishingDestination", 200] }, () => ListPublishingDestinationsRequest$, () => ListPublishingDestinationsResponse$
];
export var ListTagsForResource$: StaticOperationSchema = [9, n0, _LTFR,
  { [_ht]: ["GET", "/tags/{ResourceArn}", 200] }, () => ListTagsForResourceRequest$, () => ListTagsForResourceResponse$
];
export var ListThreatEntitySets$: StaticOperationSchema = [9, n0, _LTES,
  { [_ht]: ["GET", "/detector/{DetectorId}/threatentityset", 200] }, () => ListThreatEntitySetsRequest$, () => ListThreatEntitySetsResponse$
];
export var ListThreatIntelSets$: StaticOperationSchema = [9, n0, _LTIS,
  { [_ht]: ["GET", "/detector/{DetectorId}/threatintelset", 200] }, () => ListThreatIntelSetsRequest$, () => ListThreatIntelSetsResponse$
];
export var ListTrustedEntitySets$: StaticOperationSchema = [9, n0, _LTESi,
  { [_ht]: ["GET", "/detector/{DetectorId}/trustedentityset", 200] }, () => ListTrustedEntitySetsRequest$, () => ListTrustedEntitySetsResponse$
];
export var SendObjectMalwareScan$: StaticOperationSchema = [9, n0, _SOMS,
  { [_ht]: ["POST", "/object-malware-scan/send", 200] }, () => SendObjectMalwareScanRequest$, () => SendObjectMalwareScanResponse$
];
export var StartMalwareScan$: StaticOperationSchema = [9, n0, _SMS,
  { [_ht]: ["POST", "/malware-scan/start", 200] }, () => StartMalwareScanRequest$, () => StartMalwareScanResponse$
];
export var StartMonitoringMembers$: StaticOperationSchema = [9, n0, _SMM,
  { [_ht]: ["POST", "/detector/{DetectorId}/member/start", 200] }, () => StartMonitoringMembersRequest$, () => StartMonitoringMembersResponse$
];
export var StopMonitoringMembers$: StaticOperationSchema = [9, n0, _SMMt,
  { [_ht]: ["POST", "/detector/{DetectorId}/member/stop", 200] }, () => StopMonitoringMembersRequest$, () => StopMonitoringMembersResponse$
];
export var TagResource$: StaticOperationSchema = [9, n0, _TRa,
  { [_ht]: ["POST", "/tags/{ResourceArn}", 204] }, () => TagResourceRequest$, () => TagResourceResponse$
];
export var UnarchiveFindings$: StaticOperationSchema = [9, n0, _UFn,
  { [_ht]: ["POST", "/detector/{DetectorId}/findings/unarchive", 200] }, () => UnarchiveFindingsRequest$, () => UnarchiveFindingsResponse$
];
export var UntagResource$: StaticOperationSchema = [9, n0, _UR,
  { [_ht]: ["DELETE", "/tags/{ResourceArn}", 204] }, () => UntagResourceRequest$, () => UntagResourceResponse$
];
export var UpdateCustomDetectionRuleAssociation$: StaticOperationSchema = [9, n0, _UCDRA,
  { [_ht]: ["PUT", "/custom-detection-rule/rule/{RuleId}/association/{AssociationId}", 200] }, () => UpdateCustomDetectionRuleAssociationRequest$, () => UpdateCustomDetectionRuleAssociationResponse$
];
export var UpdateCustomDetectionRuleOrgConfiguration$: StaticOperationSchema = [9, n0, _UCDROC,
  { [_ht]: ["PUT", "/custom-detection-rule/org-configuration/{RuleId}", 200] }, () => UpdateCustomDetectionRuleOrgConfigurationRequest$, () => UpdateCustomDetectionRuleOrgConfigurationResponse$
];
export var UpdateDetector$: StaticOperationSchema = [9, n0, _UD,
  { [_ht]: ["POST", "/detector/{DetectorId}", 200] }, () => UpdateDetectorRequest$, () => UpdateDetectorResponse$
];
export var UpdateFilter$: StaticOperationSchema = [9, n0, _UFp,
  { [_ht]: ["POST", "/detector/{DetectorId}/filter/{FilterName}", 200] }, () => UpdateFilterRequest$, () => UpdateFilterResponse$
];
export var UpdateFindingsFeedback$: StaticOperationSchema = [9, n0, _UFF,
  { [_ht]: ["POST", "/detector/{DetectorId}/findings/feedback", 200] }, () => UpdateFindingsFeedbackRequest$, () => UpdateFindingsFeedbackResponse$
];
export var UpdateIPSet$: StaticOperationSchema = [9, n0, _UIPS,
  { [_ht]: ["POST", "/detector/{DetectorId}/ipset/{IpSetId}", 200] }, () => UpdateIPSetRequest$, () => UpdateIPSetResponse$
];
export var UpdateMalwareProtectionPlan$: StaticOperationSchema = [9, n0, _UMPP,
  { [_ht]: ["PATCH", "/malware-protection-plan/{MalwareProtectionPlanId}", 200] }, () => UpdateMalwareProtectionPlanRequest$, () => __Unit
];
export var UpdateMalwareScanSettings$: StaticOperationSchema = [9, n0, _UMSS,
  { [_ht]: ["POST", "/detector/{DetectorId}/malware-scan-settings", 200] }, () => UpdateMalwareScanSettingsRequest$, () => UpdateMalwareScanSettingsResponse$
];
export var UpdateMemberDetectors$: StaticOperationSchema = [9, n0, _UMD,
  { [_ht]: ["POST", "/detector/{DetectorId}/member/detector/update", 200] }, () => UpdateMemberDetectorsRequest$, () => UpdateMemberDetectorsResponse$
];
export var UpdateOrganizationConfiguration$: StaticOperationSchema = [9, n0, _UOC,
  { [_ht]: ["POST", "/detector/{DetectorId}/admin", 200] }, () => UpdateOrganizationConfigurationRequest$, () => UpdateOrganizationConfigurationResponse$
];
export var UpdatePublishingDestination$: StaticOperationSchema = [9, n0, _UPD,
  { [_ht]: ["POST", "/detector/{DetectorId}/publishingDestination/{DestinationId}", 200] }, () => UpdatePublishingDestinationRequest$, () => UpdatePublishingDestinationResponse$
];
export var UpdateThreatEntitySet$: StaticOperationSchema = [9, n0, _UTES,
  { [_ht]: ["POST", "/detector/{DetectorId}/threatentityset/{ThreatEntitySetId}", 200] }, () => UpdateThreatEntitySetRequest$, () => UpdateThreatEntitySetResponse$
];
export var UpdateThreatIntelSet$: StaticOperationSchema = [9, n0, _UTIS,
  { [_ht]: ["POST", "/detector/{DetectorId}/threatintelset/{ThreatIntelSetId}", 200] }, () => UpdateThreatIntelSetRequest$, () => UpdateThreatIntelSetResponse$
];
export var UpdateTrustedEntitySet$: StaticOperationSchema = [9, n0, _UTESp,
  { [_ht]: ["POST", "/detector/{DetectorId}/trustedentityset/{TrustedEntitySetId}", 200] }, () => UpdateTrustedEntitySetRequest$, () => UpdateTrustedEntitySetResponse$
];
