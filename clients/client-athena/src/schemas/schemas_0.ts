const _AC = "AclConfiguration";
const _ACd = "AdditionalConfigs";
const _ACdd = "AdditionalConfiguration";
const _AD = "AllocatedDpus";
const _ADPUS = "ApplicationDPUSizes";
const _ADPUSL = "ApplicationDPUSizesList";
const _AE = "AthenaError";
const _AEC = "AthenaErrorCode";
const _ARI = "ApplicationRuntimeId";
const _AT = "AuthToken";
const _ATET = "AuthTokenExpirationTime";
const _ATu = "AuthenticationType";
const _BGNQ = "BatchGetNamedQuery";
const _BGNQI = "BatchGetNamedQueryInput";
const _BGNQO = "BatchGetNamedQueryOutput";
const _BGPS = "BatchGetPreparedStatement";
const _BGPSI = "BatchGetPreparedStatementInput";
const _BGPSO = "BatchGetPreparedStatementOutput";
const _BGQE = "BatchGetQueryExecution";
const _BGQEI = "BatchGetQueryExecutionInput";
const _BGQEO = "BatchGetQueryExecutionOutput";
const _BSCPQ = "BytesScannedCutoffPerQuery";
const _C = "Classification";
const _CA = "CapacityAllocation";
const _CAC = "CapacityAssignmentConfiguration";
const _CAL = "CapacityAssignmentsList";
const _CAa = "CapacityAssignment";
const _CAap = "CapacityAssignments";
const _CB = "CodeBlock";
const _CC = "CalculationConfiguration";
const _CCEC = "CustomerContentEncryptionConfiguration";
const _CCR = "CancelCapacityReservation";
const _CCRI = "CancelCapacityReservationInput";
const _CCRIr = "CreateCapacityReservationInput";
const _CCRO = "CancelCapacityReservationOutput";
const _CCROr = "CreateCapacityReservationOutput";
const _CCRr = "CreateCapacityReservation";
const _CDC = "CreateDataCatalog";
const _CDCI = "CreateDataCatalogInput";
const _CDCO = "CreateDataCatalogOutput";
const _CDS = "CoordinatorDpuSize";
const _CDT = "CompletionDateTime";
const _CEI = "CalculationExecutionId";
const _CI = "ColumnInfo";
const _CIL = "ColumnInfoList";
const _CL = "CalculationsList";
const _CLl = "ClassificationList";
const _CLo = "ColumnList";
const _CN = "CatalogName";
const _CNI = "CreateNotebookInput";
const _CNO = "CreateNotebookOutput";
const _CNQ = "CreateNamedQuery";
const _CNQI = "CreateNamedQueryInput";
const _CNQO = "CreateNamedQueryOutput";
const _CNr = "CreateNotebook";
const _CPNU = "CreatePresignedNotebookUrl";
const _CPNUR = "CreatePresignedNotebookUrlRequest";
const _CPNURr = "CreatePresignedNotebookUrlResponse";
const _CPS = "CreatePreparedStatement";
const _CPSI = "CreatePreparedStatementInput";
const _CPSO = "CreatePreparedStatementOutput";
const _CR = "CalculationResult";
const _CRL = "CapacityReservationsList";
const _CRN = "CapacityReservationName";
const _CRT = "ClientRequestToken";
const _CRa = "CapacityReservation";
const _CRap = "CapacityReservations";
const _CS = "CalculationStatistics";
const _CSa = "CalculationStatus";
const _CSal = "CalculationSummary";
const _CSas = "CaseSensitive";
const _CT = "CreationTime";
const _CTo = "ConnectionType";
const _CTr = "CreateTime";
const _CU = "ConfigurationUpdates";
const _CULP = "CreateUserLevelPrefix";
const _CWG = "CreateWorkGroup";
const _CWGI = "CreateWorkGroupInput";
const _CWGO = "CreateWorkGroupOutput";
const _CWGT = "CopyWorkGroupTags";
const _CWLC = "CloudWatchLoggingConfiguration";
const _Ca = "Calculations";
const _Cat = "Catalog";
const _Ch = "Children";
const _Cl = "Classifications";
const _Co = "Column";
const _Col = "Columns";
const _Com = "Comment";
const _Con = "Configuration";
const _D = "Description";
const _DC = "DataCatalog";
const _DCO = "DeleteCatalogOnly";
const _DCR = "DeleteCapacityReservation";
const _DCRI = "DeleteCapacityReservationInput";
const _DCRO = "DeleteCapacityReservationOutput";
const _DCS = "DataCatalogSummary";
const _DCSL = "DataCatalogSummaryList";
const _DCSa = "DataCatalogsSummary";
const _DCp = "DpuCount";
const _DDC = "DeleteDataCatalog";
const _DDCI = "DeleteDataCatalogInput";
const _DDCO = "DeleteDataCatalogOutput";
const _DEDS = "DefaultExecutorDpuSize";
const _DEIM = "DpuExecutionInMillis";
const _DL = "DatabaseList";
const _DML = "DataManifestLocation";
const _DN = "DatabaseName";
const _DNI = "DeleteNotebookInput";
const _DNO = "DeleteNotebookOutput";
const _DNQ = "DeleteNamedQuery";
const _DNQI = "DeleteNamedQueryInput";
const _DNQO = "DeleteNamedQueryOutput";
const _DNe = "DeleteNotebook";
const _DPS = "DeletePreparedStatement";
const _DPSI = "DeletePreparedStatementInput";
const _DPSO = "DeletePreparedStatementOutput";
const _DSIB = "DataScannedInBytes";
const _DWG = "DeleteWorkGroup";
const _DWGI = "DeleteWorkGroupInput";
const _DWGO = "DeleteWorkGroupOutput";
const _Da = "Database";
const _Dat = "Datum";
const _Data = "Data";
const _E = "Enabled";
const _EBO = "ExpectedBucketOwner";
const _EC = "ErrorCategory";
const _ECn = "EncryptionConfiguration";
const _ECng = "EngineConfiguration";
const _ECr = "ErrorCode";
const _EDT = "EndDateTime";
const _EETIM = "EngineExecutionTimeInMillis";
const _EEV = "EffectiveEngineVersion";
const _EI = "ExecutorId";
const _EIC = "EnableIdentityCenter";
const _EM = "ErrorMessage";
const _EMEC = "EnableMinimumEncryptionConfiguration";
const _EN = "ExportNotebook";
const _ENI = "ExportNotebookInput";
const _ENO = "ExportNotebookOutput";
const _EO = "EncryptionOption";
const _EP = "ExecutionParameters";
const _ER = "ExecutionRole";
const _ES = "ExecutorsSummary";
const _ESAG = "EnableS3AccessGrants";
const _ESF = "ExecutorStateFilter";
const _ESL = "ExecutorsSummaryList";
const _ESx = "ExecutorState";
const _ESxe = "ExecutorSize";
const _ET = "ErrorType";
const _ETx = "ExecutorType";
const _ETxe = "ExecutionTime";
const _EU = "EndpointUrl";
const _EV = "EngineVersion";
const _EVL = "EngineVersionsList";
const _EVn = "EngineVersions";
const _EWGC = "EnforceWorkGroupConfiguration";
const _Er = "Error";
const _Ex = "Expression";
const _F = "Filters";
const _FD = "FilterDefinition";
const _GCAC = "GetCapacityAssignmentConfiguration";
const _GCACI = "GetCapacityAssignmentConfigurationInput";
const _GCACO = "GetCapacityAssignmentConfigurationOutput";
const _GCE = "GetCalculationExecution";
const _GCEC = "GetCalculationExecutionCode";
const _GCECR = "GetCalculationExecutionCodeRequest";
const _GCECRe = "GetCalculationExecutionCodeResponse";
const _GCER = "GetCalculationExecutionRequest";
const _GCERe = "GetCalculationExecutionResponse";
const _GCES = "GetCalculationExecutionStatus";
const _GCESR = "GetCalculationExecutionStatusRequest";
const _GCESRe = "GetCalculationExecutionStatusResponse";
const _GCR = "GetCapacityReservation";
const _GCRI = "GetCapacityReservationInput";
const _GCRO = "GetCapacityReservationOutput";
const _GD = "GetDatabase";
const _GDC = "GetDataCatalog";
const _GDCI = "GetDataCatalogInput";
const _GDCO = "GetDataCatalogOutput";
const _GDI = "GetDatabaseInput";
const _GDO = "GetDatabaseOutput";
const _GNM = "GetNotebookMetadata";
const _GNMI = "GetNotebookMetadataInput";
const _GNMO = "GetNotebookMetadataOutput";
const _GNQ = "GetNamedQuery";
const _GNQI = "GetNamedQueryInput";
const _GNQO = "GetNamedQueryOutput";
const _GPS = "GetPreparedStatement";
const _GPSI = "GetPreparedStatementInput";
const _GPSO = "GetPreparedStatementOutput";
const _GQE = "GetQueryExecution";
const _GQEI = "GetQueryExecutionInput";
const _GQEO = "GetQueryExecutionOutput";
const _GQR = "GetQueryResults";
const _GQRI = "GetQueryResultsInput";
const _GQRO = "GetQueryResultsOutput";
const _GQRS = "GetQueryRuntimeStatistics";
const _GQRSI = "GetQueryRuntimeStatisticsInput";
const _GQRSO = "GetQueryRuntimeStatisticsOutput";
const _GRD = "GetResourceDashboard";
const _GRDR = "GetResourceDashboardRequest";
const _GRDRe = "GetResourceDashboardResponse";
const _GS = "GetSession";
const _GSE = "GetSessionEndpoint";
const _GSER = "GetSessionEndpointRequest";
const _GSERe = "GetSessionEndpointResponse";
const _GSR = "GetSessionRequest";
const _GSRe = "GetSessionResponse";
const _GSS = "GetSessionStatus";
const _GSSR = "GetSessionStatusRequest";
const _GSSRe = "GetSessionStatusResponse";
const _GTM = "GetTableMetadata";
const _GTMI = "GetTableMetadataInput";
const _GTMO = "GetTableMetadataOutput";
const _GWG = "GetWorkGroup";
const _GWGI = "GetWorkGroupInput";
const _GWGO = "GetWorkGroupOutput";
const _I = "Identifier";
const _IB = "InputBytes";
const _ICAA = "IdentityCenterApplicationArn";
const _ICC = "IdentityCenterConfiguration";
const _ICIA = "IdentityCenterInstanceArn";
const _IN = "ImportNotebook";
const _INI = "ImportNotebookInput";
const _INO = "ImportNotebookOutput";
const _IR = "InputRows";
const _IRE = "InvalidRequestException";
const _ISDT = "IdleSinceDateTime";
const _ISE = "InternalServerException";
const _ITS = "IdleTimeoutSeconds";
const _K = "Key";
const _KK = "KmsKey";
const _L = "Label";
const _LA = "LastAllocation";
const _LADPUS = "ListApplicationDPUSizes";
const _LADPUSI = "ListApplicationDPUSizesInput";
const _LADPUSO = "ListApplicationDPUSizesOutput";
const _LAT = "LastAccessTime";
const _LCE = "ListCalculationExecutions";
const _LCER = "ListCalculationExecutionsRequest";
const _LCERi = "ListCalculationExecutionsResponse";
const _LCR = "ListCapacityReservations";
const _LCRI = "ListCapacityReservationsInput";
const _LCRO = "ListCapacityReservationsOutput";
const _LD = "ListDatabases";
const _LDC = "ListDataCatalogs";
const _LDCI = "ListDataCatalogsInput";
const _LDCO = "ListDataCatalogsOutput";
const _LDI = "ListDatabasesInput";
const _LDO = "ListDatabasesOutput";
const _LE = "ListExecutors";
const _LER = "ListExecutorsRequest";
const _LERi = "ListExecutorsResponse";
const _LEV = "ListEngineVersions";
const _LEVI = "ListEngineVersionsInput";
const _LEVO = "ListEngineVersionsOutput";
const _LG = "LogGroup";
const _LL = "LogLocation";
const _LMDT = "LastModifiedDateTime";
const _LMT = "LastModifiedTime";
const _LNM = "ListNotebookMetadata";
const _LNMI = "ListNotebookMetadataInput";
const _LNMO = "ListNotebookMetadataOutput";
const _LNQ = "ListNamedQueries";
const _LNQI = "ListNamedQueriesInput";
const _LNQO = "ListNamedQueriesOutput";
const _LNS = "ListNotebookSessions";
const _LNSR = "ListNotebookSessionsRequest";
const _LNSRi = "ListNotebookSessionsResponse";
const _LPS = "ListPreparedStatements";
const _LPSI = "ListPreparedStatementsInput";
const _LPSO = "ListPreparedStatementsOutput";
const _LQE = "ListQueryExecutions";
const _LQEI = "ListQueryExecutionsInput";
const _LQEO = "ListQueryExecutionsOutput";
const _LS = "ListSessions";
const _LSAT = "LastSuccessfulAllocationTime";
const _LSNP = "LogStreamNamePrefix";
const _LSR = "ListSessionsRequest";
const _LSRi = "ListSessionsResponse";
const _LT = "LogTypes";
const _LTFR = "ListTagsForResource";
const _LTFRI = "ListTagsForResourceInput";
const _LTFRO = "ListTagsForResourceOutput";
const _LTM = "LogTypesMap";
const _LTMI = "ListTableMetadataInput";
const _LTMO = "ListTableMetadataOutput";
const _LTMi = "ListTableMetadata";
const _LWG = "ListWorkGroups";
const _LWGI = "ListWorkGroupsInput";
const _LWGO = "ListWorkGroupsOutput";
const _M = "Message";
const _MAIM = "MaxAgeInMinutes";
const _MC = "MonitoringConfiguration";
const _MCD = "MaxConcurrentDpus";
const _ME = "MetadataException";
const _MLC = "ManagedLoggingConfiguration";
const _MQRC = "ManagedQueryResultsConfiguration";
const _MQRCU = "ManagedQueryResultsConfigurationUpdates";
const _MQREC = "ManagedQueryResultsEncryptionConfiguration";
const _MR = "MaxResults";
const _N = "Name";
const _NI = "NotebookId";
const _NM = "NotebookMetadata";
const _NMA = "NotebookMetadataArray";
const _NML = "NotebookMetadataList";
const _NQ = "NamedQueries";
const _NQI = "NamedQueryIds";
const _NQIa = "NamedQueryId";
const _NQL = "NamedQueryList";
const _NQa = "NamedQuery";
const _NSL = "NotebookSessionsList";
const _NSLU = "NotebookS3LocationUri";
const _NSS = "NotebookSessionSummary";
const _NT = "NextToken";
const _NU = "NotebookUrl";
const _NV = "NotebookVersion";
const _Nu = "Nullable";
const _OB = "OutputBytes";
const _OL = "OutputLocation";
const _OR = "OutputRows";
const _OS = "OutputStage";
const _P = "Progress";
const _PCAC = "PutCapacityAssignmentConfiguration";
const _PCACI = "PutCapacityAssignmentConfigurationInput";
const _PCACO = "PutCapacityAssignmentConfigurationOutput";
const _PCWME = "PublishCloudWatchMetricsEnabled";
const _PK = "PartitionKeys";
const _PS = "PreparedStatements";
const _PSDL = "PreparedStatementDetailsList";
const _PSL = "PreparedStatementsList";
const _PSN = "PreparedStatementNames";
const _PSS = "PreparedStatementSummary";
const _PSr = "PreparedStatement";
const _Pa = "Parameters";
const _Pay = "Payload";
const _Pr = "Properties";
const _Pre = "Precision";
const _Q = "Query";
const _QE = "QueryExecutions";
const _QEC = "QueryExecutionContext";
const _QEI = "QueryExecutionIds";
const _QEIu = "QueryExecutionId";
const _QEL = "QueryExecutionList";
const _QES = "QueryExecutionStatistics";
const _QESu = "QueryExecutionStatus";
const _QEu = "QueryExecution";
const _QPTIM = "QueryPlanningTimeInMillis";
const _QQTIM = "QueryQueueTimeInMillis";
const _QRS = "QueryRuntimeStatistics";
const _QRSAGC = "QueryResultsS3AccessGrantsConfiguration";
const _QRSR = "QueryRuntimeStatisticsRows";
const _QRST = "QueryRuntimeStatisticsTimeline";
const _QRT = "QueryResultType";
const _QS = "QueryString";
const _QSP = "QueryStagePlan";
const _QSPN = "QueryStagePlanNode";
const _QSPNu = "QueryStagePlanNodes";
const _QSu = "QueryStatement";
const _QSue = "QueryStage";
const _QSuer = "QueryStages";
const _R = "Retryable";
const _RAC = "RemoveAclConfiguration";
const _RARN = "ResourceARN";
const _RBSCPQ = "RemoveBytesScannedCutoffPerQuery";
const _RC = "ResultConfiguration";
const _RCCEC = "RemoveCustomerContentEncryptionConfiguration";
const _RCT = "RequestCompletionTime";
const _RCU = "ResultConfigurationUpdates";
const _RDO = "RecursiveDeleteOption";
const _REBO = "RemoveExpectedBucketOwner";
const _REC = "RemoveEncryptionConfiguration";
const _RL = "RowList";
const _RN = "ResourceName";
const _RNFE = "ResourceNotFoundException";
const _ROL = "RemoveOutputLocation";
const _RPE = "RequesterPaysEnabled";
const _RPR = "ReusedPreviousResult";
const _RRBAC = "ResultReuseByAgeConfiguration";
const _RRC = "ResultReuseConfiguration";
const _RRI = "ResultReuseInformation";
const _RS = "ResultSet";
const _RSM = "ResultSetMetadata";
const _RSU = "ResultS3Uri";
const _RSe = "RemoteSources";
const _RT = "ResultType";
const _RTe = "RequestTime";
const _Re = "Result";
const _Rea = "Reason";
const _Ro = "Rows";
const _Row = "Row";
const _S = "State";
const _SAEE = "SessionAlreadyExistsException";
const _SAO = "S3AclOption";
const _SC = "SessionConfiguration";
const _SCE = "StartCalculationExecution";
const _SCER = "StartCalculationExecutionRequest";
const _SCERt = "StartCalculationExecutionResponse";
const _SCERto = "StopCalculationExecutionRequest";
const _SCERtop = "StopCalculationExecutionResponse";
const _SCEt = "StopCalculationExecution";
const _SCR = "StateChangeReason";
const _SDPUS = "SupportedDPUSizes";
const _SDT = "SubmissionDateTime";
const _SDTt = "StartDateTime";
const _SESU = "StdErrorS3Uri";
const _SEV = "SelectedEngineVersion";
const _SF = "StateFilter";
const _SI = "SessionId";
const _SITIM = "SessionIdleTimeoutInMinutes";
const _SIt = "StageId";
const _SL = "SessionsList";
const _SLC = "S3LoggingConfiguration";
const _SM = "StatusMessage";
const _SN = "SchemaName";
const _SNt = "StatementName";
const _SOSU = "StdOutS3Uri";
const _SP = "SparkProperties";
const _SPPTIM = "ServicePreProcessingTimeInMillis";
const _SPTIM = "ServiceProcessingTimeInMillis";
const _SQE = "StartQueryExecution";
const _SQEI = "StartQueryExecutionInput";
const _SQEIt = "StopQueryExecutionInput";
const _SQEO = "StartQueryExecutionOutput";
const _SQEOt = "StopQueryExecutionOutput";
const _SQEt = "StopQueryExecution";
const _SS = "SubStages";
const _SSR = "StartSessionRequest";
const _SSRt = "StartSessionResponse";
const _SSe = "SessionStatistics";
const _SSes = "SessionStatus";
const _SSess = "SessionSummary";
const _SSt = "StartSession";
const _ST = "StatementType";
const _STu = "SubstatementType";
const _Sc = "Scale";
const _Se = "Sessions";
const _St = "Status";
const _Sta = "Statistics";
const _T = "Type";
const _TD = "TargetDpus";
const _TDT = "TerminationDateTime";
const _TETIM = "TotalExecutionTimeInMillis";
const _TK = "TagKeys";
const _TL = "TagList";
const _TM = "TableMetadata";
const _TML = "TableMetadataList";
const _TMRE = "TooManyRequestsException";
const _TN = "TableName";
const _TR = "TagResource";
const _TRI = "TagResourceInput";
const _TRO = "TagResourceOutput";
const _TS = "TerminateSession";
const _TSR = "TerminateSessionRequest";
const _TSRe = "TerminateSessionResponse";
const _TT = "TableType";
const _Ta = "Tags";
const _Tag = "Tag";
const _Ti = "Timeline";
const _U = "Url";
const _UC = "UpdateCount";
const _UCR = "UpdateCapacityReservation";
const _UCRI = "UpdateCapacityReservationInput";
const _UCRO = "UpdateCapacityReservationOutput";
const _UDC = "UpdateDataCatalog";
const _UDCI = "UpdateDataCatalogInput";
const _UDCO = "UpdateDataCatalogOutput";
const _UN = "UpdateNotebook";
const _UNI = "UpdateNotebookInput";
const _UNM = "UpdateNotebookMetadata";
const _UNMI = "UpdateNotebookMetadataInput";
const _UNMO = "UpdateNotebookMetadataOutput";
const _UNO = "UpdateNotebookOutput";
const _UNQ = "UpdateNamedQuery";
const _UNQI = "UnprocessedNamedQueryIds";
const _UNQIL = "UnprocessedNamedQueryIdList";
const _UNQIn = "UnprocessedNamedQueryId";
const _UNQIp = "UpdateNamedQueryInput";
const _UNQO = "UpdateNamedQueryOutput";
const _UPS = "UpdatePreparedStatement";
const _UPSI = "UpdatePreparedStatementInput";
const _UPSN = "UnprocessedPreparedStatementNames";
const _UPSNL = "UnprocessedPreparedStatementNameList";
const _UPSNn = "UnprocessedPreparedStatementName";
const _UPSO = "UpdatePreparedStatementOutput";
const _UQEI = "UnprocessedQueryExecutionIds";
const _UQEIL = "UnprocessedQueryExecutionIdList";
const _UQEIn = "UnprocessedQueryExecutionId";
const _UR = "UntagResource";
const _URI = "UntagResourceInput";
const _URO = "UntagResourceOutput";
const _UWG = "UpdateWorkGroup";
const _UWGI = "UpdateWorkGroupInput";
const _UWGO = "UpdateWorkGroupOutput";
const _V = "Value";
const _VCV = "VarCharValue";
const _WD = "WorkingDirectory";
const _WG = "WorkGroup";
const _WGC = "WorkGroupConfiguration";
const _WGCU = "WorkGroupConfigurationUpdates";
const _WGL = "WorkGroupsList";
const _WGN = "WorkGroupNames";
const _WGNo = "WorkGroupName";
const _WGS = "WorkGroupSummary";
const _WGo = "WorkGroups";
const _c = "client";
const _dL = "datumList";
const _e = "error";
const _s = "server";
const _sm = "smithy.ts.sdk.synthetic.com.amazonaws.athena";
const n0 = "com.amazonaws.athena";

// smithy-typescript generated code
import { TypeRegistry } from "@smithy/core/schema";
import type {
  StaticErrorSchema,
  StaticListSchema,
  StaticMapSchema,
  StaticOperationSchema,
  StaticStructureSchema,
} from "@smithy/types";

import { AthenaServiceException } from "../models/AthenaServiceException";
import {
  InternalServerException,
  InvalidRequestException,
  MetadataException,
  ResourceNotFoundException,
  SessionAlreadyExistsException,
  TooManyRequestsException,
} from "../models/errors";

/* eslint no-var: 0 */
export var AclConfiguration$: StaticStructureSchema = [3, n0, _AC,
  0,
  [_SAO],
  [0], 1
];
export var ApplicationDPUSizes$: StaticStructureSchema = [3, n0, _ADPUS,
  0,
  [_ARI, _SDPUS],
  [0, 64 | 1]
];
export var AthenaError$: StaticStructureSchema = [3, n0, _AE,
  0,
  [_EC, _ET, _R, _EM],
  [1, 1, 2, 0]
];
export var BatchGetNamedQueryInput$: StaticStructureSchema = [3, n0, _BGNQI,
  0,
  [_NQI],
  [64 | 0], 1
];
export var BatchGetNamedQueryOutput$: StaticStructureSchema = [3, n0, _BGNQO,
  0,
  [_NQ, _UNQI],
  [() => NamedQueryList, () => UnprocessedNamedQueryIdList]
];
export var BatchGetPreparedStatementInput$: StaticStructureSchema = [3, n0, _BGPSI,
  0,
  [_PSN, _WG],
  [64 | 0, 0], 2
];
export var BatchGetPreparedStatementOutput$: StaticStructureSchema = [3, n0, _BGPSO,
  0,
  [_PS, _UPSN],
  [() => PreparedStatementDetailsList, () => UnprocessedPreparedStatementNameList]
];
export var BatchGetQueryExecutionInput$: StaticStructureSchema = [3, n0, _BGQEI,
  0,
  [_QEI],
  [64 | 0], 1
];
export var BatchGetQueryExecutionOutput$: StaticStructureSchema = [3, n0, _BGQEO,
  0,
  [_QE, _UQEI],
  [() => QueryExecutionList, () => UnprocessedQueryExecutionIdList]
];
export var CalculationConfiguration$: StaticStructureSchema = [3, n0, _CC,
  0,
  [_CB],
  [0]
];
export var CalculationResult$: StaticStructureSchema = [3, n0, _CR,
  0,
  [_SOSU, _SESU, _RSU, _RT],
  [0, 0, 0, 0]
];
export var CalculationStatistics$: StaticStructureSchema = [3, n0, _CS,
  0,
  [_DEIM, _P],
  [1, 0]
];
export var CalculationStatus$: StaticStructureSchema = [3, n0, _CSa,
  0,
  [_SDT, _CDT, _S, _SCR],
  [4, 4, 0, 0]
];
export var CalculationSummary$: StaticStructureSchema = [3, n0, _CSal,
  0,
  [_CEI, _D, _St],
  [0, 0, () => CalculationStatus$]
];
export var CancelCapacityReservationInput$: StaticStructureSchema = [3, n0, _CCRI,
  0,
  [_N],
  [0], 1
];
export var CancelCapacityReservationOutput$: StaticStructureSchema = [3, n0, _CCRO,
  0,
  [],
  []
];
export var CapacityAllocation$: StaticStructureSchema = [3, n0, _CA,
  0,
  [_St, _RTe, _SM, _RCT],
  [0, 4, 0, 4], 2
];
export var CapacityAssignment$: StaticStructureSchema = [3, n0, _CAa,
  0,
  [_WGN],
  [64 | 0]
];
export var CapacityAssignmentConfiguration$: StaticStructureSchema = [3, n0, _CAC,
  0,
  [_CRN, _CAap],
  [0, () => CapacityAssignmentsList]
];
export var CapacityReservation$: StaticStructureSchema = [3, n0, _CRa,
  0,
  [_N, _St, _TD, _AD, _CT, _LA, _LSAT],
  [0, 0, 1, 1, 4, () => CapacityAllocation$, 4], 5
];
export var Classification$: StaticStructureSchema = [3, n0, _C,
  0,
  [_N, _Pr],
  [0, 128 | 0]
];
export var CloudWatchLoggingConfiguration$: StaticStructureSchema = [3, n0, _CWLC,
  0,
  [_E, _LG, _LSNP, _LT],
  [2, 0, 0, [2, n0, _LTM, 0, 0, 64 | 0]], 1
];
export var Column$: StaticStructureSchema = [3, n0, _Co,
  0,
  [_N, _T, _Com],
  [0, 0, 0], 1
];
export var ColumnInfo$: StaticStructureSchema = [3, n0, _CI,
  0,
  [_N, _T, _CN, _SN, _TN, _L, _Pre, _Sc, _Nu, _CSas],
  [0, 0, 0, 0, 0, 0, 1, 1, 0, 2], 2
];
export var CreateCapacityReservationInput$: StaticStructureSchema = [3, n0, _CCRIr,
  0,
  [_TD, _N, _Ta],
  [1, 0, () => TagList], 2
];
export var CreateCapacityReservationOutput$: StaticStructureSchema = [3, n0, _CCROr,
  0,
  [],
  []
];
export var CreateDataCatalogInput$: StaticStructureSchema = [3, n0, _CDCI,
  0,
  [_N, _T, _D, _Pa, _Ta],
  [0, 0, 0, 128 | 0, () => TagList], 2
];
export var CreateDataCatalogOutput$: StaticStructureSchema = [3, n0, _CDCO,
  0,
  [_DC],
  [() => DataCatalog$]
];
export var CreateNamedQueryInput$: StaticStructureSchema = [3, n0, _CNQI,
  0,
  [_N, _Da, _QS, _D, _CRT, _WG],
  [0, 0, 0, 0, [0, 4], 0], 3
];
export var CreateNamedQueryOutput$: StaticStructureSchema = [3, n0, _CNQO,
  0,
  [_NQIa],
  [0]
];
export var CreateNotebookInput$: StaticStructureSchema = [3, n0, _CNI,
  0,
  [_WG, _N, _CRT],
  [0, 0, 0], 2
];
export var CreateNotebookOutput$: StaticStructureSchema = [3, n0, _CNO,
  0,
  [_NI],
  [0]
];
export var CreatePreparedStatementInput$: StaticStructureSchema = [3, n0, _CPSI,
  0,
  [_SNt, _WG, _QSu, _D],
  [0, 0, 0, 0], 3
];
export var CreatePreparedStatementOutput$: StaticStructureSchema = [3, n0, _CPSO,
  0,
  [],
  []
];
export var CreatePresignedNotebookUrlRequest$: StaticStructureSchema = [3, n0, _CPNUR,
  0,
  [_SI],
  [0], 1
];
export var CreatePresignedNotebookUrlResponse$: StaticStructureSchema = [3, n0, _CPNURr,
  0,
  [_NU, _AT, _ATET],
  [0, 0, 1], 3
];
export var CreateWorkGroupInput$: StaticStructureSchema = [3, n0, _CWGI,
  0,
  [_N, _Con, _D, _Ta],
  [0, () => WorkGroupConfiguration$, 0, () => TagList], 1
];
export var CreateWorkGroupOutput$: StaticStructureSchema = [3, n0, _CWGO,
  0,
  [],
  []
];
export var CustomerContentEncryptionConfiguration$: StaticStructureSchema = [3, n0, _CCEC,
  0,
  [_KK],
  [0], 1
];
export var Database$: StaticStructureSchema = [3, n0, _Da,
  0,
  [_N, _D, _Pa],
  [0, 0, 128 | 0], 1
];
export var DataCatalog$: StaticStructureSchema = [3, n0, _DC,
  0,
  [_N, _T, _D, _Pa, _St, _CTo, _Er],
  [0, 0, 0, 128 | 0, 0, 0, 0], 2
];
export var DataCatalogSummary$: StaticStructureSchema = [3, n0, _DCS,
  0,
  [_CN, _T, _St, _CTo, _Er],
  [0, 0, 0, 0, 0]
];
export var Datum$: StaticStructureSchema = [3, n0, _Dat,
  0,
  [_VCV],
  [0]
];
export var DeleteCapacityReservationInput$: StaticStructureSchema = [3, n0, _DCRI,
  0,
  [_N],
  [0], 1
];
export var DeleteCapacityReservationOutput$: StaticStructureSchema = [3, n0, _DCRO,
  0,
  [],
  []
];
export var DeleteDataCatalogInput$: StaticStructureSchema = [3, n0, _DDCI,
  0,
  [_N, _DCO],
  [0, 2], 1
];
export var DeleteDataCatalogOutput$: StaticStructureSchema = [3, n0, _DDCO,
  0,
  [_DC],
  [() => DataCatalog$]
];
export var DeleteNamedQueryInput$: StaticStructureSchema = [3, n0, _DNQI,
  0,
  [_NQIa],
  [[0, 4]]
];
export var DeleteNamedQueryOutput$: StaticStructureSchema = [3, n0, _DNQO,
  0,
  [],
  []
];
export var DeleteNotebookInput$: StaticStructureSchema = [3, n0, _DNI,
  0,
  [_NI],
  [0], 1
];
export var DeleteNotebookOutput$: StaticStructureSchema = [3, n0, _DNO,
  0,
  [],
  []
];
export var DeletePreparedStatementInput$: StaticStructureSchema = [3, n0, _DPSI,
  0,
  [_SNt, _WG],
  [0, 0], 2
];
export var DeletePreparedStatementOutput$: StaticStructureSchema = [3, n0, _DPSO,
  0,
  [],
  []
];
export var DeleteWorkGroupInput$: StaticStructureSchema = [3, n0, _DWGI,
  0,
  [_WG, _RDO],
  [0, 2], 1
];
export var DeleteWorkGroupOutput$: StaticStructureSchema = [3, n0, _DWGO,
  0,
  [],
  []
];
export var EncryptionConfiguration$: StaticStructureSchema = [3, n0, _ECn,
  0,
  [_EO, _KK],
  [0, 0], 1
];
export var EngineConfiguration$: StaticStructureSchema = [3, n0, _ECng,
  0,
  [_CDS, _MCD, _DEDS, _ACd, _SP, _Cl],
  [1, 1, 1, 128 | 0, 128 | 0, () => ClassificationList]
];
export var EngineVersion$: StaticStructureSchema = [3, n0, _EV,
  0,
  [_SEV, _EEV],
  [0, 0]
];
export var ExecutorsSummary$: StaticStructureSchema = [3, n0, _ES,
  0,
  [_EI, _ETx, _SDTt, _TDT, _ESx, _ESxe],
  [0, 0, 1, 1, 0, 1], 1
];
export var ExportNotebookInput$: StaticStructureSchema = [3, n0, _ENI,
  0,
  [_NI],
  [0], 1
];
export var ExportNotebookOutput$: StaticStructureSchema = [3, n0, _ENO,
  0,
  [_NM, _Pay],
  [() => NotebookMetadata$, 0]
];
export var FilterDefinition$: StaticStructureSchema = [3, n0, _FD,
  0,
  [_N],
  [0]
];
export var GetCalculationExecutionCodeRequest$: StaticStructureSchema = [3, n0, _GCECR,
  0,
  [_CEI],
  [0], 1
];
export var GetCalculationExecutionCodeResponse$: StaticStructureSchema = [3, n0, _GCECRe,
  0,
  [_CB],
  [0]
];
export var GetCalculationExecutionRequest$: StaticStructureSchema = [3, n0, _GCER,
  0,
  [_CEI],
  [0], 1
];
export var GetCalculationExecutionResponse$: StaticStructureSchema = [3, n0, _GCERe,
  0,
  [_CEI, _SI, _D, _WD, _St, _Sta, _Re],
  [0, 0, 0, 0, () => CalculationStatus$, () => CalculationStatistics$, () => CalculationResult$]
];
export var GetCalculationExecutionStatusRequest$: StaticStructureSchema = [3, n0, _GCESR,
  0,
  [_CEI],
  [0], 1
];
export var GetCalculationExecutionStatusResponse$: StaticStructureSchema = [3, n0, _GCESRe,
  0,
  [_St, _Sta],
  [() => CalculationStatus$, () => CalculationStatistics$]
];
export var GetCapacityAssignmentConfigurationInput$: StaticStructureSchema = [3, n0, _GCACI,
  0,
  [_CRN],
  [0], 1
];
export var GetCapacityAssignmentConfigurationOutput$: StaticStructureSchema = [3, n0, _GCACO,
  0,
  [_CAC],
  [() => CapacityAssignmentConfiguration$], 1
];
export var GetCapacityReservationInput$: StaticStructureSchema = [3, n0, _GCRI,
  0,
  [_N],
  [0], 1
];
export var GetCapacityReservationOutput$: StaticStructureSchema = [3, n0, _GCRO,
  0,
  [_CRa],
  [() => CapacityReservation$], 1
];
export var GetDatabaseInput$: StaticStructureSchema = [3, n0, _GDI,
  0,
  [_CN, _DN, _WG],
  [0, 0, 0], 2
];
export var GetDatabaseOutput$: StaticStructureSchema = [3, n0, _GDO,
  0,
  [_Da],
  [() => Database$]
];
export var GetDataCatalogInput$: StaticStructureSchema = [3, n0, _GDCI,
  0,
  [_N, _WG],
  [0, 0], 1
];
export var GetDataCatalogOutput$: StaticStructureSchema = [3, n0, _GDCO,
  0,
  [_DC],
  [() => DataCatalog$]
];
export var GetNamedQueryInput$: StaticStructureSchema = [3, n0, _GNQI,
  0,
  [_NQIa],
  [0], 1
];
export var GetNamedQueryOutput$: StaticStructureSchema = [3, n0, _GNQO,
  0,
  [_NQa],
  [() => NamedQuery$]
];
export var GetNotebookMetadataInput$: StaticStructureSchema = [3, n0, _GNMI,
  0,
  [_NI],
  [0], 1
];
export var GetNotebookMetadataOutput$: StaticStructureSchema = [3, n0, _GNMO,
  0,
  [_NM],
  [() => NotebookMetadata$]
];
export var GetPreparedStatementInput$: StaticStructureSchema = [3, n0, _GPSI,
  0,
  [_SNt, _WG],
  [0, 0], 2
];
export var GetPreparedStatementOutput$: StaticStructureSchema = [3, n0, _GPSO,
  0,
  [_PSr],
  [() => PreparedStatement$]
];
export var GetQueryExecutionInput$: StaticStructureSchema = [3, n0, _GQEI,
  0,
  [_QEIu],
  [0], 1
];
export var GetQueryExecutionOutput$: StaticStructureSchema = [3, n0, _GQEO,
  0,
  [_QEu],
  [() => QueryExecution$]
];
export var GetQueryResultsInput$: StaticStructureSchema = [3, n0, _GQRI,
  0,
  [_QEIu, _NT, _MR, _QRT],
  [0, 0, 1, 0], 1
];
export var GetQueryResultsOutput$: StaticStructureSchema = [3, n0, _GQRO,
  0,
  [_UC, _RS, _NT],
  [1, () => ResultSet$, 0]
];
export var GetQueryRuntimeStatisticsInput$: StaticStructureSchema = [3, n0, _GQRSI,
  0,
  [_QEIu],
  [0], 1
];
export var GetQueryRuntimeStatisticsOutput$: StaticStructureSchema = [3, n0, _GQRSO,
  0,
  [_QRS],
  [() => QueryRuntimeStatistics$]
];
export var GetResourceDashboardRequest$: StaticStructureSchema = [3, n0, _GRDR,
  0,
  [_RARN],
  [0], 1
];
export var GetResourceDashboardResponse$: StaticStructureSchema = [3, n0, _GRDRe,
  0,
  [_U],
  [0], 1
];
export var GetSessionEndpointRequest$: StaticStructureSchema = [3, n0, _GSER,
  0,
  [_SI],
  [0], 1
];
export var GetSessionEndpointResponse$: StaticStructureSchema = [3, n0, _GSERe,
  0,
  [_EU, _AT, _ATET],
  [0, 0, 4], 3
];
export var GetSessionRequest$: StaticStructureSchema = [3, n0, _GSR,
  0,
  [_SI],
  [0], 1
];
export var GetSessionResponse$: StaticStructureSchema = [3, n0, _GSRe,
  0,
  [_SI, _D, _WG, _EV, _ECng, _NV, _MC, _SC, _St, _Sta],
  [0, 0, 0, 0, () => EngineConfiguration$, 0, () => MonitoringConfiguration$, () => SessionConfiguration$, () => SessionStatus$, () => SessionStatistics$]
];
export var GetSessionStatusRequest$: StaticStructureSchema = [3, n0, _GSSR,
  0,
  [_SI],
  [0], 1
];
export var GetSessionStatusResponse$: StaticStructureSchema = [3, n0, _GSSRe,
  0,
  [_SI, _St],
  [0, () => SessionStatus$]
];
export var GetTableMetadataInput$: StaticStructureSchema = [3, n0, _GTMI,
  0,
  [_CN, _DN, _TN, _WG],
  [0, 0, 0, 0], 3
];
export var GetTableMetadataOutput$: StaticStructureSchema = [3, n0, _GTMO,
  0,
  [_TM],
  [() => TableMetadata$]
];
export var GetWorkGroupInput$: StaticStructureSchema = [3, n0, _GWGI,
  0,
  [_WG],
  [0], 1
];
export var GetWorkGroupOutput$: StaticStructureSchema = [3, n0, _GWGO,
  0,
  [_WG],
  [() => WorkGroup$]
];
export var IdentityCenterConfiguration$: StaticStructureSchema = [3, n0, _ICC,
  0,
  [_EIC, _ICIA],
  [2, 0]
];
export var ImportNotebookInput$: StaticStructureSchema = [3, n0, _INI,
  0,
  [_WG, _N, _T, _Pay, _NSLU, _CRT],
  [0, 0, 0, 0, 0, 0], 3
];
export var ImportNotebookOutput$: StaticStructureSchema = [3, n0, _INO,
  0,
  [_NI],
  [0]
];
export var InternalServerException$: StaticErrorSchema = [-3, n0, _ISE,
  { [_e]: _s },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InternalServerException$, InternalServerException);
export var InvalidRequestException$: StaticErrorSchema = [-3, n0, _IRE,
  { [_e]: _c },
  [_AEC, _M],
  [0, 0]
];
TypeRegistry.for(n0).registerError(InvalidRequestException$, InvalidRequestException);
export var ListApplicationDPUSizesInput$: StaticStructureSchema = [3, n0, _LADPUSI,
  0,
  [_MR, _NT],
  [1, 0]
];
export var ListApplicationDPUSizesOutput$: StaticStructureSchema = [3, n0, _LADPUSO,
  0,
  [_ADPUS, _NT],
  [() => ApplicationDPUSizesList, 0]
];
export var ListCalculationExecutionsRequest$: StaticStructureSchema = [3, n0, _LCER,
  0,
  [_SI, _SF, _MR, _NT],
  [0, 0, 1, 0], 1
];
export var ListCalculationExecutionsResponse$: StaticStructureSchema = [3, n0, _LCERi,
  0,
  [_NT, _Ca],
  [0, () => CalculationsList]
];
export var ListCapacityReservationsInput$: StaticStructureSchema = [3, n0, _LCRI,
  0,
  [_NT, _MR],
  [0, 1]
];
export var ListCapacityReservationsOutput$: StaticStructureSchema = [3, n0, _LCRO,
  0,
  [_CRap, _NT],
  [() => CapacityReservationsList, 0], 1
];
export var ListDatabasesInput$: StaticStructureSchema = [3, n0, _LDI,
  0,
  [_CN, _NT, _MR, _WG],
  [0, 0, 1, 0], 1
];
export var ListDatabasesOutput$: StaticStructureSchema = [3, n0, _LDO,
  0,
  [_DL, _NT],
  [() => DatabaseList, 0]
];
export var ListDataCatalogsInput$: StaticStructureSchema = [3, n0, _LDCI,
  0,
  [_NT, _MR, _WG],
  [0, 1, 0]
];
export var ListDataCatalogsOutput$: StaticStructureSchema = [3, n0, _LDCO,
  0,
  [_DCSa, _NT],
  [() => DataCatalogSummaryList, 0]
];
export var ListEngineVersionsInput$: StaticStructureSchema = [3, n0, _LEVI,
  0,
  [_NT, _MR],
  [0, 1]
];
export var ListEngineVersionsOutput$: StaticStructureSchema = [3, n0, _LEVO,
  0,
  [_EVn, _NT],
  [() => EngineVersionsList, 0]
];
export var ListExecutorsRequest$: StaticStructureSchema = [3, n0, _LER,
  0,
  [_SI, _ESF, _MR, _NT],
  [0, 0, 1, 0], 1
];
export var ListExecutorsResponse$: StaticStructureSchema = [3, n0, _LERi,
  0,
  [_SI, _NT, _ES],
  [0, 0, () => ExecutorsSummaryList], 1
];
export var ListNamedQueriesInput$: StaticStructureSchema = [3, n0, _LNQI,
  0,
  [_NT, _MR, _WG],
  [0, 1, 0]
];
export var ListNamedQueriesOutput$: StaticStructureSchema = [3, n0, _LNQO,
  0,
  [_NQI, _NT],
  [64 | 0, 0]
];
export var ListNotebookMetadataInput$: StaticStructureSchema = [3, n0, _LNMI,
  0,
  [_WG, _F, _NT, _MR],
  [0, () => FilterDefinition$, 0, 1], 1
];
export var ListNotebookMetadataOutput$: StaticStructureSchema = [3, n0, _LNMO,
  0,
  [_NT, _NML],
  [0, () => NotebookMetadataArray]
];
export var ListNotebookSessionsRequest$: StaticStructureSchema = [3, n0, _LNSR,
  0,
  [_NI, _MR, _NT],
  [0, 1, 0], 1
];
export var ListNotebookSessionsResponse$: StaticStructureSchema = [3, n0, _LNSRi,
  0,
  [_NSL, _NT],
  [() => NotebookSessionsList, 0], 1
];
export var ListPreparedStatementsInput$: StaticStructureSchema = [3, n0, _LPSI,
  0,
  [_WG, _NT, _MR],
  [0, 0, 1], 1
];
export var ListPreparedStatementsOutput$: StaticStructureSchema = [3, n0, _LPSO,
  0,
  [_PS, _NT],
  [() => PreparedStatementsList, 0]
];
export var ListQueryExecutionsInput$: StaticStructureSchema = [3, n0, _LQEI,
  0,
  [_NT, _MR, _WG],
  [0, 1, 0]
];
export var ListQueryExecutionsOutput$: StaticStructureSchema = [3, n0, _LQEO,
  0,
  [_QEI, _NT],
  [64 | 0, 0]
];
export var ListSessionsRequest$: StaticStructureSchema = [3, n0, _LSR,
  0,
  [_WG, _SF, _MR, _NT],
  [0, 0, 1, 0], 1
];
export var ListSessionsResponse$: StaticStructureSchema = [3, n0, _LSRi,
  0,
  [_NT, _Se],
  [0, () => SessionsList]
];
export var ListTableMetadataInput$: StaticStructureSchema = [3, n0, _LTMI,
  0,
  [_CN, _DN, _Ex, _NT, _MR, _WG],
  [0, 0, 0, 0, 1, 0], 2
];
export var ListTableMetadataOutput$: StaticStructureSchema = [3, n0, _LTMO,
  0,
  [_TML, _NT],
  [() => TableMetadataList, 0]
];
export var ListTagsForResourceInput$: StaticStructureSchema = [3, n0, _LTFRI,
  0,
  [_RARN, _NT, _MR],
  [0, 0, 1], 1
];
export var ListTagsForResourceOutput$: StaticStructureSchema = [3, n0, _LTFRO,
  0,
  [_Ta, _NT],
  [() => TagList, 0]
];
export var ListWorkGroupsInput$: StaticStructureSchema = [3, n0, _LWGI,
  0,
  [_NT, _MR],
  [0, 1]
];
export var ListWorkGroupsOutput$: StaticStructureSchema = [3, n0, _LWGO,
  0,
  [_WGo, _NT],
  [() => WorkGroupsList, 0]
];
export var ManagedLoggingConfiguration$: StaticStructureSchema = [3, n0, _MLC,
  0,
  [_E, _KK],
  [2, 0], 1
];
export var ManagedQueryResultsConfiguration$: StaticStructureSchema = [3, n0, _MQRC,
  0,
  [_E, _ECn],
  [2, () => ManagedQueryResultsEncryptionConfiguration$], 1
];
export var ManagedQueryResultsConfigurationUpdates$: StaticStructureSchema = [3, n0, _MQRCU,
  0,
  [_E, _ECn, _REC],
  [2, () => ManagedQueryResultsEncryptionConfiguration$, 2]
];
export var ManagedQueryResultsEncryptionConfiguration$: StaticStructureSchema = [3, n0, _MQREC,
  0,
  [_KK],
  [0], 1
];
export var MetadataException$: StaticErrorSchema = [-3, n0, _ME,
  { [_e]: _c },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(MetadataException$, MetadataException);
export var MonitoringConfiguration$: StaticStructureSchema = [3, n0, _MC,
  0,
  [_CWLC, _MLC, _SLC],
  [() => CloudWatchLoggingConfiguration$, () => ManagedLoggingConfiguration$, () => S3LoggingConfiguration$]
];
export var NamedQuery$: StaticStructureSchema = [3, n0, _NQa,
  0,
  [_N, _Da, _QS, _D, _NQIa, _WG],
  [0, 0, 0, 0, 0, 0], 3
];
export var NotebookMetadata$: StaticStructureSchema = [3, n0, _NM,
  0,
  [_NI, _N, _WG, _CT, _T, _LMT],
  [0, 0, 0, 4, 0, 4]
];
export var NotebookSessionSummary$: StaticStructureSchema = [3, n0, _NSS,
  0,
  [_SI, _CT],
  [0, 4]
];
export var PreparedStatement$: StaticStructureSchema = [3, n0, _PSr,
  0,
  [_SNt, _QSu, _WGNo, _D, _LMT],
  [0, 0, 0, 0, 4]
];
export var PreparedStatementSummary$: StaticStructureSchema = [3, n0, _PSS,
  0,
  [_SNt, _LMT],
  [0, 4]
];
export var PutCapacityAssignmentConfigurationInput$: StaticStructureSchema = [3, n0, _PCACI,
  0,
  [_CRN, _CAap],
  [0, () => CapacityAssignmentsList], 2
];
export var PutCapacityAssignmentConfigurationOutput$: StaticStructureSchema = [3, n0, _PCACO,
  0,
  [],
  []
];
export var QueryExecution$: StaticStructureSchema = [3, n0, _QEu,
  0,
  [_QEIu, _Q, _ST, _MQRC, _RC, _RRC, _QEC, _St, _Sta, _WG, _EV, _EP, _STu, _QRSAGC],
  [0, 0, 0, () => ManagedQueryResultsConfiguration$, () => ResultConfiguration$, () => ResultReuseConfiguration$, () => QueryExecutionContext$, () => QueryExecutionStatus$, () => QueryExecutionStatistics$, 0, () => EngineVersion$, 64 | 0, 0, () => QueryResultsS3AccessGrantsConfiguration$]
];
export var QueryExecutionContext$: StaticStructureSchema = [3, n0, _QEC,
  0,
  [_Da, _Cat],
  [0, 0]
];
export var QueryExecutionStatistics$: StaticStructureSchema = [3, n0, _QES,
  0,
  [_EETIM, _DSIB, _DML, _TETIM, _QQTIM, _SPPTIM, _QPTIM, _SPTIM, _RRI, _DCp],
  [1, 1, 0, 1, 1, 1, 1, 1, () => ResultReuseInformation$, 1]
];
export var QueryExecutionStatus$: StaticStructureSchema = [3, n0, _QESu,
  0,
  [_S, _SCR, _SDT, _CDT, _AE],
  [0, 0, 4, 4, () => AthenaError$]
];
export var QueryResultsS3AccessGrantsConfiguration$: StaticStructureSchema = [3, n0, _QRSAGC,
  0,
  [_ESAG, _ATu, _CULP],
  [2, 0, 2], 2
];
export var QueryRuntimeStatistics$: StaticStructureSchema = [3, n0, _QRS,
  0,
  [_Ti, _Ro, _OS],
  [() => QueryRuntimeStatisticsTimeline$, () => QueryRuntimeStatisticsRows$, () => QueryStage$]
];
export var QueryRuntimeStatisticsRows$: StaticStructureSchema = [3, n0, _QRSR,
  0,
  [_IR, _IB, _OB, _OR],
  [1, 1, 1, 1]
];
export var QueryRuntimeStatisticsTimeline$: StaticStructureSchema = [3, n0, _QRST,
  0,
  [_QQTIM, _SPPTIM, _QPTIM, _EETIM, _SPTIM, _TETIM],
  [1, 1, 1, 1, 1, 1]
];
export var QueryStage$: StaticStructureSchema = [3, n0, _QSue,
  0,
  [_SIt, _S, _OB, _OR, _IB, _IR, _ETxe, _QSP, _SS],
  [1, 0, 1, 1, 1, 1, 1, () => QueryStagePlanNode$, () => QueryStages]
];
export var QueryStagePlanNode$: StaticStructureSchema = [3, n0, _QSPN,
  0,
  [_N, _I, _Ch, _RSe],
  [0, 0, () => QueryStagePlanNodes, 64 | 0]
];
export var ResourceNotFoundException$: StaticErrorSchema = [-3, n0, _RNFE,
  { [_e]: _c },
  [_M, _RN],
  [0, 0]
];
TypeRegistry.for(n0).registerError(ResourceNotFoundException$, ResourceNotFoundException);
export var ResultConfiguration$: StaticStructureSchema = [3, n0, _RC,
  0,
  [_OL, _ECn, _EBO, _AC],
  [0, () => EncryptionConfiguration$, 0, () => AclConfiguration$]
];
export var ResultConfigurationUpdates$: StaticStructureSchema = [3, n0, _RCU,
  0,
  [_OL, _ROL, _ECn, _REC, _EBO, _REBO, _AC, _RAC],
  [0, 2, () => EncryptionConfiguration$, 2, 0, 2, () => AclConfiguration$, 2]
];
export var ResultReuseByAgeConfiguration$: StaticStructureSchema = [3, n0, _RRBAC,
  0,
  [_E, _MAIM],
  [2, 1], 1
];
export var ResultReuseConfiguration$: StaticStructureSchema = [3, n0, _RRC,
  0,
  [_RRBAC],
  [() => ResultReuseByAgeConfiguration$]
];
export var ResultReuseInformation$: StaticStructureSchema = [3, n0, _RRI,
  0,
  [_RPR],
  [2], 1
];
export var ResultSet$: StaticStructureSchema = [3, n0, _RS,
  0,
  [_Ro, _RSM],
  [() => RowList, () => ResultSetMetadata$]
];
export var ResultSetMetadata$: StaticStructureSchema = [3, n0, _RSM,
  0,
  [_CI],
  [() => ColumnInfoList]
];
export var Row$: StaticStructureSchema = [3, n0, _Row,
  0,
  [_Data],
  [() => datumList]
];
export var S3LoggingConfiguration$: StaticStructureSchema = [3, n0, _SLC,
  0,
  [_E, _KK, _LL],
  [2, 0, 0], 1
];
export var SessionAlreadyExistsException$: StaticErrorSchema = [-3, n0, _SAEE,
  { [_e]: _c },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(SessionAlreadyExistsException$, SessionAlreadyExistsException);
export var SessionConfiguration$: StaticStructureSchema = [3, n0, _SC,
  0,
  [_ER, _WD, _ITS, _SITIM, _ECn],
  [0, 0, 1, 1, () => EncryptionConfiguration$]
];
export var SessionStatistics$: StaticStructureSchema = [3, n0, _SSe,
  0,
  [_DEIM],
  [1]
];
export var SessionStatus$: StaticStructureSchema = [3, n0, _SSes,
  0,
  [_SDTt, _LMDT, _EDT, _ISDT, _S, _SCR],
  [4, 4, 4, 4, 0, 0]
];
export var SessionSummary$: StaticStructureSchema = [3, n0, _SSess,
  0,
  [_SI, _D, _EV, _NV, _St],
  [0, 0, () => EngineVersion$, 0, () => SessionStatus$]
];
export var StartCalculationExecutionRequest$: StaticStructureSchema = [3, n0, _SCER,
  0,
  [_SI, _D, _CC, _CB, _CRT],
  [0, 0, () => CalculationConfiguration$, 0, 0], 1
];
export var StartCalculationExecutionResponse$: StaticStructureSchema = [3, n0, _SCERt,
  0,
  [_CEI, _S],
  [0, 0]
];
export var StartQueryExecutionInput$: StaticStructureSchema = [3, n0, _SQEI,
  0,
  [_QS, _CRT, _QEC, _RC, _WG, _EP, _RRC, _ECng],
  [0, [0, 4], () => QueryExecutionContext$, () => ResultConfiguration$, 0, 64 | 0, () => ResultReuseConfiguration$, () => EngineConfiguration$], 1
];
export var StartQueryExecutionOutput$: StaticStructureSchema = [3, n0, _SQEO,
  0,
  [_QEIu],
  [0]
];
export var StartSessionRequest$: StaticStructureSchema = [3, n0, _SSR,
  0,
  [_WG, _ECng, _D, _ER, _MC, _NV, _SITIM, _CRT, _Ta, _CWGT],
  [0, () => EngineConfiguration$, 0, 0, () => MonitoringConfiguration$, 0, 1, 0, () => TagList, 2], 2
];
export var StartSessionResponse$: StaticStructureSchema = [3, n0, _SSRt,
  0,
  [_SI, _S],
  [0, 0]
];
export var StopCalculationExecutionRequest$: StaticStructureSchema = [3, n0, _SCERto,
  0,
  [_CEI],
  [0], 1
];
export var StopCalculationExecutionResponse$: StaticStructureSchema = [3, n0, _SCERtop,
  0,
  [_S],
  [0]
];
export var StopQueryExecutionInput$: StaticStructureSchema = [3, n0, _SQEIt,
  0,
  [_QEIu],
  [[0, 4]]
];
export var StopQueryExecutionOutput$: StaticStructureSchema = [3, n0, _SQEOt,
  0,
  [],
  []
];
export var TableMetadata$: StaticStructureSchema = [3, n0, _TM,
  0,
  [_N, _CTr, _LAT, _TT, _Col, _PK, _Pa],
  [0, 4, 4, 0, () => ColumnList, () => ColumnList, 128 | 0], 1
];
export var Tag$: StaticStructureSchema = [3, n0, _Tag,
  0,
  [_K, _V],
  [0, 0]
];
export var TagResourceInput$: StaticStructureSchema = [3, n0, _TRI,
  0,
  [_RARN, _Ta],
  [0, () => TagList], 2
];
export var TagResourceOutput$: StaticStructureSchema = [3, n0, _TRO,
  0,
  [],
  []
];
export var TerminateSessionRequest$: StaticStructureSchema = [3, n0, _TSR,
  0,
  [_SI],
  [0], 1
];
export var TerminateSessionResponse$: StaticStructureSchema = [3, n0, _TSRe,
  0,
  [_S],
  [0]
];
export var TooManyRequestsException$: StaticErrorSchema = [-3, n0, _TMRE,
  { [_e]: _c },
  [_M, _Rea],
  [0, 0]
];
TypeRegistry.for(n0).registerError(TooManyRequestsException$, TooManyRequestsException);
export var UnprocessedNamedQueryId$: StaticStructureSchema = [3, n0, _UNQIn,
  0,
  [_NQIa, _ECr, _EM],
  [0, 0, 0]
];
export var UnprocessedPreparedStatementName$: StaticStructureSchema = [3, n0, _UPSNn,
  0,
  [_SNt, _ECr, _EM],
  [0, 0, 0]
];
export var UnprocessedQueryExecutionId$: StaticStructureSchema = [3, n0, _UQEIn,
  0,
  [_QEIu, _ECr, _EM],
  [0, 0, 0]
];
export var UntagResourceInput$: StaticStructureSchema = [3, n0, _URI,
  0,
  [_RARN, _TK],
  [0, 64 | 0], 2
];
export var UntagResourceOutput$: StaticStructureSchema = [3, n0, _URO,
  0,
  [],
  []
];
export var UpdateCapacityReservationInput$: StaticStructureSchema = [3, n0, _UCRI,
  0,
  [_TD, _N],
  [1, 0], 2
];
export var UpdateCapacityReservationOutput$: StaticStructureSchema = [3, n0, _UCRO,
  0,
  [],
  []
];
export var UpdateDataCatalogInput$: StaticStructureSchema = [3, n0, _UDCI,
  0,
  [_N, _T, _D, _Pa],
  [0, 0, 0, 128 | 0], 2
];
export var UpdateDataCatalogOutput$: StaticStructureSchema = [3, n0, _UDCO,
  0,
  [],
  []
];
export var UpdateNamedQueryInput$: StaticStructureSchema = [3, n0, _UNQIp,
  0,
  [_NQIa, _N, _QS, _D],
  [0, 0, 0, 0], 3
];
export var UpdateNamedQueryOutput$: StaticStructureSchema = [3, n0, _UNQO,
  0,
  [],
  []
];
export var UpdateNotebookInput$: StaticStructureSchema = [3, n0, _UNI,
  0,
  [_NI, _Pay, _T, _SI, _CRT],
  [0, 0, 0, 0, 0], 3
];
export var UpdateNotebookMetadataInput$: StaticStructureSchema = [3, n0, _UNMI,
  0,
  [_NI, _N, _CRT],
  [0, 0, 0], 2
];
export var UpdateNotebookMetadataOutput$: StaticStructureSchema = [3, n0, _UNMO,
  0,
  [],
  []
];
export var UpdateNotebookOutput$: StaticStructureSchema = [3, n0, _UNO,
  0,
  [],
  []
];
export var UpdatePreparedStatementInput$: StaticStructureSchema = [3, n0, _UPSI,
  0,
  [_SNt, _WG, _QSu, _D],
  [0, 0, 0, 0], 3
];
export var UpdatePreparedStatementOutput$: StaticStructureSchema = [3, n0, _UPSO,
  0,
  [],
  []
];
export var UpdateWorkGroupInput$: StaticStructureSchema = [3, n0, _UWGI,
  0,
  [_WG, _D, _CU, _S],
  [0, 0, () => WorkGroupConfigurationUpdates$, 0], 1
];
export var UpdateWorkGroupOutput$: StaticStructureSchema = [3, n0, _UWGO,
  0,
  [],
  []
];
export var WorkGroup$: StaticStructureSchema = [3, n0, _WG,
  0,
  [_N, _S, _Con, _D, _CT, _ICAA],
  [0, 0, () => WorkGroupConfiguration$, 0, 4, 0], 1
];
export var WorkGroupConfiguration$: StaticStructureSchema = [3, n0, _WGC,
  0,
  [_RC, _MQRC, _EWGC, _PCWME, _BSCPQ, _RPE, _EV, _ACdd, _ER, _MC, _ECng, _CCEC, _EMEC, _ICC, _QRSAGC],
  [() => ResultConfiguration$, () => ManagedQueryResultsConfiguration$, 2, 2, 1, 2, () => EngineVersion$, 0, 0, () => MonitoringConfiguration$, () => EngineConfiguration$, () => CustomerContentEncryptionConfiguration$, 2, () => IdentityCenterConfiguration$, () => QueryResultsS3AccessGrantsConfiguration$]
];
export var WorkGroupConfigurationUpdates$: StaticStructureSchema = [3, n0, _WGCU,
  0,
  [_EWGC, _RCU, _MQRCU, _PCWME, _BSCPQ, _RBSCPQ, _RPE, _EV, _RCCEC, _ACdd, _ER, _CCEC, _EMEC, _QRSAGC, _MC, _ECng],
  [2, () => ResultConfigurationUpdates$, () => ManagedQueryResultsConfigurationUpdates$, 2, 1, 2, 2, () => EngineVersion$, 2, 0, 0, () => CustomerContentEncryptionConfiguration$, 2, () => QueryResultsS3AccessGrantsConfiguration$, () => MonitoringConfiguration$, () => EngineConfiguration$]
];
export var WorkGroupSummary$: StaticStructureSchema = [3, n0, _WGS,
  0,
  [_N, _S, _D, _CT, _EV, _ICAA],
  [0, 0, 0, 4, () => EngineVersion$, 0]
];
export var AthenaServiceException$: StaticErrorSchema = [-3, _sm, "AthenaServiceException", 0, [], []];
TypeRegistry.for(_sm).registerError(AthenaServiceException$, AthenaServiceException);
var ApplicationDPUSizesList: StaticListSchema = [1, n0, _ADPUSL,
  0, () => ApplicationDPUSizes$
];
var CalculationsList: StaticListSchema = [1, n0, _CL,
  0, () => CalculationSummary$
];
var CapacityAssignmentsList: StaticListSchema = [1, n0, _CAL,
  0, () => CapacityAssignment$
];
var CapacityReservationsList: StaticListSchema = [1, n0, _CRL,
  0, () => CapacityReservation$
];
var ClassificationList: StaticListSchema = [1, n0, _CLl,
  0, () => Classification$
];
var ColumnInfoList: StaticListSchema = [1, n0, _CIL,
  0, () => ColumnInfo$
];
var ColumnList: StaticListSchema = [1, n0, _CLo,
  0, () => Column$
];
var DatabaseList: StaticListSchema = [1, n0, _DL,
  0, () => Database$
];
var DataCatalogSummaryList: StaticListSchema = [1, n0, _DCSL,
  0, () => DataCatalogSummary$
];
var datumList: StaticListSchema = [1, n0, _dL,
  0, () => Datum$
];
var EngineVersionsList: StaticListSchema = [1, n0, _EVL,
  0, () => EngineVersion$
];
var ExecutionParameters = 64 | 0;
var ExecutorsSummaryList: StaticListSchema = [1, n0, _ESL,
  0, () => ExecutorsSummary$
];
var LogTypeValuesList = 64 | 0;
var NamedQueryIdList = 64 | 0;
var NamedQueryList: StaticListSchema = [1, n0, _NQL,
  0, () => NamedQuery$
];
var NotebookMetadataArray: StaticListSchema = [1, n0, _NMA,
  0, () => NotebookMetadata$
];
var NotebookSessionsList: StaticListSchema = [1, n0, _NSL,
  0, () => NotebookSessionSummary$
];
var PreparedStatementDetailsList: StaticListSchema = [1, n0, _PSDL,
  0, () => PreparedStatement$
];
var PreparedStatementNameList = 64 | 0;
var PreparedStatementsList: StaticListSchema = [1, n0, _PSL,
  0, () => PreparedStatementSummary$
];
var QueryExecutionIdList = 64 | 0;
var QueryExecutionList: StaticListSchema = [1, n0, _QEL,
  0, () => QueryExecution$
];
var QueryStagePlanNodes: StaticListSchema = [1, n0, _QSPNu,
  0, () => QueryStagePlanNode$
];
var QueryStages: StaticListSchema = [1, n0, _QSuer,
  0, () => QueryStage$
];
var RowList: StaticListSchema = [1, n0, _RL,
  0, () => Row$
];
var SessionsList: StaticListSchema = [1, n0, _SL,
  0, () => SessionSummary$
];
var StringList = 64 | 0;
var SupportedDPUSizeList = 64 | 1;
var TableMetadataList: StaticListSchema = [1, n0, _TML,
  0, () => TableMetadata$
];
var TagKeyList = 64 | 0;
var TagList: StaticListSchema = [1, n0, _TL,
  0, () => Tag$
];
var UnprocessedNamedQueryIdList: StaticListSchema = [1, n0, _UNQIL,
  0, () => UnprocessedNamedQueryId$
];
var UnprocessedPreparedStatementNameList: StaticListSchema = [1, n0, _UPSNL,
  0, () => UnprocessedPreparedStatementName$
];
var UnprocessedQueryExecutionIdList: StaticListSchema = [1, n0, _UQEIL,
  0, () => UnprocessedQueryExecutionId$
];
var WorkGroupNamesList = 64 | 0;
var WorkGroupsList: StaticListSchema = [1, n0, _WGL,
  0, () => WorkGroupSummary$
];
var LogTypesMap: StaticMapSchema = [2, n0, _LTM,
  0, 0, 64 | 0
];
var ParametersMap = 128 | 0;
export var BatchGetNamedQuery$: StaticOperationSchema = [9, n0, _BGNQ,
  0, () => BatchGetNamedQueryInput$, () => BatchGetNamedQueryOutput$
];
export var BatchGetPreparedStatement$: StaticOperationSchema = [9, n0, _BGPS,
  0, () => BatchGetPreparedStatementInput$, () => BatchGetPreparedStatementOutput$
];
export var BatchGetQueryExecution$: StaticOperationSchema = [9, n0, _BGQE,
  0, () => BatchGetQueryExecutionInput$, () => BatchGetQueryExecutionOutput$
];
export var CancelCapacityReservation$: StaticOperationSchema = [9, n0, _CCR,
  2, () => CancelCapacityReservationInput$, () => CancelCapacityReservationOutput$
];
export var CreateCapacityReservation$: StaticOperationSchema = [9, n0, _CCRr,
  2, () => CreateCapacityReservationInput$, () => CreateCapacityReservationOutput$
];
export var CreateDataCatalog$: StaticOperationSchema = [9, n0, _CDC,
  0, () => CreateDataCatalogInput$, () => CreateDataCatalogOutput$
];
export var CreateNamedQuery$: StaticOperationSchema = [9, n0, _CNQ,
  2, () => CreateNamedQueryInput$, () => CreateNamedQueryOutput$
];
export var CreateNotebook$: StaticOperationSchema = [9, n0, _CNr,
  0, () => CreateNotebookInput$, () => CreateNotebookOutput$
];
export var CreatePreparedStatement$: StaticOperationSchema = [9, n0, _CPS,
  0, () => CreatePreparedStatementInput$, () => CreatePreparedStatementOutput$
];
export var CreatePresignedNotebookUrl$: StaticOperationSchema = [9, n0, _CPNU,
  0, () => CreatePresignedNotebookUrlRequest$, () => CreatePresignedNotebookUrlResponse$
];
export var CreateWorkGroup$: StaticOperationSchema = [9, n0, _CWG,
  0, () => CreateWorkGroupInput$, () => CreateWorkGroupOutput$
];
export var DeleteCapacityReservation$: StaticOperationSchema = [9, n0, _DCR,
  2, () => DeleteCapacityReservationInput$, () => DeleteCapacityReservationOutput$
];
export var DeleteDataCatalog$: StaticOperationSchema = [9, n0, _DDC,
  0, () => DeleteDataCatalogInput$, () => DeleteDataCatalogOutput$
];
export var DeleteNamedQuery$: StaticOperationSchema = [9, n0, _DNQ,
  2, () => DeleteNamedQueryInput$, () => DeleteNamedQueryOutput$
];
export var DeleteNotebook$: StaticOperationSchema = [9, n0, _DNe,
  0, () => DeleteNotebookInput$, () => DeleteNotebookOutput$
];
export var DeletePreparedStatement$: StaticOperationSchema = [9, n0, _DPS,
  0, () => DeletePreparedStatementInput$, () => DeletePreparedStatementOutput$
];
export var DeleteWorkGroup$: StaticOperationSchema = [9, n0, _DWG,
  2, () => DeleteWorkGroupInput$, () => DeleteWorkGroupOutput$
];
export var ExportNotebook$: StaticOperationSchema = [9, n0, _EN,
  0, () => ExportNotebookInput$, () => ExportNotebookOutput$
];
export var GetCalculationExecution$: StaticOperationSchema = [9, n0, _GCE,
  0, () => GetCalculationExecutionRequest$, () => GetCalculationExecutionResponse$
];
export var GetCalculationExecutionCode$: StaticOperationSchema = [9, n0, _GCEC,
  0, () => GetCalculationExecutionCodeRequest$, () => GetCalculationExecutionCodeResponse$
];
export var GetCalculationExecutionStatus$: StaticOperationSchema = [9, n0, _GCES,
  0, () => GetCalculationExecutionStatusRequest$, () => GetCalculationExecutionStatusResponse$
];
export var GetCapacityAssignmentConfiguration$: StaticOperationSchema = [9, n0, _GCAC,
  0, () => GetCapacityAssignmentConfigurationInput$, () => GetCapacityAssignmentConfigurationOutput$
];
export var GetCapacityReservation$: StaticOperationSchema = [9, n0, _GCR,
  0, () => GetCapacityReservationInput$, () => GetCapacityReservationOutput$
];
export var GetDatabase$: StaticOperationSchema = [9, n0, _GD,
  0, () => GetDatabaseInput$, () => GetDatabaseOutput$
];
export var GetDataCatalog$: StaticOperationSchema = [9, n0, _GDC,
  0, () => GetDataCatalogInput$, () => GetDataCatalogOutput$
];
export var GetNamedQuery$: StaticOperationSchema = [9, n0, _GNQ,
  0, () => GetNamedQueryInput$, () => GetNamedQueryOutput$
];
export var GetNotebookMetadata$: StaticOperationSchema = [9, n0, _GNM,
  0, () => GetNotebookMetadataInput$, () => GetNotebookMetadataOutput$
];
export var GetPreparedStatement$: StaticOperationSchema = [9, n0, _GPS,
  0, () => GetPreparedStatementInput$, () => GetPreparedStatementOutput$
];
export var GetQueryExecution$: StaticOperationSchema = [9, n0, _GQE,
  0, () => GetQueryExecutionInput$, () => GetQueryExecutionOutput$
];
export var GetQueryResults$: StaticOperationSchema = [9, n0, _GQR,
  0, () => GetQueryResultsInput$, () => GetQueryResultsOutput$
];
export var GetQueryRuntimeStatistics$: StaticOperationSchema = [9, n0, _GQRS,
  0, () => GetQueryRuntimeStatisticsInput$, () => GetQueryRuntimeStatisticsOutput$
];
export var GetResourceDashboard$: StaticOperationSchema = [9, n0, _GRD,
  0, () => GetResourceDashboardRequest$, () => GetResourceDashboardResponse$
];
export var GetSession$: StaticOperationSchema = [9, n0, _GS,
  0, () => GetSessionRequest$, () => GetSessionResponse$
];
export var GetSessionEndpoint$: StaticOperationSchema = [9, n0, _GSE,
  0, () => GetSessionEndpointRequest$, () => GetSessionEndpointResponse$
];
export var GetSessionStatus$: StaticOperationSchema = [9, n0, _GSS,
  0, () => GetSessionStatusRequest$, () => GetSessionStatusResponse$
];
export var GetTableMetadata$: StaticOperationSchema = [9, n0, _GTM,
  0, () => GetTableMetadataInput$, () => GetTableMetadataOutput$
];
export var GetWorkGroup$: StaticOperationSchema = [9, n0, _GWG,
  0, () => GetWorkGroupInput$, () => GetWorkGroupOutput$
];
export var ImportNotebook$: StaticOperationSchema = [9, n0, _IN,
  0, () => ImportNotebookInput$, () => ImportNotebookOutput$
];
export var ListApplicationDPUSizes$: StaticOperationSchema = [9, n0, _LADPUS,
  0, () => ListApplicationDPUSizesInput$, () => ListApplicationDPUSizesOutput$
];
export var ListCalculationExecutions$: StaticOperationSchema = [9, n0, _LCE,
  0, () => ListCalculationExecutionsRequest$, () => ListCalculationExecutionsResponse$
];
export var ListCapacityReservations$: StaticOperationSchema = [9, n0, _LCR,
  0, () => ListCapacityReservationsInput$, () => ListCapacityReservationsOutput$
];
export var ListDatabases$: StaticOperationSchema = [9, n0, _LD,
  0, () => ListDatabasesInput$, () => ListDatabasesOutput$
];
export var ListDataCatalogs$: StaticOperationSchema = [9, n0, _LDC,
  0, () => ListDataCatalogsInput$, () => ListDataCatalogsOutput$
];
export var ListEngineVersions$: StaticOperationSchema = [9, n0, _LEV,
  0, () => ListEngineVersionsInput$, () => ListEngineVersionsOutput$
];
export var ListExecutors$: StaticOperationSchema = [9, n0, _LE,
  0, () => ListExecutorsRequest$, () => ListExecutorsResponse$
];
export var ListNamedQueries$: StaticOperationSchema = [9, n0, _LNQ,
  0, () => ListNamedQueriesInput$, () => ListNamedQueriesOutput$
];
export var ListNotebookMetadata$: StaticOperationSchema = [9, n0, _LNM,
  0, () => ListNotebookMetadataInput$, () => ListNotebookMetadataOutput$
];
export var ListNotebookSessions$: StaticOperationSchema = [9, n0, _LNS,
  0, () => ListNotebookSessionsRequest$, () => ListNotebookSessionsResponse$
];
export var ListPreparedStatements$: StaticOperationSchema = [9, n0, _LPS,
  0, () => ListPreparedStatementsInput$, () => ListPreparedStatementsOutput$
];
export var ListQueryExecutions$: StaticOperationSchema = [9, n0, _LQE,
  0, () => ListQueryExecutionsInput$, () => ListQueryExecutionsOutput$
];
export var ListSessions$: StaticOperationSchema = [9, n0, _LS,
  0, () => ListSessionsRequest$, () => ListSessionsResponse$
];
export var ListTableMetadata$: StaticOperationSchema = [9, n0, _LTMi,
  0, () => ListTableMetadataInput$, () => ListTableMetadataOutput$
];
export var ListTagsForResource$: StaticOperationSchema = [9, n0, _LTFR,
  0, () => ListTagsForResourceInput$, () => ListTagsForResourceOutput$
];
export var ListWorkGroups$: StaticOperationSchema = [9, n0, _LWG,
  0, () => ListWorkGroupsInput$, () => ListWorkGroupsOutput$
];
export var PutCapacityAssignmentConfiguration$: StaticOperationSchema = [9, n0, _PCAC,
  2, () => PutCapacityAssignmentConfigurationInput$, () => PutCapacityAssignmentConfigurationOutput$
];
export var StartCalculationExecution$: StaticOperationSchema = [9, n0, _SCE,
  0, () => StartCalculationExecutionRequest$, () => StartCalculationExecutionResponse$
];
export var StartQueryExecution$: StaticOperationSchema = [9, n0, _SQE,
  2, () => StartQueryExecutionInput$, () => StartQueryExecutionOutput$
];
export var StartSession$: StaticOperationSchema = [9, n0, _SSt,
  0, () => StartSessionRequest$, () => StartSessionResponse$
];
export var StopCalculationExecution$: StaticOperationSchema = [9, n0, _SCEt,
  0, () => StopCalculationExecutionRequest$, () => StopCalculationExecutionResponse$
];
export var StopQueryExecution$: StaticOperationSchema = [9, n0, _SQEt,
  2, () => StopQueryExecutionInput$, () => StopQueryExecutionOutput$
];
export var TagResource$: StaticOperationSchema = [9, n0, _TR,
  0, () => TagResourceInput$, () => TagResourceOutput$
];
export var TerminateSession$: StaticOperationSchema = [9, n0, _TS,
  0, () => TerminateSessionRequest$, () => TerminateSessionResponse$
];
export var UntagResource$: StaticOperationSchema = [9, n0, _UR,
  0, () => UntagResourceInput$, () => UntagResourceOutput$
];
export var UpdateCapacityReservation$: StaticOperationSchema = [9, n0, _UCR,
  0, () => UpdateCapacityReservationInput$, () => UpdateCapacityReservationOutput$
];
export var UpdateDataCatalog$: StaticOperationSchema = [9, n0, _UDC,
  0, () => UpdateDataCatalogInput$, () => UpdateDataCatalogOutput$
];
export var UpdateNamedQuery$: StaticOperationSchema = [9, n0, _UNQ,
  2, () => UpdateNamedQueryInput$, () => UpdateNamedQueryOutput$
];
export var UpdateNotebook$: StaticOperationSchema = [9, n0, _UN,
  0, () => UpdateNotebookInput$, () => UpdateNotebookOutput$
];
export var UpdateNotebookMetadata$: StaticOperationSchema = [9, n0, _UNM,
  0, () => UpdateNotebookMetadataInput$, () => UpdateNotebookMetadataOutput$
];
export var UpdatePreparedStatement$: StaticOperationSchema = [9, n0, _UPS,
  0, () => UpdatePreparedStatementInput$, () => UpdatePreparedStatementOutput$
];
export var UpdateWorkGroup$: StaticOperationSchema = [9, n0, _UWG,
  0, () => UpdateWorkGroupInput$, () => UpdateWorkGroupOutput$
];
