const _A = "Accounts";
const _AA = "AdminAccount";
const _AAA = "AssociateAdminAccount";
const _AAAR = "AssociateAdminAccountRequest";
const _AAE = "AllAccountsEnabled";
const _AAI = "AdminAccountId";
const _AAS = "AdminAccountSummary";
const _AASL = "AdminAccountSummaryList";
const _AAd = "AdminAccounts";
const _AEIV = "AwsEc2InstanceViolation";
const _AENIV = "AwsEc2NetworkInterfaceViolations";
const _AENIVw = "AwsEc2NetworkInterfaceViolation";
const _AEO = "ActualEvaluationOrder";
const _AFE = "ActualFirewallEndpoint";
const _AFSI = "ActualFirewallSubnetId";
const _AFSR = "ActualFirewallSubnetRoutes";
const _AI = "AccountId";
const _AIGR = "ActualInternetGatewayRoutes";
const _AIs = "AssociationId";
const _AL = "AppsList";
const _ALA = "AppsListArn";
const _ALD = "AppsListData";
const _ALDS = "AppsListDataSummary";
const _ALDp = "AppsListsData";
const _ALp = "AppsLists";
const _AN = "AppName";
const _AOUE = "AllOrganizationalUnitsEnabled";
const _APTE = "AllPolicyTypesEnabled";
const _ARE = "AllRegionsEnabled";
const _AS = "AccountScope";
const _ASd = "AdminScope";
const _ASf = "AffectedSubnets";
const _AT = "ActionTarget";
const _ATPF = "AssociateThirdPartyFirewall";
const _ATPFR = "AssociateThirdPartyFirewallRequest";
const _ATPFRs = "AssociateThirdPartyFirewallResponse";
const _ATl = "AllowedTargets";
const _AVPCSGV = "AwsVPCSecurityGroupViolation";
const _AZ = "AvailabilityZone";
const _Ac = "Action";
const _Act = "Actions";
const _Ap = "App";
const _BAR = "BatchAssociateResource";
const _BARR = "BatchAssociateResourceRequest";
const _BARRa = "BatchAssociateResourceResponse";
const _BDR = "BatchDisassociateResource";
const _BDRR = "BatchDisassociateResourceRequest";
const _BDRRa = "BatchDisassociateResourceResponse";
const _C = "Code";
const _CANA = "CurrentAssociatedNetworkAcl";
const _CB = "CidrBlock";
const _CFSRT = "CurrentFirewallSubnetRouteTable";
const _CIGRT = "CurrentInternetGatewayRouteTable";
const _CNAA = "CreateNetworkAclAction";
const _CNAEA = "CreateNetworkAclEntriesAction";
const _CP = "ConflictingPriority";
const _CPD = "CurrentPolicyDescription";
const _CPI = "ConflictingPolicyId";
const _CPSM = "CustomerPolicyScopeMap";
const _CRT = "CurrentRouteTable";
const _CS = "ComplianceStatus";
const _CSo = "ContributingSubnets";
const _CT = "CreateTime";
const _CV = "ComplianceViolator";
const _CVo = "ComplianceViolators";
const _D = "Description";
const _DA = "DefaultAdmin";
const _DAA = "DisassociateAdminAccount";
const _DAAR = "DisassociateAdminAccountRequest";
const _DAL = "DeleteAppsList";
const _DALR = "DeleteAppsListRequest";
const _DAPR = "DeleteAllPolicyResources";
const _DCB = "DestinationCidrBlock";
const _DDRGV = "DnsDuplicateRuleGroupViolation";
const _DICB = "DestinationIpv6CidrBlock";
const _DL = "DefaultList";
const _DLe = "DefaultLists";
const _DNAEA = "DeleteNetworkAclEntriesAction";
const _DNC = "DeleteNotificationChannel";
const _DNCR = "DeleteNotificationChannelRequest";
const _DP = "DeletePolicy";
const _DPL = "DeleteProtocolsList";
const _DPLI = "DestinationPrefixListId";
const _DPLR = "DeleteProtocolsListRequest";
const _DPR = "DeletePolicyRequest";
const _DR = "DiscoveredResource";
const _DRGLEV = "DnsRuleGroupLimitExceededViolation";
const _DRGPCV = "DnsRuleGroupPriorityConflictViolation";
const _DRL = "DiscoveredResourceList";
const _DRS = "DeleteResourceSet";
const _DRSR = "DeleteResourceSetRequest";
const _DT = "DestinationType";
const _DTPF = "DisassociateThirdPartyFirewall";
const _DTPFR = "DisassociateThirdPartyFirewallRequest";
const _DTPFRi = "DisassociateThirdPartyFirewallResponse";
const _DUFMMR = "DeleteUnusedFMManagedResources";
const _Da = "Data";
const _De = "Destination";
const _E = "Egress";
const _EA = "ExpiredAt";
const _EAEEO = "EntryAtExpectedEvaluationOrder";
const _ECARTA = "EC2AssociateRouteTableAction";
const _ECCRA = "EC2CreateRouteAction";
const _ECCRTA = "EC2CopyRouteTableAction";
const _ECCRTAr = "EC2CreateRouteTableAction";
const _ECDRA = "EC2DeleteRouteAction";
const _ECRRA = "EC2ReplaceRouteAction";
const _ECRRTAA = "EC2ReplaceRouteTableAssociationAction";
const _ED = "EntryDescription";
const _EDn = "EntryDetail";
const _EDnt = "EntriesDescription";
const _EE = "ExpectedEntry";
const _EEO = "ExpectedEvaluationOrder";
const _EFE = "ExpectedFirewallEndpoint";
const _EFSI = "ExpectedFirewallSubnetId";
const _EFSR = "ExpectedFirewallSubnetRoutes";
const _EIGR = "ExpectedInternetGatewayRoutes";
const _ELE = "EvaluationLimitExceeded";
const _EM = "ExcludeMap";
const _EPD = "ExpectedPolicyDescription";
const _ER = "EvaluationResult";
const _ERN = "EntryRuleNumber";
const _ERT = "ExpectedRouteTable";
const _ERTx = "ExcludeResourceTags";
const _ERv = "EvaluationResults";
const _ERx = "ExpectedRoute";
const _ERxp = "ExpectedRoutes";
const _ESA = "ExcludeSpecifiedAccounts";
const _ESOU = "ExcludeSpecifiedOrganizationalUnits";
const _ET = "EntryType";
const _ETn = "EndTime";
const _EV = "EntryViolation";
const _EVR = "EntryViolationReasons";
const _EVn = "EntryViolations";
const _EWC = "EntriesWithConflicts";
const _F = "From";
const _FCC = "FirewallCreationConfig";
const _FDM = "FirewallDeploymentModel";
const _FE = "FirstEntries";
const _FEi = "FirewallEndpoint";
const _FI = "FailedItems";
const _FIL = "FailedItemList";
const _FIa = "FailedItem";
const _FMSCR = "FMSCanRemediate";
const _FMSPUFCCA = "FMSPolicyUpdateFirewallCreationConfigAction";
const _FP = "FromPort";
const _FPI = "FirewallPolicyId";
const _FPN = "FirewallPolicyName";
const _FRFFE = "ForceRemediateForFirstEntries";
const _FRFLE = "ForceRemediateForLastEntries";
const _FSI = "FirewallSubnetId";
const _FSIOOSV = "FirewallSubnetIsOutOfScopeViolation";
const _FSMVPCEV = "FirewallSubnetMissingVPCEndpointViolation";
const _FSR = "FirewallSubnetRoutes";
const _GAA = "GetAdminAccount";
const _GAAR = "GetAdminAccountRequest";
const _GAARe = "GetAdminAccountResponse";
const _GAL = "GetAppsList";
const _GALR = "GetAppsListRequest";
const _GALRe = "GetAppsListResponse";
const _GAS = "GetAdminScope";
const _GASR = "GetAdminScopeRequest";
const _GASRe = "GetAdminScopeResponse";
const _GCD = "GetComplianceDetail";
const _GCDR = "GetComplianceDetailRequest";
const _GCDRe = "GetComplianceDetailResponse";
const _GI = "GatewayId";
const _GNC = "GetNotificationChannel";
const _GNCR = "GetNotificationChannelRequest";
const _GNCRe = "GetNotificationChannelResponse";
const _GP = "GetPolicy";
const _GPL = "GetProtocolsList";
const _GPLR = "GetProtocolsListRequest";
const _GPLRe = "GetProtocolsListResponse";
const _GPR = "GetPolicyRequest";
const _GPRe = "GetPolicyResponse";
const _GPS = "GetProtectionStatus";
const _GPSR = "GetProtectionStatusRequest";
const _GPSRe = "GetProtectionStatusResponse";
const _GRS = "GetResourceSet";
const _GRSR = "GetResourceSetRequest";
const _GRSRe = "GetResourceSetResponse";
const _GTPFAS = "GetThirdPartyFirewallAssociationStatus";
const _GTPFASR = "GetThirdPartyFirewallAssociationStatusRequest";
const _GTPFASRe = "GetThirdPartyFirewallAssociationStatusResponse";
const _GVD = "GetViolationDetails";
const _GVDR = "GetViolationDetailsRequest";
const _GVDRe = "GetViolationDetailsResponse";
const _I = "Items";
const _ICB = "Ipv6CidrBlock";
const _IDA = "IsDefaultAction";
const _IEE = "InternalErrorException";
const _IGI = "InternetGatewayId";
const _IGR = "InternetGatewayRoutes";
const _IIE = "InvalidInputException";
const _IIM = "IssueInfoMap";
const _IM = "IncludeMap";
const _INAEV = "InvalidNetworkAclEntriesViolation";
const _IOE = "InvalidOperationException";
const _IPVR = "IPV4Range";
const _IPVRa = "IPV6Range";
const _IRTUIDAZ = "IsRouteTableUsedInDifferentAZ";
const _ITC = "IcmpTypeCode";
const _ITE = "InvalidTypeException";
const _IVC = "IpV4Cidr";
const _IVCp = "IpV6Cidr";
const _Id = "Identifier";
const _Id_ = "Id";
const _K = "Key";
const _LA = "ListArn";
const _LAAFO = "ListAdminAccountsForOrganization";
const _LAAFOR = "ListAdminAccountsForOrganizationRequest";
const _LAAFORi = "ListAdminAccountsForOrganizationResponse";
const _LAL = "ListAppsLists";
const _LALR = "ListAppsListsRequest";
const _LALRi = "ListAppsListsResponse";
const _LAMA = "ListAdminsManagingAccount";
const _LAMAR = "ListAdminsManagingAccountRequest";
const _LAMARi = "ListAdminsManagingAccountResponse";
const _LCS = "ListComplianceStatus";
const _LCSR = "ListComplianceStatusRequest";
const _LCSRi = "ListComplianceStatusResponse";
const _LDR = "ListDiscoveredResources";
const _LDRR = "ListDiscoveredResourcesRequest";
const _LDRRi = "ListDiscoveredResourcesResponse";
const _LE = "LastEntries";
const _LEE = "LimitExceededException";
const _LI = "ListId";
const _LMA = "ListMemberAccounts";
const _LMAR = "ListMemberAccountsRequest";
const _LMARi = "ListMemberAccountsResponse";
const _LN = "ListName";
const _LP = "ListPolicies";
const _LPL = "ListProtocolsLists";
const _LPLR = "ListProtocolsListsRequest";
const _LPLRi = "ListProtocolsListsResponse";
const _LPR = "ListPoliciesRequest";
const _LPRi = "ListPoliciesResponse";
const _LRS = "ListResourceSets";
const _LRSR = "ListResourceSetsRequest";
const _LRSRR = "ListResourceSetResourcesRequest";
const _LRSRRi = "ListResourceSetResourcesResponse";
const _LRSRi = "ListResourceSetsResponse";
const _LRSRis = "ListResourceSetResources";
const _LTFR = "ListTagsForResource";
const _LTFRR = "ListTagsForResourceRequest";
const _LTFRRi = "ListTagsForResourceResponse";
const _LTPFFP = "ListThirdPartyFirewallFirewallPolicies";
const _LTPFFPR = "ListThirdPartyFirewallFirewallPoliciesRequest";
const _LTPFFPRi = "ListThirdPartyFirewallFirewallPoliciesResponse";
const _LU = "LastUpdated";
const _LUT = "ListUpdateToken";
const _LUTa = "LastUpdateTime";
const _M = "Metadata";
const _MA = "MemberAccount";
const _MAI = "MemberAccountId";
const _MAIe = "MemberAccountIds";
const _MAe = "MemberAccounts";
const _MOS = "MarketplaceOnboardingStatus";
const _MR = "MaxResults";
const _MSD = "ManagedServiceData";
const _Me = "Message";
const _N = "Name";
const _NACP = "NetworkAclCommonPolicy";
const _NAE = "NetworkAclEntry";
const _NAES = "NetworkAclEntrySet";
const _NAETBC = "NetworkAclEntriesToBeCreated";
const _NAETBD = "NetworkAclEntriesToBeDeleted";
const _NAEe = "NetworkAclEntries";
const _NAI = "NetworkAclId";
const _NAITC = "NetworkAclIcmpTypeCode";
const _NAPR = "NetworkAclPortRange";
const _NFBHRDV = "NetworkFirewallBlackHoleRouteDetectedViolation";
const _NFIRCV = "NetworkFirewallInvalidRouteConfigurationViolation";
const _NFITNIV = "NetworkFirewallInternetTrafficNotInspectedViolation";
const _NFMERTV = "NetworkFirewallMissingExpectedRTViolation";
const _NFMERV = "NetworkFirewallMissingExpectedRoutesViolation";
const _NFMFV = "NetworkFirewallMissingFirewallViolation";
const _NFMSV = "NetworkFirewallMissingSubnetViolation";
const _NFP = "NetworkFirewallPolicy";
const _NFPD = "NetworkFirewallPolicyDescription";
const _NFPMV = "NetworkFirewallPolicyModifiedViolation";
const _NFSRGO = "NetworkFirewallStatefulRuleGroupOverride";
const _NFUFRV = "NetworkFirewallUnexpectedFirewallRoutesViolation";
const _NFUGRV = "NetworkFirewallUnexpectedGatewayRoutesViolation";
const _NORGAA = "NumberOfRuleGroupsAlreadyAssociated";
const _NT = "NextToken";
const _O = "Order";
const _OOSRL = "OutOfScopeResourceList";
const _ORA = "OrderedRemediationActions";
const _OU = "OrganizationalUnits";
const _OUS = "OrganizationalUnitScope";
const _Ov = "Override";
const _P = "Protocol";
const _PA = "PolicyArn";
const _PAA = "PutAdminAccount";
const _PAAR = "PutAdminAccountRequest";
const _PAL = "PreviousAppsList";
const _PALR = "PutAppsListRequest";
const _PALRu = "PutAppsListResponse";
const _PALu = "PutAppsList";
const _PCD = "PolicyComplianceDetail";
const _PCS = "PolicyComplianceStatus";
const _PCSL = "PolicyComplianceStatusList";
const _PD = "PolicyDescription";
const _PI = "PolicyId";
const _PL = "ProtocolsList";
const _PLA = "ProtocolsListArn";
const _PLD = "ProtocolsListData";
const _PLDS = "ProtocolsListDataSummary";
const _PLDr = "ProtocolsListsData";
const _PLI = "PrefixListId";
const _PLo = "PolicyList";
const _PLr = "ProtocolsLists";
const _PM = "PartialMatches";
const _PMa = "PartialMatch";
const _PN = "PolicyName";
const _PNC = "PutNotificationChannel";
const _PNCR = "PutNotificationChannelRequest";
const _PO = "PolicyOwner";
const _POo = "PolicyOption";
const _PP = "PutPolicy";
const _PPL = "PreviousProtocolsList";
const _PPLR = "PutProtocolsListRequest";
const _PPLRu = "PutProtocolsListResponse";
const _PPLu = "PutProtocolsList";
const _PPR = "PutPolicyRequest";
const _PPRu = "PutPolicyResponse";
const _PR = "PortRange";
const _PRA = "PossibleRemediationAction";
const _PRAL = "PossibleRemediationActionList";
const _PRAo = "PossibleRemediationActions";
const _PRS = "PutResourceSet";
const _PRSR = "PutResourceSetRequest";
const _PRSRu = "PutResourceSetResponse";
const _PS = "PolicyStatus";
const _PSGRA = "PossibleSecurityGroupRemediationActions";
const _PSL = "PolicySummaryList";
const _PSo = "PolicySummary";
const _PT = "PolicyTypes";
const _PTS = "PolicyTypeScope";
const _PUT = "PolicyUpdateToken";
const _Po = "Port";
const _Pol = "Policy";
const _Pr = "Priority";
const _R = "Reason";
const _RA = "ResourceArn";
const _RAT = "RemediationActionType";
const _RAWO = "RemediationActionWithOrder";
const _RAe = "RemediationAction";
const _RAu = "RuleAction";
const _RD = "ResourceDescription";
const _RE = "RemediationEnabled";
const _RGN = "RuleGroupName";
const _RHOOSEV = "RouteHasOutOfScopeEndpointViolation";
const _RI = "ResourceId";
const _RL = "ResourceList";
const _RNAAA = "ReplaceNetworkAclAssociationAction";
const _RNFE = "ResourceNotFoundException";
const _RO = "RuleOrder";
const _RR = "RemediationResult";
const _RS = "RegionScope";
const _RSA = "ResourceSetArn";
const _RSI = "ResourceSetIdentifier";
const _RSIe = "ResourceSetIds";
const _RSS = "ResourceSetStatus";
const _RSSL = "ResourceSetSummaryList";
const _RSSe = "ResourceSetSummary";
const _RSe = "ResourceSet";
const _RSes = "ResourceSets";
const _RSo = "RoleStatus";
const _RT = "ResourceType";
const _RTI = "RouteTableId";
const _RTL = "ResourceTypeList";
const _RTLO = "ResourceTagLogicalOperator";
const _RTe = "ResourceTags";
const _RTes = "ResourceTag";
const _RV = "ResourceViolation";
const _RVe = "ResourceViolations";
const _Re = "Reference";
const _Reg = "Regions";
const _Res = "Resource";
const _Ro = "Route";
const _Rou = "Routes";
const _S = "Status";
const _SAZ = "SubnetAvailabilityZone";
const _SAZI = "SubnetAvailabilityZoneId";
const _SCA = "StatelessCustomActions";
const _SDA = "StatelessDefaultActions";
const _SDAt = "StatefulDefaultActions";
const _SEO = "StatefulEngineOptions";
const _SEP = "StreamExceptionPolicy";
const _SFDA = "StatelessFragmentDefaultActions";
const _SGRA = "SecurityGroupRemediationAction";
const _SGRAe = "SecurityGroupRemediationActions";
const _SGRD = "SecurityGroupRuleDescription";
const _SI = "SubnetId";
const _SRG = "StatelessRuleGroups";
const _SRGL = "StatefulRuleGroupList";
const _SRGLt = "StatelessRuleGroupList";
const _SRGt = "StatefulRuleGroups";
const _SRGta = "StatefulRuleGroup";
const _SRGtat = "StatelessRuleGroup";
const _SRN = "SnsRoleName";
const _SSPD = "SecurityServicePolicyData";
const _SST = "SecurityServiceType";
const _ST = "StartTime";
const _STA = "SnsTopicArn";
const _STe = "ServiceType";
const _Su = "Subnet";
const _T = "Type";
const _TK = "TagKeys";
const _TL = "TagList";
const _TP = "ToPort";
const _TPF = "ThirdPartyFirewall";
const _TPFFP = "ThirdPartyFirewallFirewallPolicies";
const _TPFFPh = "ThirdPartyFirewallFirewallPolicy";
const _TPFMERTV = "ThirdPartyFirewallMissingExpectedRouteTableViolation";
const _TPFMFV = "ThirdPartyFirewallMissingFirewallViolation";
const _TPFMSV = "ThirdPartyFirewallMissingSubnetViolation";
const _TPFP = "ThirdPartyFirewallPolicy";
const _TPFS = "ThirdPartyFirewallStatus";
const _TR = "TagResource";
const _TRR = "TagResourceRequest";
const _TRRa = "TagResourceResponse";
const _TT = "TargetType";
const _TVR = "TargetViolationReason";
const _TVRa = "TargetViolationReasons";
const _Ta = "Target";
const _Tag = "Tag";
const _To = "To";
const _UP = "UnavailablePriorities";
const _UR = "UntagResource";
const _URI = "URI";
const _URR = "UntagResourceRequest";
const _URRn = "UntagResourceResponse";
const _UT = "UpdateToken";
const _V = "Vpc";
const _VC = "ViolatorCount";
const _VD = "ViolationDetail";
const _VEI = "VpcEndpointId";
const _VI = "VpcId";
const _VPC = "VPC";
const _VR = "ViolationReason";
const _VRi = "ViolatingRoutes";
const _VRio = "ViolatingRoute";
const _VSG = "ViolatingSecurityGroups";
const _VT = "ViolationTarget";
const _VTD = "ViolationTargetDescription";
const _Va = "Value";
const _Vi = "Violators";
const _WACLA = "WebACLArn";
const _WACLHICV = "WebACLHasIncompatibleConfigurationViolation";
const _WACLHOOSRV = "WebACLHasOutOfScopeResourcesViolation";
const _c = "client";
const _e = "error";
const _s = "smithy.ts.sdk.synthetic.com.amazonaws.fms";
const n0 = "com.amazonaws.fms";

// smithy-typescript generated code
import { TypeRegistry } from "@smithy/core/schema";
import type {
  StaticErrorSchema,
  StaticListSchema,
  StaticMapSchema,
  StaticOperationSchema,
  StaticStructureSchema,
} from "@smithy/types";

import {
  InternalErrorException,
  InvalidInputException,
  InvalidOperationException,
  InvalidTypeException,
  LimitExceededException,
  ResourceNotFoundException,
} from "../models/errors";
import { FMSServiceException } from "../models/FMSServiceException";

/* eslint no-var: 0 */
export var AccountScope$: StaticStructureSchema = [3, n0, _AS,
  0,
  [_A, _AAE, _ESA],
  [64 | 0, 2, 2]
];
export var ActionTarget$: StaticStructureSchema = [3, n0, _AT,
  0,
  [_RI, _D],
  [0, 0]
];
export var AdminAccountSummary$: StaticStructureSchema = [3, n0, _AAS,
  0,
  [_AA, _DA, _S],
  [0, 2, 0]
];
export var AdminScope$: StaticStructureSchema = [3, n0, _ASd,
  0,
  [_AS, _OUS, _RS, _PTS],
  [() => AccountScope$, () => OrganizationalUnitScope$, () => RegionScope$, () => PolicyTypeScope$]
];
export var App$: StaticStructureSchema = [3, n0, _Ap,
  0,
  [_AN, _P, _Po],
  [0, 0, 1]
];
export var AppsListData$: StaticStructureSchema = [3, n0, _ALD,
  0,
  [_LI, _LN, _LUT, _CT, _LUTa, _AL, _PAL],
  [0, 0, 0, 4, 4, () => AppsList, () => PreviousAppsList]
];
export var AppsListDataSummary$: StaticStructureSchema = [3, n0, _ALDS,
  0,
  [_LA, _LI, _LN, _AL],
  [0, 0, 0, () => AppsList]
];
export var AssociateAdminAccountRequest$: StaticStructureSchema = [3, n0, _AAAR,
  0,
  [_AA],
  [0]
];
export var AssociateThirdPartyFirewallRequest$: StaticStructureSchema = [3, n0, _ATPFR,
  0,
  [_TPF],
  [0]
];
export var AssociateThirdPartyFirewallResponse$: StaticStructureSchema = [3, n0, _ATPFRs,
  0,
  [_TPFS],
  [0]
];
export var AwsEc2InstanceViolation$: StaticStructureSchema = [3, n0, _AEIV,
  0,
  [_VT, _AENIV],
  [0, () => AwsEc2NetworkInterfaceViolations]
];
export var AwsEc2NetworkInterfaceViolation$: StaticStructureSchema = [3, n0, _AENIVw,
  0,
  [_VT, _VSG],
  [0, 64 | 0]
];
export var AwsVPCSecurityGroupViolation$: StaticStructureSchema = [3, n0, _AVPCSGV,
  0,
  [_VT, _VTD, _PM, _PSGRA],
  [0, 0, () => PartialMatches, () => SecurityGroupRemediationActions]
];
export var BatchAssociateResourceRequest$: StaticStructureSchema = [3, n0, _BARR,
  0,
  [_RSI, _I],
  [0, 64 | 0]
];
export var BatchAssociateResourceResponse$: StaticStructureSchema = [3, n0, _BARRa,
  0,
  [_RSI, _FI],
  [0, () => FailedItemList]
];
export var BatchDisassociateResourceRequest$: StaticStructureSchema = [3, n0, _BDRR,
  0,
  [_RSI, _I],
  [0, 64 | 0]
];
export var BatchDisassociateResourceResponse$: StaticStructureSchema = [3, n0, _BDRRa,
  0,
  [_RSI, _FI],
  [0, () => FailedItemList]
];
export var ComplianceViolator$: StaticStructureSchema = [3, n0, _CV,
  0,
  [_RI, _VR, _RT, _M],
  [0, 0, 0, 128 | 0]
];
export var CreateNetworkAclAction$: StaticStructureSchema = [3, n0, _CNAA,
  0,
  [_D, _V, _FMSCR],
  [0, () => ActionTarget$, 2]
];
export var CreateNetworkAclEntriesAction$: StaticStructureSchema = [3, n0, _CNAEA,
  0,
  [_D, _NAI, _NAETBC, _FMSCR],
  [0, () => ActionTarget$, () => EntriesDescription, 2]
];
export var DeleteAppsListRequest$: StaticStructureSchema = [3, n0, _DALR,
  0,
  [_LI],
  [0]
];
export var DeleteNetworkAclEntriesAction$: StaticStructureSchema = [3, n0, _DNAEA,
  0,
  [_D, _NAI, _NAETBD, _FMSCR],
  [0, () => ActionTarget$, () => EntriesDescription, 2]
];
export var DeleteNotificationChannelRequest$: StaticStructureSchema = [3, n0, _DNCR,
  0,
  [],
  []
];
export var DeletePolicyRequest$: StaticStructureSchema = [3, n0, _DPR,
  0,
  [_PI, _DAPR],
  [0, 2]
];
export var DeleteProtocolsListRequest$: StaticStructureSchema = [3, n0, _DPLR,
  0,
  [_LI],
  [0]
];
export var DeleteResourceSetRequest$: StaticStructureSchema = [3, n0, _DRSR,
  0,
  [_Id],
  [0]
];
export var DisassociateAdminAccountRequest$: StaticStructureSchema = [3, n0, _DAAR,
  0,
  [],
  []
];
export var DisassociateThirdPartyFirewallRequest$: StaticStructureSchema = [3, n0, _DTPFR,
  0,
  [_TPF],
  [0]
];
export var DisassociateThirdPartyFirewallResponse$: StaticStructureSchema = [3, n0, _DTPFRi,
  0,
  [_TPFS],
  [0]
];
export var DiscoveredResource$: StaticStructureSchema = [3, n0, _DR,
  0,
  [_URI, _AI, _T, _N],
  [0, 0, 0, 0]
];
export var DnsDuplicateRuleGroupViolation$: StaticStructureSchema = [3, n0, _DDRGV,
  0,
  [_VT, _VTD],
  [0, 0]
];
export var DnsRuleGroupLimitExceededViolation$: StaticStructureSchema = [3, n0, _DRGLEV,
  0,
  [_VT, _VTD, _NORGAA],
  [0, 0, 1]
];
export var DnsRuleGroupPriorityConflictViolation$: StaticStructureSchema = [3, n0, _DRGPCV,
  0,
  [_VT, _VTD, _CP, _CPI, _UP],
  [0, 0, 1, 0, 64 | 1]
];
export var EC2AssociateRouteTableAction$: StaticStructureSchema = [3, n0, _ECARTA,
  0,
  [_D, _RTI, _SI, _GI],
  [0, () => ActionTarget$, () => ActionTarget$, () => ActionTarget$]
];
export var EC2CopyRouteTableAction$: StaticStructureSchema = [3, n0, _ECCRTA,
  0,
  [_D, _VI, _RTI],
  [0, () => ActionTarget$, () => ActionTarget$]
];
export var EC2CreateRouteAction$: StaticStructureSchema = [3, n0, _ECCRA,
  0,
  [_D, _DCB, _DPLI, _DICB, _VEI, _GI, _RTI],
  [0, 0, 0, 0, () => ActionTarget$, () => ActionTarget$, () => ActionTarget$]
];
export var EC2CreateRouteTableAction$: StaticStructureSchema = [3, n0, _ECCRTAr,
  0,
  [_D, _VI],
  [0, () => ActionTarget$]
];
export var EC2DeleteRouteAction$: StaticStructureSchema = [3, n0, _ECDRA,
  0,
  [_D, _DCB, _DPLI, _DICB, _RTI],
  [0, 0, 0, 0, () => ActionTarget$]
];
export var EC2ReplaceRouteAction$: StaticStructureSchema = [3, n0, _ECRRA,
  0,
  [_D, _DCB, _DPLI, _DICB, _GI, _RTI],
  [0, 0, 0, 0, () => ActionTarget$, () => ActionTarget$]
];
export var EC2ReplaceRouteTableAssociationAction$: StaticStructureSchema = [3, n0, _ECRRTAA,
  0,
  [_D, _AIs, _RTI],
  [0, () => ActionTarget$, () => ActionTarget$]
];
export var EntryDescription$: StaticStructureSchema = [3, n0, _ED,
  0,
  [_EDn, _ERN, _ET],
  [() => NetworkAclEntry$, 1, 0]
];
export var EntryViolation$: StaticStructureSchema = [3, n0, _EV,
  0,
  [_EE, _EEO, _AEO, _EAEEO, _EWC, _EVR],
  [() => EntryDescription$, 0, 0, () => EntryDescription$, () => EntriesWithConflicts, 64 | 0]
];
export var EvaluationResult$: StaticStructureSchema = [3, n0, _ER,
  0,
  [_CS, _VC, _ELE],
  [0, 1, 2]
];
export var ExpectedRoute$: StaticStructureSchema = [3, n0, _ERx,
  0,
  [_IVC, _PLI, _IVCp, _CSo, _ATl, _RTI],
  [0, 0, 0, 64 | 0, 64 | 0, 0]
];
export var FailedItem$: StaticStructureSchema = [3, n0, _FIa,
  0,
  [_URI, _R],
  [0, 0]
];
export var FirewallSubnetIsOutOfScopeViolation$: StaticStructureSchema = [3, n0, _FSIOOSV,
  0,
  [_FSI, _VI, _SAZ, _SAZI, _VEI],
  [0, 0, 0, 0, 0]
];
export var FirewallSubnetMissingVPCEndpointViolation$: StaticStructureSchema = [3, n0, _FSMVPCEV,
  0,
  [_FSI, _VI, _SAZ, _SAZI],
  [0, 0, 0, 0]
];
export var FMSPolicyUpdateFirewallCreationConfigAction$: StaticStructureSchema = [3, n0, _FMSPUFCCA,
  0,
  [_D, _FCC],
  [0, 0]
];
export var GetAdminAccountRequest$: StaticStructureSchema = [3, n0, _GAAR,
  0,
  [],
  []
];
export var GetAdminAccountResponse$: StaticStructureSchema = [3, n0, _GAARe,
  0,
  [_AA, _RSo],
  [0, 0]
];
export var GetAdminScopeRequest$: StaticStructureSchema = [3, n0, _GASR,
  0,
  [_AA],
  [0]
];
export var GetAdminScopeResponse$: StaticStructureSchema = [3, n0, _GASRe,
  0,
  [_ASd, _S],
  [() => AdminScope$, 0]
];
export var GetAppsListRequest$: StaticStructureSchema = [3, n0, _GALR,
  0,
  [_LI, _DL],
  [0, 2]
];
export var GetAppsListResponse$: StaticStructureSchema = [3, n0, _GALRe,
  0,
  [_AL, _ALA],
  [() => AppsListData$, 0]
];
export var GetComplianceDetailRequest$: StaticStructureSchema = [3, n0, _GCDR,
  0,
  [_PI, _MA],
  [0, 0]
];
export var GetComplianceDetailResponse$: StaticStructureSchema = [3, n0, _GCDRe,
  0,
  [_PCD],
  [() => PolicyComplianceDetail$]
];
export var GetNotificationChannelRequest$: StaticStructureSchema = [3, n0, _GNCR,
  0,
  [],
  []
];
export var GetNotificationChannelResponse$: StaticStructureSchema = [3, n0, _GNCRe,
  0,
  [_STA, _SRN],
  [0, 0]
];
export var GetPolicyRequest$: StaticStructureSchema = [3, n0, _GPR,
  0,
  [_PI],
  [0]
];
export var GetPolicyResponse$: StaticStructureSchema = [3, n0, _GPRe,
  0,
  [_Pol, _PA],
  [() => Policy$, 0]
];
export var GetProtectionStatusRequest$: StaticStructureSchema = [3, n0, _GPSR,
  0,
  [_PI, _MAI, _ST, _ETn, _NT, _MR],
  [0, 0, 4, 4, 0, 1]
];
export var GetProtectionStatusResponse$: StaticStructureSchema = [3, n0, _GPSRe,
  0,
  [_AAI, _STe, _Da, _NT],
  [0, 0, 0, 0]
];
export var GetProtocolsListRequest$: StaticStructureSchema = [3, n0, _GPLR,
  0,
  [_LI, _DL],
  [0, 2]
];
export var GetProtocolsListResponse$: StaticStructureSchema = [3, n0, _GPLRe,
  0,
  [_PL, _PLA],
  [() => ProtocolsListData$, 0]
];
export var GetResourceSetRequest$: StaticStructureSchema = [3, n0, _GRSR,
  0,
  [_Id],
  [0]
];
export var GetResourceSetResponse$: StaticStructureSchema = [3, n0, _GRSRe,
  0,
  [_RSe, _RSA],
  [() => ResourceSet$, 0]
];
export var GetThirdPartyFirewallAssociationStatusRequest$: StaticStructureSchema = [3, n0, _GTPFASR,
  0,
  [_TPF],
  [0]
];
export var GetThirdPartyFirewallAssociationStatusResponse$: StaticStructureSchema = [3, n0, _GTPFASRe,
  0,
  [_TPFS, _MOS],
  [0, 0]
];
export var GetViolationDetailsRequest$: StaticStructureSchema = [3, n0, _GVDR,
  0,
  [_PI, _MA, _RI, _RT],
  [0, 0, 0, 0]
];
export var GetViolationDetailsResponse$: StaticStructureSchema = [3, n0, _GVDRe,
  0,
  [_VD],
  [() => ViolationDetail$]
];
export var InternalErrorException$: StaticErrorSchema = [-3, n0, _IEE,
  { [_e]: _c },
  [_Me],
  [0]
];
TypeRegistry.for(n0).registerError(InternalErrorException$, InternalErrorException);
export var InvalidInputException$: StaticErrorSchema = [-3, n0, _IIE,
  { [_e]: _c },
  [_Me],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidInputException$, InvalidInputException);
export var InvalidNetworkAclEntriesViolation$: StaticStructureSchema = [3, n0, _INAEV,
  0,
  [_V, _Su, _SAZ, _CANA, _EVn],
  [0, 0, 0, 0, () => EntryViolations]
];
export var InvalidOperationException$: StaticErrorSchema = [-3, n0, _IOE,
  { [_e]: _c },
  [_Me],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidOperationException$, InvalidOperationException);
export var InvalidTypeException$: StaticErrorSchema = [-3, n0, _ITE,
  { [_e]: _c },
  [_Me],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidTypeException$, InvalidTypeException);
export var LimitExceededException$: StaticErrorSchema = [-3, n0, _LEE,
  { [_e]: _c },
  [_Me],
  [0]
];
TypeRegistry.for(n0).registerError(LimitExceededException$, LimitExceededException);
export var ListAdminAccountsForOrganizationRequest$: StaticStructureSchema = [3, n0, _LAAFOR,
  0,
  [_NT, _MR],
  [0, 1]
];
export var ListAdminAccountsForOrganizationResponse$: StaticStructureSchema = [3, n0, _LAAFORi,
  0,
  [_AAd, _NT],
  [() => AdminAccountSummaryList, 0]
];
export var ListAdminsManagingAccountRequest$: StaticStructureSchema = [3, n0, _LAMAR,
  0,
  [_NT, _MR],
  [0, 1]
];
export var ListAdminsManagingAccountResponse$: StaticStructureSchema = [3, n0, _LAMARi,
  0,
  [_AAd, _NT],
  [64 | 0, 0]
];
export var ListAppsListsRequest$: StaticStructureSchema = [3, n0, _LALR,
  0,
  [_DLe, _NT, _MR],
  [2, 0, 1]
];
export var ListAppsListsResponse$: StaticStructureSchema = [3, n0, _LALRi,
  0,
  [_ALp, _NT],
  [() => AppsListsData, 0]
];
export var ListComplianceStatusRequest$: StaticStructureSchema = [3, n0, _LCSR,
  0,
  [_PI, _NT, _MR],
  [0, 0, 1]
];
export var ListComplianceStatusResponse$: StaticStructureSchema = [3, n0, _LCSRi,
  0,
  [_PCSL, _NT],
  [() => PolicyComplianceStatusList, 0]
];
export var ListDiscoveredResourcesRequest$: StaticStructureSchema = [3, n0, _LDRR,
  0,
  [_MAIe, _RT, _MR, _NT],
  [64 | 0, 0, 1, 0]
];
export var ListDiscoveredResourcesResponse$: StaticStructureSchema = [3, n0, _LDRRi,
  0,
  [_I, _NT],
  [() => DiscoveredResourceList, 0]
];
export var ListMemberAccountsRequest$: StaticStructureSchema = [3, n0, _LMAR,
  0,
  [_NT, _MR],
  [0, 1]
];
export var ListMemberAccountsResponse$: StaticStructureSchema = [3, n0, _LMARi,
  0,
  [_MAe, _NT],
  [64 | 0, 0]
];
export var ListPoliciesRequest$: StaticStructureSchema = [3, n0, _LPR,
  0,
  [_NT, _MR],
  [0, 1]
];
export var ListPoliciesResponse$: StaticStructureSchema = [3, n0, _LPRi,
  0,
  [_PLo, _NT],
  [() => PolicySummaryList, 0]
];
export var ListProtocolsListsRequest$: StaticStructureSchema = [3, n0, _LPLR,
  0,
  [_DLe, _NT, _MR],
  [2, 0, 1]
];
export var ListProtocolsListsResponse$: StaticStructureSchema = [3, n0, _LPLRi,
  0,
  [_PLr, _NT],
  [() => ProtocolsListsData, 0]
];
export var ListResourceSetResourcesRequest$: StaticStructureSchema = [3, n0, _LRSRR,
  0,
  [_Id, _MR, _NT],
  [0, 1, 0]
];
export var ListResourceSetResourcesResponse$: StaticStructureSchema = [3, n0, _LRSRRi,
  0,
  [_I, _NT],
  [() => ResourceList, 0]
];
export var ListResourceSetsRequest$: StaticStructureSchema = [3, n0, _LRSR,
  0,
  [_NT, _MR],
  [0, 1]
];
export var ListResourceSetsResponse$: StaticStructureSchema = [3, n0, _LRSRi,
  0,
  [_RSes, _NT],
  [() => ResourceSetSummaryList, 0]
];
export var ListTagsForResourceRequest$: StaticStructureSchema = [3, n0, _LTFRR,
  0,
  [_RA],
  [0]
];
export var ListTagsForResourceResponse$: StaticStructureSchema = [3, n0, _LTFRRi,
  0,
  [_TL],
  [() => TagList]
];
export var ListThirdPartyFirewallFirewallPoliciesRequest$: StaticStructureSchema = [3, n0, _LTPFFPR,
  0,
  [_TPF, _NT, _MR],
  [0, 0, 1]
];
export var ListThirdPartyFirewallFirewallPoliciesResponse$: StaticStructureSchema = [3, n0, _LTPFFPRi,
  0,
  [_TPFFP, _NT],
  [() => ThirdPartyFirewallFirewallPolicies, 0]
];
export var NetworkAclCommonPolicy$: StaticStructureSchema = [3, n0, _NACP,
  0,
  [_NAES],
  [() => NetworkAclEntrySet$]
];
export var NetworkAclEntry$: StaticStructureSchema = [3, n0, _NAE,
  0,
  [_ITC, _P, _PR, _CB, _ICB, _RAu, _E],
  [() => NetworkAclIcmpTypeCode$, 0, () => NetworkAclPortRange$, 0, 0, 0, 2]
];
export var NetworkAclEntrySet$: StaticStructureSchema = [3, n0, _NAES,
  0,
  [_FE, _FRFFE, _LE, _FRFLE],
  [() => NetworkAclEntries, 2, () => NetworkAclEntries, 2]
];
export var NetworkAclIcmpTypeCode$: StaticStructureSchema = [3, n0, _NAITC,
  0,
  [_C, _T],
  [1, 1]
];
export var NetworkAclPortRange$: StaticStructureSchema = [3, n0, _NAPR,
  0,
  [_F, _To],
  [1, 1]
];
export var NetworkFirewallBlackHoleRouteDetectedViolation$: StaticStructureSchema = [3, n0, _NFBHRDV,
  0,
  [_VT, _RTI, _VI, _VRi],
  [0, 0, 0, () => Routes]
];
export var NetworkFirewallInternetTrafficNotInspectedViolation$: StaticStructureSchema = [3, n0, _NFITNIV,
  0,
  [_SI, _SAZ, _RTI, _VRi, _IRTUIDAZ, _CFSRT, _EFE, _FSI, _EFSR, _AFSR, _IGI, _CIGRT, _EIGR, _AIGR, _VI],
  [0, 0, 0, () => Routes, 2, 0, 0, 0, () => ExpectedRoutes, () => Routes, 0, 0, () => ExpectedRoutes, () => Routes, 0]
];
export var NetworkFirewallInvalidRouteConfigurationViolation$: StaticStructureSchema = [3, n0, _NFIRCV,
  0,
  [_ASf, _RTI, _IRTUIDAZ, _VRio, _CFSRT, _EFE, _AFE, _EFSI, _AFSI, _EFSR, _AFSR, _IGI, _CIGRT, _EIGR, _AIGR, _VI],
  [64 | 0, 0, 2, () => Route$, 0, 0, 0, 0, 0, () => ExpectedRoutes, () => Routes, 0, 0, () => ExpectedRoutes, () => Routes, 0]
];
export var NetworkFirewallMissingExpectedRoutesViolation$: StaticStructureSchema = [3, n0, _NFMERV,
  0,
  [_VT, _ERxp, _VI],
  [0, () => ExpectedRoutes, 0]
];
export var NetworkFirewallMissingExpectedRTViolation$: StaticStructureSchema = [3, n0, _NFMERTV,
  0,
  [_VT, _VPC, _AZ, _CRT, _ERT],
  [0, 0, 0, 0, 0]
];
export var NetworkFirewallMissingFirewallViolation$: StaticStructureSchema = [3, n0, _NFMFV,
  0,
  [_VT, _VPC, _AZ, _TVR],
  [0, 0, 0, 0]
];
export var NetworkFirewallMissingSubnetViolation$: StaticStructureSchema = [3, n0, _NFMSV,
  0,
  [_VT, _VPC, _AZ, _TVR],
  [0, 0, 0, 0]
];
export var NetworkFirewallPolicy$: StaticStructureSchema = [3, n0, _NFP,
  0,
  [_FDM],
  [0]
];
export var NetworkFirewallPolicyDescription$: StaticStructureSchema = [3, n0, _NFPD,
  0,
  [_SRG, _SDA, _SFDA, _SCA, _SRGt, _SDAt, _SEO],
  [() => StatelessRuleGroupList, 64 | 0, 64 | 0, 64 | 0, () => StatefulRuleGroupList, 64 | 0, () => StatefulEngineOptions$]
];
export var NetworkFirewallPolicyModifiedViolation$: StaticStructureSchema = [3, n0, _NFPMV,
  0,
  [_VT, _CPD, _EPD],
  [0, () => NetworkFirewallPolicyDescription$, () => NetworkFirewallPolicyDescription$]
];
export var NetworkFirewallStatefulRuleGroupOverride$: StaticStructureSchema = [3, n0, _NFSRGO,
  0,
  [_Ac],
  [0]
];
export var NetworkFirewallUnexpectedFirewallRoutesViolation$: StaticStructureSchema = [3, n0, _NFUFRV,
  0,
  [_FSI, _VRi, _RTI, _FEi, _VI],
  [0, () => Routes, 0, 0, 0]
];
export var NetworkFirewallUnexpectedGatewayRoutesViolation$: StaticStructureSchema = [3, n0, _NFUGRV,
  0,
  [_GI, _VRi, _RTI, _VI],
  [0, () => Routes, 0, 0]
];
export var OrganizationalUnitScope$: StaticStructureSchema = [3, n0, _OUS,
  0,
  [_OU, _AOUE, _ESOU],
  [64 | 0, 2, 2]
];
export var PartialMatch$: StaticStructureSchema = [3, n0, _PMa,
  0,
  [_Re, _TVRa],
  [0, 64 | 0]
];
export var Policy$: StaticStructureSchema = [3, n0, _Pol,
  0,
  [_PI, _PN, _PUT, _SSPD, _RT, _RTL, _RTe, _ERTx, _RE, _DUFMMR, _IM, _EM, _RSIe, _PD, _PS, _RTLO],
  [0, 0, 0, () => SecurityServicePolicyData$, 0, 64 | 0, () => ResourceTags, 2, 2, 2, [2, n0, _CPSM, 0, 0, 64 | 0], [2, n0, _CPSM, 0, 0, 64 | 0], 64 | 0, 0, 0, 0]
];
export var PolicyComplianceDetail$: StaticStructureSchema = [3, n0, _PCD,
  0,
  [_PO, _PI, _MA, _Vi, _ELE, _EA, _IIM],
  [0, 0, 0, () => ComplianceViolators, 2, 4, 128 | 0]
];
export var PolicyComplianceStatus$: StaticStructureSchema = [3, n0, _PCS,
  0,
  [_PO, _PI, _PN, _MA, _ERv, _LU, _IIM],
  [0, 0, 0, 0, () => EvaluationResults, 4, 128 | 0]
];
export var PolicyOption$: StaticStructureSchema = [3, n0, _POo,
  0,
  [_NFP, _TPFP, _NACP],
  [() => NetworkFirewallPolicy$, () => ThirdPartyFirewallPolicy$, () => NetworkAclCommonPolicy$]
];
export var PolicySummary$: StaticStructureSchema = [3, n0, _PSo,
  0,
  [_PA, _PI, _PN, _RT, _SST, _RE, _DUFMMR, _PS],
  [0, 0, 0, 0, 0, 2, 2, 0]
];
export var PolicyTypeScope$: StaticStructureSchema = [3, n0, _PTS,
  0,
  [_PT, _APTE],
  [64 | 0, 2]
];
export var PossibleRemediationAction$: StaticStructureSchema = [3, n0, _PRA,
  0,
  [_D, _ORA, _IDA],
  [0, () => OrderedRemediationActions, 2]
];
export var PossibleRemediationActions$: StaticStructureSchema = [3, n0, _PRAo,
  0,
  [_D, _Act],
  [0, () => PossibleRemediationActionList]
];
export var ProtocolsListData$: StaticStructureSchema = [3, n0, _PLD,
  0,
  [_LI, _LN, _LUT, _CT, _LUTa, _PL, _PPL],
  [0, 0, 0, 4, 4, 64 | 0, [2, n0, _PPL, 0, 0, 64 | 0]]
];
export var ProtocolsListDataSummary$: StaticStructureSchema = [3, n0, _PLDS,
  0,
  [_LA, _LI, _LN, _PL],
  [0, 0, 0, 64 | 0]
];
export var PutAdminAccountRequest$: StaticStructureSchema = [3, n0, _PAAR,
  0,
  [_AA, _ASd],
  [0, () => AdminScope$]
];
export var PutAppsListRequest$: StaticStructureSchema = [3, n0, _PALR,
  0,
  [_AL, _TL],
  [() => AppsListData$, () => TagList]
];
export var PutAppsListResponse$: StaticStructureSchema = [3, n0, _PALRu,
  0,
  [_AL, _ALA],
  [() => AppsListData$, 0]
];
export var PutNotificationChannelRequest$: StaticStructureSchema = [3, n0, _PNCR,
  0,
  [_STA, _SRN],
  [0, 0]
];
export var PutPolicyRequest$: StaticStructureSchema = [3, n0, _PPR,
  0,
  [_Pol, _TL],
  [() => Policy$, () => TagList]
];
export var PutPolicyResponse$: StaticStructureSchema = [3, n0, _PPRu,
  0,
  [_Pol, _PA],
  [() => Policy$, 0]
];
export var PutProtocolsListRequest$: StaticStructureSchema = [3, n0, _PPLR,
  0,
  [_PL, _TL],
  [() => ProtocolsListData$, () => TagList]
];
export var PutProtocolsListResponse$: StaticStructureSchema = [3, n0, _PPLRu,
  0,
  [_PL, _PLA],
  [() => ProtocolsListData$, 0]
];
export var PutResourceSetRequest$: StaticStructureSchema = [3, n0, _PRSR,
  0,
  [_RSe, _TL],
  [() => ResourceSet$, () => TagList]
];
export var PutResourceSetResponse$: StaticStructureSchema = [3, n0, _PRSRu,
  0,
  [_RSe, _RSA],
  [() => ResourceSet$, 0]
];
export var RegionScope$: StaticStructureSchema = [3, n0, _RS,
  0,
  [_Reg, _ARE],
  [64 | 0, 2]
];
export var RemediationAction$: StaticStructureSchema = [3, n0, _RAe,
  0,
  [_D, _ECCRA, _ECRRA, _ECDRA, _ECCRTA, _ECRRTAA, _ECARTA, _ECCRTAr, _FMSPUFCCA, _CNAA, _RNAAA, _CNAEA, _DNAEA],
  [0, () => EC2CreateRouteAction$, () => EC2ReplaceRouteAction$, () => EC2DeleteRouteAction$, () => EC2CopyRouteTableAction$, () => EC2ReplaceRouteTableAssociationAction$, () => EC2AssociateRouteTableAction$, () => EC2CreateRouteTableAction$, () => FMSPolicyUpdateFirewallCreationConfigAction$, () => CreateNetworkAclAction$, () => ReplaceNetworkAclAssociationAction$, () => CreateNetworkAclEntriesAction$, () => DeleteNetworkAclEntriesAction$]
];
export var RemediationActionWithOrder$: StaticStructureSchema = [3, n0, _RAWO,
  0,
  [_RAe, _O],
  [() => RemediationAction$, 1]
];
export var ReplaceNetworkAclAssociationAction$: StaticStructureSchema = [3, n0, _RNAAA,
  0,
  [_D, _AIs, _NAI, _FMSCR],
  [0, () => ActionTarget$, () => ActionTarget$, 2]
];
export var Resource$: StaticStructureSchema = [3, n0, _Res,
  0,
  [_URI, _AI],
  [0, 0]
];
export var ResourceNotFoundException$: StaticErrorSchema = [-3, n0, _RNFE,
  { [_e]: _c },
  [_Me],
  [0]
];
TypeRegistry.for(n0).registerError(ResourceNotFoundException$, ResourceNotFoundException);
export var ResourceSet$: StaticStructureSchema = [3, n0, _RSe,
  0,
  [_Id_, _N, _D, _UT, _RTL, _LUTa, _RSS],
  [0, 0, 0, 0, 64 | 0, 4, 0]
];
export var ResourceSetSummary$: StaticStructureSchema = [3, n0, _RSSe,
  0,
  [_Id_, _N, _D, _LUTa, _RSS],
  [0, 0, 0, 4, 0]
];
export var ResourceTag$: StaticStructureSchema = [3, n0, _RTes,
  0,
  [_K, _Va],
  [0, 0]
];
export var ResourceViolation$: StaticStructureSchema = [3, n0, _RV,
  0,
  [_AVPCSGV, _AENIVw, _AEIV, _NFMFV, _NFMSV, _NFMERTV, _NFPMV, _NFITNIV, _NFIRCV, _NFBHRDV, _NFUFRV, _NFUGRV, _NFMERV, _DRGPCV, _DDRGV, _DRGLEV, _FSIOOSV, _RHOOSEV, _TPFMFV, _TPFMSV, _TPFMERTV, _FSMVPCEV, _INAEV, _PRAo, _WACLHICV, _WACLHOOSRV],
  [() => AwsVPCSecurityGroupViolation$, () => AwsEc2NetworkInterfaceViolation$, () => AwsEc2InstanceViolation$, () => NetworkFirewallMissingFirewallViolation$, () => NetworkFirewallMissingSubnetViolation$, () => NetworkFirewallMissingExpectedRTViolation$, () => NetworkFirewallPolicyModifiedViolation$, () => NetworkFirewallInternetTrafficNotInspectedViolation$, () => NetworkFirewallInvalidRouteConfigurationViolation$, () => NetworkFirewallBlackHoleRouteDetectedViolation$, () => NetworkFirewallUnexpectedFirewallRoutesViolation$, () => NetworkFirewallUnexpectedGatewayRoutesViolation$, () => NetworkFirewallMissingExpectedRoutesViolation$, () => DnsRuleGroupPriorityConflictViolation$, () => DnsDuplicateRuleGroupViolation$, () => DnsRuleGroupLimitExceededViolation$, () => FirewallSubnetIsOutOfScopeViolation$, () => RouteHasOutOfScopeEndpointViolation$, () => ThirdPartyFirewallMissingFirewallViolation$, () => ThirdPartyFirewallMissingSubnetViolation$, () => ThirdPartyFirewallMissingExpectedRouteTableViolation$, () => FirewallSubnetMissingVPCEndpointViolation$, () => InvalidNetworkAclEntriesViolation$, () => PossibleRemediationActions$, () => WebACLHasIncompatibleConfigurationViolation$, () => WebACLHasOutOfScopeResourcesViolation$]
];
export var Route$: StaticStructureSchema = [3, n0, _Ro,
  0,
  [_DT, _TT, _De, _Ta],
  [0, 0, 0, 0]
];
export var RouteHasOutOfScopeEndpointViolation$: StaticStructureSchema = [3, n0, _RHOOSEV,
  0,
  [_SI, _VI, _RTI, _VRi, _SAZ, _SAZI, _CFSRT, _FSI, _FSR, _IGI, _CIGRT, _IGR],
  [0, 0, 0, () => Routes, 0, 0, 0, 0, () => Routes, 0, 0, () => Routes]
];
export var SecurityGroupRemediationAction$: StaticStructureSchema = [3, n0, _SGRA,
  0,
  [_RAT, _D, _RR, _IDA],
  [0, 0, () => SecurityGroupRuleDescription$, 2]
];
export var SecurityGroupRuleDescription$: StaticStructureSchema = [3, n0, _SGRD,
  0,
  [_IPVR, _IPVRa, _PLI, _P, _FP, _TP],
  [0, 0, 0, 0, 1, 1]
];
export var SecurityServicePolicyData$: StaticStructureSchema = [3, n0, _SSPD,
  0,
  [_T, _MSD, _POo],
  [0, 0, () => PolicyOption$]
];
export var StatefulEngineOptions$: StaticStructureSchema = [3, n0, _SEO,
  0,
  [_RO, _SEP],
  [0, 0]
];
export var StatefulRuleGroup$: StaticStructureSchema = [3, n0, _SRGta,
  0,
  [_RGN, _RI, _Pr, _Ov],
  [0, 0, 1, () => NetworkFirewallStatefulRuleGroupOverride$]
];
export var StatelessRuleGroup$: StaticStructureSchema = [3, n0, _SRGtat,
  0,
  [_RGN, _RI, _Pr],
  [0, 0, 1]
];
export var Tag$: StaticStructureSchema = [3, n0, _Tag,
  0,
  [_K, _Va],
  [0, 0]
];
export var TagResourceRequest$: StaticStructureSchema = [3, n0, _TRR,
  0,
  [_RA, _TL],
  [0, () => TagList]
];
export var TagResourceResponse$: StaticStructureSchema = [3, n0, _TRRa,
  0,
  [],
  []
];
export var ThirdPartyFirewallFirewallPolicy$: StaticStructureSchema = [3, n0, _TPFFPh,
  0,
  [_FPI, _FPN],
  [0, 0]
];
export var ThirdPartyFirewallMissingExpectedRouteTableViolation$: StaticStructureSchema = [3, n0, _TPFMERTV,
  0,
  [_VT, _VPC, _AZ, _CRT, _ERT],
  [0, 0, 0, 0, 0]
];
export var ThirdPartyFirewallMissingFirewallViolation$: StaticStructureSchema = [3, n0, _TPFMFV,
  0,
  [_VT, _VPC, _AZ, _TVR],
  [0, 0, 0, 0]
];
export var ThirdPartyFirewallMissingSubnetViolation$: StaticStructureSchema = [3, n0, _TPFMSV,
  0,
  [_VT, _VPC, _AZ, _TVR],
  [0, 0, 0, 0]
];
export var ThirdPartyFirewallPolicy$: StaticStructureSchema = [3, n0, _TPFP,
  0,
  [_FDM],
  [0]
];
export var UntagResourceRequest$: StaticStructureSchema = [3, n0, _URR,
  0,
  [_RA, _TK],
  [0, 64 | 0]
];
export var UntagResourceResponse$: StaticStructureSchema = [3, n0, _URRn,
  0,
  [],
  []
];
export var ViolationDetail$: StaticStructureSchema = [3, n0, _VD,
  0,
  [_PI, _MA, _RI, _RT, _RVe, _RTe, _RD],
  [0, 0, 0, 0, () => ResourceViolations, () => TagList, 0]
];
export var WebACLHasIncompatibleConfigurationViolation$: StaticStructureSchema = [3, n0, _WACLHICV,
  0,
  [_WACLA, _D],
  [0, 0]
];
export var WebACLHasOutOfScopeResourcesViolation$: StaticStructureSchema = [3, n0, _WACLHOOSRV,
  0,
  [_WACLA, _OOSRL],
  [0, 64 | 0]
];
var __Unit = "unit" as const;
export var FMSServiceException$: StaticErrorSchema = [-3, _s, "FMSServiceException", 0, [], []];
TypeRegistry.for(_s).registerError(FMSServiceException$, FMSServiceException);
var AccountIdList = 64 | 0;
var AdminAccountSummaryList: StaticListSchema = [1, n0, _AASL,
  0, () => AdminAccountSummary$
];
var AppsList: StaticListSchema = [1, n0, _AL,
  0, () => App$
];
var AppsListsData: StaticListSchema = [1, n0, _ALDp,
  0, () => AppsListDataSummary$
];
var AWSAccountIdList = 64 | 0;
var AwsEc2NetworkInterfaceViolations: StaticListSchema = [1, n0, _AENIV,
  0, () => AwsEc2NetworkInterfaceViolation$
];
var AWSRegionList = 64 | 0;
var ComplianceViolators: StaticListSchema = [1, n0, _CVo,
  0, () => ComplianceViolator$
];
var CustomerPolicyScopeIdList = 64 | 0;
var DiscoveredResourceList: StaticListSchema = [1, n0, _DRL,
  0, () => DiscoveredResource$
];
var DnsRuleGroupPriorities = 64 | 1;
var EntriesDescription: StaticListSchema = [1, n0, _EDnt,
  0, () => EntryDescription$
];
var EntriesWithConflicts: StaticListSchema = [1, n0, _EWC,
  0, () => EntryDescription$
];
var EntryViolationReasons = 64 | 0;
var EntryViolations: StaticListSchema = [1, n0, _EVn,
  0, () => EntryViolation$
];
var EvaluationResults: StaticListSchema = [1, n0, _ERv,
  0, () => EvaluationResult$
];
var ExpectedRoutes: StaticListSchema = [1, n0, _ERxp,
  0, () => ExpectedRoute$
];
var FailedItemList: StaticListSchema = [1, n0, _FIL,
  0, () => FailedItem$
];
var IdentifierList = 64 | 0;
var LengthBoundedStringList = 64 | 0;
var MemberAccounts = 64 | 0;
var NetworkAclEntries: StaticListSchema = [1, n0, _NAEe,
  0, () => NetworkAclEntry$
];
var NetworkFirewallActionList = 64 | 0;
var OrderedRemediationActions: StaticListSchema = [1, n0, _ORA,
  0, () => RemediationActionWithOrder$
];
var OrganizationalUnitIdList = 64 | 0;
var PartialMatches: StaticListSchema = [1, n0, _PM,
  0, () => PartialMatch$
];
var PolicyComplianceStatusList: StaticListSchema = [1, n0, _PCSL,
  0, () => PolicyComplianceStatus$
];
var PolicySummaryList: StaticListSchema = [1, n0, _PSL,
  0, () => PolicySummary$
];
var PossibleRemediationActionList: StaticListSchema = [1, n0, _PRAL,
  0, () => PossibleRemediationAction$
];
var ProtocolsList = 64 | 0;
var ProtocolsListsData: StaticListSchema = [1, n0, _PLDr,
  0, () => ProtocolsListDataSummary$
];
var ResourceArnList = 64 | 0;
var ResourceIdList = 64 | 0;
var ResourceList: StaticListSchema = [1, n0, _RL,
  0, () => Resource$
];
var ResourceSetIds = 64 | 0;
var ResourceSetSummaryList: StaticListSchema = [1, n0, _RSSL,
  0, () => ResourceSetSummary$
];
var ResourceTags: StaticListSchema = [1, n0, _RTe,
  0, () => ResourceTag$
];
var ResourceTypeList = 64 | 0;
var ResourceViolations: StaticListSchema = [1, n0, _RVe,
  0, () => ResourceViolation$
];
var Routes: StaticListSchema = [1, n0, _Rou,
  0, () => Route$
];
var SecurityGroupRemediationActions: StaticListSchema = [1, n0, _SGRAe,
  0, () => SecurityGroupRemediationAction$
];
var SecurityServiceTypeList = 64 | 0;
var StatefulRuleGroupList: StaticListSchema = [1, n0, _SRGL,
  0, () => StatefulRuleGroup$
];
var StatelessRuleGroupList: StaticListSchema = [1, n0, _SRGLt,
  0, () => StatelessRuleGroup$
];
var TagKeyList = 64 | 0;
var TagList: StaticListSchema = [1, n0, _TL,
  0, () => Tag$
];
var TargetViolationReasons = 64 | 0;
var ThirdPartyFirewallFirewallPolicies: StaticListSchema = [1, n0, _TPFFP,
  0, () => ThirdPartyFirewallFirewallPolicy$
];
var ComplianceViolatorMetadata = 128 | 0;
var CustomerPolicyScopeMap: StaticMapSchema = [2, n0, _CPSM,
  0, 0, 64 | 0
];
var IssueInfoMap = 128 | 0;
var PreviousAppsList: StaticMapSchema = [2, n0, _PAL,
  0, 0, () => AppsList
];
var PreviousProtocolsList: StaticMapSchema = [2, n0, _PPL,
  0, 0, 64 | 0
];
export var AssociateAdminAccount$: StaticOperationSchema = [9, n0, _AAA,
  0, () => AssociateAdminAccountRequest$, () => __Unit
];
export var AssociateThirdPartyFirewall$: StaticOperationSchema = [9, n0, _ATPF,
  0, () => AssociateThirdPartyFirewallRequest$, () => AssociateThirdPartyFirewallResponse$
];
export var BatchAssociateResource$: StaticOperationSchema = [9, n0, _BAR,
  0, () => BatchAssociateResourceRequest$, () => BatchAssociateResourceResponse$
];
export var BatchDisassociateResource$: StaticOperationSchema = [9, n0, _BDR,
  0, () => BatchDisassociateResourceRequest$, () => BatchDisassociateResourceResponse$
];
export var DeleteAppsList$: StaticOperationSchema = [9, n0, _DAL,
  0, () => DeleteAppsListRequest$, () => __Unit
];
export var DeleteNotificationChannel$: StaticOperationSchema = [9, n0, _DNC,
  0, () => DeleteNotificationChannelRequest$, () => __Unit
];
export var DeletePolicy$: StaticOperationSchema = [9, n0, _DP,
  0, () => DeletePolicyRequest$, () => __Unit
];
export var DeleteProtocolsList$: StaticOperationSchema = [9, n0, _DPL,
  0, () => DeleteProtocolsListRequest$, () => __Unit
];
export var DeleteResourceSet$: StaticOperationSchema = [9, n0, _DRS,
  0, () => DeleteResourceSetRequest$, () => __Unit
];
export var DisassociateAdminAccount$: StaticOperationSchema = [9, n0, _DAA,
  0, () => DisassociateAdminAccountRequest$, () => __Unit
];
export var DisassociateThirdPartyFirewall$: StaticOperationSchema = [9, n0, _DTPF,
  0, () => DisassociateThirdPartyFirewallRequest$, () => DisassociateThirdPartyFirewallResponse$
];
export var GetAdminAccount$: StaticOperationSchema = [9, n0, _GAA,
  0, () => GetAdminAccountRequest$, () => GetAdminAccountResponse$
];
export var GetAdminScope$: StaticOperationSchema = [9, n0, _GAS,
  0, () => GetAdminScopeRequest$, () => GetAdminScopeResponse$
];
export var GetAppsList$: StaticOperationSchema = [9, n0, _GAL,
  0, () => GetAppsListRequest$, () => GetAppsListResponse$
];
export var GetComplianceDetail$: StaticOperationSchema = [9, n0, _GCD,
  0, () => GetComplianceDetailRequest$, () => GetComplianceDetailResponse$
];
export var GetNotificationChannel$: StaticOperationSchema = [9, n0, _GNC,
  0, () => GetNotificationChannelRequest$, () => GetNotificationChannelResponse$
];
export var GetPolicy$: StaticOperationSchema = [9, n0, _GP,
  0, () => GetPolicyRequest$, () => GetPolicyResponse$
];
export var GetProtectionStatus$: StaticOperationSchema = [9, n0, _GPS,
  0, () => GetProtectionStatusRequest$, () => GetProtectionStatusResponse$
];
export var GetProtocolsList$: StaticOperationSchema = [9, n0, _GPL,
  0, () => GetProtocolsListRequest$, () => GetProtocolsListResponse$
];
export var GetResourceSet$: StaticOperationSchema = [9, n0, _GRS,
  0, () => GetResourceSetRequest$, () => GetResourceSetResponse$
];
export var GetThirdPartyFirewallAssociationStatus$: StaticOperationSchema = [9, n0, _GTPFAS,
  0, () => GetThirdPartyFirewallAssociationStatusRequest$, () => GetThirdPartyFirewallAssociationStatusResponse$
];
export var GetViolationDetails$: StaticOperationSchema = [9, n0, _GVD,
  0, () => GetViolationDetailsRequest$, () => GetViolationDetailsResponse$
];
export var ListAdminAccountsForOrganization$: StaticOperationSchema = [9, n0, _LAAFO,
  0, () => ListAdminAccountsForOrganizationRequest$, () => ListAdminAccountsForOrganizationResponse$
];
export var ListAdminsManagingAccount$: StaticOperationSchema = [9, n0, _LAMA,
  0, () => ListAdminsManagingAccountRequest$, () => ListAdminsManagingAccountResponse$
];
export var ListAppsLists$: StaticOperationSchema = [9, n0, _LAL,
  0, () => ListAppsListsRequest$, () => ListAppsListsResponse$
];
export var ListComplianceStatus$: StaticOperationSchema = [9, n0, _LCS,
  0, () => ListComplianceStatusRequest$, () => ListComplianceStatusResponse$
];
export var ListDiscoveredResources$: StaticOperationSchema = [9, n0, _LDR,
  0, () => ListDiscoveredResourcesRequest$, () => ListDiscoveredResourcesResponse$
];
export var ListMemberAccounts$: StaticOperationSchema = [9, n0, _LMA,
  0, () => ListMemberAccountsRequest$, () => ListMemberAccountsResponse$
];
export var ListPolicies$: StaticOperationSchema = [9, n0, _LP,
  0, () => ListPoliciesRequest$, () => ListPoliciesResponse$
];
export var ListProtocolsLists$: StaticOperationSchema = [9, n0, _LPL,
  0, () => ListProtocolsListsRequest$, () => ListProtocolsListsResponse$
];
export var ListResourceSetResources$: StaticOperationSchema = [9, n0, _LRSRis,
  0, () => ListResourceSetResourcesRequest$, () => ListResourceSetResourcesResponse$
];
export var ListResourceSets$: StaticOperationSchema = [9, n0, _LRS,
  0, () => ListResourceSetsRequest$, () => ListResourceSetsResponse$
];
export var ListTagsForResource$: StaticOperationSchema = [9, n0, _LTFR,
  0, () => ListTagsForResourceRequest$, () => ListTagsForResourceResponse$
];
export var ListThirdPartyFirewallFirewallPolicies$: StaticOperationSchema = [9, n0, _LTPFFP,
  0, () => ListThirdPartyFirewallFirewallPoliciesRequest$, () => ListThirdPartyFirewallFirewallPoliciesResponse$
];
export var PutAdminAccount$: StaticOperationSchema = [9, n0, _PAA,
  0, () => PutAdminAccountRequest$, () => __Unit
];
export var PutAppsList$: StaticOperationSchema = [9, n0, _PALu,
  0, () => PutAppsListRequest$, () => PutAppsListResponse$
];
export var PutNotificationChannel$: StaticOperationSchema = [9, n0, _PNC,
  0, () => PutNotificationChannelRequest$, () => __Unit
];
export var PutPolicy$: StaticOperationSchema = [9, n0, _PP,
  0, () => PutPolicyRequest$, () => PutPolicyResponse$
];
export var PutProtocolsList$: StaticOperationSchema = [9, n0, _PPLu,
  0, () => PutProtocolsListRequest$, () => PutProtocolsListResponse$
];
export var PutResourceSet$: StaticOperationSchema = [9, n0, _PRS,
  0, () => PutResourceSetRequest$, () => PutResourceSetResponse$
];
export var TagResource$: StaticOperationSchema = [9, n0, _TR,
  0, () => TagResourceRequest$, () => TagResourceResponse$
];
export var UntagResource$: StaticOperationSchema = [9, n0, _UR,
  0, () => UntagResourceRequest$, () => UntagResourceResponse$
];
