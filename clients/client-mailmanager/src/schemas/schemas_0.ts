const _A = "Address";
const _AA = "ArchiveAction";
const _AAr = "ArchiveArn";
const _ABE = "ArchiveBooleanExpression";
const _ABTE = "ArchiveBooleanToEvaluate";
const _ADE = "AccessDeniedException";
const _AF = "AddressFilter";
const _AFC = "ArchiveFilterConditions";
const _AFCr = "ArchiveFilterCondition";
const _AFP = "ActionFailurePolicy";
const _AFr = "ArchiveFilters";
const _AH = "AddHeader";
const _AHA = "AddHeaderAction";
const _AI = "AddonInstance";
const _AIA = "AddonInstanceArn";
const _AII = "AddonInstanceId";
const _AId = "AddonInstances";
const _AIp = "ApplicationId";
const _AIr = "ArchiveId";
const _AL = "AddressList";
const _ALA = "AddressListArn";
const _ALI = "AddressListId";
const _ALN = "AddressListName";
const _ALd = "AddressLists";
const _ALr = "ArchivesList";
const _AMI = "ArchivedMessageId";
const _AN = "AddonName";
const _ANr = "ArchiveName";
const _AP = "AddressPrefix";
const _AR = "ARecord";
const _ARr = "ArchiveRetention";
const _AS = "AddonSubscription";
const _ASA = "AddonSubscriptionArn";
const _ASE = "ArchiveStringExpression";
const _ASI = "AddonSubscriptionId";
const _ASTE = "ArchiveStringToEvaluate";
const _ASd = "AddonSubscriptions";
const _ASr = "ArchiveState";
const _Ac = "Action";
const _Act = "Actions";
const _Ad = "Addresses";
const _An = "Analysis";
const _Ana = "Analyzer";
const _Ar = "Archive";
const _Arc = "Archives";
const _At = "Attribute";
const _Au = "Authentication";
const _B = "Body";
const _BE = "BooleanExpression";
const _C = "Conditions";
const _CA = "CreateArchive";
const _CAI = "CreateAddonInstance";
const _CAIR = "CreateAddonInstanceRequest";
const _CAIRr = "CreateAddonInstanceResponse";
const _CAL = "CreateAddressList";
const _CALIJ = "CreateAddressListImportJob";
const _CALIJR = "CreateAddressListImportJobRequest";
const _CALIJRr = "CreateAddressListImportJobResponse";
const _CALR = "CreateAddressListRequest";
const _CALRr = "CreateAddressListResponse";
const _CAR = "CreateArchiveRequest";
const _CARr = "CreateArchiveResponse";
const _CAS = "CreateAddonSubscription";
const _CASR = "CreateAddonSubscriptionRequest";
const _CASRr = "CreateAddonSubscriptionResponse";
const _CD = "CreatedDate";
const _CE = "ConflictException";
const _CIP = "CreateIngressPoint";
const _CIPR = "CreateIngressPointRequest";
const _CIPRr = "CreateIngressPointResponse";
const _CR = "CreateRelay";
const _CRR = "CreateRelayRequest";
const _CRRr = "CreateRelayResponse";
const _CRS = "CreateRuleSet";
const _CRSR = "CreateRuleSetRequest";
const _CRSRr = "CreateRuleSetResponse";
const _CS = "ConfigurationSet";
const _CT = "CreatedTimestamp";
const _CTP = "CreateTrafficPolicy";
const _CTPR = "CreateTrafficPolicyRequest";
const _CTPRr = "CreateTrafficPolicyResponse";
const _CTl = "ClientToken";
const _CTo = "CompletionTimestamp";
const _CTom = "CompletedTimestamp";
const _Cc = "Cc";
const _D = "Date";
const _DA = "DefaultAction";
const _DAI = "DeleteAddonInstance";
const _DAIR = "DeleteAddonInstanceRequest";
const _DAIRe = "DeleteAddonInstanceResponse";
const _DAL = "DeleteAddressList";
const _DALR = "DeleteAddressListRequest";
const _DALRe = "DeleteAddressListResponse";
const _DAR = "DeleteArchiveRequest";
const _DARe = "DeleteArchiveResponse";
const _DAS = "DeleteAddonSubscription";
const _DASR = "DeleteAddonSubscriptionRequest";
const _DASRe = "DeleteAddonSubscriptionResponse";
const _DAe = "DeleteArchive";
const _DAr = "DropAction";
const _DE = "DmarcExpression";
const _DIP = "DeleteIngressPoint";
const _DIPR = "DeleteIngressPointRequest";
const _DIPRe = "DeleteIngressPointResponse";
const _DMFAL = "DeregisterMemberFromAddressList";
const _DMFALR = "DeregisterMemberFromAddressListRequest";
const _DMFALRe = "DeregisterMemberFromAddressListResponse";
const _DR = "DeleteRelay";
const _DRR = "DeleteRelayRequest";
const _DRRe = "DeleteRelayResponse";
const _DRS = "DeleteRuleSet";
const _DRSR = "DeleteRuleSetRequest";
const _DRSRe = "DeleteRuleSetResponse";
const _DTM = "DeliverToMailbox";
const _DTMA = "DeliverToMailboxAction";
const _DTP = "DeleteTrafficPolicy";
const _DTPR = "DeleteTrafficPolicyRequest";
const _DTPRe = "DeleteTrafficPolicyResponse";
const _DTQB = "DeliverToQBusiness";
const _DTQBA = "DeliverToQBusinessAction";
const _Dr = "Drop";
const _E = "Evaluate";
const _EA = "EmailAddress";
const _EDC = "ExportDestinationConfiguration";
const _EI = "ExportId";
const _EM = "ErrorMessage";
const _ES = "ExportStatus";
const _ESL = "ExportSummaryList";
const _ESx = "ExportSummary";
const _En = "Envelope";
const _Enc = "Encoding";
const _Er = "Error";
const _Ex = "Exports";
const _F = "From";
const _FIC = "FailedItemsCount";
const _FT = "FromTimestamp";
const _Fi = "Filters";
const _Fil = "Filter";
const _GA = "GetArchive";
const _GAE = "GetArchiveExport";
const _GAER = "GetArchiveExportRequest";
const _GAERe = "GetArchiveExportResponse";
const _GAI = "GetAddonInstance";
const _GAIR = "GetAddonInstanceRequest";
const _GAIRe = "GetAddonInstanceResponse";
const _GAL = "GetAddressList";
const _GALIJ = "GetAddressListImportJob";
const _GALIJR = "GetAddressListImportJobRequest";
const _GALIJRe = "GetAddressListImportJobResponse";
const _GALR = "GetAddressListRequest";
const _GALRe = "GetAddressListResponse";
const _GAM = "GetArchiveMessage";
const _GAMC = "GetArchiveMessageContent";
const _GAMCR = "GetArchiveMessageContentRequest";
const _GAMCRe = "GetArchiveMessageContentResponse";
const _GAMR = "GetArchiveMessageRequest";
const _GAMRe = "GetArchiveMessageResponse";
const _GAR = "GetArchiveRequest";
const _GARe = "GetArchiveResponse";
const _GAS = "GetAddonSubscription";
const _GASR = "GetAddonSubscriptionRequest";
const _GASRR = "GetArchiveSearchResultsRequest";
const _GASRRe = "GetArchiveSearchResultsResponse";
const _GASRe = "GetAddonSubscriptionResponse";
const _GASRet = "GetArchiveSearchRequest";
const _GASRetr = "GetArchiveSearchResponse";
const _GASRetrc = "GetArchiveSearchResults";
const _GASe = "GetArchiveSearch";
const _GIP = "GetIngressPoint";
const _GIPR = "GetIngressPointRequest";
const _GIPRe = "GetIngressPointResponse";
const _GMOAL = "GetMemberOfAddressList";
const _GMOALR = "GetMemberOfAddressListRequest";
const _GMOALRe = "GetMemberOfAddressListResponse";
const _GR = "GetRelay";
const _GRR = "GetRelayRequest";
const _GRRe = "GetRelayResponse";
const _GRS = "GetRuleSet";
const _GRSR = "GetRuleSetRequest";
const _GRSRe = "GetRuleSetResponse";
const _GTP = "GetTrafficPolicy";
const _GTPR = "GetTrafficPolicyRequest";
const _GTPRe = "GetTrafficPolicyResponse";
const _H = "Helo";
const _HA = "HasAttachments";
const _HN = "HeaderName";
const _HV = "HeaderValue";
const _Ht = "Html";
const _I = "Include";
const _IA = "IngressAnalysis";
const _IBE = "IngressBooleanExpression";
const _IBTE = "IngressBooleanToEvaluate";
const _IDF = "ImportDataFormat";
const _IDT = "ImportDataType";
const _IE = "IpExpression";
const _IEp = "Ipv6Expression";
const _II = "IndexId";
const _IIAL = "IsInAddressList";
const _IIC = "ImportedItemsCount";
const _IIE = "IngressIpv4Expression";
const _IIEn = "IngressIpv6Expression";
const _IIIAL = "IngressIsInAddressList";
const _IITE = "IngressIpToEvaluate";
const _IITEn = "IngressIpv6ToEvaluate";
const _IJ = "ImportJob";
const _IJm = "ImportJobs";
const _IM = "IncludeMetadata";
const _IP = "IngressPoint";
const _IPA = "IngressPointArn";
const _IPAC = "IngressPointAuthConfiguration";
const _IPC = "IngressPointConfiguration";
const _IPI = "IngressPointId";
const _IPL = "IngressPointsList";
const _IPN = "IngressPointName";
const _IPPC = "IngressPointPasswordConfiguration";
const _IPn = "IngressPoints";
const _IRT = "InReplyTo";
const _ISE = "IngressStringExpression";
const _ISTE = "IngressStringToEvaluate";
const _IT = "IpType";
const _ITPE = "IngressTlsProtocolExpression";
const _ITPTE = "IngressTlsProtocolToEvaluate";
const _JI = "JobId";
const _K = "Key";
const _KKA = "KmsKeyArn";
const _LA = "ListArchives";
const _LAE = "ListArchiveExports";
const _LAER = "ListArchiveExportsRequest";
const _LAERi = "ListArchiveExportsResponse";
const _LAI = "ListAddonInstances";
const _LAIR = "ListAddonInstancesRequest";
const _LAIRi = "ListAddonInstancesResponse";
const _LAL = "ListAddressLists";
const _LALIJ = "ListAddressListImportJobs";
const _LALIJR = "ListAddressListImportJobsRequest";
const _LALIJRi = "ListAddressListImportJobsResponse";
const _LALR = "ListAddressListsRequest";
const _LALRi = "ListAddressListsResponse";
const _LAR = "ListArchivesRequest";
const _LARi = "ListArchivesResponse";
const _LAS = "ListAddonSubscriptions";
const _LASR = "ListAddonSubscriptionsRequest";
const _LASRi = "ListAddonSubscriptionsResponse";
const _LASRis = "ListArchiveSearchesRequest";
const _LASRist = "ListArchiveSearchesResponse";
const _LASi = "ListArchiveSearches";
const _LIP = "ListIngressPoints";
const _LIPR = "ListIngressPointsRequest";
const _LIPRi = "ListIngressPointsResponse";
const _LMD = "LastModificationDate";
const _LMOAL = "ListMembersOfAddressList";
const _LMOALR = "ListMembersOfAddressListRequest";
const _LMOALRi = "ListMembersOfAddressListResponse";
const _LMT = "LastModifiedTimestamp";
const _LR = "ListRelays";
const _LRR = "ListRelaysRequest";
const _LRRi = "ListRelaysResponse";
const _LRS = "ListRuleSets";
const _LRSR = "ListRuleSetsRequest";
const _LRSRi = "ListRuleSetsResponse";
const _LTFR = "ListTagsForResource";
const _LTFRR = "ListTagsForResourceRequest";
const _LTFRRi = "ListTagsForResourceResponse";
const _LTP = "ListTrafficPolicies";
const _LTPR = "ListTrafficPoliciesRequest";
const _LTPRi = "ListTrafficPoliciesResponse";
const _LUT = "LastUpdatedTimestamp";
const _M = "Message";
const _MA = "MailboxArn";
const _MB = "MessageBody";
const _MDL = "MessageDownloadLink";
const _MF = "MailFrom";
const _MHA = "MimeHeaderAttribute";
const _MI = "MessageId";
const _MM = "MessageMalformed";
const _MMSB = "MaxMessageSizeBytes";
const _MR = "MaxResults";
const _Me = "Metadata";
const _N = "Name";
const _NA = "NoAuthentication";
const _NC = "NetworkConfiguration";
const _NE = "NumberExpression";
const _NT = "NextToken";
const _O = "Operator";
const _PC = "PolicyConditions";
const _PCo = "PolicyCondition";
const _PNC = "PrivateNetworkConfiguration";
const _PNCu = "PublicNetworkConfiguration";
const _PS = "PolicyStatements";
const _PSL = "PolicyStatementList";
const _PSPET = "PreviousSmtpPasswordExpiryTimestamp";
const _PSPV = "PreviousSmtpPasswordVersion";
const _PSU = "PreSignedUrl";
const _PSa = "PageSize";
const _PSo = "PolicyStatement";
const _PT = "PayloadType";
const _PTS = "PublishToSns";
const _R = "Retention";
const _RA = "RoleArn";
const _RAe = "RelayArn";
const _RAel = "RelayAction";
const _RAela = "RelayAuthentication";
const _RAes = "ResourceArn";
const _RAu = "RuleActions";
const _RAul = "RuleAction";
const _RBE = "RuleBooleanExpression";
const _RBTE = "RuleBooleanToEvaluate";
const _RC = "RuleConditions";
const _RCu = "RuleCondition";
const _RDE = "RuleDmarcExpression";
const _RF = "ResultField";
const _RH = "ReceivedHeaders";
const _RI = "RelayId";
const _RIE = "RuleIpExpression";
const _RIIAL = "RuleIsInAddressList";
const _RITE = "RuleIpToEvaluate";
const _RL = "RowsList";
const _RMTAL = "RegisterMemberToAddressList";
const _RMTALR = "RegisterMemberToAddressListRequest";
const _RMTALRe = "RegisterMemberToAddressListResponse";
const _RN = "RelayName";
const _RNE = "RuleNumberExpression";
const _RNFE = "ResourceNotFoundException";
const _RNTE = "RuleNumberToEvaluate";
const _RP = "RetentionPeriod";
const _RR = "ReplaceRecipient";
const _RRA = "ReplaceRecipientAction";
const _RS = "RuleSets";
const _RSA = "RuleSetArn";
const _RSE = "RuleStringExpression";
const _RSI = "RuleSetId";
const _RSN = "RuleSetName";
const _RSTE = "RuleStringToEvaluate";
const _RSu = "RuleSet";
const _RT = "ReceivedTimestamp";
const _RVE = "RuleVerdictExpression";
const _RVTE = "RuleVerdictToEvaluate";
const _RW = "ReplaceWith";
const _Re = "Relays";
const _Rec = "Recipients";
const _Rel = "Relay";
const _Ro = "Rows";
const _Row = "Row";
const _Ru = "Rules";
const _Rul = "Rule";
const _S = "State";
const _SA = "SecretArn";
const _SAE = "StartArchiveExport";
const _SAER = "StartArchiveExportRequest";
const _SAERt = "StartArchiveExportResponse";
const _SAERto = "StopArchiveExportRequest";
const _SAERtop = "StopArchiveExportResponse";
const _SAEt = "StopArchiveExport";
const _SALIJ = "StartAddressListImportJob";
const _SALIJR = "StartAddressListImportJobRequest";
const _SALIJRt = "StartAddressListImportJobResponse";
const _SALIJRto = "StopAddressListImportJobRequest";
const _SALIJRtop = "StopAddressListImportJobResponse";
const _SALIJt = "StopAddressListImportJob";
const _SAS = "StartArchiveSearch";
const _SASR = "StartArchiveSearchRequest";
const _SASRt = "StartArchiveSearchResponse";
const _SASRto = "StopArchiveSearchRequest";
const _SASRtop = "StopArchiveSearchResponse";
const _SASt = "StopArchiveSearch";
const _SAa = "SavedAddress";
const _SAav = "SavedAddresses";
const _SAc = "S3Action";
const _SAe = "SendAction";
const _SAn = "SnsAction";
const _SAo = "SourceArn";
const _SB = "S3Bucket";
const _SE = "StringExpression";
const _SEDC = "S3ExportDestinationConfiguration";
const _SH = "SenderHostname";
const _SI = "SearchId";
const _SIA = "SenderIpAddress";
const _SIo = "SourceIdentity";
const _SL = "S3Location";
const _SM = "SendingMethod";
const _SN = "ServerName";
const _SP = "SmtpPassword";
const _SPV = "SmtpPasswordVersion";
const _SPe = "ServerPort";
const _SPen = "SendingPool";
const _SPr = "S3Prefix";
const _SQEE = "ServiceQuotaExceededException";
const _SS = "SearchStatus";
const _SSKKI = "S3SseKmsKeyId";
const _SSL = "SearchSummaryList";
const _SSe = "SearchSummary";
const _ST = "SubmissionTimestamp";
const _STU = "StatusToUpdate";
const _STt = "StartTimestamp";
const _S_ = "S3";
const _Se = "Searches";
const _Sen = "Send";
const _St = "Status";
const _Su = "Subject";
const _T = "Tags";
const _TA = "TargetArchive";
const _TAo = "TopicArn";
const _TCS = "TlsCipherSuite";
const _TE = "ThrottlingException";
const _TEl = "TlsExpression";
const _TK = "TagKeys";
const _TL = "TagList";
const _TP = "TrafficPolicies";
const _TPA = "TrafficPolicyArn";
const _TPI = "TrafficPolicyId";
const _TPL = "TrafficPolicyList";
const _TPN = "TrafficPolicyName";
const _TPl = "TlsProtocol";
const _TPr = "TrafficPolicy";
const _TR = "TagResource";
const _TRR = "TagResourceRequest";
const _TRRa = "TagResourceResponse";
const _TT = "ToTimestamp";
const _Ta = "Tag";
const _Te = "Text";
const _Ti = "Timestamp";
const _To = "To";
const _Ty = "Type";
const _U = "Unless";
const _UA = "UpdateArchive";
const _UAR = "UpdateArchiveRequest";
const _UARp = "UpdateArchiveResponse";
const _UIP = "UpdateIngressPoint";
const _UIPR = "UpdateIngressPointRequest";
const _UIPRp = "UpdateIngressPointResponse";
const _UR = "UntagResource";
const _URR = "UntagResourceRequest";
const _URRn = "UntagResourceResponse";
const _URRp = "UpdateRelayRequest";
const _URRpd = "UpdateRelayResponse";
const _URS = "UpdateRuleSet";
const _URSR = "UpdateRuleSetRequest";
const _URSRp = "UpdateRuleSetResponse";
const _URp = "UpdateRelay";
const _UTP = "UpdateTrafficPolicy";
const _UTPR = "UpdateTrafficPolicyRequest";
const _UTPRp = "UpdateTrafficPolicyResponse";
const _V = "Values";
const _VE = "ValidationException";
const _VEI = "VpcEndpointId";
const _VEe = "VerdictExpression";
const _Va = "Value";
const _WTS = "WriteToS3";
const _XM = "XMailer";
const _XOM = "XOriginalMailer";
const _XP = "XPriority";
const _c = "client";
const _e = "error";
const _hE = "httpError";
const _s = "smithy.ts.sdk.synthetic.com.amazonaws.mailmanager";
const n0 = "com.amazonaws.mailmanager";

// smithy-typescript generated code
import { TypeRegistry } from "@smithy/core/schema";
import type {
  StaticErrorSchema,
  StaticListSchema,
  StaticOperationSchema,
  StaticSimpleSchema,
  StaticStructureSchema,
  StaticUnionSchema,
} from "@smithy/types";

import {
  AccessDeniedException,
  ConflictException,
  ResourceNotFoundException,
  ServiceQuotaExceededException,
  ThrottlingException,
  ValidationException,
} from "../models/errors";
import { MailManagerServiceException } from "../models/MailManagerServiceException";

/* eslint no-var: 0 */
var Address: StaticSimpleSchema = [0, n0, _A, 8, 0];
var AddressPrefix: StaticSimpleSchema = [0, n0, _AP, 8, 0];
var EmailAddress: StaticSimpleSchema = [0, n0, _EA, 8, 0];
var PreSignedUrl: StaticSimpleSchema = [0, n0, _PSU, 8, 0];
var SenderIpAddress: StaticSimpleSchema = [0, n0, _SIA, 8, 0];
var SmtpPassword: StaticSimpleSchema = [0, n0, _SP, 8, 0];
export var AccessDeniedException$: StaticErrorSchema = [-3, n0, _ADE,
  { [_e]: _c, [_hE]: 403 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(AccessDeniedException$, AccessDeniedException);
export var AddHeaderAction$: StaticStructureSchema = [3, n0, _AHA,
  0,
  [_HN, _HV],
  [0, 0]
];
export var AddonInstance$: StaticStructureSchema = [3, n0, _AI,
  0,
  [_AII, _ASI, _AN, _AIA, _CT],
  [0, 0, 0, 0, 4]
];
export var AddonSubscription$: StaticStructureSchema = [3, n0, _AS,
  0,
  [_ASI, _AN, _ASA, _CT],
  [0, 0, 0, 4]
];
export var AddressFilter$: StaticStructureSchema = [3, n0, _AF,
  0,
  [_AP],
  [[() => AddressPrefix, 0]]
];
export var AddressList$: StaticStructureSchema = [3, n0, _AL,
  0,
  [_ALI, _ALA, _ALN, _CT, _LUT],
  [0, 0, 0, 4, 4]
];
export var Analysis$: StaticStructureSchema = [3, n0, _An,
  0,
  [_Ana, _RF],
  [0, 0]
];
export var Archive$: StaticStructureSchema = [3, n0, _Ar,
  0,
  [_AIr, _ANr, _ASr, _LUT],
  [0, 0, 0, 4]
];
export var ArchiveAction$: StaticStructureSchema = [3, n0, _AA,
  0,
  [_AFP, _TA],
  [0, 0]
];
export var ArchiveBooleanExpression$: StaticStructureSchema = [3, n0, _ABE,
  0,
  [_E, _O],
  [() => ArchiveBooleanToEvaluate$, 0]
];
export var ArchiveFilters$: StaticStructureSchema = [3, n0, _AFr,
  0,
  [_I, _U],
  [() => ArchiveFilterConditions, () => ArchiveFilterConditions]
];
export var ArchiveStringExpression$: StaticStructureSchema = [3, n0, _ASE,
  0,
  [_E, _O, _V],
  [() => ArchiveStringToEvaluate$, 0, 64 | 0]
];
export var ConflictException$: StaticErrorSchema = [-3, n0, _CE,
  { [_e]: _c, [_hE]: 409 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ConflictException$, ConflictException);
export var CreateAddonInstanceRequest$: StaticStructureSchema = [3, n0, _CAIR,
  0,
  [_CTl, _ASI, _T],
  [[0, 4], 0, () => TagList]
];
export var CreateAddonInstanceResponse$: StaticStructureSchema = [3, n0, _CAIRr,
  0,
  [_AII],
  [0]
];
export var CreateAddonSubscriptionRequest$: StaticStructureSchema = [3, n0, _CASR,
  0,
  [_CTl, _AN, _T],
  [[0, 4], 0, () => TagList]
];
export var CreateAddonSubscriptionResponse$: StaticStructureSchema = [3, n0, _CASRr,
  0,
  [_ASI],
  [0]
];
export var CreateAddressListImportJobRequest$: StaticStructureSchema = [3, n0, _CALIJR,
  0,
  [_CTl, _ALI, _N, _IDF],
  [[0, 4], 0, 0, () => ImportDataFormat$]
];
export var CreateAddressListImportJobResponse$: StaticStructureSchema = [3, n0, _CALIJRr,
  0,
  [_JI, _PSU],
  [0, [() => PreSignedUrl, 0]]
];
export var CreateAddressListRequest$: StaticStructureSchema = [3, n0, _CALR,
  0,
  [_CTl, _ALN, _T],
  [[0, 4], 0, () => TagList]
];
export var CreateAddressListResponse$: StaticStructureSchema = [3, n0, _CALRr,
  0,
  [_ALI],
  [0]
];
export var CreateArchiveRequest$: StaticStructureSchema = [3, n0, _CAR,
  0,
  [_CTl, _ANr, _R, _KKA, _T],
  [[0, 4], 0, () => ArchiveRetention$, 0, () => TagList]
];
export var CreateArchiveResponse$: StaticStructureSchema = [3, n0, _CARr,
  0,
  [_AIr],
  [0]
];
export var CreateIngressPointRequest$: StaticStructureSchema = [3, n0, _CIPR,
  0,
  [_CTl, _IPN, _Ty, _RSI, _TPI, _IPC, _NC, _T],
  [[0, 4], 0, 0, 0, 0, [() => IngressPointConfiguration$, 0], () => NetworkConfiguration$, () => TagList]
];
export var CreateIngressPointResponse$: StaticStructureSchema = [3, n0, _CIPRr,
  0,
  [_IPI],
  [0]
];
export var CreateRelayRequest$: StaticStructureSchema = [3, n0, _CRR,
  0,
  [_CTl, _RN, _SN, _SPe, _Au, _T],
  [[0, 4], 0, 0, 1, () => RelayAuthentication$, () => TagList]
];
export var CreateRelayResponse$: StaticStructureSchema = [3, n0, _CRRr,
  0,
  [_RI],
  [0]
];
export var CreateRuleSetRequest$: StaticStructureSchema = [3, n0, _CRSR,
  0,
  [_CTl, _RSN, _Ru, _T],
  [[0, 4], 0, [() => Rules, 0], () => TagList]
];
export var CreateRuleSetResponse$: StaticStructureSchema = [3, n0, _CRSRr,
  0,
  [_RSI],
  [0]
];
export var CreateTrafficPolicyRequest$: StaticStructureSchema = [3, n0, _CTPR,
  0,
  [_CTl, _TPN, _PS, _DA, _MMSB, _T],
  [[0, 4], 0, () => PolicyStatementList, 0, 1, () => TagList]
];
export var CreateTrafficPolicyResponse$: StaticStructureSchema = [3, n0, _CTPRr,
  0,
  [_TPI],
  [0]
];
export var DeleteAddonInstanceRequest$: StaticStructureSchema = [3, n0, _DAIR,
  0,
  [_AII],
  [0]
];
export var DeleteAddonInstanceResponse$: StaticStructureSchema = [3, n0, _DAIRe,
  0,
  [],
  []
];
export var DeleteAddonSubscriptionRequest$: StaticStructureSchema = [3, n0, _DASR,
  0,
  [_ASI],
  [0]
];
export var DeleteAddonSubscriptionResponse$: StaticStructureSchema = [3, n0, _DASRe,
  0,
  [],
  []
];
export var DeleteAddressListRequest$: StaticStructureSchema = [3, n0, _DALR,
  0,
  [_ALI],
  [0]
];
export var DeleteAddressListResponse$: StaticStructureSchema = [3, n0, _DALRe,
  0,
  [],
  []
];
export var DeleteArchiveRequest$: StaticStructureSchema = [3, n0, _DAR,
  0,
  [_AIr],
  [0]
];
export var DeleteArchiveResponse$: StaticStructureSchema = [3, n0, _DARe,
  0,
  [],
  []
];
export var DeleteIngressPointRequest$: StaticStructureSchema = [3, n0, _DIPR,
  0,
  [_IPI],
  [0]
];
export var DeleteIngressPointResponse$: StaticStructureSchema = [3, n0, _DIPRe,
  0,
  [],
  []
];
export var DeleteRelayRequest$: StaticStructureSchema = [3, n0, _DRR,
  0,
  [_RI],
  [0]
];
export var DeleteRelayResponse$: StaticStructureSchema = [3, n0, _DRRe,
  0,
  [],
  []
];
export var DeleteRuleSetRequest$: StaticStructureSchema = [3, n0, _DRSR,
  0,
  [_RSI],
  [0]
];
export var DeleteRuleSetResponse$: StaticStructureSchema = [3, n0, _DRSRe,
  0,
  [],
  []
];
export var DeleteTrafficPolicyRequest$: StaticStructureSchema = [3, n0, _DTPR,
  0,
  [_TPI],
  [0]
];
export var DeleteTrafficPolicyResponse$: StaticStructureSchema = [3, n0, _DTPRe,
  0,
  [],
  []
];
export var DeliverToMailboxAction$: StaticStructureSchema = [3, n0, _DTMA,
  0,
  [_AFP, _MA, _RA],
  [0, 0, 0]
];
export var DeliverToQBusinessAction$: StaticStructureSchema = [3, n0, _DTQBA,
  0,
  [_AFP, _AIp, _II, _RA],
  [0, 0, 0, 0]
];
export var DeregisterMemberFromAddressListRequest$: StaticStructureSchema = [3, n0, _DMFALR,
  0,
  [_ALI, _A],
  [0, [() => Address, 0]]
];
export var DeregisterMemberFromAddressListResponse$: StaticStructureSchema = [3, n0, _DMFALRe,
  0,
  [],
  []
];
export var DropAction$: StaticStructureSchema = [3, n0, _DAr,
  0,
  [],
  []
];
export var Envelope$: StaticStructureSchema = [3, n0, _En,
  0,
  [_H, _F, _To],
  [0, 0, 64 | 0]
];
export var ExportStatus$: StaticStructureSchema = [3, n0, _ES,
  0,
  [_ST, _CTo, _S, _EM],
  [4, 4, 0, 0]
];
export var ExportSummary$: StaticStructureSchema = [3, n0, _ESx,
  0,
  [_EI, _St],
  [0, () => ExportStatus$]
];
export var GetAddonInstanceRequest$: StaticStructureSchema = [3, n0, _GAIR,
  0,
  [_AII],
  [0]
];
export var GetAddonInstanceResponse$: StaticStructureSchema = [3, n0, _GAIRe,
  0,
  [_ASI, _AN, _AIA, _CT],
  [0, 0, 0, 4]
];
export var GetAddonSubscriptionRequest$: StaticStructureSchema = [3, n0, _GASR,
  0,
  [_ASI],
  [0]
];
export var GetAddonSubscriptionResponse$: StaticStructureSchema = [3, n0, _GASRe,
  0,
  [_AN, _ASA, _CT],
  [0, 0, 4]
];
export var GetAddressListImportJobRequest$: StaticStructureSchema = [3, n0, _GALIJR,
  0,
  [_JI],
  [0]
];
export var GetAddressListImportJobResponse$: StaticStructureSchema = [3, n0, _GALIJRe,
  0,
  [_JI, _N, _St, _PSU, _IIC, _FIC, _IDF, _ALI, _CT, _STt, _CTom, _Er],
  [0, 0, 0, [() => PreSignedUrl, 0], 1, 1, () => ImportDataFormat$, 0, 4, 4, 4, 0]
];
export var GetAddressListRequest$: StaticStructureSchema = [3, n0, _GALR,
  0,
  [_ALI],
  [0]
];
export var GetAddressListResponse$: StaticStructureSchema = [3, n0, _GALRe,
  0,
  [_ALI, _ALA, _ALN, _CT, _LUT],
  [0, 0, 0, 4, 4]
];
export var GetArchiveExportRequest$: StaticStructureSchema = [3, n0, _GAER,
  0,
  [_EI],
  [0]
];
export var GetArchiveExportResponse$: StaticStructureSchema = [3, n0, _GAERe,
  0,
  [_AIr, _Fi, _FT, _TT, _MR, _EDC, _St],
  [0, () => ArchiveFilters$, 4, 4, 1, () => ExportDestinationConfiguration$, () => ExportStatus$]
];
export var GetArchiveMessageContentRequest$: StaticStructureSchema = [3, n0, _GAMCR,
  0,
  [_AMI],
  [0]
];
export var GetArchiveMessageContentResponse$: StaticStructureSchema = [3, n0, _GAMCRe,
  0,
  [_B],
  [() => MessageBody$]
];
export var GetArchiveMessageRequest$: StaticStructureSchema = [3, n0, _GAMR,
  0,
  [_AMI],
  [0]
];
export var GetArchiveMessageResponse$: StaticStructureSchema = [3, n0, _GAMRe,
  0,
  [_MDL, _Me, _En],
  [0, [() => Metadata$, 0], () => Envelope$]
];
export var GetArchiveRequest$: StaticStructureSchema = [3, n0, _GAR,
  0,
  [_AIr],
  [0]
];
export var GetArchiveResponse$: StaticStructureSchema = [3, n0, _GARe,
  0,
  [_AIr, _ANr, _AAr, _ASr, _R, _CT, _LUT, _KKA],
  [0, 0, 0, 0, () => ArchiveRetention$, 4, 4, 0]
];
export var GetArchiveSearchRequest$: StaticStructureSchema = [3, n0, _GASRet,
  0,
  [_SI],
  [0]
];
export var GetArchiveSearchResponse$: StaticStructureSchema = [3, n0, _GASRetr,
  0,
  [_AIr, _Fi, _FT, _TT, _MR, _St],
  [0, () => ArchiveFilters$, 4, 4, 1, () => SearchStatus$]
];
export var GetArchiveSearchResultsRequest$: StaticStructureSchema = [3, n0, _GASRR,
  0,
  [_SI],
  [0]
];
export var GetArchiveSearchResultsResponse$: StaticStructureSchema = [3, n0, _GASRRe,
  0,
  [_Ro],
  [[() => RowsList, 0]]
];
export var GetIngressPointRequest$: StaticStructureSchema = [3, n0, _GIPR,
  0,
  [_IPI],
  [0]
];
export var GetIngressPointResponse$: StaticStructureSchema = [3, n0, _GIPRe,
  0,
  [_IPI, _IPN, _IPA, _St, _Ty, _AR, _RSI, _TPI, _IPAC, _NC, _CT, _LUT],
  [0, 0, 0, 0, 0, 0, 0, 0, () => IngressPointAuthConfiguration$, () => NetworkConfiguration$, 4, 4]
];
export var GetMemberOfAddressListRequest$: StaticStructureSchema = [3, n0, _GMOALR,
  0,
  [_ALI, _A],
  [0, [() => Address, 0]]
];
export var GetMemberOfAddressListResponse$: StaticStructureSchema = [3, n0, _GMOALRe,
  0,
  [_A, _CT],
  [[() => Address, 0], 4]
];
export var GetRelayRequest$: StaticStructureSchema = [3, n0, _GRR,
  0,
  [_RI],
  [0]
];
export var GetRelayResponse$: StaticStructureSchema = [3, n0, _GRRe,
  0,
  [_RI, _RAe, _RN, _SN, _SPe, _Au, _CT, _LMT],
  [0, 0, 0, 0, 1, () => RelayAuthentication$, 4, 4]
];
export var GetRuleSetRequest$: StaticStructureSchema = [3, n0, _GRSR,
  0,
  [_RSI],
  [0]
];
export var GetRuleSetResponse$: StaticStructureSchema = [3, n0, _GRSRe,
  0,
  [_RSI, _RSA, _RSN, _CD, _LMD, _Ru],
  [0, 0, 0, 4, 4, [() => Rules, 0]]
];
export var GetTrafficPolicyRequest$: StaticStructureSchema = [3, n0, _GTPR,
  0,
  [_TPI],
  [0]
];
export var GetTrafficPolicyResponse$: StaticStructureSchema = [3, n0, _GTPRe,
  0,
  [_TPN, _TPI, _TPA, _PS, _MMSB, _DA, _CT, _LUT],
  [0, 0, 0, () => PolicyStatementList, 1, 0, 4, 4]
];
export var ImportDataFormat$: StaticStructureSchema = [3, n0, _IDF,
  0,
  [_IDT],
  [0]
];
export var ImportJob$: StaticStructureSchema = [3, n0, _IJ,
  0,
  [_JI, _N, _St, _PSU, _IIC, _FIC, _IDF, _ALI, _CT, _STt, _CTom, _Er],
  [0, 0, 0, [() => PreSignedUrl, 0], 1, 1, () => ImportDataFormat$, 0, 4, 4, 4, 0]
];
export var IngressAnalysis$: StaticStructureSchema = [3, n0, _IA,
  0,
  [_Ana, _RF],
  [0, 0]
];
export var IngressBooleanExpression$: StaticStructureSchema = [3, n0, _IBE,
  0,
  [_E, _O],
  [() => IngressBooleanToEvaluate$, 0]
];
export var IngressIpv4Expression$: StaticStructureSchema = [3, n0, _IIE,
  0,
  [_E, _O, _V],
  [() => IngressIpToEvaluate$, 0, 64 | 0]
];
export var IngressIpv6Expression$: StaticStructureSchema = [3, n0, _IIEn,
  0,
  [_E, _O, _V],
  [() => IngressIpv6ToEvaluate$, 0, 64 | 0]
];
export var IngressIsInAddressList$: StaticStructureSchema = [3, n0, _IIIAL,
  0,
  [_At, _ALd],
  [0, 64 | 0]
];
export var IngressPoint$: StaticStructureSchema = [3, n0, _IP,
  0,
  [_IPN, _IPI, _St, _Ty, _AR],
  [0, 0, 0, 0, 0]
];
export var IngressPointAuthConfiguration$: StaticStructureSchema = [3, n0, _IPAC,
  0,
  [_IPPC, _SA],
  [() => IngressPointPasswordConfiguration$, 0]
];
export var IngressPointPasswordConfiguration$: StaticStructureSchema = [3, n0, _IPPC,
  0,
  [_SPV, _PSPV, _PSPET],
  [0, 0, 4]
];
export var IngressStringExpression$: StaticStructureSchema = [3, n0, _ISE,
  0,
  [_E, _O, _V],
  [() => IngressStringToEvaluate$, 0, 64 | 0]
];
export var IngressTlsProtocolExpression$: StaticStructureSchema = [3, n0, _ITPE,
  0,
  [_E, _O, _Va],
  [() => IngressTlsProtocolToEvaluate$, 0, 0]
];
export var ListAddonInstancesRequest$: StaticStructureSchema = [3, n0, _LAIR,
  0,
  [_NT, _PSa],
  [0, 1]
];
export var ListAddonInstancesResponse$: StaticStructureSchema = [3, n0, _LAIRi,
  0,
  [_AId, _NT],
  [() => AddonInstances, 0]
];
export var ListAddonSubscriptionsRequest$: StaticStructureSchema = [3, n0, _LASR,
  0,
  [_NT, _PSa],
  [0, 1]
];
export var ListAddonSubscriptionsResponse$: StaticStructureSchema = [3, n0, _LASRi,
  0,
  [_ASd, _NT],
  [() => AddonSubscriptions, 0]
];
export var ListAddressListImportJobsRequest$: StaticStructureSchema = [3, n0, _LALIJR,
  0,
  [_ALI, _NT, _PSa],
  [0, 0, 1]
];
export var ListAddressListImportJobsResponse$: StaticStructureSchema = [3, n0, _LALIJRi,
  0,
  [_IJm, _NT],
  [[() => ImportJobs, 0], 0]
];
export var ListAddressListsRequest$: StaticStructureSchema = [3, n0, _LALR,
  0,
  [_NT, _PSa],
  [0, 1]
];
export var ListAddressListsResponse$: StaticStructureSchema = [3, n0, _LALRi,
  0,
  [_ALd, _NT],
  [() => AddressLists, 0]
];
export var ListArchiveExportsRequest$: StaticStructureSchema = [3, n0, _LAER,
  0,
  [_AIr, _NT, _PSa],
  [0, 0, 1]
];
export var ListArchiveExportsResponse$: StaticStructureSchema = [3, n0, _LAERi,
  0,
  [_Ex, _NT],
  [() => ExportSummaryList, 0]
];
export var ListArchiveSearchesRequest$: StaticStructureSchema = [3, n0, _LASRis,
  0,
  [_AIr, _NT, _PSa],
  [0, 0, 1]
];
export var ListArchiveSearchesResponse$: StaticStructureSchema = [3, n0, _LASRist,
  0,
  [_Se, _NT],
  [() => SearchSummaryList, 0]
];
export var ListArchivesRequest$: StaticStructureSchema = [3, n0, _LAR,
  0,
  [_NT, _PSa],
  [0, 1]
];
export var ListArchivesResponse$: StaticStructureSchema = [3, n0, _LARi,
  0,
  [_Arc, _NT],
  [() => ArchivesList, 0]
];
export var ListIngressPointsRequest$: StaticStructureSchema = [3, n0, _LIPR,
  0,
  [_PSa, _NT],
  [1, 0]
];
export var ListIngressPointsResponse$: StaticStructureSchema = [3, n0, _LIPRi,
  0,
  [_IPn, _NT],
  [() => IngressPointsList, 0]
];
export var ListMembersOfAddressListRequest$: StaticStructureSchema = [3, n0, _LMOALR,
  0,
  [_ALI, _Fil, _NT, _PSa],
  [0, [() => AddressFilter$, 0], 0, 1]
];
export var ListMembersOfAddressListResponse$: StaticStructureSchema = [3, n0, _LMOALRi,
  0,
  [_Ad, _NT],
  [[() => SavedAddresses, 0], 0]
];
export var ListRelaysRequest$: StaticStructureSchema = [3, n0, _LRR,
  0,
  [_PSa, _NT],
  [1, 0]
];
export var ListRelaysResponse$: StaticStructureSchema = [3, n0, _LRRi,
  0,
  [_Re, _NT],
  [() => Relays, 0]
];
export var ListRuleSetsRequest$: StaticStructureSchema = [3, n0, _LRSR,
  0,
  [_NT, _PSa],
  [0, 1]
];
export var ListRuleSetsResponse$: StaticStructureSchema = [3, n0, _LRSRi,
  0,
  [_RS, _NT],
  [() => RuleSets, 0]
];
export var ListTagsForResourceRequest$: StaticStructureSchema = [3, n0, _LTFRR,
  0,
  [_RAes],
  [0]
];
export var ListTagsForResourceResponse$: StaticStructureSchema = [3, n0, _LTFRRi,
  0,
  [_T],
  [() => TagList]
];
export var ListTrafficPoliciesRequest$: StaticStructureSchema = [3, n0, _LTPR,
  0,
  [_PSa, _NT],
  [1, 0]
];
export var ListTrafficPoliciesResponse$: StaticStructureSchema = [3, n0, _LTPRi,
  0,
  [_TP, _NT],
  [() => TrafficPolicyList, 0]
];
export var MessageBody$: StaticStructureSchema = [3, n0, _MB,
  0,
  [_Te, _Ht, _MM],
  [0, 0, 2]
];
export var Metadata$: StaticStructureSchema = [3, n0, _Me,
  0,
  [_Ti, _IPI, _TPI, _RSI, _SH, _SIA, _TCS, _TPl, _SM, _SIo, _SPen, _CS, _SAo],
  [4, 0, 0, 0, 0, [() => SenderIpAddress, 0], 0, 0, 0, 0, 0, 0, 0]
];
export var NoAuthentication$: StaticStructureSchema = [3, n0, _NA,
  0,
  [],
  []
];
export var PolicyStatement$: StaticStructureSchema = [3, n0, _PSo,
  0,
  [_C, _Ac],
  [() => PolicyConditions, 0]
];
export var PrivateNetworkConfiguration$: StaticStructureSchema = [3, n0, _PNC,
  0,
  [_VEI],
  [0]
];
export var PublicNetworkConfiguration$: StaticStructureSchema = [3, n0, _PNCu,
  0,
  [_IT],
  [0]
];
export var RegisterMemberToAddressListRequest$: StaticStructureSchema = [3, n0, _RMTALR,
  0,
  [_ALI, _A],
  [0, [() => Address, 0]]
];
export var RegisterMemberToAddressListResponse$: StaticStructureSchema = [3, n0, _RMTALRe,
  0,
  [],
  []
];
export var Relay$: StaticStructureSchema = [3, n0, _Rel,
  0,
  [_RI, _RN, _LMT],
  [0, 0, 4]
];
export var RelayAction$: StaticStructureSchema = [3, n0, _RAel,
  0,
  [_AFP, _Rel, _MF],
  [0, 0, 0]
];
export var ReplaceRecipientAction$: StaticStructureSchema = [3, n0, _RRA,
  0,
  [_RW],
  [[() => Recipients, 0]]
];
export var ResourceNotFoundException$: StaticErrorSchema = [-3, n0, _RNFE,
  { [_e]: _c, [_hE]: 404 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ResourceNotFoundException$, ResourceNotFoundException);
export var Row$: StaticStructureSchema = [3, n0, _Row,
  0,
  [_AMI, _RT, _D, _To, _F, _Cc, _Su, _MI, _HA, _RH, _IRT, _XM, _XOM, _XP, _IPI, _SH, _SIA, _En, _SAo],
  [0, 4, 0, 0, 0, 0, 0, 0, 2, 64 | 0, 0, 0, 0, 0, 0, 0, [() => SenderIpAddress, 0], () => Envelope$, 0]
];
export var Rule$: StaticStructureSchema = [3, n0, _Rul,
  0,
  [_N, _C, _U, _Act],
  [0, () => RuleConditions, () => RuleConditions, [() => RuleActions, 0]]
];
export var RuleBooleanExpression$: StaticStructureSchema = [3, n0, _RBE,
  0,
  [_E, _O],
  [() => RuleBooleanToEvaluate$, 0]
];
export var RuleDmarcExpression$: StaticStructureSchema = [3, n0, _RDE,
  0,
  [_O, _V],
  [0, 64 | 0]
];
export var RuleIpExpression$: StaticStructureSchema = [3, n0, _RIE,
  0,
  [_E, _O, _V],
  [() => RuleIpToEvaluate$, 0, 64 | 0]
];
export var RuleIsInAddressList$: StaticStructureSchema = [3, n0, _RIIAL,
  0,
  [_At, _ALd],
  [0, 64 | 0]
];
export var RuleNumberExpression$: StaticStructureSchema = [3, n0, _RNE,
  0,
  [_E, _O, _Va],
  [() => RuleNumberToEvaluate$, 0, 1]
];
export var RuleSet$: StaticStructureSchema = [3, n0, _RSu,
  0,
  [_RSI, _RSN, _LMD],
  [0, 0, 4]
];
export var RuleStringExpression$: StaticStructureSchema = [3, n0, _RSE,
  0,
  [_E, _O, _V],
  [() => RuleStringToEvaluate$, 0, 64 | 0]
];
export var RuleVerdictExpression$: StaticStructureSchema = [3, n0, _RVE,
  0,
  [_E, _O, _V],
  [() => RuleVerdictToEvaluate$, 0, 64 | 0]
];
export var S3Action$: StaticStructureSchema = [3, n0, _SAc,
  0,
  [_AFP, _RA, _SB, _SPr, _SSKKI],
  [0, 0, 0, 0, 0]
];
export var S3ExportDestinationConfiguration$: StaticStructureSchema = [3, n0, _SEDC,
  0,
  [_SL],
  [0]
];
export var SavedAddress$: StaticStructureSchema = [3, n0, _SAa,
  0,
  [_A, _CT],
  [[() => Address, 0], 4]
];
export var SearchStatus$: StaticStructureSchema = [3, n0, _SS,
  0,
  [_ST, _CTo, _S, _EM],
  [4, 4, 0, 0]
];
export var SearchSummary$: StaticStructureSchema = [3, n0, _SSe,
  0,
  [_SI, _St],
  [0, () => SearchStatus$]
];
export var SendAction$: StaticStructureSchema = [3, n0, _SAe,
  0,
  [_AFP, _RA],
  [0, 0]
];
export var ServiceQuotaExceededException$: StaticErrorSchema = [-3, n0, _SQEE,
  { [_e]: _c, [_hE]: 402 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ServiceQuotaExceededException$, ServiceQuotaExceededException);
export var SnsAction$: StaticStructureSchema = [3, n0, _SAn,
  0,
  [_AFP, _TAo, _RA, _Enc, _PT],
  [0, 0, 0, 0, 0]
];
export var StartAddressListImportJobRequest$: StaticStructureSchema = [3, n0, _SALIJR,
  0,
  [_JI],
  [0]
];
export var StartAddressListImportJobResponse$: StaticStructureSchema = [3, n0, _SALIJRt,
  0,
  [],
  []
];
export var StartArchiveExportRequest$: StaticStructureSchema = [3, n0, _SAER,
  0,
  [_AIr, _Fi, _FT, _TT, _MR, _EDC, _IM],
  [0, () => ArchiveFilters$, 4, 4, 1, () => ExportDestinationConfiguration$, 2]
];
export var StartArchiveExportResponse$: StaticStructureSchema = [3, n0, _SAERt,
  0,
  [_EI],
  [0]
];
export var StartArchiveSearchRequest$: StaticStructureSchema = [3, n0, _SASR,
  0,
  [_AIr, _Fi, _FT, _TT, _MR],
  [0, () => ArchiveFilters$, 4, 4, 1]
];
export var StartArchiveSearchResponse$: StaticStructureSchema = [3, n0, _SASRt,
  0,
  [_SI],
  [0]
];
export var StopAddressListImportJobRequest$: StaticStructureSchema = [3, n0, _SALIJRto,
  0,
  [_JI],
  [0]
];
export var StopAddressListImportJobResponse$: StaticStructureSchema = [3, n0, _SALIJRtop,
  0,
  [],
  []
];
export var StopArchiveExportRequest$: StaticStructureSchema = [3, n0, _SAERto,
  0,
  [_EI],
  [0]
];
export var StopArchiveExportResponse$: StaticStructureSchema = [3, n0, _SAERtop,
  0,
  [],
  []
];
export var StopArchiveSearchRequest$: StaticStructureSchema = [3, n0, _SASRto,
  0,
  [_SI],
  [0]
];
export var StopArchiveSearchResponse$: StaticStructureSchema = [3, n0, _SASRtop,
  0,
  [],
  []
];
export var Tag$: StaticStructureSchema = [3, n0, _Ta,
  0,
  [_K, _Va],
  [0, 0]
];
export var TagResourceRequest$: StaticStructureSchema = [3, n0, _TRR,
  0,
  [_RAes, _T],
  [0, () => TagList]
];
export var TagResourceResponse$: StaticStructureSchema = [3, n0, _TRRa,
  0,
  [],
  []
];
export var ThrottlingException$: StaticErrorSchema = [-3, n0, _TE,
  { [_e]: _c, [_hE]: 429 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ThrottlingException$, ThrottlingException);
export var TrafficPolicy$: StaticStructureSchema = [3, n0, _TPr,
  0,
  [_TPN, _TPI, _DA],
  [0, 0, 0]
];
export var UntagResourceRequest$: StaticStructureSchema = [3, n0, _URR,
  0,
  [_RAes, _TK],
  [0, 64 | 0]
];
export var UntagResourceResponse$: StaticStructureSchema = [3, n0, _URRn,
  0,
  [],
  []
];
export var UpdateArchiveRequest$: StaticStructureSchema = [3, n0, _UAR,
  0,
  [_AIr, _ANr, _R],
  [0, 0, () => ArchiveRetention$]
];
export var UpdateArchiveResponse$: StaticStructureSchema = [3, n0, _UARp,
  0,
  [],
  []
];
export var UpdateIngressPointRequest$: StaticStructureSchema = [3, n0, _UIPR,
  0,
  [_IPI, _IPN, _STU, _RSI, _TPI, _IPC],
  [0, 0, 0, 0, 0, [() => IngressPointConfiguration$, 0]]
];
export var UpdateIngressPointResponse$: StaticStructureSchema = [3, n0, _UIPRp,
  0,
  [],
  []
];
export var UpdateRelayRequest$: StaticStructureSchema = [3, n0, _URRp,
  0,
  [_RI, _RN, _SN, _SPe, _Au],
  [0, 0, 0, 1, () => RelayAuthentication$]
];
export var UpdateRelayResponse$: StaticStructureSchema = [3, n0, _URRpd,
  0,
  [],
  []
];
export var UpdateRuleSetRequest$: StaticStructureSchema = [3, n0, _URSR,
  0,
  [_RSI, _RSN, _Ru],
  [0, 0, [() => Rules, 0]]
];
export var UpdateRuleSetResponse$: StaticStructureSchema = [3, n0, _URSRp,
  0,
  [],
  []
];
export var UpdateTrafficPolicyRequest$: StaticStructureSchema = [3, n0, _UTPR,
  0,
  [_TPI, _TPN, _PS, _DA, _MMSB],
  [0, 0, () => PolicyStatementList, 0, 1]
];
export var UpdateTrafficPolicyResponse$: StaticStructureSchema = [3, n0, _UTPRp,
  0,
  [],
  []
];
export var ValidationException$: StaticErrorSchema = [-3, n0, _VE,
  { [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ValidationException$, ValidationException);
export var MailManagerServiceException$: StaticErrorSchema = [-3, _s, "MailManagerServiceException", 0, [], []];
TypeRegistry.for(_s).registerError(MailManagerServiceException$, MailManagerServiceException);
var AddonInstances: StaticListSchema = [1, n0, _AId,
  0, () => AddonInstance$
];
var AddonSubscriptions: StaticListSchema = [1, n0, _ASd,
  0, () => AddonSubscription$
];
var AddressLists: StaticListSchema = [1, n0, _ALd,
  0, () => AddressList$
];
var ArchiveFilterConditions: StaticListSchema = [1, n0, _AFC,
  0, () => ArchiveFilterCondition$
];
var ArchivesList: StaticListSchema = [1, n0, _ALr,
  0, () => Archive$
];
var EmailReceivedHeadersList = 64 | 0;
var ExportSummaryList: StaticListSchema = [1, n0, _ESL,
  0, () => ExportSummary$
];
var ImportJobs: StaticListSchema = [1, n0, _IJm,
  0, [() => ImportJob$,
    0]
];
var IngressAddressListArnList = 64 | 0;
var IngressPointsList: StaticListSchema = [1, n0, _IPL,
  0, () => IngressPoint$
];
var Ipv4Cidrs = 64 | 0;
var Ipv6Cidrs = 64 | 0;
var PolicyConditions: StaticListSchema = [1, n0, _PC,
  0, () => PolicyCondition$
];
var PolicyStatementList: StaticListSchema = [1, n0, _PSL,
  0, () => PolicyStatement$
];
var Recipients: StaticListSchema = [1, n0, _Rec,
  0, [() => EmailAddress,
    0]
];
var Relays: StaticListSchema = [1, n0, _Re,
  0, () => Relay$
];
var RowsList: StaticListSchema = [1, n0, _RL,
  0, [() => Row$,
    0]
];
var RuleActions: StaticListSchema = [1, n0, _RAu,
  0, [() => RuleAction$,
    0]
];
var RuleAddressListArnList = 64 | 0;
var RuleConditions: StaticListSchema = [1, n0, _RC,
  0, () => RuleCondition$
];
var RuleDmarcValueList = 64 | 0;
var RuleIpValueList = 64 | 0;
var Rules: StaticListSchema = [1, n0, _Ru,
  0, [() => Rule$,
    0]
];
var RuleSets: StaticListSchema = [1, n0, _RS,
  0, () => RuleSet$
];
var RuleStringList = 64 | 0;
var RuleVerdictValueList = 64 | 0;
var SavedAddresses: StaticListSchema = [1, n0, _SAav,
  0, [() => SavedAddress$,
    0]
];
var SearchSummaryList: StaticListSchema = [1, n0, _SSL,
  0, () => SearchSummary$
];
var StringList = 64 | 0;
var StringValueList = 64 | 0;
var TagKeyList = 64 | 0;
var TagList: StaticListSchema = [1, n0, _TL,
  0, () => Tag$
];
var TrafficPolicyList: StaticListSchema = [1, n0, _TPL,
  0, () => TrafficPolicy$
];
export var ArchiveBooleanToEvaluate$: StaticUnionSchema = [4, n0, _ABTE,
  0,
  [_At],
  [0]
];
export var ArchiveFilterCondition$: StaticUnionSchema = [4, n0, _AFCr,
  0,
  [_SE, _BE],
  [() => ArchiveStringExpression$, () => ArchiveBooleanExpression$]
];
export var ArchiveRetention$: StaticUnionSchema = [4, n0, _ARr,
  0,
  [_RP],
  [0]
];
export var ArchiveStringToEvaluate$: StaticUnionSchema = [4, n0, _ASTE,
  0,
  [_At],
  [0]
];
export var ExportDestinationConfiguration$: StaticUnionSchema = [4, n0, _EDC,
  0,
  [_S_],
  [() => S3ExportDestinationConfiguration$]
];
export var IngressBooleanToEvaluate$: StaticUnionSchema = [4, n0, _IBTE,
  0,
  [_An, _IIAL],
  [() => IngressAnalysis$, () => IngressIsInAddressList$]
];
export var IngressIpToEvaluate$: StaticUnionSchema = [4, n0, _IITE,
  0,
  [_At],
  [0]
];
export var IngressIpv6ToEvaluate$: StaticUnionSchema = [4, n0, _IITEn,
  0,
  [_At],
  [0]
];
export var IngressPointConfiguration$: StaticUnionSchema = [4, n0, _IPC,
  0,
  [_SP, _SA],
  [[() => SmtpPassword, 0], 0]
];
export var IngressStringToEvaluate$: StaticUnionSchema = [4, n0, _ISTE,
  0,
  [_At, _An],
  [0, () => IngressAnalysis$]
];
export var IngressTlsProtocolToEvaluate$: StaticUnionSchema = [4, n0, _ITPTE,
  0,
  [_At],
  [0]
];
export var NetworkConfiguration$: StaticUnionSchema = [4, n0, _NC,
  0,
  [_PNCu, _PNC],
  [() => PublicNetworkConfiguration$, () => PrivateNetworkConfiguration$]
];
export var PolicyCondition$: StaticUnionSchema = [4, n0, _PCo,
  0,
  [_SE, _IE, _IEp, _TEl, _BE],
  [() => IngressStringExpression$, () => IngressIpv4Expression$, () => IngressIpv6Expression$, () => IngressTlsProtocolExpression$, () => IngressBooleanExpression$]
];
export var RelayAuthentication$: StaticUnionSchema = [4, n0, _RAela,
  0,
  [_SA, _NA],
  [0, () => NoAuthentication$]
];
export var RuleAction$: StaticUnionSchema = [4, n0, _RAul,
  0,
  [_Dr, _Rel, _Ar, _WTS, _Sen, _AH, _RR, _DTM, _DTQB, _PTS],
  [() => DropAction$, () => RelayAction$, () => ArchiveAction$, () => S3Action$, () => SendAction$, () => AddHeaderAction$, [() => ReplaceRecipientAction$, 0], () => DeliverToMailboxAction$, () => DeliverToQBusinessAction$, () => SnsAction$]
];
export var RuleBooleanToEvaluate$: StaticUnionSchema = [4, n0, _RBTE,
  0,
  [_At, _An, _IIAL],
  [0, () => Analysis$, () => RuleIsInAddressList$]
];
export var RuleCondition$: StaticUnionSchema = [4, n0, _RCu,
  0,
  [_BE, _SE, _NE, _IE, _VEe, _DE],
  [() => RuleBooleanExpression$, () => RuleStringExpression$, () => RuleNumberExpression$, () => RuleIpExpression$, () => RuleVerdictExpression$, () => RuleDmarcExpression$]
];
export var RuleIpToEvaluate$: StaticUnionSchema = [4, n0, _RITE,
  0,
  [_At],
  [0]
];
export var RuleNumberToEvaluate$: StaticUnionSchema = [4, n0, _RNTE,
  0,
  [_At],
  [0]
];
export var RuleStringToEvaluate$: StaticUnionSchema = [4, n0, _RSTE,
  0,
  [_At, _MHA, _An],
  [0, 0, () => Analysis$]
];
export var RuleVerdictToEvaluate$: StaticUnionSchema = [4, n0, _RVTE,
  0,
  [_At, _An],
  [0, () => Analysis$]
];
export var CreateAddonInstance$: StaticOperationSchema = [9, n0, _CAI,
  2, () => CreateAddonInstanceRequest$, () => CreateAddonInstanceResponse$
];
export var CreateAddonSubscription$: StaticOperationSchema = [9, n0, _CAS,
  2, () => CreateAddonSubscriptionRequest$, () => CreateAddonSubscriptionResponse$
];
export var CreateAddressList$: StaticOperationSchema = [9, n0, _CAL,
  2, () => CreateAddressListRequest$, () => CreateAddressListResponse$
];
export var CreateAddressListImportJob$: StaticOperationSchema = [9, n0, _CALIJ,
  2, () => CreateAddressListImportJobRequest$, () => CreateAddressListImportJobResponse$
];
export var CreateArchive$: StaticOperationSchema = [9, n0, _CA,
  2, () => CreateArchiveRequest$, () => CreateArchiveResponse$
];
export var CreateIngressPoint$: StaticOperationSchema = [9, n0, _CIP,
  2, () => CreateIngressPointRequest$, () => CreateIngressPointResponse$
];
export var CreateRelay$: StaticOperationSchema = [9, n0, _CR,
  2, () => CreateRelayRequest$, () => CreateRelayResponse$
];
export var CreateRuleSet$: StaticOperationSchema = [9, n0, _CRS,
  2, () => CreateRuleSetRequest$, () => CreateRuleSetResponse$
];
export var CreateTrafficPolicy$: StaticOperationSchema = [9, n0, _CTP,
  2, () => CreateTrafficPolicyRequest$, () => CreateTrafficPolicyResponse$
];
export var DeleteAddonInstance$: StaticOperationSchema = [9, n0, _DAI,
  2, () => DeleteAddonInstanceRequest$, () => DeleteAddonInstanceResponse$
];
export var DeleteAddonSubscription$: StaticOperationSchema = [9, n0, _DAS,
  2, () => DeleteAddonSubscriptionRequest$, () => DeleteAddonSubscriptionResponse$
];
export var DeleteAddressList$: StaticOperationSchema = [9, n0, _DAL,
  2, () => DeleteAddressListRequest$, () => DeleteAddressListResponse$
];
export var DeleteArchive$: StaticOperationSchema = [9, n0, _DAe,
  2, () => DeleteArchiveRequest$, () => DeleteArchiveResponse$
];
export var DeleteIngressPoint$: StaticOperationSchema = [9, n0, _DIP,
  2, () => DeleteIngressPointRequest$, () => DeleteIngressPointResponse$
];
export var DeleteRelay$: StaticOperationSchema = [9, n0, _DR,
  2, () => DeleteRelayRequest$, () => DeleteRelayResponse$
];
export var DeleteRuleSet$: StaticOperationSchema = [9, n0, _DRS,
  2, () => DeleteRuleSetRequest$, () => DeleteRuleSetResponse$
];
export var DeleteTrafficPolicy$: StaticOperationSchema = [9, n0, _DTP,
  2, () => DeleteTrafficPolicyRequest$, () => DeleteTrafficPolicyResponse$
];
export var DeregisterMemberFromAddressList$: StaticOperationSchema = [9, n0, _DMFAL,
  2, () => DeregisterMemberFromAddressListRequest$, () => DeregisterMemberFromAddressListResponse$
];
export var GetAddonInstance$: StaticOperationSchema = [9, n0, _GAI,
  0, () => GetAddonInstanceRequest$, () => GetAddonInstanceResponse$
];
export var GetAddonSubscription$: StaticOperationSchema = [9, n0, _GAS,
  0, () => GetAddonSubscriptionRequest$, () => GetAddonSubscriptionResponse$
];
export var GetAddressList$: StaticOperationSchema = [9, n0, _GAL,
  0, () => GetAddressListRequest$, () => GetAddressListResponse$
];
export var GetAddressListImportJob$: StaticOperationSchema = [9, n0, _GALIJ,
  0, () => GetAddressListImportJobRequest$, () => GetAddressListImportJobResponse$
];
export var GetArchive$: StaticOperationSchema = [9, n0, _GA,
  0, () => GetArchiveRequest$, () => GetArchiveResponse$
];
export var GetArchiveExport$: StaticOperationSchema = [9, n0, _GAE,
  0, () => GetArchiveExportRequest$, () => GetArchiveExportResponse$
];
export var GetArchiveMessage$: StaticOperationSchema = [9, n0, _GAM,
  0, () => GetArchiveMessageRequest$, () => GetArchiveMessageResponse$
];
export var GetArchiveMessageContent$: StaticOperationSchema = [9, n0, _GAMC,
  0, () => GetArchiveMessageContentRequest$, () => GetArchiveMessageContentResponse$
];
export var GetArchiveSearch$: StaticOperationSchema = [9, n0, _GASe,
  0, () => GetArchiveSearchRequest$, () => GetArchiveSearchResponse$
];
export var GetArchiveSearchResults$: StaticOperationSchema = [9, n0, _GASRetrc,
  0, () => GetArchiveSearchResultsRequest$, () => GetArchiveSearchResultsResponse$
];
export var GetIngressPoint$: StaticOperationSchema = [9, n0, _GIP,
  0, () => GetIngressPointRequest$, () => GetIngressPointResponse$
];
export var GetMemberOfAddressList$: StaticOperationSchema = [9, n0, _GMOAL,
  0, () => GetMemberOfAddressListRequest$, () => GetMemberOfAddressListResponse$
];
export var GetRelay$: StaticOperationSchema = [9, n0, _GR,
  0, () => GetRelayRequest$, () => GetRelayResponse$
];
export var GetRuleSet$: StaticOperationSchema = [9, n0, _GRS,
  0, () => GetRuleSetRequest$, () => GetRuleSetResponse$
];
export var GetTrafficPolicy$: StaticOperationSchema = [9, n0, _GTP,
  0, () => GetTrafficPolicyRequest$, () => GetTrafficPolicyResponse$
];
export var ListAddonInstances$: StaticOperationSchema = [9, n0, _LAI,
  0, () => ListAddonInstancesRequest$, () => ListAddonInstancesResponse$
];
export var ListAddonSubscriptions$: StaticOperationSchema = [9, n0, _LAS,
  0, () => ListAddonSubscriptionsRequest$, () => ListAddonSubscriptionsResponse$
];
export var ListAddressListImportJobs$: StaticOperationSchema = [9, n0, _LALIJ,
  0, () => ListAddressListImportJobsRequest$, () => ListAddressListImportJobsResponse$
];
export var ListAddressLists$: StaticOperationSchema = [9, n0, _LAL,
  0, () => ListAddressListsRequest$, () => ListAddressListsResponse$
];
export var ListArchiveExports$: StaticOperationSchema = [9, n0, _LAE,
  0, () => ListArchiveExportsRequest$, () => ListArchiveExportsResponse$
];
export var ListArchives$: StaticOperationSchema = [9, n0, _LA,
  0, () => ListArchivesRequest$, () => ListArchivesResponse$
];
export var ListArchiveSearches$: StaticOperationSchema = [9, n0, _LASi,
  0, () => ListArchiveSearchesRequest$, () => ListArchiveSearchesResponse$
];
export var ListIngressPoints$: StaticOperationSchema = [9, n0, _LIP,
  0, () => ListIngressPointsRequest$, () => ListIngressPointsResponse$
];
export var ListMembersOfAddressList$: StaticOperationSchema = [9, n0, _LMOAL,
  0, () => ListMembersOfAddressListRequest$, () => ListMembersOfAddressListResponse$
];
export var ListRelays$: StaticOperationSchema = [9, n0, _LR,
  0, () => ListRelaysRequest$, () => ListRelaysResponse$
];
export var ListRuleSets$: StaticOperationSchema = [9, n0, _LRS,
  0, () => ListRuleSetsRequest$, () => ListRuleSetsResponse$
];
export var ListTagsForResource$: StaticOperationSchema = [9, n0, _LTFR,
  0, () => ListTagsForResourceRequest$, () => ListTagsForResourceResponse$
];
export var ListTrafficPolicies$: StaticOperationSchema = [9, n0, _LTP,
  0, () => ListTrafficPoliciesRequest$, () => ListTrafficPoliciesResponse$
];
export var RegisterMemberToAddressList$: StaticOperationSchema = [9, n0, _RMTAL,
  2, () => RegisterMemberToAddressListRequest$, () => RegisterMemberToAddressListResponse$
];
export var StartAddressListImportJob$: StaticOperationSchema = [9, n0, _SALIJ,
  2, () => StartAddressListImportJobRequest$, () => StartAddressListImportJobResponse$
];
export var StartArchiveExport$: StaticOperationSchema = [9, n0, _SAE,
  0, () => StartArchiveExportRequest$, () => StartArchiveExportResponse$
];
export var StartArchiveSearch$: StaticOperationSchema = [9, n0, _SAS,
  0, () => StartArchiveSearchRequest$, () => StartArchiveSearchResponse$
];
export var StopAddressListImportJob$: StaticOperationSchema = [9, n0, _SALIJt,
  2, () => StopAddressListImportJobRequest$, () => StopAddressListImportJobResponse$
];
export var StopArchiveExport$: StaticOperationSchema = [9, n0, _SAEt,
  0, () => StopArchiveExportRequest$, () => StopArchiveExportResponse$
];
export var StopArchiveSearch$: StaticOperationSchema = [9, n0, _SASt,
  2, () => StopArchiveSearchRequest$, () => StopArchiveSearchResponse$
];
export var TagResource$: StaticOperationSchema = [9, n0, _TR,
  2, () => TagResourceRequest$, () => TagResourceResponse$
];
export var UntagResource$: StaticOperationSchema = [9, n0, _UR,
  2, () => UntagResourceRequest$, () => UntagResourceResponse$
];
export var UpdateArchive$: StaticOperationSchema = [9, n0, _UA,
  2, () => UpdateArchiveRequest$, () => UpdateArchiveResponse$
];
export var UpdateIngressPoint$: StaticOperationSchema = [9, n0, _UIP,
  2, () => UpdateIngressPointRequest$, () => UpdateIngressPointResponse$
];
export var UpdateRelay$: StaticOperationSchema = [9, n0, _URp,
  2, () => UpdateRelayRequest$, () => UpdateRelayResponse$
];
export var UpdateRuleSet$: StaticOperationSchema = [9, n0, _URS,
  2, () => UpdateRuleSetRequest$, () => UpdateRuleSetResponse$
];
export var UpdateTrafficPolicy$: StaticOperationSchema = [9, n0, _UTP,
  2, () => UpdateTrafficPolicyRequest$, () => UpdateTrafficPolicyResponse$
];
