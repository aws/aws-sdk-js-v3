const _B = "Body";
const _BRE = "BadRequestException";
const _C = "Code";
const _CA = "CrossAccount";
const _CD = "CreationDate";
const _CDR = "CreateDiscovererRequest";
const _CDRr = "CreateDiscovererResponse";
const _CDr = "CreatedDate";
const _CDre = "CreateDiscoverer";
const _CE = "ConflictException";
const _CR = "CreateRegistry";
const _CRR = "CreateRegistryRequest";
const _CRRr = "CreateRegistryResponse";
const _CS = "CreateSchema";
const _CSR = "CreateSchemaRequest";
const _CSRr = "CreateSchemaResponse";
const _CTI = "ClientTokenId";
const _Co = "Content";
const _D = "Description";
const _DA = "DiscovererArn";
const _DCB = "DescribeCodeBinding";
const _DCBR = "DescribeCodeBindingRequest";
const _DCBRe = "DescribeCodeBindingResponse";
const _DD = "DeleteDiscoverer";
const _DDR = "DeleteDiscovererRequest";
const _DDRe = "DescribeDiscovererRequest";
const _DDRes = "DescribeDiscovererResponse";
const _DDe = "DescribeDiscoverer";
const _DI = "DiscovererId";
const _DIP = "DiscovererIdPrefix";
const _DR = "DeleteRegistry";
const _DRP = "DeleteResourcePolicy";
const _DRPR = "DeleteResourcePolicyRequest";
const _DRR = "DeleteRegistryRequest";
const _DRRe = "DescribeRegistryRequest";
const _DRRes = "DescribeRegistryResponse";
const _DRe = "DescribeRegistry";
const _DS = "DiscovererSummary";
const _DSR = "DeleteSchemaRequest";
const _DSRe = "DescribeSchemaRequest";
const _DSRes = "DescribeSchemaResponse";
const _DSV = "DeleteSchemaVersion";
const _DSVR = "DeleteSchemaVersionRequest";
const _DSe = "DeleteSchema";
const _DSes = "DescribeSchema";
const _Di = "Discoverers";
const _E = "Events";
const _ES = "ExportSchema";
const _ESR = "ExportSchemaRequest";
const _ESRx = "ExportSchemaResponse";
const _FE = "ForbiddenException";
const _GCBS = "GetCodeBindingSource";
const _GCBSR = "GetCodeBindingSourceRequest";
const _GCBSRe = "GetCodeBindingSourceResponse";
const _GDS = "GetDiscoveredSchema";
const _GDSR = "GetDiscoveredSchemaRequest";
const _GDSRe = "GetDiscoveredSchemaResponse";
const _GE = "GoneException";
const _GRP = "GetResourcePolicy";
const _GRPR = "GetResourcePolicyRequest";
const _GRPRe = "GetResourcePolicyResponse";
const _ISEE = "InternalServerErrorException";
const _K = "Keywords";
const _L = "Language";
const _LD = "ListDiscoverers";
const _LDR = "ListDiscoverersRequest";
const _LDRi = "ListDiscoverersResponse";
const _LM = "LastModified";
const _LR = "ListRegistries";
const _LRR = "ListRegistriesRequest";
const _LRRi = "ListRegistriesResponse";
const _LS = "ListSchemas";
const _LSR = "ListSchemasRequest";
const _LSRi = "ListSchemasResponse";
const _LSV = "ListSchemaVersions";
const _LSVR = "ListSchemaVersionsRequest";
const _LSVRi = "ListSchemaVersionsResponse";
const _LTFR = "ListTagsForResource";
const _LTFRR = "ListTagsForResourceRequest";
const _LTFRRi = "ListTagsForResourceResponse";
const _Li = "Limit";
const _M = "Message";
const _NFE = "NotFoundException";
const _NT = "NextToken";
const _P = "Policy";
const _PCB = "PutCodeBinding";
const _PCBR = "PutCodeBindingRequest";
const _PCBRu = "PutCodeBindingResponse";
const _PFE = "PreconditionFailedException";
const _PRP = "PutResourcePolicy";
const _PRPR = "PutResourcePolicyRequest";
const _PRPRu = "PutResourcePolicyResponse";
const _R = "Registries";
const _RA = "RegistryArn";
const _RAe = "ResourceArn";
const _RI = "RevisionId";
const _RN = "RegistryName";
const _RNP = "RegistryNamePrefix";
const _RS = "RegistrySummary";
const _S = "State";
const _SA = "SourceArn";
const _SAP = "SourceArnPrefix";
const _SAc = "SchemaArn";
const _SD = "StartDiscoverer";
const _SDR = "StartDiscovererRequest";
const _SDRt = "StartDiscovererResponse";
const _SDRto = "StopDiscovererRequest";
const _SDRtop = "StopDiscovererResponse";
const _SDt = "StopDiscoverer";
const _SN = "SchemaName";
const _SNP = "SchemaNamePrefix";
const _SS = "SchemaSummary";
const _SSR = "SearchSchemasRequest";
const _SSRe = "SearchSchemasResponse";
const _SSS = "SearchSchemaSummary";
const _SSVS = "SearchSchemaVersionSummary";
const _SSe = "SearchSchemas";
const _SUE = "ServiceUnavailableException";
const _SV = "SchemaVersion";
const _SVS = "SchemaVersionSummary";
const _SVc = "SchemaVersions";
const _Sc = "Scope";
const _Sch = "Schemas";
const _Ss = "SynthesizedJson__string";
const _St = "Status";
const _T = "Tags";
const _TK = "TagKeys";
const _TMRE = "TooManyRequestsException";
const _TR = "TagResource";
const _TRR = "TagResourceRequest";
const _Ty = "Type";
const _UD = "UpdateDiscoverer";
const _UDR = "UpdateDiscovererRequest";
const _UDRp = "UpdateDiscovererResponse";
const _UE = "UnauthorizedException";
const _UR = "UntagResource";
const _URR = "UntagResourceRequest";
const _URRp = "UpdateRegistryRequest";
const _URRpd = "UpdateRegistryResponse";
const _URp = "UpdateRegistry";
const _US = "UpdateSchema";
const _USR = "UpdateSchemaRequest";
const _USRp = "UpdateSchemaResponse";
const _VC = "VersionCount";
const _VCD = "VersionCreatedDate";
const _a = "application/json";
const _c = "client";
const _dIP = "discovererIdPrefix";
const _e = "error";
const _h = "http";
const _hE = "httpError";
const _hQ = "httpQuery";
const _jN = "jsonName";
const _k = "keywords";
const _l = "limit";
const _lODS = "__listOfDiscovererSummary";
const _lORS = "__listOfRegistrySummary";
const _lOSS = "__listOfSchemaSummary";
const _lOSSS = "__listOfSearchSchemaSummary";
const _lOSSVS = "__listOfSearchSchemaVersionSummary";
const _lOSVS = "__listOfSchemaVersionSummary";
const _mT = "mediaType";
const _nT = "nextToken";
const _rN = "registryName";
const _rNP = "registryNamePrefix";
const _s = "server";
const _sAP = "sourceArnPrefix";
const _sNP = "schemaNamePrefix";
const _sV = "schemaVersion";
const _sc = "scope";
const _sm = "smithy.ts.sdk.synthetic.com.amazonaws.schemas";
const _t = "tags";
const _tK = "tagKeys";
const _ty = "type";
const n0 = "com.amazonaws.schemas";

// smithy-typescript generated code
import { TypeRegistry } from "@smithy/core/schema";
import type {
  StaticErrorSchema,
  StaticListSchema,
  StaticMapSchema,
  StaticOperationSchema,
  StaticSimpleSchema,
  StaticStructureSchema,
} from "@smithy/types";

import {
  BadRequestException,
  ConflictException,
  ForbiddenException,
  GoneException,
  InternalServerErrorException,
  NotFoundException,
  PreconditionFailedException,
  ServiceUnavailableException,
  TooManyRequestsException,
  UnauthorizedException,
} from "../models/errors";
import { SchemasServiceException } from "../models/SchemasServiceException";

/* eslint no-var: 0 */
var SynthesizedJson__string: StaticSimpleSchema = [0, n0, _Ss, { [_mT]: _a }, 0];
export var BadRequestException$: StaticErrorSchema = [-3, n0, _BRE,
  { [_e]: _c, [_hE]: 400 },
  [_C, _M],
  [0, 0]
];
TypeRegistry.for(n0).registerError(BadRequestException$, BadRequestException);
export var ConflictException$: StaticErrorSchema = [-3, n0, _CE,
  { [_e]: _c, [_hE]: 409 },
  [_C, _M],
  [0, 0]
];
TypeRegistry.for(n0).registerError(ConflictException$, ConflictException);
export var CreateDiscovererRequest$: StaticStructureSchema = [3, n0, _CDR,
  0,
  [_D, _SA, _CA, _T],
  [0, 0, 2, [128 | 0, { [_jN]: _t }]]
];
export var CreateDiscovererResponse$: StaticStructureSchema = [3, n0, _CDRr,
  0,
  [_D, _DA, _DI, _SA, _S, _CA, _T],
  [0, 0, 0, 0, 0, 2, [128 | 0, { [_jN]: _t }]]
];
export var CreateRegistryRequest$: StaticStructureSchema = [3, n0, _CRR,
  0,
  [_D, _RN, _T],
  [0, [0, 1], [128 | 0, { [_jN]: _t }]]
];
export var CreateRegistryResponse$: StaticStructureSchema = [3, n0, _CRRr,
  0,
  [_D, _RA, _RN, _T],
  [0, 0, 0, [128 | 0, { [_jN]: _t }]]
];
export var CreateSchemaRequest$: StaticStructureSchema = [3, n0, _CSR,
  0,
  [_Co, _D, _RN, _SN, _T, _Ty],
  [0, 0, [0, 1], [0, 1], [128 | 0, { [_jN]: _t }], 0]
];
export var CreateSchemaResponse$: StaticStructureSchema = [3, n0, _CSRr,
  0,
  [_D, _LM, _SAc, _SN, _SV, _T, _Ty, _VCD],
  [0, 5, 0, 0, 0, [128 | 0, { [_jN]: _t }], 0, 5]
];
export var DeleteDiscovererRequest$: StaticStructureSchema = [3, n0, _DDR,
  0,
  [_DI],
  [[0, 1]]
];
export var DeleteRegistryRequest$: StaticStructureSchema = [3, n0, _DRR,
  0,
  [_RN],
  [[0, 1]]
];
export var DeleteResourcePolicyRequest$: StaticStructureSchema = [3, n0, _DRPR,
  0,
  [_RN],
  [[0, { [_hQ]: _rN }]]
];
export var DeleteSchemaRequest$: StaticStructureSchema = [3, n0, _DSR,
  0,
  [_RN, _SN],
  [[0, 1], [0, 1]]
];
export var DeleteSchemaVersionRequest$: StaticStructureSchema = [3, n0, _DSVR,
  0,
  [_RN, _SN, _SV],
  [[0, 1], [0, 1], [0, 1]]
];
export var DescribeCodeBindingRequest$: StaticStructureSchema = [3, n0, _DCBR,
  0,
  [_L, _RN, _SN, _SV],
  [[0, 1], [0, 1], [0, 1], [0, { [_hQ]: _sV }]]
];
export var DescribeCodeBindingResponse$: StaticStructureSchema = [3, n0, _DCBRe,
  0,
  [_CD, _LM, _SV, _St],
  [5, 5, 0, 0]
];
export var DescribeDiscovererRequest$: StaticStructureSchema = [3, n0, _DDRe,
  0,
  [_DI],
  [[0, 1]]
];
export var DescribeDiscovererResponse$: StaticStructureSchema = [3, n0, _DDRes,
  0,
  [_D, _DA, _DI, _SA, _S, _CA, _T],
  [0, 0, 0, 0, 0, 2, [128 | 0, { [_jN]: _t }]]
];
export var DescribeRegistryRequest$: StaticStructureSchema = [3, n0, _DRRe,
  0,
  [_RN],
  [[0, 1]]
];
export var DescribeRegistryResponse$: StaticStructureSchema = [3, n0, _DRRes,
  0,
  [_D, _RA, _RN, _T],
  [0, 0, 0, [128 | 0, { [_jN]: _t }]]
];
export var DescribeSchemaRequest$: StaticStructureSchema = [3, n0, _DSRe,
  0,
  [_RN, _SN, _SV],
  [[0, 1], [0, 1], [0, { [_hQ]: _sV }]]
];
export var DescribeSchemaResponse$: StaticStructureSchema = [3, n0, _DSRes,
  0,
  [_Co, _D, _LM, _SAc, _SN, _SV, _T, _Ty, _VCD],
  [0, 0, 5, 0, 0, 0, [128 | 0, { [_jN]: _t }], 0, 5]
];
export var DiscovererSummary$: StaticStructureSchema = [3, n0, _DS,
  0,
  [_DA, _DI, _SA, _S, _CA, _T],
  [0, 0, 0, 0, 2, [128 | 0, { [_jN]: _t }]]
];
export var ExportSchemaRequest$: StaticStructureSchema = [3, n0, _ESR,
  0,
  [_RN, _SN, _SV, _Ty],
  [[0, 1], [0, 1], [0, { [_hQ]: _sV }], [0, { [_hQ]: _ty }]]
];
export var ExportSchemaResponse$: StaticStructureSchema = [3, n0, _ESRx,
  0,
  [_Co, _SAc, _SN, _SV, _Ty],
  [0, 0, 0, 0, 0]
];
export var ForbiddenException$: StaticErrorSchema = [-3, n0, _FE,
  { [_e]: _c, [_hE]: 403 },
  [_C, _M],
  [0, 0]
];
TypeRegistry.for(n0).registerError(ForbiddenException$, ForbiddenException);
export var GetCodeBindingSourceRequest$: StaticStructureSchema = [3, n0, _GCBSR,
  0,
  [_L, _RN, _SN, _SV],
  [[0, 1], [0, 1], [0, 1], [0, { [_hQ]: _sV }]]
];
export var GetCodeBindingSourceResponse$: StaticStructureSchema = [3, n0, _GCBSRe,
  0,
  [_B],
  [[21, 16]]
];
export var GetDiscoveredSchemaRequest$: StaticStructureSchema = [3, n0, _GDSR,
  0,
  [_E, _Ty],
  [64 | 0, 0]
];
export var GetDiscoveredSchemaResponse$: StaticStructureSchema = [3, n0, _GDSRe,
  0,
  [_Co],
  [0]
];
export var GetResourcePolicyRequest$: StaticStructureSchema = [3, n0, _GRPR,
  0,
  [_RN],
  [[0, { [_hQ]: _rN }]]
];
export var GetResourcePolicyResponse$: StaticStructureSchema = [3, n0, _GRPRe,
  0,
  [_P, _RI],
  [[() => SynthesizedJson__string, 0], 0]
];
export var GoneException$: StaticErrorSchema = [-3, n0, _GE,
  { [_e]: _c, [_hE]: 410 },
  [_C, _M],
  [0, 0]
];
TypeRegistry.for(n0).registerError(GoneException$, GoneException);
export var InternalServerErrorException$: StaticErrorSchema = [-3, n0, _ISEE,
  { [_e]: _s, [_hE]: 500 },
  [_C, _M],
  [0, 0]
];
TypeRegistry.for(n0).registerError(InternalServerErrorException$, InternalServerErrorException);
export var ListDiscoverersRequest$: StaticStructureSchema = [3, n0, _LDR,
  0,
  [_DIP, _Li, _NT, _SAP],
  [[0, { [_hQ]: _dIP }], [1, { [_hQ]: _l }], [0, { [_hQ]: _nT }], [0, { [_hQ]: _sAP }]]
];
export var ListDiscoverersResponse$: StaticStructureSchema = [3, n0, _LDRi,
  0,
  [_Di, _NT],
  [[() => __listOfDiscovererSummary, 0], 0]
];
export var ListRegistriesRequest$: StaticStructureSchema = [3, n0, _LRR,
  0,
  [_Li, _NT, _RNP, _Sc],
  [[1, { [_hQ]: _l }], [0, { [_hQ]: _nT }], [0, { [_hQ]: _rNP }], [0, { [_hQ]: _sc }]]
];
export var ListRegistriesResponse$: StaticStructureSchema = [3, n0, _LRRi,
  0,
  [_NT, _R],
  [0, [() => __listOfRegistrySummary, 0]]
];
export var ListSchemasRequest$: StaticStructureSchema = [3, n0, _LSR,
  0,
  [_Li, _NT, _RN, _SNP],
  [[1, { [_hQ]: _l }], [0, { [_hQ]: _nT }], [0, 1], [0, { [_hQ]: _sNP }]]
];
export var ListSchemasResponse$: StaticStructureSchema = [3, n0, _LSRi,
  0,
  [_NT, _Sch],
  [0, [() => __listOfSchemaSummary, 0]]
];
export var ListSchemaVersionsRequest$: StaticStructureSchema = [3, n0, _LSVR,
  0,
  [_Li, _NT, _RN, _SN],
  [[1, { [_hQ]: _l }], [0, { [_hQ]: _nT }], [0, 1], [0, 1]]
];
export var ListSchemaVersionsResponse$: StaticStructureSchema = [3, n0, _LSVRi,
  0,
  [_NT, _SVc],
  [0, () => __listOfSchemaVersionSummary]
];
export var ListTagsForResourceRequest$: StaticStructureSchema = [3, n0, _LTFRR,
  0,
  [_RAe],
  [[0, 1]]
];
export var ListTagsForResourceResponse$: StaticStructureSchema = [3, n0, _LTFRRi,
  0,
  [_T],
  [[128 | 0, { [_jN]: _t }]]
];
export var NotFoundException$: StaticErrorSchema = [-3, n0, _NFE,
  { [_e]: _c, [_hE]: 404 },
  [_C, _M],
  [0, 0]
];
TypeRegistry.for(n0).registerError(NotFoundException$, NotFoundException);
export var PreconditionFailedException$: StaticErrorSchema = [-3, n0, _PFE,
  { [_e]: _c, [_hE]: 412 },
  [_C, _M],
  [0, 0]
];
TypeRegistry.for(n0).registerError(PreconditionFailedException$, PreconditionFailedException);
export var PutCodeBindingRequest$: StaticStructureSchema = [3, n0, _PCBR,
  0,
  [_L, _RN, _SN, _SV],
  [[0, 1], [0, 1], [0, 1], [0, { [_hQ]: _sV }]]
];
export var PutCodeBindingResponse$: StaticStructureSchema = [3, n0, _PCBRu,
  0,
  [_CD, _LM, _SV, _St],
  [5, 5, 0, 0]
];
export var PutResourcePolicyRequest$: StaticStructureSchema = [3, n0, _PRPR,
  0,
  [_P, _RN, _RI],
  [[() => SynthesizedJson__string, 0], [0, { [_hQ]: _rN }], 0]
];
export var PutResourcePolicyResponse$: StaticStructureSchema = [3, n0, _PRPRu,
  0,
  [_P, _RI],
  [[() => SynthesizedJson__string, 0], 0]
];
export var RegistrySummary$: StaticStructureSchema = [3, n0, _RS,
  0,
  [_RA, _RN, _T],
  [0, 0, [128 | 0, { [_jN]: _t }]]
];
export var SchemaSummary$: StaticStructureSchema = [3, n0, _SS,
  0,
  [_LM, _SAc, _SN, _T, _VC],
  [5, 0, 0, [128 | 0, { [_jN]: _t }], 1]
];
export var SchemaVersionSummary$: StaticStructureSchema = [3, n0, _SVS,
  0,
  [_SAc, _SN, _SV, _Ty],
  [0, 0, 0, 0]
];
export var SearchSchemasRequest$: StaticStructureSchema = [3, n0, _SSR,
  0,
  [_K, _Li, _NT, _RN],
  [[0, { [_hQ]: _k }], [1, { [_hQ]: _l }], [0, { [_hQ]: _nT }], [0, 1]]
];
export var SearchSchemasResponse$: StaticStructureSchema = [3, n0, _SSRe,
  0,
  [_NT, _Sch],
  [0, () => __listOfSearchSchemaSummary]
];
export var SearchSchemaSummary$: StaticStructureSchema = [3, n0, _SSS,
  0,
  [_RN, _SAc, _SN, _SVc],
  [0, 0, 0, () => __listOfSearchSchemaVersionSummary]
];
export var SearchSchemaVersionSummary$: StaticStructureSchema = [3, n0, _SSVS,
  0,
  [_CDr, _SV, _Ty],
  [5, 0, 0]
];
export var ServiceUnavailableException$: StaticErrorSchema = [-3, n0, _SUE,
  { [_e]: _s, [_hE]: 503 },
  [_C, _M],
  [0, 0]
];
TypeRegistry.for(n0).registerError(ServiceUnavailableException$, ServiceUnavailableException);
export var StartDiscovererRequest$: StaticStructureSchema = [3, n0, _SDR,
  0,
  [_DI],
  [[0, 1]]
];
export var StartDiscovererResponse$: StaticStructureSchema = [3, n0, _SDRt,
  0,
  [_DI, _S],
  [0, 0]
];
export var StopDiscovererRequest$: StaticStructureSchema = [3, n0, _SDRto,
  0,
  [_DI],
  [[0, 1]]
];
export var StopDiscovererResponse$: StaticStructureSchema = [3, n0, _SDRtop,
  0,
  [_DI, _S],
  [0, 0]
];
export var TagResourceRequest$: StaticStructureSchema = [3, n0, _TRR,
  0,
  [_RAe, _T],
  [[0, 1], [128 | 0, { [_jN]: _t }]]
];
export var TooManyRequestsException$: StaticErrorSchema = [-3, n0, _TMRE,
  { [_e]: _c, [_hE]: 429 },
  [_C, _M],
  [0, 0]
];
TypeRegistry.for(n0).registerError(TooManyRequestsException$, TooManyRequestsException);
export var UnauthorizedException$: StaticErrorSchema = [-3, n0, _UE,
  { [_e]: _c, [_hE]: 401 },
  [_C, _M],
  [0, 0]
];
TypeRegistry.for(n0).registerError(UnauthorizedException$, UnauthorizedException);
export var UntagResourceRequest$: StaticStructureSchema = [3, n0, _URR,
  0,
  [_RAe, _TK],
  [[0, 1], [64 | 0, { [_hQ]: _tK }]]
];
export var UpdateDiscovererRequest$: StaticStructureSchema = [3, n0, _UDR,
  0,
  [_D, _DI, _CA],
  [0, [0, 1], 2]
];
export var UpdateDiscovererResponse$: StaticStructureSchema = [3, n0, _UDRp,
  0,
  [_D, _DA, _DI, _SA, _S, _CA, _T],
  [0, 0, 0, 0, 0, 2, [128 | 0, { [_jN]: _t }]]
];
export var UpdateRegistryRequest$: StaticStructureSchema = [3, n0, _URRp,
  0,
  [_D, _RN],
  [0, [0, 1]]
];
export var UpdateRegistryResponse$: StaticStructureSchema = [3, n0, _URRpd,
  0,
  [_D, _RA, _RN, _T],
  [0, 0, 0, [128 | 0, { [_jN]: _t }]]
];
export var UpdateSchemaRequest$: StaticStructureSchema = [3, n0, _USR,
  0,
  [_CTI, _Co, _D, _RN, _SN, _Ty],
  [[0, 4], 0, 0, [0, 1], [0, 1], 0]
];
export var UpdateSchemaResponse$: StaticStructureSchema = [3, n0, _USRp,
  0,
  [_D, _LM, _SAc, _SN, _SV, _T, _Ty, _VCD],
  [0, 5, 0, 0, 0, [128 | 0, { [_jN]: _t }], 0, 5]
];
var __Unit = "unit" as const;
export var SchemasServiceException$: StaticErrorSchema = [-3, _sm, "SchemasServiceException", 0, [], []];
TypeRegistry.for(_sm).registerError(SchemasServiceException$, SchemasServiceException);
var __listOf__string = 64 | 0;
var __listOfDiscovererSummary: StaticListSchema = [1, n0, _lODS,
  0, [() => DiscovererSummary$,
    0]
];
var __listOfGetDiscoveredSchemaVersionItemInput = 64 | 0;
var __listOfRegistrySummary: StaticListSchema = [1, n0, _lORS,
  0, [() => RegistrySummary$,
    0]
];
var __listOfSchemaSummary: StaticListSchema = [1, n0, _lOSS,
  0, [() => SchemaSummary$,
    0]
];
var __listOfSchemaVersionSummary: StaticListSchema = [1, n0, _lOSVS,
  0, () => SchemaVersionSummary$
];
var __listOfSearchSchemaSummary: StaticListSchema = [1, n0, _lOSSS,
  0, () => SearchSchemaSummary$
];
var __listOfSearchSchemaVersionSummary: StaticListSchema = [1, n0, _lOSSVS,
  0, () => SearchSchemaVersionSummary$
];
var Tags = 128 | 0;
export var CreateDiscoverer$: StaticOperationSchema = [9, n0, _CDre,
  { [_h]: ["POST", "/v1/discoverers", 201] }, () => CreateDiscovererRequest$, () => CreateDiscovererResponse$
];
export var CreateRegistry$: StaticOperationSchema = [9, n0, _CR,
  { [_h]: ["POST", "/v1/registries/name/{RegistryName}", 201] }, () => CreateRegistryRequest$, () => CreateRegistryResponse$
];
export var CreateSchema$: StaticOperationSchema = [9, n0, _CS,
  { [_h]: ["POST", "/v1/registries/name/{RegistryName}/schemas/name/{SchemaName}", 201] }, () => CreateSchemaRequest$, () => CreateSchemaResponse$
];
export var DeleteDiscoverer$: StaticOperationSchema = [9, n0, _DD,
  { [_h]: ["DELETE", "/v1/discoverers/id/{DiscovererId}", 204] }, () => DeleteDiscovererRequest$, () => __Unit
];
export var DeleteRegistry$: StaticOperationSchema = [9, n0, _DR,
  { [_h]: ["DELETE", "/v1/registries/name/{RegistryName}", 204] }, () => DeleteRegistryRequest$, () => __Unit
];
export var DeleteResourcePolicy$: StaticOperationSchema = [9, n0, _DRP,
  { [_h]: ["DELETE", "/v1/policy", 204] }, () => DeleteResourcePolicyRequest$, () => __Unit
];
export var DeleteSchema$: StaticOperationSchema = [9, n0, _DSe,
  { [_h]: ["DELETE", "/v1/registries/name/{RegistryName}/schemas/name/{SchemaName}", 204] }, () => DeleteSchemaRequest$, () => __Unit
];
export var DeleteSchemaVersion$: StaticOperationSchema = [9, n0, _DSV,
  { [_h]: ["DELETE", "/v1/registries/name/{RegistryName}/schemas/name/{SchemaName}/version/{SchemaVersion}", 204] }, () => DeleteSchemaVersionRequest$, () => __Unit
];
export var DescribeCodeBinding$: StaticOperationSchema = [9, n0, _DCB,
  { [_h]: ["GET", "/v1/registries/name/{RegistryName}/schemas/name/{SchemaName}/language/{Language}", 200] }, () => DescribeCodeBindingRequest$, () => DescribeCodeBindingResponse$
];
export var DescribeDiscoverer$: StaticOperationSchema = [9, n0, _DDe,
  { [_h]: ["GET", "/v1/discoverers/id/{DiscovererId}", 200] }, () => DescribeDiscovererRequest$, () => DescribeDiscovererResponse$
];
export var DescribeRegistry$: StaticOperationSchema = [9, n0, _DRe,
  { [_h]: ["GET", "/v1/registries/name/{RegistryName}", 200] }, () => DescribeRegistryRequest$, () => DescribeRegistryResponse$
];
export var DescribeSchema$: StaticOperationSchema = [9, n0, _DSes,
  { [_h]: ["GET", "/v1/registries/name/{RegistryName}/schemas/name/{SchemaName}", 200] }, () => DescribeSchemaRequest$, () => DescribeSchemaResponse$
];
export var ExportSchema$: StaticOperationSchema = [9, n0, _ES,
  { [_h]: ["GET", "/v1/registries/name/{RegistryName}/schemas/name/{SchemaName}/export", 200] }, () => ExportSchemaRequest$, () => ExportSchemaResponse$
];
export var GetCodeBindingSource$: StaticOperationSchema = [9, n0, _GCBS,
  { [_h]: ["GET", "/v1/registries/name/{RegistryName}/schemas/name/{SchemaName}/language/{Language}/source", 200] }, () => GetCodeBindingSourceRequest$, () => GetCodeBindingSourceResponse$
];
export var GetDiscoveredSchema$: StaticOperationSchema = [9, n0, _GDS,
  { [_h]: ["POST", "/v1/discover", 200] }, () => GetDiscoveredSchemaRequest$, () => GetDiscoveredSchemaResponse$
];
export var GetResourcePolicy$: StaticOperationSchema = [9, n0, _GRP,
  { [_h]: ["GET", "/v1/policy", 200] }, () => GetResourcePolicyRequest$, () => GetResourcePolicyResponse$
];
export var ListDiscoverers$: StaticOperationSchema = [9, n0, _LD,
  { [_h]: ["GET", "/v1/discoverers", 200] }, () => ListDiscoverersRequest$, () => ListDiscoverersResponse$
];
export var ListRegistries$: StaticOperationSchema = [9, n0, _LR,
  { [_h]: ["GET", "/v1/registries", 200] }, () => ListRegistriesRequest$, () => ListRegistriesResponse$
];
export var ListSchemas$: StaticOperationSchema = [9, n0, _LS,
  { [_h]: ["GET", "/v1/registries/name/{RegistryName}/schemas", 200] }, () => ListSchemasRequest$, () => ListSchemasResponse$
];
export var ListSchemaVersions$: StaticOperationSchema = [9, n0, _LSV,
  { [_h]: ["GET", "/v1/registries/name/{RegistryName}/schemas/name/{SchemaName}/versions", 200] }, () => ListSchemaVersionsRequest$, () => ListSchemaVersionsResponse$
];
export var ListTagsForResource$: StaticOperationSchema = [9, n0, _LTFR,
  { [_h]: ["GET", "/tags/{ResourceArn}", 200] }, () => ListTagsForResourceRequest$, () => ListTagsForResourceResponse$
];
export var PutCodeBinding$: StaticOperationSchema = [9, n0, _PCB,
  { [_h]: ["POST", "/v1/registries/name/{RegistryName}/schemas/name/{SchemaName}/language/{Language}", 202] }, () => PutCodeBindingRequest$, () => PutCodeBindingResponse$
];
export var PutResourcePolicy$: StaticOperationSchema = [9, n0, _PRP,
  { [_h]: ["PUT", "/v1/policy", 200] }, () => PutResourcePolicyRequest$, () => PutResourcePolicyResponse$
];
export var SearchSchemas$: StaticOperationSchema = [9, n0, _SSe,
  { [_h]: ["GET", "/v1/registries/name/{RegistryName}/schemas/search", 200] }, () => SearchSchemasRequest$, () => SearchSchemasResponse$
];
export var StartDiscoverer$: StaticOperationSchema = [9, n0, _SD,
  { [_h]: ["POST", "/v1/discoverers/id/{DiscovererId}/start", 200] }, () => StartDiscovererRequest$, () => StartDiscovererResponse$
];
export var StopDiscoverer$: StaticOperationSchema = [9, n0, _SDt,
  { [_h]: ["POST", "/v1/discoverers/id/{DiscovererId}/stop", 200] }, () => StopDiscovererRequest$, () => StopDiscovererResponse$
];
export var TagResource$: StaticOperationSchema = [9, n0, _TR,
  { [_h]: ["POST", "/tags/{ResourceArn}", 204] }, () => TagResourceRequest$, () => __Unit
];
export var UntagResource$: StaticOperationSchema = [9, n0, _UR,
  { [_h]: ["DELETE", "/tags/{ResourceArn}", 204] }, () => UntagResourceRequest$, () => __Unit
];
export var UpdateDiscoverer$: StaticOperationSchema = [9, n0, _UD,
  { [_h]: ["PUT", "/v1/discoverers/id/{DiscovererId}", 200] }, () => UpdateDiscovererRequest$, () => UpdateDiscovererResponse$
];
export var UpdateRegistry$: StaticOperationSchema = [9, n0, _URp,
  { [_h]: ["PUT", "/v1/registries/name/{RegistryName}", 200] }, () => UpdateRegistryRequest$, () => UpdateRegistryResponse$
];
export var UpdateSchema$: StaticOperationSchema = [9, n0, _US,
  { [_h]: ["PUT", "/v1/registries/name/{RegistryName}/schemas/name/{SchemaName}", 200] }, () => UpdateSchemaRequest$, () => UpdateSchemaResponse$
];
