const _A = "Annotation";
const _AA = "AggregationAuthorization";
const _AAA = "AggregationAuthorizationArn";
const _AAI = "AuthorizedAccountId";
const _AAL = "AggregationAuthorizationList";
const _AAR = "AllAwsRegions";
const _AARu = "AuthorizedAwsRegion";
const _AAS = "AccountAggregationSource";
const _AASL = "AccountAggregationSourceList";
const _AASc = "AccountAggregationSources";
const _AAg = "AggregationAuthorizations";
const _ACBCP = "AggregateComplianceByConformancePack";
const _ACBCPL = "AggregateComplianceByConformancePackList";
const _ACBCPg = "AggregateComplianceByConformancePacks";
const _ACBCR = "AggregateComplianceByConfigRule";
const _ACBCRL = "AggregateComplianceByConfigRuleList";
const _ACBCRg = "AggregateComplianceByConfigRules";
const _ACC = "AggregateComplianceCount";
const _ACCL = "AggregateComplianceCountList";
const _ACCg = "AggregateComplianceCounts";
const _ACPC = "AggregateConformancePackCompliance";
const _ACPCC = "AggregateConformancePackComplianceCount";
const _ACPCF = "AggregateConformancePackComplianceFilters";
const _ACPCS = "AggregateConformancePackComplianceSummary";
const _ACPCSF = "AggregateConformancePackComplianceSummaryFilters";
const _ACPCSL = "AggregateConformancePackComplianceSummaryList";
const _ACPCSg = "AggregateConformancePackComplianceSummaries";
const _AER = "AggregateEvaluationResult";
const _AERL = "AggregateEvaluationResultList";
const _AERg = "AggregateEvaluationResults";
const _AF = "AggregatorFilters";
const _AFRT = "AggregatorFilterResourceType";
const _AFSP = "AggregatorFilterServicePrincipal";
const _AI = "AccountIds";
const _AIc = "AccountId";
const _AR = "AwsRegions";
const _ARI = "AggregateResourceIdentifier";
const _ART = "AssociateResourceTypes";
const _ARTR = "AssociateResourceTypesRequest";
const _ARTRs = "AssociateResourceTypesResponse";
const _ARw = "AwsRegion";
const _ASS = "AggregatedSourceStatus";
const _ASSL = "AggregatedSourceStatusList";
const _Ar = "Arn";
const _Au = "Automatic";
const _BCI = "BaseConfigurationItem";
const _BCIa = "BaseConfigurationItems";
const _BGARC = "BatchGetAggregateResourceConfig";
const _BGARCR = "BatchGetAggregateResourceConfigRequest";
const _BGARCRa = "BatchGetAggregateResourceConfigResponse";
const _BGRC = "BatchGetResourceConfig";
const _BGRCR = "BatchGetResourceConfigRequest";
const _BGRCRa = "BatchGetResourceConfigResponse";
const _C = "Compliance";
const _CA = "ConfigurationAggregator";
const _CAA = "ConfigurationAggregatorArn";
const _CAL = "ConfigurationAggregatorList";
const _CAN = "ConfigurationAggregatorName";
const _CANo = "ConfigurationAggregatorNames";
const _CAo = "ConfigurationAggregators";
const _CB = "CreatedBy";
const _CBCR = "ComplianceByConfigRule";
const _CBCRo = "ComplianceByConfigRules";
const _CBR = "ComplianceByResource";
const _CBRo = "ComplianceByResources";
const _CBS = "CreatedByService";
const _CC = "CappedCount";
const _CCC = "ComplianceContributorCount";
const _CCPC = "CompliantConformancePackCount";
const _CE = "CapExceeded";
const _CEDI = "ConfigExportDeliveryInfo";
const _CERP = "ConcurrentExecutionRatePercentage";
const _CEo = "ConflictException";
const _CI = "ConfigurationItem";
const _CIL = "ConfigurationItemList";
const _CPA = "ConformancePackArn";
const _CPCF = "ConformancePackComplianceFilters";
const _CPCS = "ConformancePackComplianceScore";
const _CPCSF = "ConformancePackComplianceScoresFilters";
const _CPCSL = "ConformancePackComplianceSummaryList";
const _CPCSo = "ConformancePackComplianceSummary";
const _CPCSon = "ConformancePackComplianceStatus";
const _CPCSonf = "ConformancePackComplianceScores";
const _CPD = "ConformancePackDetail";
const _CPDL = "ConformancePackDetailList";
const _CPDo = "ConformancePackDetails";
const _CPDu = "CustomPolicyDetails";
const _CPEF = "ConformancePackEvaluationFilters";
const _CPER = "ConformancePackEvaluationResult";
const _CPI = "ConformancePackId";
const _CPIP = "ConformancePackInputParameters";
const _CPIPo = "ConformancePackInputParameter";
const _CPN = "ConformancePackName";
const _CPNo = "ConformancePackNames";
const _CPRC = "ConformancePackRuleCompliance";
const _CPRCL = "ConformancePackRuleComplianceList";
const _CPRER = "ConformancePackRuleEvaluationResults";
const _CPRERL = "ConformancePackRuleEvaluationResultsList";
const _CPS = "ConformancePackState";
const _CPSD = "ConformancePackStatusDetail";
const _CPSDL = "ConformancePackStatusDetailsList";
const _CPSDo = "ConformancePackStatusDetails";
const _CPSR = "ConformancePackStatusReason";
const _CPTVE = "ConformancePackTemplateValidationException";
const _CR = "ConfigurationRecorder";
const _CRA = "ConfigurationRecorderArn";
const _CRAo = "ConfigRuleArn";
const _CRC = "CompliantRuleCount";
const _CRCF = "ConfigRuleComplianceFilters";
const _CRCSF = "ConfigRuleComplianceSummaryFilters";
const _CRCo = "CompliantResourceCount";
const _CRES = "ConfigRuleEvaluationStatus";
const _CRESL = "ConfigRuleEvaluationStatusList";
const _CRESo = "ConfigRulesEvaluationStatus";
const _CRF = "ConfigurationRecorderFilter";
const _CRFL = "ConfigurationRecorderFilterList";
const _CRI = "ConfigRuleId";
const _CRIT = "ConfigRuleInvokedTime";
const _CRIo = "ComplianceResourceId";
const _CRL = "ConfigurationRecorderList";
const _CRN = "ConfigRuleName";
const _CRNo = "ConfigRuleNames";
const _CRNon = "ConfigurationRecorderName";
const _CRNonf = "ConfigurationRecorderNames";
const _CRS = "ConfigRuleState";
const _CRSL = "ConfigurationRecorderStatusList";
const _CRSo = "ConfigurationRecorderStatus";
const _CRSon = "ConfigurationRecorderSummary";
const _CRSonf = "ConfigurationRecordersStatus";
const _CRSonfi = "ConfigurationRecorderSummaries";
const _CRT = "ComplianceResourceType";
const _CRTo = "ComplianceResourceTypes";
const _CRo = "ConfigRule";
const _CRon = "ConfigRules";
const _CRonf = "ConfigurationRecorders";
const _CS = "ComplianceSummary";
const _CSBRT = "ComplianceSummaryByResourceType";
const _CSBRTo = "ComplianceSummariesByResourceType";
const _CSDI = "ConfigStreamDeliveryInfo";
const _CSDP = "ConfigSnapshotDeliveryProperties";
const _CST = "ComplianceSummaryTimestamp";
const _CT = "ComplianceType";
const _CTl = "ClientToken";
const _CTo = "ComplianceTypes";
const _CTr = "CreationTime";
const _Co = "Controls";
const _Con = "Configuration";
const _D = "Description";
const _DAA = "DeleteAggregationAuthorization";
const _DAAR = "DeleteAggregationAuthorizationRequest";
const _DAARe = "DescribeAggregationAuthorizationsRequest";
const _DAARes = "DescribeAggregationAuthorizationsResponse";
const _DAAe = "DescribeAggregationAuthorizations";
const _DACBCP = "DescribeAggregateComplianceByConformancePacks";
const _DACBCPR = "DescribeAggregateComplianceByConformancePacksRequest";
const _DACBCPRe = "DescribeAggregateComplianceByConformancePacksResponse";
const _DACBCR = "DescribeAggregateComplianceByConfigRules";
const _DACBCRR = "DescribeAggregateComplianceByConfigRulesRequest";
const _DACBCRRe = "DescribeAggregateComplianceByConfigRulesResponse";
const _DC = "DeliveryChannel";
const _DCA = "DeleteConfigurationAggregator";
const _DCAR = "DeleteConfigurationAggregatorRequest";
const _DCARe = "DescribeConfigurationAggregatorsRequest";
const _DCARes = "DescribeConfigurationAggregatorsResponse";
const _DCASS = "DescribeConfigurationAggregatorSourcesStatus";
const _DCASSR = "DescribeConfigurationAggregatorSourcesStatusRequest";
const _DCASSRe = "DescribeConfigurationAggregatorSourcesStatusResponse";
const _DCAe = "DescribeConfigurationAggregators";
const _DCBCR = "DescribeComplianceByConfigRule";
const _DCBCRR = "DescribeComplianceByConfigRuleRequest";
const _DCBCRRe = "DescribeComplianceByConfigRuleResponse";
const _DCBR = "DescribeComplianceByResource";
const _DCBRR = "DescribeComplianceByResourceRequest";
const _DCBRRe = "DescribeComplianceByResourceResponse";
const _DCL = "DeliveryChannelList";
const _DCN = "DeliveryChannelName";
const _DCNe = "DeliveryChannelNames";
const _DCP = "DeleteConformancePack";
const _DCPC = "DescribeConformancePackCompliance";
const _DCPCR = "DescribeConformancePackComplianceRequest";
const _DCPCRe = "DescribeConformancePackComplianceResponse";
const _DCPR = "DeleteConformancePackRequest";
const _DCPRe = "DescribeConformancePacksRequest";
const _DCPRes = "DescribeConformancePacksResponse";
const _DCPS = "DescribeConformancePackStatus";
const _DCPSR = "DescribeConformancePackStatusRequest";
const _DCPSRe = "DescribeConformancePackStatusResponse";
const _DCPe = "DescribeConformancePacks";
const _DCR = "DeleteConfigRule";
const _DCRES = "DescribeConfigRuleEvaluationStatus";
const _DCRESR = "DescribeConfigRuleEvaluationStatusRequest";
const _DCRESRe = "DescribeConfigRuleEvaluationStatusResponse";
const _DCRF = "DescribeConfigRulesFilters";
const _DCRR = "DeleteConfigRuleRequest";
const _DCRRe = "DeleteConfigurationRecorderRequest";
const _DCRRes = "DescribeConfigRulesRequest";
const _DCRResc = "DescribeConfigRulesResponse";
const _DCRRescr = "DescribeConfigurationRecordersRequest";
const _DCRRescri = "DescribeConfigurationRecordersResponse";
const _DCRS = "DescribeConfigurationRecorderStatus";
const _DCRSR = "DescribeConfigurationRecorderStatusRequest";
const _DCRSRe = "DescribeConfigurationRecorderStatusResponse";
const _DCRe = "DeleteConfigurationRecorder";
const _DCRes = "DescribeConfigRules";
const _DCResc = "DescribeConfigurationRecorders";
const _DCS = "DeliveryChannelStatus";
const _DCSL = "DeliveryChannelStatusList";
const _DCSR = "DeliverConfigSnapshotRequest";
const _DCSRe = "DeliverConfigSnapshotResponse";
const _DCSe = "DeliveryChannelsStatus";
const _DCSel = "DeliverConfigSnapshot";
const _DCe = "DeliveryChannels";
const _DDC = "DeleteDeliveryChannel";
const _DDCR = "DeleteDeliveryChannelRequest";
const _DDCRe = "DescribeDeliveryChannelsRequest";
const _DDCRes = "DescribeDeliveryChannelsResponse";
const _DDCS = "DescribeDeliveryChannelStatus";
const _DDCSR = "DescribeDeliveryChannelStatusRequest";
const _DDCSRe = "DescribeDeliveryChannelStatusResponse";
const _DDCe = "DescribeDeliveryChannels";
const _DER = "DeleteEvaluationResults";
const _DERR = "DeleteEvaluationResultsRequest";
const _DERRe = "DeleteEvaluationResultsResponse";
const _DLDA = "DebugLogDeliveryAccounts";
const _DN = "DocumentName";
const _DOCP = "DeleteOrganizationConformancePack";
const _DOCPR = "DeleteOrganizationConformancePackRequest";
const _DOCPRe = "DescribeOrganizationConformancePacksRequest";
const _DOCPRes = "DescribeOrganizationConformancePacksResponse";
const _DOCPS = "DescribeOrganizationConformancePackStatuses";
const _DOCPSR = "DescribeOrganizationConformancePackStatusesRequest";
const _DOCPSRe = "DescribeOrganizationConformancePackStatusesResponse";
const _DOCPe = "DescribeOrganizationConformancePacks";
const _DOCR = "DeleteOrganizationConfigRule";
const _DOCRR = "DeleteOrganizationConfigRuleRequest";
const _DOCRRe = "DescribeOrganizationConfigRulesRequest";
const _DOCRRes = "DescribeOrganizationConfigRulesResponse";
const _DOCRS = "DescribeOrganizationConfigRuleStatuses";
const _DOCRSR = "DescribeOrganizationConfigRuleStatusesRequest";
const _DOCRSRe = "DescribeOrganizationConfigRuleStatusesResponse";
const _DOCRe = "DescribeOrganizationConfigRules";
const _DPAR = "DeletePendingAggregationRequest";
const _DPARR = "DeletePendingAggregationRequestRequest";
const _DPARRe = "DescribePendingAggregationRequestsRequest";
const _DPARRes = "DescribePendingAggregationRequestsResponse";
const _DPARe = "DescribePendingAggregationRequests";
const _DRC = "DeleteRemediationConfiguration";
const _DRCR = "DeleteRemediationConfigurationRequest";
const _DRCRe = "DeleteRemediationConfigurationResponse";
const _DRCRel = "DeleteResourceConfigRequest";
const _DRCRele = "DeleteRetentionConfigurationRequest";
const _DRCRes = "DescribeRemediationConfigurationsRequest";
const _DRCResc = "DescribeRemediationConfigurationsResponse";
const _DRCRescr = "DescribeRetentionConfigurationsRequest";
const _DRCRescri = "DescribeRetentionConfigurationsResponse";
const _DRCe = "DeleteResourceConfig";
const _DRCel = "DeleteRetentionConfiguration";
const _DRCes = "DescribeRemediationConfigurations";
const _DRCesc = "DescribeRetentionConfigurations";
const _DRE = "DeleteRemediationExceptions";
const _DRER = "DeleteRemediationExceptionsRequest";
const _DRERe = "DeleteRemediationExceptionsResponse";
const _DRERes = "DescribeRemediationExceptionsRequest";
const _DREResc = "DescribeRemediationExceptionsResponse";
const _DRES = "DescribeRemediationExecutionStatus";
const _DRESR = "DescribeRemediationExecutionStatusRequest";
const _DRESRe = "DescribeRemediationExecutionStatusResponse";
const _DREe = "DescribeRemediationExceptions";
const _DRIL = "DiscoveredResourceIdentifierList";
const _DRT = "DisassociateResourceTypes";
const _DRTR = "DisassociateResourceTypesRequest";
const _DRTRi = "DisassociateResourceTypesResponse";
const _DSB = "DeliveryS3Bucket";
const _DSKP = "DeliveryS3KeyPrefix";
const _DSLCR = "DeleteServiceLinkedConfigurationRecorder";
const _DSLCRR = "DeleteServiceLinkedConfigurationRecorderRequest";
const _DSLCRRe = "DeleteServiceLinkedConfigurationRecorderResponse";
const _DSQ = "DeleteStoredQuery";
const _DSQR = "DeleteStoredQueryRequest";
const _DSQRe = "DeleteStoredQueryResponse";
const _DV = "DocumentVersion";
const _E = "Evaluation";
const _EA = "ExcludedAccounts";
const _EBRT = "ExclusionByResourceTypes";
const _EC = "EvaluationContext";
const _ECI = "EvaluationContextIdentifier";
const _ECr = "ErrorCode";
const _ECx = "ExecutionControls";
const _EDLD = "EnableDebugLogDelivery";
const _EE = "ExternalEvaluation";
const _EM = "EvaluationModes";
const _EMC = "EvaluationModeConfiguration";
const _EMr = "ErrorMessage";
const _EMv = "EvaluationMode";
const _EP = "ErrorPercentage";
const _ER = "EvaluationResult";
const _ERI = "EvaluationResultIdentifier";
const _ERQ = "EvaluationResultQualifier";
const _ERv = "EvaluationResults";
const _ES = "EvaluationStatus";
const _EST = "EvaluationStartTimestamp";
const _ESv = "EventSource";
const _ET = "ExpirationTime";
const _ETn = "EndTime";
const _ETv = "EvaluationTimeout";
const _Ev = "Evaluations";
const _Ex = "Expression";
const _F = "Filters";
const _FAT = "FirstActivatedTime";
const _FB = "FailedBatches";
const _FDREB = "FailedDeleteRemediationExceptionsBatch";
const _FDREBa = "FailedDeleteRemediationExceptionsBatches";
const _FE = "FailedEvaluations";
const _FES = "FirstEvaluationStarted";
const _FI = "FailedItems";
const _FIL = "FieldInfoList";
const _FIi = "FieldInfo";
const _FM = "FailureMessage";
const _FR = "FailureReason";
const _FRB = "FailedRemediationBatch";
const _FRBa = "FailedRemediationBatches";
const _FREB = "FailedRemediationExceptionBatch";
const _FREBa = "FailedRemediationExceptionBatches";
const _GACDBCR = "GetAggregateComplianceDetailsByConfigRule";
const _GACDBCRR = "GetAggregateComplianceDetailsByConfigRuleRequest";
const _GACDBCRRe = "GetAggregateComplianceDetailsByConfigRuleResponse";
const _GACPCS = "GetAggregateConformancePackComplianceSummary";
const _GACPCSR = "GetAggregateConformancePackComplianceSummaryRequest";
const _GACPCSRe = "GetAggregateConformancePackComplianceSummaryResponse";
const _GACRCS = "GetAggregateConfigRuleComplianceSummary";
const _GACRCSR = "GetAggregateConfigRuleComplianceSummaryRequest";
const _GACRCSRe = "GetAggregateConfigRuleComplianceSummaryResponse";
const _GADRC = "GetAggregateDiscoveredResourceCounts";
const _GADRCR = "GetAggregateDiscoveredResourceCountsRequest";
const _GADRCRe = "GetAggregateDiscoveredResourceCountsResponse";
const _GARC = "GetAggregateResourceConfig";
const _GARCR = "GetAggregateResourceConfigRequest";
const _GARCRe = "GetAggregateResourceConfigResponse";
const _GBK = "GroupByKey";
const _GCDBCR = "GetComplianceDetailsByConfigRule";
const _GCDBCRR = "GetComplianceDetailsByConfigRuleRequest";
const _GCDBCRRe = "GetComplianceDetailsByConfigRuleResponse";
const _GCDBR = "GetComplianceDetailsByResource";
const _GCDBRR = "GetComplianceDetailsByResourceRequest";
const _GCDBRRe = "GetComplianceDetailsByResourceResponse";
const _GCPCD = "GetConformancePackComplianceDetails";
const _GCPCDR = "GetConformancePackComplianceDetailsRequest";
const _GCPCDRe = "GetConformancePackComplianceDetailsResponse";
const _GCPCS = "GetConformancePackComplianceSummary";
const _GCPCSR = "GetConformancePackComplianceSummaryRequest";
const _GCPCSRe = "GetConformancePackComplianceSummaryResponse";
const _GCRP = "GetCustomRulePolicy";
const _GCRPR = "GetCustomRulePolicyRequest";
const _GCRPRe = "GetCustomRulePolicyResponse";
const _GCSBCR = "GetComplianceSummaryByConfigRule";
const _GCSBCRR = "GetComplianceSummaryByConfigRuleResponse";
const _GCSBRT = "GetComplianceSummaryByResourceType";
const _GCSBRTR = "GetComplianceSummaryByResourceTypeRequest";
const _GCSBRTRe = "GetComplianceSummaryByResourceTypeResponse";
const _GDRC = "GetDiscoveredResourceCounts";
const _GDRCR = "GetDiscoveredResourceCountsRequest";
const _GDRCRe = "GetDiscoveredResourceCountsResponse";
const _GN = "GroupName";
const _GOCPDS = "GetOrganizationConformancePackDetailedStatus";
const _GOCPDSR = "GetOrganizationConformancePackDetailedStatusRequest";
const _GOCPDSRe = "GetOrganizationConformancePackDetailedStatusResponse";
const _GOCRDS = "GetOrganizationConfigRuleDetailedStatus";
const _GOCRDSR = "GetOrganizationConfigRuleDetailedStatusRequest";
const _GOCRDSRe = "GetOrganizationConfigRuleDetailedStatusResponse";
const _GOCRP = "GetOrganizationCustomRulePolicy";
const _GOCRPR = "GetOrganizationCustomRulePolicyRequest";
const _GOCRPRe = "GetOrganizationCustomRulePolicyResponse";
const _GRC = "GroupedResourceCounts";
const _GRCH = "GetResourceConfigHistory";
const _GRCHR = "GetResourceConfigHistoryRequest";
const _GRCHRe = "GetResourceConfigHistoryResponse";
const _GRCL = "GroupedResourceCountList";
const _GRCr = "GroupedResourceCount";
const _GRES = "GetResourceEvaluationSummary";
const _GRESR = "GetResourceEvaluationSummaryRequest";
const _GRESRe = "GetResourceEvaluationSummaryResponse";
const _GSQ = "GetStoredQuery";
const _GSQR = "GetStoredQueryRequest";
const _GSQRe = "GetStoredQueryResponse";
const _ICRNE = "InvalidConfigurationRecorderNameException";
const _IDCNE = "InvalidDeliveryChannelNameException";
const _IDPE = "InsufficientDeliveryPolicyException";
const _IEE = "InvalidExpressionException";
const _ILE = "InvalidLimitException";
const _INTE = "InvalidNextTokenException";
const _IP = "InputParameters";
const _IPE = "InsufficientPermissionsException";
const _IPM = "IdempotentParameterMismatch";
const _IPVE = "InvalidParameterValueException";
const _IRE = "InvalidRoleException";
const _IRGE = "InvalidRecordingGroupException";
const _IRTE = "InvalidResultTokenException";
const _ISKKAE = "InvalidS3KmsKeyArnException";
const _ISKPE = "InvalidS3KeyPrefixException";
const _ISNSTARNE = "InvalidSNSTopicARNException";
const _IT = "InvocationTime";
const _ITRE = "InvalidTimeRangeException";
const _K = "Key";
const _L = "Limit";
const _LADR = "ListAggregateDiscoveredResources";
const _LADRR = "ListAggregateDiscoveredResourcesRequest";
const _LADRRi = "ListAggregateDiscoveredResourcesResponse";
const _LCPCS = "ListConformancePackComplianceScores";
const _LCPCSR = "ListConformancePackComplianceScoresRequest";
const _LCPCSRi = "ListConformancePackComplianceScoresResponse";
const _LCR = "ListConfigurationRecorders";
const _LCRR = "ListConfigurationRecordersRequest";
const _LCRRi = "ListConfigurationRecordersResponse";
const _LDCDFE = "LastDeliveryChannelDeleteFailedException";
const _LDLDS = "LastDebugLogDeliveryStatus";
const _LDLDSR = "LastDebugLogDeliveryStatusReason";
const _LDLDT = "LastDebugLogDeliveryTime";
const _LDR = "ListDiscoveredResources";
const _LDRR = "ListDiscoveredResourcesRequest";
const _LDRRi = "ListDiscoveredResourcesResponse";
const _LDT = "LastDeactivatedTime";
const _LEC = "LastErrorCode";
const _LEE = "LimitExceededException";
const _LEM = "LastErrorMessage";
const _LFA = "LambdaFunctionArn";
const _LFET = "LastFailedEvaluationTime";
const _LFIT = "LastFailedInvocationTime";
const _LRE = "ListResourceEvaluations";
const _LRER = "ListResourceEvaluationsRequest";
const _LRERi = "ListResourceEvaluationsResponse";
const _LSET = "LastSuccessfulEvaluationTime";
const _LSIT = "LastSuccessfulInvocationTime";
const _LSQ = "ListStoredQueries";
const _LSQR = "ListStoredQueriesRequest";
const _LSQRi = "ListStoredQueriesResponse";
const _LTFR = "ListTagsForResource";
const _LTFRR = "ListTagsForResourceRequest";
const _LTFRRi = "ListTagsForResourceResponse";
const _LUCT = "LastUpdateCompletedTime";
const _LURT = "LastUpdateRequestedTime";
const _LUS = "LastUpdateStatus";
const _LUT = "LastUpdateTime";
const _LUTa = "LastUpdatedTime";
const _M = "Mode";
const _MAA = "MaximumAutomaticAttempts";
const _MAREE = "MaxActiveResourcesExceededException";
const _MARS = "MemberAccountRuleStatus";
const _MAS = "MemberAccountStatus";
const _MEF = "MaximumExecutionFrequency";
const _MNOCPEE = "MaxNumberOfConformancePacksExceededException";
const _MNOCREE = "MaxNumberOfConfigRulesExceededException";
const _MNOCREEa = "MaxNumberOfConfigurationRecordersExceededException";
const _MNODCEE = "MaxNumberOfDeliveryChannelsExceededException";
const _MNOOCPEE = "MaxNumberOfOrganizationConformancePacksExceededException";
const _MNOOCREE = "MaxNumberOfOrganizationConfigRulesExceededException";
const _MNORCEE = "MaxNumberOfRetentionConfigurationsExceededException";
const _MR = "MaxResults";
const _MT = "MessageType";
const _Me = "Message";
const _N = "Name";
const _NACRE = "NoAvailableConfigurationRecorderException";
const _NADCE = "NoAvailableDeliveryChannelException";
const _NAOE = "NoAvailableOrganizationException";
const _NCCPC = "NonCompliantConformancePackCount";
const _NCRC = "NonCompliantRuleCount";
const _NCRCo = "NonCompliantResourceCount";
const _NRCRE = "NoRunningConfigurationRecorderException";
const _NSBE = "NoSuchBucketException";
const _NSCAE = "NoSuchConfigurationAggregatorException";
const _NSCPE = "NoSuchConformancePackException";
const _NSCRE = "NoSuchConfigRuleException";
const _NSCREo = "NoSuchConfigurationRecorderException";
const _NSCRICPE = "NoSuchConfigRuleInConformancePackException";
const _NSDCE = "NoSuchDeliveryChannelException";
const _NSOCPE = "NoSuchOrganizationConformancePackException";
const _NSOCRE = "NoSuchOrganizationConfigRuleException";
const _NSRCE = "NoSuchRemediationConfigurationException";
const _NSRCEo = "NoSuchRetentionConfigurationException";
const _NSREE = "NoSuchRemediationExceptionException";
const _NT = "NextToken";
const _O = "Owner";
const _OADE = "OrganizationAccessDeniedException";
const _OAFNEE = "OrganizationAllFeaturesNotEnabledException";
const _OAS = "OrganizationAggregationSource";
const _OCIE = "OversizedConfigurationItemException";
const _OCP = "OrganizationConformancePacks";
const _OCPA = "OrganizationConformancePackArn";
const _OCPDS = "OrganizationConformancePackDetailedStatuses";
const _OCPDSr = "OrganizationConformancePackDetailedStatus";
const _OCPN = "OrganizationConformancePackName";
const _OCPNr = "OrganizationConformancePackNames";
const _OCPRM = "OrganizationCustomPolicyRuleMetadata";
const _OCPRMNP = "OrganizationCustomPolicyRuleMetadataNoPolicy";
const _OCPS = "OrganizationConformancePackStatuses";
const _OCPSr = "OrganizationConformancePackStatus";
const _OCPTVE = "OrganizationConformancePackTemplateValidationException";
const _OCPr = "OrganizationConformancePack";
const _OCR = "OrganizationConfigRules";
const _OCRA = "OrganizationConfigRuleArn";
const _OCRDS = "OrganizationConfigRuleDetailedStatus";
const _OCRM = "OrganizationCustomRuleMetadata";
const _OCRN = "OrganizationConfigRuleName";
const _OCRNr = "OrganizationConfigRuleNames";
const _OCRS = "OrganizationConfigRuleStatuses";
const _OCRSr = "OrganizationConfigRuleStatus";
const _OCRTT = "OrganizationConfigRuleTriggerTypes";
const _OCRr = "OrganizationConfigRule";
const _OMRM = "OrganizationManagedRuleMetadata";
const _ORDSF = "OrganizationResourceDetailedStatusFilters";
const _ORS = "OrganizationRuleStatus";
const _OT = "OrderingTimestamp";
const _P = "Parameters";
const _PAA = "PutAggregationAuthorization";
const _PAAR = "PutAggregationAuthorizationRequest";
const _PAARu = "PutAggregationAuthorizationResponse";
const _PAR = "PendingAggregationRequests";
const _PARL = "PendingAggregationRequestList";
const _PARe = "PendingAggregationRequest";
const _PCA = "PutConfigurationAggregator";
const _PCAR = "PutConfigurationAggregatorRequest";
const _PCARu = "PutConfigurationAggregatorResponse";
const _PCP = "PutConformancePack";
const _PCPR = "PutConformancePackRequest";
const _PCPRu = "PutConformancePackResponse";
const _PCR = "PutConfigRule";
const _PCRR = "PutConfigRuleRequest";
const _PCRRu = "PutConfigurationRecorderRequest";
const _PCRu = "PutConfigurationRecorder";
const _PDC = "PutDeliveryChannel";
const _PDCR = "PutDeliveryChannelRequest";
const _PE = "PutEvaluations";
const _PEE = "PutExternalEvaluation";
const _PEER = "PutExternalEvaluationRequest";
const _PEERu = "PutExternalEvaluationResponse";
const _PER = "PutEvaluationsRequest";
const _PERu = "PutEvaluationsResponse";
const _PN = "ParameterName";
const _POCP = "PutOrganizationConformancePack";
const _POCPR = "PutOrganizationConformancePackRequest";
const _POCPRu = "PutOrganizationConformancePackResponse";
const _POCR = "PutOrganizationConfigRule";
const _POCRR = "PutOrganizationConfigRuleRequest";
const _POCRRu = "PutOrganizationConfigRuleResponse";
const _PR = "PolicyRuntime";
const _PRC = "PutRemediationConfigurations";
const _PRCR = "PutRemediationConfigurationsRequest";
const _PRCRu = "PutRemediationConfigurationsResponse";
const _PRCRut = "PutResourceConfigRequest";
const _PRCRute = "PutRetentionConfigurationRequest";
const _PRCRutet = "PutRetentionConfigurationResponse";
const _PRCu = "PutResourceConfig";
const _PRCut = "PutRetentionConfiguration";
const _PRE = "PutRemediationExceptions";
const _PRER = "PutRemediationExceptionsRequest";
const _PRERu = "PutRemediationExceptionsResponse";
const _PSLCR = "PutServiceLinkedConfigurationRecorder";
const _PSLCRR = "PutServiceLinkedConfigurationRecorderRequest";
const _PSLCRRu = "PutServiceLinkedConfigurationRecorderResponse";
const _PSQ = "PutStoredQuery";
const _PSQR = "PutStoredQueryRequest";
const _PSQRu = "PutStoredQueryResponse";
const _PT = "PolicyText";
const _PV = "ParameterValue";
const _QA = "QueryArn";
const _QI = "QueryInfo";
const _QIu = "QueryId";
const _QN = "QueryName";
const _R = "Relationship";
const _RA = "ResourceArn";
const _RAI = "RequesterAccountId";
const _RAR = "RequesterAwsRegion";
const _RAS = "RetryAttemptSeconds";
const _RAo = "RoleArn";
const _RC = "RemediationConfigurations";
const _RCF = "ResourceCountFilters";
const _RCL = "RetentionConfigurationList";
const _RCME = "ResourceConcurrentModificationException";
const _RCN = "RetentionConfigurationName";
const _RCNe = "RetentionConfigurationNames";
const _RCST = "ResourceConfigurationSchemaType";
const _RCe = "RetentionConfigurations";
const _RCem = "RemediationConfiguration";
const _RCes = "ResourceCount";
const _RCeso = "ResourceConfiguration";
const _RCesou = "ResourceCounts";
const _RCet = "RetentionConfiguration";
const _RD = "ResourceDetails";
const _RE = "RemediationExceptions";
const _REF = "ResourceEvaluationFilters";
const _REI = "ResourceEvaluationId";
const _RERK = "RemediationExceptionResourceKey";
const _RERKe = "RemediationExceptionResourceKeys";
const _RES = "RemediationExecutionStatuses";
const _RESe = "RemediationExecutionStatus";
const _RESem = "RemediationExecutionStep";
const _RESeme = "RemediationExecutionSteps";
const _REe = "ResourceEvaluations";
const _REem = "RemediationException";
const _REes = "ResourceEvaluation";
const _RF = "ResourceFilters";
const _RG = "RecordingGroup";
const _RI = "ResourceId";
const _RIL = "ResourceIdentifierList";
const _RILe = "ResourceIdentifiersList";
const _RIPE = "RemediationInProgressException";
const _RIS = "ResourceIdScope";
const _RIUE = "ResourceInUseException";
const _RIe = "ResourceIdentifiers";
const _RIes = "ResourceIds";
const _RIeso = "ResourceIdentifier";
const _RIu = "RuleIdentifier";
const _RK = "ResourceKeys";
const _RKe = "ResourceKey";
const _RL = "RelationshipList";
const _RM = "RecordingMode";
const _RMO = "RecordingModeOverride";
const _RMOe = "RecordingModeOverrides";
const _RN = "ResourceName";
const _RNDE = "ResourceNotDiscoveredException";
const _RNFE = "ResourceNotFoundException";
const _RP = "RemediationParameters";
const _RPID = "RetentionPeriodInDays";
const _RPV = "RemediationParameterValue";
const _RRT = "ResultRecordedTime";
const _RS = "RecordingStrategy";
const _RT = "ResourceType";
const _RTS = "ResourceTypesScope";
const _RTe = "ResourceTypes";
const _RTes = "ResultToken";
const _RV = "ResourceValue";
const _Re = "Region";
const _Res = "Results";
const _S = "Scope";
const _SA = "StackArn";
const _SAI = "SourceAccountId";
const _SARC = "SelectAggregateResourceConfig";
const _SARCR = "SelectAggregateResourceConfigRequest";
const _SARCRe = "SelectAggregateResourceConfigResponse";
const _SB = "SortBy";
const _SC = "SsmControls";
const _SCR = "StartConfigurationRecorder";
const _SCRE = "StartConfigRulesEvaluation";
const _SCRER = "StartConfigRulesEvaluationRequest";
const _SCRERt = "StartConfigRulesEvaluationResponse";
const _SCRR = "StartConfigurationRecorderRequest";
const _SCRRt = "StopConfigurationRecorderRequest";
const _SCRt = "StopConfigurationRecorder";
const _SD = "StepDetails";
const _SDF = "StatusDetailFilters";
const _SDo = "SourceDetails";
const _SDou = "SourceDetail";
const _SF = "SelectFields";
const _SI = "SourceId";
const _SIo = "SourceIdentifier";
const _SO = "SortOrder";
const _SP = "ServicePrincipal";
const _SQ = "StoredQuery";
const _SQM = "StoredQueryMetadata";
const _SQML = "StoredQueryMetadataList";
const _SR = "SourceRegion";
const _SRC = "SelectResourceConfig";
const _SRCR = "SelectResourceConfigRequest";
const _SRCRe = "SelectResourceConfigResponse";
const _SRE = "StartRemediationExecution";
const _SRER = "StartRemediationExecutionRequest";
const _SRERt = "StartRemediationExecutionResponse";
const _SRERta = "StartResourceEvaluationRequest";
const _SRERtar = "StartResourceEvaluationResponse";
const _SREt = "StartResourceEvaluation";
const _ST = "SourceType";
const _STt = "StartTime";
const _STto = "StopTime";
const _SV = "StaticValue";
const _SVI = "SchemaVersionId";
const _Sc = "Score";
const _So = "Source";
const _St = "Status";
const _Sta = "State";
const _T = "Type";
const _TB = "TemplateBody";
const _TDR = "TotalDiscoveredResources";
const _TI = "TargetId";
const _TK = "TagKey";
const _TKS = "TagKeyScope";
const _TKa = "TagKeys";
const _TL = "TagList";
const _TLa = "TagsList";
const _TM = "TestMode";
const _TMTE = "TooManyTagsException";
const _TR = "TagResource";
const _TRC = "TotalRuleCount";
const _TRR = "TagResourceRequest";
const _TSSMDD = "TemplateSSMDocumentDetails";
const _TSU = "TemplateS3Uri";
const _TT = "TargetType";
const _TV = "TargetVersion";
const _TVS = "TagValueScope";
const _TVa = "TagValue";
const _TW = "TimeWindow";
const _Ta = "Tags";
const _Tag = "Tag";
const _UEE = "UnmodifiableEntityException";
const _UR = "UntagResource";
const _URI = "UnprocessedResourceIdentifiers";
const _URIL = "UnprocessedResourceIdentifierList";
const _URR = "UntagResourceRequest";
const _US = "UpdateStatus";
const _V = "Value";
const _VE = "ValidationException";
const _Va = "Values";
const _a = "arn";
const _aI = "accountId";
const _aR = "awsRegion";
const _aS = "allSupported";
const _aZ = "availabilityZone";
const _bCI = "baseConfigurationItems";
const _c = "configuration";
const _cHDI = "configHistoryDeliveryInfo";
const _cI = "configurationItems";
const _cICT = "configurationItemCaptureTime";
const _cIDT = "configurationItemDeliveryTime";
const _cIMDH = "configurationItemMD5Hash";
const _cIS = "configurationItemStatus";
const _cO = "chronologicalOrder";
const _cSDI = "configSnapshotDeliveryInfo";
const _cSDIo = "configStreamDeliveryInfo";
const _cSDP = "configSnapshotDeliveryProperties";
const _cSI = "configurationStateId";
const _cSIo = "configSnapshotId";
const _cl = "client";
const _co = "count";
const _d = "description";
const _dCN = "deliveryChannelName";
const _dF = "deliveryFrequency";
const _e = "error";
const _eBRT = "exclusionByResourceTypes";
const _eT = "earlierTime";
const _fN = "filterName";
const _fV = "filterValue";
const _hE = "httpError";
const _iDR = "includeDeletedResources";
const _iGRT = "includeGlobalResourceTypes";
const _l = "limit";
const _lAT = "lastAttemptTime";
const _lEC = "lastErrorCode";
const _lEM = "lastErrorMessage";
const _lS = "lastStatus";
const _lSCT = "lastStatusChangeTime";
const _lST = "lastSuccessfulTime";
const _lSTa = "lastStartTime";
const _lSTas = "lastStopTime";
const _lT = "laterTime";
const _m = "message";
const _n = "name";
const _nDT = "nextDeliveryTime";
const _nT = "nextToken";
const _r = "relationships";
const _rARN = "roleARN";
const _rC = "resourceCounts";
const _rCT = "resourceCreationTime";
const _rDT = "resourceDeletionTime";
const _rE = "relatedEvents";
const _rF = "recordingFrequency";
const _rG = "recordingGroup";
const _rI = "resourceId";
const _rIe = "resourceIds";
const _rIes = "resourceIdentifiers";
const _rK = "resourceKeys";
const _rM = "recordingMode";
const _rMO = "recordingModeOverrides";
const _rN = "resourceName";
const _rNe = "relationshipName";
const _rS = "recordingScope";
const _rSe = "recordingStrategy";
const _rT = "resourceType";
const _rTe = "resourceTypes";
const _re = "recording";
const _s = "smithy.ts.sdk.synthetic.com.amazonaws.configservice";
const _sBN = "s3BucketName";
const _sC = "supplementaryConfiguration";
const _sKKA = "s3KmsKeyArn";
const _sKP = "s3KeyPrefix";
const _sP = "servicePrincipal";
const _sTARN = "snsTopicARN";
const _t = "tags";
const _tDR = "totalDiscoveredResources";
const _uO = "useOnly";
const _uRK = "unprocessedResourceKeys";
const _v = "version";
const n0 = "com.amazonaws.configservice";

// smithy-typescript generated code
import { TypeRegistry } from "@smithy/core/schema";
import type {
  StaticErrorSchema,
  StaticListSchema,
  StaticMapSchema,
  StaticOperationSchema,
  StaticStructureSchema,
} from "@smithy/types";

import { ConfigServiceServiceException } from "../models/ConfigServiceServiceException";
import {
  ConflictException,
  ConformancePackTemplateValidationException,
  IdempotentParameterMismatch,
  InsufficientDeliveryPolicyException,
  InsufficientPermissionsException,
  InvalidConfigurationRecorderNameException,
  InvalidDeliveryChannelNameException,
  InvalidExpressionException,
  InvalidLimitException,
  InvalidNextTokenException,
  InvalidParameterValueException,
  InvalidRecordingGroupException,
  InvalidResultTokenException,
  InvalidRoleException,
  InvalidS3KeyPrefixException,
  InvalidS3KmsKeyArnException,
  InvalidSNSTopicARNException,
  InvalidTimeRangeException,
  LastDeliveryChannelDeleteFailedException,
  LimitExceededException,
  MaxActiveResourcesExceededException,
  MaxNumberOfConfigRulesExceededException,
  MaxNumberOfConfigurationRecordersExceededException,
  MaxNumberOfConformancePacksExceededException,
  MaxNumberOfDeliveryChannelsExceededException,
  MaxNumberOfOrganizationConfigRulesExceededException,
  MaxNumberOfOrganizationConformancePacksExceededException,
  MaxNumberOfRetentionConfigurationsExceededException,
  NoAvailableConfigurationRecorderException,
  NoAvailableDeliveryChannelException,
  NoAvailableOrganizationException,
  NoRunningConfigurationRecorderException,
  NoSuchBucketException,
  NoSuchConfigRuleException,
  NoSuchConfigRuleInConformancePackException,
  NoSuchConfigurationAggregatorException,
  NoSuchConfigurationRecorderException,
  NoSuchConformancePackException,
  NoSuchDeliveryChannelException,
  NoSuchOrganizationConfigRuleException,
  NoSuchOrganizationConformancePackException,
  NoSuchRemediationConfigurationException,
  NoSuchRemediationExceptionException,
  NoSuchRetentionConfigurationException,
  OrganizationAccessDeniedException,
  OrganizationAllFeaturesNotEnabledException,
  OrganizationConformancePackTemplateValidationException,
  OversizedConfigurationItemException,
  RemediationInProgressException,
  ResourceConcurrentModificationException,
  ResourceInUseException,
  ResourceNotDiscoveredException,
  ResourceNotFoundException,
  TooManyTagsException,
  UnmodifiableEntityException,
  ValidationException,
} from "../models/errors";

/* eslint no-var: 0 */
export var AccountAggregationSource$: StaticStructureSchema = [3, n0, _AAS,
  0,
  [_AI, _AAR, _AR],
  [64 | 0, 2, 64 | 0]
];
export var AggregateComplianceByConfigRule$: StaticStructureSchema = [3, n0, _ACBCR,
  0,
  [_CRN, _C, _AIc, _ARw],
  [0, () => Compliance$, 0, 0]
];
export var AggregateComplianceByConformancePack$: StaticStructureSchema = [3, n0, _ACBCP,
  0,
  [_CPN, _C, _AIc, _ARw],
  [0, () => AggregateConformancePackCompliance$, 0, 0]
];
export var AggregateComplianceCount$: StaticStructureSchema = [3, n0, _ACC,
  0,
  [_GN, _CS],
  [0, () => ComplianceSummary$]
];
export var AggregateConformancePackCompliance$: StaticStructureSchema = [3, n0, _ACPC,
  0,
  [_CT, _CRC, _NCRC, _TRC],
  [0, 1, 1, 1]
];
export var AggregateConformancePackComplianceCount$: StaticStructureSchema = [3, n0, _ACPCC,
  0,
  [_CCPC, _NCCPC],
  [1, 1]
];
export var AggregateConformancePackComplianceFilters$: StaticStructureSchema = [3, n0, _ACPCF,
  0,
  [_CPN, _CT, _AIc, _ARw],
  [0, 0, 0, 0]
];
export var AggregateConformancePackComplianceSummary$: StaticStructureSchema = [3, n0, _ACPCS,
  0,
  [_CS, _GN],
  [() => AggregateConformancePackComplianceCount$, 0]
];
export var AggregateConformancePackComplianceSummaryFilters$: StaticStructureSchema = [3, n0, _ACPCSF,
  0,
  [_AIc, _ARw],
  [0, 0]
];
export var AggregatedSourceStatus$: StaticStructureSchema = [3, n0, _ASS,
  0,
  [_SI, _ST, _ARw, _LUS, _LUT, _LEC, _LEM],
  [0, 0, 0, 0, 4, 0, 0]
];
export var AggregateEvaluationResult$: StaticStructureSchema = [3, n0, _AER,
  0,
  [_ERI, _CT, _RRT, _CRIT, _A, _AIc, _ARw],
  [() => EvaluationResultIdentifier$, 0, 4, 4, 0, 0, 0]
];
export var AggregateResourceIdentifier$: StaticStructureSchema = [3, n0, _ARI,
  0,
  [_SAI, _SR, _RI, _RT, _RN],
  [0, 0, 0, 0, 0]
];
export var AggregationAuthorization$: StaticStructureSchema = [3, n0, _AA,
  0,
  [_AAA, _AAI, _AARu, _CTr],
  [0, 0, 0, 4]
];
export var AggregatorFilterResourceType$: StaticStructureSchema = [3, n0, _AFRT,
  0,
  [_T, _V],
  [0, 64 | 0]
];
export var AggregatorFilters$: StaticStructureSchema = [3, n0, _AF,
  0,
  [_RT, _SP],
  [() => AggregatorFilterResourceType$, () => AggregatorFilterServicePrincipal$]
];
export var AggregatorFilterServicePrincipal$: StaticStructureSchema = [3, n0, _AFSP,
  0,
  [_T, _V],
  [0, 64 | 0]
];
export var AssociateResourceTypesRequest$: StaticStructureSchema = [3, n0, _ARTR,
  0,
  [_CRA, _RTe],
  [0, 64 | 0]
];
export var AssociateResourceTypesResponse$: StaticStructureSchema = [3, n0, _ARTRs,
  0,
  [_CR],
  [() => ConfigurationRecorder$]
];
export var BaseConfigurationItem$: StaticStructureSchema = [3, n0, _BCI,
  0,
  [_v, _aI, _cICT, _cIS, _cSI, _a, _rT, _rI, _rN, _aR, _aZ, _rCT, _c, _sC, _rF, _cIDT],
  [0, 0, 4, 0, 0, 0, 0, 0, 0, 0, 0, 4, 0, 128 | 0, 0, 4]
];
export var BatchGetAggregateResourceConfigRequest$: StaticStructureSchema = [3, n0, _BGARCR,
  0,
  [_CAN, _RIe],
  [0, () => ResourceIdentifiersList]
];
export var BatchGetAggregateResourceConfigResponse$: StaticStructureSchema = [3, n0, _BGARCRa,
  0,
  [_BCIa, _URI],
  [() => BaseConfigurationItems, () => UnprocessedResourceIdentifierList]
];
export var BatchGetResourceConfigRequest$: StaticStructureSchema = [3, n0, _BGRCR,
  0,
  [_rK],
  [() => ResourceKeys]
];
export var BatchGetResourceConfigResponse$: StaticStructureSchema = [3, n0, _BGRCRa,
  0,
  [_bCI, _uRK],
  [() => BaseConfigurationItems, () => ResourceKeys]
];
export var Compliance$: StaticStructureSchema = [3, n0, _C,
  0,
  [_CT, _CCC],
  [0, () => ComplianceContributorCount$]
];
export var ComplianceByConfigRule$: StaticStructureSchema = [3, n0, _CBCR,
  0,
  [_CRN, _C],
  [0, () => Compliance$]
];
export var ComplianceByResource$: StaticStructureSchema = [3, n0, _CBR,
  0,
  [_RT, _RI, _C],
  [0, 0, () => Compliance$]
];
export var ComplianceContributorCount$: StaticStructureSchema = [3, n0, _CCC,
  0,
  [_CC, _CE],
  [1, 2]
];
export var ComplianceSummary$: StaticStructureSchema = [3, n0, _CS,
  0,
  [_CRCo, _NCRCo, _CST],
  [() => ComplianceContributorCount$, () => ComplianceContributorCount$, 4]
];
export var ComplianceSummaryByResourceType$: StaticStructureSchema = [3, n0, _CSBRT,
  0,
  [_RT, _CS],
  [0, () => ComplianceSummary$]
];
export var ConfigExportDeliveryInfo$: StaticStructureSchema = [3, n0, _CEDI,
  0,
  [_lS, _lEC, _lEM, _lAT, _lST, _nDT],
  [0, 0, 0, 4, 4, 4]
];
export var ConfigRule$: StaticStructureSchema = [3, n0, _CRo,
  0,
  [_CRN, _CRAo, _CRI, _D, _S, _So, _IP, _MEF, _CRS, _CB, _EM],
  [0, 0, 0, 0, () => Scope$, () => Source$, 0, 0, 0, 0, () => EvaluationModes]
];
export var ConfigRuleComplianceFilters$: StaticStructureSchema = [3, n0, _CRCF,
  0,
  [_CRN, _CT, _AIc, _ARw],
  [0, 0, 0, 0]
];
export var ConfigRuleComplianceSummaryFilters$: StaticStructureSchema = [3, n0, _CRCSF,
  0,
  [_AIc, _ARw],
  [0, 0]
];
export var ConfigRuleEvaluationStatus$: StaticStructureSchema = [3, n0, _CRES,
  0,
  [_CRN, _CRAo, _CRI, _LSIT, _LFIT, _LSET, _LFET, _FAT, _LDT, _LEC, _LEM, _FES, _LDLDS, _LDLDSR, _LDLDT],
  [0, 0, 0, 4, 4, 4, 4, 4, 4, 0, 0, 2, 0, 0, 4]
];
export var ConfigSnapshotDeliveryProperties$: StaticStructureSchema = [3, n0, _CSDP,
  0,
  [_dF],
  [0]
];
export var ConfigStreamDeliveryInfo$: StaticStructureSchema = [3, n0, _CSDI,
  0,
  [_lS, _lEC, _lEM, _lSCT],
  [0, 0, 0, 4]
];
export var ConfigurationAggregator$: StaticStructureSchema = [3, n0, _CA,
  0,
  [_CAN, _CAA, _AASc, _OAS, _CTr, _LUTa, _CB, _AF],
  [0, 0, () => AccountAggregationSourceList, () => OrganizationAggregationSource$, 4, 4, 0, () => AggregatorFilters$]
];
export var ConfigurationItem$: StaticStructureSchema = [3, n0, _CI,
  0,
  [_v, _aI, _cICT, _cIS, _cSI, _cIMDH, _a, _rT, _rI, _rN, _aR, _aZ, _rCT, _t, _rE, _r, _c, _sC, _rF, _cIDT],
  [0, 0, 4, 0, 0, 0, 0, 0, 0, 0, 0, 0, 4, 128 | 0, 64 | 0, () => RelationshipList, 0, 128 | 0, 0, 4]
];
export var ConfigurationRecorder$: StaticStructureSchema = [3, n0, _CR,
  0,
  [_a, _n, _rARN, _rG, _rM, _rS, _sP],
  [0, 0, 0, () => RecordingGroup$, () => RecordingMode$, 0, 0]
];
export var ConfigurationRecorderFilter$: StaticStructureSchema = [3, n0, _CRF,
  0,
  [_fN, _fV],
  [0, 64 | 0]
];
export var ConfigurationRecorderStatus$: StaticStructureSchema = [3, n0, _CRSo,
  0,
  [_a, _n, _lSTa, _lSTas, _re, _lS, _lEC, _lEM, _lSCT, _sP],
  [0, 0, 4, 4, 2, 0, 0, 0, 4, 0]
];
export var ConfigurationRecorderSummary$: StaticStructureSchema = [3, n0, _CRSon,
  0,
  [_a, _n, _sP, _rS],
  [0, 0, 0, 0]
];
export var ConflictException$: StaticErrorSchema = [-3, n0, _CEo,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(ConflictException$, ConflictException);
export var ConformancePackComplianceFilters$: StaticStructureSchema = [3, n0, _CPCF,
  0,
  [_CRNo, _CT],
  [64 | 0, 0]
];
export var ConformancePackComplianceScore$: StaticStructureSchema = [3, n0, _CPCS,
  0,
  [_Sc, _CPN, _LUTa],
  [0, 0, 4]
];
export var ConformancePackComplianceScoresFilters$: StaticStructureSchema = [3, n0, _CPCSF,
  0,
  [_CPNo],
  [64 | 0]
];
export var ConformancePackComplianceSummary$: StaticStructureSchema = [3, n0, _CPCSo,
  0,
  [_CPN, _CPCSon],
  [0, 0]
];
export var ConformancePackDetail$: StaticStructureSchema = [3, n0, _CPD,
  0,
  [_CPN, _CPA, _CPI, _DSB, _DSKP, _CPIP, _LURT, _CB, _TSSMDD],
  [0, 0, 0, 0, 0, () => ConformancePackInputParameters, 4, 0, () => TemplateSSMDocumentDetails$]
];
export var ConformancePackEvaluationFilters$: StaticStructureSchema = [3, n0, _CPEF,
  0,
  [_CRNo, _CT, _RT, _RIes],
  [64 | 0, 0, 0, 64 | 0]
];
export var ConformancePackEvaluationResult$: StaticStructureSchema = [3, n0, _CPER,
  0,
  [_CT, _ERI, _CRIT, _RRT, _A],
  [0, () => EvaluationResultIdentifier$, 4, 4, 0]
];
export var ConformancePackInputParameter$: StaticStructureSchema = [3, n0, _CPIPo,
  0,
  [_PN, _PV],
  [0, 0]
];
export var ConformancePackRuleCompliance$: StaticStructureSchema = [3, n0, _CPRC,
  0,
  [_CRN, _CT, _Co],
  [0, 0, 64 | 0]
];
export var ConformancePackStatusDetail$: StaticStructureSchema = [3, n0, _CPSD,
  0,
  [_CPN, _CPI, _CPA, _CPS, _SA, _CPSR, _LURT, _LUCT],
  [0, 0, 0, 0, 0, 0, 4, 4]
];
export var ConformancePackTemplateValidationException$: StaticErrorSchema = [-3, n0, _CPTVE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(ConformancePackTemplateValidationException$, ConformancePackTemplateValidationException);
export var CustomPolicyDetails$: StaticStructureSchema = [3, n0, _CPDu,
  0,
  [_PR, _PT, _EDLD],
  [0, 0, 2]
];
export var DeleteAggregationAuthorizationRequest$: StaticStructureSchema = [3, n0, _DAAR,
  0,
  [_AAI, _AARu],
  [0, 0]
];
export var DeleteConfigRuleRequest$: StaticStructureSchema = [3, n0, _DCRR,
  0,
  [_CRN],
  [0]
];
export var DeleteConfigurationAggregatorRequest$: StaticStructureSchema = [3, n0, _DCAR,
  0,
  [_CAN],
  [0]
];
export var DeleteConfigurationRecorderRequest$: StaticStructureSchema = [3, n0, _DCRRe,
  0,
  [_CRNon],
  [0]
];
export var DeleteConformancePackRequest$: StaticStructureSchema = [3, n0, _DCPR,
  0,
  [_CPN],
  [0]
];
export var DeleteDeliveryChannelRequest$: StaticStructureSchema = [3, n0, _DDCR,
  0,
  [_DCN],
  [0]
];
export var DeleteEvaluationResultsRequest$: StaticStructureSchema = [3, n0, _DERR,
  0,
  [_CRN],
  [0]
];
export var DeleteEvaluationResultsResponse$: StaticStructureSchema = [3, n0, _DERRe,
  0,
  [],
  []
];
export var DeleteOrganizationConfigRuleRequest$: StaticStructureSchema = [3, n0, _DOCRR,
  0,
  [_OCRN],
  [0]
];
export var DeleteOrganizationConformancePackRequest$: StaticStructureSchema = [3, n0, _DOCPR,
  0,
  [_OCPN],
  [0]
];
export var DeletePendingAggregationRequestRequest$: StaticStructureSchema = [3, n0, _DPARR,
  0,
  [_RAI, _RAR],
  [0, 0]
];
export var DeleteRemediationConfigurationRequest$: StaticStructureSchema = [3, n0, _DRCR,
  0,
  [_CRN, _RT],
  [0, 0]
];
export var DeleteRemediationConfigurationResponse$: StaticStructureSchema = [3, n0, _DRCRe,
  0,
  [],
  []
];
export var DeleteRemediationExceptionsRequest$: StaticStructureSchema = [3, n0, _DRER,
  0,
  [_CRN, _RK],
  [0, () => RemediationExceptionResourceKeys]
];
export var DeleteRemediationExceptionsResponse$: StaticStructureSchema = [3, n0, _DRERe,
  0,
  [_FB],
  [() => FailedDeleteRemediationExceptionsBatches]
];
export var DeleteResourceConfigRequest$: StaticStructureSchema = [3, n0, _DRCRel,
  0,
  [_RT, _RI],
  [0, 0]
];
export var DeleteRetentionConfigurationRequest$: StaticStructureSchema = [3, n0, _DRCRele,
  0,
  [_RCN],
  [0]
];
export var DeleteServiceLinkedConfigurationRecorderRequest$: StaticStructureSchema = [3, n0, _DSLCRR,
  0,
  [_SP],
  [0]
];
export var DeleteServiceLinkedConfigurationRecorderResponse$: StaticStructureSchema = [3, n0, _DSLCRRe,
  0,
  [_Ar, _N],
  [0, 0]
];
export var DeleteStoredQueryRequest$: StaticStructureSchema = [3, n0, _DSQR,
  0,
  [_QN],
  [0]
];
export var DeleteStoredQueryResponse$: StaticStructureSchema = [3, n0, _DSQRe,
  0,
  [],
  []
];
export var DeliverConfigSnapshotRequest$: StaticStructureSchema = [3, n0, _DCSR,
  0,
  [_dCN],
  [0]
];
export var DeliverConfigSnapshotResponse$: StaticStructureSchema = [3, n0, _DCSRe,
  0,
  [_cSIo],
  [0]
];
export var DeliveryChannel$: StaticStructureSchema = [3, n0, _DC,
  0,
  [_n, _sBN, _sKP, _sKKA, _sTARN, _cSDP],
  [0, 0, 0, 0, 0, () => ConfigSnapshotDeliveryProperties$]
];
export var DeliveryChannelStatus$: StaticStructureSchema = [3, n0, _DCS,
  0,
  [_n, _cSDI, _cHDI, _cSDIo],
  [0, () => ConfigExportDeliveryInfo$, () => ConfigExportDeliveryInfo$, () => ConfigStreamDeliveryInfo$]
];
export var DescribeAggregateComplianceByConfigRulesRequest$: StaticStructureSchema = [3, n0, _DACBCRR,
  0,
  [_CAN, _F, _L, _NT],
  [0, () => ConfigRuleComplianceFilters$, 1, 0]
];
export var DescribeAggregateComplianceByConfigRulesResponse$: StaticStructureSchema = [3, n0, _DACBCRRe,
  0,
  [_ACBCRg, _NT],
  [() => AggregateComplianceByConfigRuleList, 0]
];
export var DescribeAggregateComplianceByConformancePacksRequest$: StaticStructureSchema = [3, n0, _DACBCPR,
  0,
  [_CAN, _F, _L, _NT],
  [0, () => AggregateConformancePackComplianceFilters$, 1, 0]
];
export var DescribeAggregateComplianceByConformancePacksResponse$: StaticStructureSchema = [3, n0, _DACBCPRe,
  0,
  [_ACBCPg, _NT],
  [() => AggregateComplianceByConformancePackList, 0]
];
export var DescribeAggregationAuthorizationsRequest$: StaticStructureSchema = [3, n0, _DAARe,
  0,
  [_L, _NT],
  [1, 0]
];
export var DescribeAggregationAuthorizationsResponse$: StaticStructureSchema = [3, n0, _DAARes,
  0,
  [_AAg, _NT],
  [() => AggregationAuthorizationList, 0]
];
export var DescribeComplianceByConfigRuleRequest$: StaticStructureSchema = [3, n0, _DCBCRR,
  0,
  [_CRNo, _CTo, _NT],
  [64 | 0, 64 | 0, 0]
];
export var DescribeComplianceByConfigRuleResponse$: StaticStructureSchema = [3, n0, _DCBCRRe,
  0,
  [_CBCRo, _NT],
  [() => ComplianceByConfigRules, 0]
];
export var DescribeComplianceByResourceRequest$: StaticStructureSchema = [3, n0, _DCBRR,
  0,
  [_RT, _RI, _CTo, _L, _NT],
  [0, 0, 64 | 0, 1, 0]
];
export var DescribeComplianceByResourceResponse$: StaticStructureSchema = [3, n0, _DCBRRe,
  0,
  [_CBRo, _NT],
  [() => ComplianceByResources, 0]
];
export var DescribeConfigRuleEvaluationStatusRequest$: StaticStructureSchema = [3, n0, _DCRESR,
  0,
  [_CRNo, _NT, _L],
  [64 | 0, 0, 1]
];
export var DescribeConfigRuleEvaluationStatusResponse$: StaticStructureSchema = [3, n0, _DCRESRe,
  0,
  [_CRESo, _NT],
  [() => ConfigRuleEvaluationStatusList, 0]
];
export var DescribeConfigRulesFilters$: StaticStructureSchema = [3, n0, _DCRF,
  0,
  [_EMv],
  [0]
];
export var DescribeConfigRulesRequest$: StaticStructureSchema = [3, n0, _DCRRes,
  0,
  [_CRNo, _NT, _F],
  [64 | 0, 0, () => DescribeConfigRulesFilters$]
];
export var DescribeConfigRulesResponse$: StaticStructureSchema = [3, n0, _DCRResc,
  0,
  [_CRon, _NT],
  [() => ConfigRules, 0]
];
export var DescribeConfigurationAggregatorSourcesStatusRequest$: StaticStructureSchema = [3, n0, _DCASSR,
  0,
  [_CAN, _US, _NT, _L],
  [0, 64 | 0, 0, 1]
];
export var DescribeConfigurationAggregatorSourcesStatusResponse$: StaticStructureSchema = [3, n0, _DCASSRe,
  0,
  [_ASSL, _NT],
  [() => AggregatedSourceStatusList, 0]
];
export var DescribeConfigurationAggregatorsRequest$: StaticStructureSchema = [3, n0, _DCARe,
  0,
  [_CANo, _NT, _L],
  [64 | 0, 0, 1]
];
export var DescribeConfigurationAggregatorsResponse$: StaticStructureSchema = [3, n0, _DCARes,
  0,
  [_CAo, _NT],
  [() => ConfigurationAggregatorList, 0]
];
export var DescribeConfigurationRecordersRequest$: StaticStructureSchema = [3, n0, _DCRRescr,
  0,
  [_CRNonf, _SP, _Ar],
  [64 | 0, 0, 0]
];
export var DescribeConfigurationRecordersResponse$: StaticStructureSchema = [3, n0, _DCRRescri,
  0,
  [_CRonf],
  [() => ConfigurationRecorderList]
];
export var DescribeConfigurationRecorderStatusRequest$: StaticStructureSchema = [3, n0, _DCRSR,
  0,
  [_CRNonf, _SP, _Ar],
  [64 | 0, 0, 0]
];
export var DescribeConfigurationRecorderStatusResponse$: StaticStructureSchema = [3, n0, _DCRSRe,
  0,
  [_CRSonf],
  [() => ConfigurationRecorderStatusList]
];
export var DescribeConformancePackComplianceRequest$: StaticStructureSchema = [3, n0, _DCPCR,
  0,
  [_CPN, _F, _L, _NT],
  [0, () => ConformancePackComplianceFilters$, 1, 0]
];
export var DescribeConformancePackComplianceResponse$: StaticStructureSchema = [3, n0, _DCPCRe,
  0,
  [_CPN, _CPRCL, _NT],
  [0, () => ConformancePackRuleComplianceList, 0]
];
export var DescribeConformancePacksRequest$: StaticStructureSchema = [3, n0, _DCPRe,
  0,
  [_CPNo, _L, _NT],
  [64 | 0, 1, 0]
];
export var DescribeConformancePacksResponse$: StaticStructureSchema = [3, n0, _DCPRes,
  0,
  [_CPDo, _NT],
  [() => ConformancePackDetailList, 0]
];
export var DescribeConformancePackStatusRequest$: StaticStructureSchema = [3, n0, _DCPSR,
  0,
  [_CPNo, _L, _NT],
  [64 | 0, 1, 0]
];
export var DescribeConformancePackStatusResponse$: StaticStructureSchema = [3, n0, _DCPSRe,
  0,
  [_CPSDo, _NT],
  [() => ConformancePackStatusDetailsList, 0]
];
export var DescribeDeliveryChannelsRequest$: StaticStructureSchema = [3, n0, _DDCRe,
  0,
  [_DCNe],
  [64 | 0]
];
export var DescribeDeliveryChannelsResponse$: StaticStructureSchema = [3, n0, _DDCRes,
  0,
  [_DCe],
  [() => DeliveryChannelList]
];
export var DescribeDeliveryChannelStatusRequest$: StaticStructureSchema = [3, n0, _DDCSR,
  0,
  [_DCNe],
  [64 | 0]
];
export var DescribeDeliveryChannelStatusResponse$: StaticStructureSchema = [3, n0, _DDCSRe,
  0,
  [_DCSe],
  [() => DeliveryChannelStatusList]
];
export var DescribeOrganizationConfigRulesRequest$: StaticStructureSchema = [3, n0, _DOCRRe,
  0,
  [_OCRNr, _L, _NT],
  [64 | 0, 1, 0]
];
export var DescribeOrganizationConfigRulesResponse$: StaticStructureSchema = [3, n0, _DOCRRes,
  0,
  [_OCR, _NT],
  [() => OrganizationConfigRules, 0]
];
export var DescribeOrganizationConfigRuleStatusesRequest$: StaticStructureSchema = [3, n0, _DOCRSR,
  0,
  [_OCRNr, _L, _NT],
  [64 | 0, 1, 0]
];
export var DescribeOrganizationConfigRuleStatusesResponse$: StaticStructureSchema = [3, n0, _DOCRSRe,
  0,
  [_OCRS, _NT],
  [() => OrganizationConfigRuleStatuses, 0]
];
export var DescribeOrganizationConformancePacksRequest$: StaticStructureSchema = [3, n0, _DOCPRe,
  0,
  [_OCPNr, _L, _NT],
  [64 | 0, 1, 0]
];
export var DescribeOrganizationConformancePacksResponse$: StaticStructureSchema = [3, n0, _DOCPRes,
  0,
  [_OCP, _NT],
  [() => OrganizationConformancePacks, 0]
];
export var DescribeOrganizationConformancePackStatusesRequest$: StaticStructureSchema = [3, n0, _DOCPSR,
  0,
  [_OCPNr, _L, _NT],
  [64 | 0, 1, 0]
];
export var DescribeOrganizationConformancePackStatusesResponse$: StaticStructureSchema = [3, n0, _DOCPSRe,
  0,
  [_OCPS, _NT],
  [() => OrganizationConformancePackStatuses, 0]
];
export var DescribePendingAggregationRequestsRequest$: StaticStructureSchema = [3, n0, _DPARRe,
  0,
  [_L, _NT],
  [1, 0]
];
export var DescribePendingAggregationRequestsResponse$: StaticStructureSchema = [3, n0, _DPARRes,
  0,
  [_PAR, _NT],
  [() => PendingAggregationRequestList, 0]
];
export var DescribeRemediationConfigurationsRequest$: StaticStructureSchema = [3, n0, _DRCRes,
  0,
  [_CRNo],
  [64 | 0]
];
export var DescribeRemediationConfigurationsResponse$: StaticStructureSchema = [3, n0, _DRCResc,
  0,
  [_RC],
  [() => RemediationConfigurations]
];
export var DescribeRemediationExceptionsRequest$: StaticStructureSchema = [3, n0, _DRERes,
  0,
  [_CRN, _RK, _L, _NT],
  [0, () => RemediationExceptionResourceKeys, 1, 0]
];
export var DescribeRemediationExceptionsResponse$: StaticStructureSchema = [3, n0, _DREResc,
  0,
  [_RE, _NT],
  [() => RemediationExceptions, 0]
];
export var DescribeRemediationExecutionStatusRequest$: StaticStructureSchema = [3, n0, _DRESR,
  0,
  [_CRN, _RK, _L, _NT],
  [0, () => ResourceKeys, 1, 0]
];
export var DescribeRemediationExecutionStatusResponse$: StaticStructureSchema = [3, n0, _DRESRe,
  0,
  [_RES, _NT],
  [() => RemediationExecutionStatuses, 0]
];
export var DescribeRetentionConfigurationsRequest$: StaticStructureSchema = [3, n0, _DRCRescr,
  0,
  [_RCNe, _NT],
  [64 | 0, 0]
];
export var DescribeRetentionConfigurationsResponse$: StaticStructureSchema = [3, n0, _DRCRescri,
  0,
  [_RCe, _NT],
  [() => RetentionConfigurationList, 0]
];
export var DisassociateResourceTypesRequest$: StaticStructureSchema = [3, n0, _DRTR,
  0,
  [_CRA, _RTe],
  [0, 64 | 0]
];
export var DisassociateResourceTypesResponse$: StaticStructureSchema = [3, n0, _DRTRi,
  0,
  [_CR],
  [() => ConfigurationRecorder$]
];
export var Evaluation$: StaticStructureSchema = [3, n0, _E,
  0,
  [_CRT, _CRIo, _CT, _A, _OT],
  [0, 0, 0, 0, 4]
];
export var EvaluationContext$: StaticStructureSchema = [3, n0, _EC,
  0,
  [_ECI],
  [0]
];
export var EvaluationModeConfiguration$: StaticStructureSchema = [3, n0, _EMC,
  0,
  [_M],
  [0]
];
export var EvaluationResult$: StaticStructureSchema = [3, n0, _ER,
  0,
  [_ERI, _CT, _RRT, _CRIT, _A, _RTes],
  [() => EvaluationResultIdentifier$, 0, 4, 4, 0, 0]
];
export var EvaluationResultIdentifier$: StaticStructureSchema = [3, n0, _ERI,
  0,
  [_ERQ, _OT, _REI],
  [() => EvaluationResultQualifier$, 4, 0]
];
export var EvaluationResultQualifier$: StaticStructureSchema = [3, n0, _ERQ,
  0,
  [_CRN, _RT, _RI, _EMv],
  [0, 0, 0, 0]
];
export var EvaluationStatus$: StaticStructureSchema = [3, n0, _ES,
  0,
  [_St, _FR],
  [0, 0]
];
export var ExclusionByResourceTypes$: StaticStructureSchema = [3, n0, _EBRT,
  0,
  [_rTe],
  [64 | 0]
];
export var ExecutionControls$: StaticStructureSchema = [3, n0, _ECx,
  0,
  [_SC],
  [() => SsmControls$]
];
export var ExternalEvaluation$: StaticStructureSchema = [3, n0, _EE,
  0,
  [_CRT, _CRIo, _CT, _A, _OT],
  [0, 0, 0, 0, 4]
];
export var FailedDeleteRemediationExceptionsBatch$: StaticStructureSchema = [3, n0, _FDREB,
  0,
  [_FM, _FI],
  [0, () => RemediationExceptionResourceKeys]
];
export var FailedRemediationBatch$: StaticStructureSchema = [3, n0, _FRB,
  0,
  [_FM, _FI],
  [0, () => RemediationConfigurations]
];
export var FailedRemediationExceptionBatch$: StaticStructureSchema = [3, n0, _FREB,
  0,
  [_FM, _FI],
  [0, () => RemediationExceptions]
];
export var FieldInfo$: StaticStructureSchema = [3, n0, _FIi,
  0,
  [_N],
  [0]
];
export var GetAggregateComplianceDetailsByConfigRuleRequest$: StaticStructureSchema = [3, n0, _GACDBCRR,
  0,
  [_CAN, _CRN, _AIc, _ARw, _CT, _L, _NT],
  [0, 0, 0, 0, 0, 1, 0]
];
export var GetAggregateComplianceDetailsByConfigRuleResponse$: StaticStructureSchema = [3, n0, _GACDBCRRe,
  0,
  [_AERg, _NT],
  [() => AggregateEvaluationResultList, 0]
];
export var GetAggregateConfigRuleComplianceSummaryRequest$: StaticStructureSchema = [3, n0, _GACRCSR,
  0,
  [_CAN, _F, _GBK, _L, _NT],
  [0, () => ConfigRuleComplianceSummaryFilters$, 0, 1, 0]
];
export var GetAggregateConfigRuleComplianceSummaryResponse$: StaticStructureSchema = [3, n0, _GACRCSRe,
  0,
  [_GBK, _ACCg, _NT],
  [0, () => AggregateComplianceCountList, 0]
];
export var GetAggregateConformancePackComplianceSummaryRequest$: StaticStructureSchema = [3, n0, _GACPCSR,
  0,
  [_CAN, _F, _GBK, _L, _NT],
  [0, () => AggregateConformancePackComplianceSummaryFilters$, 0, 1, 0]
];
export var GetAggregateConformancePackComplianceSummaryResponse$: StaticStructureSchema = [3, n0, _GACPCSRe,
  0,
  [_ACPCSg, _GBK, _NT],
  [() => AggregateConformancePackComplianceSummaryList, 0, 0]
];
export var GetAggregateDiscoveredResourceCountsRequest$: StaticStructureSchema = [3, n0, _GADRCR,
  0,
  [_CAN, _F, _GBK, _L, _NT],
  [0, () => ResourceCountFilters$, 0, 1, 0]
];
export var GetAggregateDiscoveredResourceCountsResponse$: StaticStructureSchema = [3, n0, _GADRCRe,
  0,
  [_TDR, _GBK, _GRC, _NT],
  [1, 0, () => GroupedResourceCountList, 0]
];
export var GetAggregateResourceConfigRequest$: StaticStructureSchema = [3, n0, _GARCR,
  0,
  [_CAN, _RIeso],
  [0, () => AggregateResourceIdentifier$]
];
export var GetAggregateResourceConfigResponse$: StaticStructureSchema = [3, n0, _GARCRe,
  0,
  [_CI],
  [() => ConfigurationItem$]
];
export var GetComplianceDetailsByConfigRuleRequest$: StaticStructureSchema = [3, n0, _GCDBCRR,
  0,
  [_CRN, _CTo, _L, _NT],
  [0, 64 | 0, 1, 0]
];
export var GetComplianceDetailsByConfigRuleResponse$: StaticStructureSchema = [3, n0, _GCDBCRRe,
  0,
  [_ERv, _NT],
  [() => EvaluationResults, 0]
];
export var GetComplianceDetailsByResourceRequest$: StaticStructureSchema = [3, n0, _GCDBRR,
  0,
  [_RT, _RI, _CTo, _NT, _REI],
  [0, 0, 64 | 0, 0, 0]
];
export var GetComplianceDetailsByResourceResponse$: StaticStructureSchema = [3, n0, _GCDBRRe,
  0,
  [_ERv, _NT],
  [() => EvaluationResults, 0]
];
export var GetComplianceSummaryByConfigRuleResponse$: StaticStructureSchema = [3, n0, _GCSBCRR,
  0,
  [_CS],
  [() => ComplianceSummary$]
];
export var GetComplianceSummaryByResourceTypeRequest$: StaticStructureSchema = [3, n0, _GCSBRTR,
  0,
  [_RTe],
  [64 | 0]
];
export var GetComplianceSummaryByResourceTypeResponse$: StaticStructureSchema = [3, n0, _GCSBRTRe,
  0,
  [_CSBRTo],
  [() => ComplianceSummariesByResourceType]
];
export var GetConformancePackComplianceDetailsRequest$: StaticStructureSchema = [3, n0, _GCPCDR,
  0,
  [_CPN, _F, _L, _NT],
  [0, () => ConformancePackEvaluationFilters$, 1, 0]
];
export var GetConformancePackComplianceDetailsResponse$: StaticStructureSchema = [3, n0, _GCPCDRe,
  0,
  [_CPN, _CPRER, _NT],
  [0, () => ConformancePackRuleEvaluationResultsList, 0]
];
export var GetConformancePackComplianceSummaryRequest$: StaticStructureSchema = [3, n0, _GCPCSR,
  0,
  [_CPNo, _L, _NT],
  [64 | 0, 1, 0]
];
export var GetConformancePackComplianceSummaryResponse$: StaticStructureSchema = [3, n0, _GCPCSRe,
  0,
  [_CPCSL, _NT],
  [() => ConformancePackComplianceSummaryList, 0]
];
export var GetCustomRulePolicyRequest$: StaticStructureSchema = [3, n0, _GCRPR,
  0,
  [_CRN],
  [0]
];
export var GetCustomRulePolicyResponse$: StaticStructureSchema = [3, n0, _GCRPRe,
  0,
  [_PT],
  [0]
];
export var GetDiscoveredResourceCountsRequest$: StaticStructureSchema = [3, n0, _GDRCR,
  0,
  [_rTe, _l, _nT],
  [64 | 0, 1, 0]
];
export var GetDiscoveredResourceCountsResponse$: StaticStructureSchema = [3, n0, _GDRCRe,
  0,
  [_tDR, _rC, _nT],
  [1, () => ResourceCounts, 0]
];
export var GetOrganizationConfigRuleDetailedStatusRequest$: StaticStructureSchema = [3, n0, _GOCRDSR,
  0,
  [_OCRN, _F, _L, _NT],
  [0, () => StatusDetailFilters$, 1, 0]
];
export var GetOrganizationConfigRuleDetailedStatusResponse$: StaticStructureSchema = [3, n0, _GOCRDSRe,
  0,
  [_OCRDS, _NT],
  [() => OrganizationConfigRuleDetailedStatus, 0]
];
export var GetOrganizationConformancePackDetailedStatusRequest$: StaticStructureSchema = [3, n0, _GOCPDSR,
  0,
  [_OCPN, _F, _L, _NT],
  [0, () => OrganizationResourceDetailedStatusFilters$, 1, 0]
];
export var GetOrganizationConformancePackDetailedStatusResponse$: StaticStructureSchema = [3, n0, _GOCPDSRe,
  0,
  [_OCPDS, _NT],
  [() => OrganizationConformancePackDetailedStatuses, 0]
];
export var GetOrganizationCustomRulePolicyRequest$: StaticStructureSchema = [3, n0, _GOCRPR,
  0,
  [_OCRN],
  [0]
];
export var GetOrganizationCustomRulePolicyResponse$: StaticStructureSchema = [3, n0, _GOCRPRe,
  0,
  [_PT],
  [0]
];
export var GetResourceConfigHistoryRequest$: StaticStructureSchema = [3, n0, _GRCHR,
  0,
  [_rT, _rI, _lT, _eT, _cO, _l, _nT],
  [0, 0, 4, 4, 0, 1, 0]
];
export var GetResourceConfigHistoryResponse$: StaticStructureSchema = [3, n0, _GRCHRe,
  0,
  [_cI, _nT],
  [() => ConfigurationItemList, 0]
];
export var GetResourceEvaluationSummaryRequest$: StaticStructureSchema = [3, n0, _GRESR,
  0,
  [_REI],
  [0]
];
export var GetResourceEvaluationSummaryResponse$: StaticStructureSchema = [3, n0, _GRESRe,
  0,
  [_REI, _EMv, _ES, _EST, _C, _EC, _RD],
  [0, 0, () => EvaluationStatus$, 4, 0, () => EvaluationContext$, () => ResourceDetails$]
];
export var GetStoredQueryRequest$: StaticStructureSchema = [3, n0, _GSQR,
  0,
  [_QN],
  [0]
];
export var GetStoredQueryResponse$: StaticStructureSchema = [3, n0, _GSQRe,
  0,
  [_SQ],
  [() => StoredQuery$]
];
export var GroupedResourceCount$: StaticStructureSchema = [3, n0, _GRCr,
  0,
  [_GN, _RCes],
  [0, 1]
];
export var IdempotentParameterMismatch$: StaticErrorSchema = [-3, n0, _IPM,
  { [_e]: _cl, [_hE]: 400 },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(IdempotentParameterMismatch$, IdempotentParameterMismatch);
export var InsufficientDeliveryPolicyException$: StaticErrorSchema = [-3, n0, _IDPE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(InsufficientDeliveryPolicyException$, InsufficientDeliveryPolicyException);
export var InsufficientPermissionsException$: StaticErrorSchema = [-3, n0, _IPE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(InsufficientPermissionsException$, InsufficientPermissionsException);
export var InvalidConfigurationRecorderNameException$: StaticErrorSchema = [-3, n0, _ICRNE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidConfigurationRecorderNameException$, InvalidConfigurationRecorderNameException);
export var InvalidDeliveryChannelNameException$: StaticErrorSchema = [-3, n0, _IDCNE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidDeliveryChannelNameException$, InvalidDeliveryChannelNameException);
export var InvalidExpressionException$: StaticErrorSchema = [-3, n0, _IEE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidExpressionException$, InvalidExpressionException);
export var InvalidLimitException$: StaticErrorSchema = [-3, n0, _ILE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidLimitException$, InvalidLimitException);
export var InvalidNextTokenException$: StaticErrorSchema = [-3, n0, _INTE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidNextTokenException$, InvalidNextTokenException);
export var InvalidParameterValueException$: StaticErrorSchema = [-3, n0, _IPVE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidParameterValueException$, InvalidParameterValueException);
export var InvalidRecordingGroupException$: StaticErrorSchema = [-3, n0, _IRGE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidRecordingGroupException$, InvalidRecordingGroupException);
export var InvalidResultTokenException$: StaticErrorSchema = [-3, n0, _IRTE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidResultTokenException$, InvalidResultTokenException);
export var InvalidRoleException$: StaticErrorSchema = [-3, n0, _IRE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidRoleException$, InvalidRoleException);
export var InvalidS3KeyPrefixException$: StaticErrorSchema = [-3, n0, _ISKPE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidS3KeyPrefixException$, InvalidS3KeyPrefixException);
export var InvalidS3KmsKeyArnException$: StaticErrorSchema = [-3, n0, _ISKKAE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidS3KmsKeyArnException$, InvalidS3KmsKeyArnException);
export var InvalidSNSTopicARNException$: StaticErrorSchema = [-3, n0, _ISNSTARNE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidSNSTopicARNException$, InvalidSNSTopicARNException);
export var InvalidTimeRangeException$: StaticErrorSchema = [-3, n0, _ITRE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidTimeRangeException$, InvalidTimeRangeException);
export var LastDeliveryChannelDeleteFailedException$: StaticErrorSchema = [-3, n0, _LDCDFE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(LastDeliveryChannelDeleteFailedException$, LastDeliveryChannelDeleteFailedException);
export var LimitExceededException$: StaticErrorSchema = [-3, n0, _LEE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(LimitExceededException$, LimitExceededException);
export var ListAggregateDiscoveredResourcesRequest$: StaticStructureSchema = [3, n0, _LADRR,
  0,
  [_CAN, _RT, _F, _L, _NT],
  [0, 0, () => ResourceFilters$, 1, 0]
];
export var ListAggregateDiscoveredResourcesResponse$: StaticStructureSchema = [3, n0, _LADRRi,
  0,
  [_RIe, _NT],
  [() => DiscoveredResourceIdentifierList, 0]
];
export var ListConfigurationRecordersRequest$: StaticStructureSchema = [3, n0, _LCRR,
  0,
  [_F, _MR, _NT],
  [() => ConfigurationRecorderFilterList, 1, 0]
];
export var ListConfigurationRecordersResponse$: StaticStructureSchema = [3, n0, _LCRRi,
  0,
  [_CRSonfi, _NT],
  [() => ConfigurationRecorderSummaries, 0]
];
export var ListConformancePackComplianceScoresRequest$: StaticStructureSchema = [3, n0, _LCPCSR,
  0,
  [_F, _SO, _SB, _L, _NT],
  [() => ConformancePackComplianceScoresFilters$, 0, 0, 1, 0]
];
export var ListConformancePackComplianceScoresResponse$: StaticStructureSchema = [3, n0, _LCPCSRi,
  0,
  [_NT, _CPCSonf],
  [0, () => ConformancePackComplianceScores]
];
export var ListDiscoveredResourcesRequest$: StaticStructureSchema = [3, n0, _LDRR,
  0,
  [_rT, _rIe, _rN, _l, _iDR, _nT],
  [0, 64 | 0, 0, 1, 2, 0]
];
export var ListDiscoveredResourcesResponse$: StaticStructureSchema = [3, n0, _LDRRi,
  0,
  [_rIes, _nT],
  [() => ResourceIdentifierList, 0]
];
export var ListResourceEvaluationsRequest$: StaticStructureSchema = [3, n0, _LRER,
  0,
  [_F, _L, _NT],
  [() => ResourceEvaluationFilters$, 1, 0]
];
export var ListResourceEvaluationsResponse$: StaticStructureSchema = [3, n0, _LRERi,
  0,
  [_REe, _NT],
  [() => ResourceEvaluations, 0]
];
export var ListStoredQueriesRequest$: StaticStructureSchema = [3, n0, _LSQR,
  0,
  [_NT, _MR],
  [0, 1]
];
export var ListStoredQueriesResponse$: StaticStructureSchema = [3, n0, _LSQRi,
  0,
  [_SQM, _NT],
  [() => StoredQueryMetadataList, 0]
];
export var ListTagsForResourceRequest$: StaticStructureSchema = [3, n0, _LTFRR,
  0,
  [_RA, _L, _NT],
  [0, 1, 0]
];
export var ListTagsForResourceResponse$: StaticStructureSchema = [3, n0, _LTFRRi,
  0,
  [_Ta, _NT],
  [() => TagList, 0]
];
export var MaxActiveResourcesExceededException$: StaticErrorSchema = [-3, n0, _MAREE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(MaxActiveResourcesExceededException$, MaxActiveResourcesExceededException);
export var MaxNumberOfConfigRulesExceededException$: StaticErrorSchema = [-3, n0, _MNOCREE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(MaxNumberOfConfigRulesExceededException$, MaxNumberOfConfigRulesExceededException);
export var MaxNumberOfConfigurationRecordersExceededException$: StaticErrorSchema = [-3, n0, _MNOCREEa,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(MaxNumberOfConfigurationRecordersExceededException$, MaxNumberOfConfigurationRecordersExceededException);
export var MaxNumberOfConformancePacksExceededException$: StaticErrorSchema = [-3, n0, _MNOCPEE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(MaxNumberOfConformancePacksExceededException$, MaxNumberOfConformancePacksExceededException);
export var MaxNumberOfDeliveryChannelsExceededException$: StaticErrorSchema = [-3, n0, _MNODCEE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(MaxNumberOfDeliveryChannelsExceededException$, MaxNumberOfDeliveryChannelsExceededException);
export var MaxNumberOfOrganizationConfigRulesExceededException$: StaticErrorSchema = [-3, n0, _MNOOCREE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(MaxNumberOfOrganizationConfigRulesExceededException$, MaxNumberOfOrganizationConfigRulesExceededException);
export var MaxNumberOfOrganizationConformancePacksExceededException$: StaticErrorSchema = [-3, n0, _MNOOCPEE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(MaxNumberOfOrganizationConformancePacksExceededException$, MaxNumberOfOrganizationConformancePacksExceededException);
export var MaxNumberOfRetentionConfigurationsExceededException$: StaticErrorSchema = [-3, n0, _MNORCEE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(MaxNumberOfRetentionConfigurationsExceededException$, MaxNumberOfRetentionConfigurationsExceededException);
export var MemberAccountStatus$: StaticStructureSchema = [3, n0, _MAS,
  0,
  [_AIc, _CRN, _MARS, _ECr, _EMr, _LUT],
  [0, 0, 0, 0, 0, 4]
];
export var NoAvailableConfigurationRecorderException$: StaticErrorSchema = [-3, n0, _NACRE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(NoAvailableConfigurationRecorderException$, NoAvailableConfigurationRecorderException);
export var NoAvailableDeliveryChannelException$: StaticErrorSchema = [-3, n0, _NADCE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(NoAvailableDeliveryChannelException$, NoAvailableDeliveryChannelException);
export var NoAvailableOrganizationException$: StaticErrorSchema = [-3, n0, _NAOE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(NoAvailableOrganizationException$, NoAvailableOrganizationException);
export var NoRunningConfigurationRecorderException$: StaticErrorSchema = [-3, n0, _NRCRE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(NoRunningConfigurationRecorderException$, NoRunningConfigurationRecorderException);
export var NoSuchBucketException$: StaticErrorSchema = [-3, n0, _NSBE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(NoSuchBucketException$, NoSuchBucketException);
export var NoSuchConfigRuleException$: StaticErrorSchema = [-3, n0, _NSCRE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(NoSuchConfigRuleException$, NoSuchConfigRuleException);
export var NoSuchConfigRuleInConformancePackException$: StaticErrorSchema = [-3, n0, _NSCRICPE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(NoSuchConfigRuleInConformancePackException$, NoSuchConfigRuleInConformancePackException);
export var NoSuchConfigurationAggregatorException$: StaticErrorSchema = [-3, n0, _NSCAE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(NoSuchConfigurationAggregatorException$, NoSuchConfigurationAggregatorException);
export var NoSuchConfigurationRecorderException$: StaticErrorSchema = [-3, n0, _NSCREo,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(NoSuchConfigurationRecorderException$, NoSuchConfigurationRecorderException);
export var NoSuchConformancePackException$: StaticErrorSchema = [-3, n0, _NSCPE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(NoSuchConformancePackException$, NoSuchConformancePackException);
export var NoSuchDeliveryChannelException$: StaticErrorSchema = [-3, n0, _NSDCE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(NoSuchDeliveryChannelException$, NoSuchDeliveryChannelException);
export var NoSuchOrganizationConfigRuleException$: StaticErrorSchema = [-3, n0, _NSOCRE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(NoSuchOrganizationConfigRuleException$, NoSuchOrganizationConfigRuleException);
export var NoSuchOrganizationConformancePackException$: StaticErrorSchema = [-3, n0, _NSOCPE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(NoSuchOrganizationConformancePackException$, NoSuchOrganizationConformancePackException);
export var NoSuchRemediationConfigurationException$: StaticErrorSchema = [-3, n0, _NSRCE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(NoSuchRemediationConfigurationException$, NoSuchRemediationConfigurationException);
export var NoSuchRemediationExceptionException$: StaticErrorSchema = [-3, n0, _NSREE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(NoSuchRemediationExceptionException$, NoSuchRemediationExceptionException);
export var NoSuchRetentionConfigurationException$: StaticErrorSchema = [-3, n0, _NSRCEo,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(NoSuchRetentionConfigurationException$, NoSuchRetentionConfigurationException);
export var OrganizationAccessDeniedException$: StaticErrorSchema = [-3, n0, _OADE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(OrganizationAccessDeniedException$, OrganizationAccessDeniedException);
export var OrganizationAggregationSource$: StaticStructureSchema = [3, n0, _OAS,
  0,
  [_RAo, _AR, _AAR],
  [0, 64 | 0, 2]
];
export var OrganizationAllFeaturesNotEnabledException$: StaticErrorSchema = [-3, n0, _OAFNEE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(OrganizationAllFeaturesNotEnabledException$, OrganizationAllFeaturesNotEnabledException);
export var OrganizationConfigRule$: StaticStructureSchema = [3, n0, _OCRr,
  0,
  [_OCRN, _OCRA, _OMRM, _OCRM, _EA, _LUT, _OCPRM],
  [0, 0, () => OrganizationManagedRuleMetadata$, () => OrganizationCustomRuleMetadata$, 64 | 0, 4, () => OrganizationCustomPolicyRuleMetadataNoPolicy$]
];
export var OrganizationConfigRuleStatus$: StaticStructureSchema = [3, n0, _OCRSr,
  0,
  [_OCRN, _ORS, _ECr, _EMr, _LUT],
  [0, 0, 0, 0, 4]
];
export var OrganizationConformancePack$: StaticStructureSchema = [3, n0, _OCPr,
  0,
  [_OCPN, _OCPA, _DSB, _DSKP, _CPIP, _EA, _LUT],
  [0, 0, 0, 0, () => ConformancePackInputParameters, 64 | 0, 4]
];
export var OrganizationConformancePackDetailedStatus$: StaticStructureSchema = [3, n0, _OCPDSr,
  0,
  [_AIc, _CPN, _St, _ECr, _EMr, _LUT],
  [0, 0, 0, 0, 0, 4]
];
export var OrganizationConformancePackStatus$: StaticStructureSchema = [3, n0, _OCPSr,
  0,
  [_OCPN, _St, _ECr, _EMr, _LUT],
  [0, 0, 0, 0, 4]
];
export var OrganizationConformancePackTemplateValidationException$: StaticErrorSchema = [-3, n0, _OCPTVE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(OrganizationConformancePackTemplateValidationException$, OrganizationConformancePackTemplateValidationException);
export var OrganizationCustomPolicyRuleMetadata$: StaticStructureSchema = [3, n0, _OCPRM,
  0,
  [_D, _OCRTT, _IP, _MEF, _RTS, _RIS, _TKS, _TVS, _PR, _PT, _DLDA],
  [0, 64 | 0, 0, 0, 64 | 0, 0, 0, 0, 0, 0, 64 | 0]
];
export var OrganizationCustomPolicyRuleMetadataNoPolicy$: StaticStructureSchema = [3, n0, _OCPRMNP,
  0,
  [_D, _OCRTT, _IP, _MEF, _RTS, _RIS, _TKS, _TVS, _PR, _DLDA],
  [0, 64 | 0, 0, 0, 64 | 0, 0, 0, 0, 0, 64 | 0]
];
export var OrganizationCustomRuleMetadata$: StaticStructureSchema = [3, n0, _OCRM,
  0,
  [_D, _LFA, _OCRTT, _IP, _MEF, _RTS, _RIS, _TKS, _TVS],
  [0, 0, 64 | 0, 0, 0, 64 | 0, 0, 0, 0]
];
export var OrganizationManagedRuleMetadata$: StaticStructureSchema = [3, n0, _OMRM,
  0,
  [_D, _RIu, _IP, _MEF, _RTS, _RIS, _TKS, _TVS],
  [0, 0, 0, 0, 64 | 0, 0, 0, 0]
];
export var OrganizationResourceDetailedStatusFilters$: StaticStructureSchema = [3, n0, _ORDSF,
  0,
  [_AIc, _St],
  [0, 0]
];
export var OversizedConfigurationItemException$: StaticErrorSchema = [-3, n0, _OCIE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(OversizedConfigurationItemException$, OversizedConfigurationItemException);
export var PendingAggregationRequest$: StaticStructureSchema = [3, n0, _PARe,
  0,
  [_RAI, _RAR],
  [0, 0]
];
export var PutAggregationAuthorizationRequest$: StaticStructureSchema = [3, n0, _PAAR,
  0,
  [_AAI, _AARu, _Ta],
  [0, 0, () => TagsList]
];
export var PutAggregationAuthorizationResponse$: StaticStructureSchema = [3, n0, _PAARu,
  0,
  [_AA],
  [() => AggregationAuthorization$]
];
export var PutConfigRuleRequest$: StaticStructureSchema = [3, n0, _PCRR,
  0,
  [_CRo, _Ta],
  [() => ConfigRule$, () => TagsList]
];
export var PutConfigurationAggregatorRequest$: StaticStructureSchema = [3, n0, _PCAR,
  0,
  [_CAN, _AASc, _OAS, _Ta, _AF],
  [0, () => AccountAggregationSourceList, () => OrganizationAggregationSource$, () => TagsList, () => AggregatorFilters$]
];
export var PutConfigurationAggregatorResponse$: StaticStructureSchema = [3, n0, _PCARu,
  0,
  [_CA],
  [() => ConfigurationAggregator$]
];
export var PutConfigurationRecorderRequest$: StaticStructureSchema = [3, n0, _PCRRu,
  0,
  [_CR, _Ta],
  [() => ConfigurationRecorder$, () => TagsList]
];
export var PutConformancePackRequest$: StaticStructureSchema = [3, n0, _PCPR,
  0,
  [_CPN, _TSU, _TB, _DSB, _DSKP, _CPIP, _TSSMDD],
  [0, 0, 0, 0, 0, () => ConformancePackInputParameters, () => TemplateSSMDocumentDetails$]
];
export var PutConformancePackResponse$: StaticStructureSchema = [3, n0, _PCPRu,
  0,
  [_CPA],
  [0]
];
export var PutDeliveryChannelRequest$: StaticStructureSchema = [3, n0, _PDCR,
  0,
  [_DC],
  [() => DeliveryChannel$]
];
export var PutEvaluationsRequest$: StaticStructureSchema = [3, n0, _PER,
  0,
  [_Ev, _RTes, _TM],
  [() => Evaluations, 0, 2]
];
export var PutEvaluationsResponse$: StaticStructureSchema = [3, n0, _PERu,
  0,
  [_FE],
  [() => Evaluations]
];
export var PutExternalEvaluationRequest$: StaticStructureSchema = [3, n0, _PEER,
  0,
  [_CRN, _EE],
  [0, () => ExternalEvaluation$]
];
export var PutExternalEvaluationResponse$: StaticStructureSchema = [3, n0, _PEERu,
  0,
  [],
  []
];
export var PutOrganizationConfigRuleRequest$: StaticStructureSchema = [3, n0, _POCRR,
  0,
  [_OCRN, _OMRM, _OCRM, _EA, _OCPRM],
  [0, () => OrganizationManagedRuleMetadata$, () => OrganizationCustomRuleMetadata$, 64 | 0, () => OrganizationCustomPolicyRuleMetadata$]
];
export var PutOrganizationConfigRuleResponse$: StaticStructureSchema = [3, n0, _POCRRu,
  0,
  [_OCRA],
  [0]
];
export var PutOrganizationConformancePackRequest$: StaticStructureSchema = [3, n0, _POCPR,
  0,
  [_OCPN, _TSU, _TB, _DSB, _DSKP, _CPIP, _EA],
  [0, 0, 0, 0, 0, () => ConformancePackInputParameters, 64 | 0]
];
export var PutOrganizationConformancePackResponse$: StaticStructureSchema = [3, n0, _POCPRu,
  0,
  [_OCPA],
  [0]
];
export var PutRemediationConfigurationsRequest$: StaticStructureSchema = [3, n0, _PRCR,
  0,
  [_RC],
  [() => RemediationConfigurations]
];
export var PutRemediationConfigurationsResponse$: StaticStructureSchema = [3, n0, _PRCRu,
  0,
  [_FB],
  [() => FailedRemediationBatches]
];
export var PutRemediationExceptionsRequest$: StaticStructureSchema = [3, n0, _PRER,
  0,
  [_CRN, _RK, _Me, _ET],
  [0, () => RemediationExceptionResourceKeys, 0, 4]
];
export var PutRemediationExceptionsResponse$: StaticStructureSchema = [3, n0, _PRERu,
  0,
  [_FB],
  [() => FailedRemediationExceptionBatches]
];
export var PutResourceConfigRequest$: StaticStructureSchema = [3, n0, _PRCRut,
  0,
  [_RT, _SVI, _RI, _RN, _Con, _Ta],
  [0, 0, 0, 0, 0, 128 | 0]
];
export var PutRetentionConfigurationRequest$: StaticStructureSchema = [3, n0, _PRCRute,
  0,
  [_RPID],
  [1]
];
export var PutRetentionConfigurationResponse$: StaticStructureSchema = [3, n0, _PRCRutet,
  0,
  [_RCet],
  [() => RetentionConfiguration$]
];
export var PutServiceLinkedConfigurationRecorderRequest$: StaticStructureSchema = [3, n0, _PSLCRR,
  0,
  [_SP, _Ta],
  [0, () => TagsList]
];
export var PutServiceLinkedConfigurationRecorderResponse$: StaticStructureSchema = [3, n0, _PSLCRRu,
  0,
  [_Ar, _N],
  [0, 0]
];
export var PutStoredQueryRequest$: StaticStructureSchema = [3, n0, _PSQR,
  0,
  [_SQ, _Ta],
  [() => StoredQuery$, () => TagsList]
];
export var PutStoredQueryResponse$: StaticStructureSchema = [3, n0, _PSQRu,
  0,
  [_QA],
  [0]
];
export var QueryInfo$: StaticStructureSchema = [3, n0, _QI,
  0,
  [_SF],
  [() => FieldInfoList]
];
export var RecordingGroup$: StaticStructureSchema = [3, n0, _RG,
  0,
  [_aS, _iGRT, _rTe, _eBRT, _rSe],
  [2, 2, 64 | 0, () => ExclusionByResourceTypes$, () => RecordingStrategy$]
];
export var RecordingMode$: StaticStructureSchema = [3, n0, _RM,
  0,
  [_rF, _rMO],
  [0, () => RecordingModeOverrides]
];
export var RecordingModeOverride$: StaticStructureSchema = [3, n0, _RMO,
  0,
  [_d, _rTe, _rF],
  [0, 64 | 0, 0]
];
export var RecordingStrategy$: StaticStructureSchema = [3, n0, _RS,
  0,
  [_uO],
  [0]
];
export var Relationship$: StaticStructureSchema = [3, n0, _R,
  0,
  [_rT, _rI, _rN, _rNe],
  [0, 0, 0, 0]
];
export var RemediationConfiguration$: StaticStructureSchema = [3, n0, _RCem,
  0,
  [_CRN, _TT, _TI, _TV, _P, _RT, _Au, _ECx, _MAA, _RAS, _Ar, _CBS],
  [0, 0, 0, 0, () => RemediationParameters, 0, 2, () => ExecutionControls$, 1, 1, 0, 0]
];
export var RemediationException$: StaticStructureSchema = [3, n0, _REem,
  0,
  [_CRN, _RT, _RI, _Me, _ET],
  [0, 0, 0, 0, 4]
];
export var RemediationExceptionResourceKey$: StaticStructureSchema = [3, n0, _RERK,
  0,
  [_RT, _RI],
  [0, 0]
];
export var RemediationExecutionStatus$: StaticStructureSchema = [3, n0, _RESe,
  0,
  [_RKe, _Sta, _SD, _IT, _LUTa],
  [() => ResourceKey$, 0, () => RemediationExecutionSteps, 4, 4]
];
export var RemediationExecutionStep$: StaticStructureSchema = [3, n0, _RESem,
  0,
  [_N, _Sta, _EMr, _STt, _STto],
  [0, 0, 0, 4, 4]
];
export var RemediationInProgressException$: StaticErrorSchema = [-3, n0, _RIPE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(RemediationInProgressException$, RemediationInProgressException);
export var RemediationParameterValue$: StaticStructureSchema = [3, n0, _RPV,
  0,
  [_RV, _SV],
  [() => ResourceValue$, () => StaticValue$]
];
export var ResourceConcurrentModificationException$: StaticErrorSchema = [-3, n0, _RCME,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(ResourceConcurrentModificationException$, ResourceConcurrentModificationException);
export var ResourceCount$: StaticStructureSchema = [3, n0, _RCes,
  0,
  [_rT, _co],
  [0, 1]
];
export var ResourceCountFilters$: StaticStructureSchema = [3, n0, _RCF,
  0,
  [_RT, _AIc, _Re],
  [0, 0, 0]
];
export var ResourceDetails$: StaticStructureSchema = [3, n0, _RD,
  0,
  [_RI, _RT, _RCeso, _RCST],
  [0, 0, 0, 0]
];
export var ResourceEvaluation$: StaticStructureSchema = [3, n0, _REes,
  0,
  [_REI, _EMv, _EST],
  [0, 0, 4]
];
export var ResourceEvaluationFilters$: StaticStructureSchema = [3, n0, _REF,
  0,
  [_EMv, _TW, _ECI],
  [0, () => TimeWindow$, 0]
];
export var ResourceFilters$: StaticStructureSchema = [3, n0, _RF,
  0,
  [_AIc, _RI, _RN, _Re],
  [0, 0, 0, 0]
];
export var ResourceIdentifier$: StaticStructureSchema = [3, n0, _RIeso,
  0,
  [_rT, _rI, _rN, _rDT],
  [0, 0, 0, 4]
];
export var ResourceInUseException$: StaticErrorSchema = [-3, n0, _RIUE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(ResourceInUseException$, ResourceInUseException);
export var ResourceKey$: StaticStructureSchema = [3, n0, _RKe,
  0,
  [_rT, _rI],
  [0, 0]
];
export var ResourceNotDiscoveredException$: StaticErrorSchema = [-3, n0, _RNDE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(ResourceNotDiscoveredException$, ResourceNotDiscoveredException);
export var ResourceNotFoundException$: StaticErrorSchema = [-3, n0, _RNFE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(ResourceNotFoundException$, ResourceNotFoundException);
export var ResourceValue$: StaticStructureSchema = [3, n0, _RV,
  0,
  [_V],
  [0]
];
export var RetentionConfiguration$: StaticStructureSchema = [3, n0, _RCet,
  0,
  [_N, _RPID],
  [0, 1]
];
export var Scope$: StaticStructureSchema = [3, n0, _S,
  0,
  [_CRTo, _TK, _TVa, _CRIo],
  [64 | 0, 0, 0, 0]
];
export var SelectAggregateResourceConfigRequest$: StaticStructureSchema = [3, n0, _SARCR,
  0,
  [_Ex, _CAN, _L, _MR, _NT],
  [0, 0, 1, 1, 0]
];
export var SelectAggregateResourceConfigResponse$: StaticStructureSchema = [3, n0, _SARCRe,
  0,
  [_Res, _QI, _NT],
  [64 | 0, () => QueryInfo$, 0]
];
export var SelectResourceConfigRequest$: StaticStructureSchema = [3, n0, _SRCR,
  0,
  [_Ex, _L, _NT],
  [0, 1, 0]
];
export var SelectResourceConfigResponse$: StaticStructureSchema = [3, n0, _SRCRe,
  0,
  [_Res, _QI, _NT],
  [64 | 0, () => QueryInfo$, 0]
];
export var Source$: StaticStructureSchema = [3, n0, _So,
  0,
  [_O, _SIo, _SDo, _CPDu],
  [0, 0, () => SourceDetails, () => CustomPolicyDetails$]
];
export var SourceDetail$: StaticStructureSchema = [3, n0, _SDou,
  0,
  [_ESv, _MT, _MEF],
  [0, 0, 0]
];
export var SsmControls$: StaticStructureSchema = [3, n0, _SC,
  0,
  [_CERP, _EP],
  [1, 1]
];
export var StartConfigRulesEvaluationRequest$: StaticStructureSchema = [3, n0, _SCRER,
  0,
  [_CRNo],
  [64 | 0]
];
export var StartConfigRulesEvaluationResponse$: StaticStructureSchema = [3, n0, _SCRERt,
  0,
  [],
  []
];
export var StartConfigurationRecorderRequest$: StaticStructureSchema = [3, n0, _SCRR,
  0,
  [_CRNon],
  [0]
];
export var StartRemediationExecutionRequest$: StaticStructureSchema = [3, n0, _SRER,
  0,
  [_CRN, _RK],
  [0, () => ResourceKeys]
];
export var StartRemediationExecutionResponse$: StaticStructureSchema = [3, n0, _SRERt,
  0,
  [_FM, _FI],
  [0, () => ResourceKeys]
];
export var StartResourceEvaluationRequest$: StaticStructureSchema = [3, n0, _SRERta,
  0,
  [_RD, _EC, _EMv, _ETv, _CTl],
  [() => ResourceDetails$, () => EvaluationContext$, 0, 1, 0]
];
export var StartResourceEvaluationResponse$: StaticStructureSchema = [3, n0, _SRERtar,
  0,
  [_REI],
  [0]
];
export var StaticValue$: StaticStructureSchema = [3, n0, _SV,
  0,
  [_Va],
  [64 | 0]
];
export var StatusDetailFilters$: StaticStructureSchema = [3, n0, _SDF,
  0,
  [_AIc, _MARS],
  [0, 0]
];
export var StopConfigurationRecorderRequest$: StaticStructureSchema = [3, n0, _SCRRt,
  0,
  [_CRNon],
  [0]
];
export var StoredQuery$: StaticStructureSchema = [3, n0, _SQ,
  0,
  [_QIu, _QA, _QN, _D, _Ex],
  [0, 0, 0, 0, 0]
];
export var StoredQueryMetadata$: StaticStructureSchema = [3, n0, _SQM,
  0,
  [_QIu, _QA, _QN, _D],
  [0, 0, 0, 0]
];
export var Tag$: StaticStructureSchema = [3, n0, _Tag,
  0,
  [_K, _V],
  [0, 0]
];
export var TagResourceRequest$: StaticStructureSchema = [3, n0, _TRR,
  0,
  [_RA, _Ta],
  [0, () => TagList]
];
export var TemplateSSMDocumentDetails$: StaticStructureSchema = [3, n0, _TSSMDD,
  0,
  [_DN, _DV],
  [0, 0]
];
export var TimeWindow$: StaticStructureSchema = [3, n0, _TW,
  0,
  [_STt, _ETn],
  [4, 4]
];
export var TooManyTagsException$: StaticErrorSchema = [-3, n0, _TMTE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(TooManyTagsException$, TooManyTagsException);
export var UnmodifiableEntityException$: StaticErrorSchema = [-3, n0, _UEE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(UnmodifiableEntityException$, UnmodifiableEntityException);
export var UntagResourceRequest$: StaticStructureSchema = [3, n0, _URR,
  0,
  [_RA, _TKa],
  [0, 64 | 0]
];
export var ValidationException$: StaticErrorSchema = [-3, n0, _VE,
  { [_e]: _cl },
  [_m],
  [0]
];
TypeRegistry.for(n0).registerError(ValidationException$, ValidationException);
var __Unit = "unit" as const;
export var ConfigServiceServiceException$: StaticErrorSchema = [-3, _s, "ConfigServiceServiceException", 0, [], []];
TypeRegistry.for(_s).registerError(ConfigServiceServiceException$, ConfigServiceServiceException);
var AccountAggregationSourceAccountList = 64 | 0;
var AccountAggregationSourceList: StaticListSchema = [1, n0, _AASL,
  0, () => AccountAggregationSource$
];
var AggregateComplianceByConfigRuleList: StaticListSchema = [1, n0, _ACBCRL,
  0, () => AggregateComplianceByConfigRule$
];
var AggregateComplianceByConformancePackList: StaticListSchema = [1, n0, _ACBCPL,
  0, () => AggregateComplianceByConformancePack$
];
var AggregateComplianceCountList: StaticListSchema = [1, n0, _ACCL,
  0, () => AggregateComplianceCount$
];
var AggregateConformancePackComplianceSummaryList: StaticListSchema = [1, n0, _ACPCSL,
  0, () => AggregateConformancePackComplianceSummary$
];
var AggregatedSourceStatusList: StaticListSchema = [1, n0, _ASSL,
  0, () => AggregatedSourceStatus$
];
var AggregatedSourceStatusTypeList = 64 | 0;
var AggregateEvaluationResultList: StaticListSchema = [1, n0, _AERL,
  0, () => AggregateEvaluationResult$
];
var AggregationAuthorizationList: StaticListSchema = [1, n0, _AAL,
  0, () => AggregationAuthorization$
];
var AggregatorRegionList = 64 | 0;
var BaseConfigurationItems: StaticListSchema = [1, n0, _BCIa,
  0, () => BaseConfigurationItem$
];
var ComplianceByConfigRules: StaticListSchema = [1, n0, _CBCRo,
  0, () => ComplianceByConfigRule$
];
var ComplianceByResources: StaticListSchema = [1, n0, _CBRo,
  0, () => ComplianceByResource$
];
var ComplianceResourceTypes = 64 | 0;
var ComplianceSummariesByResourceType: StaticListSchema = [1, n0, _CSBRTo,
  0, () => ComplianceSummaryByResourceType$
];
var ComplianceTypes = 64 | 0;
var ConfigRuleEvaluationStatusList: StaticListSchema = [1, n0, _CRESL,
  0, () => ConfigRuleEvaluationStatus$
];
var ConfigRuleNames = 64 | 0;
var ConfigRules: StaticListSchema = [1, n0, _CRon,
  0, () => ConfigRule$
];
var ConfigurationAggregatorList: StaticListSchema = [1, n0, _CAL,
  0, () => ConfigurationAggregator$
];
var ConfigurationAggregatorNameList = 64 | 0;
var ConfigurationItemList: StaticListSchema = [1, n0, _CIL,
  0, () => ConfigurationItem$
];
var ConfigurationRecorderFilterList: StaticListSchema = [1, n0, _CRFL,
  0, () => ConfigurationRecorderFilter$
];
var ConfigurationRecorderFilterValues = 64 | 0;
var ConfigurationRecorderList: StaticListSchema = [1, n0, _CRL,
  0, () => ConfigurationRecorder$
];
var ConfigurationRecorderNameList = 64 | 0;
var ConfigurationRecorderStatusList: StaticListSchema = [1, n0, _CRSL,
  0, () => ConfigurationRecorderStatus$
];
var ConfigurationRecorderSummaries: StaticListSchema = [1, n0, _CRSonfi,
  0, () => ConfigurationRecorderSummary$
];
var ConformancePackComplianceResourceIds = 64 | 0;
var ConformancePackComplianceScores: StaticListSchema = [1, n0, _CPCSonf,
  0, () => ConformancePackComplianceScore$
];
var ConformancePackComplianceSummaryList: StaticListSchema = [1, n0, _CPCSL,
  0, () => ConformancePackComplianceSummary$
];
var ConformancePackConfigRuleNames = 64 | 0;
var ConformancePackDetailList: StaticListSchema = [1, n0, _CPDL,
  0, () => ConformancePackDetail$
];
var ConformancePackInputParameters: StaticListSchema = [1, n0, _CPIP,
  0, () => ConformancePackInputParameter$
];
var ConformancePackNameFilter = 64 | 0;
var ConformancePackNamesList = 64 | 0;
var ConformancePackNamesToSummarizeList = 64 | 0;
var ConformancePackRuleComplianceList: StaticListSchema = [1, n0, _CPRCL,
  0, () => ConformancePackRuleCompliance$
];
var ConformancePackRuleEvaluationResultsList: StaticListSchema = [1, n0, _CPRERL,
  0, () => ConformancePackEvaluationResult$
];
var ConformancePackStatusDetailsList: StaticListSchema = [1, n0, _CPSDL,
  0, () => ConformancePackStatusDetail$
];
var ControlsList = 64 | 0;
var DebugLogDeliveryAccounts = 64 | 0;
var DeliveryChannelList: StaticListSchema = [1, n0, _DCL,
  0, () => DeliveryChannel$
];
var DeliveryChannelNameList = 64 | 0;
var DeliveryChannelStatusList: StaticListSchema = [1, n0, _DCSL,
  0, () => DeliveryChannelStatus$
];
var DiscoveredResourceIdentifierList: StaticListSchema = [1, n0, _DRIL,
  0, () => AggregateResourceIdentifier$
];
var EvaluationModes: StaticListSchema = [1, n0, _EM,
  0, () => EvaluationModeConfiguration$
];
var EvaluationResults: StaticListSchema = [1, n0, _ERv,
  0, () => EvaluationResult$
];
var Evaluations: StaticListSchema = [1, n0, _Ev,
  0, () => Evaluation$
];
var ExcludedAccounts = 64 | 0;
var FailedDeleteRemediationExceptionsBatches: StaticListSchema = [1, n0, _FDREBa,
  0, () => FailedDeleteRemediationExceptionsBatch$
];
var FailedRemediationBatches: StaticListSchema = [1, n0, _FRBa,
  0, () => FailedRemediationBatch$
];
var FailedRemediationExceptionBatches: StaticListSchema = [1, n0, _FREBa,
  0, () => FailedRemediationExceptionBatch$
];
var FieldInfoList: StaticListSchema = [1, n0, _FIL,
  0, () => FieldInfo$
];
var GroupedResourceCountList: StaticListSchema = [1, n0, _GRCL,
  0, () => GroupedResourceCount$
];
var OrganizationConfigRuleDetailedStatus: StaticListSchema = [1, n0, _OCRDS,
  0, () => MemberAccountStatus$
];
var OrganizationConfigRuleNames = 64 | 0;
var OrganizationConfigRules: StaticListSchema = [1, n0, _OCR,
  0, () => OrganizationConfigRule$
];
var OrganizationConfigRuleStatuses: StaticListSchema = [1, n0, _OCRS,
  0, () => OrganizationConfigRuleStatus$
];
var OrganizationConfigRuleTriggerTypeNoSNs = 64 | 0;
var OrganizationConfigRuleTriggerTypes = 64 | 0;
var OrganizationConformancePackDetailedStatuses: StaticListSchema = [1, n0, _OCPDS,
  0, () => OrganizationConformancePackDetailedStatus$
];
var OrganizationConformancePackNames = 64 | 0;
var OrganizationConformancePacks: StaticListSchema = [1, n0, _OCP,
  0, () => OrganizationConformancePack$
];
var OrganizationConformancePackStatuses: StaticListSchema = [1, n0, _OCPS,
  0, () => OrganizationConformancePackStatus$
];
var PendingAggregationRequestList: StaticListSchema = [1, n0, _PARL,
  0, () => PendingAggregationRequest$
];
var RecordingModeOverrides: StaticListSchema = [1, n0, _RMOe,
  0, () => RecordingModeOverride$
];
var RecordingModeResourceTypesList = 64 | 0;
var ReevaluateConfigRuleNames = 64 | 0;
var RelatedEventList = 64 | 0;
var RelationshipList: StaticListSchema = [1, n0, _RL,
  0, () => Relationship$
];
var RemediationConfigurations: StaticListSchema = [1, n0, _RC,
  0, () => RemediationConfiguration$
];
var RemediationExceptionResourceKeys: StaticListSchema = [1, n0, _RERKe,
  0, () => RemediationExceptionResourceKey$
];
var RemediationExceptions: StaticListSchema = [1, n0, _RE,
  0, () => RemediationException$
];
var RemediationExecutionStatuses: StaticListSchema = [1, n0, _RES,
  0, () => RemediationExecutionStatus$
];
var RemediationExecutionSteps: StaticListSchema = [1, n0, _RESeme,
  0, () => RemediationExecutionStep$
];
var ResourceCounts: StaticListSchema = [1, n0, _RCesou,
  0, () => ResourceCount$
];
var ResourceEvaluations: StaticListSchema = [1, n0, _REe,
  0, () => ResourceEvaluation$
];
var ResourceIdentifierList: StaticListSchema = [1, n0, _RIL,
  0, () => ResourceIdentifier$
];
var ResourceIdentifiersList: StaticListSchema = [1, n0, _RILe,
  0, () => AggregateResourceIdentifier$
];
var ResourceIdList = 64 | 0;
var ResourceKeys: StaticListSchema = [1, n0, _RK,
  0, () => ResourceKey$
];
var ResourceTypeList = 64 | 0;
var ResourceTypes = 64 | 0;
var ResourceTypesScope = 64 | 0;
var ResourceTypeValueList = 64 | 0;
var Results = 64 | 0;
var RetentionConfigurationList: StaticListSchema = [1, n0, _RCL,
  0, () => RetentionConfiguration$
];
var RetentionConfigurationNameList = 64 | 0;
var ServicePrincipalValueList = 64 | 0;
var SourceDetails: StaticListSchema = [1, n0, _SDo,
  0, () => SourceDetail$
];
var StaticParameterValues = 64 | 0;
var StoredQueryMetadataList: StaticListSchema = [1, n0, _SQML,
  0, () => StoredQueryMetadata$
];
var TagKeyList = 64 | 0;
var TagList: StaticListSchema = [1, n0, _TL,
  0, () => Tag$
];
var TagsList: StaticListSchema = [1, n0, _TLa,
  0, () => Tag$
];
var UnprocessedResourceIdentifierList: StaticListSchema = [1, n0, _URIL,
  0, () => AggregateResourceIdentifier$
];
var RemediationParameters: StaticMapSchema = [2, n0, _RP,
  0, 0, () => RemediationParameterValue$
];
var SupplementaryConfiguration = 128 | 0;
var Tags = 128 | 0;
export var AssociateResourceTypes$: StaticOperationSchema = [9, n0, _ART,
  0, () => AssociateResourceTypesRequest$, () => AssociateResourceTypesResponse$
];
export var BatchGetAggregateResourceConfig$: StaticOperationSchema = [9, n0, _BGARC,
  0, () => BatchGetAggregateResourceConfigRequest$, () => BatchGetAggregateResourceConfigResponse$
];
export var BatchGetResourceConfig$: StaticOperationSchema = [9, n0, _BGRC,
  0, () => BatchGetResourceConfigRequest$, () => BatchGetResourceConfigResponse$
];
export var DeleteAggregationAuthorization$: StaticOperationSchema = [9, n0, _DAA,
  0, () => DeleteAggregationAuthorizationRequest$, () => __Unit
];
export var DeleteConfigRule$: StaticOperationSchema = [9, n0, _DCR,
  0, () => DeleteConfigRuleRequest$, () => __Unit
];
export var DeleteConfigurationAggregator$: StaticOperationSchema = [9, n0, _DCA,
  0, () => DeleteConfigurationAggregatorRequest$, () => __Unit
];
export var DeleteConfigurationRecorder$: StaticOperationSchema = [9, n0, _DCRe,
  0, () => DeleteConfigurationRecorderRequest$, () => __Unit
];
export var DeleteConformancePack$: StaticOperationSchema = [9, n0, _DCP,
  0, () => DeleteConformancePackRequest$, () => __Unit
];
export var DeleteDeliveryChannel$: StaticOperationSchema = [9, n0, _DDC,
  0, () => DeleteDeliveryChannelRequest$, () => __Unit
];
export var DeleteEvaluationResults$: StaticOperationSchema = [9, n0, _DER,
  0, () => DeleteEvaluationResultsRequest$, () => DeleteEvaluationResultsResponse$
];
export var DeleteOrganizationConfigRule$: StaticOperationSchema = [9, n0, _DOCR,
  0, () => DeleteOrganizationConfigRuleRequest$, () => __Unit
];
export var DeleteOrganizationConformancePack$: StaticOperationSchema = [9, n0, _DOCP,
  0, () => DeleteOrganizationConformancePackRequest$, () => __Unit
];
export var DeletePendingAggregationRequest$: StaticOperationSchema = [9, n0, _DPAR,
  0, () => DeletePendingAggregationRequestRequest$, () => __Unit
];
export var DeleteRemediationConfiguration$: StaticOperationSchema = [9, n0, _DRC,
  0, () => DeleteRemediationConfigurationRequest$, () => DeleteRemediationConfigurationResponse$
];
export var DeleteRemediationExceptions$: StaticOperationSchema = [9, n0, _DRE,
  0, () => DeleteRemediationExceptionsRequest$, () => DeleteRemediationExceptionsResponse$
];
export var DeleteResourceConfig$: StaticOperationSchema = [9, n0, _DRCe,
  0, () => DeleteResourceConfigRequest$, () => __Unit
];
export var DeleteRetentionConfiguration$: StaticOperationSchema = [9, n0, _DRCel,
  0, () => DeleteRetentionConfigurationRequest$, () => __Unit
];
export var DeleteServiceLinkedConfigurationRecorder$: StaticOperationSchema = [9, n0, _DSLCR,
  0, () => DeleteServiceLinkedConfigurationRecorderRequest$, () => DeleteServiceLinkedConfigurationRecorderResponse$
];
export var DeleteStoredQuery$: StaticOperationSchema = [9, n0, _DSQ,
  0, () => DeleteStoredQueryRequest$, () => DeleteStoredQueryResponse$
];
export var DeliverConfigSnapshot$: StaticOperationSchema = [9, n0, _DCSel,
  0, () => DeliverConfigSnapshotRequest$, () => DeliverConfigSnapshotResponse$
];
export var DescribeAggregateComplianceByConfigRules$: StaticOperationSchema = [9, n0, _DACBCR,
  0, () => DescribeAggregateComplianceByConfigRulesRequest$, () => DescribeAggregateComplianceByConfigRulesResponse$
];
export var DescribeAggregateComplianceByConformancePacks$: StaticOperationSchema = [9, n0, _DACBCP,
  0, () => DescribeAggregateComplianceByConformancePacksRequest$, () => DescribeAggregateComplianceByConformancePacksResponse$
];
export var DescribeAggregationAuthorizations$: StaticOperationSchema = [9, n0, _DAAe,
  0, () => DescribeAggregationAuthorizationsRequest$, () => DescribeAggregationAuthorizationsResponse$
];
export var DescribeComplianceByConfigRule$: StaticOperationSchema = [9, n0, _DCBCR,
  0, () => DescribeComplianceByConfigRuleRequest$, () => DescribeComplianceByConfigRuleResponse$
];
export var DescribeComplianceByResource$: StaticOperationSchema = [9, n0, _DCBR,
  0, () => DescribeComplianceByResourceRequest$, () => DescribeComplianceByResourceResponse$
];
export var DescribeConfigRuleEvaluationStatus$: StaticOperationSchema = [9, n0, _DCRES,
  0, () => DescribeConfigRuleEvaluationStatusRequest$, () => DescribeConfigRuleEvaluationStatusResponse$
];
export var DescribeConfigRules$: StaticOperationSchema = [9, n0, _DCRes,
  0, () => DescribeConfigRulesRequest$, () => DescribeConfigRulesResponse$
];
export var DescribeConfigurationAggregators$: StaticOperationSchema = [9, n0, _DCAe,
  0, () => DescribeConfigurationAggregatorsRequest$, () => DescribeConfigurationAggregatorsResponse$
];
export var DescribeConfigurationAggregatorSourcesStatus$: StaticOperationSchema = [9, n0, _DCASS,
  0, () => DescribeConfigurationAggregatorSourcesStatusRequest$, () => DescribeConfigurationAggregatorSourcesStatusResponse$
];
export var DescribeConfigurationRecorders$: StaticOperationSchema = [9, n0, _DCResc,
  0, () => DescribeConfigurationRecordersRequest$, () => DescribeConfigurationRecordersResponse$
];
export var DescribeConfigurationRecorderStatus$: StaticOperationSchema = [9, n0, _DCRS,
  0, () => DescribeConfigurationRecorderStatusRequest$, () => DescribeConfigurationRecorderStatusResponse$
];
export var DescribeConformancePackCompliance$: StaticOperationSchema = [9, n0, _DCPC,
  0, () => DescribeConformancePackComplianceRequest$, () => DescribeConformancePackComplianceResponse$
];
export var DescribeConformancePacks$: StaticOperationSchema = [9, n0, _DCPe,
  0, () => DescribeConformancePacksRequest$, () => DescribeConformancePacksResponse$
];
export var DescribeConformancePackStatus$: StaticOperationSchema = [9, n0, _DCPS,
  0, () => DescribeConformancePackStatusRequest$, () => DescribeConformancePackStatusResponse$
];
export var DescribeDeliveryChannels$: StaticOperationSchema = [9, n0, _DDCe,
  0, () => DescribeDeliveryChannelsRequest$, () => DescribeDeliveryChannelsResponse$
];
export var DescribeDeliveryChannelStatus$: StaticOperationSchema = [9, n0, _DDCS,
  0, () => DescribeDeliveryChannelStatusRequest$, () => DescribeDeliveryChannelStatusResponse$
];
export var DescribeOrganizationConfigRules$: StaticOperationSchema = [9, n0, _DOCRe,
  0, () => DescribeOrganizationConfigRulesRequest$, () => DescribeOrganizationConfigRulesResponse$
];
export var DescribeOrganizationConfigRuleStatuses$: StaticOperationSchema = [9, n0, _DOCRS,
  0, () => DescribeOrganizationConfigRuleStatusesRequest$, () => DescribeOrganizationConfigRuleStatusesResponse$
];
export var DescribeOrganizationConformancePacks$: StaticOperationSchema = [9, n0, _DOCPe,
  0, () => DescribeOrganizationConformancePacksRequest$, () => DescribeOrganizationConformancePacksResponse$
];
export var DescribeOrganizationConformancePackStatuses$: StaticOperationSchema = [9, n0, _DOCPS,
  0, () => DescribeOrganizationConformancePackStatusesRequest$, () => DescribeOrganizationConformancePackStatusesResponse$
];
export var DescribePendingAggregationRequests$: StaticOperationSchema = [9, n0, _DPARe,
  0, () => DescribePendingAggregationRequestsRequest$, () => DescribePendingAggregationRequestsResponse$
];
export var DescribeRemediationConfigurations$: StaticOperationSchema = [9, n0, _DRCes,
  0, () => DescribeRemediationConfigurationsRequest$, () => DescribeRemediationConfigurationsResponse$
];
export var DescribeRemediationExceptions$: StaticOperationSchema = [9, n0, _DREe,
  0, () => DescribeRemediationExceptionsRequest$, () => DescribeRemediationExceptionsResponse$
];
export var DescribeRemediationExecutionStatus$: StaticOperationSchema = [9, n0, _DRES,
  0, () => DescribeRemediationExecutionStatusRequest$, () => DescribeRemediationExecutionStatusResponse$
];
export var DescribeRetentionConfigurations$: StaticOperationSchema = [9, n0, _DRCesc,
  0, () => DescribeRetentionConfigurationsRequest$, () => DescribeRetentionConfigurationsResponse$
];
export var DisassociateResourceTypes$: StaticOperationSchema = [9, n0, _DRT,
  0, () => DisassociateResourceTypesRequest$, () => DisassociateResourceTypesResponse$
];
export var GetAggregateComplianceDetailsByConfigRule$: StaticOperationSchema = [9, n0, _GACDBCR,
  0, () => GetAggregateComplianceDetailsByConfigRuleRequest$, () => GetAggregateComplianceDetailsByConfigRuleResponse$
];
export var GetAggregateConfigRuleComplianceSummary$: StaticOperationSchema = [9, n0, _GACRCS,
  0, () => GetAggregateConfigRuleComplianceSummaryRequest$, () => GetAggregateConfigRuleComplianceSummaryResponse$
];
export var GetAggregateConformancePackComplianceSummary$: StaticOperationSchema = [9, n0, _GACPCS,
  0, () => GetAggregateConformancePackComplianceSummaryRequest$, () => GetAggregateConformancePackComplianceSummaryResponse$
];
export var GetAggregateDiscoveredResourceCounts$: StaticOperationSchema = [9, n0, _GADRC,
  0, () => GetAggregateDiscoveredResourceCountsRequest$, () => GetAggregateDiscoveredResourceCountsResponse$
];
export var GetAggregateResourceConfig$: StaticOperationSchema = [9, n0, _GARC,
  0, () => GetAggregateResourceConfigRequest$, () => GetAggregateResourceConfigResponse$
];
export var GetComplianceDetailsByConfigRule$: StaticOperationSchema = [9, n0, _GCDBCR,
  0, () => GetComplianceDetailsByConfigRuleRequest$, () => GetComplianceDetailsByConfigRuleResponse$
];
export var GetComplianceDetailsByResource$: StaticOperationSchema = [9, n0, _GCDBR,
  0, () => GetComplianceDetailsByResourceRequest$, () => GetComplianceDetailsByResourceResponse$
];
export var GetComplianceSummaryByConfigRule$: StaticOperationSchema = [9, n0, _GCSBCR,
  0, () => __Unit, () => GetComplianceSummaryByConfigRuleResponse$
];
export var GetComplianceSummaryByResourceType$: StaticOperationSchema = [9, n0, _GCSBRT,
  0, () => GetComplianceSummaryByResourceTypeRequest$, () => GetComplianceSummaryByResourceTypeResponse$
];
export var GetConformancePackComplianceDetails$: StaticOperationSchema = [9, n0, _GCPCD,
  0, () => GetConformancePackComplianceDetailsRequest$, () => GetConformancePackComplianceDetailsResponse$
];
export var GetConformancePackComplianceSummary$: StaticOperationSchema = [9, n0, _GCPCS,
  0, () => GetConformancePackComplianceSummaryRequest$, () => GetConformancePackComplianceSummaryResponse$
];
export var GetCustomRulePolicy$: StaticOperationSchema = [9, n0, _GCRP,
  0, () => GetCustomRulePolicyRequest$, () => GetCustomRulePolicyResponse$
];
export var GetDiscoveredResourceCounts$: StaticOperationSchema = [9, n0, _GDRC,
  0, () => GetDiscoveredResourceCountsRequest$, () => GetDiscoveredResourceCountsResponse$
];
export var GetOrganizationConfigRuleDetailedStatus$: StaticOperationSchema = [9, n0, _GOCRDS,
  0, () => GetOrganizationConfigRuleDetailedStatusRequest$, () => GetOrganizationConfigRuleDetailedStatusResponse$
];
export var GetOrganizationConformancePackDetailedStatus$: StaticOperationSchema = [9, n0, _GOCPDS,
  0, () => GetOrganizationConformancePackDetailedStatusRequest$, () => GetOrganizationConformancePackDetailedStatusResponse$
];
export var GetOrganizationCustomRulePolicy$: StaticOperationSchema = [9, n0, _GOCRP,
  0, () => GetOrganizationCustomRulePolicyRequest$, () => GetOrganizationCustomRulePolicyResponse$
];
export var GetResourceConfigHistory$: StaticOperationSchema = [9, n0, _GRCH,
  0, () => GetResourceConfigHistoryRequest$, () => GetResourceConfigHistoryResponse$
];
export var GetResourceEvaluationSummary$: StaticOperationSchema = [9, n0, _GRES,
  0, () => GetResourceEvaluationSummaryRequest$, () => GetResourceEvaluationSummaryResponse$
];
export var GetStoredQuery$: StaticOperationSchema = [9, n0, _GSQ,
  0, () => GetStoredQueryRequest$, () => GetStoredQueryResponse$
];
export var ListAggregateDiscoveredResources$: StaticOperationSchema = [9, n0, _LADR,
  0, () => ListAggregateDiscoveredResourcesRequest$, () => ListAggregateDiscoveredResourcesResponse$
];
export var ListConfigurationRecorders$: StaticOperationSchema = [9, n0, _LCR,
  0, () => ListConfigurationRecordersRequest$, () => ListConfigurationRecordersResponse$
];
export var ListConformancePackComplianceScores$: StaticOperationSchema = [9, n0, _LCPCS,
  0, () => ListConformancePackComplianceScoresRequest$, () => ListConformancePackComplianceScoresResponse$
];
export var ListDiscoveredResources$: StaticOperationSchema = [9, n0, _LDR,
  0, () => ListDiscoveredResourcesRequest$, () => ListDiscoveredResourcesResponse$
];
export var ListResourceEvaluations$: StaticOperationSchema = [9, n0, _LRE,
  0, () => ListResourceEvaluationsRequest$, () => ListResourceEvaluationsResponse$
];
export var ListStoredQueries$: StaticOperationSchema = [9, n0, _LSQ,
  0, () => ListStoredQueriesRequest$, () => ListStoredQueriesResponse$
];
export var ListTagsForResource$: StaticOperationSchema = [9, n0, _LTFR,
  0, () => ListTagsForResourceRequest$, () => ListTagsForResourceResponse$
];
export var PutAggregationAuthorization$: StaticOperationSchema = [9, n0, _PAA,
  0, () => PutAggregationAuthorizationRequest$, () => PutAggregationAuthorizationResponse$
];
export var PutConfigRule$: StaticOperationSchema = [9, n0, _PCR,
  0, () => PutConfigRuleRequest$, () => __Unit
];
export var PutConfigurationAggregator$: StaticOperationSchema = [9, n0, _PCA,
  0, () => PutConfigurationAggregatorRequest$, () => PutConfigurationAggregatorResponse$
];
export var PutConfigurationRecorder$: StaticOperationSchema = [9, n0, _PCRu,
  0, () => PutConfigurationRecorderRequest$, () => __Unit
];
export var PutConformancePack$: StaticOperationSchema = [9, n0, _PCP,
  0, () => PutConformancePackRequest$, () => PutConformancePackResponse$
];
export var PutDeliveryChannel$: StaticOperationSchema = [9, n0, _PDC,
  0, () => PutDeliveryChannelRequest$, () => __Unit
];
export var PutEvaluations$: StaticOperationSchema = [9, n0, _PE,
  0, () => PutEvaluationsRequest$, () => PutEvaluationsResponse$
];
export var PutExternalEvaluation$: StaticOperationSchema = [9, n0, _PEE,
  0, () => PutExternalEvaluationRequest$, () => PutExternalEvaluationResponse$
];
export var PutOrganizationConfigRule$: StaticOperationSchema = [9, n0, _POCR,
  0, () => PutOrganizationConfigRuleRequest$, () => PutOrganizationConfigRuleResponse$
];
export var PutOrganizationConformancePack$: StaticOperationSchema = [9, n0, _POCP,
  0, () => PutOrganizationConformancePackRequest$, () => PutOrganizationConformancePackResponse$
];
export var PutRemediationConfigurations$: StaticOperationSchema = [9, n0, _PRC,
  0, () => PutRemediationConfigurationsRequest$, () => PutRemediationConfigurationsResponse$
];
export var PutRemediationExceptions$: StaticOperationSchema = [9, n0, _PRE,
  0, () => PutRemediationExceptionsRequest$, () => PutRemediationExceptionsResponse$
];
export var PutResourceConfig$: StaticOperationSchema = [9, n0, _PRCu,
  0, () => PutResourceConfigRequest$, () => __Unit
];
export var PutRetentionConfiguration$: StaticOperationSchema = [9, n0, _PRCut,
  0, () => PutRetentionConfigurationRequest$, () => PutRetentionConfigurationResponse$
];
export var PutServiceLinkedConfigurationRecorder$: StaticOperationSchema = [9, n0, _PSLCR,
  0, () => PutServiceLinkedConfigurationRecorderRequest$, () => PutServiceLinkedConfigurationRecorderResponse$
];
export var PutStoredQuery$: StaticOperationSchema = [9, n0, _PSQ,
  0, () => PutStoredQueryRequest$, () => PutStoredQueryResponse$
];
export var SelectAggregateResourceConfig$: StaticOperationSchema = [9, n0, _SARC,
  0, () => SelectAggregateResourceConfigRequest$, () => SelectAggregateResourceConfigResponse$
];
export var SelectResourceConfig$: StaticOperationSchema = [9, n0, _SRC,
  0, () => SelectResourceConfigRequest$, () => SelectResourceConfigResponse$
];
export var StartConfigRulesEvaluation$: StaticOperationSchema = [9, n0, _SCRE,
  0, () => StartConfigRulesEvaluationRequest$, () => StartConfigRulesEvaluationResponse$
];
export var StartConfigurationRecorder$: StaticOperationSchema = [9, n0, _SCR,
  0, () => StartConfigurationRecorderRequest$, () => __Unit
];
export var StartRemediationExecution$: StaticOperationSchema = [9, n0, _SRE,
  0, () => StartRemediationExecutionRequest$, () => StartRemediationExecutionResponse$
];
export var StartResourceEvaluation$: StaticOperationSchema = [9, n0, _SREt,
  0, () => StartResourceEvaluationRequest$, () => StartResourceEvaluationResponse$
];
export var StopConfigurationRecorder$: StaticOperationSchema = [9, n0, _SCRt,
  0, () => StopConfigurationRecorderRequest$, () => __Unit
];
export var TagResource$: StaticOperationSchema = [9, n0, _TR,
  0, () => TagResourceRequest$, () => __Unit
];
export var UntagResource$: StaticOperationSchema = [9, n0, _UR,
  0, () => UntagResourceRequest$, () => __Unit
];
