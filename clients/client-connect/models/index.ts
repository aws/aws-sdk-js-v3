...
export interface CreateUserRequest {
  ...
  /**
   *
   *          <p>The identifier of the user account in the directory used for identity management.
   *    If Amazon Connect cannot access the directory, you can specify this identifier to authenticate users.
   *    If you include the identifier, we assume that Amazon Connect cannot access the directory. Otherwise,
   *    the identity information is used to authenticate users from your directory.</p>
  ...
  /**
   *
   *          <p>The user name for the account. For instances not using SAML for identity management,
   *    the user name can include up to 20 characters. If you are using SAML for identity management,
   *    the user name can include up to 64 characters from [a-zA-Z0-9_-.\@]+.</p>
   *
   */
...
export interface GetCurrentMetricDataRequest {
  ...
  /**
   *
   *          <p>The metrics to retrieve. Specify the name and unit for each metric.
   ...
   *    The only supported channel is <code>VOICE</code>.</p>
   *
   */
  ...
  /**
   *
   *          <p>The grouping applied to the metrics returned. For example, when grouped by <code>QUEUE</code>, the
   *    metrics returned apply to each queue rather than aggregated for all queues. If you group by
   *    <code>CHANNEL</code>, you should include a Channels filter. The only supported channel is <code>VOICE</code>.</p>
...
export interface GetMetricDataRequest {
   ...
   *    retrieval of historical metrics data. The time must be specified using an interval of 5 minutes, such as 11:00, 11:05, 11:10,
   *    and must be later than the start time timestamp.</p>
  ...
  /**
   *
   *          <p>The metrics to retrieve. Specify the name, unit, and statistic for each metric.
   *     The following historical metrics are available:</p>
   *          <dl>
   *             <dt>ABANDON_TIME</dt>
   *             <dd>
   *                <p>Unit: SECONDS</p>
   ...
   *          <p>The start time cannot be earlier than 24 hours before the time of the request.
...
export interface StartOutboundVoiceContactRequest {
   ...
   *          <p>There can be up to 32,768 UTF-8 bytes across all key-value pairs per contact. Attribute keys can include
   *    only alphanumeric, dash, and underscore characters.</p>
   *
   */
  Attributes?: { [key: string]: string };
  ...
  /**
   *
   *          <p>The queue for the call. If you specify a queue, the phone displayed for caller ID is
   *    the phone number specified in the queue. If you do not specify a queue, the queue defined in the
   *    contact flow is used. If you do not specify a queue, you must specify a source phone number.</p>
   *
   */
  QueueId?: string;
  ...
  /**
   *
   *          <p>The phone number associated with the Amazon Connect instance, in E.164 format. If you do not
   *     specify a source phone number, you must specify a queue.</p>
   *
   */
...
export interface UpdateContactAttributesRequest {
   ...
   *          <p>You can have up to 32,768 UTF-8 bytes across all attributes for a contact. Attribute keys can include
   *    only alphanumeric, dash, and underscore characters.</p>
   *
   */
