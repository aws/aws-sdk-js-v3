...
`LaunchWizard` extends `LaunchWizardClient` and additionally supports all operations, waiters, and paginators as methods.
This style may be familiar to you from the AWS SDK for JavaScript v2.

If you are bundling the AWS SDK, we recommend using only the bare-bones client (`LaunchWizardClient`).
...
const client = new LaunchWizard({ region: "REGION" });
