...
async function smokeTestRunner() {
  ...
  console.log("1..2");
  console.log("# Running tests for WAF.");
  ...
  await (async () => {
    ...
    try {
      ...
      console.log("ok WAF ListRules - no error expected from service");
    } catch (err) {
      console.log("not ok WAF ListRules - no error expected from service");
  ...
  await (async () => {
    ...
    try {
      const result = await client.send(command);
      console.log(
        "not ok WAF CreateSqlInjectionMatchSet - error expected from service"
      );
      testFailed = true;
    } catch (err) {
      console.log(
        "ok WAF CreateSqlInjectionMatchSet - error expected from service"
      );
    }
  })();
