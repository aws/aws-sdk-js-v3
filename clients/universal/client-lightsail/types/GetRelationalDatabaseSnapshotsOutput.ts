import { _UnmarshalledRelationalDatabaseSnapshot } from "./_RelationalDatabaseSnapshot";
import * as __aws_sdk_types from "@aws-sdk/types";

/**
 * GetRelationalDatabaseSnapshotsOutput shape
 */
export interface GetRelationalDatabaseSnapshotsOutput
  extends __aws_sdk_types.MetadataBearer {
  /**
   * <p>An object describing the result of your get relational database snapshots request.</p>
   */
  relationalDatabaseSnapshots?: Array<_UnmarshalledRelationalDatabaseSnapshot>;

  /**
   * <p>A token used for advancing to the next page of results from your get relational database snapshots request.</p>
   */
  nextPageToken?: string;

  /**
   * Metadata about the response received, including the HTTP status code, HTTP headers, and any request identifiers recognized by the SDK.
   */
  $metadata: __aws_sdk_types.ResponseMetadata;
}
