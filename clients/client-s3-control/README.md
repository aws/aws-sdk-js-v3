...
`S3Control` extends `S3ControlClient` and additionally supports all operations, waiters, and paginators as methods.
This style may be familiar to you from the AWS SDK for JavaScript v2.

If you are bundling the AWS SDK, we recommend using only the bare-bones client (`S3ControlClient`).
...
const client = new S3Control({ region: "REGION" });
