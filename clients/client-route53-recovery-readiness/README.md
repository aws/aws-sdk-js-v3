...
`Route53RecoveryReadiness` extends `Route53RecoveryReadinessClient` and additionally supports all operations, waiters, and paginators as methods.
This style may be familiar to you from the AWS SDK for JavaScript v2.

If you are bundling the AWS SDK, we recommend using only the bare-bones client (`Route53RecoveryReadinessClient`).
...
const client = new Route53RecoveryReadiness({ region: "REGION" });
