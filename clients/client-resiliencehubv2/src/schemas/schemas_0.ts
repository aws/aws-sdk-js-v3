const _A = "Achievability";
const _AC = "AssessmentCost";
const _ACM = "AssertionCreatedMetadata";
const _ADE = "AccessDeniedException";
const _ADM = "AssertionDeletedMetadata";
const _AL = "AssertionList";
const _AS = "AssessmentSummary";
const _ASL = "AssessmentSummaryList";
const _ASLs = "AssociatedSystemList";
const _ASs = "AssociatedSystem";
const _ASv = "AvailabilitySlo";
const _AUM = "AssertionUpdatedMetadata";
const _As = "Assertion";
const _CA = "CreateAssertion";
const _CAR = "CreateAssertionRequest";
const _CARL = "CrossAccountRoleList";
const _CARr = "CreateAssertionResponse";
const _CARro = "CrossAccountRole";
const _CE = "ConflictException";
const _CIS = "CreateInputSource";
const _CISR = "CreateInputSourceRequest";
const _CISRr = "CreateInputSourceResponse";
const _CP = "CreatePolicy";
const _CPR = "CreatePolicyRequest";
const _CPRr = "CreatePolicyResponse";
const _CR = "CreateReport";
const _CRR = "CreateReportRequest";
const _CRRr = "CreateReportResponse";
const _CS = "CreateService";
const _CSF = "CreateServiceFunction";
const _CSFR = "CreateServiceFunctionRequest";
const _CSFRR = "CreateServiceFunctionResourcesRequest";
const _CSFRRr = "CreateServiceFunctionResourcesResponse";
const _CSFRr = "CreateServiceFunctionResponse";
const _CSFRre = "CreateServiceFunctionResources";
const _CSR = "CreateServiceRequest";
const _CSRr = "CreateServiceResponse";
const _CSRre = "CreateSystemRequest";
const _CSRrea = "CreateSystemResponse";
const _CSr = "CreateSystem";
const _CUJ = "CreateUserJourney";
const _CUJR = "CreateUserJourneyRequest";
const _CUJRr = "CreateUserJourneyResponse";
const _DA = "DeleteAssertion";
const _DAR = "DeleteAssertionRequest";
const _DARe = "DeleteAssertionResponse";
const _DDC = "DependencyDiscoveryConfig";
const _DIS = "DeleteInputSource";
const _DISR = "DeleteInputSourceRequest";
const _DISRe = "DeleteInputSourceResponse";
const _DP = "DeletePolicy";
const _DPR = "DeletePolicyRequest";
const _DPRe = "DeletePolicyResponse";
const _DRS = "DisasterRecoverySource";
const _DRT = "DataRecoveryTargets";
const _DS = "DependencySummary";
const _DSF = "DeleteServiceFunction";
const _DSFR = "DeleteServiceFunctionRequest";
const _DSFRR = "DeleteServiceFunctionResourcesRequest";
const _DSFRRe = "DeleteServiceFunctionResourcesResponse";
const _DSFRe = "DeleteServiceFunctionResponse";
const _DSFRel = "DeleteServiceFunctionResources";
const _DSL = "DependencySummaryList";
const _DSR = "DeleteServiceRequest";
const _DSRe = "DeleteServiceResponse";
const _DSRel = "DeleteSystemRequest";
const _DSRele = "DeleteSystemResponse";
const _DSe = "DeleteService";
const _DSel = "DeleteSystem";
const _DUJ = "DeleteUserJourney";
const _DUJR = "DeleteUserJourneyRequest";
const _DUJRe = "DeleteUserJourneyResponse";
const _EA = "EventActor";
const _EPL = "EdgePropertyList";
const _EPS = "EdgePropertySummary";
const _EPV = "EffectivePolicyValues";
const _ES = "EksSource";
const _F = "Finding";
const _FL = "FindingsList";
const _FRO = "FailedReportOutput";
const _FS = "FindingSummary";
const _GFMF = "GetFailureModeFinding";
const _GFMFR = "GetFailureModeFindingRequest";
const _GFMFRe = "GetFailureModeFindingResponse";
const _GP = "GetPolicy";
const _GPR = "GetPolicyRequest";
const _GPRe = "GetPolicyResponse";
const _GS = "GetService";
const _GSR = "GetServiceRequest";
const _GSRe = "GetServiceResponse";
const _GSRet = "GetSystemRequest";
const _GSRety = "GetSystemResponse";
const _GSe = "GetSystem";
const _GUJ = "GetUserJourney";
const _GUJR = "GetUserJourneyRequest";
const _GUJRe = "GetUserJourneyResponse";
const _IA = "ImportApp";
const _IACR = "InfrastructureAndCodeRecommendation";
const _IACRL = "InfrastructureAndCodeRecommendationsList";
const _IAR = "ImportAppRequest";
const _IARm = "ImportAppResponse";
const _IP = "ImportPolicy";
const _IPR = "ImportPolicyRequest";
const _IPRm = "ImportPolicyResponse";
const _IS = "InputSource";
const _ISE = "InternalServerException";
const _ISS = "InputSourceSummary";
const _ISSL = "InputSourceSummaryList";
const _LA = "ListAssertions";
const _LAR = "ListAssertionsRequest";
const _LARi = "ListAssertionsResponse";
const _LD = "ListDependencies";
const _LDR = "ListDependenciesRequest";
const _LDRi = "ListDependenciesResponse";
const _LFMA = "ListFailureModeAssessments";
const _LFMAR = "ListFailureModeAssessmentsRequest";
const _LFMARi = "ListFailureModeAssessmentsResponse";
const _LFMF = "ListFailureModeFindings";
const _LFMFR = "ListFailureModeFindingsRequest";
const _LFMFRi = "ListFailureModeFindingsResponse";
const _LIS = "ListInputSources";
const _LISR = "ListInputSourcesRequest";
const _LISRi = "ListInputSourcesResponse";
const _LP = "ListPolicies";
const _LPR = "ListPoliciesRequest";
const _LPRi = "ListPoliciesResponse";
const _LR = "ListReports";
const _LRR = "ListReportsRequest";
const _LRRi = "ListReportsResponse";
const _LRRis = "ListResourcesRequest";
const _LRRist = "ListResourcesResponse";
const _LRi = "ListResources";
const _LS = "ListServices";
const _LSE = "ListServiceEvents";
const _LSER = "ListServiceEventsRequest";
const _LSERi = "ListServiceEventsResponse";
const _LSERis = "ListSystemEventsRequest";
const _LSERist = "ListSystemEventsResponse";
const _LSEi = "ListSystemEvents";
const _LSF = "ListServiceFunctions";
const _LSFR = "ListServiceFunctionsRequest";
const _LSFRi = "ListServiceFunctionsResponse";
const _LSR = "ListServicesRequest";
const _LSRi = "ListServicesResponse";
const _LSRis = "ListSystemsRequest";
const _LSRist = "ListSystemsResponse";
const _LSTE = "ListServiceTopologyEdges";
const _LSTER = "ListServiceTopologyEdgesRequest";
const _LSTERi = "ListServiceTopologyEdgesResponse";
const _LSi = "ListSystems";
const _LTFR = "ListTagsForResource";
const _LTFRR = "ListTagsForResourceRequest";
const _LTFRRi = "ListTagsForResourceResponse";
const _LUJ = "ListUserJourneys";
const _LUJR = "ListUserJourneysRequest";
const _LUJRi = "ListUserJourneysResponse";
const _MAT = "MultiAzTargets";
const _MRT = "MultiRegionTargets";
const _OR = "ObservabilityRecommendation";
const _ORL = "ObservabilityRecommendationsList";
const _P = "Policy";
const _PM = "PermissionModel";
const _PS = "PolicySummary";
const _PSL = "PolicySummaryList";
const _QDP = "QueryDataPoint";
const _QDPL = "QueryDataPointList";
const _QR = "QueryRange";
const _R = "Resource";
const _RC = "ResourceConfiguration";
const _RDS = "ResourceDiscoveryStatus";
const _RGR = "ReportGenerationResult";
const _RGRL = "ReportGenerationResultList";
const _RNFE = "ResourceNotFoundException";
const _RO = "ReportOutput";
const _ROC = "ReportOutputConfiguration";
const _ROCL = "ReportOutputConfigurationList";
const _RT = "ResourceTag";
const _RTL = "ResourceTagList";
const _S = "Service";
const _SAUM = "ServiceAchievabilityUpdatedMetadata";
const _SC = "StringChange";
const _SCM = "ServiceCreatedMetadata";
const _SCMy = "SystemCreatedMetadata";
const _SDM = "ServiceDeletedMetadata";
const _SDMy = "SystemDeletedMetadata";
const _SE = "ServiceEvent";
const _SED = "ServiceEventDetails";
const _SEDy = "SystemEventDetails";
const _SEL = "ServiceEventList";
const _SELy = "SystemEventList";
const _SEM = "ServiceEventMetadata";
const _SEMy = "SystemEventMetadata";
const _SEy = "SystemEvent";
const _SF = "ServiceFunction";
const _SFCM = "ServiceFunctionCreatedMetadata";
const _SFDM = "ServiceFunctionDeletedMetadata";
const _SFL = "ServiceFunctionList";
const _SFMA = "StartFailureModeAssessment";
const _SFMAR = "StartFailureModeAssessmentRequest";
const _SFMARt = "StartFailureModeAssessmentResponse";
const _SFRAM = "ServiceFunctionResourcesAddedMetadata";
const _SFRRM = "ServiceFunctionResourcesRemovedMetadata";
const _SFUM = "ServiceFunctionUpdatedMetadata";
const _SISUM = "ServiceInputSourcesUpdatedMetadata";
const _SPAM = "ServicePolicyAssociatedMetadata";
const _SPAMy = "SystemPolicyAssociatedMetadata";
const _SPDM = "ServicePolicyDisassociatedMetadata";
const _SPDMy = "SystemPolicyDisassociatedMetadata";
const _SQEE = "ServiceQuotaExceededException";
const _SR = "ServiceReference";
const _SRAM = "ServiceResourcesAssociatedMetadata";
const _SRC = "ServiceReferenceChanges";
const _SRCe = "ServiceReportConfiguration";
const _SRDM = "ServiceResourcesDisassociatedMetadata";
const _SRL = "ServiceReferenceList";
const _SRLe = "ServiceResourceList";
const _SRO = "S3ReportOutput";
const _SROC = "S3ReportOutputConfiguration";
const _SRe = "ServiceResource";
const _SS = "ServiceSummary";
const _SSAM = "ServiceSystemAssociatedMetadata";
const _SSAMy = "SystemServiceAssociatedMetadata";
const _SSDM = "ServiceSystemDisassociatedMetadata";
const _SSDMy = "SystemServiceDisassociatedMetadata";
const _SSL = "ServiceSummaryList";
const _SSLy = "SystemSummaryList";
const _SSl = "SloSource";
const _SSy = "SystemSummary";
const _STES = "ServiceTopologyEdgeSummary";
const _STESL = "ServiceTopologyEdgeSummaryList";
const _SUJCM = "SystemUserJourneyCreatedMetadata";
const _SUJDM = "SystemUserJourneyDeletedMetadata";
const _SUJUM = "SystemUserJourneyUpdatedMetadata";
const _SWUM = "ServiceWorkflowUpdatedMetadata";
const _Sy = "System";
const _TE = "ThrottlingException";
const _TKL = "TagKeyList";
const _TM = "TagMap";
const _TR = "TestingRecommendation";
const _TRL = "TestingRecommendationsList";
const _TRR = "TagResourceRequest";
const _TRRa = "TagResourceResponse";
const _TRa = "TagResource";
const _TS = "TargetSource";
const _UA = "UpdateAssertion";
const _UAR = "UpdateAssertionRequest";
const _UARp = "UpdateAssertionResponse";
const _UD = "UpdateDependency";
const _UDR = "UpdateDependencyRequest";
const _UDRp = "UpdateDependencyResponse";
const _UFMF = "UpdateFailureModeFinding";
const _UFMFR = "UpdateFailureModeFindingRequest";
const _UFMFRp = "UpdateFailureModeFindingResponse";
const _UJ = "UserJourney";
const _UJC = "UserJourneyChanges";
const _UJS = "UserJourneySummary";
const _UJSL = "UserJourneySummaryList";
const _UP = "UpdatePolicy";
const _UPR = "UpdatePolicyRequest";
const _UPRp = "UpdatePolicyResponse";
const _UR = "UntagResource";
const _URR = "UntagResourceRequest";
const _URRn = "UntagResourceResponse";
const _US = "UpdateService";
const _USF = "UpdateServiceFunction";
const _USFR = "UpdateServiceFunctionRequest";
const _USFRp = "UpdateServiceFunctionResponse";
const _USR = "UpdateServiceRequest";
const _USRp = "UpdateServiceResponse";
const _USRpd = "UpdateSystemRequest";
const _USRpda = "UpdateSystemResponse";
const _USp = "UpdateSystem";
const _UUJ = "UpdateUserJourney";
const _UUJR = "UpdateUserJourneyRequest";
const _UUJRp = "UpdateUserJourneyResponse";
const _VE = "ValidationException";
const _VEF = "ValidationExceptionField";
const _VEFL = "ValidationExceptionFieldList";
const _a = "amount";
const _aAI = "awsAccountId";
const _aC = "assessmentCost";
const _aCs = "assertionCreated";
const _aD = "assertionDeleted";
const _aI = "assertionId";
const _aIc = "accountId";
const _aIs = "assessmentId";
const _aN = "assertionName";
const _aR = "awsRegion";
const _aS = "availabilitySlo";
const _aSAD = "associatedServicesAtDeletion";
const _aSC = "associatedServiceCount";
const _aSs = "assessmentStatus";
const _aSss = "assessmentStep";
const _aSsse = "assessmentStatuses";
const _aSsses = "assessmentSummaries";
const _aSsso = "associatedSystems";
const _aSssoc = "associatedServices";
const _aU = "assertionUpdated";
const _ac = "achievability";
const _act = "actor";
const _ad = "added";
const _as = "assertion";
const _ass = "assertions";
const _b = "billable";
const _bAUC = "billableAssessmentUnitCount";
const _bO = "bucketOwner";
const _bP = "bucketPath";
const _c = "client";
const _cA = "createdAt";
const _cAR = "crossAccountRoles";
const _cARA = "crossAccountRoleArn";
const _cAl = "clusterArn";
const _cSA = "cfnStackArn";
const _cT = "clientToken";
const _ch = "changes";
const _co = "comment";
const _cr = "criticality";
const _cu = "currency";
const _d = "description";
const _dA = "destinationAccount";
const _dD = "dependencyDiscovery";
const _dFSU = "designFileS3Url";
const _dI = "dependencyId";
const _dN = "dependencyName";
const _dNn = "dnsName";
const _dP = "dataPoints";
const _dR = "dataRecovery";
const _dRA = "disasterRecoveryApproach";
const _dRI = "destinationResourceIdentifier";
const _dRTBB = "dataRecoveryTimeBetweenBackups";
const _dRe = "destinationRegion";
const _dS = "dependencySummaries";
const _e = "error";
const _eA = "endedAt";
const _eAC = "estimatedAssessmentCost";
const _eB = "endedBefore";
const _eC = "errorCode";
const _eD = "eventDetails";
const _eI = "externalId";
const _eIv = "eventId";
const _eM = "errorMessage";
const _eMv = "eventMetadata";
const _ePV = "effectivePolicyValues";
const _eRC = "eligibleResourceCount";
const _eT = "eventTypes";
const _eTn = "endTime";
const _eTv = "eventType";
const _ek = "eks";
const _ev = "events";
const _f = "finding";
const _fC = "failureCategory";
const _fI = "findingId";
const _fL = "fieldList";
const _fRO = "failedReportOutput";
const _fS = "findingsSummary";
const _g = "granularity";
const _h = "http";
const _hE = "httpError";
const _hQ = "httpQuery";
const _i = "identifier";
const _iACR = "infrastructureAndCodeRecommendations";
const _iRN = "invokerRoleName";
const _iS = "inputSource";
const _iSI = "inputSourceId";
const _iSS = "inputSourceSummaries";
const _jD = "journeyDescription";
const _k = "key";
const _kKI = "kmsKeyId";
const _l = "location";
const _lDT = "lastDetectedTime";
const _lRA = "lastRunAt";
const _la = "label";
const _m = "message";
const _mA = "multiAz";
const _mADA = "multiAzDrApproach";
const _mADRA = "multiAzDisasterRecoveryApproach";
const _mAR = "multiAzRto";
const _mARR = "multiAzRtoRpo";
const _mARu = "multiAzRpo";
const _mR = "multiRegion";
const _mRDA = "multiRegionDrApproach";
const _mRDRA = "multiRegionDisasterRecoveryApproach";
const _mRR = "multiRegionRto";
const _mRRR = "multiRegionRtoRpo";
const _mRRu = "multiRegionRpo";
const _mRa = "maxResults";
const _n = "name";
const _nT = "nextToken";
const _nV = "newValue";
const _na = "namespaces";
const _oFC = "openFindingsCount";
const _oI = "ouId";
const _oIr = "organizationId";
const _oR = "observabilityRecommendations";
const _oV = "oldValue";
const _p = "policy";
const _pA = "policyArn";
const _pC = "policyComponent";
const _pI = "principalId";
const _pM = "permissionModel";
const _pN = "policyName";
const _pS = "policySummaries";
const _pr = "provider";
const _pro = "properties";
const _qC = "queryCount";
const _qR = "queryRange";
const _qRET = "queryRangeEndTime";
const _qRG = "queryRangeGranularity";
const _qRST = "queryRangeStartTime";
const _r = "reason";
const _rA = "resourceArn";
const _rAS = "retryAfterSeconds";
const _rAe = "rerunAssessment";
const _rAes = "resourcesAdded";
const _rC = "resourceConfiguration";
const _rCe = "reportConfiguration";
const _rCes = "resourceCount";
const _rD = "resourceDiscovery";
const _rFC = "resolvedFindingsCount";
const _rGR = "reportGenerationResult";
const _rGRe = "reportGenerationResults";
const _rI = "resourceId";
const _rIM = "rtoInMinutes";
const _rIMp = "rpoInMinutes";
const _rIe = "resourceIdentifier";
const _rO = "reportOutput";
const _rOe = "reportOutputs";
const _rR = "resourcesRemoved";
const _rT = "resourceType";
const _rTe = "reportType";
const _rTes = "resourceTags";
const _rTeso = "resourceTypes";
const _re = "resources";
const _rea = "reasoning";
const _reg = "regions";
const _rem = "removed";
const _res = "resource";
const _s = "smithy.ts.sdk.synthetic.com.amazonaws.resiliencehubv2";
const _sA = "serviceArn";
const _sAU = "serviceAchievabilityUpdated";
const _sAo = "sourceAccount";
const _sAt = "startedAt";
const _sAta = "startedAfter";
const _sAy = "systemArn";
const _sB = "sortBy";
const _sC = "suggestedChanges";
const _sCe = "servicesCount";
const _sCer = "serviceCreated";
const _sCy = "systemCreated";
const _sD = "serviceDeleted";
const _sDy = "systemDeleted";
const _sE = "sharingEnabled";
const _sF = "serviceFunction";
const _sFC = "serviceFunctionCreated";
const _sFD = "serviceFunctionDeleted";
const _sFI = "serviceFunctionId";
const _sFN = "serviceFunctionName";
const _sFRA = "serviceFunctionResourcesAdded";
const _sFRR = "serviceFunctionResourcesRemoved";
const _sFU = "serviceFunctionUpdated";
const _sFe = "serviceFunctions";
const _sI = "serviceId";
const _sISU = "serviceInputSourcesUpdated";
const _sIy = "systemId";
const _sMAR = "skipManuallyAddedResources";
const _sN = "systemName";
const _sNe = "serviceName";
const _sO = "sortOrder";
const _sOK = "s3ObjectKey";
const _sPA = "servicePolicyAssociated";
const _sPAy = "systemPolicyAssociated";
const _sPD = "servicePolicyDisassociated";
const _sPDy = "systemPolicyDisassociated";
const _sR = "sourceRegions";
const _sRA = "serviceResourcesAssociated";
const _sRD = "serviceResourcesDisassociated";
const _sRI = "sourceResourceIdentifier";
const _sRO = "s3ReportOutput";
const _sRe = "serviceResources";
const _sRo = "sourceRegion";
const _sS = "serviceSummaries";
const _sSA = "serviceSystemAssociated";
const _sSAy = "systemServiceAssociated";
const _sSD = "serviceSystemDisassociated";
const _sSDy = "systemServiceDisassociated";
const _sSy = "systemSummaries";
const _sT = "startTime";
const _sTES = "serviceTopologyEdgeSummaries";
const _sUJC = "systemUserJourneyCreated";
const _sUJD = "systemUserJourneyDeleted";
const _sUJU = "systemUserJourneyUpdated";
const _sWU = "serviceWorkflowUpdated";
const _s_ = "s3";
const _se = "server";
const _ser = "service";
const _sev = "severity";
const _so = "source";
const _st = "status";
const _sy = "system";
const _t = "text";
const _tBBIM = "timeBetweenBackupsInMinutes";
const _tF = "totalFindings";
const _tK = "tagKeys";
const _tR = "testingRecommendations";
const _tSFU = "tfStateFileUrl";
const _tT = "topologyType";
const _ta = "target";
const _tag = "tags";
const _ti = "timestamp";
const _tit = "title";
const _ty = "type";
const _uA = "updatedAt";
const _uJ = "userJourney";
const _uJA = "userJourneysAffected";
const _uJC = "userJourneysCount";
const _uJI = "userJourneyIds";
const _uJIs = "userJourneyId";
const _uJN = "userJourneyName";
const _uJS = "userJourneySummaries";
const _uJs = "userJourneys";
const _uN = "userName";
const _v = "value";
const _vAA = "v1AppArn";
const _vPA = "v1PolicyArn";
const _va = "values";
const n0 = "com.amazonaws.resiliencehubv2";

// smithy-typescript generated code
import { TypeRegistry } from "@smithy/core/schema";
import type {
  StaticErrorSchema,
  StaticListSchema,
  StaticMapSchema,
  StaticOperationSchema,
  StaticStructureSchema,
  StaticUnionSchema,
} from "@smithy/types";

import {
  AccessDeniedException,
  ConflictException,
  InternalServerException,
  ResourceNotFoundException,
  ServiceQuotaExceededException,
  ThrottlingException,
  ValidationException,
} from "../models/errors";
import { Resiliencehubv2ServiceException } from "../models/Resiliencehubv2ServiceException";

/* eslint no-var: 0 */
const _s_registry = TypeRegistry.for(_s);
export var Resiliencehubv2ServiceException$: StaticErrorSchema = [-3, _s, "Resiliencehubv2ServiceException", 0, [], []];
_s_registry.registerError(Resiliencehubv2ServiceException$, Resiliencehubv2ServiceException);
const n0_registry = TypeRegistry.for(n0);
export var AccessDeniedException$: StaticErrorSchema = [-3, n0, _ADE,
  { [_e]: _c, [_hE]: 403 },
  [_m],
  [0], 1
];
n0_registry.registerError(AccessDeniedException$, AccessDeniedException);
export var ConflictException$: StaticErrorSchema = [-3, n0, _CE,
  { [_e]: _c, [_hE]: 409 },
  [_m],
  [0], 1
];
n0_registry.registerError(ConflictException$, ConflictException);
export var InternalServerException$: StaticErrorSchema = [-3, n0, _ISE,
  { [_e]: _se, [_hE]: 500 },
  [_m],
  [0], 1
];
n0_registry.registerError(InternalServerException$, InternalServerException);
export var ResourceNotFoundException$: StaticErrorSchema = [-3, n0, _RNFE,
  { [_e]: _c, [_hE]: 404 },
  [_m, _rI, _rT],
  [0, 0, 0], 1
];
n0_registry.registerError(ResourceNotFoundException$, ResourceNotFoundException);
export var ServiceQuotaExceededException$: StaticErrorSchema = [-3, n0, _SQEE,
  { [_e]: _c, [_hE]: 402 },
  [_m],
  [0], 1
];
n0_registry.registerError(ServiceQuotaExceededException$, ServiceQuotaExceededException);
export var ThrottlingException$: StaticErrorSchema = [-3, n0, _TE,
  { [_e]: _c, [_hE]: 429 },
  [_m, _rAS],
  [0, 1], 1
];
n0_registry.registerError(ThrottlingException$, ThrottlingException);
export var ValidationException$: StaticErrorSchema = [-3, n0, _VE,
  { [_e]: _c, [_hE]: 400 },
  [_m, _r, _fL],
  [0, 0, () => ValidationExceptionFieldList], 1
];
n0_registry.registerError(ValidationException$, ValidationException);
/**
 * TypeRegistry instances containing modeled errors.
 * @internal
 *
 */
export const errorTypeRegistries = [
  _s_registry,
  n0_registry,
]
export var Achievability$: StaticStructureSchema = [3, n0, _A,
  0,
  [_aS, _mARR, _mRRR, _dRTBB],
  [0, 0, 0, 0]
];
export var Assertion$: StaticStructureSchema = [3, n0, _As,
  0,
  [_sA, _aI, _t, _so, _cA, _uA],
  [0, 0, 0, 0, 4, 4], 4
];
export var AssertionCreatedMetadata$: StaticStructureSchema = [3, n0, _ACM,
  0,
  [_aI, _aN],
  [0, 0]
];
export var AssertionDeletedMetadata$: StaticStructureSchema = [3, n0, _ADM,
  0,
  [_aI, _aN],
  [0, 0]
];
export var AssertionUpdatedMetadata$: StaticStructureSchema = [3, n0, _AUM,
  0,
  [_aI, _aN],
  [0, 0]
];
export var AssessmentCost$: StaticStructureSchema = [3, n0, _AC,
  0,
  [_a, _cu],
  [1, 0]
];
export var AssessmentSummary$: StaticStructureSchema = [3, n0, _AS,
  0,
  [_aIs, _sA, _aSs, _aSss, _tF, _sAt, _eA, _eM, _eC, _aC, _bAUC, _ac],
  [0, 0, 0, 0, 1, 4, 4, 0, 0, () => AssessmentCost$, 1, () => Achievability$], 2
];
export var AssociatedSystem$: StaticStructureSchema = [3, n0, _ASs,
  0,
  [_sAy, _sN, _uJI],
  [0, 0, 64 | 0], 1
];
export var AvailabilitySlo$: StaticStructureSchema = [3, n0, _ASv,
  0,
  [_ta],
  [1]
];
export var CreateAssertionRequest$: StaticStructureSchema = [3, n0, _CAR,
  0,
  [_sA, _t, _cT],
  [0, 0, [0, 4]], 2
];
export var CreateAssertionResponse$: StaticStructureSchema = [3, n0, _CARr,
  0,
  [_as],
  [() => Assertion$], 1
];
export var CreateInputSourceRequest$: StaticStructureSchema = [3, n0, _CISR,
  0,
  [_sA, _rC, _cT],
  [0, () => ResourceConfiguration$, [0, 4]], 2
];
export var CreateInputSourceResponse$: StaticStructureSchema = [3, n0, _CISRr,
  0,
  [_sA, _iSI],
  [0, 0], 2
];
export var CreatePolicyRequest$: StaticStructureSchema = [3, n0, _CPR,
  0,
  [_n, _d, _aS, _mA, _mR, _dR, _kKI, _tag, _cT],
  [0, 0, () => AvailabilitySlo$, () => MultiAzTargets$, () => MultiRegionTargets$, () => DataRecoveryTargets$, 0, [() => TagMap, 0], [0, 4]], 1
];
export var CreatePolicyResponse$: StaticStructureSchema = [3, n0, _CPRr,
  0,
  [_p],
  [[() => Policy$, 0]], 1
];
export var CreateReportRequest$: StaticStructureSchema = [3, n0, _CRR,
  0,
  [_sA, _rTe, _cT],
  [0, 0, [0, 4]], 2
];
export var CreateReportResponse$: StaticStructureSchema = [3, n0, _CRRr,
  0,
  [_rGR],
  [() => ReportGenerationResult$], 1
];
export var CreateServiceFunctionRequest$: StaticStructureSchema = [3, n0, _CSFR,
  0,
  [_n, _sA, _cr, _d, _cT],
  [0, 0, 0, 0, [0, 4]], 3
];
export var CreateServiceFunctionResourcesRequest$: StaticStructureSchema = [3, n0, _CSFRR,
  0,
  [_sA, _sFI, _re],
  [0, 0, 64 | 0], 3
];
export var CreateServiceFunctionResourcesResponse$: StaticStructureSchema = [3, n0, _CSFRRr,
  0,
  [_sA, _sFI, _re],
  [0, 0, 64 | 0]
];
export var CreateServiceFunctionResponse$: StaticStructureSchema = [3, n0, _CSFRr,
  0,
  [_sF],
  [() => ServiceFunction$], 1
];
export var CreateServiceRequest$: StaticStructureSchema = [3, n0, _CSR,
  0,
  [_n, _reg, _pM, _d, _aSsso, _pA, _dD, _rCe, _kKI, _tag, _cT],
  [0, 64 | 0, () => PermissionModel$, 0, () => AssociatedSystemList, 0, 0, () => ServiceReportConfiguration$, 0, [() => TagMap, 0], [0, 4]], 3
];
export var CreateServiceResponse$: StaticStructureSchema = [3, n0, _CSRr,
  0,
  [_ser],
  [[() => Service$, 0]], 1
];
export var CreateSystemRequest$: StaticStructureSchema = [3, n0, _CSRre,
  0,
  [_n, _d, _sE, _kKI, _tag, _cT],
  [0, 0, 2, 0, [() => TagMap, 0], [0, 4]], 1
];
export var CreateSystemResponse$: StaticStructureSchema = [3, n0, _CSRrea,
  0,
  [_sy],
  [[() => System$, 0]], 1
];
export var CreateUserJourneyRequest$: StaticStructureSchema = [3, n0, _CUJR,
  0,
  [_sAy, _n, _d, _pA, _cT],
  [0, 0, 0, 0, [0, 4]], 2
];
export var CreateUserJourneyResponse$: StaticStructureSchema = [3, n0, _CUJRr,
  0,
  [_uJ],
  [() => UserJourney$], 1
];
export var CrossAccountRole$: StaticStructureSchema = [3, n0, _CARro,
  0,
  [_cARA, _eI],
  [0, 0], 1
];
export var DataRecoveryTargets$: StaticStructureSchema = [3, n0, _DRT,
  0,
  [_tBBIM],
  [1]
];
export var DeleteAssertionRequest$: StaticStructureSchema = [3, n0, _DAR,
  0,
  [_sA, _aI],
  [0, 0], 2
];
export var DeleteAssertionResponse$: StaticStructureSchema = [3, n0, _DARe,
  0,
  [_aI],
  [0]
];
export var DeleteInputSourceRequest$: StaticStructureSchema = [3, n0, _DISR,
  0,
  [_sA, _iSI],
  [0, 0], 2
];
export var DeleteInputSourceResponse$: StaticStructureSchema = [3, n0, _DISRe,
  0,
  [_sA, _iSI],
  [0, 0], 2
];
export var DeletePolicyRequest$: StaticStructureSchema = [3, n0, _DPR,
  0,
  [_pA],
  [0], 1
];
export var DeletePolicyResponse$: StaticStructureSchema = [3, n0, _DPRe,
  0,
  [_pA],
  [0], 1
];
export var DeleteServiceFunctionRequest$: StaticStructureSchema = [3, n0, _DSFR,
  0,
  [_sA, _sFI],
  [0, 0], 2
];
export var DeleteServiceFunctionResourcesRequest$: StaticStructureSchema = [3, n0, _DSFRR,
  0,
  [_sA, _sFI, _re],
  [0, 0, 64 | 0], 3
];
export var DeleteServiceFunctionResourcesResponse$: StaticStructureSchema = [3, n0, _DSFRRe,
  0,
  [_sA, _sFI, _re],
  [0, 0, 64 | 0]
];
export var DeleteServiceFunctionResponse$: StaticStructureSchema = [3, n0, _DSFRe,
  0,
  [_sFI],
  [0]
];
export var DeleteServiceRequest$: StaticStructureSchema = [3, n0, _DSR,
  0,
  [_sA],
  [0], 1
];
export var DeleteServiceResponse$: StaticStructureSchema = [3, n0, _DSRe,
  0,
  [_sA],
  [0], 1
];
export var DeleteSystemRequest$: StaticStructureSchema = [3, n0, _DSRel,
  0,
  [_sAy],
  [0], 1
];
export var DeleteSystemResponse$: StaticStructureSchema = [3, n0, _DSRele,
  0,
  [_sAy],
  [0], 1
];
export var DeleteUserJourneyRequest$: StaticStructureSchema = [3, n0, _DUJR,
  0,
  [_sAy, _uJIs],
  [0, 0], 2
];
export var DeleteUserJourneyResponse$: StaticStructureSchema = [3, n0, _DUJRe,
  0,
  [_uJIs],
  [0], 1
];
export var DependencyDiscoveryConfig$: StaticStructureSchema = [3, n0, _DDC,
  0,
  [_st, _uA, _eRC, _m],
  [0, 4, 1, 0], 1
];
export var DependencySummary$: StaticStructureSchema = [3, n0, _DS,
  0,
  [_dI, _sA, _dN, _dNn, _l, _lDT, _sR, _qR, _cr, _pr, _co],
  [0, 0, 0, 0, 0, 4, 64 | 0, () => QueryRange$, 0, 0, 0], 9
];
export var DisasterRecoverySource$: StaticStructureSchema = [3, n0, _DRS,
  0,
  [_v, _pN, _so],
  [0, 0, 0]
];
export var EdgePropertySummary$: StaticStructureSchema = [3, n0, _EPS,
  0,
  [_tT, _la],
  [0, 0]
];
export var EffectivePolicyValues$: StaticStructureSchema = [3, n0, _EPV,
  0,
  [_aS, _mAR, _mARu, _mADA, _mRR, _mRRu, _mRDA, _dRTBB],
  [() => SloSource$, () => TargetSource$, () => TargetSource$, () => DisasterRecoverySource$, () => TargetSource$, () => TargetSource$, () => DisasterRecoverySource$, () => TargetSource$]
];
export var EksSource$: StaticStructureSchema = [3, n0, _ES,
  0,
  [_cAl, _na],
  [0, 64 | 0], 2
];
export var EventActor$: StaticStructureSchema = [3, n0, _EA,
  0,
  [_ty, _pI, _aIc, _uN],
  [0, 0, 0, 0], 2
];
export var FailedReportOutput$: StaticStructureSchema = [3, n0, _FRO,
  0,
  [_eC, _eM],
  [0, 0], 1
];
export var Finding$: StaticStructureSchema = [3, n0, _F,
  0,
  [_fI, _n, _d, _fC, _st, _rea, _co, _sev, _sFe, _pC, _iACR, _oR, _tR, _uA],
  [0, 0, 0, 0, 0, 0, 0, 0, 64 | 0, 0, () => InfrastructureAndCodeRecommendationsList, () => ObservabilityRecommendationsList, () => TestingRecommendationsList, 4]
];
export var FindingSummary$: StaticStructureSchema = [3, n0, _FS,
  0,
  [_sA, _fI, _n, _d, _fC, _sev, _st, _pC, _uA],
  [0, 0, 0, 0, 0, 0, 0, 0, 4]
];
export var GetFailureModeFindingRequest$: StaticStructureSchema = [3, n0, _GFMFR,
  0,
  [_fI, _sA],
  [[0, { [_hQ]: _fI }], [0, { [_hQ]: _sA }]], 2
];
export var GetFailureModeFindingResponse$: StaticStructureSchema = [3, n0, _GFMFRe,
  0,
  [_f],
  [() => Finding$]
];
export var GetPolicyRequest$: StaticStructureSchema = [3, n0, _GPR,
  0,
  [_pA],
  [[0, { [_hQ]: _pA }]], 1
];
export var GetPolicyResponse$: StaticStructureSchema = [3, n0, _GPRe,
  0,
  [_p],
  [[() => Policy$, 0]], 1
];
export var GetServiceRequest$: StaticStructureSchema = [3, n0, _GSR,
  0,
  [_sA],
  [[0, { [_hQ]: _sA }]], 1
];
export var GetServiceResponse$: StaticStructureSchema = [3, n0, _GSRe,
  0,
  [_ser],
  [[() => Service$, 0]], 1
];
export var GetSystemRequest$: StaticStructureSchema = [3, n0, _GSRet,
  0,
  [_sAy],
  [[0, { [_hQ]: _sAy }]], 1
];
export var GetSystemResponse$: StaticStructureSchema = [3, n0, _GSRety,
  0,
  [_sy],
  [[() => System$, 0]], 1
];
export var GetUserJourneyRequest$: StaticStructureSchema = [3, n0, _GUJR,
  0,
  [_sAy, _uJIs],
  [[0, { [_hQ]: _sAy }], [0, { [_hQ]: _uJIs }]], 2
];
export var GetUserJourneyResponse$: StaticStructureSchema = [3, n0, _GUJRe,
  0,
  [_uJ],
  [() => UserJourney$], 1
];
export var ImportAppRequest$: StaticStructureSchema = [3, n0, _IAR,
  0,
  [_vAA, _pA, _kKI, _sMAR, _aSsso, _tag, _cT],
  [0, 0, 0, 2, () => AssociatedSystemList, [() => TagMap, 0], [0, 4]], 1
];
export var ImportAppResponse$: StaticStructureSchema = [3, n0, _IARm,
  0,
  [_ser],
  [[() => Service$, 0]], 1
];
export var ImportPolicyRequest$: StaticStructureSchema = [3, n0, _IPR,
  0,
  [_vPA, _kKI, _aS, _mADRA, _mRDRA, _tag, _cT],
  [0, 0, () => AvailabilitySlo$, 0, 0, [() => TagMap, 0], [0, 4]], 1
];
export var ImportPolicyResponse$: StaticStructureSchema = [3, n0, _IPRm,
  0,
  [_p],
  [[() => Policy$, 0]], 1
];
export var InfrastructureAndCodeRecommendation$: StaticStructureSchema = [3, n0, _IACR,
  0,
  [_sC],
  [64 | 0]
];
export var InputSource$: StaticStructureSchema = [3, n0, _IS,
  0,
  [_i, _ty],
  [0, 0], 2
];
export var InputSourceSummary$: StaticStructureSchema = [3, n0, _ISS,
  0,
  [_iSI, _ty, _rTes, _cSA, _tSFU, _ek, _dFSU, _cA],
  [0, 0, () => ResourceTagList, 0, 0, () => EksSource$, 0, 4], 1
];
export var ListAssertionsRequest$: StaticStructureSchema = [3, n0, _LAR,
  0,
  [_sA, _so, _mRa, _nT],
  [[0, { [_hQ]: _sA }], [0, { [_hQ]: _so }], [1, { [_hQ]: _mRa }], [0, { [_hQ]: _nT }]], 1
];
export var ListAssertionsResponse$: StaticStructureSchema = [3, n0, _LARi,
  0,
  [_ass, _nT],
  [() => AssertionList, 0], 1
];
export var ListDependenciesRequest$: StaticStructureSchema = [3, n0, _LDR,
  0,
  [_sA, _qRST, _qRET, _qRG, _mRa, _nT],
  [[0, { [_hQ]: _sA }], [4, { [_hQ]: _qRST }], [4, { [_hQ]: _qRET }], [0, { [_hQ]: _qRG }], [1, { [_hQ]: _mRa }], [0, { [_hQ]: _nT }]]
];
export var ListDependenciesResponse$: StaticStructureSchema = [3, n0, _LDRi,
  0,
  [_dS, _nT],
  [() => DependencySummaryList, 0], 1
];
export var ListFailureModeAssessmentsRequest$: StaticStructureSchema = [3, n0, _LFMAR,
  0,
  [_sA, _aSsse, _sAta, _eB, _sB, _sO, _mRa, _nT],
  [[0, { [_hQ]: _sA }], [64 | 0, { [_hQ]: _aSsse }], [4, { [_hQ]: _sAta }], [4, { [_hQ]: _eB }], [0, { [_hQ]: _sB }], [0, { [_hQ]: _sO }], [1, { [_hQ]: _mRa }], [0, { [_hQ]: _nT }]], 1
];
export var ListFailureModeAssessmentsResponse$: StaticStructureSchema = [3, n0, _LFMARi,
  0,
  [_aSsses, _nT],
  [() => AssessmentSummaryList, 0], 1
];
export var ListFailureModeFindingsRequest$: StaticStructureSchema = [3, n0, _LFMFR,
  0,
  [_sA, _sev, _fC, _st, _mRa, _nT],
  [[0, { [_hQ]: _sA }], [0, { [_hQ]: _sev }], [0, { [_hQ]: _fC }], [0, { [_hQ]: _st }], [1, { [_hQ]: _mRa }], [0, { [_hQ]: _nT }]], 1
];
export var ListFailureModeFindingsResponse$: StaticStructureSchema = [3, n0, _LFMFRi,
  0,
  [_fS, _nT],
  [() => FindingsList, 0], 1
];
export var ListInputSourcesRequest$: StaticStructureSchema = [3, n0, _LISR,
  0,
  [_sA, _ty, _mRa, _nT],
  [[0, { [_hQ]: _sA }], [0, { [_hQ]: _ty }], [1, { [_hQ]: _mRa }], [0, { [_hQ]: _nT }]], 1
];
export var ListInputSourcesResponse$: StaticStructureSchema = [3, n0, _LISRi,
  0,
  [_iSS, _nT],
  [() => InputSourceSummaryList, 0], 1
];
export var ListPoliciesRequest$: StaticStructureSchema = [3, n0, _LPR,
  0,
  [_mRa, _nT],
  [[1, { [_hQ]: _mRa }], [0, { [_hQ]: _nT }]]
];
export var ListPoliciesResponse$: StaticStructureSchema = [3, n0, _LPRi,
  0,
  [_pS, _nT],
  [() => PolicySummaryList, 0], 1
];
export var ListReportsRequest$: StaticStructureSchema = [3, n0, _LRR,
  0,
  [_sA, _rTe, _mRa, _nT],
  [[0, { [_hQ]: _sA }], [0, { [_hQ]: _rTe }], [1, { [_hQ]: _mRa }], [0, { [_hQ]: _nT }]]
];
export var ListReportsResponse$: StaticStructureSchema = [3, n0, _LRRi,
  0,
  [_rGRe, _nT],
  [() => ReportGenerationResultList, 0], 1
];
export var ListResourcesRequest$: StaticStructureSchema = [3, n0, _LRRis,
  0,
  [_sA, _sFI, _aR, _rTeso, _b, _mRa, _nT],
  [[0, { [_hQ]: _sA }], [0, { [_hQ]: _sFI }], [0, { [_hQ]: _aR }], [64 | 0, { [_hQ]: _rTeso }], [2, { [_hQ]: _b }], [1, { [_hQ]: _mRa }], [0, { [_hQ]: _nT }]], 1
];
export var ListResourcesResponse$: StaticStructureSchema = [3, n0, _LRRist,
  0,
  [_sFI, _sRe, _nT],
  [0, () => ServiceResourceList, 0]
];
export var ListServiceEventsRequest$: StaticStructureSchema = [3, n0, _LSER,
  0,
  [_sA, _eT, _sT, _eTn, _mRa, _nT],
  [[0, { [_hQ]: _sA }], [64 | 0, { [_hQ]: _eT }], [4, { [_hQ]: _sT }], [4, { [_hQ]: _eTn }], [1, { [_hQ]: _mRa }], [0, { [_hQ]: _nT }]], 1
];
export var ListServiceEventsResponse$: StaticStructureSchema = [3, n0, _LSERi,
  0,
  [_ev, _nT],
  [() => ServiceEventList, 0], 1
];
export var ListServiceFunctionsRequest$: StaticStructureSchema = [3, n0, _LSFR,
  0,
  [_sA, _mRa, _nT],
  [[0, { [_hQ]: _sA }], [1, { [_hQ]: _mRa }], [0, { [_hQ]: _nT }]], 1
];
export var ListServiceFunctionsResponse$: StaticStructureSchema = [3, n0, _LSFRi,
  0,
  [_sFe, _nT],
  [() => ServiceFunctionList, 0], 1
];
export var ListServicesRequest$: StaticStructureSchema = [3, n0, _LSR,
  0,
  [_sAy, _uJIs, _oI, _aIc, _aSs, _pA, _mRa, _nT],
  [[0, { [_hQ]: _sAy }], [0, { [_hQ]: _uJIs }], [0, { [_hQ]: _oI }], [0, { [_hQ]: _aIc }], [0, { [_hQ]: _aSs }], [0, { [_hQ]: _pA }], [1, { [_hQ]: _mRa }], [0, { [_hQ]: _nT }]]
];
export var ListServicesResponse$: StaticStructureSchema = [3, n0, _LSRi,
  0,
  [_sS, _nT],
  [() => ServiceSummaryList, 0], 1
];
export var ListServiceTopologyEdgesRequest$: StaticStructureSchema = [3, n0, _LSTER,
  0,
  [_sA, _mRa, _nT],
  [[0, { [_hQ]: _sA }], [1, { [_hQ]: _mRa }], [0, { [_hQ]: _nT }]], 1
];
export var ListServiceTopologyEdgesResponse$: StaticStructureSchema = [3, n0, _LSTERi,
  0,
  [_sTES, _nT],
  [() => ServiceTopologyEdgeSummaryList, 0]
];
export var ListSystemEventsRequest$: StaticStructureSchema = [3, n0, _LSERis,
  0,
  [_sAy, _eT, _sT, _eTn, _mRa, _nT],
  [[0, { [_hQ]: _sAy }], [64 | 0, { [_hQ]: _eT }], [4, { [_hQ]: _sT }], [4, { [_hQ]: _eTn }], [1, { [_hQ]: _mRa }], [0, { [_hQ]: _nT }]], 1
];
export var ListSystemEventsResponse$: StaticStructureSchema = [3, n0, _LSERist,
  0,
  [_ev, _nT],
  [() => SystemEventList, 0], 1
];
export var ListSystemsRequest$: StaticStructureSchema = [3, n0, _LSRis,
  0,
  [_oI, _mRa, _nT],
  [[0, { [_hQ]: _oI }], [1, { [_hQ]: _mRa }], [0, { [_hQ]: _nT }]]
];
export var ListSystemsResponse$: StaticStructureSchema = [3, n0, _LSRist,
  0,
  [_sSy, _nT],
  [() => SystemSummaryList, 0], 1
];
export var ListTagsForResourceRequest$: StaticStructureSchema = [3, n0, _LTFRR,
  0,
  [_rA],
  [[0, 1]], 1
];
export var ListTagsForResourceResponse$: StaticStructureSchema = [3, n0, _LTFRRi,
  0,
  [_tag],
  [[() => TagMap, 0]]
];
export var ListUserJourneysRequest$: StaticStructureSchema = [3, n0, _LUJR,
  0,
  [_sAy, _mRa, _nT],
  [[0, { [_hQ]: _sAy }], [1, { [_hQ]: _mRa }], [0, { [_hQ]: _nT }]], 1
];
export var ListUserJourneysResponse$: StaticStructureSchema = [3, n0, _LUJRi,
  0,
  [_uJS, _nT],
  [() => UserJourneySummaryList, 0], 1
];
export var MultiAzTargets$: StaticStructureSchema = [3, n0, _MAT,
  0,
  [_rIM, _rIMp, _dRA],
  [1, 1, 0]
];
export var MultiRegionTargets$: StaticStructureSchema = [3, n0, _MRT,
  0,
  [_rIM, _rIMp, _dRA],
  [1, 1, 0]
];
export var ObservabilityRecommendation$: StaticStructureSchema = [3, n0, _OR,
  0,
  [_sC],
  [64 | 0]
];
export var PermissionModel$: StaticStructureSchema = [3, n0, _PM,
  0,
  [_iRN, _cAR],
  [0, () => CrossAccountRoleList], 1
];
export var Policy$: StaticStructureSchema = [3, n0, _P,
  0,
  [_pA, _n, _d, _aS, _mA, _mR, _dR, _kKI, _tag, _aSC, _cA, _uA],
  [0, 0, 0, () => AvailabilitySlo$, () => MultiAzTargets$, () => MultiRegionTargets$, () => DataRecoveryTargets$, 0, [() => TagMap, 0], 1, 4, 4], 2
];
export var PolicySummary$: StaticStructureSchema = [3, n0, _PS,
  0,
  [_pA, _n, _aS, _mA, _mR, _dR, _aSC, _cA, _uA],
  [0, 0, () => AvailabilitySlo$, () => MultiAzTargets$, () => MultiRegionTargets$, () => DataRecoveryTargets$, 1, 4, 4], 2
];
export var QueryDataPoint$: StaticStructureSchema = [3, n0, _QDP,
  0,
  [_ti, _qC],
  [4, 1], 2
];
export var QueryRange$: StaticStructureSchema = [3, n0, _QR,
  0,
  [_sT, _eTn, _g, _dP],
  [4, 4, 0, () => QueryDataPointList], 4
];
export var ReportGenerationResult$: StaticStructureSchema = [3, n0, _RGR,
  0,
  [_rTe, _st, _sA, _aIs, _cA, _rO],
  [0, 0, 0, 0, 4, () => ReportOutput$], 2
];
export var Resource$: StaticStructureSchema = [3, n0, _R,
  0,
  [_i, _aR, _aAI, _rT],
  [0, 0, 0, 0], 1
];
export var ResourceDiscoveryStatus$: StaticStructureSchema = [3, n0, _RDS,
  0,
  [_st, _lRA, _eC, _eM],
  [0, 4, 0, 0]
];
export var ResourceTag$: StaticStructureSchema = [3, n0, _RT,
  0,
  [_k, _va],
  [0, 64 | 0], 2
];
export var S3ReportOutput$: StaticStructureSchema = [3, n0, _SRO,
  0,
  [_sOK],
  [0], 1
];
export var S3ReportOutputConfiguration$: StaticStructureSchema = [3, n0, _SROC,
  0,
  [_bP, _bO],
  [0, 0], 2
];
export var Service$: StaticStructureSchema = [3, n0, _S,
  0,
  [_sA, _n, _d, _aSsso, _pA, _reg, _pM, _dD, _ePV, _ac, _rCe, _kKI, _tag, _eAC, _rD, _aSs, _rAe, _oFC, _rFC, _oIr, _oI, _aIc, _cA, _uA],
  [0, 0, 0, () => AssociatedSystemList, 0, 64 | 0, () => PermissionModel$, () => DependencyDiscoveryConfig$, () => EffectivePolicyValues$, () => Achievability$, () => ServiceReportConfiguration$, 0, [() => TagMap, 0], () => AssessmentCost$, () => ResourceDiscoveryStatus$, 0, 2, 1, 1, 0, 0, 0, 4, 4], 2
];
export var ServiceAchievabilityUpdatedMetadata$: StaticStructureSchema = [3, n0, _SAUM,
  0,
  [_aIs, _aS, _mARR, _mRRR],
  [0, 0, 0, 0]
];
export var ServiceCreatedMetadata$: StaticStructureSchema = [3, n0, _SCM,
  0,
  [],
  []
];
export var ServiceDeletedMetadata$: StaticStructureSchema = [3, n0, _SDM,
  0,
  [],
  []
];
export var ServiceEvent$: StaticStructureSchema = [3, n0, _SE,
  0,
  [_eIv, _ti, _eTv, _sA, _act, _eD],
  [0, 4, 0, 0, () => EventActor$, () => ServiceEventDetails$], 6
];
export var ServiceEventDetails$: StaticStructureSchema = [3, n0, _SED,
  0,
  [_tit, _d, _eMv],
  [0, 0, () => ServiceEventMetadata$], 2
];
export var ServiceFunction$: StaticStructureSchema = [3, n0, _SF,
  0,
  [_sA, _sFI, _n, _cr, _d, _rCes, _so, _cA, _uA],
  [0, 0, 0, 0, 0, 1, 0, 4, 4], 4
];
export var ServiceFunctionCreatedMetadata$: StaticStructureSchema = [3, n0, _SFCM,
  0,
  [_sFI, _sFN],
  [0, 0]
];
export var ServiceFunctionDeletedMetadata$: StaticStructureSchema = [3, n0, _SFDM,
  0,
  [_sFI, _sFN],
  [0, 0]
];
export var ServiceFunctionResourcesAddedMetadata$: StaticStructureSchema = [3, n0, _SFRAM,
  0,
  [_sFI, _sFN, _rAes],
  [0, 0, 64 | 0]
];
export var ServiceFunctionResourcesRemovedMetadata$: StaticStructureSchema = [3, n0, _SFRRM,
  0,
  [_sFI, _sFN, _rR],
  [0, 0, 64 | 0]
];
export var ServiceFunctionUpdatedMetadata$: StaticStructureSchema = [3, n0, _SFUM,
  0,
  [_sFI, _sFN, _rAes, _rR],
  [0, 0, 64 | 0, 64 | 0]
];
export var ServiceInputSourcesUpdatedMetadata$: StaticStructureSchema = [3, n0, _SISUM,
  0,
  [],
  []
];
export var ServicePolicyAssociatedMetadata$: StaticStructureSchema = [3, n0, _SPAM,
  0,
  [_pN, _pA],
  [0, 0]
];
export var ServicePolicyDisassociatedMetadata$: StaticStructureSchema = [3, n0, _SPDM,
  0,
  [_pN, _pA],
  [0, 0]
];
export var ServiceReference$: StaticStructureSchema = [3, n0, _SR,
  0,
  [_sI, _sNe],
  [0, 0]
];
export var ServiceReferenceChanges$: StaticStructureSchema = [3, n0, _SRC,
  0,
  [_ad, _rem],
  [() => ServiceReferenceList, () => ServiceReferenceList]
];
export var ServiceReportConfiguration$: StaticStructureSchema = [3, n0, _SRCe,
  0,
  [_rOe],
  [() => ReportOutputConfigurationList], 1
];
export var ServiceResource$: StaticStructureSchema = [3, n0, _SRe,
  0,
  [_rIe, _res, _iS],
  [0, () => Resource$, () => InputSource$], 2
];
export var ServiceResourcesAssociatedMetadata$: StaticStructureSchema = [3, n0, _SRAM,
  0,
  [_rCes, _rTeso],
  [1, 64 | 0]
];
export var ServiceResourcesDisassociatedMetadata$: StaticStructureSchema = [3, n0, _SRDM,
  0,
  [_rCes, _rTeso],
  [1, 64 | 0]
];
export var ServiceSummary$: StaticStructureSchema = [3, n0, _SS,
  0,
  [_sA, _n, _aSsso, _reg, _pA, _aSs, _oFC, _rFC, _dD, _ac, _oIr, _oI, _aIc, _cA, _uA],
  [0, 0, () => AssociatedSystemList, 64 | 0, 0, 0, 1, 1, () => DependencyDiscoveryConfig$, () => Achievability$, 0, 0, 0, 4, 4], 2
];
export var ServiceSystemAssociatedMetadata$: StaticStructureSchema = [3, n0, _SSAM,
  0,
  [_sN, _sAy],
  [0, 0]
];
export var ServiceSystemDisassociatedMetadata$: StaticStructureSchema = [3, n0, _SSDM,
  0,
  [_sIy, _sN, _sAy],
  [0, 0, 0]
];
export var ServiceTopologyEdgeSummary$: StaticStructureSchema = [3, n0, _STES,
  0,
  [_sRI, _dRI, _sRo, _dRe, _sAo, _dA, _pro],
  [0, 0, 0, 0, 0, 0, () => EdgePropertyList], 2
];
export var ServiceWorkflowUpdatedMetadata$: StaticStructureSchema = [3, n0, _SWUM,
  0,
  [_sFI, _sFN],
  [0, 0]
];
export var SloSource$: StaticStructureSchema = [3, n0, _SSl,
  0,
  [_v, _pN, _so],
  [1, 0, 0]
];
export var StartFailureModeAssessmentRequest$: StaticStructureSchema = [3, n0, _SFMAR,
  0,
  [_sA, _cT],
  [0, [0, 4]], 1
];
export var StartFailureModeAssessmentResponse$: StaticStructureSchema = [3, n0, _SFMARt,
  0,
  [_aIs, _sA, _aSs, _sAt],
  [0, 0, 0, 4]
];
export var StringChange$: StaticStructureSchema = [3, n0, _SC,
  0,
  [_oV, _nV],
  [0, 0]
];
export var System$: StaticStructureSchema = [3, n0, _Sy,
  0,
  [_sAy, _sIy, _n, _d, _sE, _tag, _kKI, _oIr, _oI, _cA, _uA],
  [0, 0, 0, 0, 2, [() => TagMap, 0], 0, 0, 0, 4, 4], 3
];
export var SystemCreatedMetadata$: StaticStructureSchema = [3, n0, _SCMy,
  0,
  [],
  []
];
export var SystemDeletedMetadata$: StaticStructureSchema = [3, n0, _SDMy,
  0,
  [],
  []
];
export var SystemEvent$: StaticStructureSchema = [3, n0, _SEy,
  0,
  [_eIv, _ti, _eTv, _sAy, _act, _eD],
  [0, 4, 0, 0, () => EventActor$, () => SystemEventDetails$], 6
];
export var SystemEventDetails$: StaticStructureSchema = [3, n0, _SEDy,
  0,
  [_tit, _d, _eMv],
  [0, 0, () => SystemEventMetadata$], 2
];
export var SystemPolicyAssociatedMetadata$: StaticStructureSchema = [3, n0, _SPAMy,
  0,
  [_pN, _pA],
  [0, 0]
];
export var SystemPolicyDisassociatedMetadata$: StaticStructureSchema = [3, n0, _SPDMy,
  0,
  [_pN, _pA],
  [0, 0]
];
export var SystemServiceAssociatedMetadata$: StaticStructureSchema = [3, n0, _SSAMy,
  0,
  [_sNe, _sA, _uJs],
  [0, 0, 64 | 0]
];
export var SystemServiceDisassociatedMetadata$: StaticStructureSchema = [3, n0, _SSDMy,
  0,
  [_sNe, _sA, _uJA, _co],
  [0, 0, 64 | 0, 0]
];
export var SystemSummary$: StaticStructureSchema = [3, n0, _SSy,
  0,
  [_sIy, _n, _sAy, _uJC, _sCe, _oIr, _oI, _cA, _uA],
  [0, 0, 0, 1, 1, 0, 0, 4, 4], 2
];
export var SystemUserJourneyCreatedMetadata$: StaticStructureSchema = [3, n0, _SUJCM,
  0,
  [_uJN, _aSssoc],
  [0, () => ServiceReferenceList]
];
export var SystemUserJourneyDeletedMetadata$: StaticStructureSchema = [3, n0, _SUJDM,
  0,
  [_uJN, _aSAD],
  [0, () => ServiceReferenceList]
];
export var SystemUserJourneyUpdatedMetadata$: StaticStructureSchema = [3, n0, _SUJUM,
  0,
  [_uJN, _ch],
  [0, () => UserJourneyChanges$]
];
export var TagResourceRequest$: StaticStructureSchema = [3, n0, _TRR,
  0,
  [_rA, _tag],
  [[0, 1], [() => TagMap, 0]], 2
];
export var TagResourceResponse$: StaticStructureSchema = [3, n0, _TRRa,
  0,
  [],
  []
];
export var TargetSource$: StaticStructureSchema = [3, n0, _TS,
  0,
  [_v, _pN, _so],
  [1, 0, 0]
];
export var TestingRecommendation$: StaticStructureSchema = [3, n0, _TR,
  0,
  [_sC],
  [64 | 0]
];
export var UntagResourceRequest$: StaticStructureSchema = [3, n0, _URR,
  0,
  [_rA, _tK],
  [[0, 1], [() => TagKeyList, { [_hQ]: _tK }]], 2
];
export var UntagResourceResponse$: StaticStructureSchema = [3, n0, _URRn,
  0,
  [],
  []
];
export var UpdateAssertionRequest$: StaticStructureSchema = [3, n0, _UAR,
  0,
  [_sA, _aI, _t],
  [0, 0, 0], 2
];
export var UpdateAssertionResponse$: StaticStructureSchema = [3, n0, _UARp,
  0,
  [_as],
  [() => Assertion$], 1
];
export var UpdateDependencyRequest$: StaticStructureSchema = [3, n0, _UDR,
  0,
  [_sA, _dI, _cr, _co],
  [0, 0, 0, 0], 2
];
export var UpdateDependencyResponse$: StaticStructureSchema = [3, n0, _UDRp,
  0,
  [_dI, _dN, _l, _cr, _uA, _co, _pr],
  [0, 0, 0, 0, 4, 0, 0], 5
];
export var UpdateFailureModeFindingRequest$: StaticStructureSchema = [3, n0, _UFMFR,
  0,
  [_fI, _st, _sA, _co],
  [0, 0, 0, 0], 3
];
export var UpdateFailureModeFindingResponse$: StaticStructureSchema = [3, n0, _UFMFRp,
  0,
  [_f],
  [() => Finding$]
];
export var UpdatePolicyRequest$: StaticStructureSchema = [3, n0, _UPR,
  0,
  [_pA, _d, _aS, _mA, _mR, _dR],
  [0, 0, () => AvailabilitySlo$, () => MultiAzTargets$, () => MultiRegionTargets$, () => DataRecoveryTargets$], 1
];
export var UpdatePolicyResponse$: StaticStructureSchema = [3, n0, _UPRp,
  0,
  [_p],
  [[() => Policy$, 0]], 1
];
export var UpdateServiceFunctionRequest$: StaticStructureSchema = [3, n0, _USFR,
  0,
  [_sA, _sFI, _n, _d, _cr],
  [0, 0, 0, 0, 0], 2
];
export var UpdateServiceFunctionResponse$: StaticStructureSchema = [3, n0, _USFRp,
  0,
  [_sF],
  [() => ServiceFunction$], 1
];
export var UpdateServiceRequest$: StaticStructureSchema = [3, n0, _USR,
  0,
  [_sA, _d, _aSsso, _pA, _reg, _pM, _dD, _rCe],
  [0, 0, () => AssociatedSystemList, 0, 64 | 0, () => PermissionModel$, 0, () => ServiceReportConfiguration$], 1
];
export var UpdateServiceResponse$: StaticStructureSchema = [3, n0, _USRp,
  0,
  [_ser],
  [[() => Service$, 0]], 1
];
export var UpdateSystemRequest$: StaticStructureSchema = [3, n0, _USRpd,
  0,
  [_sAy, _d, _sE],
  [0, 0, 2], 1
];
export var UpdateSystemResponse$: StaticStructureSchema = [3, n0, _USRpda,
  0,
  [_sy],
  [[() => System$, 0]], 1
];
export var UpdateUserJourneyRequest$: StaticStructureSchema = [3, n0, _UUJR,
  0,
  [_sAy, _uJIs, _n, _d, _pA],
  [0, 0, 0, 0, 0], 2
];
export var UpdateUserJourneyResponse$: StaticStructureSchema = [3, n0, _UUJRp,
  0,
  [_uJ],
  [() => UserJourney$], 1
];
export var UserJourney$: StaticStructureSchema = [3, n0, _UJ,
  0,
  [_uJIs, _n, _d, _pA, _cA, _uA],
  [0, 0, 0, 0, 4, 4], 2
];
export var UserJourneyChanges$: StaticStructureSchema = [3, n0, _UJC,
  0,
  [_jD, _aSssoc],
  [() => StringChange$, () => ServiceReferenceChanges$]
];
export var UserJourneySummary$: StaticStructureSchema = [3, n0, _UJS,
  0,
  [_uJIs, _n, _cA, _uA],
  [0, 0, 4, 4], 2
];
export var ValidationExceptionField$: StaticStructureSchema = [3, n0, _VEF,
  0,
  [_n, _m],
  [0, 0], 2
];
var ArnList = 64 | 0;
var AssertionList: StaticListSchema = [1, n0, _AL,
  0, () => Assertion$
];
var AssessmentStatusList = 64 | 0;
var AssessmentSummaryList: StaticListSchema = [1, n0, _ASL,
  0, () => AssessmentSummary$
];
var AssociatedSystemList: StaticListSchema = [1, n0, _ASLs,
  0, () => AssociatedSystem$
];
var CrossAccountRoleList: StaticListSchema = [1, n0, _CARL,
  0, () => CrossAccountRole$
];
var DependencySummaryList: StaticListSchema = [1, n0, _DSL,
  0, () => DependencySummary$
];
var EdgePropertyList: StaticListSchema = [1, n0, _EPL,
  0, () => EdgePropertySummary$
];
var EksNamespaceList = 64 | 0;
var FindingsList: StaticListSchema = [1, n0, _FL,
  0, () => FindingSummary$
];
var FunctionsList = 64 | 0;
var InfrastructureAndCodeRecommendationsList: StaticListSchema = [1, n0, _IACRL,
  0, () => InfrastructureAndCodeRecommendation$
];
var InputSourceSummaryList: StaticListSchema = [1, n0, _ISSL,
  0, () => InputSourceSummary$
];
var ObservabilityRecommendationsList: StaticListSchema = [1, n0, _ORL,
  0, () => ObservabilityRecommendation$
];
var PolicySummaryList: StaticListSchema = [1, n0, _PSL,
  0, () => PolicySummary$
];
var QueryDataPointList: StaticListSchema = [1, n0, _QDPL,
  0, () => QueryDataPoint$
];
var RegionList = 64 | 0;
var ReportGenerationResultList: StaticListSchema = [1, n0, _RGRL,
  0, () => ReportGenerationResult$
];
var ReportOutputConfigurationList: StaticListSchema = [1, n0, _ROCL,
  0, () => ReportOutputConfiguration$
];
var ResourceList = 64 | 0;
var ResourceTagList: StaticListSchema = [1, n0, _RTL,
  0, () => ResourceTag$
];
var ResourceTypeFilterList = 64 | 0;
var ResourceTypeList = 64 | 0;
var ServiceEventList: StaticListSchema = [1, n0, _SEL,
  0, () => ServiceEvent$
];
var ServiceEventTypeList = 64 | 0;
var ServiceFunctionList: StaticListSchema = [1, n0, _SFL,
  0, () => ServiceFunction$
];
var ServiceReferenceList: StaticListSchema = [1, n0, _SRL,
  0, () => ServiceReference$
];
var ServiceResourceList: StaticListSchema = [1, n0, _SRLe,
  0, () => ServiceResource$
];
var ServiceSummaryList: StaticListSchema = [1, n0, _SSL,
  0, () => ServiceSummary$
];
var ServiceTopologyEdgeSummaryList: StaticListSchema = [1, n0, _STESL,
  0, () => ServiceTopologyEdgeSummary$
];
var SuggestedChangesList = 64 | 0;
var SystemEventList: StaticListSchema = [1, n0, _SELy,
  0, () => SystemEvent$
];
var SystemEventTypeList = 64 | 0;
var SystemSummaryList: StaticListSchema = [1, n0, _SSLy,
  0, () => SystemSummary$
];
var TagKeyList: StaticListSchema = [1, n0, _TKL,
  8, 0
];
var TagValueList = 64 | 0;
var TestingRecommendationsList: StaticListSchema = [1, n0, _TRL,
  0, () => TestingRecommendation$
];
var UserJourneyIdList = 64 | 0;
var UserJourneyNameList = 64 | 0;
var UserJourneySummaryList: StaticListSchema = [1, n0, _UJSL,
  0, () => UserJourneySummary$
];
var ValidationExceptionFieldList: StaticListSchema = [1, n0, _VEFL,
  0, () => ValidationExceptionField$
];
var TagMap: StaticMapSchema = [2, n0, _TM,
  8, 0, 0
];
export var ReportOutput$: StaticUnionSchema = [4, n0, _RO,
  0,
  [_sRO, _fRO],
  [() => S3ReportOutput$, () => FailedReportOutput$]
];
export var ReportOutputConfiguration$: StaticUnionSchema = [4, n0, _ROC,
  0,
  [_s_],
  [() => S3ReportOutputConfiguration$]
];
export var ResourceConfiguration$: StaticUnionSchema = [4, n0, _RC,
  0,
  [_rTes, _cSA, _tSFU, _ek, _dFSU],
  [() => ResourceTagList, 0, 0, () => EksSource$, 0]
];
export var ServiceEventMetadata$: StaticUnionSchema = [4, n0, _SEM,
  0,
  [_sCer, _sD, _sSA, _sSD, _sRA, _sRD, _sWU, _sISU, _sPA, _sPD, _sFC, _sFU, _sFD, _sFRA, _sFRR, _sAU, _aCs, _aU, _aD],
  [() => ServiceCreatedMetadata$, () => ServiceDeletedMetadata$, () => ServiceSystemAssociatedMetadata$, () => ServiceSystemDisassociatedMetadata$, () => ServiceResourcesAssociatedMetadata$, () => ServiceResourcesDisassociatedMetadata$, () => ServiceWorkflowUpdatedMetadata$, () => ServiceInputSourcesUpdatedMetadata$, () => ServicePolicyAssociatedMetadata$, () => ServicePolicyDisassociatedMetadata$, () => ServiceFunctionCreatedMetadata$, () => ServiceFunctionUpdatedMetadata$, () => ServiceFunctionDeletedMetadata$, () => ServiceFunctionResourcesAddedMetadata$, () => ServiceFunctionResourcesRemovedMetadata$, () => ServiceAchievabilityUpdatedMetadata$, () => AssertionCreatedMetadata$, () => AssertionUpdatedMetadata$, () => AssertionDeletedMetadata$]
];
export var SystemEventMetadata$: StaticUnionSchema = [4, n0, _SEMy,
  0,
  [_sCy, _sDy, _sUJC, _sUJU, _sUJD, _sSAy, _sSDy, _sPAy, _sPDy],
  [() => SystemCreatedMetadata$, () => SystemDeletedMetadata$, () => SystemUserJourneyCreatedMetadata$, () => SystemUserJourneyUpdatedMetadata$, () => SystemUserJourneyDeletedMetadata$, () => SystemServiceAssociatedMetadata$, () => SystemServiceDisassociatedMetadata$, () => SystemPolicyAssociatedMetadata$, () => SystemPolicyDisassociatedMetadata$]
];
export var CreateAssertion$: StaticOperationSchema = [9, n0, _CA,
  { [_h]: ["POST", "/v2/create-assertion", 200] }, () => CreateAssertionRequest$, () => CreateAssertionResponse$
];
export var CreateInputSource$: StaticOperationSchema = [9, n0, _CIS,
  { [_h]: ["POST", "/v2/create-input-source", 200] }, () => CreateInputSourceRequest$, () => CreateInputSourceResponse$
];
export var CreatePolicy$: StaticOperationSchema = [9, n0, _CP,
  { [_h]: ["POST", "/v2/create-policy", 200] }, () => CreatePolicyRequest$, () => CreatePolicyResponse$
];
export var CreateReport$: StaticOperationSchema = [9, n0, _CR,
  { [_h]: ["POST", "/v2/create-report", 200] }, () => CreateReportRequest$, () => CreateReportResponse$
];
export var CreateService$: StaticOperationSchema = [9, n0, _CS,
  { [_h]: ["POST", "/v2/create-service", 200] }, () => CreateServiceRequest$, () => CreateServiceResponse$
];
export var CreateServiceFunction$: StaticOperationSchema = [9, n0, _CSF,
  { [_h]: ["POST", "/v2/create-service-function", 200] }, () => CreateServiceFunctionRequest$, () => CreateServiceFunctionResponse$
];
export var CreateServiceFunctionResources$: StaticOperationSchema = [9, n0, _CSFRre,
  { [_h]: ["POST", "/v2/create-service-function-resources", 200] }, () => CreateServiceFunctionResourcesRequest$, () => CreateServiceFunctionResourcesResponse$
];
export var CreateSystem$: StaticOperationSchema = [9, n0, _CSr,
  { [_h]: ["POST", "/v2/create-system", 200] }, () => CreateSystemRequest$, () => CreateSystemResponse$
];
export var CreateUserJourney$: StaticOperationSchema = [9, n0, _CUJ,
  { [_h]: ["POST", "/v2/create-user-journey", 200] }, () => CreateUserJourneyRequest$, () => CreateUserJourneyResponse$
];
export var DeleteAssertion$: StaticOperationSchema = [9, n0, _DA,
  { [_h]: ["POST", "/v2/delete-assertion", 200] }, () => DeleteAssertionRequest$, () => DeleteAssertionResponse$
];
export var DeleteInputSource$: StaticOperationSchema = [9, n0, _DIS,
  { [_h]: ["POST", "/v2/delete-input-source", 200] }, () => DeleteInputSourceRequest$, () => DeleteInputSourceResponse$
];
export var DeletePolicy$: StaticOperationSchema = [9, n0, _DP,
  { [_h]: ["POST", "/v2/delete-policy", 200] }, () => DeletePolicyRequest$, () => DeletePolicyResponse$
];
export var DeleteService$: StaticOperationSchema = [9, n0, _DSe,
  { [_h]: ["POST", "/v2/delete-service", 200] }, () => DeleteServiceRequest$, () => DeleteServiceResponse$
];
export var DeleteServiceFunction$: StaticOperationSchema = [9, n0, _DSF,
  { [_h]: ["POST", "/v2/delete-function", 200] }, () => DeleteServiceFunctionRequest$, () => DeleteServiceFunctionResponse$
];
export var DeleteServiceFunctionResources$: StaticOperationSchema = [9, n0, _DSFRel,
  { [_h]: ["POST", "/v2/delete-service-function-resources", 200] }, () => DeleteServiceFunctionResourcesRequest$, () => DeleteServiceFunctionResourcesResponse$
];
export var DeleteSystem$: StaticOperationSchema = [9, n0, _DSel,
  { [_h]: ["POST", "/v2/delete-system", 200] }, () => DeleteSystemRequest$, () => DeleteSystemResponse$
];
export var DeleteUserJourney$: StaticOperationSchema = [9, n0, _DUJ,
  { [_h]: ["POST", "/v2/delete-user-journey", 200] }, () => DeleteUserJourneyRequest$, () => DeleteUserJourneyResponse$
];
export var GetFailureModeFinding$: StaticOperationSchema = [9, n0, _GFMF,
  { [_h]: ["GET", "/v2/get-failure-mode-finding", 200] }, () => GetFailureModeFindingRequest$, () => GetFailureModeFindingResponse$
];
export var GetPolicy$: StaticOperationSchema = [9, n0, _GP,
  { [_h]: ["GET", "/v2/get-policy", 200] }, () => GetPolicyRequest$, () => GetPolicyResponse$
];
export var GetService$: StaticOperationSchema = [9, n0, _GS,
  { [_h]: ["GET", "/v2/get-service", 200] }, () => GetServiceRequest$, () => GetServiceResponse$
];
export var GetSystem$: StaticOperationSchema = [9, n0, _GSe,
  { [_h]: ["GET", "/v2/get-system", 200] }, () => GetSystemRequest$, () => GetSystemResponse$
];
export var GetUserJourney$: StaticOperationSchema = [9, n0, _GUJ,
  { [_h]: ["GET", "/v2/get-user-journey", 200] }, () => GetUserJourneyRequest$, () => GetUserJourneyResponse$
];
export var ImportApp$: StaticOperationSchema = [9, n0, _IA,
  { [_h]: ["POST", "/v2/import-app", 200] }, () => ImportAppRequest$, () => ImportAppResponse$
];
export var ImportPolicy$: StaticOperationSchema = [9, n0, _IP,
  { [_h]: ["POST", "/v2/import-policy", 200] }, () => ImportPolicyRequest$, () => ImportPolicyResponse$
];
export var ListAssertions$: StaticOperationSchema = [9, n0, _LA,
  { [_h]: ["GET", "/v2/list-assertions", 200] }, () => ListAssertionsRequest$, () => ListAssertionsResponse$
];
export var ListDependencies$: StaticOperationSchema = [9, n0, _LD,
  { [_h]: ["GET", "/v2/list-dependencies", 200] }, () => ListDependenciesRequest$, () => ListDependenciesResponse$
];
export var ListFailureModeAssessments$: StaticOperationSchema = [9, n0, _LFMA,
  { [_h]: ["GET", "/v2/list-failure-mode-assessments", 200] }, () => ListFailureModeAssessmentsRequest$, () => ListFailureModeAssessmentsResponse$
];
export var ListFailureModeFindings$: StaticOperationSchema = [9, n0, _LFMF,
  { [_h]: ["GET", "/v2/list-failure-mode-findings", 200] }, () => ListFailureModeFindingsRequest$, () => ListFailureModeFindingsResponse$
];
export var ListInputSources$: StaticOperationSchema = [9, n0, _LIS,
  { [_h]: ["GET", "/v2/list-input-sources", 200] }, () => ListInputSourcesRequest$, () => ListInputSourcesResponse$
];
export var ListPolicies$: StaticOperationSchema = [9, n0, _LP,
  { [_h]: ["GET", "/v2/list-policies", 200] }, () => ListPoliciesRequest$, () => ListPoliciesResponse$
];
export var ListReports$: StaticOperationSchema = [9, n0, _LR,
  { [_h]: ["GET", "/v2/list-reports", 200] }, () => ListReportsRequest$, () => ListReportsResponse$
];
export var ListResources$: StaticOperationSchema = [9, n0, _LRi,
  { [_h]: ["GET", "/v2/list-resources", 200] }, () => ListResourcesRequest$, () => ListResourcesResponse$
];
export var ListServiceEvents$: StaticOperationSchema = [9, n0, _LSE,
  { [_h]: ["GET", "/v2/list-service-events", 200] }, () => ListServiceEventsRequest$, () => ListServiceEventsResponse$
];
export var ListServiceFunctions$: StaticOperationSchema = [9, n0, _LSF,
  { [_h]: ["GET", "/v2/list-functions", 200] }, () => ListServiceFunctionsRequest$, () => ListServiceFunctionsResponse$
];
export var ListServices$: StaticOperationSchema = [9, n0, _LS,
  { [_h]: ["GET", "/v2/list-services", 200] }, () => ListServicesRequest$, () => ListServicesResponse$
];
export var ListServiceTopologyEdges$: StaticOperationSchema = [9, n0, _LSTE,
  { [_h]: ["GET", "/v2/list-service-topology-edges", 200] }, () => ListServiceTopologyEdgesRequest$, () => ListServiceTopologyEdgesResponse$
];
export var ListSystemEvents$: StaticOperationSchema = [9, n0, _LSEi,
  { [_h]: ["GET", "/v2/list-system-events", 200] }, () => ListSystemEventsRequest$, () => ListSystemEventsResponse$
];
export var ListSystems$: StaticOperationSchema = [9, n0, _LSi,
  { [_h]: ["GET", "/v2/list-systems", 200] }, () => ListSystemsRequest$, () => ListSystemsResponse$
];
export var ListTagsForResource$: StaticOperationSchema = [9, n0, _LTFR,
  { [_h]: ["GET", "/v2/tags/{resourceArn}", 200] }, () => ListTagsForResourceRequest$, () => ListTagsForResourceResponse$
];
export var ListUserJourneys$: StaticOperationSchema = [9, n0, _LUJ,
  { [_h]: ["GET", "/v2/list-user-journeys", 200] }, () => ListUserJourneysRequest$, () => ListUserJourneysResponse$
];
export var StartFailureModeAssessment$: StaticOperationSchema = [9, n0, _SFMA,
  { [_h]: ["POST", "/v2/start-failure-mode-assessment", 200] }, () => StartFailureModeAssessmentRequest$, () => StartFailureModeAssessmentResponse$
];
export var TagResource$: StaticOperationSchema = [9, n0, _TRa,
  { [_h]: ["POST", "/v2/tags/{resourceArn}", 200] }, () => TagResourceRequest$, () => TagResourceResponse$
];
export var UntagResource$: StaticOperationSchema = [9, n0, _UR,
  { [_h]: ["DELETE", "/v2/tags/{resourceArn}", 200] }, () => UntagResourceRequest$, () => UntagResourceResponse$
];
export var UpdateAssertion$: StaticOperationSchema = [9, n0, _UA,
  { [_h]: ["POST", "/v2/update-assertion", 200] }, () => UpdateAssertionRequest$, () => UpdateAssertionResponse$
];
export var UpdateDependency$: StaticOperationSchema = [9, n0, _UD,
  { [_h]: ["POST", "/v2/update-dependency", 200] }, () => UpdateDependencyRequest$, () => UpdateDependencyResponse$
];
export var UpdateFailureModeFinding$: StaticOperationSchema = [9, n0, _UFMF,
  { [_h]: ["POST", "/v2/update-failure-mode-finding", 200] }, () => UpdateFailureModeFindingRequest$, () => UpdateFailureModeFindingResponse$
];
export var UpdatePolicy$: StaticOperationSchema = [9, n0, _UP,
  { [_h]: ["POST", "/v2/update-policy", 200] }, () => UpdatePolicyRequest$, () => UpdatePolicyResponse$
];
export var UpdateService$: StaticOperationSchema = [9, n0, _US,
  { [_h]: ["POST", "/v2/update-service", 200] }, () => UpdateServiceRequest$, () => UpdateServiceResponse$
];
export var UpdateServiceFunction$: StaticOperationSchema = [9, n0, _USF,
  { [_h]: ["POST", "/v2/update-function", 200] }, () => UpdateServiceFunctionRequest$, () => UpdateServiceFunctionResponse$
];
export var UpdateSystem$: StaticOperationSchema = [9, n0, _USp,
  { [_h]: ["POST", "/v2/update-system", 200] }, () => UpdateSystemRequest$, () => UpdateSystemResponse$
];
export var UpdateUserJourney$: StaticOperationSchema = [9, n0, _UUJ,
  { [_h]: ["POST", "/v2/update-user-journey", 200] }, () => UpdateUserJourneyRequest$, () => UpdateUserJourneyResponse$
];
