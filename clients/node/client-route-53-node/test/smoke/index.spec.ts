...
async function smokeTestRunner() {
  ...
  await (async () => {
    ...
    const command = new ListHostedZonesCommand({} as any);

    try {
      const result = await client.send(command);
      console.log(
        "ok Route 53 ListHostedZones - no error expected from service"
      );
    } catch (err) {
      console.log(
        "not ok Route 53 ListHostedZones - no error expected from service"
      );
  ...
  await (async () => {
    ...
    try {
      const result = await client.send(command);
      console.log(
        "not ok Route 53 GetHostedZone - error expected from service"
      );
      testFailed = true;
    } catch (err) {
