...
async function smokeTestRunner() {
  ...
  await (async () => {
    ...
    const command = new ListRepositoriesCommand({} as any);

    try {
      const result = await client.send(command);
      console.log(
        "ok CodeCommit ListRepositories - no error expected from service"
      );
    } catch (err) {
      console.log(
        "not ok CodeCommit ListRepositories - no error expected from service"
      );
  ...
  await (async () => {
    ...
    try {
      const result = await client.send(command);
      console.log(
        "not ok CodeCommit ListBranches - error expected from service"
      );
      testFailed = true;
    } catch (err) {
