const _AC = "AggregationConfiguration";
const _ACg = "AggregationConfigurations";
const _ADE = "AccessDeniedException";
const _AES = "AdvancedEventSelector";
const _AESd = "AdvancedEventSelectors";
const _AFS = "AdvancedFieldSelector";
const _AFSd = "AdvancedFieldSelectors";
const _AHOIE = "AccountHasOngoingImportException";
const _AK = "AttributeKey";
const _AKI = "AccessKeyId";
const _ANFE = "AccountNotFoundException";
const _ANRE = "AccountNotRegisteredException";
const _ARE = "AccountRegisteredException";
const _AT = "AddTags";
const _ATAR = "ApplyToAllRegions";
const _ATR = "AddTagsRequest";
const _ATRd = "AddTagsResponse";
const _AV = "AttributeValue";
const _BM = "BillingMode";
const _BS = "BytesScanned";
const _C = "Channel";
const _CA = "ChannelArn";
const _CAEE = "ChannelAlreadyExistsException";
const _CARNIE = "ChannelARNInvalidException";
const _CC = "CreateChannel";
const _CCR = "CreateChannelRequest";
const _CCRr = "CreateChannelResponse";
const _CD = "CreateDashboard";
const _CDMAE = "CannotDelegateManagementAccountException";
const _CDR = "CreateDashboardRequest";
const _CDRr = "CreateDashboardResponse";
const _CE = "ConflictException";
const _CEDS = "CreateEventDataStore";
const _CEDSR = "CreateEventDataStoreRequest";
const _CEDSRr = "CreateEventDataStoreResponse";
const _CEFEDSE = "ChannelExistsForEDSException";
const _CKS = "ContextKeySelector";
const _CKSo = "ContextKeySelectors";
const _CME = "ConcurrentModificationException";
const _CMLEE = "ChannelMaxLimitExceededException";
const _CNFE = "ChannelNotFoundException";
const _CQ = "CancelQuery";
const _CQR = "CancelQueryRequest";
const _CQRa = "CancelQueryResponse";
const _CT = "CreatedTimestamp";
const _CTANEE = "CloudTrailAccessNotEnabledException";
const _CTARNIE = "CloudTrailARNInvalidException";
const _CTE = "CloudTrailEvent";
const _CTICTIE = "CloudTrailInvalidClientTokenIdException";
const _CTR = "CreateTrailRequest";
const _CTRr = "CreateTrailResponse";
const _CTr = "CreationTime";
const _CTre = "CreateTrail";
const _CWLDUE = "CloudWatchLogsDeliveryUnavailableException";
const _CWLLGA = "CloudWatchLogsLogGroupArn";
const _CWLRA = "CloudWatchLogsRoleArn";
const _Ch = "Channels";
const _D = "Destinations";
const _DA = "DashboardArn";
const _DAAI = "DelegatedAdminAccountId";
const _DAALEE = "DelegatedAdminAccountLimitExceededException";
const _DARP = "DelegatedAdminResourcePolicy";
const _DC = "DeleteChannel";
const _DCR = "DeleteChannelRequest";
const _DCRe = "DeleteChannelResponse";
const _DD = "DashboardDetail";
const _DDR = "DeleteDashboardRequest";
const _DDRe = "DeleteDashboardResponse";
const _DDe = "DeleteDashboard";
const _DEDS = "DeleteEventDataStore";
const _DEDSR = "DeleteEventDataStoreRequest";
const _DEDSRe = "DeleteEventDataStoreResponse";
const _DF = "DisableFederation";
const _DFR = "DisableFederationRequest";
const _DFRi = "DisableFederationResponse";
const _DI = "DashboardId";
const _DODA = "DeregisterOrganizationDelegatedAdmin";
const _DODAR = "DeregisterOrganizationDelegatedAdminRequest";
const _DODARe = "DeregisterOrganizationDelegatedAdminResponse";
const _DQ = "DescribeQuery";
const _DQR = "DescribeQueryRequest";
const _DQRe = "DescribeQueryResponse";
const _DR = "DataResource";
const _DRP = "DeleteResourcePolicy";
const _DRPR = "DeleteResourcePolicyRequest";
const _DRPRe = "DeleteResourcePolicyResponse";
const _DRa = "DataResources";
const _DS = "DeliveryStatus";
const _DSU = "DeliveryS3Uri";
const _DT = "DataType";
const _DTR = "DeleteTrailRequest";
const _DTRe = "DeleteTrailResponse";
const _DTRes = "DescribeTrailsRequest";
const _DTResc = "DescribeTrailsResponse";
const _DTe = "DeleteTrail";
const _DTes = "DescribeTrails";
const _Da = "Dashboards";
const _De = "Destination";
const _Des = "Description";
const _Di = "Dimensions";
const _E = "Equals";
const _EC = "EventCategory";
const _ECr = "ErrorCode";
const _ECv = "EventsCompleted";
const _ECve = "EventCategories";
const _EDS = "EventDataStore";
const _EDSA = "EventDataStoreArn";
const _EDSAEE = "EventDataStoreAlreadyExistsException";
const _EDSARNIE = "EventDataStoreARNInvalidException";
const _EDSFEE = "EventDataStoreFederationEnabledException";
const _EDSHOIE = "EventDataStoreHasOngoingImportException";
const _EDSMLEE = "EventDataStoreMaxLimitExceededException";
const _EDSNFE = "EventDataStoreNotFoundException";
const _EDSOAI = "EventDataStoreOwnerAccountId";
const _EDSTPE = "EventDataStoreTerminationProtectedException";
const _EDSv = "EventDataStores";
const _EET = "EndEventTime";
const _EF = "EnableFederation";
const _EFR = "EnableFederationRequest";
const _EFRn = "EnableFederationResponse";
const _EI = "EventId";
const _EL = "EventsList";
const _ELFV = "EnableLogFileValidation";
const _EM = "ErrorMessage";
const _EMES = "ExcludeManagementEventSources";
const _EMv = "EventsMatched";
const _EN = "EventName";
const _ES = "EventSource";
const _ESv = "EventSelector";
const _ESve = "EventSelectors";
const _ESven = "EventsScanned";
const _ET = "EventTime";
const _ETIM = "ExecutionTimeInMillis";
const _ETn = "EndTime";
const _ETr = "ErrorType";
const _EW = "EndsWith";
const _Ev = "Event";
const _Eve = "Events";
const _F = "Field";
const _FC = "FilesCompleted";
const _FE = "FailedEntries";
const _FRA = "FederationRoleArn";
const _FS = "FieldSelectors";
const _FSe = "FederationStatus";
const _Fa = "Failures";
const _Fi = "Fingerprint";
const _Fr = "Frequency";
const _GC = "GetChannel";
const _GCR = "GetChannelRequest";
const _GCRe = "GetChannelResponse";
const _GD = "GetDashboard";
const _GDR = "GetDashboardRequest";
const _GDRe = "GetDashboardResponse";
const _GEC = "GetEventConfiguration";
const _GECR = "GetEventConfigurationRequest";
const _GECRe = "GetEventConfigurationResponse";
const _GEDS = "GetEventDataStore";
const _GEDSR = "GetEventDataStoreRequest";
const _GEDSRe = "GetEventDataStoreResponse";
const _GES = "GetEventSelectors";
const _GESR = "GetEventSelectorsRequest";
const _GESRe = "GetEventSelectorsResponse";
const _GI = "GetImport";
const _GIR = "GetImportRequest";
const _GIRe = "GetImportResponse";
const _GIS = "GetInsightSelectors";
const _GISR = "GetInsightSelectorsRequest";
const _GISRe = "GetInsightSelectorsResponse";
const _GQ = "GenerateQuery";
const _GQR = "GenerateQueryRequest";
const _GQRR = "GetQueryResultsRequest";
const _GQRRe = "GetQueryResultsResponse";
const _GQRe = "GenerateQueryResponse";
const _GQRet = "GetQueryResults";
const _GRE = "GenerateResponseException";
const _GRP = "GetResourcePolicy";
const _GRPR = "GetResourcePolicyRequest";
const _GRPRe = "GetResourcePolicyResponse";
const _GT = "GetTrail";
const _GTR = "GetTrailRequest";
const _GTRe = "GetTrailResponse";
const _GTS = "GetTrailStatus";
const _GTSR = "GetTrailStatusRequest";
const _GTSRe = "GetTrailStatusResponse";
const _HCES = "HasCustomEventSelectors";
const _HIS = "HasInsightSelectors";
const _HR = "HomeRegion";
const _I = "Imports";
const _ICWLLGAE = "InvalidCloudWatchLogsLogGroupArnException";
const _ICWLRAE = "InvalidCloudWatchLogsRoleArnException";
const _ID = "InsightsDestination";
const _IDRE = "InvalidDateRangeException";
const _IDSAPE = "InsufficientDependencyServiceAccessPermissionException";
const _IECE = "InvalidEventCategoryException";
const _IEDSCE = "InvalidEventDataStoreCategoryException";
const _IEDSE = "InactiveEventDataStoreException";
const _IEDSSE = "InvalidEventDataStoreStatusException";
const _IEPE = "InsufficientEncryptionPolicyException";
const _IESE = "InvalidEventSelectorsException";
const _IFL = "ImportFailureList";
const _IFLI = "ImportFailureListItem";
const _IGSE = "IncludeGlobalServiceEvents";
const _IHRE = "InvalidHomeRegionException";
const _II = "ImportId";
const _IIAMAPE = "InsufficientIAMAccessPermissionException";
const _IISE = "InvalidImportSourceException";
const _IISEn = "InvalidInsightSelectorsException";
const _IKKIE = "InvalidKmsKeyIdException";
const _IL = "IsLogging";
const _ILAE = "InvalidLookupAttributesException";
const _ILI = "ImportsListItem";
const _ILm = "ImportsList";
const _IME = "IncludeManagementEvents";
const _IMRE = "InvalidMaxResultsException";
const _IMRT = "IsMultiRegionTrail";
const _INEE = "InsightNotEnabledException";
const _INFE = "ImportNotFoundException";
const _INTE = "InvalidNextTokenException";
const _IOT = "IsOrganizationTrail";
const _IPCE = "InvalidParameterCombinationException";
const _IPE = "InvalidParameterException";
const _IQE = "InactiveQueryException";
const _IQSE = "InvalidQueryStatementException";
const _IQSEn = "InvalidQueryStatusException";
const _IS = "IngestionStatus";
const _ISBNE = "InvalidS3BucketNameException";
const _ISBPE = "InsufficientS3BucketPolicyException";
const _ISE = "InvalidSourceException";
const _ISPE = "InvalidS3PrefixException";
const _ISTNE = "InvalidSnsTopicNameException";
const _ISTPE = "InsufficientSnsTopicPolicyException";
const _ISm = "ImportSource";
const _ISmp = "ImportStatus";
const _ISmpo = "ImportStatistics";
const _ISn = "InsightSelectors";
const _ISns = "InsightSelector";
const _ISnsi = "InsightSource";
const _IT = "InsightType";
const _ITE = "InvalidTokenException";
const _ITNE = "InvalidTrailNameException";
const _ITPE = "InvalidTagParameterException";
const _ITRE = "InvalidTimeRangeException";
const _K = "Key";
const _KE = "KmsException";
const _KKDE = "KmsKeyDisabledException";
const _KKI = "KmsKeyId";
const _KKNFE = "KmsKeyNotFoundException";
const _L = "Location";
const _LA = "LookupAttribute";
const _LAL = "LookupAttributesList";
const _LAo = "LookupAttributes";
const _LC = "ListChannels";
const _LCR = "ListChannelsRequest";
const _LCRi = "ListChannelsResponse";
const _LCWLDE = "LatestCloudWatchLogsDeliveryError";
const _LCWLDT = "LatestCloudWatchLogsDeliveryTime";
const _LD = "ListDashboards";
const _LDAS = "LatestDeliveryAttemptSucceeded";
const _LDAT = "LatestDeliveryAttemptTime";
const _LDDE = "LatestDigestDeliveryError";
const _LDDT = "LatestDigestDeliveryTime";
const _LDE = "LatestDeliveryError";
const _LDR = "ListDashboardsRequest";
const _LDRi = "ListDashboardsResponse";
const _LDT = "LatestDeliveryTime";
const _LE = "LookupEvents";
const _LEDS = "ListEventDataStores";
const _LEDSR = "ListEventDataStoresRequest";
const _LEDSRi = "ListEventDataStoresResponse";
const _LER = "LookupEventsRequest";
const _LERo = "LookupEventsResponse";
const _LFVE = "LogFileValidationEnabled";
const _LI = "ListImports";
const _LIAEID = "LatestIngestionAttemptEventID";
const _LIAT = "LatestIngestionAttemptTime";
const _LID = "ListInsightsData";
const _LIDR = "ListInsightsDataRequest";
const _LIDRi = "ListInsightsDataResponse";
const _LIEC = "LatestIngestionErrorCode";
const _LIF = "ListImportFailures";
const _LIFR = "ListImportFailuresRequest";
const _LIFRi = "ListImportFailuresResponse";
const _LIMD = "ListInsightsMetricData";
const _LIMDR = "ListInsightsMetricDataRequest";
const _LIMDRi = "ListInsightsMetricDataResponse";
const _LIR = "ListImportsRequest";
const _LIRi = "ListImportsResponse";
const _LISEID = "LatestIngestionSuccessEventID";
const _LIST = "LatestIngestionSuccessTime";
const _LNAS = "LatestNotificationAttemptSucceeded";
const _LNAT = "LatestNotificationAttemptTime";
const _LNE = "LatestNotificationError";
const _LNT = "LatestNotificationTime";
const _LPK = "ListPublicKeys";
const _LPKR = "ListPublicKeysRequest";
const _LPKRi = "ListPublicKeysResponse";
const _LQ = "ListQueries";
const _LQR = "ListQueriesRequest";
const _LQRi = "ListQueriesResponse";
const _LRFR = "LastRefreshFailureReason";
const _LRI = "LastRefreshId";
const _LT = "ListTags";
const _LTR = "ListTagsRequest";
const _LTRi = "ListTagsResponse";
const _LTRis = "ListTrailsRequest";
const _LTRist = "ListTrailsResponse";
const _LTi = "ListTrails";
const _LUT = "LastUpdatedTime";
const _M = "Message";
const _MAI = "MemberAccountId";
const _MCQE = "MaxConcurrentQueriesException";
const _MES = "MaxEventSize";
const _MNOTEE = "MaximumNumberOfTrailsExceededException";
const _MQR = "MaxQueryResults";
const _MR = "MaxResults";
const _MRE = "MultiRegionEnabled";
const _N = "Name";
const _NE = "NotEquals";
const _NEW = "NotEndsWith";
const _NMASLREE = "NoManagementAccountSLRExistsException";
const _NOMAE = "NotOrganizationManagementAccountException";
const _NOMAEo = "NotOrganizationMasterAccountException";
const _NP = "NamePrefix";
const _NSW = "NotStartsWith";
const _NT = "NextToken";
const _OE = "OrganizationEnabled";
const _ONIAFME = "OrganizationNotInAllFeaturesModeException";
const _ONIUE = "OrganizationsNotInUseException";
const _ONPE = "OperationNotPermittedException";
const _P = "Prompt";
const _PC = "PrefixesCompleted";
const _PEC = "PutEventConfiguration";
const _PECR = "PutEventConfigurationRequest";
const _PECRu = "PutEventConfigurationResponse";
const _PES = "PutEventSelectors";
const _PESR = "PutEventSelectorsRequest";
const _PESRu = "PutEventSelectorsResponse";
const _PF = "PrefixesFound";
const _PIS = "PutInsightSelectors";
const _PISR = "PutInsightSelectorsRequest";
const _PISRu = "PutInsightSelectorsResponse";
const _PK = "PartitionKeys";
const _PKL = "PublicKeyList";
const _PKLa = "PartitionKeyList";
const _PKa = "PartitionKey";
const _PKu = "PublicKey";
const _PRP = "PutResourcePolicy";
const _PRPR = "PutResourcePolicyRequest";
const _PRPRu = "PutResourcePolicyResponse";
const _Pe = "Period";
const _Q = "Queries";
const _QA = "QueryAlias";
const _QI = "QueryId";
const _QINFE = "QueryIdNotFoundException";
const _QP = "QueryParameters";
const _QPV = "QueryParameterValues";
const _QRR = "QueryResultRows";
const _QRRu = "QueryResultRow";
const _QS = "QueryStatus";
const _QSFDQ = "QueryStatisticsForDescribeQuery";
const _QSu = "QueryString";
const _QSue = "QueryStatistics";
const _QSuer = "QueryStatement";
const _Qu = "Query";
const _R = "Resources";
const _RA = "ResourceArn";
const _RARNNVE = "ResourceARNNotValidException";
const _RC = "ResultsCount";
const _REDS = "RestoreEventDataStore";
const _REDSR = "RestoreEventDataStoreRequest";
const _REDSRe = "RestoreEventDataStoreResponse";
const _RI = "ResourceId";
const _RIL = "ResourceIdList";
const _RIe = "RefreshId";
const _RL = "ResourceList";
const _RN = "ResourceName";
const _RNFE = "ResourceNotFoundException";
const _RO = "ReadOnly";
const _RODA = "RegisterOrganizationDelegatedAdmin";
const _RODAR = "RegisterOrganizationDelegatedAdminRequest";
const _RODARe = "RegisterOrganizationDelegatedAdminResponse";
const _RP = "RetentionPeriod";
const _RPNFE = "ResourcePolicyNotFoundException";
const _RPNVE = "ResourcePolicyNotValidException";
const _RPe = "ResourcePolicy";
const _RS = "RefreshSchedule";
const _RSF = "RefreshScheduleFrequency";
const _RT = "ResourceType";
const _RTL = "ResourceTagList";
const _RTNSE = "ResourceTypeNotSupportedException";
const _RTR = "RemoveTagsRequest";
const _RTRe = "RemoveTagsResponse";
const _RTe = "ResourceTag";
const _RTem = "RemoveTags";
const _RW = "RequestWidget";
const _RWL = "RequestWidgetList";
const _RWT = "ReadWriteType";
const _Re = "Resource";
const _Rel = "Relevance";
const _S = "Source";
const _SBARA = "S3BucketAccessRoleArn";
const _SBDNEE = "S3BucketDoesNotExistException";
const _SBN = "S3BucketName";
const _SBR = "S3BucketRegion";
const _SC = "SourceConfig";
const _SDR = "StartDashboardRefresh";
const _SDRR = "StartDashboardRefreshRequest";
const _SDRRt = "StartDashboardRefreshResponse";
const _SEDSI = "StartEventDataStoreIngestion";
const _SEDSIR = "StartEventDataStoreIngestionRequest";
const _SEDSIRt = "StartEventDataStoreIngestionResponse";
const _SEDSIRto = "StopEventDataStoreIngestionRequest";
const _SEDSIRtop = "StopEventDataStoreIngestionResponse";
const _SEDSIt = "StopEventDataStoreIngestion";
const _SET = "StartEventTime";
const _SI = "StartIngestion";
const _SIR = "StartImportRequest";
const _SIRt = "StartImportResponse";
const _SIRto = "StopImportRequest";
const _SIRtop = "StopImportResponse";
const _SIS = "S3ImportSource";
const _SIt = "StartImport";
const _SIto = "StopImport";
const _SKP = "S3KeyPrefix";
const _SL = "StartLogging";
const _SLR = "StartLoggingRequest";
const _SLRt = "StartLoggingResponse";
const _SLRto = "StopLoggingRequest";
const _SLRtop = "StopLoggingResponse";
const _SLT = "StartLoggingTime";
const _SLTt = "StopLoggingTime";
const _SLU = "S3LocationUri";
const _SLt = "StopLogging";
const _SP = "SearchPhrase";
const _SQ = "StartQuery";
const _SQEE = "ServiceQuotaExceededException";
const _SQL = "SQL";
const _SQR = "StartQueryRequest";
const _SQRt = "StartQueryResponse";
const _SR = "SearchResults";
const _SSQ = "SearchSampleQueries";
const _SSQR = "SearchSampleQueriesRequest";
const _SSQRe = "SearchSampleQueriesResponse";
const _SSQSR = "SearchSampleQueriesSearchResult";
const _SSQSRe = "SearchSampleQueriesSearchResults";
const _ST = "StartTime";
const _STARN = "SnsTopicARN";
const _STN = "SnsTopicName";
const _SW = "StartsWith";
const _S_ = "S3";
const _St = "Status";
const _T = "Templates";
const _TAEE = "TrailAlreadyExistsException";
const _TARN = "TrailARN";
const _TE = "ThrottlingException";
const _TI = "TrailInfo";
const _TL = "TagsList";
const _TLEE = "TagsLimitExceededException";
const _TLS = "TimeLoggingStarted";
const _TLSi = "TimeLoggingStopped";
const _TLr = "TrailList";
const _TN = "TrailName";
const _TNFE = "TrailNotFoundException";
const _TNPE = "TrailNotProvidedException";
const _TOD = "TimeOfDay";
const _TPE = "TerminationProtectionEnabled";
const _TRC = "TotalResultsCount";
const _Ta = "Tags";
const _Tag = "Tag";
const _Ti = "Timestamps";
const _Tr = "Trail";
const _Tra = "Trails";
const _Ty = "Type";
const _U = "Username";
const _UC = "UpdateChannel";
const _UCR = "UpdateChannelRequest";
const _UCRp = "UpdateChannelResponse";
const _UD = "UpdateDashboard";
const _UDR = "UpdateDashboardRequest";
const _UDRp = "UpdateDashboardResponse";
const _UEDS = "UpdateEventDataStore";
const _UEDSR = "UpdateEventDataStoreRequest";
const _UEDSRp = "UpdateEventDataStoreResponse";
const _UOE = "UnsupportedOperationException";
const _UT = "UpdatedTimestamp";
const _UTR = "UpdateTrailRequest";
const _UTRp = "UpdateTrailResponse";
const _UTp = "UpdateTrail";
const _Un = "Unit";
const _V = "Values";
const _VET = "ValidityEndTime";
const _VP = "ViewProperties";
const _VST = "ValidityStartTime";
const _Va = "Value";
const _W = "Widgets";
const _WL = "WidgetList";
const _Wi = "Widget";
const _aQE = "awsQueryError";
const _c = "client";
const _e = "error";
const _hE = "httpError";
const _iST = "includeShadowTrails";
const _s = "smithy.ts.sdk.synthetic.com.amazonaws.cloudtrail";
const _tL = "trailList";
const _tNL = "trailNameList";
const n0 = "com.amazonaws.cloudtrail";

// smithy-typescript generated code
import { TypeRegistry } from "@smithy/core/schema";
import type {
  StaticErrorSchema,
  StaticListSchema,
  StaticMapSchema,
  StaticOperationSchema,
  StaticStructureSchema,
} from "@smithy/types";

import { CloudTrailServiceException } from "../models/CloudTrailServiceException";
import {
  AccessDeniedException,
  AccountHasOngoingImportException,
  AccountNotFoundException,
  AccountNotRegisteredException,
  AccountRegisteredException,
  CannotDelegateManagementAccountException,
  ChannelAlreadyExistsException,
  ChannelARNInvalidException,
  ChannelExistsForEDSException,
  ChannelMaxLimitExceededException,
  ChannelNotFoundException,
  CloudTrailAccessNotEnabledException,
  CloudTrailARNInvalidException,
  CloudTrailInvalidClientTokenIdException,
  CloudWatchLogsDeliveryUnavailableException,
  ConcurrentModificationException,
  ConflictException,
  DelegatedAdminAccountLimitExceededException,
  EventDataStoreAlreadyExistsException,
  EventDataStoreARNInvalidException,
  EventDataStoreFederationEnabledException,
  EventDataStoreHasOngoingImportException,
  EventDataStoreMaxLimitExceededException,
  EventDataStoreNotFoundException,
  EventDataStoreTerminationProtectedException,
  GenerateResponseException,
  ImportNotFoundException,
  InactiveEventDataStoreException,
  InactiveQueryException,
  InsightNotEnabledException,
  InsufficientDependencyServiceAccessPermissionException,
  InsufficientEncryptionPolicyException,
  InsufficientIAMAccessPermissionException,
  InsufficientS3BucketPolicyException,
  InsufficientSnsTopicPolicyException,
  InvalidCloudWatchLogsLogGroupArnException,
  InvalidCloudWatchLogsRoleArnException,
  InvalidDateRangeException,
  InvalidEventCategoryException,
  InvalidEventDataStoreCategoryException,
  InvalidEventDataStoreStatusException,
  InvalidEventSelectorsException,
  InvalidHomeRegionException,
  InvalidImportSourceException,
  InvalidInsightSelectorsException,
  InvalidKmsKeyIdException,
  InvalidLookupAttributesException,
  InvalidMaxResultsException,
  InvalidNextTokenException,
  InvalidParameterCombinationException,
  InvalidParameterException,
  InvalidQueryStatementException,
  InvalidQueryStatusException,
  InvalidS3BucketNameException,
  InvalidS3PrefixException,
  InvalidSnsTopicNameException,
  InvalidSourceException,
  InvalidTagParameterException,
  InvalidTimeRangeException,
  InvalidTokenException,
  InvalidTrailNameException,
  KmsException,
  KmsKeyDisabledException,
  KmsKeyNotFoundException,
  MaxConcurrentQueriesException,
  MaximumNumberOfTrailsExceededException,
  NoManagementAccountSLRExistsException,
  NotOrganizationManagementAccountException,
  NotOrganizationMasterAccountException,
  OperationNotPermittedException,
  OrganizationNotInAllFeaturesModeException,
  OrganizationsNotInUseException,
  QueryIdNotFoundException,
  ResourceARNNotValidException,
  ResourceNotFoundException,
  ResourcePolicyNotFoundException,
  ResourcePolicyNotValidException,
  ResourceTypeNotSupportedException,
  S3BucketDoesNotExistException,
  ServiceQuotaExceededException,
  TagsLimitExceededException,
  ThrottlingException,
  TrailAlreadyExistsException,
  TrailNotFoundException,
  TrailNotProvidedException,
  UnsupportedOperationException,
} from "../models/errors";

/* eslint no-var: 0 */
export var AccessDeniedException$: StaticErrorSchema = [-3, n0, _ADE,
  { [_aQE]: [`ResourceAccessDenied`, 403], [_e]: _c, [_hE]: 403 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(AccessDeniedException$, AccessDeniedException);
export var AccountHasOngoingImportException$: StaticErrorSchema = [-3, n0, _AHOIE,
  { [_aQE]: [`AccountHasOngoingImport`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(AccountHasOngoingImportException$, AccountHasOngoingImportException);
export var AccountNotFoundException$: StaticErrorSchema = [-3, n0, _ANFE,
  { [_aQE]: [`AccountNotFound`, 404], [_e]: _c, [_hE]: 404 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(AccountNotFoundException$, AccountNotFoundException);
export var AccountNotRegisteredException$: StaticErrorSchema = [-3, n0, _ANRE,
  { [_aQE]: [`AccountNotRegistered`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(AccountNotRegisteredException$, AccountNotRegisteredException);
export var AccountRegisteredException$: StaticErrorSchema = [-3, n0, _ARE,
  { [_aQE]: [`AccountRegistered`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(AccountRegisteredException$, AccountRegisteredException);
export var AddTagsRequest$: StaticStructureSchema = [3, n0, _ATR,
  0,
  [_RI, _TL],
  [0, () => TagsList]
];
export var AddTagsResponse$: StaticStructureSchema = [3, n0, _ATRd,
  0,
  [],
  []
];
export var AdvancedEventSelector$: StaticStructureSchema = [3, n0, _AES,
  0,
  [_N, _FS],
  [0, () => AdvancedFieldSelectors]
];
export var AdvancedFieldSelector$: StaticStructureSchema = [3, n0, _AFS,
  0,
  [_F, _E, _SW, _EW, _NE, _NSW, _NEW],
  [0, 64 | 0, 64 | 0, 64 | 0, 64 | 0, 64 | 0, 64 | 0]
];
export var AggregationConfiguration$: StaticStructureSchema = [3, n0, _AC,
  0,
  [_T, _EC],
  [64 | 0, 0]
];
export var CancelQueryRequest$: StaticStructureSchema = [3, n0, _CQR,
  0,
  [_EDS, _QI, _EDSOAI],
  [0, 0, 0]
];
export var CancelQueryResponse$: StaticStructureSchema = [3, n0, _CQRa,
  0,
  [_QI, _QS, _EDSOAI],
  [0, 0, 0]
];
export var CannotDelegateManagementAccountException$: StaticErrorSchema = [-3, n0, _CDMAE,
  { [_aQE]: [`CannotDelegateManagementAccount`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(CannotDelegateManagementAccountException$, CannotDelegateManagementAccountException);
export var Channel$: StaticStructureSchema = [3, n0, _C,
  0,
  [_CA, _N],
  [0, 0]
];
export var ChannelAlreadyExistsException$: StaticErrorSchema = [-3, n0, _CAEE,
  { [_aQE]: [`ChannelAlreadyExists`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ChannelAlreadyExistsException$, ChannelAlreadyExistsException);
export var ChannelARNInvalidException$: StaticErrorSchema = [-3, n0, _CARNIE,
  { [_aQE]: [`ChannelARNInvalid`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ChannelARNInvalidException$, ChannelARNInvalidException);
export var ChannelExistsForEDSException$: StaticErrorSchema = [-3, n0, _CEFEDSE,
  { [_aQE]: [`ChannelExistsForEDS`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ChannelExistsForEDSException$, ChannelExistsForEDSException);
export var ChannelMaxLimitExceededException$: StaticErrorSchema = [-3, n0, _CMLEE,
  { [_aQE]: [`ChannelMaxLimitExceeded`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ChannelMaxLimitExceededException$, ChannelMaxLimitExceededException);
export var ChannelNotFoundException$: StaticErrorSchema = [-3, n0, _CNFE,
  { [_aQE]: [`ChannelNotFound`, 404], [_e]: _c, [_hE]: 404 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ChannelNotFoundException$, ChannelNotFoundException);
export var CloudTrailAccessNotEnabledException$: StaticErrorSchema = [-3, n0, _CTANEE,
  { [_aQE]: [`CloudTrailAccessNotEnabled`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(CloudTrailAccessNotEnabledException$, CloudTrailAccessNotEnabledException);
export var CloudTrailARNInvalidException$: StaticErrorSchema = [-3, n0, _CTARNIE,
  { [_aQE]: [`CloudTrailARNInvalid`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(CloudTrailARNInvalidException$, CloudTrailARNInvalidException);
export var CloudTrailInvalidClientTokenIdException$: StaticErrorSchema = [-3, n0, _CTICTIE,
  { [_aQE]: [`CloudTrailInvalidClientTokenId`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(CloudTrailInvalidClientTokenIdException$, CloudTrailInvalidClientTokenIdException);
export var CloudWatchLogsDeliveryUnavailableException$: StaticErrorSchema = [-3, n0, _CWLDUE,
  { [_aQE]: [`CloudWatchLogsDeliveryUnavailable`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(CloudWatchLogsDeliveryUnavailableException$, CloudWatchLogsDeliveryUnavailableException);
export var ConcurrentModificationException$: StaticErrorSchema = [-3, n0, _CME,
  { [_aQE]: [`ConcurrentModification`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ConcurrentModificationException$, ConcurrentModificationException);
export var ConflictException$: StaticErrorSchema = [-3, n0, _CE,
  { [_aQE]: [`ConflictException`, 409], [_e]: _c, [_hE]: 409 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ConflictException$, ConflictException);
export var ContextKeySelector$: StaticStructureSchema = [3, n0, _CKS,
  0,
  [_Ty, _E],
  [0, 64 | 0]
];
export var CreateChannelRequest$: StaticStructureSchema = [3, n0, _CCR,
  0,
  [_N, _S, _D, _Ta],
  [0, 0, () => Destinations, () => TagsList]
];
export var CreateChannelResponse$: StaticStructureSchema = [3, n0, _CCRr,
  0,
  [_CA, _N, _S, _D, _Ta],
  [0, 0, 0, () => Destinations, () => TagsList]
];
export var CreateDashboardRequest$: StaticStructureSchema = [3, n0, _CDR,
  0,
  [_N, _RS, _TL, _TPE, _W],
  [0, () => RefreshSchedule$, () => TagsList, 2, () => RequestWidgetList]
];
export var CreateDashboardResponse$: StaticStructureSchema = [3, n0, _CDRr,
  0,
  [_DA, _N, _Ty, _W, _TL, _RS, _TPE],
  [0, 0, 0, () => WidgetList, () => TagsList, () => RefreshSchedule$, 2]
];
export var CreateEventDataStoreRequest$: StaticStructureSchema = [3, n0, _CEDSR,
  0,
  [_N, _AESd, _MRE, _OE, _RP, _TPE, _TL, _KKI, _SI, _BM],
  [0, () => AdvancedEventSelectors, 2, 2, 1, 2, () => TagsList, 0, 2, 0]
];
export var CreateEventDataStoreResponse$: StaticStructureSchema = [3, n0, _CEDSRr,
  0,
  [_EDSA, _N, _St, _AESd, _MRE, _OE, _RP, _TPE, _TL, _CT, _UT, _KKI, _BM],
  [0, 0, 0, () => AdvancedEventSelectors, 2, 2, 1, 2, () => TagsList, 4, 4, 0, 0]
];
export var CreateTrailRequest$: StaticStructureSchema = [3, n0, _CTR,
  0,
  [_N, _SBN, _SKP, _STN, _IGSE, _IMRT, _ELFV, _CWLLGA, _CWLRA, _KKI, _IOT, _TL],
  [0, 0, 0, 0, 2, 2, 2, 0, 0, 0, 2, () => TagsList]
];
export var CreateTrailResponse$: StaticStructureSchema = [3, n0, _CTRr,
  0,
  [_N, _SBN, _SKP, _STN, _STARN, _IGSE, _IMRT, _TARN, _LFVE, _CWLLGA, _CWLRA, _KKI, _IOT],
  [0, 0, 0, 0, 0, 2, 2, 0, 2, 0, 0, 0, 2]
];
export var DashboardDetail$: StaticStructureSchema = [3, n0, _DD,
  0,
  [_DA, _Ty],
  [0, 0]
];
export var DataResource$: StaticStructureSchema = [3, n0, _DR,
  0,
  [_Ty, _V],
  [0, 64 | 0]
];
export var DelegatedAdminAccountLimitExceededException$: StaticErrorSchema = [-3, n0, _DAALEE,
  { [_aQE]: [`DelegatedAdminAccountLimitExceeded`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(DelegatedAdminAccountLimitExceededException$, DelegatedAdminAccountLimitExceededException);
export var DeleteChannelRequest$: StaticStructureSchema = [3, n0, _DCR,
  0,
  [_C],
  [0]
];
export var DeleteChannelResponse$: StaticStructureSchema = [3, n0, _DCRe,
  0,
  [],
  []
];
export var DeleteDashboardRequest$: StaticStructureSchema = [3, n0, _DDR,
  0,
  [_DI],
  [0]
];
export var DeleteDashboardResponse$: StaticStructureSchema = [3, n0, _DDRe,
  0,
  [],
  []
];
export var DeleteEventDataStoreRequest$: StaticStructureSchema = [3, n0, _DEDSR,
  0,
  [_EDS],
  [0]
];
export var DeleteEventDataStoreResponse$: StaticStructureSchema = [3, n0, _DEDSRe,
  0,
  [],
  []
];
export var DeleteResourcePolicyRequest$: StaticStructureSchema = [3, n0, _DRPR,
  0,
  [_RA],
  [0]
];
export var DeleteResourcePolicyResponse$: StaticStructureSchema = [3, n0, _DRPRe,
  0,
  [],
  []
];
export var DeleteTrailRequest$: StaticStructureSchema = [3, n0, _DTR,
  0,
  [_N],
  [0]
];
export var DeleteTrailResponse$: StaticStructureSchema = [3, n0, _DTRe,
  0,
  [],
  []
];
export var DeregisterOrganizationDelegatedAdminRequest$: StaticStructureSchema = [3, n0, _DODAR,
  0,
  [_DAAI],
  [0]
];
export var DeregisterOrganizationDelegatedAdminResponse$: StaticStructureSchema = [3, n0, _DODARe,
  0,
  [],
  []
];
export var DescribeQueryRequest$: StaticStructureSchema = [3, n0, _DQR,
  0,
  [_EDS, _QI, _QA, _RIe, _EDSOAI],
  [0, 0, 0, 0, 0]
];
export var DescribeQueryResponse$: StaticStructureSchema = [3, n0, _DQRe,
  0,
  [_QI, _QSu, _QS, _QSue, _EM, _DSU, _DS, _P, _EDSOAI],
  [0, 0, 0, () => QueryStatisticsForDescribeQuery$, 0, 0, 0, 0, 0]
];
export var DescribeTrailsRequest$: StaticStructureSchema = [3, n0, _DTRes,
  0,
  [_tNL, _iST],
  [64 | 0, 2]
];
export var DescribeTrailsResponse$: StaticStructureSchema = [3, n0, _DTResc,
  0,
  [_tL],
  [() => TrailList]
];
export var Destination$: StaticStructureSchema = [3, n0, _De,
  0,
  [_Ty, _L],
  [0, 0]
];
export var DisableFederationRequest$: StaticStructureSchema = [3, n0, _DFR,
  0,
  [_EDS],
  [0]
];
export var DisableFederationResponse$: StaticStructureSchema = [3, n0, _DFRi,
  0,
  [_EDSA, _FSe],
  [0, 0]
];
export var EnableFederationRequest$: StaticStructureSchema = [3, n0, _EFR,
  0,
  [_EDS, _FRA],
  [0, 0]
];
export var EnableFederationResponse$: StaticStructureSchema = [3, n0, _EFRn,
  0,
  [_EDSA, _FSe, _FRA],
  [0, 0, 0]
];
export var Event$: StaticStructureSchema = [3, n0, _Ev,
  0,
  [_EI, _EN, _RO, _AKI, _ET, _ES, _U, _R, _CTE],
  [0, 0, 0, 0, 4, 0, 0, () => ResourceList, 0]
];
export var EventDataStore$: StaticStructureSchema = [3, n0, _EDS,
  0,
  [_EDSA, _N, _TPE, _St, _AESd, _MRE, _OE, _RP, _CT, _UT],
  [0, 0, 2, 0, () => AdvancedEventSelectors, 2, 2, 1, 4, 4]
];
export var EventDataStoreAlreadyExistsException$: StaticErrorSchema = [-3, n0, _EDSAEE,
  { [_aQE]: [`EventDataStoreAlreadyExists`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(EventDataStoreAlreadyExistsException$, EventDataStoreAlreadyExistsException);
export var EventDataStoreARNInvalidException$: StaticErrorSchema = [-3, n0, _EDSARNIE,
  { [_aQE]: [`EventDataStoreARNInvalid`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(EventDataStoreARNInvalidException$, EventDataStoreARNInvalidException);
export var EventDataStoreFederationEnabledException$: StaticErrorSchema = [-3, n0, _EDSFEE,
  { [_aQE]: [`EventDataStoreFederationEnabled`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(EventDataStoreFederationEnabledException$, EventDataStoreFederationEnabledException);
export var EventDataStoreHasOngoingImportException$: StaticErrorSchema = [-3, n0, _EDSHOIE,
  { [_aQE]: [`EventDataStoreHasOngoingImport`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(EventDataStoreHasOngoingImportException$, EventDataStoreHasOngoingImportException);
export var EventDataStoreMaxLimitExceededException$: StaticErrorSchema = [-3, n0, _EDSMLEE,
  { [_aQE]: [`EventDataStoreMaxLimitExceeded`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(EventDataStoreMaxLimitExceededException$, EventDataStoreMaxLimitExceededException);
export var EventDataStoreNotFoundException$: StaticErrorSchema = [-3, n0, _EDSNFE,
  { [_aQE]: [`EventDataStoreNotFound`, 404], [_e]: _c, [_hE]: 404 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(EventDataStoreNotFoundException$, EventDataStoreNotFoundException);
export var EventDataStoreTerminationProtectedException$: StaticErrorSchema = [-3, n0, _EDSTPE,
  { [_aQE]: [`EventDataStoreTerminationProtectedException`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(EventDataStoreTerminationProtectedException$, EventDataStoreTerminationProtectedException);
export var EventSelector$: StaticStructureSchema = [3, n0, _ESv,
  0,
  [_RWT, _IME, _DRa, _EMES],
  [0, 2, () => DataResources, 64 | 0]
];
export var GenerateQueryRequest$: StaticStructureSchema = [3, n0, _GQR,
  0,
  [_EDSv, _P],
  [64 | 0, 0]
];
export var GenerateQueryResponse$: StaticStructureSchema = [3, n0, _GQRe,
  0,
  [_QSuer, _QA, _EDSOAI],
  [0, 0, 0]
];
export var GenerateResponseException$: StaticErrorSchema = [-3, n0, _GRE,
  { [_aQE]: [`GenerateResponse`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(GenerateResponseException$, GenerateResponseException);
export var GetChannelRequest$: StaticStructureSchema = [3, n0, _GCR,
  0,
  [_C],
  [0]
];
export var GetChannelResponse$: StaticStructureSchema = [3, n0, _GCRe,
  0,
  [_CA, _N, _S, _SC, _D, _IS],
  [0, 0, 0, () => SourceConfig$, () => Destinations, () => IngestionStatus$]
];
export var GetDashboardRequest$: StaticStructureSchema = [3, n0, _GDR,
  0,
  [_DI],
  [0]
];
export var GetDashboardResponse$: StaticStructureSchema = [3, n0, _GDRe,
  0,
  [_DA, _Ty, _St, _W, _RS, _CT, _UT, _LRI, _LRFR, _TPE],
  [0, 0, 0, () => WidgetList, () => RefreshSchedule$, 4, 4, 0, 0, 2]
];
export var GetEventConfigurationRequest$: StaticStructureSchema = [3, n0, _GECR,
  0,
  [_TN, _EDS],
  [0, 0]
];
export var GetEventConfigurationResponse$: StaticStructureSchema = [3, n0, _GECRe,
  0,
  [_TARN, _EDSA, _MES, _CKSo, _ACg],
  [0, 0, 0, () => ContextKeySelectors, () => AggregationConfigurations]
];
export var GetEventDataStoreRequest$: StaticStructureSchema = [3, n0, _GEDSR,
  0,
  [_EDS],
  [0]
];
export var GetEventDataStoreResponse$: StaticStructureSchema = [3, n0, _GEDSRe,
  0,
  [_EDSA, _N, _St, _AESd, _MRE, _OE, _RP, _TPE, _CT, _UT, _KKI, _BM, _FSe, _FRA, _PK],
  [0, 0, 0, () => AdvancedEventSelectors, 2, 2, 1, 2, 4, 4, 0, 0, 0, 0, () => PartitionKeyList]
];
export var GetEventSelectorsRequest$: StaticStructureSchema = [3, n0, _GESR,
  0,
  [_TN],
  [0]
];
export var GetEventSelectorsResponse$: StaticStructureSchema = [3, n0, _GESRe,
  0,
  [_TARN, _ESve, _AESd],
  [0, () => EventSelectors, () => AdvancedEventSelectors]
];
export var GetImportRequest$: StaticStructureSchema = [3, n0, _GIR,
  0,
  [_II],
  [0]
];
export var GetImportResponse$: StaticStructureSchema = [3, n0, _GIRe,
  0,
  [_II, _D, _ISm, _SET, _EET, _ISmp, _CT, _UT, _ISmpo],
  [0, 64 | 0, () => ImportSource$, 4, 4, 0, 4, 4, () => ImportStatistics$]
];
export var GetInsightSelectorsRequest$: StaticStructureSchema = [3, n0, _GISR,
  0,
  [_TN, _EDS],
  [0, 0]
];
export var GetInsightSelectorsResponse$: StaticStructureSchema = [3, n0, _GISRe,
  0,
  [_TARN, _ISn, _EDSA, _ID],
  [0, () => InsightSelectors, 0, 0]
];
export var GetQueryResultsRequest$: StaticStructureSchema = [3, n0, _GQRR,
  0,
  [_EDS, _QI, _NT, _MQR, _EDSOAI],
  [0, 0, 0, 1, 0]
];
export var GetQueryResultsResponse$: StaticStructureSchema = [3, n0, _GQRRe,
  0,
  [_QS, _QSue, _QRR, _NT, _EM],
  [0, () => QueryStatistics$, [1, n0, _QRR, 0, [1, n0, _QRRu, 0, 128 | 0]], 0, 0]
];
export var GetResourcePolicyRequest$: StaticStructureSchema = [3, n0, _GRPR,
  0,
  [_RA],
  [0]
];
export var GetResourcePolicyResponse$: StaticStructureSchema = [3, n0, _GRPRe,
  0,
  [_RA, _RPe, _DARP],
  [0, 0, 0]
];
export var GetTrailRequest$: StaticStructureSchema = [3, n0, _GTR,
  0,
  [_N],
  [0]
];
export var GetTrailResponse$: StaticStructureSchema = [3, n0, _GTRe,
  0,
  [_Tr],
  [() => Trail$]
];
export var GetTrailStatusRequest$: StaticStructureSchema = [3, n0, _GTSR,
  0,
  [_N],
  [0]
];
export var GetTrailStatusResponse$: StaticStructureSchema = [3, n0, _GTSRe,
  0,
  [_IL, _LDE, _LNE, _LDT, _LNT, _SLT, _SLTt, _LCWLDE, _LCWLDT, _LDDT, _LDDE, _LDAT, _LNAT, _LNAS, _LDAS, _TLS, _TLSi],
  [2, 0, 0, 4, 4, 4, 4, 0, 4, 4, 0, 0, 0, 0, 0, 0, 0]
];
export var ImportFailureListItem$: StaticStructureSchema = [3, n0, _IFLI,
  0,
  [_L, _St, _ETr, _EM, _LUT],
  [0, 0, 0, 0, 4]
];
export var ImportNotFoundException$: StaticErrorSchema = [-3, n0, _INFE,
  { [_aQE]: [`ImportNotFound`, 404], [_e]: _c, [_hE]: 404 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ImportNotFoundException$, ImportNotFoundException);
export var ImportsListItem$: StaticStructureSchema = [3, n0, _ILI,
  0,
  [_II, _ISmp, _D, _CT, _UT],
  [0, 0, 64 | 0, 4, 4]
];
export var ImportSource$: StaticStructureSchema = [3, n0, _ISm,
  0,
  [_S_],
  [() => S3ImportSource$]
];
export var ImportStatistics$: StaticStructureSchema = [3, n0, _ISmpo,
  0,
  [_PF, _PC, _FC, _ECv, _FE],
  [1, 1, 1, 1, 1]
];
export var InactiveEventDataStoreException$: StaticErrorSchema = [-3, n0, _IEDSE,
  { [_aQE]: [`InactiveEventDataStore`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InactiveEventDataStoreException$, InactiveEventDataStoreException);
export var InactiveQueryException$: StaticErrorSchema = [-3, n0, _IQE,
  { [_aQE]: [`InactiveQuery`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InactiveQueryException$, InactiveQueryException);
export var IngestionStatus$: StaticStructureSchema = [3, n0, _IS,
  0,
  [_LIST, _LISEID, _LIEC, _LIAT, _LIAEID],
  [4, 0, 0, 4, 0]
];
export var InsightNotEnabledException$: StaticErrorSchema = [-3, n0, _INEE,
  { [_aQE]: [`InsightNotEnabled`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InsightNotEnabledException$, InsightNotEnabledException);
export var InsightSelector$: StaticStructureSchema = [3, n0, _ISns,
  0,
  [_IT, _ECve],
  [0, 64 | 0]
];
export var InsufficientDependencyServiceAccessPermissionException$: StaticErrorSchema = [-3, n0, _IDSAPE,
  { [_aQE]: [`InsufficientDependencyServiceAccessPermission`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InsufficientDependencyServiceAccessPermissionException$, InsufficientDependencyServiceAccessPermissionException);
export var InsufficientEncryptionPolicyException$: StaticErrorSchema = [-3, n0, _IEPE,
  { [_aQE]: [`InsufficientEncryptionPolicy`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InsufficientEncryptionPolicyException$, InsufficientEncryptionPolicyException);
export var InsufficientIAMAccessPermissionException$: StaticErrorSchema = [-3, n0, _IIAMAPE,
  { [_aQE]: [`InsufficientIAMAccessPermission`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InsufficientIAMAccessPermissionException$, InsufficientIAMAccessPermissionException);
export var InsufficientS3BucketPolicyException$: StaticErrorSchema = [-3, n0, _ISBPE,
  { [_aQE]: [`InsufficientS3BucketPolicy`, 403], [_e]: _c, [_hE]: 403 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InsufficientS3BucketPolicyException$, InsufficientS3BucketPolicyException);
export var InsufficientSnsTopicPolicyException$: StaticErrorSchema = [-3, n0, _ISTPE,
  { [_aQE]: [`InsufficientSnsTopicPolicy`, 403], [_e]: _c, [_hE]: 403 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InsufficientSnsTopicPolicyException$, InsufficientSnsTopicPolicyException);
export var InvalidCloudWatchLogsLogGroupArnException$: StaticErrorSchema = [-3, n0, _ICWLLGAE,
  { [_aQE]: [`InvalidCloudWatchLogsLogGroupArn`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidCloudWatchLogsLogGroupArnException$, InvalidCloudWatchLogsLogGroupArnException);
export var InvalidCloudWatchLogsRoleArnException$: StaticErrorSchema = [-3, n0, _ICWLRAE,
  { [_aQE]: [`InvalidCloudWatchLogsRoleArn`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidCloudWatchLogsRoleArnException$, InvalidCloudWatchLogsRoleArnException);
export var InvalidDateRangeException$: StaticErrorSchema = [-3, n0, _IDRE,
  { [_aQE]: [`InvalidDateRange`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidDateRangeException$, InvalidDateRangeException);
export var InvalidEventCategoryException$: StaticErrorSchema = [-3, n0, _IECE,
  { [_aQE]: [`InvalidEventCategory`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidEventCategoryException$, InvalidEventCategoryException);
export var InvalidEventDataStoreCategoryException$: StaticErrorSchema = [-3, n0, _IEDSCE,
  { [_aQE]: [`InvalidEventDataStoreCategory`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidEventDataStoreCategoryException$, InvalidEventDataStoreCategoryException);
export var InvalidEventDataStoreStatusException$: StaticErrorSchema = [-3, n0, _IEDSSE,
  { [_aQE]: [`InvalidEventDataStoreStatus`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidEventDataStoreStatusException$, InvalidEventDataStoreStatusException);
export var InvalidEventSelectorsException$: StaticErrorSchema = [-3, n0, _IESE,
  { [_aQE]: [`InvalidEventSelectors`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidEventSelectorsException$, InvalidEventSelectorsException);
export var InvalidHomeRegionException$: StaticErrorSchema = [-3, n0, _IHRE,
  { [_aQE]: [`InvalidHomeRegion`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidHomeRegionException$, InvalidHomeRegionException);
export var InvalidImportSourceException$: StaticErrorSchema = [-3, n0, _IISE,
  { [_aQE]: [`InvalidImportSource`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidImportSourceException$, InvalidImportSourceException);
export var InvalidInsightSelectorsException$: StaticErrorSchema = [-3, n0, _IISEn,
  { [_aQE]: [`InvalidInsightSelectors`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidInsightSelectorsException$, InvalidInsightSelectorsException);
export var InvalidKmsKeyIdException$: StaticErrorSchema = [-3, n0, _IKKIE,
  { [_aQE]: [`InvalidKmsKeyId`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidKmsKeyIdException$, InvalidKmsKeyIdException);
export var InvalidLookupAttributesException$: StaticErrorSchema = [-3, n0, _ILAE,
  { [_aQE]: [`InvalidLookupAttributes`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidLookupAttributesException$, InvalidLookupAttributesException);
export var InvalidMaxResultsException$: StaticErrorSchema = [-3, n0, _IMRE,
  { [_aQE]: [`InvalidMaxResults`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidMaxResultsException$, InvalidMaxResultsException);
export var InvalidNextTokenException$: StaticErrorSchema = [-3, n0, _INTE,
  { [_aQE]: [`InvalidNextToken`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidNextTokenException$, InvalidNextTokenException);
export var InvalidParameterCombinationException$: StaticErrorSchema = [-3, n0, _IPCE,
  { [_aQE]: [`InvalidParameterCombinationError`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidParameterCombinationException$, InvalidParameterCombinationException);
export var InvalidParameterException$: StaticErrorSchema = [-3, n0, _IPE,
  { [_aQE]: [`InvalidParameter`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidParameterException$, InvalidParameterException);
export var InvalidQueryStatementException$: StaticErrorSchema = [-3, n0, _IQSE,
  { [_aQE]: [`InvalidQueryStatement`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidQueryStatementException$, InvalidQueryStatementException);
export var InvalidQueryStatusException$: StaticErrorSchema = [-3, n0, _IQSEn,
  { [_aQE]: [`InvalidQueryStatus`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidQueryStatusException$, InvalidQueryStatusException);
export var InvalidS3BucketNameException$: StaticErrorSchema = [-3, n0, _ISBNE,
  { [_aQE]: [`InvalidS3BucketName`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidS3BucketNameException$, InvalidS3BucketNameException);
export var InvalidS3PrefixException$: StaticErrorSchema = [-3, n0, _ISPE,
  { [_aQE]: [`InvalidS3Prefix`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidS3PrefixException$, InvalidS3PrefixException);
export var InvalidSnsTopicNameException$: StaticErrorSchema = [-3, n0, _ISTNE,
  { [_aQE]: [`InvalidSnsTopicName`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidSnsTopicNameException$, InvalidSnsTopicNameException);
export var InvalidSourceException$: StaticErrorSchema = [-3, n0, _ISE,
  { [_aQE]: [`InvalidSource`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidSourceException$, InvalidSourceException);
export var InvalidTagParameterException$: StaticErrorSchema = [-3, n0, _ITPE,
  { [_aQE]: [`InvalidTagParameter`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidTagParameterException$, InvalidTagParameterException);
export var InvalidTimeRangeException$: StaticErrorSchema = [-3, n0, _ITRE,
  { [_aQE]: [`InvalidTimeRange`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidTimeRangeException$, InvalidTimeRangeException);
export var InvalidTokenException$: StaticErrorSchema = [-3, n0, _ITE,
  { [_aQE]: [`InvalidToken`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidTokenException$, InvalidTokenException);
export var InvalidTrailNameException$: StaticErrorSchema = [-3, n0, _ITNE,
  { [_aQE]: [`InvalidTrailName`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(InvalidTrailNameException$, InvalidTrailNameException);
export var KmsException$: StaticErrorSchema = [-3, n0, _KE,
  { [_aQE]: [`KmsException`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(KmsException$, KmsException);
export var KmsKeyDisabledException$: StaticErrorSchema = [-3, n0, _KKDE,
  { [_aQE]: [`KmsKeyDisabled`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(KmsKeyDisabledException$, KmsKeyDisabledException);
export var KmsKeyNotFoundException$: StaticErrorSchema = [-3, n0, _KKNFE,
  { [_aQE]: [`KmsKeyNotFound`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(KmsKeyNotFoundException$, KmsKeyNotFoundException);
export var ListChannelsRequest$: StaticStructureSchema = [3, n0, _LCR,
  0,
  [_MR, _NT],
  [1, 0]
];
export var ListChannelsResponse$: StaticStructureSchema = [3, n0, _LCRi,
  0,
  [_Ch, _NT],
  [() => Channels, 0]
];
export var ListDashboardsRequest$: StaticStructureSchema = [3, n0, _LDR,
  0,
  [_NP, _Ty, _NT, _MR],
  [0, 0, 0, 1]
];
export var ListDashboardsResponse$: StaticStructureSchema = [3, n0, _LDRi,
  0,
  [_Da, _NT],
  [() => Dashboards, 0]
];
export var ListEventDataStoresRequest$: StaticStructureSchema = [3, n0, _LEDSR,
  0,
  [_NT, _MR],
  [0, 1]
];
export var ListEventDataStoresResponse$: StaticStructureSchema = [3, n0, _LEDSRi,
  0,
  [_EDSv, _NT],
  [() => EventDataStores, 0]
];
export var ListImportFailuresRequest$: StaticStructureSchema = [3, n0, _LIFR,
  0,
  [_II, _MR, _NT],
  [0, 1, 0]
];
export var ListImportFailuresResponse$: StaticStructureSchema = [3, n0, _LIFRi,
  0,
  [_Fa, _NT],
  [() => ImportFailureList, 0]
];
export var ListImportsRequest$: StaticStructureSchema = [3, n0, _LIR,
  0,
  [_MR, _De, _ISmp, _NT],
  [1, 0, 0, 0]
];
export var ListImportsResponse$: StaticStructureSchema = [3, n0, _LIRi,
  0,
  [_I, _NT],
  [() => ImportsList, 0]
];
export var ListInsightsDataRequest$: StaticStructureSchema = [3, n0, _LIDR,
  0,
  [_ISnsi, _DT, _Di, _ST, _ETn, _MR, _NT],
  [0, 0, 128 | 0, 4, 4, 1, 0]
];
export var ListInsightsDataResponse$: StaticStructureSchema = [3, n0, _LIDRi,
  0,
  [_Eve, _NT],
  [() => EventsList, 0]
];
export var ListInsightsMetricDataRequest$: StaticStructureSchema = [3, n0, _LIMDR,
  0,
  [_TN, _ES, _EN, _IT, _ECr, _ST, _ETn, _Pe, _DT, _MR, _NT],
  [0, 0, 0, 0, 0, 4, 4, 1, 0, 1, 0]
];
export var ListInsightsMetricDataResponse$: StaticStructureSchema = [3, n0, _LIMDRi,
  0,
  [_TARN, _ES, _EN, _IT, _ECr, _Ti, _V, _NT],
  [0, 0, 0, 0, 0, 64 | 4, 64 | 1, 0]
];
export var ListPublicKeysRequest$: StaticStructureSchema = [3, n0, _LPKR,
  0,
  [_ST, _ETn, _NT],
  [4, 4, 0]
];
export var ListPublicKeysResponse$: StaticStructureSchema = [3, n0, _LPKRi,
  0,
  [_PKL, _NT],
  [() => PublicKeyList, 0]
];
export var ListQueriesRequest$: StaticStructureSchema = [3, n0, _LQR,
  0,
  [_EDS, _NT, _MR, _ST, _ETn, _QS],
  [0, 0, 1, 4, 4, 0]
];
export var ListQueriesResponse$: StaticStructureSchema = [3, n0, _LQRi,
  0,
  [_Q, _NT],
  [() => Queries, 0]
];
export var ListTagsRequest$: StaticStructureSchema = [3, n0, _LTR,
  0,
  [_RIL, _NT],
  [64 | 0, 0]
];
export var ListTagsResponse$: StaticStructureSchema = [3, n0, _LTRi,
  0,
  [_RTL, _NT],
  [() => ResourceTagList, 0]
];
export var ListTrailsRequest$: StaticStructureSchema = [3, n0, _LTRis,
  0,
  [_NT],
  [0]
];
export var ListTrailsResponse$: StaticStructureSchema = [3, n0, _LTRist,
  0,
  [_Tra, _NT],
  [() => Trails, 0]
];
export var LookupAttribute$: StaticStructureSchema = [3, n0, _LA,
  0,
  [_AK, _AV],
  [0, 0]
];
export var LookupEventsRequest$: StaticStructureSchema = [3, n0, _LER,
  0,
  [_LAo, _ST, _ETn, _EC, _MR, _NT],
  [() => LookupAttributesList, 4, 4, 0, 1, 0]
];
export var LookupEventsResponse$: StaticStructureSchema = [3, n0, _LERo,
  0,
  [_Eve, _NT],
  [() => EventsList, 0]
];
export var MaxConcurrentQueriesException$: StaticErrorSchema = [-3, n0, _MCQE,
  { [_aQE]: [`MaxConcurrentQueries`, 429], [_e]: _c, [_hE]: 429 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(MaxConcurrentQueriesException$, MaxConcurrentQueriesException);
export var MaximumNumberOfTrailsExceededException$: StaticErrorSchema = [-3, n0, _MNOTEE,
  { [_aQE]: [`MaximumNumberOfTrailsExceeded`, 403], [_e]: _c, [_hE]: 403 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(MaximumNumberOfTrailsExceededException$, MaximumNumberOfTrailsExceededException);
export var NoManagementAccountSLRExistsException$: StaticErrorSchema = [-3, n0, _NMASLREE,
  { [_aQE]: [`NoManagementAccountSLRExists`, 403], [_e]: _c, [_hE]: 403 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(NoManagementAccountSLRExistsException$, NoManagementAccountSLRExistsException);
export var NotOrganizationManagementAccountException$: StaticErrorSchema = [-3, n0, _NOMAE,
  { [_aQE]: [`NotOrganizationManagementAccount`, 403], [_e]: _c, [_hE]: 403 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(NotOrganizationManagementAccountException$, NotOrganizationManagementAccountException);
export var NotOrganizationMasterAccountException$: StaticErrorSchema = [-3, n0, _NOMAEo,
  { [_aQE]: [`NotOrganizationMasterAccount`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(NotOrganizationMasterAccountException$, NotOrganizationMasterAccountException);
export var OperationNotPermittedException$: StaticErrorSchema = [-3, n0, _ONPE,
  { [_aQE]: [`OperationNotPermitted`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(OperationNotPermittedException$, OperationNotPermittedException);
export var OrganizationNotInAllFeaturesModeException$: StaticErrorSchema = [-3, n0, _ONIAFME,
  { [_aQE]: [`OrganizationNotInAllFeaturesMode`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(OrganizationNotInAllFeaturesModeException$, OrganizationNotInAllFeaturesModeException);
export var OrganizationsNotInUseException$: StaticErrorSchema = [-3, n0, _ONIUE,
  { [_aQE]: [`OrganizationsNotInUse`, 404], [_e]: _c, [_hE]: 404 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(OrganizationsNotInUseException$, OrganizationsNotInUseException);
export var PartitionKey$: StaticStructureSchema = [3, n0, _PKa,
  0,
  [_N, _Ty],
  [0, 0]
];
export var PublicKey$: StaticStructureSchema = [3, n0, _PKu,
  0,
  [_Va, _VST, _VET, _Fi],
  [21, 4, 4, 0]
];
export var PutEventConfigurationRequest$: StaticStructureSchema = [3, n0, _PECR,
  0,
  [_TN, _EDS, _MES, _CKSo, _ACg],
  [0, 0, 0, () => ContextKeySelectors, () => AggregationConfigurations]
];
export var PutEventConfigurationResponse$: StaticStructureSchema = [3, n0, _PECRu,
  0,
  [_TARN, _EDSA, _MES, _CKSo, _ACg],
  [0, 0, 0, () => ContextKeySelectors, () => AggregationConfigurations]
];
export var PutEventSelectorsRequest$: StaticStructureSchema = [3, n0, _PESR,
  0,
  [_TN, _ESve, _AESd],
  [0, () => EventSelectors, () => AdvancedEventSelectors]
];
export var PutEventSelectorsResponse$: StaticStructureSchema = [3, n0, _PESRu,
  0,
  [_TARN, _ESve, _AESd],
  [0, () => EventSelectors, () => AdvancedEventSelectors]
];
export var PutInsightSelectorsRequest$: StaticStructureSchema = [3, n0, _PISR,
  0,
  [_TN, _ISn, _EDS, _ID],
  [0, () => InsightSelectors, 0, 0]
];
export var PutInsightSelectorsResponse$: StaticStructureSchema = [3, n0, _PISRu,
  0,
  [_TARN, _ISn, _EDSA, _ID],
  [0, () => InsightSelectors, 0, 0]
];
export var PutResourcePolicyRequest$: StaticStructureSchema = [3, n0, _PRPR,
  0,
  [_RA, _RPe],
  [0, 0]
];
export var PutResourcePolicyResponse$: StaticStructureSchema = [3, n0, _PRPRu,
  0,
  [_RA, _RPe, _DARP],
  [0, 0, 0]
];
export var Query$: StaticStructureSchema = [3, n0, _Qu,
  0,
  [_QI, _QS, _CTr],
  [0, 0, 4]
];
export var QueryIdNotFoundException$: StaticErrorSchema = [-3, n0, _QINFE,
  { [_aQE]: [`QueryIdNotFound`, 404], [_e]: _c, [_hE]: 404 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(QueryIdNotFoundException$, QueryIdNotFoundException);
export var QueryStatistics$: StaticStructureSchema = [3, n0, _QSue,
  0,
  [_RC, _TRC, _BS],
  [1, 1, 1]
];
export var QueryStatisticsForDescribeQuery$: StaticStructureSchema = [3, n0, _QSFDQ,
  0,
  [_EMv, _ESven, _BS, _ETIM, _CTr],
  [1, 1, 1, 1, 4]
];
export var RefreshSchedule$: StaticStructureSchema = [3, n0, _RS,
  0,
  [_Fr, _St, _TOD],
  [() => RefreshScheduleFrequency$, 0, 0]
];
export var RefreshScheduleFrequency$: StaticStructureSchema = [3, n0, _RSF,
  0,
  [_Un, _Va],
  [0, 1]
];
export var RegisterOrganizationDelegatedAdminRequest$: StaticStructureSchema = [3, n0, _RODAR,
  0,
  [_MAI],
  [0]
];
export var RegisterOrganizationDelegatedAdminResponse$: StaticStructureSchema = [3, n0, _RODARe,
  0,
  [],
  []
];
export var RemoveTagsRequest$: StaticStructureSchema = [3, n0, _RTR,
  0,
  [_RI, _TL],
  [0, () => TagsList]
];
export var RemoveTagsResponse$: StaticStructureSchema = [3, n0, _RTRe,
  0,
  [],
  []
];
export var RequestWidget$: StaticStructureSchema = [3, n0, _RW,
  0,
  [_QSuer, _QP, _VP],
  [0, 64 | 0, 128 | 0]
];
export var Resource$: StaticStructureSchema = [3, n0, _Re,
  0,
  [_RT, _RN],
  [0, 0]
];
export var ResourceARNNotValidException$: StaticErrorSchema = [-3, n0, _RARNNVE,
  { [_aQE]: [`ResourceARNNotValid`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ResourceARNNotValidException$, ResourceARNNotValidException);
export var ResourceNotFoundException$: StaticErrorSchema = [-3, n0, _RNFE,
  { [_aQE]: [`ResourceNotFound`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ResourceNotFoundException$, ResourceNotFoundException);
export var ResourcePolicyNotFoundException$: StaticErrorSchema = [-3, n0, _RPNFE,
  { [_aQE]: [`ResourcePolicyNotFound`, 404], [_e]: _c, [_hE]: 404 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ResourcePolicyNotFoundException$, ResourcePolicyNotFoundException);
export var ResourcePolicyNotValidException$: StaticErrorSchema = [-3, n0, _RPNVE,
  { [_aQE]: [`ResourcePolicyNotValid`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ResourcePolicyNotValidException$, ResourcePolicyNotValidException);
export var ResourceTag$: StaticStructureSchema = [3, n0, _RTe,
  0,
  [_RI, _TL],
  [0, () => TagsList]
];
export var ResourceTypeNotSupportedException$: StaticErrorSchema = [-3, n0, _RTNSE,
  { [_aQE]: [`ResourceTypeNotSupported`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ResourceTypeNotSupportedException$, ResourceTypeNotSupportedException);
export var RestoreEventDataStoreRequest$: StaticStructureSchema = [3, n0, _REDSR,
  0,
  [_EDS],
  [0]
];
export var RestoreEventDataStoreResponse$: StaticStructureSchema = [3, n0, _REDSRe,
  0,
  [_EDSA, _N, _St, _AESd, _MRE, _OE, _RP, _TPE, _CT, _UT, _KKI, _BM],
  [0, 0, 0, () => AdvancedEventSelectors, 2, 2, 1, 2, 4, 4, 0, 0]
];
export var S3BucketDoesNotExistException$: StaticErrorSchema = [-3, n0, _SBDNEE,
  { [_aQE]: [`S3BucketDoesNotExist`, 404], [_e]: _c, [_hE]: 404 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(S3BucketDoesNotExistException$, S3BucketDoesNotExistException);
export var S3ImportSource$: StaticStructureSchema = [3, n0, _SIS,
  0,
  [_SLU, _SBR, _SBARA],
  [0, 0, 0]
];
export var SearchSampleQueriesRequest$: StaticStructureSchema = [3, n0, _SSQR,
  0,
  [_SP, _MR, _NT],
  [0, 1, 0]
];
export var SearchSampleQueriesResponse$: StaticStructureSchema = [3, n0, _SSQRe,
  0,
  [_SR, _NT],
  [() => SearchSampleQueriesSearchResults, 0]
];
export var SearchSampleQueriesSearchResult$: StaticStructureSchema = [3, n0, _SSQSR,
  0,
  [_N, _Des, _SQL, _Rel],
  [0, 0, 0, 1]
];
export var ServiceQuotaExceededException$: StaticErrorSchema = [-3, n0, _SQEE,
  { [_aQE]: [`ServiceQuotaExceeded`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ServiceQuotaExceededException$, ServiceQuotaExceededException);
export var SourceConfig$: StaticStructureSchema = [3, n0, _SC,
  0,
  [_ATAR, _AESd],
  [2, () => AdvancedEventSelectors]
];
export var StartDashboardRefreshRequest$: StaticStructureSchema = [3, n0, _SDRR,
  0,
  [_DI, _QPV],
  [0, 128 | 0]
];
export var StartDashboardRefreshResponse$: StaticStructureSchema = [3, n0, _SDRRt,
  0,
  [_RIe],
  [0]
];
export var StartEventDataStoreIngestionRequest$: StaticStructureSchema = [3, n0, _SEDSIR,
  0,
  [_EDS],
  [0]
];
export var StartEventDataStoreIngestionResponse$: StaticStructureSchema = [3, n0, _SEDSIRt,
  0,
  [],
  []
];
export var StartImportRequest$: StaticStructureSchema = [3, n0, _SIR,
  0,
  [_D, _ISm, _SET, _EET, _II],
  [64 | 0, () => ImportSource$, 4, 4, 0]
];
export var StartImportResponse$: StaticStructureSchema = [3, n0, _SIRt,
  0,
  [_II, _D, _ISm, _SET, _EET, _ISmp, _CT, _UT],
  [0, 64 | 0, () => ImportSource$, 4, 4, 0, 4, 4]
];
export var StartLoggingRequest$: StaticStructureSchema = [3, n0, _SLR,
  0,
  [_N],
  [0]
];
export var StartLoggingResponse$: StaticStructureSchema = [3, n0, _SLRt,
  0,
  [],
  []
];
export var StartQueryRequest$: StaticStructureSchema = [3, n0, _SQR,
  0,
  [_QSuer, _DSU, _QA, _QP, _EDSOAI],
  [0, 0, 0, 64 | 0, 0]
];
export var StartQueryResponse$: StaticStructureSchema = [3, n0, _SQRt,
  0,
  [_QI, _EDSOAI],
  [0, 0]
];
export var StopEventDataStoreIngestionRequest$: StaticStructureSchema = [3, n0, _SEDSIRto,
  0,
  [_EDS],
  [0]
];
export var StopEventDataStoreIngestionResponse$: StaticStructureSchema = [3, n0, _SEDSIRtop,
  0,
  [],
  []
];
export var StopImportRequest$: StaticStructureSchema = [3, n0, _SIRto,
  0,
  [_II],
  [0]
];
export var StopImportResponse$: StaticStructureSchema = [3, n0, _SIRtop,
  0,
  [_II, _ISm, _D, _ISmp, _CT, _UT, _SET, _EET, _ISmpo],
  [0, () => ImportSource$, 64 | 0, 0, 4, 4, 4, 4, () => ImportStatistics$]
];
export var StopLoggingRequest$: StaticStructureSchema = [3, n0, _SLRto,
  0,
  [_N],
  [0]
];
export var StopLoggingResponse$: StaticStructureSchema = [3, n0, _SLRtop,
  0,
  [],
  []
];
export var Tag$: StaticStructureSchema = [3, n0, _Tag,
  0,
  [_K, _Va],
  [0, 0]
];
export var TagsLimitExceededException$: StaticErrorSchema = [-3, n0, _TLEE,
  { [_aQE]: [`TagsLimitExceeded`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(TagsLimitExceededException$, TagsLimitExceededException);
export var ThrottlingException$: StaticErrorSchema = [-3, n0, _TE,
  { [_aQE]: [`ThrottlingException`, 429], [_e]: _c, [_hE]: 429 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(ThrottlingException$, ThrottlingException);
export var Trail$: StaticStructureSchema = [3, n0, _Tr,
  0,
  [_N, _SBN, _SKP, _STN, _STARN, _IGSE, _IMRT, _HR, _TARN, _LFVE, _CWLLGA, _CWLRA, _KKI, _HCES, _HIS, _IOT],
  [0, 0, 0, 0, 0, 2, 2, 0, 0, 2, 0, 0, 0, 2, 2, 2]
];
export var TrailAlreadyExistsException$: StaticErrorSchema = [-3, n0, _TAEE,
  { [_aQE]: [`TrailAlreadyExists`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(TrailAlreadyExistsException$, TrailAlreadyExistsException);
export var TrailInfo$: StaticStructureSchema = [3, n0, _TI,
  0,
  [_TARN, _N, _HR],
  [0, 0, 0]
];
export var TrailNotFoundException$: StaticErrorSchema = [-3, n0, _TNFE,
  { [_aQE]: [`TrailNotFound`, 404], [_e]: _c, [_hE]: 404 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(TrailNotFoundException$, TrailNotFoundException);
export var TrailNotProvidedException$: StaticErrorSchema = [-3, n0, _TNPE,
  { [_aQE]: [`TrailNotProvided`, 404], [_e]: _c, [_hE]: 404 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(TrailNotProvidedException$, TrailNotProvidedException);
export var UnsupportedOperationException$: StaticErrorSchema = [-3, n0, _UOE,
  { [_aQE]: [`UnsupportedOperation`, 400], [_e]: _c, [_hE]: 400 },
  [_M],
  [0]
];
TypeRegistry.for(n0).registerError(UnsupportedOperationException$, UnsupportedOperationException);
export var UpdateChannelRequest$: StaticStructureSchema = [3, n0, _UCR,
  0,
  [_C, _D, _N],
  [0, () => Destinations, 0]
];
export var UpdateChannelResponse$: StaticStructureSchema = [3, n0, _UCRp,
  0,
  [_CA, _N, _S, _D],
  [0, 0, 0, () => Destinations]
];
export var UpdateDashboardRequest$: StaticStructureSchema = [3, n0, _UDR,
  0,
  [_DI, _W, _RS, _TPE],
  [0, () => RequestWidgetList, () => RefreshSchedule$, 2]
];
export var UpdateDashboardResponse$: StaticStructureSchema = [3, n0, _UDRp,
  0,
  [_DA, _N, _Ty, _W, _RS, _TPE, _CT, _UT],
  [0, 0, 0, () => WidgetList, () => RefreshSchedule$, 2, 4, 4]
];
export var UpdateEventDataStoreRequest$: StaticStructureSchema = [3, n0, _UEDSR,
  0,
  [_EDS, _N, _AESd, _MRE, _OE, _RP, _TPE, _KKI, _BM],
  [0, 0, () => AdvancedEventSelectors, 2, 2, 1, 2, 0, 0]
];
export var UpdateEventDataStoreResponse$: StaticStructureSchema = [3, n0, _UEDSRp,
  0,
  [_EDSA, _N, _St, _AESd, _MRE, _OE, _RP, _TPE, _CT, _UT, _KKI, _BM, _FSe, _FRA],
  [0, 0, 0, () => AdvancedEventSelectors, 2, 2, 1, 2, 4, 4, 0, 0, 0, 0]
];
export var UpdateTrailRequest$: StaticStructureSchema = [3, n0, _UTR,
  0,
  [_N, _SBN, _SKP, _STN, _IGSE, _IMRT, _ELFV, _CWLLGA, _CWLRA, _KKI, _IOT],
  [0, 0, 0, 0, 2, 2, 2, 0, 0, 0, 2]
];
export var UpdateTrailResponse$: StaticStructureSchema = [3, n0, _UTRp,
  0,
  [_N, _SBN, _SKP, _STN, _STARN, _IGSE, _IMRT, _TARN, _LFVE, _CWLLGA, _CWLRA, _KKI, _IOT],
  [0, 0, 0, 0, 0, 2, 2, 0, 2, 0, 0, 0, 2]
];
export var Widget$: StaticStructureSchema = [3, n0, _Wi,
  0,
  [_QA, _QSuer, _QP, _VP],
  [0, 0, 64 | 0, 128 | 0]
];
export var CloudTrailServiceException$: StaticErrorSchema = [-3, _s, "CloudTrailServiceException", 0, [], []];
TypeRegistry.for(_s).registerError(CloudTrailServiceException$, CloudTrailServiceException);
var AdvancedEventSelectors: StaticListSchema = [1, n0, _AESd,
  0, () => AdvancedEventSelector$
];
var AdvancedFieldSelectors: StaticListSchema = [1, n0, _AFSd,
  0, () => AdvancedFieldSelector$
];
var AggregationConfigurations: StaticListSchema = [1, n0, _ACg,
  0, () => AggregationConfiguration$
];
var Channels: StaticListSchema = [1, n0, _Ch,
  0, () => Channel$
];
var ContextKeySelectors: StaticListSchema = [1, n0, _CKSo,
  0, () => ContextKeySelector$
];
var Dashboards: StaticListSchema = [1, n0, _Da,
  0, () => DashboardDetail$
];
var DataResources: StaticListSchema = [1, n0, _DRa,
  0, () => DataResource$
];
var DataResourceValues = 64 | 0;
var Destinations: StaticListSchema = [1, n0, _D,
  0, () => Destination$
];
var EventDataStoreList = 64 | 0;
var EventDataStores: StaticListSchema = [1, n0, _EDSv,
  0, () => EventDataStore$
];
var EventSelectors: StaticListSchema = [1, n0, _ESve,
  0, () => EventSelector$
];
var EventsList: StaticListSchema = [1, n0, _EL,
  0, () => Event$
];
var ExcludeManagementEventSources = 64 | 0;
var ImportDestinations = 64 | 0;
var ImportFailureList: StaticListSchema = [1, n0, _IFL,
  0, () => ImportFailureListItem$
];
var ImportsList: StaticListSchema = [1, n0, _ILm,
  0, () => ImportsListItem$
];
var InsightSelectors: StaticListSchema = [1, n0, _ISn,
  0, () => InsightSelector$
];
var InsightsMetricValues = 64 | 1;
var LookupAttributesList: StaticListSchema = [1, n0, _LAL,
  0, () => LookupAttribute$
];
var Operator = 64 | 0;
var OperatorTargetList = 64 | 0;
var PartitionKeyList: StaticListSchema = [1, n0, _PKLa,
  0, () => PartitionKey$
];
var PublicKeyList: StaticListSchema = [1, n0, _PKL,
  0, () => PublicKey$
];
var Queries: StaticListSchema = [1, n0, _Q,
  0, () => Query$
];
var QueryParameters = 64 | 0;
var QueryResultRow: StaticListSchema = [1, n0, _QRRu,
  0, 128 | 0
];
var QueryResultRows: StaticListSchema = [1, n0, _QRR,
  0, [1, n0, _QRRu, 0, 128 | 0]
];
var RequestWidgetList: StaticListSchema = [1, n0, _RWL,
  0, () => RequestWidget$
];
var ResourceIdList = 64 | 0;
var ResourceList: StaticListSchema = [1, n0, _RL,
  0, () => Resource$
];
var ResourceTagList: StaticListSchema = [1, n0, _RTL,
  0, () => ResourceTag$
];
var SearchSampleQueriesSearchResults: StaticListSchema = [1, n0, _SSQSRe,
  0, () => SearchSampleQueriesSearchResult$
];
var SourceEventCategories = 64 | 0;
var TagsList: StaticListSchema = [1, n0, _TL,
  0, () => Tag$
];
var Templates = 64 | 0;
var Timestamps = 64 | 4;
var TrailList: StaticListSchema = [1, n0, _TLr,
  0, () => Trail$
];
var TrailNameList = 64 | 0;
var Trails: StaticListSchema = [1, n0, _Tra,
  0, () => TrailInfo$
];
var WidgetList: StaticListSchema = [1, n0, _WL,
  0, () => Widget$
];
var ListInsightsDataDimensions = 128 | 0;
var QueryParameterValues = 128 | 0;
var QueryResultColumn = 128 | 0;
var ViewPropertiesMap = 128 | 0;
export var AddTags$: StaticOperationSchema = [9, n0, _AT,
  2, () => AddTagsRequest$, () => AddTagsResponse$
];
export var CancelQuery$: StaticOperationSchema = [9, n0, _CQ,
  2, () => CancelQueryRequest$, () => CancelQueryResponse$
];
export var CreateChannel$: StaticOperationSchema = [9, n0, _CC,
  0, () => CreateChannelRequest$, () => CreateChannelResponse$
];
export var CreateDashboard$: StaticOperationSchema = [9, n0, _CD,
  2, () => CreateDashboardRequest$, () => CreateDashboardResponse$
];
export var CreateEventDataStore$: StaticOperationSchema = [9, n0, _CEDS,
  0, () => CreateEventDataStoreRequest$, () => CreateEventDataStoreResponse$
];
export var CreateTrail$: StaticOperationSchema = [9, n0, _CTre,
  2, () => CreateTrailRequest$, () => CreateTrailResponse$
];
export var DeleteChannel$: StaticOperationSchema = [9, n0, _DC,
  0, () => DeleteChannelRequest$, () => DeleteChannelResponse$
];
export var DeleteDashboard$: StaticOperationSchema = [9, n0, _DDe,
  2, () => DeleteDashboardRequest$, () => DeleteDashboardResponse$
];
export var DeleteEventDataStore$: StaticOperationSchema = [9, n0, _DEDS,
  0, () => DeleteEventDataStoreRequest$, () => DeleteEventDataStoreResponse$
];
export var DeleteResourcePolicy$: StaticOperationSchema = [9, n0, _DRP,
  2, () => DeleteResourcePolicyRequest$, () => DeleteResourcePolicyResponse$
];
export var DeleteTrail$: StaticOperationSchema = [9, n0, _DTe,
  2, () => DeleteTrailRequest$, () => DeleteTrailResponse$
];
export var DeregisterOrganizationDelegatedAdmin$: StaticOperationSchema = [9, n0, _DODA,
  2, () => DeregisterOrganizationDelegatedAdminRequest$, () => DeregisterOrganizationDelegatedAdminResponse$
];
export var DescribeQuery$: StaticOperationSchema = [9, n0, _DQ,
  2, () => DescribeQueryRequest$, () => DescribeQueryResponse$
];
export var DescribeTrails$: StaticOperationSchema = [9, n0, _DTes,
  2, () => DescribeTrailsRequest$, () => DescribeTrailsResponse$
];
export var DisableFederation$: StaticOperationSchema = [9, n0, _DF,
  0, () => DisableFederationRequest$, () => DisableFederationResponse$
];
export var EnableFederation$: StaticOperationSchema = [9, n0, _EF,
  0, () => EnableFederationRequest$, () => EnableFederationResponse$
];
export var GenerateQuery$: StaticOperationSchema = [9, n0, _GQ,
  2, () => GenerateQueryRequest$, () => GenerateQueryResponse$
];
export var GetChannel$: StaticOperationSchema = [9, n0, _GC,
  2, () => GetChannelRequest$, () => GetChannelResponse$
];
export var GetDashboard$: StaticOperationSchema = [9, n0, _GD,
  2, () => GetDashboardRequest$, () => GetDashboardResponse$
];
export var GetEventConfiguration$: StaticOperationSchema = [9, n0, _GEC,
  2, () => GetEventConfigurationRequest$, () => GetEventConfigurationResponse$
];
export var GetEventDataStore$: StaticOperationSchema = [9, n0, _GEDS,
  2, () => GetEventDataStoreRequest$, () => GetEventDataStoreResponse$
];
export var GetEventSelectors$: StaticOperationSchema = [9, n0, _GES,
  2, () => GetEventSelectorsRequest$, () => GetEventSelectorsResponse$
];
export var GetImport$: StaticOperationSchema = [9, n0, _GI,
  0, () => GetImportRequest$, () => GetImportResponse$
];
export var GetInsightSelectors$: StaticOperationSchema = [9, n0, _GIS,
  2, () => GetInsightSelectorsRequest$, () => GetInsightSelectorsResponse$
];
export var GetQueryResults$: StaticOperationSchema = [9, n0, _GQRet,
  0, () => GetQueryResultsRequest$, () => GetQueryResultsResponse$
];
export var GetResourcePolicy$: StaticOperationSchema = [9, n0, _GRP,
  2, () => GetResourcePolicyRequest$, () => GetResourcePolicyResponse$
];
export var GetTrail$: StaticOperationSchema = [9, n0, _GT,
  2, () => GetTrailRequest$, () => GetTrailResponse$
];
export var GetTrailStatus$: StaticOperationSchema = [9, n0, _GTS,
  2, () => GetTrailStatusRequest$, () => GetTrailStatusResponse$
];
export var ListChannels$: StaticOperationSchema = [9, n0, _LC,
  2, () => ListChannelsRequest$, () => ListChannelsResponse$
];
export var ListDashboards$: StaticOperationSchema = [9, n0, _LD,
  2, () => ListDashboardsRequest$, () => ListDashboardsResponse$
];
export var ListEventDataStores$: StaticOperationSchema = [9, n0, _LEDS,
  2, () => ListEventDataStoresRequest$, () => ListEventDataStoresResponse$
];
export var ListImportFailures$: StaticOperationSchema = [9, n0, _LIF,
  2, () => ListImportFailuresRequest$, () => ListImportFailuresResponse$
];
export var ListImports$: StaticOperationSchema = [9, n0, _LI,
  2, () => ListImportsRequest$, () => ListImportsResponse$
];
export var ListInsightsData$: StaticOperationSchema = [9, n0, _LID,
  2, () => ListInsightsDataRequest$, () => ListInsightsDataResponse$
];
export var ListInsightsMetricData$: StaticOperationSchema = [9, n0, _LIMD,
  2, () => ListInsightsMetricDataRequest$, () => ListInsightsMetricDataResponse$
];
export var ListPublicKeys$: StaticOperationSchema = [9, n0, _LPK,
  2, () => ListPublicKeysRequest$, () => ListPublicKeysResponse$
];
export var ListQueries$: StaticOperationSchema = [9, n0, _LQ,
  2, () => ListQueriesRequest$, () => ListQueriesResponse$
];
export var ListTags$: StaticOperationSchema = [9, n0, _LT,
  2, () => ListTagsRequest$, () => ListTagsResponse$
];
export var ListTrails$: StaticOperationSchema = [9, n0, _LTi,
  2, () => ListTrailsRequest$, () => ListTrailsResponse$
];
export var LookupEvents$: StaticOperationSchema = [9, n0, _LE,
  2, () => LookupEventsRequest$, () => LookupEventsResponse$
];
export var PutEventConfiguration$: StaticOperationSchema = [9, n0, _PEC,
  2, () => PutEventConfigurationRequest$, () => PutEventConfigurationResponse$
];
export var PutEventSelectors$: StaticOperationSchema = [9, n0, _PES,
  2, () => PutEventSelectorsRequest$, () => PutEventSelectorsResponse$
];
export var PutInsightSelectors$: StaticOperationSchema = [9, n0, _PIS,
  2, () => PutInsightSelectorsRequest$, () => PutInsightSelectorsResponse$
];
export var PutResourcePolicy$: StaticOperationSchema = [9, n0, _PRP,
  2, () => PutResourcePolicyRequest$, () => PutResourcePolicyResponse$
];
export var RegisterOrganizationDelegatedAdmin$: StaticOperationSchema = [9, n0, _RODA,
  2, () => RegisterOrganizationDelegatedAdminRequest$, () => RegisterOrganizationDelegatedAdminResponse$
];
export var RemoveTags$: StaticOperationSchema = [9, n0, _RTem,
  2, () => RemoveTagsRequest$, () => RemoveTagsResponse$
];
export var RestoreEventDataStore$: StaticOperationSchema = [9, n0, _REDS,
  0, () => RestoreEventDataStoreRequest$, () => RestoreEventDataStoreResponse$
];
export var SearchSampleQueries$: StaticOperationSchema = [9, n0, _SSQ,
  2, () => SearchSampleQueriesRequest$, () => SearchSampleQueriesResponse$
];
export var StartDashboardRefresh$: StaticOperationSchema = [9, n0, _SDR,
  2, () => StartDashboardRefreshRequest$, () => StartDashboardRefreshResponse$
];
export var StartEventDataStoreIngestion$: StaticOperationSchema = [9, n0, _SEDSI,
  0, () => StartEventDataStoreIngestionRequest$, () => StartEventDataStoreIngestionResponse$
];
export var StartImport$: StaticOperationSchema = [9, n0, _SIt,
  0, () => StartImportRequest$, () => StartImportResponse$
];
export var StartLogging$: StaticOperationSchema = [9, n0, _SL,
  2, () => StartLoggingRequest$, () => StartLoggingResponse$
];
export var StartQuery$: StaticOperationSchema = [9, n0, _SQ,
  2, () => StartQueryRequest$, () => StartQueryResponse$
];
export var StopEventDataStoreIngestion$: StaticOperationSchema = [9, n0, _SEDSIt,
  0, () => StopEventDataStoreIngestionRequest$, () => StopEventDataStoreIngestionResponse$
];
export var StopImport$: StaticOperationSchema = [9, n0, _SIto,
  0, () => StopImportRequest$, () => StopImportResponse$
];
export var StopLogging$: StaticOperationSchema = [9, n0, _SLt,
  2, () => StopLoggingRequest$, () => StopLoggingResponse$
];
export var UpdateChannel$: StaticOperationSchema = [9, n0, _UC,
  2, () => UpdateChannelRequest$, () => UpdateChannelResponse$
];
export var UpdateDashboard$: StaticOperationSchema = [9, n0, _UD,
  2, () => UpdateDashboardRequest$, () => UpdateDashboardResponse$
];
export var UpdateEventDataStore$: StaticOperationSchema = [9, n0, _UEDS,
  2, () => UpdateEventDataStoreRequest$, () => UpdateEventDataStoreResponse$
];
export var UpdateTrail$: StaticOperationSchema = [9, n0, _UTp,
  2, () => UpdateTrailRequest$, () => UpdateTrailResponse$
];
