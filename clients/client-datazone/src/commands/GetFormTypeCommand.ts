...
import { GetFormTypeInput, GetFormTypeOutput, GetFormTypeOutputFilterSensitiveLog } from "../models/models_0";
...
> {
   ...
   * @public
   */
  constructor(readonly input: GetFormTypeCommandInput) {
  ...
  ): Handler<GetFormTypeCommandInput, GetFormTypeCommandOutput> {
    this.middlewareStack.use(getSerdePlugin(configuration, this.serialize, this.deserialize));
    this.middlewareStack.use(getEndpointPlugin(configuration, GetFormTypeCommand.getEndpointParameterInstructions()));

    const stack = clientStack.concat(this.middlewareStack);

    const { logger } = configuration;
    const clientName = "DataZoneClient";
    const commandName = "GetFormTypeCommand";
    const handlerExecutionContext: HandlerExecutionContext = {
      logger,
      clientName,
      commandName,
      inputFilterSensitiveLog: (_: any) => _,
      outputFilterSensitiveLog: GetFormTypeOutputFilterSensitiveLog,
      [SMITHY_CONTEXT_KEY]: {
        service: "DataZone",
   ...
   * @internal
   */
  private serialize(input: GetFormTypeCommandInput, context: __SerdeContext): Promise<__HttpRequest> {
    return se_GetFormTypeCommand(input, context);
  }

  /**
   * @internal
   */
  private deserialize(output: __HttpResponse, context: __SerdeContext): Promise<GetFormTypeCommandOutput> {
    return de_GetFormTypeCommand(output, context);
  }
