const _A = "Arn";
const _AA = "AntiAlias";
const _AAO = "AdAvailOffset";
const _AAR = "AutomatedAbrRule";
const _AAS = "AutomatedAbrSettings";
const _AB = "AvailBlanking";
const _ABI = "AvailBlankingImage";
const _ABM = "AudioBufferModel";
const _ABl = "AlphaBehavior";
const _AC = "AlgorithmControl";
const _ACA = "AuthConnectionArn";
const _ACCSIU = "AudioChannelConfigSchemeIdUri";
const _ACH = "AccessibilityCaptionHints";
const _ACR = "AssociateCertificateRequest";
const _ACRs = "AssociateCertificateResponse";
const _ACS = "AudioCodecSettings";
const _ACTS = "AudioChannelTaggingSettings";
const _ACc = "AccessControl";
const _ACs = "AssociateCertificate";
const _ACt = "AttenuationControl";
const _AD = "AudioDescription";
const _ADAC = "AudioDescriptionAudioChannel";
const _ADBM = "AudioDescriptionBroadcasterMix";
const _ADC = "AudioDurationCorrection";
const _ADDC = "AudioDescriptionDataChannel";
const _ADM = "AudioDescriptionMix";
const _ADu = "AudioDuration";
const _ADud = "AudioDeduplication";
const _ADudi = "AudioDescriptions";
const _AES = "AutomatedEncodingSettings";
const _AF = "AdiFilename";
const _AFC = "ApplyFontColor";
const _AFPP = "AudioFramesPerPes";
const _AGI = "AudioGroupId";
const _AI = "AssetId";
const _AIC = "AvcIntraClass";
const _AIF = "AdvancedInputFilter";
const _AIFS = "AdvancedInputFilterSettings";
const _AIS = "AvcIntraSettings";
const _AIUS = "AvcIntraUhdSettings";
const _AM = "AdditionalManifests";
const _AMd = "AdMarkers";
const _AMg = "AggressiveMode";
const _AN = "AssetName";
const _ANS = "AudioNormalizationSettings";
const _AOC = "AudioOnlyContainer";
const _AOH = "AudioOnlyHeader";
const _AP = "AudioProperties";
const _APCS = "AudioPitchCorrectionSettings";
const _APOD = "AudioPtsOffsetDelta";
const _APu = "AudioPids";
const _AQ = "AdaptiveQuantization";
const _AQS = "Av1QvbrSettings";
const _AR = "AspectRatio";
const _ARS = "AllowedRenditionSize";
const _ARSu = "AudioRenditionSets";
const _ARl = "AllowedRenditions";
const _AS = "AacSettings";
const _ASG = "AudioSelectorGroup";
const _ASGu = "AudioSelectorGroups";
const _ASN = "AudioSourceName";
const _ASNu = "AudioSelectorNames";
const _ASS = "AncillarySourceSettings";
const _ASb = "AbrSettings";
const _ASc = "Ac3Settings";
const _AScc = "AccelerationSettings";
const _AScce = "AccelerationStatus";
const _ASce = "Ac4Settings";
const _ASf = "AfdSignaling";
const _ASi = "AiffSettings";
const _ASu = "AudioSelector";
const _ASud = "AudioSelectors";
const _ASv = "Av1Settings";
const _AT = "AddTexture";
const _ATC = "AudioTypeControl";
const _ATFS = "AlternateTransferFunctionSei";
const _ATI = "AudioTrackIndexes";
const _ATT = "AudioTrackType";
const _ATu = "AudioType";
const _AWP = "ActiveWatermarkProcess";
const _Ac = "Accessibility";
const _Al = "Algorithm";
const _Ali = "Alignment";
const _An = "Anchor";
const _B = "Bitrate";
const _BC = "BackgroundColor";
const _BCi = "BitrateClass";
const _BCr = "BreakoutCode";
const _BD = "BitDepth";
const _BDS = "BurninDestinationSettings";
const _BL = "BaseLayer";
const _BM = "BitstreamMode";
const _BMu = "BufferModel";
const _BO = "BackgroundOpacity";
const _BPX = "BluePrimaryX";
const _BPY = "BluePrimaryY";
const _BR = "BitRate";
const _BRE = "BadRequestException";
const _BRF = "BandwidthReductionFilter";
const _BRL = "ByteRateLimit";
const _BTS = "BillingTagsSource";
const _BU = "BaseUrl";
const _Br = "Brightness";
const _C = "Channels";
const _CA = "CreatedAt";
const _CAM = "CmafAdditionalManifest";
const _CAa = "CannedAcl";
const _CAe = "CertificateArn";
const _CAl = "ClapAtom";
const _CAs = "CslgAtom";
const _CC = "ClientCache";
const _CCDLUTS = "ColorConversion3DLUTSetting";
const _CCDLUTSo = "ColorConversion3DLUTSettings";
const _CCT = "CaptionContainerType";
const _CCa = "CaptionChannel";
const _CCo = "ColorCorrector";
const _CD = "CaptionDescription";
const _CDP = "CaptionDescriptionPreset";
const _CDS = "CaptionDestinationSettings";
const _CDa = "CaptionDescriptions";
const _CE = "ConflictException";
const _CES = "CmafEncryptionSettings";
const _CFR = "CodedFrameRate";
const _CGL = "CorrectionGateLevel";
const _CGS = "CmafGroupSettings";
const _CI = "ChannelsIn";
const _CIBTPS = "CmafImageBasedTrickPlaySettings";
const _CIBTPV = "CmafImageBasedTrickPlayVariant";
const _CIV = "ConstantInitializationVector";
const _CJ = "ConcurrentJobs";
const _CJR = "CancelJobRequest";
const _CJRa = "CancelJobResponse";
const _CJRr = "CreateJobRequest";
const _CJRre = "CreateJobResponse";
const _CJT = "CreateJobTemplate";
const _CJTR = "CreateJobTemplateRequest";
const _CJTRr = "CreateJobTemplateResponse";
const _CJa = "CancelJob";
const _CJr = "CreateJob";
const _CL = "ChannelLayout";
const _CLC = "CustomLanguageCode";
const _CLL = "ContentLightLevel";
const _CLM = "CaptionLanguageMappings";
const _CLS = "ClearLeadSegments";
const _CLSa = "CaptionLanguageSetting";
const _CLl = "ClipLimits";
const _CLo = "CodecLevel";
const _CM = "CodingMode";
const _CMh = "ChannelMapping";
const _CMo = "CodecMetadata";
const _CMol = "ColorMetadata";
const _CMp = "C2paManifest";
const _CN = "ChannelName";
const _CNu = "CustomName";
const _CO = "ChannelsOut";
const _CP = "CodecProfile";
const _CPA = "CopyProtectionAction";
const _CPM = "ChromaPositionMode";
const _CPR = "CreatePresetRequest";
const _CPRr = "CreatePresetResponse";
const _CPTP = "ConvertPaintToPop";
const _CPo = "ColorPrimaries";
const _CPr = "CreatePreset";
const _CPu = "CurrentPhase";
const _CQ = "CreateQueue";
const _CQR = "CreateQueueRequest";
const _CQRr = "CreateQueueResponse";
const _CR = "ContentReference";
const _CRS = "CreateResourceShare";
const _CRSR = "CreateResourceShareRequest";
const _CRSRr = "CreateResourceShareResponse";
const _CRT = "ClientRequestToken";
const _CS = "CodecSettings";
const _CSC = "ColorSpaceConversion";
const _CSF = "CaptionSourceFramerate";
const _CSI = "CbetSourceId";
const _CSLC = "CaptionSegmentLengthControl";
const _CSN = "CaptionSelectorName";
const _CSNr = "CredentialsSecretName";
const _CSS = "CaptionSourceSettings";
const _CSU = "ColorSpaceUsage";
const _CSa = "CaptionSelector";
const _CSap = "CaptionSelectors";
const _CSe = "CertificateSecret";
const _CSh = "ChromaSubsampling";
const _CShr = "ChromaSampling";
const _CSm = "CmfcSettings";
const _CSo = "CodecSpecification";
const _CSol = "ColorSpace";
const _CSon = "ContainerSettings";
const _CT = "Convert608To708";
const _CTh = "ChannelTag";
const _CTha = "ChannelTags";
const _CV = "CttsVersion";
const _Ca = "Category";
const _Co = "Codec";
const _Cod = "Code";
const _Com = "Compatibility";
const _Comm = "Commitment";
const _Con = "Contrast";
const _Cont = "Container";
const _Contr = "Control";
const _Cou = "Count";
const _Cr = "Crop";
const _D = "Dialnorm";
const _DAM = "DashAdditionalManifest";
const _DAR = "DisplayAspectRatio";
const _DAS = "DynamicAudioSelector";
const _DASy = "DynamicAudioSelectors";
const _DC = "DurationControl";
const _DCN = "Destination608ChannelNumber";
const _DCR = "DisassociateCertificateRequest";
const _DCRi = "DisassociateCertificateResponse";
const _DCi = "DisassociateCertificate";
const _DCo = "DownmixControl";
const _DE = "DescribeEndpoints";
const _DER = "DescribeEndpointsRequest";
const _DERe = "DescribeEndpointsResponse";
const _DF = "DcFilter";
const _DFT = "DropFrameTimecode";
const _DFe = "DeblockFilter";
const _DFen = "DenoiseFilter";
const _DH = "DdsHandling";
const _DI = "DialogueIntelligence";
const _DIES = "DashIsoEncryptionSettings";
const _DIFTPNM = "DashIFrameTrickPlayNameModifier";
const _DIGS = "DashIsoGroupSettings";
const _DIIBTPS = "DashIsoImageBasedTrickPlaySettings";
const _DIIBTPV = "DashIsoImageBasedTrickPlayVariant";
const _DIM = "DurationInMs";
const _DIi = "DistributorId";
const _DJT = "DeleteJobTemplate";
const _DJTR = "DeleteJobTemplateRequest";
const _DJTRe = "DeleteJobTemplateResponse";
const _DM = "DecryptionMode";
const _DMS = "DashManifestStyle";
const _DMu = "DurationMode";
const _DNS = "DvbNitSettings";
const _DP = "DataProperties";
const _DPR = "DeletePolicyRequest";
const _DPRe = "DeletePolicyResponse";
const _DPRel = "DeletePresetRequest";
const _DPRele = "DeletePresetResponse";
const _DPTSC = "DataPTSControl";
const _DPe = "DeletePolicy";
const _DPel = "DeletePreset";
const _DQ = "DestinationQueue";
const _DQR = "DeleteQueueRequest";
const _DQRe = "DeleteQueueResponse";
const _DQe = "DeleteQueue";
const _DRC = "DynamicRangeControl";
const _DRCFPT = "DynamicRangeCompressionFlatPanelTv";
const _DRCHT = "DynamicRangeCompressionHomeTheater";
const _DRCL = "DynamicRangeCompressionLine";
const _DRCP = "DynamicRangeCompressionProfile";
const _DRCPH = "DynamicRangeCompressionPortableHeadphones";
const _DRCPS = "DynamicRangeCompressionPortableSpeakers";
const _DRCR = "DynamicRangeCompressionRf";
const _DS = "DefaultSelection";
const _DSDS = "DvbSubDestinationSettings";
const _DSG = "DynamicSubGop";
const _DSN = "Destination708ServiceNumber";
const _DSP = "DvbSubPids";
const _DSS = "DvbSdtSettings";
const _DSSI = "DashSignaledSystemIds";
const _DSSS = "DvbSubSourceSettings";
const _DSe = "DestinationSettings";
const _DSec = "DecryptionSettings";
const _DSi = "DirectoryStructure";
const _DT = "DestinationType";
const _DTI = "DataTrackIndexes";
const _DTP = "DvbTeletextPid";
const _DTS = "DvbTdtSettings";
const _DV = "DolbyVision";
const _DVLM = "DolbyVisionLevel6Metadata";
const _DVM = "DolbyVisionMetadata";
const _DVMX = "DolbyVisionMetadataXml";
const _DVSF = "DescriptiveVideoServiceFlag";
const _DXC = "DdsXCoordinate";
const _DYC = "DdsYCoordinate";
const _De = "Denominator";
const _Deb = "Deblocking";
const _Dei = "Deinterlacer";
const _Des = "Destination";
const _Desc = "Description";
const _Du = "Duration";
const _E = "Encryption";
const _EA = "ExpiresAt";
const _EAFI = "ExternalAudioFileInput";
const _EAI = "EbpAudioInterval";
const _EAS = "Eac3AtmosSettings";
const _EC = "ErrorCode";
const _ECC = "EncryptionContractConfiguration";
const _ED = "ExpirationDate";
const _EDK = "EncryptedDecryptionKey";
const _EDS = "EmbeddedDestinationSettings";
const _EDSx = "ExtendedDataServices";
const _EE = "EntropyEncoding";
const _EI = "EpisodeId";
const _EIC = "ElementalInferenceConfiguration";
const _EIF = "ElementalInferenceFeed";
const _EL = "EnhancementLayer";
const _EM = "EncryptionMethod";
const _EMCCN = "EsamManifestConfirmConditionNotification";
const _EMr = "ErrorMessage";
const _EOSM = "EndOfStreamMarkers";
const _EP = "EbpPlacement";
const _EPn = "EndPosition";
const _ERIP = "EsRateInPes";
const _ES = "Eac3Settings";
const _ESPN = "EsamSignalProcessingNotification";
const _ESS = "EmbeddedSourceSettings";
const _ESs = "EsamSettings";
const _ET = "EndTimecode";
const _ETO = "EmbeddedTimecodeOverride";
const _ETa = "ETag";
const _ETn = "EncryptionType";
const _EWP = "ExplicitWeightedPrediction";
const _En = "Endpoints";
const _End = "Endpoint";
const _Es = "Esam";
const _Ex = "Extension";
const _F = "Format";
const _FA = "FixedAfd";
const _FAQ = "FlickerAdaptiveQuantization";
const _FC = "FramerateControl";
const _FCA = "FramerateConversionAlgorithm";
const _FCS = "FrameCaptureSettings";
const _FCo = "FontColor";
const _FCr = "FrameControl";
const _FD = "FramerateDenominator";
const _FE = "ForbiddenException";
const _FEi = "FieldEncoding";
const _FEil = "FilterEnable";
const _FF = "FallbackFont";
const _FFB = "FontFileBold";
const _FFBI = "FontFileBoldItalic";
const _FFI = "FontFileItalic";
const _FFR = "FontFileRegular";
const _FGS = "FilmGrainSynthesis";
const _FGSi = "FileGroupSettings";
const _FI = "FileInput";
const _FIR = "ForceIncludeRenditions";
const _FIRS = "ForceIncludeRenditionSize";
const _FIa = "FadeIn";
const _FL = "FragmentLength";
const _FLC = "FragmentLengthControl";
const _FLi = "FilterList";
const _FMS = "FeedManagementState";
const _FN = "FramerateNumerator";
const _FO = "FontOpacity";
const _FOa = "FadeOut";
const _FOi = "FieldOrder";
const _FOil = "FileOffset";
const _FR = "FrameRate";
const _FRo = "FontResolution";
const _FS = "FlacSettings";
const _FSB = "FreeSpaceBox";
const _FSS = "FileSourceSettings";
const _FSi = "FilterStrength";
const _FSil = "FileSize";
const _FSilt = "FilterSettings";
const _FSo = "FontScript";
const _FSol = "FollowSource";
const _FSon = "FontSize";
const _FSont = "FontStyle";
const _FSv = "F4vSettings";
const _FT = "FragmentTime";
const _FTVEO = "ForceTsVideoEbpOrder";
const _FTi = "FinishTime";
const _FU = "FileUrl";
const _FW = "FontWeight";
const _Fe = "Features";
const _Fee = "Feeds";
const _Fi = "Filter";
const _Fo = "Fourcc";
const _Fr = "Framerate";
const _GBR = "GopBReference";
const _GCC = "GopClosedCadence";
const _GH = "GapHandling";
const _GJ = "GetJob";
const _GJQR = "GetJobsQueryResults";
const _GJQRR = "GetJobsQueryResultsRequest";
const _GJQRRe = "GetJobsQueryResultsResponse";
const _GJR = "GetJobRequest";
const _GJRe = "GetJobResponse";
const _GJT = "GetJobTemplate";
const _GJTR = "GetJobTemplateRequest";
const _GJTRe = "GetJobTemplateResponse";
const _GP = "GetPolicy";
const _GPR = "GetPolicyRequest";
const _GPRe = "GetPolicyResponse";
const _GPRet = "GetPresetRequest";
const _GPRetr = "GetPresetResponse";
const _GPS = "GopsPerSegment";
const _GPX = "GreenPrimaryX";
const _GPY = "GreenPrimaryY";
const _GPe = "GetPreset";
const _GQ = "GetQueue";
const _GQR = "GetQueueRequest";
const _GQRe = "GetQueueResponse";
const _GS = "GopSize";
const _GSU = "GopSizeUnits";
const _GSi = "GifSettings";
const _H = "Height";
const _HAM = "HlsAdditionalManifest";
const _HBFFP = "HrdBufferFinalFillPercentage";
const _HBIFP = "HrdBufferInitialFillPercentage";
const _HBS = "HrdBufferSize";
const _HC = "HbbtvCompliance";
const _HCLM = "HlsCaptionLanguageMapping";
const _HD = "HopDestinations";
const _HDo = "HopDestination";
const _HES = "HlsEncryptionSettings";
const _HFC = "HexFontColor";
const _HGS = "HlsGroupSettings";
const _HI = "HttpInputs";
const _HIBTPS = "HlsImageBasedTrickPlaySettings";
const _HIBTPV = "HlsImageBasedTrickPlayVariant";
const _HIP = "HeightInPx";
const _HIt = "HttpsInputs";
const _HM = "Hdr10Metadata";
const _HMd = "HdrMetadata";
const _HP = "Hdr10Plus";
const _HPP = "Hdr10PlusPresence";
const _HQS = "H264QvbrSettings";
const _HQSv = "H265QvbrSettings";
const _HRGS = "HlsRenditionGroupSettings";
const _HS = "H264Settings";
const _HSSI = "HlsSignaledSystemIds";
const _HSe = "H265Settings";
const _HSl = "HlsSettings";
const _HTSTM = "HdrToSdrToneMapper";
const _Hu = "Hue";
const _I = "Id";
const _IBTP = "ImageBasedTrickPlay";
const _IBTPS = "ImageBasedTrickPlaySettings";
const _IBTPV = "ImageBasedTrickPlayVariants";
const _IC = "IntervalCadence";
const _ICFT = "InputChannelsFineTune";
const _ICS = "InputColorSpace";
const _ICn = "InputClippings";
const _ICnp = "InputClipping";
const _ICnpu = "InputChannels";
const _IDMCD = "IntegerDurationMaximumCompressionDenominator";
const _IDMCN = "IntegerDurationMaximumCompressionNumerator";
const _IDP = "IntraDcPrecision";
const _IDS = "ImscDestinationSettings";
const _IDSn = "InputDecryptionSettings";
const _IDTTM = "IntegerDurationTrimThresholdMilliseconds";
const _IF = "InputFiles";
const _IFOM = "IFrameOnlyManifest";
const _IFn = "InputFile";
const _II = "Id3Insertion";
const _III = "ImageInserterInput";
const _IId = "Id3Insertions";
const _IIm = "ImageInserter";
const _IIma = "ImageInput";
const _IIn = "InsertableImages";
const _IIns = "InsertableImage";
const _IM = "InterlaceMode";
const _IML = "InputMasteringLuminance";
const _IMn = "InsertionMode";
const _IP = "InitialPosition";
const _ISEE = "InternalServerErrorException";
const _IST = "InputScanType";
const _IT = "InputTemplate";
const _ITS = "InputTamsSettings";
const _IV = "InitializationVector";
const _IVG = "InputVideoGenerator";
const _IVIM = "InitializationVectorInManifest";
const _IX = "ImageX";
const _IY = "ImageY";
const _Id = "Id3";
const _In = "Input";
const _Ind = "Index";
const _Inf = "Info";
const _Inp = "Inputs";
const _J = "Job";
const _JEV = "JobEngineVersion";
const _JEVR = "JobEngineVersionRequested";
const _JEVU = "JobEngineVersionUsed";
const _JI = "JobId";
const _JM = "JobMessages";
const _JPC = "JobPercentComplete";
const _JQF = "JobsQueryFilter";
const _JS = "JobSettings";
const _JT = "JobTemplate";
const _JTS = "JobTemplateSettings";
const _JTo = "JobTemplates";
const _Jo = "Jobs";
const _K = "Key";
const _KEC = "KmsEncryptionContext";
const _KF = "KeyFormat";
const _KFV = "KeyFormatVersions";
const _KKA = "KmsKeyArn";
const _KKR = "KmsKeyRegion";
const _KLI = "KantarLicenseId";
const _KM = "KlvMetadata";
const _KSU = "KantarServerUrl";
const _KW = "KantarWatermark";
const _KWS = "KantarWatermarkSettings";
const _L = "Level";
const _LB = "ListBy";
const _LC = "LanguageCode";
const _LCC = "LanguageCodeControl";
const _LCf = "LfeControl";
const _LD = "LanguageDescription";
const _LDo = "LogDestination";
const _LF = "LfeFilter";
const _LJ = "ListJobs";
const _LJR = "ListJobsRequest";
const _LJRi = "ListJobsResponse";
const _LJT = "ListJobTemplates";
const _LJTR = "ListJobTemplatesRequest";
const _LJTRi = "ListJobTemplatesResponse";
const _LL = "LoudnessLogging";
const _LM = "L6Metadata";
const _LMM = "LoudnessMeasurementMode";
const _LMa = "LastModified";
const _LMo = "L6Mode";
const _LP = "ListPresets";
const _LPR = "ListPresetsRequest";
const _LPRi = "ListPresetsResponse";
const _LQ = "ListQueues";
const _LQR = "ListQueuesRequest";
const _LQRi = "ListQueuesResponse";
const _LRCML = "LoRoCenterMixLevel";
const _LRCMLt = "LtRtCenterMixLevel";
const _LRSML = "LoRoSurroundMixLevel";
const _LRSMLt = "LtRtSurroundMixLevel";
const _LSD = "LastShareDetails";
const _LTFR = "ListTagsForResource";
const _LTFRR = "ListTagsForResourceRequest";
const _LTFRRi = "ListTagsForResourceResponse";
const _LU = "LastUpdated";
const _LV = "ListVersions";
const _LVR = "ListVersionsRequest";
const _LVRi = "ListVersionsResponse";
const _La = "Layer";
const _Li = "License";
const _M = "Message";
const _MAB = "MaxAbrBitrate";
const _MABa = "MaxAverageBitrate";
const _MABi = "MinAbrBitrate";
const _MADS = "MaxAncDataSize";
const _MB = "MaxBitrate";
const _MBRS = "MinBottomRenditionSize";
const _MBT = "MinBufferTime";
const _MC = "MetadataControl";
const _MCCN = "ManifestConfirmConditionNotification";
const _MCF = "MaximumConcurrentFeeds";
const _MCLL = "MaxContentLightLevel";
const _MCa = "ManifestCompression";
const _MCat = "MatrixCoefficients";
const _MCax = "MaxCll";
const _MCaxa = "MaxCaptures";
const _MD = "MetadataDestination";
const _MDCV = "MasteringDisplayColorVolume";
const _MDF = "ManifestDurationFormat";
const _ME = "ManifestEncoding";
const _MEI = "MinEbpInterval";
const _MF = "MaxFall";
const _MFALL = "MaxFrameAverageLightLevel";
const _MFCCC = "Mpeg2FourCCControl";
const _MFSL = "MinFinalSegmentLength";
const _MII = "MinIInterval";
const _MIIF = "MotionImageInsertionFramerate";
const _MIIO = "MotionImageInsertionOffset";
const _MIIo = "MotionImageInserter";
const _MIIot = "MotionImageInserters";
const _ML = "MaxLuminance";
const _MLi = "MinLuminance";
const _MM = "MeteringMode";
const _MMB = "Mp4MajorBrand";
const _MMBT = "MpdManifestBandwidthType";
const _MMN = "MasteringMonitorNits";
const _MMS = "ManifestMetadataSignaling";
const _MNM = "ManifestNameModifier";
const _MOPB = "MvOverPictureBoundaries";
const _MP = "MpdProfile";
const _MPI = "MaxPcrInterval";
const _MPo = "MoovPlacement";
const _MQL = "MaxQualityLevel";
const _MR = "MaxRenditions";
const _MRGBT = "MaximumRGBTolerance";
const _MRGBTi = "MinimumRGBTolerance";
const _MRa = "MaxResults";
const _MS = "Mp2Settings";
const _MSAM = "MsSmoothAdditionalManifest";
const _MSE = "M2tsScte35Esam";
const _MSES = "MsSmoothEncryptionSettings";
const _MSGS = "MsSmoothGroupSettings";
const _MSL = "MinSegmentLength";
const _MSo = "MovSettings";
const _MSp = "Mp3Settings";
const _MSpd = "MpdSettings";
const _MSpe = "Mp4Settings";
const _MSpeg = "Mpeg2Settings";
const _MSt = "M2tsSettings";
const _MSu = "M3u8Settings";
const _MSx = "MxfSettings";
const _MT = "MimeType";
const _MTP = "MvTemporalPredictor";
const _MTRS = "MinTopRenditionSize";
const _MVI = "MultiViewInput";
const _MVS = "MultiViewSettings";
const _MX = "MccXml";
const _MXPS = "MxfXavcProfileSettings";
const _MYUV = "MaximumYUV";
const _MYUVi = "MinimumYUV";
const _Ma = "Mapping";
const _Me = "Messages";
const _Met = "Metadata3";
const _Meta = "Metadata4";
const _Metad = "Metadata5";
const _Metada = "Metadata6";
const _Metadat = "Metadata7";
const _Metadata = "Metadata8";
const _Metadata_ = "Metadata";
const _Mo = "Mode";
const _N = "Numerator";
const _NBFBRF = "NumberBFramesBetweenReferenceFrames";
const _NC = "NielsenConfiguration";
const _NFE = "NotFoundException";
const _NFMS = "NexguardFileMarkerSettings";
const _NGFMS = "NexGuardFileMarkerSettings";
const _NI = "NetworkId";
const _NIi = "NitInterval";
const _NIie = "NielsenId3";
const _NM = "NameModifier";
const _NN = "NetworkName";
const _NNLW = "NielsenNonLinearWatermark";
const _NNLWS = "NielsenNonLinearWatermarkSettings";
const _NPB = "NullPacketBitrate";
const _NR = "NoiseReducer";
const _NRF = "NumberReferenceFrames";
const _NRFS = "NoiseReducerFilterSettings";
const _NRSFS = "NoiseReducerSpatialFilterSettings";
const _NRTFS = "NoiseReducerTemporalFilterSettings";
const _NT = "NextToken";
const _Na = "Name";
const _O = "Offset";
const _OC = "ObjectCount";
const _OCM = "OutputChannelMapping";
const _OCS = "OutputColorSpace";
const _OCu = "OutlineColor";
const _OCut = "OutputChannels";
const _OD = "OutputDetail";
const _ODu = "OutputDetails";
const _OE = "OfflineEncrypted";
const _OG = "OutputGroups";
const _OGD = "OutputGroupDetails";
const _OGDu = "OutputGroupDetail";
const _OGS = "OutputGroupSettings";
const _OGu = "OutputGroup";
const _OML = "OutputMasteringLuminance";
const _OS = "OpusSettings";
const _OSu = "OutlineSize";
const _OSut = "OutputSdt";
const _OSutp = "OutputSelection";
const _OSutpu = "OutputSettings";
const _OV = "OverrideValue";
const _Op = "Opacity";
const _Or = "Order";
const _Ou = "Output";
const _Out = "Outputs";
const _P = "Pids";
const _PA = "PurchasedAt";
const _PBU = "PreventBufferUnderflow";
const _PC = "PassthroughControl";
const _PCa = "ParControl";
const _PCad = "PaddingControl";
const _PCc = "PcrControl";
const _PCe = "PeakCalculation";
const _PCh = "PhaseControl";
const _PCs = "PsiControl";
const _PD = "ParDenominator";
const _PDC = "PlaybackDeviceCompatibility";
const _PDT = "ProgramDateTime";
const _PDTP = "ProgramDateTimePeriod";
const _PFM = "PerFrameMetrics";
const _PFSS = "PostFilterSharpenStrength";
const _PI = "PatInterval";
const _PIF = "ProbeInputFile";
const _PIm = "PmtInterval";
const _PJC = "ProgressingJobsCount";
const _PMP = "PrivateMetadataPid";
const _PN = "ParNumerator";
const _PNa = "PageNumber";
const _PNr = "ProgramNumber";
const _PO = "PtsOffset";
const _POHFBF = "PtsOffsetHandlingForBFrames";
const _POM = "PtsOffsetMode";
const _PP = "PricingPlan";
const _PPR = "PutPolicyRequest";
const _PPRu = "PutPolicyResponse";
const _PPc = "PcrPid";
const _PPm = "PmtPid";
const _PPu = "PutPolicy";
const _PR = "ProbeRequest";
const _PRr = "ProbeResponse";
const _PRro = "ProbeResults";
const _PRrob = "ProbeResult";
const _PS = "ProgramSelection";
const _PSa = "PassthroughSettings";
const _PSr = "PresetSettings";
const _PSro = "ProresSettings";
const _PT = "PageTypes";
const _PTS = "PostTemporalSharpening";
const _PTSS = "PostTemporalSharpeningStrength";
const _PV = "PadVideo";
const _PW = "PartnerWatermarking";
const _PWHM = "ProgressiveWriteHlsManifest";
const _Pa = "Payload";
const _Pi = "Pid";
const _Pl = "Playback";
const _Po = "Policy";
const _Pos = "Position";
const _Pr = "Profile";
const _Pre = "Preset";
const _Pref = "Prefix";
const _Pres = "Presets";
const _Pri = "Priority";
const _Pro = "Probe";
const _Q = "Queue";
const _QQL = "QvbrQualityLevel";
const _QQLFT = "QvbrQualityLevelFineTune";
const _QS = "QvbrSettings";
const _QT = "QueueTransitions";
const _QTL = "QualityTuningLevel";
const _QTu = "QueueTransition";
const _Qu = "Quality";
const _Que = "Queues";
const _R = "Required";
const _RC = "RetryCount";
const _RCM = "RateControlMode";
const _RF = "RawFormat";
const _RGI = "RenditionGroupId";
const _RI = "RapInterval";
const _RIe = "ResourceId";
const _RLC = "RenditionLanguageCode";
const _RM = "RateMode";
const _RN = "RenditionName";
const _RP = "RepeatPps";
const _RPS = "ReservationPlanSettings";
const _RPX = "RedPrimaryX";
const _RPY = "RedPrimaryY";
const _RPe = "ReservationPlan";
const _RRRA = "RemoveRubyReserveAttributes";
const _RS = "RemixSettings";
const _RSP = "ResponseSignalPreroll";
const _RSe = "ReservedSlots";
const _RT = "ResourceTags";
const _RTA = "RespondToAfd";
const _RTe = "RenewalType";
const _Re = "Reference";
const _Rec = "Rectangle";
const _Ro = "Rotation";
const _Rol = "Role";
const _Rot = "Rotate";
const _Rp = "Rpu";
const _Ru = "Rules";
const _S = "Specification";
const _SACN = "SourceAncillaryChannelNumber";
const _SAE = "SaliencyAwareEncoding";
const _SAOFM = "SampleAdaptiveOffsetFilterMode";
const _SAP = "SpekeAudioPreset";
const _SAQ = "SpatialAdaptiveQuantization";
const _SAR = "SampleAspectRatio";
const _SB = "ScalingBehavior";
const _SC = "ShadowColor";
const _SCD = "SceneChangeDetect";
const _SCI = "SupportCaseId";
const _SCN = "Source608ChannelNumber";
const _SCe = "SegmentControl";
const _SCt = "StorageClass";
const _SD = "StereoDownmix";
const _SDAC = "S3DestinationAccessControl";
const _SDS = "SccDestinationSettings";
const _SDSe = "S3DestinationSettings";
const _SDSr = "SrtDestinationSettings";
const _SE = "Scte35Esam";
const _SEM = "SurroundExMode";
const _SEP = "Scte35EsamPid";
const _SES = "S3EncryptionSettings";
const _SF = "SourceFile";
const _SFS = "SpatialFilterSettings";
const _SI = "SdtInterval";
const _SIR = "StreamInfResolution";
const _SIn = "S3Inputs";
const _SIo = "SourceId";
const _SIu = "SupplementalImps";
const _SIy = "SystemIds";
const _SJ = "SearchJobs";
const _SJC = "SubmittedJobsCount";
const _SJQ = "StartJobsQuery";
const _SJQR = "StartJobsQueryRequest";
const _SJQRt = "StartJobsQueryResponse";
const _SJR = "SearchJobsRequest";
const _SJRe = "SearchJobsResponse";
const _SKK = "SigningKmsKey";
const _SKP = "SpekeKeyProvider";
const _SKPC = "SpekeKeyProviderCmaf";
const _SKPt = "StaticKeyProvider";
const _SKV = "StaticKeyValue";
const _SL = "SegmentLength";
const _SLC = "SegmentLengthControl";
const _SM = "SurroundMode";
const _SMe = "SegmentModifier";
const _SMeg = "SegmentationMarkers";
const _SMegm = "SegmentationMode";
const _SN = "StreamName";
const _SNe = "ServiceName";
const _SNt = "StreamNumber";
const _SO = "ShadowOpacity";
const _SOe = "SelectedOutputs";
const _SOer = "ServiceOverrides";
const _SOerv = "ServiceOverride";
const _SP = "Smpte337Passthrough";
const _SPN = "ServiceProviderName";
const _SPNi = "SignalProcessingNotification";
const _SPPC = "SlowPalPitchCorrection";
const _SPS = "SegmentsPerSubdirectory";
const _SPc = "Scte35Pid";
const _SPl = "SlowPal";
const _SPt = "StylePassthrough";
const _SQ = "SourceQueue";
const _SQEE = "ServiceQuotaExceededException";
const _SR = "SampleRate";
const _SRC = "SampleRangeConversion";
const _SRQ = "SimulateReservedQueue";
const _SRWL = "SdrReferenceWhiteLevel";
const _SRa = "SampleRange";
const _SS = "SourceSettings";
const _SSc = "Scte35Source";
const _SSe = "S3Settings";
const _SSeg = "SegmentationStyle";
const _SSh = "ShareStatus";
const _ST = "SelectorType";
const _STCM = "ScanTypeConversionMode";
const _STN = "Source608TrackNumber";
const _STc = "ScanType";
const _STe = "SegmentationTime";
const _STo = "SourceType";
const _STp = "SpeechThreshold";
const _STt = "StartTimecode";
const _STta = "StartTime";
const _STu = "SubtitlingType";
const _STub = "SubmitTime";
const _SUI = "StatusUpdateInterval";
const _SVP = "SpekeVideoPreset";
const _SWS = "SourceWatermarkStatus";
const _SX = "SccXml";
const _SXO = "ShadowXOffset";
const _SYO = "ShadowYOffset";
const _Sa = "Saturation";
const _Se = "Settings";
const _Sh = "Sharpening";
const _Sha = "Sharpness";
const _Sl = "Slices";
const _So = "Softness";
const _Sou = "Source";
const _Sp = "Speed";
const _St = "Streams";
const _Sta = "Status";
const _Star = "Start";
const _Str = "Strength";
const _Sy = "Syntax";
const _T = "Tracks";
const _TAQ = "TemporalAdaptiveQuantization";
const _TB = "TimecodeBurnin";
const _TBS = "TreeBlockSize";
const _TC = "TerminateCaptions";
const _TCJ = "TotalConcurrentJobs";
const _TCi = "TimecodeConfig";
const _TCr = "TransferCharacteristics";
const _TD = "TimeDelta";
const _TDCM = "TargetDurationCompatibilityMode";
const _TDM = "TimestampDeltaMilliseconds";
const _TDS = "TeletextDestinationSettings";
const _TDSt = "TtmlDestinationSettings";
const _TDU = "TimeDeltaUnits";
const _TDe = "TextDecoration";
const _TFS = "TemporalFilterSettings";
const _TH = "ThumbnailHeight";
const _THi = "TileHeight";
const _TI = "ThumbnailInterval";
const _TId = "TdtInterval";
const _TIe = "TemporalIds";
const _TIi = "TimecodeInsertion";
const _TK = "TagKeys";
const _TL = "TargetLkfs";
const _TLR = "TargetLoudnessRange";
const _TM = "TimedMetadata";
const _TMBV = "TimedMetadataBoxVersion";
const _TMI = "TimedMetadataInsertion";
const _TMIF = "TimedMetadataId3Frame";
const _TMIP = "TimedMetadataId3Period";
const _TMN = "TargetMonitorNits";
const _TMP = "TimedMetadataPid";
const _TMRE = "TooManyRequestsException";
const _TMSIU = "TimedMetadataSchemeIdUri";
const _TMV = "TimedMetadataValue";
const _TMr = "TrackMappings";
const _TMra = "TrackMapping";
const _TN = "TrackNumber";
const _TO = "TimestampOffset";
const _TP = "TilePadding";
const _TPLT = "TruePeakLimiterThreshold";
const _TR = "TagResource";
const _TRR = "TagResourceRequest";
const _TRRa = "TagResourceResponse";
const _TS = "TeletextSpacing";
const _TSI = "TransportStreamId";
const _TSS = "TeletextSourceSettings";
const _TSSr = "TrackSourceSettings";
const _TSU = "TicServerUrl";
const _TSa = "TamsSettings";
const _TSi = "TimecodeSource";
const _TSim = "TimecodeStart";
const _TT = "TrackType";
const _TTi = "TimecodeTrack";
const _TW = "ThumbnailWidth";
const _TWi = "TileWidth";
const _Ta = "Tags";
const _Te = "Telecine";
const _Ti = "Tiles";
const _Tim = "Timecode";
const _Time = "Timerange";
const _Times = "Timestamp";
const _Timi = "Timing";
const _Tr = "Track";
const _Tra = "Transitions";
const _Ty = "Type";
const _U = "Url";
const _UAW = "UncompressedAudioWrapping";
const _UCJ = "UnallocatedConcurrentJobs";
const _UEE = "UnprocessableEntityException";
const _UJT = "UpdateJobTemplate";
const _UJTR = "UpdateJobTemplateRequest";
const _UJTRp = "UpdateJobTemplateResponse";
const _UM = "UserMetadata";
const _UP = "UpdatePreset";
const _UPR = "UpdatePresetRequest";
const _UPRp = "UpdatePresetResponse";
const _UQ = "UpdateQueue";
const _UQR = "UpdateQueueRequest";
const _UQRp = "UpdateQueueResponse";
const _UR = "UntagResource";
const _URR = "UntagResourceRequest";
const _URRn = "UntagResourceResponse";
const _US = "UncompressedSettings";
const _UST = "UnregisteredSeiTimecode";
const _USTLTT = "UpconvertSTLToTeletext";
const _UTPAT = "UniqueTicPerAudioTrack";
const _Un = "Unit";
const _V = "Version";
const _VA = "VchipAction";
const _VC = "Vc3Class";
const _VCO = "VideoCompositionOffsets";
const _VCS = "VideoCodecSettings";
const _VD = "VideoDescription";
const _VDi = "VideoDetails";
const _VDid = "VideoDetail";
const _VG = "VideoGenerator";
const _VO = "VideoOverlays";
const _VOC = "VideoOverlayCrop";
const _VOI = "VideoOverlayInput";
const _VOIC = "VideoOverlayInputClipping";
const _VOP = "VideoOverlayPosition";
const _VOT = "VideoOverlayTransition";
const _VOi = "VideoOverlay";
const _VP = "VideoPid";
const _VPi = "VideoProperties";
const _VPid = "VideoPreprocessors";
const _VPide = "VideoPreprocessor";
const _VQ = "VbrQuality";
const _VS = "VorbisSettings";
const _VSM = "VideoSelectorMode";
const _VSc = "Vc3Settings";
const _VSi = "VideoSelector";
const _VSp = "Vp8Settings";
const _VSpe = "Vp9Settings";
const _VTI = "VideoTrackIndexes";
const _Va = "Values";
const _Val = "Value";
const _Ve = "Versions";
const _W = "Width";
const _WDM = "WriteDashManifest";
const _WDS = "WebvttDestinationSettings";
const _WG = "WarningGroup";
const _WHM = "WriteHlsManifest";
const _WHSS = "WebvttHlsSourceSettings";
const _WIP = "WidthInPx";
const _WM = "WaitMinutes";
const _WMPT = "WriteMp4PackagingType";
const _WPX = "WhitePointX";
const _WPY = "WhitePointY";
const _WS = "WavSettings";
const _WSTIR = "WriteSegmentTimelineInRepresentation";
const _Wa = "Warnings";
const _War = "Warning";
const _X = "X";
const _XC = "XavcClass";
const _XHICPS = "XavcHdIntraCbgProfileSettings";
const _XHPS = "XavcHdProfileSettings";
const _XICPS = "Xavc4kIntraCbgProfileSettings";
const _XIVPS = "Xavc4kIntraVbrProfileSettings";
const _XP = "XPosition";
const _XPS = "XavcProfileSettings";
const _XPSa = "Xavc4kProfileSettings";
const _XS = "XavcSettings";
const _Y = "Y";
const _YP = "YPosition";
const _a = "arn";
const _aA = "antiAlias";
const _aAO = "adAvailOffset";
const _aB = "availBlanking";
const _aBI = "availBlankingImage";
const _aBM = "audioBufferModel";
const _aBl = "alphaBehavior";
const _aC = "algorithmControl";
const _aCA = "authConnectionArn";
const _aCCSIU = "audioChannelConfigSchemeIdUri";
const _aCH = "accessibilityCaptionHints";
const _aCTS = "audioChannelTaggingSettings";
const _aCc = "accessControl";
const _aCt = "attenuationControl";
const _aD = "audioDuration";
const _aDAC = "audioDescriptionAudioChannel";
const _aDBM = "audioDescriptionBroadcasterMix";
const _aDC = "audioDurationCorrection";
const _aDDC = "audioDescriptionDataChannel";
const _aDM = "audioDescriptionMix";
const _aDu = "audioDeduplication";
const _aDud = "audioDescriptions";
const _aES = "automatedEncodingSettings";
const _aF = "adiFilename";
const _aFC = "applyFontColor";
const _aFPP = "audioFramesPerPes";
const _aGI = "audioGroupId";
const _aI = "assetId";
const _aIC = "avcIntraClass";
const _aIF = "advancedInputFilter";
const _aIFS = "advancedInputFilterSettings";
const _aIS = "avcIntraSettings";
const _aIUS = "avcIntraUhdSettings";
const _aM = "additionalManifests";
const _aMd = "adMarkers";
const _aMg = "aggressiveMode";
const _aN = "assetName";
const _aNS = "audioNormalizationSettings";
const _aOC = "audioOnlyContainer";
const _aOH = "audioOnlyHeader";
const _aP = "audioPids";
const _aPCS = "audioPitchCorrectionSettings";
const _aPOD = "audioPtsOffsetDelta";
const _aPu = "audioProperties";
const _aQ = "adaptiveQuantization";
const _aR = "allowedRenditions";
const _aRS = "audioRenditionSets";
const _aS = "aacSettings";
const _aSG = "audioSelectorGroups";
const _aSN = "audioSourceName";
const _aSNu = "audioSelectorNames";
const _aSS = "ancillarySourceSettings";
const _aSb = "abrSettings";
const _aSc = "ac3Settings";
const _aScc = "accelerationSettings";
const _aScce = "accelerationStatus";
const _aSce = "ac4Settings";
const _aSf = "afdSignaling";
const _aSi = "aiffSettings";
const _aSu = "audioSelectors";
const _aSv = "av1Settings";
const _aT = "addTexture";
const _aTC = "audioTypeControl";
const _aTFS = "alternateTransferFunctionSei";
const _aTI = "audioTrackIndexes";
const _aTT = "audioTrackType";
const _aTu = "audioType";
const _aWP = "activeWatermarkProcess";
const _ac = "accessibility";
const _al = "algorithm";
const _ali = "alignment";
const _an = "anchor";
const _b = "bitrate";
const _bC = "backgroundColor";
const _bCi = "bitrateClass";
const _bCr = "breakoutCode";
const _bD = "bitDepth";
const _bDS = "burninDestinationSettings";
const _bL = "baseLayer";
const _bM = "bitstreamMode";
const _bMu = "bufferModel";
const _bO = "backgroundOpacity";
const _bPX = "bluePrimaryX";
const _bPY = "bluePrimaryY";
const _bR = "bitRate";
const _bRF = "bandwidthReductionFilter";
const _bRL = "byteRateLimit";
const _bTS = "billingTagsSource";
const _bU = "baseUrl";
const _br = "brightness";
const _c = "client";
const _cA = "createdAt";
const _cAa = "cannedAcl";
const _cAe = "certificateArn";
const _cAl = "clapAtom";
const _cAs = "cslgAtom";
const _cC = "clientCache";
const _cCDLUTS = "colorConversion3DLUTSettings";
const _cCT = "captionContainerType";
const _cCa = "captionChannel";
const _cCo = "colorCorrector";
const _cD = "captionDescriptions";
const _cFR = "codedFrameRate";
const _cGL = "correctionGateLevel";
const _cGS = "cmafGroupSettings";
const _cI = "channelsIn";
const _cIV = "constantInitializationVector";
const _cJ = "concurrentJobs";
const _cL = "channelLayout";
const _cLC = "customLanguageCode";
const _cLL = "contentLightLevel";
const _cLM = "captionLanguageMappings";
const _cLS = "clearLeadSegments";
const _cLSa = "captionLanguageSetting";
const _cLl = "clipLimits";
const _cLo = "codecLevel";
const _cM = "codingMode";
const _cMh = "channelMapping";
const _cMo = "colorMetadata";
const _cMod = "codecMetadata";
const _cMp = "c2paManifest";
const _cN = "channelName";
const _cNu = "customName";
const _cO = "channelsOut";
const _cP = "codecProfile";
const _cPA = "copyProtectionAction";
const _cPM = "chromaPositionMode";
const _cPTP = "convertPaintToPop";
const _cPo = "colorPrimaries";
const _cPu = "currentPhase";
const _cR = "contentReference";
const _cRT = "clientRequestToken";
const _cS = "codecSettings";
const _cSC = "colorSpaceConversion";
const _cSI = "cbetSourceId";
const _cSLC = "captionSegmentLengthControl";
const _cSN = "captionSelectorName";
const _cSNr = "credentialsSecretName";
const _cSU = "colorSpaceUsage";
const _cSa = "captionSelectors";
const _cSe = "certificateSecret";
const _cSh = "chromaSubsampling";
const _cShr = "chromaSampling";
const _cSm = "cmfcSettings";
const _cSo = "codecSpecification";
const _cSol = "colorSpace";
const _cSon = "containerSettings";
const _cT = "convert608To708";
const _cTh = "channelTag";
const _cTha = "channelTags";
const _cV = "cttsVersion";
const _ca = "category";
const _ch = "channels";
const _co = "codec";
const _cod = "code";
const _com = "compatibility";
const _comm = "commitment";
const _con = "contrast";
const _cont = "container";
const _contr = "control";
const _cou = "count";
const _cr = "crop";
const _d = "dialnorm";
const _dAR = "displayAspectRatio";
const _dAS = "dynamicAudioSelectors";
const _dC = "downmixControl";
const _dCN = "destination608ChannelNumber";
const _dCu = "durationControl";
const _dF = "dcFilter";
const _dFT = "dropFrameTimecode";
const _dFe = "deblockFilter";
const _dFen = "denoiseFilter";
const _dH = "ddsHandling";
const _dI = "dialogueIntelligence";
const _dIFTPNM = "dashIFrameTrickPlayNameModifier";
const _dIGS = "dashIsoGroupSettings";
const _dIM = "durationInMs";
const _dIi = "distributorId";
const _dM = "decryptionMode";
const _dMS = "dashManifestStyle";
const _dMu = "durationMode";
const _dNS = "dvbNitSettings";
const _dP = "dataProperties";
const _dPTSC = "dataPTSControl";
const _dQ = "destinationQueue";
const _dRC = "dynamicRangeControl";
const _dRCFPT = "dynamicRangeCompressionFlatPanelTv";
const _dRCHT = "dynamicRangeCompressionHomeTheater";
const _dRCL = "dynamicRangeCompressionLine";
const _dRCP = "dynamicRangeCompressionProfile";
const _dRCPH = "dynamicRangeCompressionPortableHeadphones";
const _dRCPS = "dynamicRangeCompressionPortableSpeakers";
const _dRCR = "dynamicRangeCompressionRf";
const _dS = "defaultSelection";
const _dSDS = "dvbSubDestinationSettings";
const _dSG = "dynamicSubGop";
const _dSN = "destination708ServiceNumber";
const _dSP = "dvbSubPids";
const _dSS = "dvbSdtSettings";
const _dSSI = "dashSignaledSystemIds";
const _dSSS = "dvbSubSourceSettings";
const _dSe = "destinationSettings";
const _dSec = "decryptionSettings";
const _dSi = "directoryStructure";
const _dT = "destinationType";
const _dTI = "dataTrackIndexes";
const _dTP = "dvbTeletextPid";
const _dTS = "dvbTdtSettings";
const _dV = "dolbyVision";
const _dVMX = "dolbyVisionMetadataXml";
const _dVSF = "descriptiveVideoServiceFlag";
const _dXC = "ddsXCoordinate";
const _dYC = "ddsYCoordinate";
const _de = "denominator";
const _deb = "deblocking";
const _dei = "deinterlacer";
const _des = "destination";
const _desc = "description";
const _du = "duration";
const _e = "error";
const _eA = "expiresAt";
const _eAFI = "externalAudioFileInput";
const _eAI = "ebpAudioInterval";
const _eAS = "eac3AtmosSettings";
const _eC = "errorCode";
const _eCC = "encryptionContractConfiguration";
const _eD = "expirationDate";
const _eDK = "encryptedDecryptionKey";
const _eDS = "embeddedDestinationSettings";
const _eDSx = "extendedDataServices";
const _eE = "entropyEncoding";
const _eI = "episodeId";
const _eIC = "elementalInferenceConfiguration";
const _eL = "enhancementLayer";
const _eM = "encryptionMethod";
const _eMr = "errorMessage";
const _eOSM = "endOfStreamMarkers";
const _eP = "ebpPlacement";
const _ePn = "endPosition";
const _eRIP = "esRateInPes";
const _eS = "eac3Settings";
const _eSS = "embeddedSourceSettings";
const _eT = "endTimecode";
const _eTO = "embeddedTimecodeOverride";
const _eTa = "eTag";
const _eTn = "encryptionType";
const _eWP = "explicitWeightedPrediction";
const _en = "encryption";
const _end = "endpoints";
const _es = "esam";
const _ex = "extension";
const _f = "format";
const _fA = "fixedAfd";
const _fAQ = "flickerAdaptiveQuantization";
const _fC = "framerateControl";
const _fCA = "framerateConversionAlgorithm";
const _fCS = "frameCaptureSettings";
const _fCo = "fontColor";
const _fCr = "frameControl";
const _fD = "framerateDenominator";
const _fE = "fieldEncoding";
const _fEi = "filterEnable";
const _fF = "fallbackFont";
const _fFB = "fontFileBold";
const _fFBI = "fontFileBoldItalic";
const _fFI = "fontFileItalic";
const _fFR = "fontFileRegular";
const _fGS = "filmGrainSynthesis";
const _fGSi = "fileGroupSettings";
const _fI = "fileInput";
const _fIR = "forceIncludeRenditions";
const _fIa = "fadeIn";
const _fL = "fragmentLength";
const _fLC = "fragmentLengthControl";
const _fLi = "filterList";
const _fMS = "feedManagementState";
const _fN = "framerateNumerator";
const _fO = "fontOpacity";
const _fOa = "fadeOut";
const _fOi = "fieldOrder";
const _fOil = "fileOffset";
const _fR = "frameRate";
const _fRo = "fontResolution";
const _fS = "flacSettings";
const _fSB = "freeSpaceBox";
const _fSS = "fileSourceSettings";
const _fSi = "filterStrength";
const _fSil = "fileSize";
const _fSilt = "filterSettings";
const _fSo = "fontScript";
const _fSol = "followSource";
const _fSon = "fontSize";
const _fSont = "fontStyle";
const _fSv = "f4vSettings";
const _fT = "fragmentTime";
const _fTVEO = "forceTsVideoEbpOrder";
const _fTi = "finishTime";
const _fU = "fileUrl";
const _fW = "fontWeight";
const _fe = "features";
const _fee = "feeds";
const _fi = "filter";
const _fo = "fourcc";
const _fr = "framerate";
const _gBR = "gopBReference";
const _gCC = "gopClosedCadence";
const _gH = "gapHandling";
const _gPS = "gopsPerSegment";
const _gPX = "greenPrimaryX";
const _gPY = "greenPrimaryY";
const _gS = "gopSize";
const _gSU = "gopSizeUnits";
const _gSi = "gifSettings";
const _h = "height";
const _hBFFP = "hrdBufferFinalFillPercentage";
const _hBIFP = "hrdBufferInitialFillPercentage";
const _hBS = "hrdBufferSize";
const _hC = "hbbtvCompliance";
const _hD = "hopDestinations";
const _hE = "httpError";
const _hFC = "hexFontColor";
const _hGS = "hlsGroupSettings";
const _hI = "httpInputs";
const _hIP = "heightInPx";
const _hIt = "httpsInputs";
const _hM = "hdr10Metadata";
const _hMd = "hdrMetadata";
const _hP = "hdr10Plus";
const _hPP = "hdr10PlusPresence";
const _hQ = "httpQuery";
const _hRGS = "hlsRenditionGroupSettings";
const _hS = "hlsSettings";
const _hSSI = "hlsSignaledSystemIds";
const _hSe = "h264Settings";
const _hSet = "h265Settings";
const _hTSTM = "hdrToSdrToneMapper";
const _ht = "http";
const _hu = "hue";
const _i = "id3";
const _iBTP = "imageBasedTrickPlay";
const _iBTPS = "imageBasedTrickPlaySettings";
const _iBTPV = "imageBasedTrickPlayVariants";
const _iC = "intervalCadence";
const _iCFT = "inputChannelsFineTune";
const _iCS = "inputColorSpace";
const _iCn = "inputClippings";
const _iCnp = "inputChannels";
const _iDMCD = "integerDurationMaximumCompressionDenominator";
const _iDMCN = "integerDurationMaximumCompressionNumerator";
const _iDP = "intraDcPrecision";
const _iDS = "imscDestinationSettings";
const _iDTTM = "integerDurationTrimThresholdMilliseconds";
const _iF = "inputFiles";
const _iFOM = "iFrameOnlyManifest";
const _iFn = "inputFile";
const _iI = "insertableImages";
const _iII = "imageInserterInput";
const _iId = "id3Insertions";
const _iIm = "imageInserter";
const _iIma = "imageInput";
const _iM = "interlaceMode";
const _iML = "inputMasteringLuminance";
const _iMn = "insertionMode";
const _iP = "initialPosition";
const _iST = "inputScanType";
const _iT = "idempotencyToken";
const _iV = "initializationVector";
const _iVIM = "initializationVectorInManifest";
const _iX = "imageX";
const _iY = "imageY";
const _id = "id";
const _in = "info";
const _ind = "index";
const _inp = "inputs";
const _inpu = "input";
const _j = "job";
const _jEV = "jobEngineVersion";
const _jEVR = "jobEngineVersionRequested";
const _jEVU = "jobEngineVersionUsed";
const _jI = "jobId";
const _jN = "jsonName";
const _jPC = "jobPercentComplete";
const _jT = "jobTemplate";
const _jTo = "jobTemplates";
const _jo = "jobs";
const _k = "key";
const _kEC = "kmsEncryptionContext";
const _kF = "keyFormat";
const _kFV = "keyFormatVersions";
const _kKA = "kmsKeyArn";
const _kKR = "kmsKeyRegion";
const _kLI = "kantarLicenseId";
const _kM = "klvMetadata";
const _kSU = "kantarServerUrl";
const _kW = "kantarWatermark";
const _l = "level";
const _lB = "listBy";
const _lC = "languageCode";
const _lCC = "languageCodeControl";
const _lCf = "lfeControl";
const _lD = "languageDescription";
const _lDo = "logDestination";
const _lF = "lfeFilter";
const _lL = "loudnessLogging";
const _lM = "l6Metadata";
const _lMM = "loudnessMeasurementMode";
const _lMa = "lastModified";
const _lMo = "l6Mode";
const _lOAAR = "__listOfAutomatedAbrRule";
const _lOAD = "__listOfAudioDescription";
const _lOARS = "__listOfAllowedRenditionSize";
const _lOCAM = "__listOfCmafAdditionalManifest";
const _lOCCDLUTS = "__listOfColorConversion3DLUTSetting";
const _lOCD = "__listOfCaptionDescription";
const _lOCDP = "__listOfCaptionDescriptionPreset";
const _lOCIBTPV = "__listOfCmafImageBasedTrickPlayVariant";
const _lODAM = "__listOfDashAdditionalManifest";
const _lODIIBTPV = "__listOfDashIsoImageBasedTrickPlayVariant";
const _lOE = "__listOfEndpoint";
const _lOEIF = "__listOfElementalInferenceFeed";
const _lOFIRS = "__listOfForceIncludeRenditionSize";
const _lOHAM = "__listOfHlsAdditionalManifest";
const _lOHCLM = "__listOfHlsCaptionLanguageMapping";
const _lOHD = "__listOfHopDestination";
const _lOHIBTPV = "__listOfHlsImageBasedTrickPlayVariant";
const _lOI = "__listOfInput";
const _lOIC = "__listOfInputClipping";
const _lOII = "__listOfId3Insertion";
const _lOIIi = "__listOfInsertableImage";
const _lOIT = "__listOfInputTemplate";
const _lOJ = "__listOfJob";
const _lOJEV = "__listOfJobEngineVersion";
const _lOJQF = "__listOfJobsQueryFilter";
const _lOJT = "__listOfJobTemplate";
const _lOMII = "__listOfMotionImageInserter";
const _lOMSAM = "__listOfMsSmoothAdditionalManifest";
const _lOMVS = "__listOfMultiViewSettings";
const _lOO = "__listOfOutput";
const _lOOCM = "__listOfOutputChannelMapping";
const _lOOD = "__listOfOutputDetail";
const _lOOG = "__listOfOutputGroup";
const _lOOGD = "__listOfOutputGroupDetail";
const _lOP = "__listOfPreset";
const _lOPIF = "__listOfProbeInputFile";
const _lOPR = "__listOfProbeResult";
const _lOQ = "__listOfQueue";
const _lOQT = "__listOfQueueTransition";
const _lOSO = "__listOfServiceOverride";
const _lOT = "__listOfTrack";
const _lOTM = "__listOfTrackMapping";
const _lOVO = "__listOfVideoOverlay";
const _lOVOIC = "__listOfVideoOverlayInputClipping";
const _lOVOT = "__listOfVideoOverlayTransition";
const _lOWG = "__listOfWarningGroup";
const _lRCML = "loRoCenterMixLevel";
const _lRCMLt = "ltRtCenterMixLevel";
const _lRSML = "loRoSurroundMixLevel";
const _lRSMLt = "ltRtSurroundMixLevel";
const _lSD = "lastShareDetails";
const _lU = "lastUpdated";
const _la = "layer";
const _li = "license";
const _m = "message";
const _mAB = "maxAbrBitrate";
const _mABa = "maxAverageBitrate";
const _mABi = "minAbrBitrate";
const _mADS = "maxAncDataSize";
const _mB = "maxBitrate";
const _mBRS = "minBottomRenditionSize";
const _mBT = "minBufferTime";
const _mC = "metadataControl";
const _mCCN = "manifestConfirmConditionNotification";
const _mCF = "maximumConcurrentFeeds";
const _mCLL = "maxContentLightLevel";
const _mCa = "manifestCompression";
const _mCat = "matrixCoefficients";
const _mCax = "maxCll";
const _mCaxa = "maxCaptures";
const _mD = "metadataDestination";
const _mDCV = "masteringDisplayColorVolume";
const _mDF = "manifestDurationFormat";
const _mE = "manifestEncoding";
const _mEI = "minEbpInterval";
const _mF = "maxFall";
const _mFALL = "maxFrameAverageLightLevel";
const _mFCCC = "mpeg2FourCCControl";
const _mFSL = "minFinalSegmentLength";
const _mII = "minIInterval";
const _mIIo = "motionImageInserter";
const _mIIot = "motionImageInserters";
const _mL = "maxLuminance";
const _mLi = "minLuminance";
const _mM = "meteringMode";
const _mMB = "mp4MajorBrand";
const _mMBT = "mpdManifestBandwidthType";
const _mMN = "masteringMonitorNits";
const _mMS = "manifestMetadataSignaling";
const _mNM = "manifestNameModifier";
const _mOAS = "__mapOfAudioSelector";
const _mOASG = "__mapOfAudioSelectorGroup";
const _mOCS = "__mapOfCaptionSelector";
const _mODAS = "__mapOfDynamicAudioSelector";
const _mOPB = "mvOverPictureBoundaries";
const _mP = "mpdProfile";
const _mPI = "maxPcrInterval";
const _mPo = "moovPlacement";
const _mQL = "maxQualityLevel";
const _mR = "maxRenditions";
const _mRGBT = "maximumRGBTolerance";
const _mRGBTi = "minimumRGBTolerance";
const _mRa = "maxResults";
const _mS = "mp2Settings";
const _mSGS = "msSmoothGroupSettings";
const _mSL = "minSegmentLength";
const _mSo = "movSettings";
const _mSp = "mp3Settings";
const _mSpd = "mpdSettings";
const _mSpe = "mp4Settings";
const _mSpeg = "mpeg2Settings";
const _mSt = "m2tsSettings";
const _mSu = "m3u8Settings";
const _mSx = "mxfSettings";
const _mT = "mimeType";
const _mTP = "mvTemporalPredictor";
const _mTRS = "minTopRenditionSize";
const _mVS = "multiViewSettings";
const _mX = "mccXml";
const _mYUV = "maximumYUV";
const _mYUVi = "minimumYUV";
const _ma = "mapping";
const _me = "messages";
const _met = "metadata3";
const _meta = "metadata4";
const _metad = "metadata5";
const _metada = "metadata6";
const _metadat = "metadata7";
const _metadata = "metadata8";
const _metadata_ = "metadata";
const _mo = "mode";
const _n = "numerator";
const _nBFBRF = "numberBFramesBetweenReferenceFrames";
const _nC = "nielsenConfiguration";
const _nFMS = "nexguardFileMarkerSettings";
const _nI = "networkId";
const _nIi = "nitInterval";
const _nIie = "nielsenId3";
const _nM = "nameModifier";
const _nN = "networkName";
const _nNLW = "nielsenNonLinearWatermark";
const _nPB = "nullPacketBitrate";
const _nR = "noiseReducer";
const _nRF = "numberReferenceFrames";
const _nT = "nextToken";
const _na = "name";
const _o = "offset";
const _oC = "objectCount";
const _oCS = "outputColorSpace";
const _oCu = "outlineColor";
const _oCut = "outputChannels";
const _oD = "outputDetails";
const _oE = "offlineEncrypted";
const _oG = "outputGroups";
const _oGD = "outputGroupDetails";
const _oGS = "outputGroupSettings";
const _oML = "outputMasteringLuminance";
const _oS = "opusSettings";
const _oSu = "outlineSize";
const _oSut = "outputSdt";
const _oSutp = "outputSelection";
const _oSutpu = "outputSettings";
const _oV = "overrideValue";
const _op = "opacity";
const _or = "order";
const _ou = "outputs";
const _p = "pids";
const _pA = "purchasedAt";
const _pBU = "preventBufferUnderflow";
const _pC = "passthroughControl";
const _pCa = "parControl";
const _pCad = "paddingControl";
const _pCc = "pcrControl";
const _pCe = "peakCalculation";
const _pCh = "phaseControl";
const _pCs = "psiControl";
const _pD = "parDenominator";
const _pDC = "playbackDeviceCompatibility";
const _pDT = "programDateTime";
const _pDTP = "programDateTimePeriod";
const _pFM = "perFrameMetrics";
const _pFSS = "postFilterSharpenStrength";
const _pI = "patInterval";
const _pIm = "pmtInterval";
const _pJC = "progressingJobsCount";
const _pMP = "privateMetadataPid";
const _pN = "parNumerator";
const _pNa = "pageNumber";
const _pNr = "programNumber";
const _pO = "ptsOffset";
const _pOHFBF = "ptsOffsetHandlingForBFrames";
const _pOM = "ptsOffsetMode";
const _pP = "pricingPlan";
const _pPc = "pcrPid";
const _pPm = "pmtPid";
const _pR = "probeResults";
const _pS = "programSelection";
const _pSa = "passthroughSettings";
const _pSr = "proresSettings";
const _pT = "pageTypes";
const _pTS = "postTemporalSharpening";
const _pTSS = "postTemporalSharpeningStrength";
const _pV = "padVideo";
const _pW = "partnerWatermarking";
const _pWHM = "progressiveWriteHlsManifest";
const _pa = "payload";
const _pi = "pid";
const _pl = "playback";
const _po = "policy";
const _pos = "position";
const _pr = "profile";
const _pre = "preset";
const _pref = "prefix";
const _pres = "presets";
const _pri = "priority";
const _q = "queue";
const _qQL = "qvbrQualityLevel";
const _qQLFT = "qvbrQualityLevelFineTune";
const _qS = "qvbrSettings";
const _qT = "queueTransitions";
const _qTL = "qualityTuningLevel";
const _qu = "quality";
const _que = "queues";
const _r = "required";
const _rC = "retryCount";
const _rCM = "rateControlMode";
const _rF = "rawFormat";
const _rGI = "renditionGroupId";
const _rI = "rapInterval";
const _rIe = "resourceId";
const _rLC = "renditionLanguageCode";
const _rM = "rateMode";
const _rN = "renditionName";
const _rP = "repeatPps";
const _rPS = "reservationPlanSettings";
const _rPX = "redPrimaryX";
const _rPY = "redPrimaryY";
const _rPe = "reservationPlan";
const _rRRA = "removeRubyReserveAttributes";
const _rS = "remixSettings";
const _rSP = "responseSignalPreroll";
const _rSe = "reservedSlots";
const _rT = "resourceTags";
const _rTA = "respondToAfd";
const _rTe = "renewalType";
const _re = "reference";
const _ro = "rotation";
const _rol = "role";
const _rot = "rotate";
const _rp = "rpu";
const _ru = "rules";
const _s = "smithy.ts.sdk.synthetic.com.amazonaws.mediaconvert";
const _sACN = "sourceAncillaryChannelNumber";
const _sAE = "saliencyAwareEncoding";
const _sAOFM = "sampleAdaptiveOffsetFilterMode";
const _sAP = "spekeAudioPreset";
const _sAQ = "spatialAdaptiveQuantization";
const _sAR = "sampleAspectRatio";
const _sB = "scalingBehavior";
const _sC = "shadowColor";
const _sCD = "sceneChangeDetect";
const _sCI = "supportCaseId";
const _sCN = "source608ChannelNumber";
const _sCe = "segmentControl";
const _sCt = "storageClass";
const _sD = "stereoDownmix";
const _sDS = "sccDestinationSettings";
const _sDSr = "srtDestinationSettings";
const _sE = "scte35Esam";
const _sEM = "surroundExMode";
const _sEP = "scte35EsamPid";
const _sF = "sourceFile";
const _sFS = "spatialFilterSettings";
const _sI = "sdtInterval";
const _sIR = "streamInfResolution";
const _sIn = "s3Inputs";
const _sIo = "sourceId";
const _sIu = "supplementalImps";
const _sIy = "systemIds";
const _sJC = "submittedJobsCount";
const _sKK = "signingKmsKey";
const _sKP = "spekeKeyProvider";
const _sKPt = "staticKeyProvider";
const _sKV = "staticKeyValue";
const _sL = "segmentLength";
const _sLC = "segmentLengthControl";
const _sM = "surroundMode";
const _sMe = "segmentModifier";
const _sMeg = "segmentationMarkers";
const _sMegm = "segmentationMode";
const _sN = "streamName";
const _sNe = "serviceName";
const _sNt = "streamNumber";
const _sO = "shadowOpacity";
const _sOe = "selectedOutputs";
const _sOer = "serviceOverrides";
const _sP = "smpte337Passthrough";
const _sPN = "serviceProviderName";
const _sPNi = "signalProcessingNotification";
const _sPPC = "slowPalPitchCorrection";
const _sPS = "segmentsPerSubdirectory";
const _sPc = "scte35Pid";
const _sPl = "slowPal";
const _sPt = "stylePassthrough";
const _sQ = "sourceQueue";
const _sR = "sampleRate";
const _sRC = "sampleRangeConversion";
const _sRQ = "simulateReservedQueue";
const _sRWL = "sdrReferenceWhiteLevel";
const _sRa = "sampleRange";
const _sS = "sourceSettings";
const _sSc = "scte35Source";
const _sSe = "s3Settings";
const _sSeg = "segmentationStyle";
const _sSh = "shareStatus";
const _sT = "selectorType";
const _sTCM = "scanTypeConversionMode";
const _sTN = "source608TrackNumber";
const _sTc = "scanType";
const _sTe = "segmentationTime";
const _sTo = "sourceType";
const _sTp = "speechThreshold";
const _sTt = "startTimecode";
const _sTta = "startTime";
const _sTu = "subtitlingType";
const _sTub = "submitTime";
const _sUI = "statusUpdateInterval";
const _sVP = "spekeVideoPreset";
const _sWS = "sourceWatermarkStatus";
const _sX = "sccXml";
const _sXO = "shadowXOffset";
const _sYO = "shadowYOffset";
const _sa = "saturation";
const _se = "server";
const _set = "settings";
const _sh = "sharpening";
const _sha = "sharpness";
const _sl = "slices";
const _so = "softness";
const _sou = "source";
const _sp = "specification";
const _spe = "speed";
const _st = "streams";
const _sta = "status";
const _star = "start";
const _str = "strength";
const _sy = "syntax";
const _t = "tracks";
const _tAQ = "temporalAdaptiveQuantization";
const _tB = "timecodeBurnin";
const _tBS = "treeBlockSize";
const _tC = "terminateCaptions";
const _tCJ = "totalConcurrentJobs";
const _tCi = "timecodeConfig";
const _tCr = "transferCharacteristics";
const _tD = "timeDelta";
const _tDCM = "targetDurationCompatibilityMode";
const _tDM = "timestampDeltaMilliseconds";
const _tDS = "teletextDestinationSettings";
const _tDSt = "ttmlDestinationSettings";
const _tDU = "timeDeltaUnits";
const _tDe = "textDecoration";
const _tFS = "temporalFilterSettings";
const _tH = "thumbnailHeight";
const _tHi = "tileHeight";
const _tI = "thumbnailInterval";
const _tId = "tdtInterval";
const _tIe = "temporalIds";
const _tIi = "timecodeInsertion";
const _tK = "tagKeys";
const _tL = "targetLkfs";
const _tLR = "targetLoudnessRange";
const _tM = "timedMetadata";
const _tMBV = "timedMetadataBoxVersion";
const _tMI = "timedMetadataInsertion";
const _tMIF = "timedMetadataId3Frame";
const _tMIP = "timedMetadataId3Period";
const _tMN = "targetMonitorNits";
const _tMP = "timedMetadataPid";
const _tMSIU = "timedMetadataSchemeIdUri";
const _tMV = "timedMetadataValue";
const _tMr = "trackMappings";
const _tN = "trackNumber";
const _tO = "timestampOffset";
const _tP = "tilePadding";
const _tPLT = "truePeakLimiterThreshold";
const _tS = "teletextSpacing";
const _tSI = "transportStreamId";
const _tSS = "teletextSourceSettings";
const _tSSr = "trackSourceSettings";
const _tSU = "ticServerUrl";
const _tSa = "tamsSettings";
const _tSi = "timecodeSource";
const _tSim = "timecodeStart";
const _tT = "trackType";
const _tTi = "timecodeTrack";
const _tW = "thumbnailWidth";
const _tWi = "tileWidth";
const _ta = "tags";
const _te = "telecine";
const _ti = "tiles";
const _tim = "timecode";
const _time = "timerange";
const _times = "timestamp";
const _timi = "timing";
const _tr = "transitions";
const _ty = "type";
const _u = "url";
const _uAW = "uncompressedAudioWrapping";
const _uCJ = "unallocatedConcurrentJobs";
const _uM = "userMetadata";
const _uS = "uncompressedSettings";
const _uST = "unregisteredSeiTimecode";
const _uSTLTT = "upconvertSTLToTeletext";
const _uTPAT = "uniqueTicPerAudioTrack";
const _un = "unit";
const _v = "version";
const _vA = "vchipAction";
const _vC = "vc3Class";
const _vCO = "videoCompositionOffsets";
const _vD = "videoDescription";
const _vDi = "videoDetails";
const _vG = "videoGenerator";
const _vO = "videoOverlays";
const _vP = "videoPid";
const _vPi = "videoProperties";
const _vPid = "videoPreprocessors";
const _vQ = "vbrQuality";
const _vS = "vorbisSettings";
const _vSM = "videoSelectorMode";
const _vSc = "vc3Settings";
const _vSi = "videoSelector";
const _vSp = "vp8Settings";
const _vSpe = "vp9Settings";
const _vTI = "videoTrackIndexes";
const _va = "values";
const _val = "value";
const _ve = "versions";
const _w = "width";
const _wDM = "writeDashManifest";
const _wDS = "webvttDestinationSettings";
const _wHM = "writeHlsManifest";
const _wHSS = "webvttHlsSourceSettings";
const _wIP = "widthInPx";
const _wM = "waitMinutes";
const _wMPT = "writeMp4PackagingType";
const _wPX = "whitePointX";
const _wPY = "whitePointY";
const _wS = "wavSettings";
const _wSTIR = "writeSegmentTimelineInRepresentation";
const _wa = "warnings";
const _war = "warning";
const _x = "x";
const _xC = "xavcClass";
const _xHICPS = "xavcHdIntraCbgProfileSettings";
const _xHPS = "xavcHdProfileSettings";
const _xICPS = "xavc4kIntraCbgProfileSettings";
const _xIVPS = "xavc4kIntraVbrProfileSettings";
const _xP = "xPosition";
const _xPS = "xavcProfileSettings";
const _xPSa = "xavc4kProfileSettings";
const _xS = "xavcSettings";
const _y = "y";
const _yP = "yPosition";
const n0 = "com.amazonaws.mediaconvert";

// smithy-typescript generated code
import { TypeRegistry } from "@smithy/core/schema";
import type {
  StaticErrorSchema,
  StaticListSchema,
  StaticMapSchema,
  StaticOperationSchema,
  StaticStructureSchema,
} from "@smithy/types";

import {
  BadRequestException,
  ConflictException,
  ForbiddenException,
  InternalServerErrorException,
  NotFoundException,
  ServiceQuotaExceededException,
  TooManyRequestsException,
  UnprocessableEntityException,
} from "../models/errors";
import { MediaConvertServiceException } from "../models/MediaConvertServiceException";

/* eslint no-var: 0 */
const _s_registry = new TypeRegistry(_s);
export var MediaConvertServiceException$: StaticErrorSchema = [-3, _s, "MediaConvertServiceException", 0, [], []];
_s_registry.registerError(MediaConvertServiceException$, MediaConvertServiceException);
const n0_registry = new TypeRegistry(n0);
export var BadRequestException$: StaticErrorSchema = [-3, n0, _BRE,
  { [_e]: _c, [_hE]: 400 },
  [_M],
  [[0, { [_jN]: _m }]]
];
n0_registry.registerError(BadRequestException$, BadRequestException);
export var ConflictException$: StaticErrorSchema = [-3, n0, _CE,
  { [_e]: _c, [_hE]: 409 },
  [_M],
  [[0, { [_jN]: _m }]]
];
n0_registry.registerError(ConflictException$, ConflictException);
export var ForbiddenException$: StaticErrorSchema = [-3, n0, _FE,
  { [_e]: _c, [_hE]: 403 },
  [_M],
  [[0, { [_jN]: _m }]]
];
n0_registry.registerError(ForbiddenException$, ForbiddenException);
export var InternalServerErrorException$: StaticErrorSchema = [-3, n0, _ISEE,
  { [_e]: _se, [_hE]: 500 },
  [_M],
  [[0, { [_jN]: _m }]]
];
n0_registry.registerError(InternalServerErrorException$, InternalServerErrorException);
export var NotFoundException$: StaticErrorSchema = [-3, n0, _NFE,
  { [_e]: _c, [_hE]: 404 },
  [_M],
  [[0, { [_jN]: _m }]]
];
n0_registry.registerError(NotFoundException$, NotFoundException);
export var ServiceQuotaExceededException$: StaticErrorSchema = [-3, n0, _SQEE,
  { [_e]: _c, [_hE]: 402 },
  [_M],
  [[0, { [_jN]: _m }]]
];
n0_registry.registerError(ServiceQuotaExceededException$, ServiceQuotaExceededException);
export var TooManyRequestsException$: StaticErrorSchema = [-3, n0, _TMRE,
  { [_e]: _c, [_hE]: 429 },
  [_M],
  [[0, { [_jN]: _m }]]
];
n0_registry.registerError(TooManyRequestsException$, TooManyRequestsException);
export var UnprocessableEntityException$: StaticErrorSchema = [-3, n0, _UEE,
  { [_e]: _c, [_hE]: 422 },
  [_M],
  [[0, { [_jN]: _m }]]
];
n0_registry.registerError(UnprocessableEntityException$, UnprocessableEntityException);
/**
 * TypeRegistry instances containing modeled errors.
 * @internal
 *
 */
export const errorTypeRegistries = [
  _s_registry,
  n0_registry,
]
export var AacSettings$: StaticStructureSchema = [3, n0, _AS,
  0,
  [_ADBM, _B, _CP, _CM, _LMM, _PC, _RI, _RCM, _RF, _SR, _S, _TLR, _VQ],
  [[0, { [_jN]: _aDBM }], [1, { [_jN]: _b }], [0, { [_jN]: _cP }], [0, { [_jN]: _cM }], [0, { [_jN]: _lMM }], [0, { [_jN]: _pC }], [1, { [_jN]: _rI }], [0, { [_jN]: _rCM }], [0, { [_jN]: _rF }], [1, { [_jN]: _sR }], [0, { [_jN]: _sp }], [1, { [_jN]: _tLR }], [0, { [_jN]: _vQ }]]
];
export var Ac3Settings$: StaticStructureSchema = [3, n0, _ASc,
  0,
  [_B, _BM, _CM, _D, _DRCL, _DRCP, _DRCR, _LF, _MC, _SR],
  [[1, { [_jN]: _b }], [0, { [_jN]: _bM }], [0, { [_jN]: _cM }], [1, { [_jN]: _d }], [0, { [_jN]: _dRCL }], [0, { [_jN]: _dRCP }], [0, { [_jN]: _dRCR }], [0, { [_jN]: _lF }], [0, { [_jN]: _mC }], [1, { [_jN]: _sR }]]
];
export var Ac4Settings$: StaticStructureSchema = [3, n0, _ASce,
  0,
  [_B, _BM, _CM, _DRCFPT, _DRCHT, _DRCPH, _DRCPS, _LRCML, _LRSML, _LRCMLt, _LRSMLt, _SR, _SD],
  [[1, { [_jN]: _b }], [0, { [_jN]: _bM }], [0, { [_jN]: _cM }], [0, { [_jN]: _dRCFPT }], [0, { [_jN]: _dRCHT }], [0, { [_jN]: _dRCPH }], [0, { [_jN]: _dRCPS }], [1, { [_jN]: _lRCML }], [1, { [_jN]: _lRSML }], [1, { [_jN]: _lRCMLt }], [1, { [_jN]: _lRSMLt }], [1, { [_jN]: _sR }], [0, { [_jN]: _sD }]]
];
export var AccelerationSettings$: StaticStructureSchema = [3, n0, _AScc,
  0,
  [_Mo],
  [[0, { [_jN]: _mo }]], 1
];
export var AdvancedInputFilterSettings$: StaticStructureSchema = [3, n0, _AIFS,
  0,
  [_AT, _Sh],
  [[0, { [_jN]: _aT }], [0, { [_jN]: _sh }]]
];
export var AiffSettings$: StaticStructureSchema = [3, n0, _ASi,
  0,
  [_BD, _C, _SR],
  [[1, { [_jN]: _bD }], [1, { [_jN]: _ch }], [1, { [_jN]: _sR }]]
];
export var AllowedRenditionSize$: StaticStructureSchema = [3, n0, _ARS,
  0,
  [_H, _R, _W],
  [[1, { [_jN]: _h }], [0, { [_jN]: _r }], [1, { [_jN]: _w }]]
];
export var AncillarySourceSettings$: StaticStructureSchema = [3, n0, _ASS,
  0,
  [_CT, _SACN, _TC],
  [[0, { [_jN]: _cT }], [1, { [_jN]: _sACN }], [0, { [_jN]: _tC }]]
];
export var AspectRatio$: StaticStructureSchema = [3, n0, _AR,
  0,
  [_De, _N],
  [[1, { [_jN]: _de }], [1, { [_jN]: _n }]]
];
export var AssociateCertificateRequest$: StaticStructureSchema = [3, n0, _ACR,
  0,
  [_A],
  [[0, { [_jN]: _a }]], 1
];
export var AssociateCertificateResponse$: StaticStructureSchema = [3, n0, _ACRs,
  0,
  [],
  []
];
export var AudioChannelTaggingSettings$: StaticStructureSchema = [3, n0, _ACTS,
  0,
  [_CTh, _CTha],
  [[0, { [_jN]: _cTh }], [64 | 0, { [_jN]: _cTha }]]
];
export var AudioCodecSettings$: StaticStructureSchema = [3, n0, _ACS,
  0,
  [_AS, _ASc, _ASce, _ASi, _Co, _EAS, _ES, _FS, _MS, _MSp, _OS, _VS, _WS],
  [[() => AacSettings$, { [_jN]: _aS }], [() => Ac3Settings$, { [_jN]: _aSc }], [() => Ac4Settings$, { [_jN]: _aSce }], [() => AiffSettings$, { [_jN]: _aSi }], [0, { [_jN]: _co }], [() => Eac3AtmosSettings$, { [_jN]: _eAS }], [() => Eac3Settings$, { [_jN]: _eS }], [() => FlacSettings$, { [_jN]: _fS }], [() => Mp2Settings$, { [_jN]: _mS }], [() => Mp3Settings$, { [_jN]: _mSp }], [() => OpusSettings$, { [_jN]: _oS }], [() => VorbisSettings$, { [_jN]: _vS }], [() => WavSettings$, { [_jN]: _wS }]]
];
export var AudioDescription$: StaticStructureSchema = [3, n0, _AD,
  0,
  [_ACTS, _ANS, _APCS, _ASN, _ATu, _ATC, _CS, _CLC, _LC, _LCC, _RS, _SN],
  [[() => AudioChannelTaggingSettings$, { [_jN]: _aCTS }], [() => AudioNormalizationSettings$, { [_jN]: _aNS }], [() => AudioPitchCorrectionSettings$, { [_jN]: _aPCS }], [0, { [_jN]: _aSN }], [1, { [_jN]: _aTu }], [0, { [_jN]: _aTC }], [() => AudioCodecSettings$, { [_jN]: _cS }], [0, { [_jN]: _cLC }], [0, { [_jN]: _lC }], [0, { [_jN]: _lCC }], [() => RemixSettings$, { [_jN]: _rS }], [0, { [_jN]: _sN }]]
];
export var AudioNormalizationSettings$: StaticStructureSchema = [3, n0, _ANS,
  0,
  [_Al, _AC, _CGL, _LL, _PCe, _TL, _TPLT],
  [[0, { [_jN]: _al }], [0, { [_jN]: _aC }], [1, { [_jN]: _cGL }], [0, { [_jN]: _lL }], [0, { [_jN]: _pCe }], [1, { [_jN]: _tL }], [1, { [_jN]: _tPLT }]]
];
export var AudioPitchCorrectionSettings$: StaticStructureSchema = [3, n0, _APCS,
  0,
  [_SPPC],
  [[0, { [_jN]: _sPPC }]]
];
export var AudioProperties$: StaticStructureSchema = [3, n0, _AP,
  0,
  [_BD, _BR, _CL, _C, _FR, _LC, _OC, _SR],
  [[1, { [_jN]: _bD }], [1, { [_jN]: _bR }], [0, { [_jN]: _cL }], [1, { [_jN]: _ch }], [() => FrameRate$, { [_jN]: _fR }], [0, { [_jN]: _lC }], [1, { [_jN]: _oC }], [1, { [_jN]: _sR }]]
];
export var AudioSelector$: StaticStructureSchema = [3, n0, _ASu,
  0,
  [_ADC, _CLC, _DS, _EAFI, _HRGS, _LC, _O, _P, _PS, _RS, _ST, _SP, _St, _T],
  [[0, { [_jN]: _aDC }], [0, { [_jN]: _cLC }], [0, { [_jN]: _dS }], [0, { [_jN]: _eAFI }], [() => HlsRenditionGroupSettings$, { [_jN]: _hRGS }], [0, { [_jN]: _lC }], [1, { [_jN]: _o }], [64 | 1, { [_jN]: _p }], [1, { [_jN]: _pS }], [() => RemixSettings$, { [_jN]: _rS }], [0, { [_jN]: _sT }], [0, { [_jN]: _sP }], [64 | 1, { [_jN]: _st }], [64 | 1, { [_jN]: _t }]]
];
export var AudioSelectorGroup$: StaticStructureSchema = [3, n0, _ASG,
  0,
  [_ASNu],
  [[64 | 0, { [_jN]: _aSNu }]]
];
export var AutomatedAbrRule$: StaticStructureSchema = [3, n0, _AAR,
  0,
  [_ARl, _FIR, _MBRS, _MTRS, _Ty],
  [[() => __listOfAllowedRenditionSize, { [_jN]: _aR }], [() => __listOfForceIncludeRenditionSize, { [_jN]: _fIR }], [() => MinBottomRenditionSize$, { [_jN]: _mBRS }], [() => MinTopRenditionSize$, { [_jN]: _mTRS }], [0, { [_jN]: _ty }]]
];
export var AutomatedAbrSettings$: StaticStructureSchema = [3, n0, _AAS,
  0,
  [_MAB, _MQL, _MR, _MABi, _Ru],
  [[1, { [_jN]: _mAB }], [1, { [_jN]: _mQL }], [1, { [_jN]: _mR }], [1, { [_jN]: _mABi }], [() => __listOfAutomatedAbrRule, { [_jN]: _ru }]]
];
export var AutomatedEncodingSettings$: StaticStructureSchema = [3, n0, _AES,
  0,
  [_ASb],
  [[() => AutomatedAbrSettings$, { [_jN]: _aSb }]]
];
export var Av1QvbrSettings$: StaticStructureSchema = [3, n0, _AQS,
  0,
  [_QQL, _QQLFT],
  [[1, { [_jN]: _qQL }], [1, { [_jN]: _qQLFT }]]
];
export var Av1Settings$: StaticStructureSchema = [3, n0, _ASv,
  0,
  [_AQ, _BD, _FGS, _FC, _FCA, _FD, _FN, _GS, _MB, _NBFBRF, _PFM, _QS, _RCM, _Sl, _SAQ],
  [[0, { [_jN]: _aQ }], [0, { [_jN]: _bD }], [0, { [_jN]: _fGS }], [0, { [_jN]: _fC }], [0, { [_jN]: _fCA }], [1, { [_jN]: _fD }], [1, { [_jN]: _fN }], [1, { [_jN]: _gS }], [1, { [_jN]: _mB }], [1, { [_jN]: _nBFBRF }], [64 | 0, { [_jN]: _pFM }], [() => Av1QvbrSettings$, { [_jN]: _qS }], [0, { [_jN]: _rCM }], [1, { [_jN]: _sl }], [0, { [_jN]: _sAQ }]]
];
export var AvailBlanking$: StaticStructureSchema = [3, n0, _AB,
  0,
  [_ABI],
  [[0, { [_jN]: _aBI }]]
];
export var AvcIntraSettings$: StaticStructureSchema = [3, n0, _AIS,
  0,
  [_AIC, _AIUS, _FC, _FCA, _FD, _FN, _IM, _PFM, _STCM, _SPl, _Te],
  [[0, { [_jN]: _aIC }], [() => AvcIntraUhdSettings$, { [_jN]: _aIUS }], [0, { [_jN]: _fC }], [0, { [_jN]: _fCA }], [1, { [_jN]: _fD }], [1, { [_jN]: _fN }], [0, { [_jN]: _iM }], [64 | 0, { [_jN]: _pFM }], [0, { [_jN]: _sTCM }], [0, { [_jN]: _sPl }], [0, { [_jN]: _te }]]
];
export var AvcIntraUhdSettings$: StaticStructureSchema = [3, n0, _AIUS,
  0,
  [_QTL],
  [[0, { [_jN]: _qTL }]]
];
export var BandwidthReductionFilter$: StaticStructureSchema = [3, n0, _BRF,
  0,
  [_Sh, _Str],
  [[0, { [_jN]: _sh }], [0, { [_jN]: _str }]]
];
export var BurninDestinationSettings$: StaticStructureSchema = [3, n0, _BDS,
  0,
  [_Ali, _AFC, _BC, _BO, _FF, _FCo, _FFB, _FFBI, _FFI, _FFR, _FO, _FRo, _FSo, _FSon, _HFC, _OCu, _OSu, _RRRA, _SC, _SO, _SXO, _SYO, _SPt, _TS, _XP, _YP],
  [[0, { [_jN]: _ali }], [0, { [_jN]: _aFC }], [0, { [_jN]: _bC }], [1, { [_jN]: _bO }], [0, { [_jN]: _fF }], [0, { [_jN]: _fCo }], [0, { [_jN]: _fFB }], [0, { [_jN]: _fFBI }], [0, { [_jN]: _fFI }], [0, { [_jN]: _fFR }], [1, { [_jN]: _fO }], [1, { [_jN]: _fRo }], [0, { [_jN]: _fSo }], [1, { [_jN]: _fSon }], [0, { [_jN]: _hFC }], [0, { [_jN]: _oCu }], [1, { [_jN]: _oSu }], [0, { [_jN]: _rRRA }], [0, { [_jN]: _sC }], [1, { [_jN]: _sO }], [1, { [_jN]: _sXO }], [1, { [_jN]: _sYO }], [0, { [_jN]: _sPt }], [0, { [_jN]: _tS }], [1, { [_jN]: _xP }], [1, { [_jN]: _yP }]]
];
export var CancelJobRequest$: StaticStructureSchema = [3, n0, _CJR,
  0,
  [_I],
  [[0, 1]], 1
];
export var CancelJobResponse$: StaticStructureSchema = [3, n0, _CJRa,
  0,
  [],
  []
];
export var CaptionDescription$: StaticStructureSchema = [3, n0, _CD,
  0,
  [_CSN, _CLC, _DSe, _LC, _LD],
  [[0, { [_jN]: _cSN }], [0, { [_jN]: _cLC }], [() => CaptionDestinationSettings$, { [_jN]: _dSe }], [0, { [_jN]: _lC }], [0, { [_jN]: _lD }]]
];
export var CaptionDescriptionPreset$: StaticStructureSchema = [3, n0, _CDP,
  0,
  [_CLC, _DSe, _LC, _LD],
  [[0, { [_jN]: _cLC }], [() => CaptionDestinationSettings$, { [_jN]: _dSe }], [0, { [_jN]: _lC }], [0, { [_jN]: _lD }]]
];
export var CaptionDestinationSettings$: StaticStructureSchema = [3, n0, _CDS,
  0,
  [_BDS, _DT, _DSDS, _EDS, _IDS, _SDS, _SDSr, _TDS, _TDSt, _WDS],
  [[() => BurninDestinationSettings$, { [_jN]: _bDS }], [0, { [_jN]: _dT }], [() => DvbSubDestinationSettings$, { [_jN]: _dSDS }], [() => EmbeddedDestinationSettings$, { [_jN]: _eDS }], [() => ImscDestinationSettings$, { [_jN]: _iDS }], [() => SccDestinationSettings$, { [_jN]: _sDS }], [() => SrtDestinationSettings$, { [_jN]: _sDSr }], [() => TeletextDestinationSettings$, { [_jN]: _tDS }], [() => TtmlDestinationSettings$, { [_jN]: _tDSt }], [() => WebvttDestinationSettings$, { [_jN]: _wDS }]]
];
export var CaptionSelector$: StaticStructureSchema = [3, n0, _CSa,
  0,
  [_CLC, _LC, _SS],
  [[0, { [_jN]: _cLC }], [0, { [_jN]: _lC }], [() => CaptionSourceSettings$, { [_jN]: _sS }]]
];
export var CaptionSourceFramerate$: StaticStructureSchema = [3, n0, _CSF,
  0,
  [_FD, _FN],
  [[1, { [_jN]: _fD }], [1, { [_jN]: _fN }]]
];
export var CaptionSourceSettings$: StaticStructureSchema = [3, n0, _CSS,
  0,
  [_ASS, _DSSS, _ESS, _FSS, _STo, _TSS, _TSSr, _WHSS],
  [[() => AncillarySourceSettings$, { [_jN]: _aSS }], [() => DvbSubSourceSettings$, { [_jN]: _dSSS }], [() => EmbeddedSourceSettings$, { [_jN]: _eSS }], [() => FileSourceSettings$, { [_jN]: _fSS }], [0, { [_jN]: _sTo }], [() => TeletextSourceSettings$, { [_jN]: _tSS }], [() => TrackSourceSettings$, { [_jN]: _tSSr }], [() => WebvttHlsSourceSettings$, { [_jN]: _wHSS }]]
];
export var ChannelMapping$: StaticStructureSchema = [3, n0, _CMh,
  0,
  [_OCut],
  [[() => __listOfOutputChannelMapping, { [_jN]: _oCut }]]
];
export var ClipLimits$: StaticStructureSchema = [3, n0, _CLl,
  0,
  [_MRGBT, _MYUV, _MRGBTi, _MYUVi],
  [[1, { [_jN]: _mRGBT }], [1, { [_jN]: _mYUV }], [1, { [_jN]: _mRGBTi }], [1, { [_jN]: _mYUVi }]]
];
export var CmafAdditionalManifest$: StaticStructureSchema = [3, n0, _CAM,
  0,
  [_MNM, _SOe],
  [[0, { [_jN]: _mNM }], [64 | 0, { [_jN]: _sOe }]]
];
export var CmafEncryptionSettings$: StaticStructureSchema = [3, n0, _CES,
  0,
  [_CLS, _CIV, _EM, _IVIM, _SKP, _SKPt, _Ty],
  [[1, { [_jN]: _cLS }], [0, { [_jN]: _cIV }], [0, { [_jN]: _eM }], [0, { [_jN]: _iVIM }], [() => SpekeKeyProviderCmaf$, { [_jN]: _sKP }], [() => StaticKeyProvider$, { [_jN]: _sKPt }], [0, { [_jN]: _ty }]]
];
export var CmafGroupSettings$: StaticStructureSchema = [3, n0, _CGS,
  0,
  [_AM, _BU, _CC, _CSo, _DIFTPNM, _DMS, _Des, _DSe, _E, _FL, _IBTP, _IBTPS, _IBTPV, _MCa, _MDF, _MBT, _MFSL, _MMBT, _MP, _POHFBF, _SCe, _SL, _SLC, _SIR, _TDCM, _VCO, _WDM, _WHM, _WSTIR],
  [[() => __listOfCmafAdditionalManifest, { [_jN]: _aM }], [0, { [_jN]: _bU }], [0, { [_jN]: _cC }], [0, { [_jN]: _cSo }], [0, { [_jN]: _dIFTPNM }], [0, { [_jN]: _dMS }], [0, { [_jN]: _des }], [() => DestinationSettings$, { [_jN]: _dSe }], [() => CmafEncryptionSettings$, { [_jN]: _en }], [1, { [_jN]: _fL }], [0, { [_jN]: _iBTP }], [() => CmafImageBasedTrickPlaySettings$, { [_jN]: _iBTPS }], [() => __listOfCmafImageBasedTrickPlayVariant, { [_jN]: _iBTPV }], [0, { [_jN]: _mCa }], [0, { [_jN]: _mDF }], [1, { [_jN]: _mBT }], [1, { [_jN]: _mFSL }], [0, { [_jN]: _mMBT }], [0, { [_jN]: _mP }], [0, { [_jN]: _pOHFBF }], [0, { [_jN]: _sCe }], [1, { [_jN]: _sL }], [0, { [_jN]: _sLC }], [0, { [_jN]: _sIR }], [0, { [_jN]: _tDCM }], [0, { [_jN]: _vCO }], [0, { [_jN]: _wDM }], [0, { [_jN]: _wHM }], [0, { [_jN]: _wSTIR }]]
];
export var CmafImageBasedTrickPlaySettings$: StaticStructureSchema = [3, n0, _CIBTPS,
  0,
  [_IC, _TH, _TI, _TW, _THi, _TWi],
  [[0, { [_jN]: _iC }], [1, { [_jN]: _tH }], [1, { [_jN]: _tI }], [1, { [_jN]: _tW }], [1, { [_jN]: _tHi }], [1, { [_jN]: _tWi }]]
];
export var CmafImageBasedTrickPlayVariant$: StaticStructureSchema = [3, n0, _CIBTPV,
  0,
  [_IC, _TH, _TI, _TW, _THi, _TWi],
  [[0, { [_jN]: _iC }], [1, { [_jN]: _tH }], [1, { [_jN]: _tI }], [1, { [_jN]: _tW }], [1, { [_jN]: _tHi }], [1, { [_jN]: _tWi }]]
];
export var CmfcSettings$: StaticStructureSchema = [3, n0, _CSm,
  0,
  [_ADu, _AGI, _ARSu, _ATT, _CMp, _CSe, _DVSF, _IFOM, _KM, _MMS, _SE, _SSc, _SKK, _TM, _TMBV, _TMSIU, _TMV],
  [[0, { [_jN]: _aD }], [0, { [_jN]: _aGI }], [0, { [_jN]: _aRS }], [0, { [_jN]: _aTT }], [0, { [_jN]: _cMp }], [0, { [_jN]: _cSe }], [0, { [_jN]: _dVSF }], [0, { [_jN]: _iFOM }], [0, { [_jN]: _kM }], [0, { [_jN]: _mMS }], [0, { [_jN]: _sE }], [0, { [_jN]: _sSc }], [0, { [_jN]: _sKK }], [0, { [_jN]: _tM }], [0, { [_jN]: _tMBV }], [0, { [_jN]: _tMSIU }], [0, { [_jN]: _tMV }]]
];
export var CodecMetadata$: StaticStructureSchema = [3, n0, _CMo,
  0,
  [_BD, _CSh, _CFR, _CPo, _CLL, _DAR, _DV, _FOi, _HPP, _H, _L, _MCat, _Pr, _Ro, _SAR, _STc, _TCr, _W],
  [[1, { [_jN]: _bD }], [0, { [_jN]: _cSh }], [() => FrameRate$, { [_jN]: _cFR }], [0, { [_jN]: _cPo }], [() => ContentLightLevel$, { [_jN]: _cLL }], [() => AspectRatio$, { [_jN]: _dAR }], [() => DolbyVisionMetadata$, { [_jN]: _dV }], [0, { [_jN]: _fOi }], [0, { [_jN]: _hPP }], [1, { [_jN]: _h }], [0, { [_jN]: _l }], [0, { [_jN]: _mCat }], [0, { [_jN]: _pr }], [1, { [_jN]: _ro }], [() => AspectRatio$, { [_jN]: _sAR }], [0, { [_jN]: _sTc }], [0, { [_jN]: _tCr }], [1, { [_jN]: _w }]]
];
export var ColorConversion3DLUTSetting$: StaticStructureSchema = [3, n0, _CCDLUTS,
  0,
  [_FI, _ICS, _IML, _OCS, _OML],
  [[0, { [_jN]: _fI }], [0, { [_jN]: _iCS }], [1, { [_jN]: _iML }], [0, { [_jN]: _oCS }], [1, { [_jN]: _oML }]]
];
export var ColorCorrector$: StaticStructureSchema = [3, n0, _CCo,
  0,
  [_Br, _CLl, _CSC, _Con, _HM, _HTSTM, _Hu, _ML, _SRC, _Sa, _SRWL],
  [[1, { [_jN]: _br }], [() => ClipLimits$, { [_jN]: _cLl }], [0, { [_jN]: _cSC }], [1, { [_jN]: _con }], [() => Hdr10Metadata$, { [_jN]: _hM }], [0, { [_jN]: _hTSTM }], [1, { [_jN]: _hu }], [1, { [_jN]: _mL }], [0, { [_jN]: _sRC }], [1, { [_jN]: _sa }], [1, { [_jN]: _sRWL }]]
];
export var Container$: StaticStructureSchema = [3, n0, _Cont,
  0,
  [_BR, _Du, _F, _STt, _T],
  [[1, { [_jN]: _bR }], [1, { [_jN]: _du }], [0, { [_jN]: _f }], [0, { [_jN]: _sTt }], [() => __listOfTrack, { [_jN]: _t }]]
];
export var ContainerSettings$: StaticStructureSchema = [3, n0, _CSon,
  0,
  [_CSm, _Cont, _FSv, _MSt, _MSu, _MSo, _MSpe, _MSpd, _MSx],
  [[() => CmfcSettings$, { [_jN]: _cSm }], [0, { [_jN]: _cont }], [() => F4vSettings$, { [_jN]: _fSv }], [() => M2tsSettings$, { [_jN]: _mSt }], [() => M3u8Settings$, { [_jN]: _mSu }], [() => MovSettings$, { [_jN]: _mSo }], [() => Mp4Settings$, { [_jN]: _mSpe }], [() => MpdSettings$, { [_jN]: _mSpd }], [() => MxfSettings$, { [_jN]: _mSx }]]
];
export var ContentLightLevel$: StaticStructureSchema = [3, n0, _CLL,
  0,
  [_MCLL, _MFALL],
  [[1, { [_jN]: _mCLL }], [1, { [_jN]: _mFALL }]]
];
export var CreateJobRequest$: StaticStructureSchema = [3, n0, _CJRr,
  0,
  [_Rol, _Se, _AScc, _BTS, _CRT, _HD, _JEV, _JT, _Pri, _Q, _SRQ, _SUI, _Ta, _UM],
  [[0, { [_jN]: _rol }], [() => JobSettings$, { [_jN]: _set }], [() => AccelerationSettings$, { [_jN]: _aScc }], [0, { [_jN]: _bTS }], [0, { [_iT]: 1, [_jN]: _cRT }], [() => __listOfHopDestination, { [_jN]: _hD }], [0, { [_jN]: _jEV }], [0, { [_jN]: _jT }], [1, { [_jN]: _pri }], [0, { [_jN]: _q }], [0, { [_jN]: _sRQ }], [0, { [_jN]: _sUI }], [128 | 0, { [_jN]: _ta }], [128 | 0, { [_jN]: _uM }]], 2
];
export var CreateJobResponse$: StaticStructureSchema = [3, n0, _CJRre,
  0,
  [_J],
  [[() => Job$, { [_jN]: _j }]]
];
export var CreateJobTemplateRequest$: StaticStructureSchema = [3, n0, _CJTR,
  0,
  [_Na, _Se, _AScc, _Ca, _Desc, _HD, _Pri, _Q, _SUI, _Ta],
  [[0, { [_jN]: _na }], [() => JobTemplateSettings$, { [_jN]: _set }], [() => AccelerationSettings$, { [_jN]: _aScc }], [0, { [_jN]: _ca }], [0, { [_jN]: _desc }], [() => __listOfHopDestination, { [_jN]: _hD }], [1, { [_jN]: _pri }], [0, { [_jN]: _q }], [0, { [_jN]: _sUI }], [128 | 0, { [_jN]: _ta }]], 2
];
export var CreateJobTemplateResponse$: StaticStructureSchema = [3, n0, _CJTRr,
  0,
  [_JT],
  [[() => JobTemplate$, { [_jN]: _jT }]]
];
export var CreatePresetRequest$: StaticStructureSchema = [3, n0, _CPR,
  0,
  [_Na, _Se, _Ca, _Desc, _Ta],
  [[0, { [_jN]: _na }], [() => PresetSettings$, { [_jN]: _set }], [0, { [_jN]: _ca }], [0, { [_jN]: _desc }], [128 | 0, { [_jN]: _ta }]], 2
];
export var CreatePresetResponse$: StaticStructureSchema = [3, n0, _CPRr,
  0,
  [_Pre],
  [[() => Preset$, { [_jN]: _pre }]]
];
export var CreateQueueRequest$: StaticStructureSchema = [3, n0, _CQR,
  0,
  [_Na, _CJ, _Desc, _MCF, _PP, _RPS, _Sta, _Ta],
  [[0, { [_jN]: _na }], [1, { [_jN]: _cJ }], [0, { [_jN]: _desc }], [1, { [_jN]: _mCF }], [0, { [_jN]: _pP }], [() => ReservationPlanSettings$, { [_jN]: _rPS }], [0, { [_jN]: _sta }], [128 | 0, { [_jN]: _ta }]], 1
];
export var CreateQueueResponse$: StaticStructureSchema = [3, n0, _CQRr,
  0,
  [_Q],
  [[() => Queue$, { [_jN]: _q }]]
];
export var CreateResourceShareRequest$: StaticStructureSchema = [3, n0, _CRSR,
  0,
  [_JI, _SCI],
  [[0, { [_jN]: _jI }], [0, { [_jN]: _sCI }]], 2
];
export var CreateResourceShareResponse$: StaticStructureSchema = [3, n0, _CRSRr,
  0,
  [],
  []
];
export var DashAdditionalManifest$: StaticStructureSchema = [3, n0, _DAM,
  0,
  [_MNM, _SOe],
  [[0, { [_jN]: _mNM }], [64 | 0, { [_jN]: _sOe }]]
];
export var DashIsoEncryptionSettings$: StaticStructureSchema = [3, n0, _DIES,
  0,
  [_PDC, _SKP],
  [[0, { [_jN]: _pDC }], [() => SpekeKeyProvider$, { [_jN]: _sKP }]]
];
export var DashIsoGroupSettings$: StaticStructureSchema = [3, n0, _DIGS,
  0,
  [_AM, _ACCSIU, _BU, _DIFTPNM, _DMS, _Des, _DSe, _E, _FL, _HC, _IBTP, _IBTPS, _IBTPV, _MBT, _MFSL, _MMBT, _MP, _POHFBF, _SCe, _SL, _SLC, _VCO, _WSTIR],
  [[() => __listOfDashAdditionalManifest, { [_jN]: _aM }], [0, { [_jN]: _aCCSIU }], [0, { [_jN]: _bU }], [0, { [_jN]: _dIFTPNM }], [0, { [_jN]: _dMS }], [0, { [_jN]: _des }], [() => DestinationSettings$, { [_jN]: _dSe }], [() => DashIsoEncryptionSettings$, { [_jN]: _en }], [1, { [_jN]: _fL }], [0, { [_jN]: _hC }], [0, { [_jN]: _iBTP }], [() => DashIsoImageBasedTrickPlaySettings$, { [_jN]: _iBTPS }], [() => __listOfDashIsoImageBasedTrickPlayVariant, { [_jN]: _iBTPV }], [1, { [_jN]: _mBT }], [1, { [_jN]: _mFSL }], [0, { [_jN]: _mMBT }], [0, { [_jN]: _mP }], [0, { [_jN]: _pOHFBF }], [0, { [_jN]: _sCe }], [1, { [_jN]: _sL }], [0, { [_jN]: _sLC }], [0, { [_jN]: _vCO }], [0, { [_jN]: _wSTIR }]]
];
export var DashIsoImageBasedTrickPlaySettings$: StaticStructureSchema = [3, n0, _DIIBTPS,
  0,
  [_IC, _TH, _TI, _TW, _THi, _TWi],
  [[0, { [_jN]: _iC }], [1, { [_jN]: _tH }], [1, { [_jN]: _tI }], [1, { [_jN]: _tW }], [1, { [_jN]: _tHi }], [1, { [_jN]: _tWi }]]
];
export var DashIsoImageBasedTrickPlayVariant$: StaticStructureSchema = [3, n0, _DIIBTPV,
  0,
  [_IC, _TH, _TI, _TW, _THi, _TWi],
  [[0, { [_jN]: _iC }], [1, { [_jN]: _tH }], [1, { [_jN]: _tI }], [1, { [_jN]: _tW }], [1, { [_jN]: _tHi }], [1, { [_jN]: _tWi }]]
];
export var DataProperties$: StaticStructureSchema = [3, n0, _DP,
  0,
  [_LC],
  [[0, { [_jN]: _lC }]]
];
export var Deinterlacer$: StaticStructureSchema = [3, n0, _Dei,
  0,
  [_Al, _Contr, _Mo],
  [[0, { [_jN]: _al }], [0, { [_jN]: _contr }], [0, { [_jN]: _mo }]]
];
export var DeleteJobTemplateRequest$: StaticStructureSchema = [3, n0, _DJTR,
  0,
  [_Na],
  [[0, 1]], 1
];
export var DeleteJobTemplateResponse$: StaticStructureSchema = [3, n0, _DJTRe,
  0,
  [],
  []
];
export var DeletePolicyRequest$: StaticStructureSchema = [3, n0, _DPR,
  0,
  [],
  []
];
export var DeletePolicyResponse$: StaticStructureSchema = [3, n0, _DPRe,
  0,
  [],
  []
];
export var DeletePresetRequest$: StaticStructureSchema = [3, n0, _DPRel,
  0,
  [_Na],
  [[0, 1]], 1
];
export var DeletePresetResponse$: StaticStructureSchema = [3, n0, _DPRele,
  0,
  [],
  []
];
export var DeleteQueueRequest$: StaticStructureSchema = [3, n0, _DQR,
  0,
  [_Na],
  [[0, 1]], 1
];
export var DeleteQueueResponse$: StaticStructureSchema = [3, n0, _DQRe,
  0,
  [],
  []
];
export var DescribeEndpointsRequest$: StaticStructureSchema = [3, n0, _DER,
  0,
  [_MRa, _Mo, _NT],
  [[1, { [_jN]: _mRa }], [0, { [_jN]: _mo }], [0, { [_jN]: _nT }]]
];
export var DescribeEndpointsResponse$: StaticStructureSchema = [3, n0, _DERe,
  0,
  [_En, _NT],
  [[() => __listOfEndpoint, { [_jN]: _end }], [0, { [_jN]: _nT }]]
];
export var DestinationSettings$: StaticStructureSchema = [3, n0, _DSe,
  0,
  [_SSe],
  [[() => S3DestinationSettings$, { [_jN]: _sSe }]]
];
export var DisassociateCertificateRequest$: StaticStructureSchema = [3, n0, _DCR,
  0,
  [_A],
  [[0, 1]], 1
];
export var DisassociateCertificateResponse$: StaticStructureSchema = [3, n0, _DCRi,
  0,
  [],
  []
];
export var DolbyVision$: StaticStructureSchema = [3, n0, _DV,
  0,
  [_Com, _LM, _LMo, _Ma, _Pr],
  [[0, { [_jN]: _com }], [() => DolbyVisionLevel6Metadata$, { [_jN]: _lM }], [0, { [_jN]: _lMo }], [0, { [_jN]: _ma }], [0, { [_jN]: _pr }]]
];
export var DolbyVisionLevel6Metadata$: StaticStructureSchema = [3, n0, _DVLM,
  0,
  [_MCax, _MF],
  [[1, { [_jN]: _mCax }], [1, { [_jN]: _mF }]]
];
export var DolbyVisionMetadata$: StaticStructureSchema = [3, n0, _DVM,
  0,
  [_BL, _EL, _L, _Pr, _Rp],
  [[0, { [_jN]: _bL }], [0, { [_jN]: _eL }], [1, { [_jN]: _l }], [1, { [_jN]: _pr }], [0, { [_jN]: _rp }]]
];
export var DurationControl$: StaticStructureSchema = [3, n0, _DC,
  0,
  [_IDMCD, _IDMCN, _IDTTM],
  [[1, { [_jN]: _iDMCD }], [1, { [_jN]: _iDMCN }], [1, { [_jN]: _iDTTM }]]
];
export var DvbNitSettings$: StaticStructureSchema = [3, n0, _DNS,
  0,
  [_NI, _NN, _NIi],
  [[1, { [_jN]: _nI }], [0, { [_jN]: _nN }], [1, { [_jN]: _nIi }]]
];
export var DvbSdtSettings$: StaticStructureSchema = [3, n0, _DSS,
  0,
  [_OSut, _SI, _SNe, _SPN],
  [[0, { [_jN]: _oSut }], [1, { [_jN]: _sI }], [0, { [_jN]: _sNe }], [0, { [_jN]: _sPN }]]
];
export var DvbSubDestinationSettings$: StaticStructureSchema = [3, n0, _DSDS,
  0,
  [_Ali, _AFC, _BC, _BO, _DH, _DXC, _DYC, _FF, _FCo, _FFB, _FFBI, _FFI, _FFR, _FO, _FRo, _FSo, _FSon, _H, _HFC, _OCu, _OSu, _SC, _SO, _SXO, _SYO, _SPt, _STu, _TS, _W, _XP, _YP],
  [[0, { [_jN]: _ali }], [0, { [_jN]: _aFC }], [0, { [_jN]: _bC }], [1, { [_jN]: _bO }], [0, { [_jN]: _dH }], [1, { [_jN]: _dXC }], [1, { [_jN]: _dYC }], [0, { [_jN]: _fF }], [0, { [_jN]: _fCo }], [0, { [_jN]: _fFB }], [0, { [_jN]: _fFBI }], [0, { [_jN]: _fFI }], [0, { [_jN]: _fFR }], [1, { [_jN]: _fO }], [1, { [_jN]: _fRo }], [0, { [_jN]: _fSo }], [1, { [_jN]: _fSon }], [1, { [_jN]: _h }], [0, { [_jN]: _hFC }], [0, { [_jN]: _oCu }], [1, { [_jN]: _oSu }], [0, { [_jN]: _sC }], [1, { [_jN]: _sO }], [1, { [_jN]: _sXO }], [1, { [_jN]: _sYO }], [0, { [_jN]: _sPt }], [0, { [_jN]: _sTu }], [0, { [_jN]: _tS }], [1, { [_jN]: _w }], [1, { [_jN]: _xP }], [1, { [_jN]: _yP }]]
];
export var DvbSubSourceSettings$: StaticStructureSchema = [3, n0, _DSSS,
  0,
  [_Pi],
  [[1, { [_jN]: _pi }]]
];
export var DvbTdtSettings$: StaticStructureSchema = [3, n0, _DTS,
  0,
  [_TId],
  [[1, { [_jN]: _tId }]]
];
export var DynamicAudioSelector$: StaticStructureSchema = [3, n0, _DAS,
  0,
  [_ADC, _EAFI, _LC, _O, _ST],
  [[0, { [_jN]: _aDC }], [0, { [_jN]: _eAFI }], [0, { [_jN]: _lC }], [1, { [_jN]: _o }], [0, { [_jN]: _sT }]]
];
export var Eac3AtmosSettings$: StaticStructureSchema = [3, n0, _EAS,
  0,
  [_B, _BM, _CM, _DI, _DCo, _DRCL, _DRCR, _DRC, _LRCML, _LRSML, _LRCMLt, _LRSMLt, _MM, _SR, _STp, _SD, _SEM],
  [[1, { [_jN]: _b }], [0, { [_jN]: _bM }], [0, { [_jN]: _cM }], [0, { [_jN]: _dI }], [0, { [_jN]: _dC }], [0, { [_jN]: _dRCL }], [0, { [_jN]: _dRCR }], [0, { [_jN]: _dRC }], [1, { [_jN]: _lRCML }], [1, { [_jN]: _lRSML }], [1, { [_jN]: _lRCMLt }], [1, { [_jN]: _lRSMLt }], [0, { [_jN]: _mM }], [1, { [_jN]: _sR }], [1, { [_jN]: _sTp }], [0, { [_jN]: _sD }], [0, { [_jN]: _sEM }]]
];
export var Eac3Settings$: StaticStructureSchema = [3, n0, _ES,
  0,
  [_ACt, _B, _BM, _CM, _DF, _D, _DRCL, _DRCR, _LCf, _LF, _LRCML, _LRSML, _LRCMLt, _LRSMLt, _MC, _PC, _PCh, _SR, _SD, _SEM, _SM],
  [[0, { [_jN]: _aCt }], [1, { [_jN]: _b }], [0, { [_jN]: _bM }], [0, { [_jN]: _cM }], [0, { [_jN]: _dF }], [1, { [_jN]: _d }], [0, { [_jN]: _dRCL }], [0, { [_jN]: _dRCR }], [0, { [_jN]: _lCf }], [0, { [_jN]: _lF }], [1, { [_jN]: _lRCML }], [1, { [_jN]: _lRSML }], [1, { [_jN]: _lRCMLt }], [1, { [_jN]: _lRSMLt }], [0, { [_jN]: _mC }], [0, { [_jN]: _pC }], [0, { [_jN]: _pCh }], [1, { [_jN]: _sR }], [0, { [_jN]: _sD }], [0, { [_jN]: _sEM }], [0, { [_jN]: _sM }]]
];
export var ElementalInferenceConfiguration$: StaticStructureSchema = [3, n0, _EIC,
  0,
  [_Fe, _Fee],
  [[64 | 0, { [_jN]: _fe }], [() => __listOfElementalInferenceFeed, { [_jN]: _fee }]]
];
export var ElementalInferenceFeed$: StaticStructureSchema = [3, n0, _EIF,
  0,
  [_A, _FMS],
  [[0, { [_jN]: _a }], [0, { [_jN]: _fMS }]]
];
export var EmbeddedDestinationSettings$: StaticStructureSchema = [3, n0, _EDS,
  0,
  [_DCN, _DSN],
  [[1, { [_jN]: _dCN }], [1, { [_jN]: _dSN }]]
];
export var EmbeddedSourceSettings$: StaticStructureSchema = [3, n0, _ESS,
  0,
  [_CT, _SCN, _STN, _TC],
  [[0, { [_jN]: _cT }], [1, { [_jN]: _sCN }], [1, { [_jN]: _sTN }], [0, { [_jN]: _tC }]]
];
export var EncryptionContractConfiguration$: StaticStructureSchema = [3, n0, _ECC,
  0,
  [_SAP, _SVP],
  [[0, { [_jN]: _sAP }], [0, { [_jN]: _sVP }]]
];
export var Endpoint$: StaticStructureSchema = [3, n0, _End,
  0,
  [_U],
  [[0, { [_jN]: _u }]]
];
export var EsamManifestConfirmConditionNotification$: StaticStructureSchema = [3, n0, _EMCCN,
  0,
  [_MX],
  [[0, { [_jN]: _mX }]]
];
export var EsamSettings$: StaticStructureSchema = [3, n0, _ESs,
  0,
  [_MCCN, _RSP, _SPNi],
  [[() => EsamManifestConfirmConditionNotification$, { [_jN]: _mCCN }], [1, { [_jN]: _rSP }], [() => EsamSignalProcessingNotification$, { [_jN]: _sPNi }]]
];
export var EsamSignalProcessingNotification$: StaticStructureSchema = [3, n0, _ESPN,
  0,
  [_SX],
  [[0, { [_jN]: _sX }]]
];
export var ExtendedDataServices$: StaticStructureSchema = [3, n0, _EDSx,
  0,
  [_CPA, _VA],
  [[0, { [_jN]: _cPA }], [0, { [_jN]: _vA }]]
];
export var F4vSettings$: StaticStructureSchema = [3, n0, _FSv,
  0,
  [_MPo],
  [[0, { [_jN]: _mPo }]]
];
export var FileGroupSettings$: StaticStructureSchema = [3, n0, _FGSi,
  0,
  [_Des, _DSe],
  [[0, { [_jN]: _des }], [() => DestinationSettings$, { [_jN]: _dSe }]]
];
export var FileSourceSettings$: StaticStructureSchema = [3, n0, _FSS,
  0,
  [_BRL, _CT, _CPTP, _Fr, _SF, _TD, _TDU, _USTLTT],
  [[0, { [_jN]: _bRL }], [0, { [_jN]: _cT }], [0, { [_jN]: _cPTP }], [() => CaptionSourceFramerate$, { [_jN]: _fr }], [0, { [_jN]: _sF }], [1, { [_jN]: _tD }], [0, { [_jN]: _tDU }], [0, { [_jN]: _uSTLTT }]]
];
export var FlacSettings$: StaticStructureSchema = [3, n0, _FS,
  0,
  [_BD, _C, _SR],
  [[1, { [_jN]: _bD }], [1, { [_jN]: _ch }], [1, { [_jN]: _sR }]]
];
export var ForceIncludeRenditionSize$: StaticStructureSchema = [3, n0, _FIRS,
  0,
  [_H, _W],
  [[1, { [_jN]: _h }], [1, { [_jN]: _w }]]
];
export var FrameCaptureSettings$: StaticStructureSchema = [3, n0, _FCS,
  0,
  [_FD, _FN, _MCaxa, _Qu],
  [[1, { [_jN]: _fD }], [1, { [_jN]: _fN }], [1, { [_jN]: _mCaxa }], [1, { [_jN]: _qu }]]
];
export var FrameRate$: StaticStructureSchema = [3, n0, _FR,
  0,
  [_De, _N],
  [[1, { [_jN]: _de }], [1, { [_jN]: _n }]]
];
export var GetJobRequest$: StaticStructureSchema = [3, n0, _GJR,
  0,
  [_I],
  [[0, 1]], 1
];
export var GetJobResponse$: StaticStructureSchema = [3, n0, _GJRe,
  0,
  [_J],
  [[() => Job$, { [_jN]: _j }]]
];
export var GetJobsQueryResultsRequest$: StaticStructureSchema = [3, n0, _GJQRR,
  0,
  [_I],
  [[0, 1]], 1
];
export var GetJobsQueryResultsResponse$: StaticStructureSchema = [3, n0, _GJQRRe,
  0,
  [_Jo, _NT, _Sta],
  [[() => __listOfJob, { [_jN]: _jo }], [0, { [_jN]: _nT }], [0, { [_jN]: _sta }]]
];
export var GetJobTemplateRequest$: StaticStructureSchema = [3, n0, _GJTR,
  0,
  [_Na],
  [[0, 1]], 1
];
export var GetJobTemplateResponse$: StaticStructureSchema = [3, n0, _GJTRe,
  0,
  [_JT],
  [[() => JobTemplate$, { [_jN]: _jT }]]
];
export var GetPolicyRequest$: StaticStructureSchema = [3, n0, _GPR,
  0,
  [],
  []
];
export var GetPolicyResponse$: StaticStructureSchema = [3, n0, _GPRe,
  0,
  [_Po],
  [[() => Policy$, { [_jN]: _po }]]
];
export var GetPresetRequest$: StaticStructureSchema = [3, n0, _GPRet,
  0,
  [_Na],
  [[0, 1]], 1
];
export var GetPresetResponse$: StaticStructureSchema = [3, n0, _GPRetr,
  0,
  [_Pre],
  [[() => Preset$, { [_jN]: _pre }]]
];
export var GetQueueRequest$: StaticStructureSchema = [3, n0, _GQR,
  0,
  [_Na],
  [[0, 1]], 1
];
export var GetQueueResponse$: StaticStructureSchema = [3, n0, _GQRe,
  0,
  [_Q],
  [[() => Queue$, { [_jN]: _q }]]
];
export var GifSettings$: StaticStructureSchema = [3, n0, _GSi,
  0,
  [_FC, _FCA, _FD, _FN],
  [[0, { [_jN]: _fC }], [0, { [_jN]: _fCA }], [1, { [_jN]: _fD }], [1, { [_jN]: _fN }]]
];
export var H264QvbrSettings$: StaticStructureSchema = [3, n0, _HQS,
  0,
  [_MABa, _QQL, _QQLFT],
  [[1, { [_jN]: _mABa }], [1, { [_jN]: _qQL }], [1, { [_jN]: _qQLFT }]]
];
export var H264Settings$: StaticStructureSchema = [3, n0, _HS,
  0,
  [_AQ, _BRF, _B, _CLo, _CP, _DSG, _EOSM, _EE, _EWP, _FEi, _FAQ, _FC, _FCA, _FD, _FN, _GBR, _GCC, _GS, _GSU, _HBFFP, _HBIFP, _HBS, _IM, _MB, _MII, _NBFBRF, _NRF, _PCa, _PD, _PN, _PFM, _QTL, _QS, _RCM, _RP, _SAE, _STCM, _SCD, _Sl, _SPl, _So, _SAQ, _Sy, _Te, _TAQ, _UST, _WMPT],
  [[0, { [_jN]: _aQ }], [() => BandwidthReductionFilter$, { [_jN]: _bRF }], [1, { [_jN]: _b }], [0, { [_jN]: _cLo }], [0, { [_jN]: _cP }], [0, { [_jN]: _dSG }], [0, { [_jN]: _eOSM }], [0, { [_jN]: _eE }], [0, { [_jN]: _eWP }], [0, { [_jN]: _fE }], [0, { [_jN]: _fAQ }], [0, { [_jN]: _fC }], [0, { [_jN]: _fCA }], [1, { [_jN]: _fD }], [1, { [_jN]: _fN }], [0, { [_jN]: _gBR }], [1, { [_jN]: _gCC }], [1, { [_jN]: _gS }], [0, { [_jN]: _gSU }], [1, { [_jN]: _hBFFP }], [1, { [_jN]: _hBIFP }], [1, { [_jN]: _hBS }], [0, { [_jN]: _iM }], [1, { [_jN]: _mB }], [1, { [_jN]: _mII }], [1, { [_jN]: _nBFBRF }], [1, { [_jN]: _nRF }], [0, { [_jN]: _pCa }], [1, { [_jN]: _pD }], [1, { [_jN]: _pN }], [64 | 0, { [_jN]: _pFM }], [0, { [_jN]: _qTL }], [() => H264QvbrSettings$, { [_jN]: _qS }], [0, { [_jN]: _rCM }], [0, { [_jN]: _rP }], [0, { [_jN]: _sAE }], [0, { [_jN]: _sTCM }], [0, { [_jN]: _sCD }], [1, { [_jN]: _sl }], [0, { [_jN]: _sPl }], [1, { [_jN]: _so }], [0, { [_jN]: _sAQ }], [0, { [_jN]: _sy }], [0, { [_jN]: _te }], [0, { [_jN]: _tAQ }], [0, { [_jN]: _uST }], [0, { [_jN]: _wMPT }]]
];
export var H265QvbrSettings$: StaticStructureSchema = [3, n0, _HQSv,
  0,
  [_MABa, _QQL, _QQLFT],
  [[1, { [_jN]: _mABa }], [1, { [_jN]: _qQL }], [1, { [_jN]: _qQLFT }]]
];
export var H265Settings$: StaticStructureSchema = [3, n0, _HSe,
  0,
  [_AQ, _ATFS, _BRF, _B, _CLo, _CP, _Deb, _DSG, _EOSM, _FAQ, _FC, _FCA, _FD, _FN, _GBR, _GCC, _GS, _GSU, _HBFFP, _HBIFP, _HBS, _IM, _MB, _MII, _MOPB, _MTP, _NBFBRF, _NRF, _PCa, _PD, _PN, _PFM, _QTL, _QS, _RCM, _SAOFM, _STCM, _SCD, _Sl, _SPl, _SAQ, _Te, _TAQ, _TIe, _THi, _TP, _TWi, _Ti, _TBS, _UST, _WMPT],
  [[0, { [_jN]: _aQ }], [0, { [_jN]: _aTFS }], [() => BandwidthReductionFilter$, { [_jN]: _bRF }], [1, { [_jN]: _b }], [0, { [_jN]: _cLo }], [0, { [_jN]: _cP }], [0, { [_jN]: _deb }], [0, { [_jN]: _dSG }], [0, { [_jN]: _eOSM }], [0, { [_jN]: _fAQ }], [0, { [_jN]: _fC }], [0, { [_jN]: _fCA }], [1, { [_jN]: _fD }], [1, { [_jN]: _fN }], [0, { [_jN]: _gBR }], [1, { [_jN]: _gCC }], [1, { [_jN]: _gS }], [0, { [_jN]: _gSU }], [1, { [_jN]: _hBFFP }], [1, { [_jN]: _hBIFP }], [1, { [_jN]: _hBS }], [0, { [_jN]: _iM }], [1, { [_jN]: _mB }], [1, { [_jN]: _mII }], [0, { [_jN]: _mOPB }], [0, { [_jN]: _mTP }], [1, { [_jN]: _nBFBRF }], [1, { [_jN]: _nRF }], [0, { [_jN]: _pCa }], [1, { [_jN]: _pD }], [1, { [_jN]: _pN }], [64 | 0, { [_jN]: _pFM }], [0, { [_jN]: _qTL }], [() => H265QvbrSettings$, { [_jN]: _qS }], [0, { [_jN]: _rCM }], [0, { [_jN]: _sAOFM }], [0, { [_jN]: _sTCM }], [0, { [_jN]: _sCD }], [1, { [_jN]: _sl }], [0, { [_jN]: _sPl }], [0, { [_jN]: _sAQ }], [0, { [_jN]: _te }], [0, { [_jN]: _tAQ }], [0, { [_jN]: _tIe }], [1, { [_jN]: _tHi }], [0, { [_jN]: _tP }], [1, { [_jN]: _tWi }], [0, { [_jN]: _ti }], [0, { [_jN]: _tBS }], [0, { [_jN]: _uST }], [0, { [_jN]: _wMPT }]]
];
export var Hdr10Metadata$: StaticStructureSchema = [3, n0, _HM,
  0,
  [_BPX, _BPY, _GPX, _GPY, _MCLL, _MFALL, _ML, _MLi, _RPX, _RPY, _WPX, _WPY],
  [[1, { [_jN]: _bPX }], [1, { [_jN]: _bPY }], [1, { [_jN]: _gPX }], [1, { [_jN]: _gPY }], [1, { [_jN]: _mCLL }], [1, { [_jN]: _mFALL }], [1, { [_jN]: _mL }], [1, { [_jN]: _mLi }], [1, { [_jN]: _rPX }], [1, { [_jN]: _rPY }], [1, { [_jN]: _wPX }], [1, { [_jN]: _wPY }]]
];
export var Hdr10Plus$: StaticStructureSchema = [3, n0, _HP,
  0,
  [_MMN, _TMN],
  [[1, { [_jN]: _mMN }], [1, { [_jN]: _tMN }]]
];
export var HdrMetadata$: StaticStructureSchema = [3, n0, _HMd,
  0,
  [_CLL, _MDCV],
  [[() => ContentLightLevel$, { [_jN]: _cLL }], [() => MasteringDisplayColorVolume$, { [_jN]: _mDCV }]]
];
export var HlsAdditionalManifest$: StaticStructureSchema = [3, n0, _HAM,
  0,
  [_MNM, _SOe],
  [[0, { [_jN]: _mNM }], [64 | 0, { [_jN]: _sOe }]]
];
export var HlsCaptionLanguageMapping$: StaticStructureSchema = [3, n0, _HCLM,
  0,
  [_CCa, _CLC, _LC, _LD],
  [[1, { [_jN]: _cCa }], [0, { [_jN]: _cLC }], [0, { [_jN]: _lC }], [0, { [_jN]: _lD }]]
];
export var HlsEncryptionSettings$: StaticStructureSchema = [3, n0, _HES,
  0,
  [_CIV, _EM, _IVIM, _OE, _SKP, _SKPt, _Ty],
  [[0, { [_jN]: _cIV }], [0, { [_jN]: _eM }], [0, { [_jN]: _iVIM }], [0, { [_jN]: _oE }], [() => SpekeKeyProvider$, { [_jN]: _sKP }], [() => StaticKeyProvider$, { [_jN]: _sKPt }], [0, { [_jN]: _ty }]]
];
export var HlsGroupSettings$: StaticStructureSchema = [3, n0, _HGS,
  0,
  [_AMd, _AM, _AOH, _BU, _CLM, _CLSa, _CSLC, _CC, _CSo, _Des, _DSe, _DSi, _E, _IBTP, _IBTPS, _IBTPV, _MCa, _MDF, _MFSL, _MSL, _OSutp, _PDT, _PDTP, _PWHM, _SCe, _SL, _SLC, _SPS, _SIR, _TDCM, _TMIF, _TMIP, _TDM],
  [[64 | 0, { [_jN]: _aMd }], [() => __listOfHlsAdditionalManifest, { [_jN]: _aM }], [0, { [_jN]: _aOH }], [0, { [_jN]: _bU }], [() => __listOfHlsCaptionLanguageMapping, { [_jN]: _cLM }], [0, { [_jN]: _cLSa }], [0, { [_jN]: _cSLC }], [0, { [_jN]: _cC }], [0, { [_jN]: _cSo }], [0, { [_jN]: _des }], [() => DestinationSettings$, { [_jN]: _dSe }], [0, { [_jN]: _dSi }], [() => HlsEncryptionSettings$, { [_jN]: _en }], [0, { [_jN]: _iBTP }], [() => HlsImageBasedTrickPlaySettings$, { [_jN]: _iBTPS }], [() => __listOfHlsImageBasedTrickPlayVariant, { [_jN]: _iBTPV }], [0, { [_jN]: _mCa }], [0, { [_jN]: _mDF }], [1, { [_jN]: _mFSL }], [1, { [_jN]: _mSL }], [0, { [_jN]: _oSutp }], [0, { [_jN]: _pDT }], [1, { [_jN]: _pDTP }], [0, { [_jN]: _pWHM }], [0, { [_jN]: _sCe }], [1, { [_jN]: _sL }], [0, { [_jN]: _sLC }], [1, { [_jN]: _sPS }], [0, { [_jN]: _sIR }], [0, { [_jN]: _tDCM }], [0, { [_jN]: _tMIF }], [1, { [_jN]: _tMIP }], [1, { [_jN]: _tDM }]]
];
export var HlsImageBasedTrickPlaySettings$: StaticStructureSchema = [3, n0, _HIBTPS,
  0,
  [_IC, _TH, _TI, _TW, _THi, _TWi],
  [[0, { [_jN]: _iC }], [1, { [_jN]: _tH }], [1, { [_jN]: _tI }], [1, { [_jN]: _tW }], [1, { [_jN]: _tHi }], [1, { [_jN]: _tWi }]]
];
export var HlsImageBasedTrickPlayVariant$: StaticStructureSchema = [3, n0, _HIBTPV,
  0,
  [_IC, _TH, _TI, _TW, _THi, _TWi],
  [[0, { [_jN]: _iC }], [1, { [_jN]: _tH }], [1, { [_jN]: _tI }], [1, { [_jN]: _tW }], [1, { [_jN]: _tHi }], [1, { [_jN]: _tWi }]]
];
export var HlsRenditionGroupSettings$: StaticStructureSchema = [3, n0, _HRGS,
  0,
  [_RGI, _RLC, _RN],
  [[0, { [_jN]: _rGI }], [0, { [_jN]: _rLC }], [0, { [_jN]: _rN }]]
];
export var HlsSettings$: StaticStructureSchema = [3, n0, _HSl,
  0,
  [_AGI, _AOC, _ARSu, _ATT, _DVSF, _IFOM, _SMe],
  [[0, { [_jN]: _aGI }], [0, { [_jN]: _aOC }], [0, { [_jN]: _aRS }], [0, { [_jN]: _aTT }], [0, { [_jN]: _dVSF }], [0, { [_jN]: _iFOM }], [0, { [_jN]: _sMe }]]
];
export var HopDestination$: StaticStructureSchema = [3, n0, _HDo,
  0,
  [_Pri, _Q, _WM],
  [[1, { [_jN]: _pri }], [0, { [_jN]: _q }], [1, { [_jN]: _wM }]]
];
export var Id3Insertion$: StaticStructureSchema = [3, n0, _II,
  0,
  [_Id, _Tim],
  [[0, { [_jN]: _i }], [0, { [_jN]: _tim }]]
];
export var ImageInserter$: StaticStructureSchema = [3, n0, _IIm,
  0,
  [_IIn, _SRWL],
  [[() => __listOfInsertableImage, { [_jN]: _iI }], [1, { [_jN]: _sRWL }]]
];
export var ImscDestinationSettings$: StaticStructureSchema = [3, n0, _IDS,
  0,
  [_Ac, _SPt],
  [[0, { [_jN]: _ac }], [0, { [_jN]: _sPt }]]
];
export var Input$: StaticStructureSchema = [3, n0, _In,
  0,
  [_AIF, _AIFS, _ASGu, _ASud, _CSap, _Cr, _DFe, _DSec, _DFen, _DVMX, _DASy, _FI, _FEil, _FSi, _IIm, _ICn, _IST, _MVS, _Pos, _PNr, _PCs, _SIu, _TSa, _TSi, _TSim, _VG, _VO, _VSi],
  [[0, { [_jN]: _aIF }], [() => AdvancedInputFilterSettings$, { [_jN]: _aIFS }], [() => __mapOfAudioSelectorGroup, { [_jN]: _aSG }], [() => __mapOfAudioSelector, { [_jN]: _aSu }], [() => __mapOfCaptionSelector, { [_jN]: _cSa }], [() => Rectangle$, { [_jN]: _cr }], [0, { [_jN]: _dFe }], [() => InputDecryptionSettings$, { [_jN]: _dSec }], [0, { [_jN]: _dFen }], [0, { [_jN]: _dVMX }], [() => __mapOfDynamicAudioSelector, { [_jN]: _dAS }], [0, { [_jN]: _fI }], [0, { [_jN]: _fEi }], [1, { [_jN]: _fSi }], [() => ImageInserter$, { [_jN]: _iIm }], [() => __listOfInputClipping, { [_jN]: _iCn }], [0, { [_jN]: _iST }], [() => __listOfMultiViewSettings, { [_jN]: _mVS }], [() => Rectangle$, { [_jN]: _pos }], [1, { [_jN]: _pNr }], [0, { [_jN]: _pCs }], [64 | 0, { [_jN]: _sIu }], [() => InputTamsSettings$, { [_jN]: _tSa }], [0, { [_jN]: _tSi }], [0, { [_jN]: _tSim }], [() => InputVideoGenerator$, { [_jN]: _vG }], [() => __listOfVideoOverlay, { [_jN]: _vO }], [() => VideoSelector$, { [_jN]: _vSi }]]
];
export var InputClipping$: StaticStructureSchema = [3, n0, _ICnp,
  0,
  [_ET, _STt],
  [[0, { [_jN]: _eT }], [0, { [_jN]: _sTt }]]
];
export var InputDecryptionSettings$: StaticStructureSchema = [3, n0, _IDSn,
  0,
  [_DM, _EDK, _IV, _KKR],
  [[0, { [_jN]: _dM }], [0, { [_jN]: _eDK }], [0, { [_jN]: _iV }], [0, { [_jN]: _kKR }]]
];
export var InputTamsSettings$: StaticStructureSchema = [3, n0, _ITS,
  0,
  [_ACA, _GH, _SIo, _Time],
  [[0, { [_jN]: _aCA }], [0, { [_jN]: _gH }], [0, { [_jN]: _sIo }], [0, { [_jN]: _time }]]
];
export var InputTemplate$: StaticStructureSchema = [3, n0, _IT,
  0,
  [_AIF, _AIFS, _ASGu, _ASud, _CSap, _Cr, _DFe, _DFen, _DVMX, _DASy, _FEil, _FSi, _IIm, _ICn, _IST, _MVS, _Pos, _PNr, _PCs, _TSi, _TSim, _VO, _VSi],
  [[0, { [_jN]: _aIF }], [() => AdvancedInputFilterSettings$, { [_jN]: _aIFS }], [() => __mapOfAudioSelectorGroup, { [_jN]: _aSG }], [() => __mapOfAudioSelector, { [_jN]: _aSu }], [() => __mapOfCaptionSelector, { [_jN]: _cSa }], [() => Rectangle$, { [_jN]: _cr }], [0, { [_jN]: _dFe }], [0, { [_jN]: _dFen }], [0, { [_jN]: _dVMX }], [() => __mapOfDynamicAudioSelector, { [_jN]: _dAS }], [0, { [_jN]: _fEi }], [1, { [_jN]: _fSi }], [() => ImageInserter$, { [_jN]: _iIm }], [() => __listOfInputClipping, { [_jN]: _iCn }], [0, { [_jN]: _iST }], [() => __listOfMultiViewSettings, { [_jN]: _mVS }], [() => Rectangle$, { [_jN]: _pos }], [1, { [_jN]: _pNr }], [0, { [_jN]: _pCs }], [0, { [_jN]: _tSi }], [0, { [_jN]: _tSim }], [() => __listOfVideoOverlay, { [_jN]: _vO }], [() => VideoSelector$, { [_jN]: _vSi }]]
];
export var InputVideoGenerator$: StaticStructureSchema = [3, n0, _IVG,
  0,
  [_C, _Du, _FD, _FN, _H, _IIma, _SR, _W],
  [[1, { [_jN]: _ch }], [1, { [_jN]: _du }], [1, { [_jN]: _fD }], [1, { [_jN]: _fN }], [1, { [_jN]: _h }], [0, { [_jN]: _iIma }], [1, { [_jN]: _sR }], [1, { [_jN]: _w }]]
];
export var InsertableImage$: StaticStructureSchema = [3, n0, _IIns,
  0,
  [_Du, _FIa, _FOa, _H, _III, _IX, _IY, _La, _Op, _STta, _W],
  [[1, { [_jN]: _du }], [1, { [_jN]: _fIa }], [1, { [_jN]: _fOa }], [1, { [_jN]: _h }], [0, { [_jN]: _iII }], [1, { [_jN]: _iX }], [1, { [_jN]: _iY }], [1, { [_jN]: _la }], [1, { [_jN]: _op }], [0, { [_jN]: _sTta }], [1, { [_jN]: _w }]]
];
export var Job$: StaticStructureSchema = [3, n0, _J,
  0,
  [_Rol, _Se, _AScc, _AScce, _A, _BTS, _CRT, _CA, _CPu, _EIC, _EC, _EMr, _HD, _I, _JEVR, _JEVU, _JPC, _JT, _LSD, _Me, _OGD, _Pri, _Q, _QT, _RC, _SSh, _SRQ, _Sta, _SUI, _Timi, _UM, _Wa],
  [[0, { [_jN]: _rol }], [() => JobSettings$, { [_jN]: _set }], [() => AccelerationSettings$, { [_jN]: _aScc }], [0, { [_jN]: _aScce }], [0, { [_jN]: _a }], [0, { [_jN]: _bTS }], [0, { [_jN]: _cRT }], [7, { [_jN]: _cA }], [0, { [_jN]: _cPu }], [() => ElementalInferenceConfiguration$, { [_jN]: _eIC }], [1, { [_jN]: _eC }], [0, { [_jN]: _eMr }], [() => __listOfHopDestination, { [_jN]: _hD }], [0, { [_jN]: _id }], [0, { [_jN]: _jEVR }], [0, { [_jN]: _jEVU }], [1, { [_jN]: _jPC }], [0, { [_jN]: _jT }], [0, { [_jN]: _lSD }], [() => JobMessages$, { [_jN]: _me }], [() => __listOfOutputGroupDetail, { [_jN]: _oGD }], [1, { [_jN]: _pri }], [0, { [_jN]: _q }], [() => __listOfQueueTransition, { [_jN]: _qT }], [1, { [_jN]: _rC }], [0, { [_jN]: _sSh }], [0, { [_jN]: _sRQ }], [0, { [_jN]: _sta }], [0, { [_jN]: _sUI }], [() => Timing$, { [_jN]: _timi }], [128 | 0, { [_jN]: _uM }], [() => __listOfWarningGroup, { [_jN]: _wa }]], 2
];
export var JobEngineVersion$: StaticStructureSchema = [3, n0, _JEV,
  0,
  [_ED, _V],
  [[7, { [_jN]: _eD }], [0, { [_jN]: _v }]]
];
export var JobMessages$: StaticStructureSchema = [3, n0, _JM,
  0,
  [_Inf, _War],
  [[64 | 0, { [_jN]: _in }], [64 | 0, { [_jN]: _war }]]
];
export var JobSettings$: StaticStructureSchema = [3, n0, _JS,
  0,
  [_AAO, _AB, _CCDLUTSo, _Es, _EDSx, _FSol, _Inp, _KW, _MIIo, _MIIot, _NC, _NNLW, _OG, _TCi, _TMI],
  [[1, { [_jN]: _aAO }], [() => AvailBlanking$, { [_jN]: _aB }], [() => __listOfColorConversion3DLUTSetting, { [_jN]: _cCDLUTS }], [() => EsamSettings$, { [_jN]: _es }], [() => ExtendedDataServices$, { [_jN]: _eDSx }], [1, { [_jN]: _fSol }], [() => __listOfInput, { [_jN]: _inp }], [() => KantarWatermarkSettings$, { [_jN]: _kW }], [() => MotionImageInserter$, { [_jN]: _mIIo }], [() => __listOfMotionImageInserter, { [_jN]: _mIIot }], [() => NielsenConfiguration$, { [_jN]: _nC }], [() => NielsenNonLinearWatermarkSettings$, { [_jN]: _nNLW }], [() => __listOfOutputGroup, { [_jN]: _oG }], [() => TimecodeConfig$, { [_jN]: _tCi }], [() => TimedMetadataInsertion$, { [_jN]: _tMI }]]
];
export var JobsQueryFilter$: StaticStructureSchema = [3, n0, _JQF,
  0,
  [_K, _Va],
  [[0, { [_jN]: _k }], [64 | 0, { [_jN]: _va }]]
];
export var JobTemplate$: StaticStructureSchema = [3, n0, _JT,
  0,
  [_Na, _Se, _AScc, _A, _Ca, _CA, _Desc, _HD, _LU, _Pri, _Q, _SUI, _Ty],
  [[0, { [_jN]: _na }], [() => JobTemplateSettings$, { [_jN]: _set }], [() => AccelerationSettings$, { [_jN]: _aScc }], [0, { [_jN]: _a }], [0, { [_jN]: _ca }], [7, { [_jN]: _cA }], [0, { [_jN]: _desc }], [() => __listOfHopDestination, { [_jN]: _hD }], [7, { [_jN]: _lU }], [1, { [_jN]: _pri }], [0, { [_jN]: _q }], [0, { [_jN]: _sUI }], [0, { [_jN]: _ty }]], 2
];
export var JobTemplateSettings$: StaticStructureSchema = [3, n0, _JTS,
  0,
  [_AAO, _AB, _CCDLUTSo, _Es, _EDSx, _FSol, _Inp, _KW, _MIIo, _MIIot, _NC, _NNLW, _OG, _TCi, _TMI],
  [[1, { [_jN]: _aAO }], [() => AvailBlanking$, { [_jN]: _aB }], [() => __listOfColorConversion3DLUTSetting, { [_jN]: _cCDLUTS }], [() => EsamSettings$, { [_jN]: _es }], [() => ExtendedDataServices$, { [_jN]: _eDSx }], [1, { [_jN]: _fSol }], [() => __listOfInputTemplate, { [_jN]: _inp }], [() => KantarWatermarkSettings$, { [_jN]: _kW }], [() => MotionImageInserter$, { [_jN]: _mIIo }], [() => __listOfMotionImageInserter, { [_jN]: _mIIot }], [() => NielsenConfiguration$, { [_jN]: _nC }], [() => NielsenNonLinearWatermarkSettings$, { [_jN]: _nNLW }], [() => __listOfOutputGroup, { [_jN]: _oG }], [() => TimecodeConfig$, { [_jN]: _tCi }], [() => TimedMetadataInsertion$, { [_jN]: _tMI }]]
];
export var KantarWatermarkSettings$: StaticStructureSchema = [3, n0, _KWS,
  0,
  [_CN, _CR, _CSNr, _FOil, _KLI, _KSU, _LDo, _Met, _Meta, _Metad, _Metada, _Metadat, _Metadata],
  [[0, { [_jN]: _cN }], [0, { [_jN]: _cR }], [0, { [_jN]: _cSNr }], [1, { [_jN]: _fOil }], [1, { [_jN]: _kLI }], [0, { [_jN]: _kSU }], [0, { [_jN]: _lDo }], [0, { [_jN]: _met }], [0, { [_jN]: _meta }], [0, { [_jN]: _metad }], [0, { [_jN]: _metada }], [0, { [_jN]: _metadat }], [0, { [_jN]: _metadata }]]
];
export var ListJobsRequest$: StaticStructureSchema = [3, n0, _LJR,
  0,
  [_MRa, _NT, _Or, _Q, _Sta],
  [[1, { [_hQ]: _mRa }], [0, { [_hQ]: _nT }], [0, { [_hQ]: _or }], [0, { [_hQ]: _q }], [0, { [_hQ]: _sta }]]
];
export var ListJobsResponse$: StaticStructureSchema = [3, n0, _LJRi,
  0,
  [_Jo, _NT],
  [[() => __listOfJob, { [_jN]: _jo }], [0, { [_jN]: _nT }]]
];
export var ListJobTemplatesRequest$: StaticStructureSchema = [3, n0, _LJTR,
  0,
  [_Ca, _LB, _MRa, _NT, _Or],
  [[0, { [_hQ]: _ca }], [0, { [_hQ]: _lB }], [1, { [_hQ]: _mRa }], [0, { [_hQ]: _nT }], [0, { [_hQ]: _or }]]
];
export var ListJobTemplatesResponse$: StaticStructureSchema = [3, n0, _LJTRi,
  0,
  [_JTo, _NT],
  [[() => __listOfJobTemplate, { [_jN]: _jTo }], [0, { [_jN]: _nT }]]
];
export var ListPresetsRequest$: StaticStructureSchema = [3, n0, _LPR,
  0,
  [_Ca, _LB, _MRa, _NT, _Or],
  [[0, { [_hQ]: _ca }], [0, { [_hQ]: _lB }], [1, { [_hQ]: _mRa }], [0, { [_hQ]: _nT }], [0, { [_hQ]: _or }]]
];
export var ListPresetsResponse$: StaticStructureSchema = [3, n0, _LPRi,
  0,
  [_NT, _Pres],
  [[0, { [_jN]: _nT }], [() => __listOfPreset, { [_jN]: _pres }]]
];
export var ListQueuesRequest$: StaticStructureSchema = [3, n0, _LQR,
  0,
  [_LB, _MRa, _NT, _Or],
  [[0, { [_hQ]: _lB }], [1, { [_hQ]: _mRa }], [0, { [_hQ]: _nT }], [0, { [_hQ]: _or }]]
];
export var ListQueuesResponse$: StaticStructureSchema = [3, n0, _LQRi,
  0,
  [_NT, _Que, _TCJ, _UCJ],
  [[0, { [_jN]: _nT }], [() => __listOfQueue, { [_jN]: _que }], [1, { [_jN]: _tCJ }], [1, { [_jN]: _uCJ }]]
];
export var ListTagsForResourceRequest$: StaticStructureSchema = [3, n0, _LTFRR,
  0,
  [_A],
  [[0, 1]], 1
];
export var ListTagsForResourceResponse$: StaticStructureSchema = [3, n0, _LTFRRi,
  0,
  [_RT],
  [[() => ResourceTags$, { [_jN]: _rT }]]
];
export var ListVersionsRequest$: StaticStructureSchema = [3, n0, _LVR,
  0,
  [_MRa, _NT],
  [[1, { [_hQ]: _mRa }], [0, { [_hQ]: _nT }]]
];
export var ListVersionsResponse$: StaticStructureSchema = [3, n0, _LVRi,
  0,
  [_NT, _Ve],
  [[0, { [_jN]: _nT }], [() => __listOfJobEngineVersion, { [_jN]: _ve }]]
];
export var M2tsScte35Esam$: StaticStructureSchema = [3, n0, _MSE,
  0,
  [_SEP],
  [[1, { [_jN]: _sEP }]]
];
export var M2tsSettings$: StaticStructureSchema = [3, n0, _MSt,
  0,
  [_ABM, _ADu, _AFPP, _APu, _APOD, _B, _BMu, _DPTSC, _DNS, _DSS, _DSP, _DTS, _DTP, _EAI, _EP, _ERIP, _FTVEO, _FT, _KM, _MPI, _MEI, _NIie, _NPB, _PI, _PCc, _PPc, _PIm, _PPm, _PBU, _PMP, _PNr, _PO, _POM, _RM, _SE, _SPc, _SSc, _SMeg, _SSeg, _STe, _TMP, _TSI, _VP],
  [[0, { [_jN]: _aBM }], [0, { [_jN]: _aD }], [1, { [_jN]: _aFPP }], [64 | 1, { [_jN]: _aP }], [1, { [_jN]: _aPOD }], [1, { [_jN]: _b }], [0, { [_jN]: _bMu }], [0, { [_jN]: _dPTSC }], [() => DvbNitSettings$, { [_jN]: _dNS }], [() => DvbSdtSettings$, { [_jN]: _dSS }], [64 | 1, { [_jN]: _dSP }], [() => DvbTdtSettings$, { [_jN]: _dTS }], [1, { [_jN]: _dTP }], [0, { [_jN]: _eAI }], [0, { [_jN]: _eP }], [0, { [_jN]: _eRIP }], [0, { [_jN]: _fTVEO }], [1, { [_jN]: _fT }], [0, { [_jN]: _kM }], [1, { [_jN]: _mPI }], [1, { [_jN]: _mEI }], [0, { [_jN]: _nIie }], [1, { [_jN]: _nPB }], [1, { [_jN]: _pI }], [0, { [_jN]: _pCc }], [1, { [_jN]: _pPc }], [1, { [_jN]: _pIm }], [1, { [_jN]: _pPm }], [0, { [_jN]: _pBU }], [1, { [_jN]: _pMP }], [1, { [_jN]: _pNr }], [1, { [_jN]: _pO }], [0, { [_jN]: _pOM }], [0, { [_jN]: _rM }], [() => M2tsScte35Esam$, { [_jN]: _sE }], [1, { [_jN]: _sPc }], [0, { [_jN]: _sSc }], [0, { [_jN]: _sMeg }], [0, { [_jN]: _sSeg }], [1, { [_jN]: _sTe }], [1, { [_jN]: _tMP }], [1, { [_jN]: _tSI }], [1, { [_jN]: _vP }]]
];
export var M3u8Settings$: StaticStructureSchema = [3, n0, _MSu,
  0,
  [_ADu, _AFPP, _APu, _APOD, _DPTSC, _MPI, _NIie, _PI, _PCc, _PPc, _PIm, _PPm, _PMP, _PNr, _PO, _POM, _SPc, _SSc, _TM, _TMP, _TSI, _VP],
  [[0, { [_jN]: _aD }], [1, { [_jN]: _aFPP }], [64 | 1, { [_jN]: _aP }], [1, { [_jN]: _aPOD }], [0, { [_jN]: _dPTSC }], [1, { [_jN]: _mPI }], [0, { [_jN]: _nIie }], [1, { [_jN]: _pI }], [0, { [_jN]: _pCc }], [1, { [_jN]: _pPc }], [1, { [_jN]: _pIm }], [1, { [_jN]: _pPm }], [1, { [_jN]: _pMP }], [1, { [_jN]: _pNr }], [1, { [_jN]: _pO }], [0, { [_jN]: _pOM }], [1, { [_jN]: _sPc }], [0, { [_jN]: _sSc }], [0, { [_jN]: _tM }], [1, { [_jN]: _tMP }], [1, { [_jN]: _tSI }], [1, { [_jN]: _vP }]]
];
export var MasteringDisplayColorVolume$: StaticStructureSchema = [3, n0, _MDCV,
  0,
  [_BPX, _BPY, _GPX, _GPY, _ML, _MLi, _RPX, _RPY, _WPX, _WPY],
  [[1, { [_jN]: _bPX }], [1, { [_jN]: _bPY }], [1, { [_jN]: _gPX }], [1, { [_jN]: _gPY }], [1, { [_jN]: _mL }], [1, { [_jN]: _mLi }], [1, { [_jN]: _rPX }], [1, { [_jN]: _rPY }], [1, { [_jN]: _wPX }], [1, { [_jN]: _wPY }]]
];
export var Metadata$: StaticStructureSchema = [3, n0, _Metadata_,
  0,
  [_ETa, _FSil, _LMa, _MT],
  [[0, { [_jN]: _eTa }], [1, { [_jN]: _fSil }], [7, { [_jN]: _lMa }], [0, { [_jN]: _mT }]]
];
export var MinBottomRenditionSize$: StaticStructureSchema = [3, n0, _MBRS,
  0,
  [_H, _W],
  [[1, { [_jN]: _h }], [1, { [_jN]: _w }]]
];
export var MinTopRenditionSize$: StaticStructureSchema = [3, n0, _MTRS,
  0,
  [_H, _W],
  [[1, { [_jN]: _h }], [1, { [_jN]: _w }]]
];
export var MotionImageInserter$: StaticStructureSchema = [3, n0, _MIIo,
  0,
  [_Fr, _In, _IMn, _O, _Pl, _STta],
  [[() => MotionImageInsertionFramerate$, { [_jN]: _fr }], [0, { [_jN]: _inpu }], [0, { [_jN]: _iMn }], [() => MotionImageInsertionOffset$, { [_jN]: _o }], [0, { [_jN]: _pl }], [0, { [_jN]: _sTta }]]
];
export var MotionImageInsertionFramerate$: StaticStructureSchema = [3, n0, _MIIF,
  0,
  [_FD, _FN],
  [[1, { [_jN]: _fD }], [1, { [_jN]: _fN }]]
];
export var MotionImageInsertionOffset$: StaticStructureSchema = [3, n0, _MIIO,
  0,
  [_IX, _IY],
  [[1, { [_jN]: _iX }], [1, { [_jN]: _iY }]]
];
export var MovSettings$: StaticStructureSchema = [3, n0, _MSo,
  0,
  [_ADu, _CAl, _CAs, _MFCCC, _PCad, _Re],
  [[0, { [_jN]: _aD }], [0, { [_jN]: _cAl }], [0, { [_jN]: _cAs }], [0, { [_jN]: _mFCCC }], [0, { [_jN]: _pCad }], [0, { [_jN]: _re }]]
];
export var Mp2Settings$: StaticStructureSchema = [3, n0, _MS,
  0,
  [_ADM, _B, _C, _SR],
  [[0, { [_jN]: _aDM }], [1, { [_jN]: _b }], [1, { [_jN]: _ch }], [1, { [_jN]: _sR }]]
];
export var Mp3Settings$: StaticStructureSchema = [3, n0, _MSp,
  0,
  [_B, _C, _RCM, _SR, _VQ],
  [[1, { [_jN]: _b }], [1, { [_jN]: _ch }], [0, { [_jN]: _rCM }], [1, { [_jN]: _sR }], [1, { [_jN]: _vQ }]]
];
export var Mp4Settings$: StaticStructureSchema = [3, n0, _MSpe,
  0,
  [_ADu, _CMp, _CSe, _CAs, _CV, _FSB, _MPo, _MMB, _SKK],
  [[0, { [_jN]: _aD }], [0, { [_jN]: _cMp }], [0, { [_jN]: _cSe }], [0, { [_jN]: _cAs }], [1, { [_jN]: _cV }], [0, { [_jN]: _fSB }], [0, { [_jN]: _mPo }], [0, { [_jN]: _mMB }], [0, { [_jN]: _sKK }]]
];
export var MpdSettings$: StaticStructureSchema = [3, n0, _MSpd,
  0,
  [_ACH, _ADu, _CMp, _CCT, _CSe, _KM, _MMS, _SE, _SSc, _SKK, _TM, _TMBV, _TMSIU, _TMV],
  [[0, { [_jN]: _aCH }], [0, { [_jN]: _aD }], [0, { [_jN]: _cMp }], [0, { [_jN]: _cCT }], [0, { [_jN]: _cSe }], [0, { [_jN]: _kM }], [0, { [_jN]: _mMS }], [0, { [_jN]: _sE }], [0, { [_jN]: _sSc }], [0, { [_jN]: _sKK }], [0, { [_jN]: _tM }], [0, { [_jN]: _tMBV }], [0, { [_jN]: _tMSIU }], [0, { [_jN]: _tMV }]]
];
export var Mpeg2Settings$: StaticStructureSchema = [3, n0, _MSpeg,
  0,
  [_AQ, _B, _CLo, _CP, _DSG, _FC, _FCA, _FD, _FN, _GCC, _GS, _GSU, _HBFFP, _HBIFP, _HBS, _IM, _IDP, _MB, _MII, _NBFBRF, _PCa, _PD, _PN, _PFM, _QTL, _RCM, _STCM, _SCD, _SPl, _So, _SAQ, _Sy, _Te, _TAQ],
  [[0, { [_jN]: _aQ }], [1, { [_jN]: _b }], [0, { [_jN]: _cLo }], [0, { [_jN]: _cP }], [0, { [_jN]: _dSG }], [0, { [_jN]: _fC }], [0, { [_jN]: _fCA }], [1, { [_jN]: _fD }], [1, { [_jN]: _fN }], [1, { [_jN]: _gCC }], [1, { [_jN]: _gS }], [0, { [_jN]: _gSU }], [1, { [_jN]: _hBFFP }], [1, { [_jN]: _hBIFP }], [1, { [_jN]: _hBS }], [0, { [_jN]: _iM }], [0, { [_jN]: _iDP }], [1, { [_jN]: _mB }], [1, { [_jN]: _mII }], [1, { [_jN]: _nBFBRF }], [0, { [_jN]: _pCa }], [1, { [_jN]: _pD }], [1, { [_jN]: _pN }], [64 | 0, { [_jN]: _pFM }], [0, { [_jN]: _qTL }], [0, { [_jN]: _rCM }], [0, { [_jN]: _sTCM }], [0, { [_jN]: _sCD }], [0, { [_jN]: _sPl }], [1, { [_jN]: _so }], [0, { [_jN]: _sAQ }], [0, { [_jN]: _sy }], [0, { [_jN]: _te }], [0, { [_jN]: _tAQ }]]
];
export var MsSmoothAdditionalManifest$: StaticStructureSchema = [3, n0, _MSAM,
  0,
  [_MNM, _SOe],
  [[0, { [_jN]: _mNM }], [64 | 0, { [_jN]: _sOe }]]
];
export var MsSmoothEncryptionSettings$: StaticStructureSchema = [3, n0, _MSES,
  0,
  [_SKP],
  [[() => SpekeKeyProvider$, { [_jN]: _sKP }]]
];
export var MsSmoothGroupSettings$: StaticStructureSchema = [3, n0, _MSGS,
  0,
  [_AM, _ADud, _Des, _DSe, _E, _FL, _FLC, _ME],
  [[() => __listOfMsSmoothAdditionalManifest, { [_jN]: _aM }], [0, { [_jN]: _aDu }], [0, { [_jN]: _des }], [() => DestinationSettings$, { [_jN]: _dSe }], [() => MsSmoothEncryptionSettings$, { [_jN]: _en }], [1, { [_jN]: _fL }], [0, { [_jN]: _fLC }], [0, { [_jN]: _mE }]]
];
export var MultiViewInput$: StaticStructureSchema = [3, n0, _MVI,
  0,
  [_FI],
  [[0, { [_jN]: _fI }]]
];
export var MultiViewSettings$: StaticStructureSchema = [3, n0, _MVS,
  0,
  [_In],
  [[() => MultiViewInput$, { [_jN]: _inpu }]]
];
export var MxfSettings$: StaticStructureSchema = [3, n0, _MSx,
  0,
  [_ASf, _Pr, _UAW, _XPS],
  [[0, { [_jN]: _aSf }], [0, { [_jN]: _pr }], [0, { [_jN]: _uAW }], [() => MxfXavcProfileSettings$, { [_jN]: _xPS }]]
];
export var MxfXavcProfileSettings$: StaticStructureSchema = [3, n0, _MXPS,
  0,
  [_DMu, _MADS],
  [[0, { [_jN]: _dMu }], [1, { [_jN]: _mADS }]]
];
export var NexGuardFileMarkerSettings$: StaticStructureSchema = [3, n0, _NGFMS,
  0,
  [_Li, _Pa, _Pre, _Str],
  [[0, { [_jN]: _li }], [1, { [_jN]: _pa }], [0, { [_jN]: _pre }], [0, { [_jN]: _str }]]
];
export var NielsenConfiguration$: StaticStructureSchema = [3, n0, _NC,
  0,
  [_BCr, _DIi],
  [[1, { [_jN]: _bCr }], [0, { [_jN]: _dIi }]]
];
export var NielsenNonLinearWatermarkSettings$: StaticStructureSchema = [3, n0, _NNLWS,
  0,
  [_AWP, _AF, _AI, _AN, _CSI, _EI, _MD, _SIo, _SWS, _TSU, _UTPAT],
  [[0, { [_jN]: _aWP }], [0, { [_jN]: _aF }], [0, { [_jN]: _aI }], [0, { [_jN]: _aN }], [0, { [_jN]: _cSI }], [0, { [_jN]: _eI }], [0, { [_jN]: _mD }], [1, { [_jN]: _sIo }], [0, { [_jN]: _sWS }], [0, { [_jN]: _tSU }], [0, { [_jN]: _uTPAT }]]
];
export var NoiseReducer$: StaticStructureSchema = [3, n0, _NR,
  0,
  [_Fi, _FSilt, _SFS, _TFS],
  [[0, { [_jN]: _fi }], [() => NoiseReducerFilterSettings$, { [_jN]: _fSilt }], [() => NoiseReducerSpatialFilterSettings$, { [_jN]: _sFS }], [() => NoiseReducerTemporalFilterSettings$, { [_jN]: _tFS }]]
];
export var NoiseReducerFilterSettings$: StaticStructureSchema = [3, n0, _NRFS,
  0,
  [_Str],
  [[1, { [_jN]: _str }]]
];
export var NoiseReducerSpatialFilterSettings$: StaticStructureSchema = [3, n0, _NRSFS,
  0,
  [_PFSS, _Sp, _Str],
  [[1, { [_jN]: _pFSS }], [1, { [_jN]: _spe }], [1, { [_jN]: _str }]]
];
export var NoiseReducerTemporalFilterSettings$: StaticStructureSchema = [3, n0, _NRTFS,
  0,
  [_AMg, _PTS, _PTSS, _Sp, _Str],
  [[1, { [_jN]: _aMg }], [0, { [_jN]: _pTS }], [0, { [_jN]: _pTSS }], [1, { [_jN]: _spe }], [1, { [_jN]: _str }]]
];
export var OpusSettings$: StaticStructureSchema = [3, n0, _OS,
  0,
  [_B, _C, _SR],
  [[1, { [_jN]: _b }], [1, { [_jN]: _ch }], [1, { [_jN]: _sR }]]
];
export var Output$: StaticStructureSchema = [3, n0, _Ou,
  0,
  [_ADudi, _CDa, _CSon, _Ex, _NM, _OSutpu, _Pre, _VD],
  [[() => __listOfAudioDescription, { [_jN]: _aDud }], [() => __listOfCaptionDescription, { [_jN]: _cD }], [() => ContainerSettings$, { [_jN]: _cSon }], [0, { [_jN]: _ex }], [0, { [_jN]: _nM }], [() => OutputSettings$, { [_jN]: _oSutpu }], [0, { [_jN]: _pre }], [() => VideoDescription$, { [_jN]: _vD }]]
];
export var OutputChannelMapping$: StaticStructureSchema = [3, n0, _OCM,
  0,
  [_ICnpu, _ICFT],
  [[64 | 1, { [_jN]: _iCnp }], [64 | 1, { [_jN]: _iCFT }]]
];
export var OutputDetail$: StaticStructureSchema = [3, n0, _OD,
  0,
  [_DIM, _VDi],
  [[1, { [_jN]: _dIM }], [() => VideoDetail$, { [_jN]: _vDi }]]
];
export var OutputGroup$: StaticStructureSchema = [3, n0, _OGu,
  0,
  [_AES, _CNu, _Na, _OGS, _Out],
  [[() => AutomatedEncodingSettings$, { [_jN]: _aES }], [0, { [_jN]: _cNu }], [0, { [_jN]: _na }], [() => OutputGroupSettings$, { [_jN]: _oGS }], [() => __listOfOutput, { [_jN]: _ou }]]
];
export var OutputGroupDetail$: StaticStructureSchema = [3, n0, _OGDu,
  0,
  [_ODu],
  [[() => __listOfOutputDetail, { [_jN]: _oD }]]
];
export var OutputGroupSettings$: StaticStructureSchema = [3, n0, _OGS,
  0,
  [_CGS, _DIGS, _FGSi, _HGS, _MSGS, _PFM, _Ty],
  [[() => CmafGroupSettings$, { [_jN]: _cGS }], [() => DashIsoGroupSettings$, { [_jN]: _dIGS }], [() => FileGroupSettings$, { [_jN]: _fGSi }], [() => HlsGroupSettings$, { [_jN]: _hGS }], [() => MsSmoothGroupSettings$, { [_jN]: _mSGS }], [64 | 0, { [_jN]: _pFM }], [0, { [_jN]: _ty }]]
];
export var OutputSettings$: StaticStructureSchema = [3, n0, _OSutpu,
  0,
  [_HSl],
  [[() => HlsSettings$, { [_jN]: _hS }]]
];
export var PartnerWatermarking$: StaticStructureSchema = [3, n0, _PW,
  0,
  [_NFMS],
  [[() => NexGuardFileMarkerSettings$, { [_jN]: _nFMS }]]
];
export var PassthroughSettings$: StaticStructureSchema = [3, n0, _PSa,
  0,
  [_FCr, _GPS, _SMegm, _VSM],
  [[0, { [_jN]: _fCr }], [1, { [_jN]: _gPS }], [0, { [_jN]: _sMegm }], [0, { [_jN]: _vSM }]]
];
export var Policy$: StaticStructureSchema = [3, n0, _Po,
  0,
  [_HI, _HIt, _SIn],
  [[0, { [_jN]: _hI }], [0, { [_jN]: _hIt }], [0, { [_jN]: _sIn }]]
];
export var Preset$: StaticStructureSchema = [3, n0, _Pre,
  0,
  [_Na, _Se, _A, _Ca, _CA, _Desc, _LU, _Ty],
  [[0, { [_jN]: _na }], [() => PresetSettings$, { [_jN]: _set }], [0, { [_jN]: _a }], [0, { [_jN]: _ca }], [7, { [_jN]: _cA }], [0, { [_jN]: _desc }], [7, { [_jN]: _lU }], [0, { [_jN]: _ty }]], 2
];
export var PresetSettings$: StaticStructureSchema = [3, n0, _PSr,
  0,
  [_ADudi, _CDa, _CSon, _VD],
  [[() => __listOfAudioDescription, { [_jN]: _aDud }], [() => __listOfCaptionDescriptionPreset, { [_jN]: _cD }], [() => ContainerSettings$, { [_jN]: _cSon }], [() => VideoDescription$, { [_jN]: _vD }]]
];
export var ProbeInputFile$: StaticStructureSchema = [3, n0, _PIF,
  0,
  [_FU],
  [[0, { [_jN]: _fU }]]
];
export var ProbeRequest$: StaticStructureSchema = [3, n0, _PR,
  0,
  [_IF],
  [[() => __listOfProbeInputFile, { [_jN]: _iF }]]
];
export var ProbeResponse$: StaticStructureSchema = [3, n0, _PRr,
  0,
  [_PRro],
  [[() => __listOfProbeResult, { [_jN]: _pR }]]
];
export var ProbeResult$: StaticStructureSchema = [3, n0, _PRrob,
  0,
  [_Cont, _Metadata_, _TMr],
  [[() => Container$, { [_jN]: _cont }], [() => Metadata$, { [_jN]: _metadata_ }], [() => __listOfTrackMapping, { [_jN]: _tMr }]]
];
export var ProresSettings$: StaticStructureSchema = [3, n0, _PSro,
  0,
  [_CShr, _CP, _FC, _FCA, _FD, _FN, _IM, _PCa, _PD, _PN, _PFM, _STCM, _SPl, _Te],
  [[0, { [_jN]: _cShr }], [0, { [_jN]: _cP }], [0, { [_jN]: _fC }], [0, { [_jN]: _fCA }], [1, { [_jN]: _fD }], [1, { [_jN]: _fN }], [0, { [_jN]: _iM }], [0, { [_jN]: _pCa }], [1, { [_jN]: _pD }], [1, { [_jN]: _pN }], [64 | 0, { [_jN]: _pFM }], [0, { [_jN]: _sTCM }], [0, { [_jN]: _sPl }], [0, { [_jN]: _te }]]
];
export var PutPolicyRequest$: StaticStructureSchema = [3, n0, _PPR,
  0,
  [_Po],
  [[() => Policy$, { [_jN]: _po }]], 1
];
export var PutPolicyResponse$: StaticStructureSchema = [3, n0, _PPRu,
  0,
  [_Po],
  [[() => Policy$, { [_jN]: _po }]]
];
export var Queue$: StaticStructureSchema = [3, n0, _Q,
  0,
  [_Na, _A, _CJ, _CA, _Desc, _LU, _MCF, _PP, _PJC, _RPe, _SOer, _Sta, _SJC, _Ty],
  [[0, { [_jN]: _na }], [0, { [_jN]: _a }], [1, { [_jN]: _cJ }], [7, { [_jN]: _cA }], [0, { [_jN]: _desc }], [7, { [_jN]: _lU }], [1, { [_jN]: _mCF }], [0, { [_jN]: _pP }], [1, { [_jN]: _pJC }], [() => ReservationPlan$, { [_jN]: _rPe }], [() => __listOfServiceOverride, { [_jN]: _sOer }], [0, { [_jN]: _sta }], [1, { [_jN]: _sJC }], [0, { [_jN]: _ty }]], 1
];
export var QueueTransition$: StaticStructureSchema = [3, n0, _QTu,
  0,
  [_DQ, _SQ, _Times],
  [[0, { [_jN]: _dQ }], [0, { [_jN]: _sQ }], [7, { [_jN]: _times }]]
];
export var Rectangle$: StaticStructureSchema = [3, n0, _Rec,
  0,
  [_H, _W, _X, _Y],
  [[1, { [_jN]: _h }], [1, { [_jN]: _w }], [1, { [_jN]: _x }], [1, { [_jN]: _y }]]
];
export var RemixSettings$: StaticStructureSchema = [3, n0, _RS,
  0,
  [_ADAC, _ADDC, _CMh, _CI, _CO],
  [[1, { [_jN]: _aDAC }], [1, { [_jN]: _aDDC }], [() => ChannelMapping$, { [_jN]: _cMh }], [1, { [_jN]: _cI }], [1, { [_jN]: _cO }]]
];
export var ReservationPlan$: StaticStructureSchema = [3, n0, _RPe,
  0,
  [_Comm, _EA, _PA, _RTe, _RSe, _Sta],
  [[0, { [_jN]: _comm }], [7, { [_jN]: _eA }], [7, { [_jN]: _pA }], [0, { [_jN]: _rTe }], [1, { [_jN]: _rSe }], [0, { [_jN]: _sta }]]
];
export var ReservationPlanSettings$: StaticStructureSchema = [3, n0, _RPS,
  0,
  [_Comm, _RTe, _RSe],
  [[0, { [_jN]: _comm }], [0, { [_jN]: _rTe }], [1, { [_jN]: _rSe }]], 3
];
export var ResourceTags$: StaticStructureSchema = [3, n0, _RT,
  0,
  [_A, _Ta],
  [[0, { [_jN]: _a }], [128 | 0, { [_jN]: _ta }]]
];
export var S3DestinationAccessControl$: StaticStructureSchema = [3, n0, _SDAC,
  0,
  [_CAa],
  [[0, { [_jN]: _cAa }]]
];
export var S3DestinationSettings$: StaticStructureSchema = [3, n0, _SDSe,
  0,
  [_ACc, _E, _SCt],
  [[() => S3DestinationAccessControl$, { [_jN]: _aCc }], [() => S3EncryptionSettings$, { [_jN]: _en }], [0, { [_jN]: _sCt }]]
];
export var S3EncryptionSettings$: StaticStructureSchema = [3, n0, _SES,
  0,
  [_ETn, _KEC, _KKA],
  [[0, { [_jN]: _eTn }], [0, { [_jN]: _kEC }], [0, { [_jN]: _kKA }]]
];
export var SccDestinationSettings$: StaticStructureSchema = [3, n0, _SDS,
  0,
  [_Fr],
  [[0, { [_jN]: _fr }]]
];
export var SearchJobsRequest$: StaticStructureSchema = [3, n0, _SJR,
  0,
  [_IFn, _MRa, _NT, _Or, _Q, _Sta],
  [[0, { [_hQ]: _iFn }], [1, { [_hQ]: _mRa }], [0, { [_hQ]: _nT }], [0, { [_hQ]: _or }], [0, { [_hQ]: _q }], [0, { [_hQ]: _sta }]]
];
export var SearchJobsResponse$: StaticStructureSchema = [3, n0, _SJRe,
  0,
  [_Jo, _NT],
  [[() => __listOfJob, { [_jN]: _jo }], [0, { [_jN]: _nT }]]
];
export var ServiceOverride$: StaticStructureSchema = [3, n0, _SOerv,
  0,
  [_M, _Na, _OV, _Val],
  [[0, { [_jN]: _m }], [0, { [_jN]: _na }], [0, { [_jN]: _oV }], [0, { [_jN]: _val }]]
];
export var SpekeKeyProvider$: StaticStructureSchema = [3, n0, _SKP,
  0,
  [_CAe, _ECC, _RIe, _SIy, _U],
  [[0, { [_jN]: _cAe }], [() => EncryptionContractConfiguration$, { [_jN]: _eCC }], [0, { [_jN]: _rIe }], [64 | 0, { [_jN]: _sIy }], [0, { [_jN]: _u }]]
];
export var SpekeKeyProviderCmaf$: StaticStructureSchema = [3, n0, _SKPC,
  0,
  [_CAe, _DSSI, _ECC, _HSSI, _RIe, _U],
  [[0, { [_jN]: _cAe }], [64 | 0, { [_jN]: _dSSI }], [() => EncryptionContractConfiguration$, { [_jN]: _eCC }], [64 | 0, { [_jN]: _hSSI }], [0, { [_jN]: _rIe }], [0, { [_jN]: _u }]]
];
export var SrtDestinationSettings$: StaticStructureSchema = [3, n0, _SDSr,
  0,
  [_SPt],
  [[0, { [_jN]: _sPt }]]
];
export var StartJobsQueryRequest$: StaticStructureSchema = [3, n0, _SJQR,
  0,
  [_FLi, _MRa, _NT, _Or],
  [[() => __listOfJobsQueryFilter, { [_jN]: _fLi }], [1, { [_jN]: _mRa }], [0, { [_jN]: _nT }], [0, { [_jN]: _or }]]
];
export var StartJobsQueryResponse$: StaticStructureSchema = [3, n0, _SJQRt,
  0,
  [_I],
  [[0, { [_jN]: _id }]]
];
export var StaticKeyProvider$: StaticStructureSchema = [3, n0, _SKPt,
  0,
  [_KF, _KFV, _SKV, _U],
  [[0, { [_jN]: _kF }], [0, { [_jN]: _kFV }], [0, { [_jN]: _sKV }], [0, { [_jN]: _u }]]
];
export var TagResourceRequest$: StaticStructureSchema = [3, n0, _TRR,
  0,
  [_A, _Ta],
  [[0, { [_jN]: _a }], [128 | 0, { [_jN]: _ta }]], 2
];
export var TagResourceResponse$: StaticStructureSchema = [3, n0, _TRRa,
  0,
  [],
  []
];
export var TeletextDestinationSettings$: StaticStructureSchema = [3, n0, _TDS,
  0,
  [_PNa, _PT],
  [[0, { [_jN]: _pNa }], [64 | 0, { [_jN]: _pT }]]
];
export var TeletextSourceSettings$: StaticStructureSchema = [3, n0, _TSS,
  0,
  [_PNa],
  [[0, { [_jN]: _pNa }]]
];
export var TimecodeBurnin$: StaticStructureSchema = [3, n0, _TB,
  0,
  [_FSon, _Pos, _Pref],
  [[1, { [_jN]: _fSon }], [0, { [_jN]: _pos }], [0, { [_jN]: _pref }]]
];
export var TimecodeConfig$: StaticStructureSchema = [3, n0, _TCi,
  0,
  [_An, _Sou, _Star, _TO],
  [[0, { [_jN]: _an }], [0, { [_jN]: _sou }], [0, { [_jN]: _star }], [0, { [_jN]: _tO }]]
];
export var TimedMetadataInsertion$: StaticStructureSchema = [3, n0, _TMI,
  0,
  [_IId],
  [[() => __listOfId3Insertion, { [_jN]: _iId }]]
];
export var Timing$: StaticStructureSchema = [3, n0, _Timi,
  0,
  [_FTi, _STta, _STub],
  [[7, { [_jN]: _fTi }], [7, { [_jN]: _sTta }], [7, { [_jN]: _sTub }]]
];
export var Track$: StaticStructureSchema = [3, n0, _Tr,
  0,
  [_AP, _Co, _DP, _Du, _Ind, _TT, _VPi],
  [[() => AudioProperties$, { [_jN]: _aPu }], [0, { [_jN]: _co }], [() => DataProperties$, { [_jN]: _dP }], [1, { [_jN]: _du }], [1, { [_jN]: _ind }], [0, { [_jN]: _tT }], [() => VideoProperties$, { [_jN]: _vPi }]]
];
export var TrackMapping$: StaticStructureSchema = [3, n0, _TMra,
  0,
  [_ATI, _DTI, _VTI],
  [[64 | 1, { [_jN]: _aTI }], [64 | 1, { [_jN]: _dTI }], [64 | 1, { [_jN]: _vTI }]]
];
export var TrackSourceSettings$: StaticStructureSchema = [3, n0, _TSSr,
  0,
  [_SNt, _TN],
  [[1, { [_jN]: _sNt }], [1, { [_jN]: _tN }]]
];
export var TtmlDestinationSettings$: StaticStructureSchema = [3, n0, _TDSt,
  0,
  [_BC, _BO, _FCo, _FO, _FSon, _FSont, _FW, _SPt, _TDe],
  [[0, { [_jN]: _bC }], [1, { [_jN]: _bO }], [0, { [_jN]: _fCo }], [1, { [_jN]: _fO }], [1, { [_jN]: _fSon }], [0, { [_jN]: _fSont }], [0, { [_jN]: _fW }], [0, { [_jN]: _sPt }], [0, { [_jN]: _tDe }]]
];
export var UncompressedSettings$: StaticStructureSchema = [3, n0, _US,
  0,
  [_Fo, _FC, _FCA, _FD, _FN, _IM, _STCM, _SPl, _Te],
  [[0, { [_jN]: _fo }], [0, { [_jN]: _fC }], [0, { [_jN]: _fCA }], [1, { [_jN]: _fD }], [1, { [_jN]: _fN }], [0, { [_jN]: _iM }], [0, { [_jN]: _sTCM }], [0, { [_jN]: _sPl }], [0, { [_jN]: _te }]]
];
export var UntagResourceRequest$: StaticStructureSchema = [3, n0, _URR,
  0,
  [_A, _TK],
  [[0, 1], [64 | 0, { [_jN]: _tK }]], 1
];
export var UntagResourceResponse$: StaticStructureSchema = [3, n0, _URRn,
  0,
  [],
  []
];
export var UpdateJobTemplateRequest$: StaticStructureSchema = [3, n0, _UJTR,
  0,
  [_Na, _AScc, _Ca, _Desc, _HD, _Pri, _Q, _Se, _SUI],
  [[0, 1], [() => AccelerationSettings$, { [_jN]: _aScc }], [0, { [_jN]: _ca }], [0, { [_jN]: _desc }], [() => __listOfHopDestination, { [_jN]: _hD }], [1, { [_jN]: _pri }], [0, { [_jN]: _q }], [() => JobTemplateSettings$, { [_jN]: _set }], [0, { [_jN]: _sUI }]], 1
];
export var UpdateJobTemplateResponse$: StaticStructureSchema = [3, n0, _UJTRp,
  0,
  [_JT],
  [[() => JobTemplate$, { [_jN]: _jT }]]
];
export var UpdatePresetRequest$: StaticStructureSchema = [3, n0, _UPR,
  0,
  [_Na, _Ca, _Desc, _Se],
  [[0, 1], [0, { [_jN]: _ca }], [0, { [_jN]: _desc }], [() => PresetSettings$, { [_jN]: _set }]], 1
];
export var UpdatePresetResponse$: StaticStructureSchema = [3, n0, _UPRp,
  0,
  [_Pre],
  [[() => Preset$, { [_jN]: _pre }]]
];
export var UpdateQueueRequest$: StaticStructureSchema = [3, n0, _UQR,
  0,
  [_Na, _CJ, _Desc, _MCF, _RPS, _Sta],
  [[0, 1], [1, { [_jN]: _cJ }], [0, { [_jN]: _desc }], [1, { [_jN]: _mCF }], [() => ReservationPlanSettings$, { [_jN]: _rPS }], [0, { [_jN]: _sta }]], 1
];
export var UpdateQueueResponse$: StaticStructureSchema = [3, n0, _UQRp,
  0,
  [_Q],
  [[() => Queue$, { [_jN]: _q }]]
];
export var Vc3Settings$: StaticStructureSchema = [3, n0, _VSc,
  0,
  [_FC, _FCA, _FD, _FN, _IM, _STCM, _SPl, _Te, _VC],
  [[0, { [_jN]: _fC }], [0, { [_jN]: _fCA }], [1, { [_jN]: _fD }], [1, { [_jN]: _fN }], [0, { [_jN]: _iM }], [0, { [_jN]: _sTCM }], [0, { [_jN]: _sPl }], [0, { [_jN]: _te }], [0, { [_jN]: _vC }]]
];
export var VideoCodecSettings$: StaticStructureSchema = [3, n0, _VCS,
  0,
  [_ASv, _AIS, _Co, _FCS, _GSi, _HS, _HSe, _MSpeg, _PSa, _PSro, _US, _VSc, _VSp, _VSpe, _XS],
  [[() => Av1Settings$, { [_jN]: _aSv }], [() => AvcIntraSettings$, { [_jN]: _aIS }], [0, { [_jN]: _co }], [() => FrameCaptureSettings$, { [_jN]: _fCS }], [() => GifSettings$, { [_jN]: _gSi }], [() => H264Settings$, { [_jN]: _hSe }], [() => H265Settings$, { [_jN]: _hSet }], [() => Mpeg2Settings$, { [_jN]: _mSpeg }], [() => PassthroughSettings$, { [_jN]: _pSa }], [() => ProresSettings$, { [_jN]: _pSr }], [() => UncompressedSettings$, { [_jN]: _uS }], [() => Vc3Settings$, { [_jN]: _vSc }], [() => Vp8Settings$, { [_jN]: _vSp }], [() => Vp9Settings$, { [_jN]: _vSpe }], [() => XavcSettings$, { [_jN]: _xS }]]
];
export var VideoDescription$: StaticStructureSchema = [3, n0, _VD,
  0,
  [_ASf, _AA, _CPM, _CS, _CMol, _Cr, _DFT, _FA, _H, _Pos, _RTA, _SB, _Sha, _TIi, _TTi, _VPid, _W],
  [[0, { [_jN]: _aSf }], [0, { [_jN]: _aA }], [0, { [_jN]: _cPM }], [() => VideoCodecSettings$, { [_jN]: _cS }], [0, { [_jN]: _cMo }], [() => Rectangle$, { [_jN]: _cr }], [0, { [_jN]: _dFT }], [1, { [_jN]: _fA }], [1, { [_jN]: _h }], [() => Rectangle$, { [_jN]: _pos }], [0, { [_jN]: _rTA }], [0, { [_jN]: _sB }], [1, { [_jN]: _sha }], [0, { [_jN]: _tIi }], [0, { [_jN]: _tTi }], [() => VideoPreprocessor$, { [_jN]: _vPid }], [1, { [_jN]: _w }]]
];
export var VideoDetail$: StaticStructureSchema = [3, n0, _VDid,
  0,
  [_HIP, _WIP],
  [[1, { [_jN]: _hIP }], [1, { [_jN]: _wIP }]]
];
export var VideoOverlay$: StaticStructureSchema = [3, n0, _VOi,
  0,
  [_Cr, _ET, _IP, _In, _Pl, _STt, _Tra],
  [[() => VideoOverlayCrop$, { [_jN]: _cr }], [0, { [_jN]: _eT }], [() => VideoOverlayPosition$, { [_jN]: _iP }], [() => VideoOverlayInput$, { [_jN]: _inpu }], [0, { [_jN]: _pl }], [0, { [_jN]: _sTt }], [() => __listOfVideoOverlayTransition, { [_jN]: _tr }]]
];
export var VideoOverlayCrop$: StaticStructureSchema = [3, n0, _VOC,
  0,
  [_H, _Un, _W, _X, _Y],
  [[1, { [_jN]: _h }], [0, { [_jN]: _un }], [1, { [_jN]: _w }], [1, { [_jN]: _x }], [1, { [_jN]: _y }]]
];
export var VideoOverlayInput$: StaticStructureSchema = [3, n0, _VOI,
  0,
  [_ASud, _FI, _ICn, _TSi, _TSim],
  [[() => __mapOfAudioSelector, { [_jN]: _aSu }], [0, { [_jN]: _fI }], [() => __listOfVideoOverlayInputClipping, { [_jN]: _iCn }], [0, { [_jN]: _tSi }], [0, { [_jN]: _tSim }]]
];
export var VideoOverlayInputClipping$: StaticStructureSchema = [3, n0, _VOIC,
  0,
  [_ET, _STt],
  [[0, { [_jN]: _eT }], [0, { [_jN]: _sTt }]]
];
export var VideoOverlayPosition$: StaticStructureSchema = [3, n0, _VOP,
  0,
  [_H, _Op, _Un, _W, _XP, _YP],
  [[1, { [_jN]: _h }], [1, { [_jN]: _op }], [0, { [_jN]: _un }], [1, { [_jN]: _w }], [1, { [_jN]: _xP }], [1, { [_jN]: _yP }]]
];
export var VideoOverlayTransition$: StaticStructureSchema = [3, n0, _VOT,
  0,
  [_EPn, _ET, _STt],
  [[() => VideoOverlayPosition$, { [_jN]: _ePn }], [0, { [_jN]: _eT }], [0, { [_jN]: _sTt }]]
];
export var VideoPreprocessor$: StaticStructureSchema = [3, n0, _VPide,
  0,
  [_CCo, _Dei, _DV, _DC, _HP, _IIm, _NR, _PW, _TB],
  [[() => ColorCorrector$, { [_jN]: _cCo }], [() => Deinterlacer$, { [_jN]: _dei }], [() => DolbyVision$, { [_jN]: _dV }], [() => DurationControl$, { [_jN]: _dCu }], [() => Hdr10Plus$, { [_jN]: _hP }], [() => ImageInserter$, { [_jN]: _iIm }], [() => NoiseReducer$, { [_jN]: _nR }], [() => PartnerWatermarking$, { [_jN]: _pW }], [() => TimecodeBurnin$, { [_jN]: _tB }]]
];
export var VideoProperties$: StaticStructureSchema = [3, n0, _VPi,
  0,
  [_BD, _BR, _CMo, _CPo, _DAR, _FR, _HMd, _H, _MCat, _Ro, _SAR, _TCr, _W],
  [[1, { [_jN]: _bD }], [1, { [_jN]: _bR }], [() => CodecMetadata$, { [_jN]: _cMod }], [0, { [_jN]: _cPo }], [() => AspectRatio$, { [_jN]: _dAR }], [() => FrameRate$, { [_jN]: _fR }], [() => HdrMetadata$, { [_jN]: _hMd }], [1, { [_jN]: _h }], [0, { [_jN]: _mCat }], [1, { [_jN]: _ro }], [() => AspectRatio$, { [_jN]: _sAR }], [0, { [_jN]: _tCr }], [1, { [_jN]: _w }]]
];
export var VideoSelector$: StaticStructureSchema = [3, n0, _VSi,
  0,
  [_ABl, _CSol, _CSU, _ETO, _HM, _ML, _PV, _Pi, _PNr, _Rot, _SRa, _ST, _St],
  [[0, { [_jN]: _aBl }], [0, { [_jN]: _cSol }], [0, { [_jN]: _cSU }], [0, { [_jN]: _eTO }], [() => Hdr10Metadata$, { [_jN]: _hM }], [1, { [_jN]: _mL }], [0, { [_jN]: _pV }], [1, { [_jN]: _pi }], [1, { [_jN]: _pNr }], [0, { [_jN]: _rot }], [0, { [_jN]: _sRa }], [0, { [_jN]: _sT }], [64 | 1, { [_jN]: _st }]]
];
export var VorbisSettings$: StaticStructureSchema = [3, n0, _VS,
  0,
  [_C, _SR, _VQ],
  [[1, { [_jN]: _ch }], [1, { [_jN]: _sR }], [1, { [_jN]: _vQ }]]
];
export var Vp8Settings$: StaticStructureSchema = [3, n0, _VSp,
  0,
  [_B, _FC, _FCA, _FD, _FN, _GS, _HBS, _MB, _PCa, _PD, _PN, _QTL, _RCM],
  [[1, { [_jN]: _b }], [0, { [_jN]: _fC }], [0, { [_jN]: _fCA }], [1, { [_jN]: _fD }], [1, { [_jN]: _fN }], [1, { [_jN]: _gS }], [1, { [_jN]: _hBS }], [1, { [_jN]: _mB }], [0, { [_jN]: _pCa }], [1, { [_jN]: _pD }], [1, { [_jN]: _pN }], [0, { [_jN]: _qTL }], [0, { [_jN]: _rCM }]]
];
export var Vp9Settings$: StaticStructureSchema = [3, n0, _VSpe,
  0,
  [_B, _FC, _FCA, _FD, _FN, _GS, _HBS, _MB, _PCa, _PD, _PN, _QTL, _RCM],
  [[1, { [_jN]: _b }], [0, { [_jN]: _fC }], [0, { [_jN]: _fCA }], [1, { [_jN]: _fD }], [1, { [_jN]: _fN }], [1, { [_jN]: _gS }], [1, { [_jN]: _hBS }], [1, { [_jN]: _mB }], [0, { [_jN]: _pCa }], [1, { [_jN]: _pD }], [1, { [_jN]: _pN }], [0, { [_jN]: _qTL }], [0, { [_jN]: _rCM }]]
];
export var WarningGroup$: StaticStructureSchema = [3, n0, _WG,
  0,
  [_Cod, _Cou],
  [[1, { [_jN]: _cod }], [1, { [_jN]: _cou }]], 2
];
export var WavSettings$: StaticStructureSchema = [3, n0, _WS,
  0,
  [_BD, _C, _F, _SR],
  [[1, { [_jN]: _bD }], [1, { [_jN]: _ch }], [0, { [_jN]: _f }], [1, { [_jN]: _sR }]]
];
export var WebvttDestinationSettings$: StaticStructureSchema = [3, n0, _WDS,
  0,
  [_Ac, _SPt],
  [[0, { [_jN]: _ac }], [0, { [_jN]: _sPt }]]
];
export var WebvttHlsSourceSettings$: StaticStructureSchema = [3, n0, _WHSS,
  0,
  [_RGI, _RLC, _RN],
  [[0, { [_jN]: _rGI }], [0, { [_jN]: _rLC }], [0, { [_jN]: _rN }]]
];
export var Xavc4kIntraCbgProfileSettings$: StaticStructureSchema = [3, n0, _XICPS,
  0,
  [_XC],
  [[0, { [_jN]: _xC }]]
];
export var Xavc4kIntraVbrProfileSettings$: StaticStructureSchema = [3, n0, _XIVPS,
  0,
  [_XC],
  [[0, { [_jN]: _xC }]]
];
export var Xavc4kProfileSettings$: StaticStructureSchema = [3, n0, _XPSa,
  0,
  [_BCi, _CP, _FAQ, _GBR, _GCC, _HBS, _QTL, _Sl],
  [[0, { [_jN]: _bCi }], [0, { [_jN]: _cP }], [0, { [_jN]: _fAQ }], [0, { [_jN]: _gBR }], [1, { [_jN]: _gCC }], [1, { [_jN]: _hBS }], [0, { [_jN]: _qTL }], [1, { [_jN]: _sl }]]
];
export var XavcHdIntraCbgProfileSettings$: StaticStructureSchema = [3, n0, _XHICPS,
  0,
  [_IM, _XC],
  [[0, { [_jN]: _iM }], [0, { [_jN]: _xC }]]
];
export var XavcHdProfileSettings$: StaticStructureSchema = [3, n0, _XHPS,
  0,
  [_BCi, _FAQ, _GBR, _GCC, _HBS, _IM, _QTL, _Sl, _Te],
  [[0, { [_jN]: _bCi }], [0, { [_jN]: _fAQ }], [0, { [_jN]: _gBR }], [1, { [_jN]: _gCC }], [1, { [_jN]: _hBS }], [0, { [_jN]: _iM }], [0, { [_jN]: _qTL }], [1, { [_jN]: _sl }], [0, { [_jN]: _te }]]
];
export var XavcSettings$: StaticStructureSchema = [3, n0, _XS,
  0,
  [_AQ, _EE, _FC, _FCA, _FD, _FN, _PFM, _Pr, _SPl, _So, _SAQ, _TAQ, _XICPS, _XIVPS, _XPSa, _XHICPS, _XHPS],
  [[0, { [_jN]: _aQ }], [0, { [_jN]: _eE }], [0, { [_jN]: _fC }], [0, { [_jN]: _fCA }], [1, { [_jN]: _fD }], [1, { [_jN]: _fN }], [64 | 0, { [_jN]: _pFM }], [0, { [_jN]: _pr }], [0, { [_jN]: _sPl }], [1, { [_jN]: _so }], [0, { [_jN]: _sAQ }], [0, { [_jN]: _tAQ }], [() => Xavc4kIntraCbgProfileSettings$, { [_jN]: _xICPS }], [() => Xavc4kIntraVbrProfileSettings$, { [_jN]: _xIVPS }], [() => Xavc4kProfileSettings$, { [_jN]: _xPSa }], [() => XavcHdIntraCbgProfileSettings$, { [_jN]: _xHICPS }], [() => XavcHdProfileSettings$, { [_jN]: _xHPS }]]
];
var __listOf__doubleMinNegative60Max6 = 64 | 1;
var __listOf__integer = 64 | 1;
var __listOf__integerMin1Max2147483647 = 64 | 1;
var __listOf__integerMin32Max8182 = 64 | 1;
var __listOf__integerMinNegative60Max6 = 64 | 1;
var __listOf__string = 64 | 0;
var __listOf__stringMax100 = 64 | 0;
var __listOf__stringMin1 = 64 | 0;
var __listOf__stringMin36Max36Pattern09aFAF809aFAF409aFAF409aFAF409aFAF12 = 64 | 0;
var __listOf__stringPattern09aFAF809aFAF409aFAF409aFAF409aFAF12 = 64 | 0;
var __listOf__stringPatternS3ASSETMAPXml = 64 | 0;
var __listOfAllowedRenditionSize: StaticListSchema = [1, n0, _lOARS,
  0, [() => AllowedRenditionSize$,
    0]
];
var __listOfAudioChannelTag = 64 | 0;
var __listOfAudioDescription: StaticListSchema = [1, n0, _lOAD,
  0, [() => AudioDescription$,
    0]
];
var __listOfAutomatedAbrRule: StaticListSchema = [1, n0, _lOAAR,
  0, [() => AutomatedAbrRule$,
    0]
];
var __listOfCaptionDescription: StaticListSchema = [1, n0, _lOCD,
  0, [() => CaptionDescription$,
    0]
];
var __listOfCaptionDescriptionPreset: StaticListSchema = [1, n0, _lOCDP,
  0, [() => CaptionDescriptionPreset$,
    0]
];
var __listOfCmafAdditionalManifest: StaticListSchema = [1, n0, _lOCAM,
  0, [() => CmafAdditionalManifest$,
    0]
];
var __listOfCmafImageBasedTrickPlayVariant: StaticListSchema = [1, n0, _lOCIBTPV,
  0, [() => CmafImageBasedTrickPlayVariant$,
    0]
];
var __listOfColorConversion3DLUTSetting: StaticListSchema = [1, n0, _lOCCDLUTS,
  0, [() => ColorConversion3DLUTSetting$,
    0]
];
var __listOfDashAdditionalManifest: StaticListSchema = [1, n0, _lODAM,
  0, [() => DashAdditionalManifest$,
    0]
];
var __listOfDashIsoImageBasedTrickPlayVariant: StaticListSchema = [1, n0, _lODIIBTPV,
  0, [() => DashIsoImageBasedTrickPlayVariant$,
    0]
];
var __listOfElementalInferenceFeature = 64 | 0;
var __listOfElementalInferenceFeed: StaticListSchema = [1, n0, _lOEIF,
  0, [() => ElementalInferenceFeed$,
    0]
];
var __listOfEndpoint: StaticListSchema = [1, n0, _lOE,
  0, [() => Endpoint$,
    0]
];
var __listOfForceIncludeRenditionSize: StaticListSchema = [1, n0, _lOFIRS,
  0, [() => ForceIncludeRenditionSize$,
    0]
];
var __listOfFrameMetricType = 64 | 0;
var __listOfHlsAdditionalManifest: StaticListSchema = [1, n0, _lOHAM,
  0, [() => HlsAdditionalManifest$,
    0]
];
var __listOfHlsAdMarkers = 64 | 0;
var __listOfHlsCaptionLanguageMapping: StaticListSchema = [1, n0, _lOHCLM,
  0, [() => HlsCaptionLanguageMapping$,
    0]
];
var __listOfHlsImageBasedTrickPlayVariant: StaticListSchema = [1, n0, _lOHIBTPV,
  0, [() => HlsImageBasedTrickPlayVariant$,
    0]
];
var __listOfHopDestination: StaticListSchema = [1, n0, _lOHD,
  0, [() => HopDestination$,
    0]
];
var __listOfId3Insertion: StaticListSchema = [1, n0, _lOII,
  0, [() => Id3Insertion$,
    0]
];
var __listOfInput: StaticListSchema = [1, n0, _lOI,
  0, [() => Input$,
    0]
];
var __listOfInputClipping: StaticListSchema = [1, n0, _lOIC,
  0, [() => InputClipping$,
    0]
];
var __listOfInputTemplate: StaticListSchema = [1, n0, _lOIT,
  0, [() => InputTemplate$,
    0]
];
var __listOfInsertableImage: StaticListSchema = [1, n0, _lOIIi,
  0, [() => InsertableImage$,
    0]
];
var __listOfJob: StaticListSchema = [1, n0, _lOJ,
  0, [() => Job$,
    0]
];
var __listOfJobEngineVersion: StaticListSchema = [1, n0, _lOJEV,
  0, [() => JobEngineVersion$,
    0]
];
var __listOfJobsQueryFilter: StaticListSchema = [1, n0, _lOJQF,
  0, [() => JobsQueryFilter$,
    0]
];
var __listOfJobTemplate: StaticListSchema = [1, n0, _lOJT,
  0, [() => JobTemplate$,
    0]
];
var __listOfMotionImageInserter: StaticListSchema = [1, n0, _lOMII,
  0, [() => MotionImageInserter$,
    0]
];
var __listOfMsSmoothAdditionalManifest: StaticListSchema = [1, n0, _lOMSAM,
  0, [() => MsSmoothAdditionalManifest$,
    0]
];
var __listOfMultiViewSettings: StaticListSchema = [1, n0, _lOMVS,
  0, [() => MultiViewSettings$,
    0]
];
var __listOfOutput: StaticListSchema = [1, n0, _lOO,
  0, [() => Output$,
    0]
];
var __listOfOutputChannelMapping: StaticListSchema = [1, n0, _lOOCM,
  0, [() => OutputChannelMapping$,
    0]
];
var __listOfOutputDetail: StaticListSchema = [1, n0, _lOOD,
  0, [() => OutputDetail$,
    0]
];
var __listOfOutputGroup: StaticListSchema = [1, n0, _lOOG,
  0, [() => OutputGroup$,
    0]
];
var __listOfOutputGroupDetail: StaticListSchema = [1, n0, _lOOGD,
  0, [() => OutputGroupDetail$,
    0]
];
var __listOfPreset: StaticListSchema = [1, n0, _lOP,
  0, [() => Preset$,
    0]
];
var __listOfProbeInputFile: StaticListSchema = [1, n0, _lOPIF,
  0, [() => ProbeInputFile$,
    0]
];
var __listOfProbeResult: StaticListSchema = [1, n0, _lOPR,
  0, [() => ProbeResult$,
    0]
];
var __listOfQueue: StaticListSchema = [1, n0, _lOQ,
  0, [() => Queue$,
    0]
];
var __listOfQueueTransition: StaticListSchema = [1, n0, _lOQT,
  0, [() => QueueTransition$,
    0]
];
var __listOfServiceOverride: StaticListSchema = [1, n0, _lOSO,
  0, [() => ServiceOverride$,
    0]
];
var __listOfTeletextPageType = 64 | 0;
var __listOfTrack: StaticListSchema = [1, n0, _lOT,
  0, [() => Track$,
    0]
];
var __listOfTrackMapping: StaticListSchema = [1, n0, _lOTM,
  0, [() => TrackMapping$,
    0]
];
var __listOfVideoOverlay: StaticListSchema = [1, n0, _lOVO,
  0, [() => VideoOverlay$,
    0]
];
var __listOfVideoOverlayInputClipping: StaticListSchema = [1, n0, _lOVOIC,
  0, [() => VideoOverlayInputClipping$,
    0]
];
var __listOfVideoOverlayTransition: StaticListSchema = [1, n0, _lOVOT,
  0, [() => VideoOverlayTransition$,
    0]
];
var __listOfWarningGroup: StaticListSchema = [1, n0, _lOWG,
  0, [() => WarningGroup$,
    0]
];
var __mapOf__string = 128 | 0;
var __mapOfAudioSelector: StaticMapSchema = [2, n0, _mOAS,
  0, [0,
    0]
  , [() => AudioSelector$,
    0]
];
var __mapOfAudioSelectorGroup: StaticMapSchema = [2, n0, _mOASG,
  0, [0,
    0]
  , [() => AudioSelectorGroup$,
    0]
];
var __mapOfCaptionSelector: StaticMapSchema = [2, n0, _mOCS,
  0, [0,
    0]
  , [() => CaptionSelector$,
    0]
];
var __mapOfDynamicAudioSelector: StaticMapSchema = [2, n0, _mODAS,
  0, [0,
    0]
  , [() => DynamicAudioSelector$,
    0]
];
export var AssociateCertificate$: StaticOperationSchema = [9, n0, _ACs,
  { [_ht]: ["POST", "/2017-08-29/certificates", 201] }, () => AssociateCertificateRequest$, () => AssociateCertificateResponse$
];
export var CancelJob$: StaticOperationSchema = [9, n0, _CJa,
  { [_ht]: ["DELETE", "/2017-08-29/jobs/{Id}", 202] }, () => CancelJobRequest$, () => CancelJobResponse$
];
export var CreateJob$: StaticOperationSchema = [9, n0, _CJr,
  { [_ht]: ["POST", "/2017-08-29/jobs", 201] }, () => CreateJobRequest$, () => CreateJobResponse$
];
export var CreateJobTemplate$: StaticOperationSchema = [9, n0, _CJT,
  { [_ht]: ["POST", "/2017-08-29/jobTemplates", 201] }, () => CreateJobTemplateRequest$, () => CreateJobTemplateResponse$
];
export var CreatePreset$: StaticOperationSchema = [9, n0, _CPr,
  { [_ht]: ["POST", "/2017-08-29/presets", 201] }, () => CreatePresetRequest$, () => CreatePresetResponse$
];
export var CreateQueue$: StaticOperationSchema = [9, n0, _CQ,
  { [_ht]: ["POST", "/2017-08-29/queues", 201] }, () => CreateQueueRequest$, () => CreateQueueResponse$
];
export var CreateResourceShare$: StaticOperationSchema = [9, n0, _CRS,
  { [_ht]: ["POST", "/2017-08-29/resourceShares", 202] }, () => CreateResourceShareRequest$, () => CreateResourceShareResponse$
];
export var DeleteJobTemplate$: StaticOperationSchema = [9, n0, _DJT,
  { [_ht]: ["DELETE", "/2017-08-29/jobTemplates/{Name}", 202] }, () => DeleteJobTemplateRequest$, () => DeleteJobTemplateResponse$
];
export var DeletePolicy$: StaticOperationSchema = [9, n0, _DPe,
  { [_ht]: ["DELETE", "/2017-08-29/policy", 200] }, () => DeletePolicyRequest$, () => DeletePolicyResponse$
];
export var DeletePreset$: StaticOperationSchema = [9, n0, _DPel,
  { [_ht]: ["DELETE", "/2017-08-29/presets/{Name}", 202] }, () => DeletePresetRequest$, () => DeletePresetResponse$
];
export var DeleteQueue$: StaticOperationSchema = [9, n0, _DQe,
  { [_ht]: ["DELETE", "/2017-08-29/queues/{Name}", 202] }, () => DeleteQueueRequest$, () => DeleteQueueResponse$
];
export var DescribeEndpoints$: StaticOperationSchema = [9, n0, _DE,
  { [_ht]: ["POST", "/2017-08-29/endpoints", 200] }, () => DescribeEndpointsRequest$, () => DescribeEndpointsResponse$
];
export var DisassociateCertificate$: StaticOperationSchema = [9, n0, _DCi,
  { [_ht]: ["DELETE", "/2017-08-29/certificates/{Arn}", 202] }, () => DisassociateCertificateRequest$, () => DisassociateCertificateResponse$
];
export var GetJob$: StaticOperationSchema = [9, n0, _GJ,
  { [_ht]: ["GET", "/2017-08-29/jobs/{Id}", 200] }, () => GetJobRequest$, () => GetJobResponse$
];
export var GetJobsQueryResults$: StaticOperationSchema = [9, n0, _GJQR,
  { [_ht]: ["GET", "/2017-08-29/jobsQueries/{Id}", 200] }, () => GetJobsQueryResultsRequest$, () => GetJobsQueryResultsResponse$
];
export var GetJobTemplate$: StaticOperationSchema = [9, n0, _GJT,
  { [_ht]: ["GET", "/2017-08-29/jobTemplates/{Name}", 200] }, () => GetJobTemplateRequest$, () => GetJobTemplateResponse$
];
export var GetPolicy$: StaticOperationSchema = [9, n0, _GP,
  { [_ht]: ["GET", "/2017-08-29/policy", 200] }, () => GetPolicyRequest$, () => GetPolicyResponse$
];
export var GetPreset$: StaticOperationSchema = [9, n0, _GPe,
  { [_ht]: ["GET", "/2017-08-29/presets/{Name}", 200] }, () => GetPresetRequest$, () => GetPresetResponse$
];
export var GetQueue$: StaticOperationSchema = [9, n0, _GQ,
  { [_ht]: ["GET", "/2017-08-29/queues/{Name}", 200] }, () => GetQueueRequest$, () => GetQueueResponse$
];
export var ListJobs$: StaticOperationSchema = [9, n0, _LJ,
  { [_ht]: ["GET", "/2017-08-29/jobs", 200] }, () => ListJobsRequest$, () => ListJobsResponse$
];
export var ListJobTemplates$: StaticOperationSchema = [9, n0, _LJT,
  { [_ht]: ["GET", "/2017-08-29/jobTemplates", 200] }, () => ListJobTemplatesRequest$, () => ListJobTemplatesResponse$
];
export var ListPresets$: StaticOperationSchema = [9, n0, _LP,
  { [_ht]: ["GET", "/2017-08-29/presets", 200] }, () => ListPresetsRequest$, () => ListPresetsResponse$
];
export var ListQueues$: StaticOperationSchema = [9, n0, _LQ,
  { [_ht]: ["GET", "/2017-08-29/queues", 200] }, () => ListQueuesRequest$, () => ListQueuesResponse$
];
export var ListTagsForResource$: StaticOperationSchema = [9, n0, _LTFR,
  { [_ht]: ["GET", "/2017-08-29/tags/{Arn}", 200] }, () => ListTagsForResourceRequest$, () => ListTagsForResourceResponse$
];
export var ListVersions$: StaticOperationSchema = [9, n0, _LV,
  { [_ht]: ["GET", "/2017-08-29/versions", 200] }, () => ListVersionsRequest$, () => ListVersionsResponse$
];
export var Probe$: StaticOperationSchema = [9, n0, _Pro,
  { [_ht]: ["POST", "/2017-08-29/probe", 200] }, () => ProbeRequest$, () => ProbeResponse$
];
export var PutPolicy$: StaticOperationSchema = [9, n0, _PPu,
  { [_ht]: ["PUT", "/2017-08-29/policy", 200] }, () => PutPolicyRequest$, () => PutPolicyResponse$
];
export var SearchJobs$: StaticOperationSchema = [9, n0, _SJ,
  { [_ht]: ["GET", "/2017-08-29/search", 200] }, () => SearchJobsRequest$, () => SearchJobsResponse$
];
export var StartJobsQuery$: StaticOperationSchema = [9, n0, _SJQ,
  { [_ht]: ["POST", "/2017-08-29/jobsQueries", 201] }, () => StartJobsQueryRequest$, () => StartJobsQueryResponse$
];
export var TagResource$: StaticOperationSchema = [9, n0, _TR,
  { [_ht]: ["POST", "/2017-08-29/tags", 200] }, () => TagResourceRequest$, () => TagResourceResponse$
];
export var UntagResource$: StaticOperationSchema = [9, n0, _UR,
  { [_ht]: ["PUT", "/2017-08-29/tags/{Arn}", 200] }, () => UntagResourceRequest$, () => UntagResourceResponse$
];
export var UpdateJobTemplate$: StaticOperationSchema = [9, n0, _UJT,
  { [_ht]: ["PUT", "/2017-08-29/jobTemplates/{Name}", 200] }, () => UpdateJobTemplateRequest$, () => UpdateJobTemplateResponse$
];
export var UpdatePreset$: StaticOperationSchema = [9, n0, _UP,
  { [_ht]: ["PUT", "/2017-08-29/presets/{Name}", 200] }, () => UpdatePresetRequest$, () => UpdatePresetResponse$
];
export var UpdateQueue$: StaticOperationSchema = [9, n0, _UQ,
  { [_ht]: ["PUT", "/2017-08-29/queues/{Name}", 200] }, () => UpdateQueueRequest$, () => UpdateQueueResponse$
];
