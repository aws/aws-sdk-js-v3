 ...
 * @internal
 */
export function resolveLogins(
    logins: Logins
): Promise<ResolvedLogins> {
    return Promise.all(
        Object.keys(logins).reduce(
            (arr: Array<[string, string]|Promise<[string, string]>>, name: string) => {
                ...
                if (typeof tokenOrProvider === 'string') {
                    arr.push(
                        [name, tokenOrProvider] as [string, string]
                    );
                } else {
                    arr.push(
                        tokenOrProvider()
                            .then(token => [name, token] as [string, string])
                    );
                }
                return arr;
            },
    ...
    ).then(resolvedPairs => resolvedPairs.reduce(
