...
describe("getSigningKey", () => {
  ...
  describe("caching", () => {
    ...
    it("should cache a maximum of 50 entries", () => {
      ...
      expect(keyPromises[1]).toBe(
        getSigningKey(Sha256, credentials, shortDate, `us-foo-1`, service)
      );

      // the oldest member should not be in the cache
      expect(keyPromises[0]).not.toBe(
        getSigningKey(Sha256, credentials, shortDate, `us-foo-0`, service)
      );
    });
  });
});
