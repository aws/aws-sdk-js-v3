...
describe("XmlBodyParser", () => {
  ...
  describe("list", () => {
    ...
    it("should parse list with attributes in tags", () => {
      let xml =
        '<xml><Item xsi:name="Jon"><Age>20</Age></Item><Item xsi:name="Lee"><Age>18</Age></Item></xml>';
      let rules: Member = {
        shape: {
          type: "structure",
          required: [],
  ...
  describe("maps", () => {
    ...
    it("should parse map with only one key-value entry", () => {
      let xml =
        "<xml><SummaryMap><entry><key>firstName</key><value>Daniel</value></entry></SummaryMap></xml>";
      expect(parser.parse(rules, xml)).toEqual({
        SummaryMap: {
          firstName: "Daniel"
        }
