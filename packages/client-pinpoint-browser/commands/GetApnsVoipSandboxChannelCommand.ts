...
export class GetApnsVoipSandboxChannelCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<GetApnsVoipSandboxChannelInput, GetApnsVoipSandboxChannelOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: GetApnsVoipSandboxChannel
        };

        return stack.resolve(
            handler<GetApnsVoipSandboxChannelInput, GetApnsVoipSandboxChannelOutput>(handlerExecutionContext),
