...
export class DeleteApnsSandboxChannelCommand implements __aws_sdk_types.Command<
    InputTypesUnion,
...
> {
    ...
    ): __aws_sdk_types.Handler<DeleteApnsSandboxChannelInput, DeleteApnsSandboxChannelOutput> {
        const {handler} = configuration;
        const stack = clientStack.concat(this.middlewareStack);

        const handlerExecutionContext: __aws_sdk_types.HandlerExecutionContext = {
            logger: {} as any,
            model: DeleteApnsSandboxChannel
        };

        return stack.resolve(
            handler<DeleteApnsSandboxChannelInput, DeleteApnsSandboxChannelOutput>(handlerExecutionContext),
