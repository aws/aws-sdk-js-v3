...
export class JsonParser implements BodyParser {
  ...
  public parse<OutputType>(shape: Member, input: string): OutputType {
  ...
  private unmarshall(shape: SerializationModel, input: JsonValue): any {
    if (shape.type === "structure") {
      ...
      return Object.keys(shape.members).reduce(
        (output: { [key: string]: any }, memberName: string) => {
          const {
    ...
    if (shape.type === "map") {
      ...
      return Object.keys(input).reduce(
        (output: { [key: string]: any }, key: string) => {
          output[key] = this.unmarshall(valueShape, (input as Json)[key]);
          return output;
        },
        {}
      );
