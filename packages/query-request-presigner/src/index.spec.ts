...
describe("presignRequestQuery", () => {
  ...
  const request: HttpRequest = {
    ...
    body:
      "Action=CopySnapshot&Version=2016-11-15&DestinationRegion=mock-region&SourceRegion=src-region&SourceSnapshotId=snap%3Asnap-123456789"
  };
  ...
  it("should update endpoint in signed request", async () => {
    ...
    expect(presignedUrl.hostname).toEqual("ec2.src-region.amazonaws.com");
  ...
  it("should move query string in body to request query", async () => {
    ...
    expect(query).toMatchObject({
      ...
      SourceSnapshotId: "snap:snap-123456789"
    });
  });
  ...
  it("should sign the request with sigV4", async () => {
    ...
    const dateString = now
      ...
      .split("-")
      .join("");
    expect(query["X-Amz-Credential"]).toMatch(
      new RegExp(`akid/${dateString}/src-region/ec2/aws4_request`)
    );
