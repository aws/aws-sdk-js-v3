...
export const getDefaultRetryToken = (
  ...
  const getCapacityAmount = (errorType: RetryErrorType) =>
    errorType === RetryErrorType.TRANSIENT ? timeoutRetryCost : retryCost;
  ...
  const retrieveRetryTokens = (errorInfo: RetryErrorInfo) => {
    ...
    const delayBase = errorType === RetryErrorType.THROTTLING ? THROTTLING_RETRY_DELAY_BASE : DEFAULT_RETRY_DELAY_BASE;
    const delayFromErrorType = Math.floor(Math.min(MAXIMUM_RETRY_DELAY, Math.random() * 2 ** retryCount * delayBase));
