...
describe("renameShape", () => {
  for (let shapeName of Object.keys(model.shapes)) {
    ...
    it(`should rename references to ${shapeName}`, () => {
      ...
      expect(isMember(modelClone, newName)).toBe(isMember(model, shapeName));
    });
    ...
    it(`should rename operation usages of ${shapeName}`, () => {
      ...
      expect(isReferencedByOperation(modelClone, newName)).toBe(
        isReferencedByOperation(model, shapeName)
      );
    });
  }
