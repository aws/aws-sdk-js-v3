...
describe("isReferencedByOperation", () => {
  ...
  it("should return true when an operation uses the shape as its input", () => {
  ...
  it("should return true when an operation uses the shape as its output", () => {
    for (let operationName of Object.keys(operations)) {
  ...
  it("should return true when an operation uses the shape as its errors", () => {
    for (let operationName of Object.keys(operations)) {
      const { errors } = operations[operationName];
      for (let error of errors) {
        expect(isReferencedByOperation(model, error.shape)).toBe(true);
      }
    }
  });

  it("should return false for shapes not referenced directly by operations", () => {
