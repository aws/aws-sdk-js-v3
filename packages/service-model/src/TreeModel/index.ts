...
function fromApiModelShapeMap(
  shapeMap: ShapeMap,
  metadata: ServiceMetadata
): TreeModelShapeMap {
  const map: { [name: string]: any } = Object.keys(shapeMap).reduce(
...
function fromApiModelOperationMap(
  ...
  return Object.keys(operationMap).reduce(
    (carry: TreeModelOperationMap, operationName: string) => {
      const { authtype, name, errors, http, input, output } = operationMap[
        operationName
      ];
      const { documentation = `${name} operation` } = operationMap[
        operationName
      ];
...
function visitBlob(toPopulate: Partial<ProtocolBlob>, sourceData: Blob): void {
...
function visitTimestamp(
  ...
  toPopulate.timestampFormat =
    sourceData.timestampFormat || serviceTimestampFormat;
}
...
function visitStructure(
  ...
  if (sourceData.exception) {
    ...
    toPopulate.exceptionCode =
      (sourceData.error && sourceData.error.code) || void 0;
  }

  toPopulate.members = Object.keys(sourceData.members).reduce(
    (members: { [key: string]: TreeModelMember }, memberName: string) => {
