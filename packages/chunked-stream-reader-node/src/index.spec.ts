...
describe('streamReader', () => {
    ...
    it(
        'calls onChunk with remaining bytes during last call',
        async () => {
            const buffers = [
                Buffer.alloc(1000, 0),
                Buffer.alloc(1000, 1),
    ...
    it(
        'buffers "data" chunks until the chunkSize is reached',
        async () => {
            const buffers = [
                Buffer.alloc(100, 0),
                Buffer.alloc(100, 1),
    ...
    it(
        'calls onChunk if end of stream is reached before chunkSize is reached',
        async () => {
            const buffers = [
                Buffer.alloc(100, 0),
                Buffer.alloc(100, 1),
    ...
    it(
        'rejects if an error is encountered in the read stream',
        async () => {
