...
export const jsonErrorUnmarshaller: ServiceExceptionParser = (
  ...
  let errorName =
    parseErrorCodeFromHeader(response.headers, errorCodeHeaderName) ||
  ...
  for (let errorShape of errors) {
    ...
    if (
      ...
      let rawException = errorStructure.members
...
function parseErrorCodeFromHeader(
  headers: HeaderBag,
...
function parseErrorCodeFromBody(
  body: string,
  errorCodeFieldNameList: string[]
): string | undefined {
  if (body) {
