...
import { epoch, iso8601, rfc822 } from "@aws-sdk/protocol-timestamp";
import {
  Blob as BlobShape,
  BodySerializer,
...
export class XmlBodyBuilder implements BodySerializer {
  ...
  private serializeMap(
    ...
    const {
      ...
      key: { locationName: xmlKey = "key" },
      value: { locationName: xmlValue = "value" }
    } = shape;
  ...
  private formatMap(
    ...
    return new XmlNode(name, [keyNode, valueNode]);
  }
  ...
  private serializeTimestamp(node: XmlNode, member: Member, input: any) {
    const timestampFormat = member.timestampFormat
      ? member.timestampFormat
      : (member.shape as TimestampShape).timestampFormat;
