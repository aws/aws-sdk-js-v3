...
import { ERR_RESP_SHAPE_LEGACY, ParsedLegacyErrorResponse } from "./constants";
...
export const ec2ErrorUnmarshaller: ServiceExceptionParser = (
  ...
  const { errorName, errorMessage, requestId } = parseErrorCommonProperties(
    errorBodyParser,
    body
  );
  if (!errorName) {
...
function parseErrorCommonProperties(
  ...
  let parsedLegacyErrorResponse = errorBodyParser.parse<
    ParsedLegacyErrorResponse
  >(ERR_RESP_SHAPE_LEGACY, body);
  let requestId = parsedLegacyErrorResponse.$metadata
    ? parsedLegacyErrorResponse.$metadata.requestId
    : undefined;
