...
export function defaultProvider(configuration: RegionProviderConfiguration = {}): Provider<string> {
  return memoize(chain(fromEnv(configuration), fromSharedConfigFiles(configuration)));
}
