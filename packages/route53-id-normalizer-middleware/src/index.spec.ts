...
describe('locationConstrainMiddleware', () => {
    for (const paramName of idParams) {
        for (const prefixed of prefixedProps) {
            it(
                `should strip the prefix from the ${paramName} parameter`,
                async () => {
                    const next = jest.fn(() => ({}));
...
describe('changeBatchAliasTargetIdNormalizerMiddleware', () => {
    for (const prefixed of prefixedProps) {
        it(
            `should strip the prefix from the ChangeBatch.Changes[*].ResourceRecordSet.AliasTarget.HostedZoneId parameter`,
            async () => {
                const next = jest.fn(() => ({}));
                const input = {
                    ChangeBatch: {
