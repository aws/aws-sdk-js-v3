...
export class QueryBuilder implements BodySerializer {
  ...
  public build({ operation, input }: BodySerializerBuildOptions): string {
  ...
  private serialize(prefix: string, input: any, member: Member): string {
    ...
    if (shape.type === "structure") {
      ...
      throw new Error(`cannot parse type ${(shape as any).type}.`);
    }
  }
  ...
  private serializeStructure(
    ...
    if (typeof input !== "object" || input === null) {
      throw new Error(
        `Unable to serialize value of type ${typeof input} as a` + " structure"
      );
    }
    for (let key of Object.keys(input).sort()) {
      ...
      const { locationName = key, queryName } = shape.members[key];
  ...
  private serializeList(prefix: string, input: any, shape: List): string {
    ...
    const { locationName = "member" } = shape.member;
    let listIndex = 0;
  ...
  private serializeMap(prefix: string, input: any, shape: Map): string {
    ...
    if (isIterable(input)) {
      ...
      for (let [key, value] of input) {
        serialized.push(
          this.serializeMapEntry(prefix, entryCount, key, value, shape)
        );
        entryCount += 1;
      }
    } else {
      ...
      for (let key of Object.keys(input)) {
        serialized.push(
          this.serializeMapEntry(prefix, entryCount, key, input[key], shape)
        );
        entryCount += 1;
      }
    }
