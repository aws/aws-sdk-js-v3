...
describe('bucketHostname', () => {
    ...
    it(
        'should use a path-style endpoint when the bucket name contains a dot',
        () => {
            const baseHostname = 's3.us-west-2.amazonaws.com';
            const {bucketEndpoint, hostname} = bucketHostname({
                bucketName: 'foo.bar',
    ...
    it(
        'should use a virtual-hosted-style endpoint when SSL compatibility is not requested and the bucket name contains a dot',
        () => {
            const baseHostname = 's3.us-west-2.amazonaws.com';
            const {bucketEndpoint, hostname} = bucketHostname({
                bucketName: 'foo.bar',
    ...
    for (const [baseHostname, dualstackHostname] of [
        ...
        it(
            `should derive a virtual-hosted-style accelerate hostname of s3-accelerate.amazonaws.com for the base hostname of ${baseHostname}`,
            () => {
                const {bucketEndpoint, hostname} = bucketHostname({
                    bucketName: 'foo',
                    baseHostname,
        ...
        it(
            `should derive a virtual-hosted-style accelerate dualstack hostname of s3-accelerate.amazonaws.com for the base hostname of ${baseHostname}`,
            () => {
                const {bucketEndpoint, hostname} = bucketHostname({
                    bucketName: 'foo',
                    baseHostname,
        ...
        it(
            `should derive a virtual-hosted-style dualstack hostname of ${dualstackHostname} for the base hostname of ${baseHostname}`,
            () => {
                const {bucketEndpoint, hostname} = bucketHostname({
                    bucketName: 'foo',
                    baseHostname,
        ...
        it(
            `should derive a path-style dualstack hostname of ${dualstackHostname} for the base hostname of ${baseHostname}`,
            () => {
                const {bucketEndpoint, hostname} = bucketHostname({
                    bucketName: 'foo',
                    baseHostname,
    ...
    for (const nonDnsCompliantBucketName of [
        ...
        it(
            `should use a path-style endpoint for the non-DNS-compliant bucket name of ${nonDnsCompliantBucketName}`,
            () => {
                const baseHostname = 's3.us-west-2.amazonaws.com';
                const {bucketEndpoint, hostname} = bucketHostname({
