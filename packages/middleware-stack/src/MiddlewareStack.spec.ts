...
describe("MiddlewareStack", () => {
  it("should resolve the stack into a composed handler", async () => {
    ...
    stack.add(getConcatMiddleware("seventh") as FinalizeRequestMiddleware<input, output>, {
      step: "finalizeRequest"
    });
  ...
  it("should allow the removal of middleware by constructor identity", async () => {
    const stack = new MiddlewareStack<input, output>();
    stack.add(getConcatMiddleware("don't remove me") as InitializeMiddleware<input, output>, {
      name: "notRemove"
    });
