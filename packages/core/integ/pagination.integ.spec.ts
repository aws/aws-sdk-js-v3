...
describe("pagination", () => {
   ...
   */
  it("client test - batch items until all items are processed or an error is received", async () => {
    const ddb = new DynamoDB({
      credentials: {
        accessKeyId: "INTEG_TEST",
        secretAccessKey: "INTEG_TEST",
    ...
    const requestParams: BatchGetItemCommandInput = {
      RequestItems: {
        test: {
          Keys: [{ id: { S: "1" } }, { id: { S: "2" } }, { id: { S: "3" } }, { id: { S: "4" } }, { id: { S: "5" } }],
        },
      },
    };

    let pages = 0;
    for await (const page of paginatedBatchGetItem({ client: ddb }, requestParams)) {
      pages += 1;
      if (pages === 1) {
    ...
    try {
      for await (const page of paginatedBatchGetItem(
        { client: ddb },
        {
          RequestItems: {
            test: { Keys: [{ id: { S: "1" } }, { id: { S: "1" } }] },
          },
        }
      )) {
        void page;
  ...
  it("lib test - processes batch items until all items are processed or an error is received", async () => {
    const ddb = new DynamoDB({
      credentials: {
        accessKeyId: "INTEG_TEST",
        secretAccessKey: "INTEG_TEST",
