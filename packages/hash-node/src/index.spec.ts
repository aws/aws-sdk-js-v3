    ...
    for (const supportedHash of ['md5', 'sha256']) {
        describe(`${supportedHash} test vectors`, () => {
            for (const {input, ...results} of hashVectors) {
                ...
                it(
                    `should calculate a ${supportedHash} hash of ${expected} for an input of ${input}`,
                    async () => {
                        const hash = new Hash(supportedHash);
                        hash.update(fromString(input, 'base64'));
                        const {buffer} = await hash.digest();
                        expect(fromArrayBuffer(buffer).toString('hex'))
                            .toBe(expected);
                    }
                );
            }
            ...
            for (const {key, data, ...results} of hmacVectors) {
                const expected = results[supportedHash];
                it(
                    `should calculate a ${supportedHash} hmac of ${expected} for an input of ${data} and a key of ${key}`,
                    async () => {
                        const hash = new Hash(
                            supportedHash,
                            fromString(key, 'hex')
                        ...
                        const {buffer} = await hash.digest();
                        expect(
                            fromArrayBuffer(buffer).toString('hex')
                        ).toMatch(expected);
                    }
                );
            }
