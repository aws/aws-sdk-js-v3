...
describe("JsonBuilder", () => {
  describe("structures", () => {
    ...
    it("should serialize known properties of a structure", () => {
      ...
      expect(jsonBody.build({ operation, input: toSerialize })).toBe(
    ...
    it("should ignore unknown properties", () => {
      const toSerialize = { foo: "fizz", bar: "buzz" };
      expect(
        jsonBody.build({ operation, input: { ...toSerialize, pop: "weasel" } })
      ).toBe(JSON.stringify(toSerialize));
    });

    it("should serialize properties to the locationNames", () => {
      expect(jsonBody.build({ operation, input: { baz: "value" } })).toEqual(
  ...
  describe("lists", () => {
    ...
    it("should serialize arrays", () => {
      expect(
        jsonBody.build({ operation, input: { list: ["foo", "bar", "baz"] } })
    ...
    it("should serialize iterators", () => {
      ...
      expect(
        jsonBody.build({ operation, input: { list: iterator() } })
      ).toEqual(JSON.stringify({ list: ["foo", "bar", "baz"] }));
    });
    ...
    it("should throw if a non-iterable value is provided", () => {
      for (let nonIterable of [{}, 123, true]) {
        expect(() =>
          jsonBody.build({ operation, input: { list: nonIterable } })
        ).toThrow();
      }
    });
  });
  ...
  describe("maps", () => {
    ...
    it("should serialize objects", () => {
      ...
      expect(jsonBody.build({ operation, input: object })).toEqual(
    ...
    it("should serialize [key, value] iterables (like ES6 maps)", () => {
      ...
      expect(jsonBody.build({ operation, input: { map: iterator() } })).toEqual(
        JSON.stringify({
          map: {
            foo: 0,
            bar: 1,
    ...
    it("should throw if a non-iterable and non-object value is provided", () => {
      for (let nonIterable of [123, true]) {
        expect(() =>
          jsonBody.build({ operation, input: { map: nonIterable } })
        ).toThrow();
      }
    });
  });
  ...
  describe("blobs", () => {
    ...
    it("should throw if a non-binary value is provided", () => {
      for (let nonBinary of [[], {}, 123, true]) {
        expect(() =>
          jsonBody.build({ operation, input: { blob: nonBinary } })
        ).toThrow();
      }
    });
  });
  ...
  describe("timestamps", () => {
    ...
    it("should convert Date objects to epoch timestamps", () => {
      expect(jsonBody.build({ operation, input: { timestamp: date } })).toBe(
    ...
    it("should convert date strings to epoch timestamps", () => {
      expect(
        jsonBody.build({
          operation,
          input: { timestamp: date.toUTCString() as any }
        })
    ...
    it("should preserve numbers as epoch timestamps", () => {
      expect(
        jsonBody.build({ operation, input: { timestamp: timestamp as any } })
      ).toBe(JSON.stringify({ timestamp }));
    });

    it("should format dates using timestampFormat trait of shape", () => {
      expect(
        jsonBody.build({ operation, input: { timestampCustom: date } })
      ).toBe(JSON.stringify({ timestampCustom }));
    });

    it("should format dates using timestampFormat trait of member", () => {
      expect(
        jsonBody.build({ operation, input: { timestampMember: date } })
    ...
    it("should throw if a value that cannot be converted to a time object is provided", () => {
      for (let nonTime of [[], {}, true, new ArrayBuffer(0)]) {
        expect(() =>
          jsonBody.build({ operation, input: { timestamp: nonTime } })
        ).toThrow();
      }
    });
  });
  ...
  describe("scalars", () => {
    it("should echo back scalars in their JSON-ified form", () => {
      ...
      for (let [shape, scalar] of cases) {
        ...
        expect(jsonBody.build({ operation, input: scalar })).toBe(
