...
describe('defaultUserAgent', () => {
    it('should response basic node default user agent', () => {
        ...
        expect(defaultUserAgent('client-s3-node', '0.1.0')).toBe(`aws-sdk-nodejs-v3-client-s3-node/0.1.0 ${process.platform}/${process.version}`);
    ...
    it('should add AWS_EXECUTION_ENV', () => {
        ...
        expect(defaultUserAgent('client-s3-node', '0.1.0')).toBe(`aws-sdk-nodejs-v3-client-s3-node/0.1.0 ${process.platform}/${process.version} exec-env/ecs`);
        if (originEnv) process.env.AWS_EXECUTION_ENV = originEnv;
    });
