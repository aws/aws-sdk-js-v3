...
export class TypeGenerator {
  ...
  constructor(
    ...
    for (const operationName of Object.keys(commands)) {
      ...
      this.scopedCustomizations[outputName] = customParams.filter(
        isOutputParam
      );
    }
  }
  ...
  *[Symbol.iterator](): Iterator<[string, string]> {
    ...
    for (let shapeName of Object.keys(shapes)) {
      ...
      if (shape.type === "structure") {
        if (shape.topLevel === "input") {
          ...
          yield [shapeName, new Exception(shape).toString()];
        } else {
          yield [shapeName, new ModeledStructure(shape).toString()];
        }
      }
    }
    ...
    for (let operationName of Object.keys(operations)) {
      const deduplicatedErrorNames = operations[operationName].errors.reduce<
        Set<string>
      ...
      yield [
        ...
        new Union(
          Array.from(deduplicatedErrorNames),
          this.exceptionTypesUnion(operationName)
        ).toString()
      ];
    }
...
function isParamCustomization(
  ...
  return (
    arg.type === "SyntheticParameter" || arg.type === "ParameterSuppression"
  );
}
