...
import { dependenciesFromCustomization } from "./Components/helpers/dependenciesFromCustomization";
import { getServiceCustomizations } from "./ServiceCustomizations";

export class CommandGenerator {
  private readonly commandToCustomizations: Map<
    string,
    CustomizationDefinition[]
  >;
  ...
  *[Symbol.iterator](): Iterator<[string, string]> {
    ...
    for (let operationName of Object.keys(operations)) {
      ...
      const command = new Command(
        operation,
        this.target,
        this.commandToCustomizations.get(operationName.toLowerCase())
      );
