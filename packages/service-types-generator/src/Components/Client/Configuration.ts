...
${new IndentedSection(this.configurationProperties())}
  ...
  private configuration(): string {
    ...
    for (const key of Object.keys(this.config).sort()) {
      let { documentation, inputType, internal, ...property } = this.config[
        key
      ];
      if (internal) {
        continue;
      ...
      if (property.type === "unified") {
        ...
        const { required: req, additionalDocumentation } = property[
          this.target
        ];
        required = req;
...
${key}${required ? "" : "?"}: ${inputType};`
  ...
  private configurationProperties(): string {
    ...
    for (const key of Object.keys(this.config)) {
      const value = new ConfigurationProperty(this.target, this.config[key]);
  ...
  private resolvableConfiguration(): string {
    const properties: Array<string> = [];
    for (const key of Object.keys(this.config).sort()) {
      const { documentation, inputType, internal } = this.config[key];
...
${key}: ${inputType};`
  ...
  private resolvedConfiguration(): string {
    ...
    for (const key of Object.keys(this.config).sort()) {
      ...
      const { default: dflt, required } =
        property.type === "unified" ? property : property[this.target];

      const { inputType, resolvedType, internal } = property;

      if (internal || resolvedType || (!required && dflt)) {
  ...
  private imports(): string {
    const allImports: Import[] = [IMPORTS["types"]];
