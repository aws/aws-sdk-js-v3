...
describe("getMemberType", () => {
  it("should translate blob to buffer", () => {
    expect(getMemberType(NonStreamingBlob)).toEqual("Uint8Array");
  });
  ...
  it("should allow streams for streaming structure members", () => {
    expect(
      getMemberType(NonStreamingBlob, {
  ...
  it("should return arrays for lists", () => {
    expect(getMemberType(NodeList)).toEqual("Array<string>");
  });

  it("should return objects for maps", () => {
    expect(getMemberType(PrimaryLocationMap)).toEqual(
      `{[key: string]: string}`
    );
  });
  ...
  it("should return objects matching an interface for structures", () => {
    expect(getMemberType(ValidationException)).toEqual(
      getUnmarshalledShapeName(ValidationException.name)
    );
  });
});
