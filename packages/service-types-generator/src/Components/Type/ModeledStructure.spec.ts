...
describe("ModeledStructure", () => {
  it("should emit documentation, an empty interface, and an aliased type for an empty structure", () => {
    const name = "DataStructure";
    const structure: TreeModelStructure = {
      name,
      type: "structure",
      documentation:
        "Structured data that is nested in operation input or output",
      required: [],
      members: {}
    };
...
export type ${getUnmarshalledShapeName(name)} = ${name};`
  ...
  it("should emit an output structure that is an alias of the main (input) structure when all members are scalar", () => {
    const name = "DataStructure";
    const structure: TreeModelStructure = {
      name: "DataStructure",
      ...
      members: {
        boolean: {
          shape: { type: "boolean", name: "boolean", documentation: "boolean" }
        },
        float: {
          shape: { type: "float", name: "float", documentation: "float" }
        },
        integer: {
          shape: { type: "integer", name: "integer", documentation: "integer" }
        },
        string: {
          shape: { type: "string", name: "string", documentation: "string" }
        }
      }
    };
...
export type ${getUnmarshalledShapeName(structure.name)} = ${structure.name};`
  ...
  it("should define a child interface for output when a member has a stricter unmarshalled type than marshalled type", () => {
    const name = "DataStructure";
    const structure: TreeModelStructure = {
      name,
      documentation: "a structure",
...
export interface ${getUnmarshalledShapeName(name)} extends ${name} {
  ...
  it("should import structure shapes in both input and output forms", () => {
    ...
    expect(new ModeledStructure(structure).toString()).toEqual(
      `import {${nestedStructure.name}, ${getUnmarshalledShapeName(
        nestedStructure.name
      )}} from './${nestedStructure.name}';
