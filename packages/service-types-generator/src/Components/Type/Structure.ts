    ...
    return [
      ...new Set(
        Object.keys(this.shape.members)
          ...
          .reduce(
            (shapes: Array<TreeModelShape>, shape: TreeModelShape) =>
 ...
 */`;
  ...
  protected getInterfaceDefinition(memberName: string): string {
    ...
    return `
${this.docBlock(
  member.documentation ? member.documentation : member.shape.documentation
)}
${memberName}${this.isRequired(memberName) ? "" : "?"}: ${getInterfaceType(
      member.shape,
      member
    )};
        `.trim();
...
${memberName}${this.isRequired(memberName) ? "" : "?"}: ${getMemberType(
      member.shape,
      member
    )};
        `.trim();
  ...
  private importableShapesFor(shape: TreeModelShape): Array<TreeModelShape> {
    if (shape.type === "list") {
      ...
      return this.importableShapesFor(shape.key.shape).concat(
        this.importableShapesFor(shape.value.shape)
      );
  ...
  protected get memberNames(): Array<string> {
    ...
    return Object.keys(this.shape.members).filter(
      name => suppressions.indexOf(name) === -1
    );
  }

  protected get syntheticParameters(): Array<[string, string]> {
    return this.customizations.filter(isSyntheticParameter).map(
      ({ documentation, name, required, typeExpression }) =>
        [
          name,
