...
export class Input extends Structure {
  constructor(
    ...
    super(shape, customizations.concat(INPUT_CONTROL_PROPERTIES));
  }
  ...
  [
    ...new Map<string, string>(
      this.memberNames
        .map(
          memberName =>
            [memberName, this.getInterfaceDefinition(memberName)] as [
              string,
              string
            ]
        )
    ...
    switch (this.runtime) {
      case "node":
        if (hasStreamingBody(this.shape)) {
          return [INPUT_TYPES_IMPORT_NODE, new FullPackageImport("stream")];
        }
