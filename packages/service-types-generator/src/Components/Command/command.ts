...
export class Command {
  ...
  get prefix() {
    return serviceIdFromMetadata(this.operation.metadata).replace(/\s/g, "");
  }
  ...
  toString(): string {
    ...
    const middlewareStackPackage = packageNameToVariable(
...
${this.customizations
  ...
  .map((definition: any) => {
    return new IndentedSection(
      customizationFromMiddleware(definition, "stack"),
      2
    );
  })
  .join("\n")}
