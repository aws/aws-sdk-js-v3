import {Import} from '@aws/build-types';

/**
 * @internal
 */
export const IMPORTS: {[key: string]: Import} = {
    'abort-controller': {
        package: '@aws/abort-controller',
        version: '^0.0.1',
    },
    'add-glacier-checksum-headers-browser': {
        package: '@aws/add-glacier-checksum-headers-browser',
        version: '^0.0.1',
    },
    'add-glacier-checksum-headers-node': {
        package: '@aws/add-glacier-checksum-headers-node',
        version: '^0.0.1',
    },
    'add-glacier-checksum-headers-universal': {
        package: '@aws/add-glacier-checksum-headers-universal',
        version: '^0.0.1',
    },
    'apply-body-checksum-middleware': {
        package: '@aws/apply-body-checksum-middleware',
        version: '^0.0.1',
    },
    'bucket-endpoint-middleware': {
        package: '@aws/bucket-endpoint-middleware',
        version: '^0.0.1',
    },
    'build-types': {
        package: '@aws/build-types',
        version: '^0.0.1',
    },
    'chunked-blob-reader': {
        package: '@aws/chunked-blob-reader',
        version: '^0.0.1',
    },
    'chunked-stream-reader-node': {
        package: '@aws/chunked-stream-reader-node',
        version: '^0.0.1',
    },
    'client-documentation-generator': {
        package: '@aws/client-documentation-generator',
        version: '^0.0.1',
    },
    'config-resolver': {
        package: '@aws/config-resolver',
        version: '^0.0.1',
    },
    'core-handler': {
        package: '@aws/core-handler',
        version: '^0.0.1',
    },
    'crc32': {
        package: '@aws/crc32',
        version: '^0.0.1',
    },
    'credential-provider-cognito-identity': {
        package: '@aws/credential-provider-cognito-identity',
        version: '^0.0.1',
    },
    'credential-provider-env': {
        package: '@aws/credential-provider-env',
        version: '^0.0.1',
    },
    'credential-provider-imds': {
        package: '@aws/credential-provider-imds',
        version: '^0.0.1',
    },
    'credential-provider-ini': {
        package: '@aws/credential-provider-ini',
        version: '^0.0.1',
    },
    'credential-provider-node': {
        package: '@aws/credential-provider-node',
        version: '^0.0.1',
    },
    'crypto-ie11-detection': {
        package: '@aws/crypto-ie11-detection',
        version: '^0.0.1',
    },
    'crypto-random-source-browser': {
        package: '@aws/crypto-random-source-browser',
        version: '^0.0.1',
    },
    'crypto-random-source-node': {
        package: '@aws/crypto-random-source-node',
        version: '^0.0.1',
    },
    'crypto-random-source-universal': {
        package: '@aws/crypto-random-source-universal',
        version: '^0.0.1',
    },
    'crypto-sha256-browser': {
        package: '@aws/crypto-sha256-browser',
        version: '^0.0.1',
    },
    'crypto-sha256-js': {
        package: '@aws/crypto-sha256-js',
        version: '^0.0.1',
    },
    'crypto-sha256-universal': {
        package: '@aws/crypto-sha256-universal',
        version: '^0.0.1',
    },
    'crypto-supports-webCrypto': {
        package: '@aws/crypto-supports-webCrypto',
        version: '^0.0.1',
    },
    'ec2-error-unmarshaller': {
        package: '@aws/ec2-error-unmarshaller',
        version: '^0.0.1',
    },
    'eventstream-marshaller': {
        package: '@aws/eventstream-marshaller',
        version: '^0.0.1',
    },
    'fetch-http-handler': {
        package: '@aws/fetch-http-handler',
        version: '^0.0.1',
    },
    'hash-blob-browser': {
        package: '@aws/hash-blob-browser',
        version: '^0.0.1',
    },
    'hash-node': {
        package: '@aws/hash-node',
        version: '^0.0.1',
    },
    'hash-stream-node': {
        package: '@aws/hash-stream-node',
        version: '^0.0.1',
    },
    'http-handler': {
        package: '@aws/http-handler',
        version: '^0.0.1',
    },
    'http-serialization': {
        package: '@aws/http-serialization',
        version: '^0.0.1',
    },
    'is-array-buffer': {
        package: '@aws/is-array-buffer',
        version: '^0.0.1',
    },
    'is-iterable': {
        package: '@aws/is-iterable',
        version: '^0.0.1',
    },
    'is-node': {
        package: '@aws/is-node',
        version: '^0.0.1',
    },
    'json-builder': {
        package: '@aws/json-builder',
        version: '^0.0.1',
    },
    'json-error-unmarshaller': {
        package: '@aws/json-error-unmarshaller',
        version: '^0.0.1',
    },
    'json-parser': {
        package: '@aws/json-parser',
        version: '^0.0.1',
    },
    'location-constraint-middleware': {
        package: '@aws/location-constraint-middleware',
        version: '^0.0.1',
    },
    'logger': {
        package: '@aws/logger',
        version: '^0.0.1',
    },
    'md5-js': {
        package: '@aws/md5-js',
        version: '^0.0.1',
    },
    'md5-universal': {
        package: '@aws/md5-universal',
        version: '^0.0.1',
    },
    'middleware-content-length': {
        package: '@aws/middleware-content-length',
        version: '^0.0.1',
    },
    'middleware-ec2-copysnapshot': {
        package: '@aws/middleware-ec2-copysnapshot',
        version: '^0.0.1',
    },
    'middleware-header-default': {
        package: '@aws/middleware-header-default',
        version: '^0.0.1',
    },
    'middleware-input-default': {
        package: '@aws/middleware-input-default',
        version: '^0.0.1',
    },
    'middleware-operation-logging': {
        package: '@aws/middleware-operation-logging',
        version: '^0.0.1',
    },
    'middleware-rds-presignedurl': {
        package: '@aws/middleware-rds-presignedurl',
        version: '^0.0.1',
    },
    'middleware-sdk-api-gateway': {
        package: '@aws/middleware-sdk-api-gateway',
        version: '^0.0.1',
    },
    'middleware-sdk-glacier': {
        package: '@aws/middleware-sdk-glacier',
        version: '^0.0.1',
    },
    'middleware-serializer': {
        package: '@aws/middleware-serializer',
        version: '^0.0.1',
    },
    'middleware-stack': {
        package: '@aws/middleware-stack',
        version: '^0.0.1',
    },
    'modeled-endpoint-middleware': {
        package: '@aws/modeled-endpoint-middleware',
        version: '^0.0.1',
    },
    'node-http-handler': {
        package: '@aws/node-http-handler',
        version: '^0.0.1',
    },
    'package-generator': {
        package: '@aws/package-generator',
        version: '^0.0.1',
    },
    'property-provider': {
        package: '@aws/property-provider',
        version: '^0.0.1',
    },
    'protocol-json-rpc': {
        package: '@aws/protocol-json-rpc',
        version: '^0.0.1',
    },
    'protocol-query': {
        package: '@aws/protocol-query',
        version: '^0.0.1',
    },
    'protocol-rest': {
        package: '@aws/protocol-rest',
        version: '^0.0.1',
    },
    'protocol-timestamp': {
        package: '@aws/protocol-timestamp',
        version: '^0.0.1',
    },
    'query-builder': {
        package: '@aws/query-builder',
        version: '^0.0.1',
    },
    'query-error-unmarshaller': {
        package: '@aws/query-error-unmarshaller',
        version: '^0.0.1',
    },
    'query-request-presigner': {
        package: '@aws/query-request-presigner',
        version: '^0.0.1',
    },
    'querystring-builder': {
        package: '@aws/querystring-builder',
        version: '^0.0.1',
    },
    'querystring-parser': {
        package: '@aws/querystring-parser',
        version: '^0.0.1',
    },
    'region-provider': {
        package: '@aws/region-provider',
        version: '^0.0.1',
    },
    'remove-sensitive-logs': {
        package: '@aws/remove-sensitive-logs',
        version: '^0.0.1',
    },
    'response-metadata-extractor': {
        package: '@aws/response-metadata-extractor',
        version: '^0.0.1',
    },
    'retry-middleware': {
        package: '@aws/retry-middleware',
        version: '^0.0.1',
    },
    'route53-id-normalizer-middleware': {
        package: '@aws/route53-id-normalizer-middleware',
        version: '^0.0.1',
    },
    'sdk-codecommit-browser': {
        package: '@aws/sdk-codecommit-browser',
        version: '^0.0.1',
    },
    'sdk-codecommit-node': {
        package: '@aws/sdk-codecommit-node',
        version: '^0.0.1',
    },
    'sdk-cognito-identity-browser': {
        package: '@aws/sdk-cognito-identity-browser',
        version: '^0.0.1',
    },
    'sdk-glacier-browser': {
        package: '@aws/sdk-glacier-browser',
        version: '^0.0.1',
    },
    'sdk-glacier-node': {
        package: '@aws/sdk-glacier-node',
        version: '^0.0.1',
    },
    'sdk-kms-node': {
        package: '@aws/sdk-kms-node',
        version: '^0.0.1',
    },
    'sdk-machine-learning-node': {
        package: '@aws/sdk-machine-learning-node',
        version: '^0.0.1',
    },
    'sdk-route-53-node': {
        package: '@aws/sdk-route-53-node',
        version: '^0.0.1',
    },
    'sdk-s3-node': {
        package: '@aws/sdk-s3-node',
        version: '^0.0.1',
    },
    'service-error-classification': {
        package: '@aws/service-error-classification',
        version: '^0.0.1',
    },
    'service-model': {
        package: '@aws/service-model',
        version: '^0.0.1',
    },
    'service-types-generator': {
        package: '@aws/service-types-generator',
        version: '^0.0.1',
    },
    'sha256-tree-hash': {
        package: '@aws/sha256-tree-hash',
        version: '^0.0.1',
    },
    'shared-ini-file-loader': {
        package: '@aws/shared-ini-file-loader',
        version: '^0.0.1',
    },
    'signature-v4': {
        package: '@aws/signature-v4',
        version: '^0.0.1',
    },
    'signature-v4-browser': {
        package: '@aws/signature-v4-browser',
        version: '^0.0.1',
    },
    'signature-v4-node': {
        package: '@aws/signature-v4-node',
        version: '^0.0.1',
    },
    'signature-v4-universal': {
        package: '@aws/signature-v4-universal',
        version: '^0.0.1',
    },
    'signing-middleware': {
        package: '@aws/signing-middleware',
        version: '^0.0.1',
    },
    'sns-message-validator': {
        package: '@aws/sns-message-validator',
        version: '^0.4.0',
    },
    'ssec-middleware': {
        package: '@aws/ssec-middleware',
        version: '^0.0.1',
    },
    'stream-collector-browser': {
        package: '@aws/stream-collector-browser',
        version: '^0.0.1',
    },
    'stream-collector-node': {
        package: '@aws/stream-collector-node',
        version: '^0.0.1',
    },
    'test-protocol-rest-xml': {
        package: '@aws/test-protocol-rest-xml',
        version: '^0.0.1',
    },
    'types': {
        package: '@aws/types',
        version: '^0.0.1',
    },
    'url-parser-browser': {
        package: '@aws/url-parser-browser',
        version: '^0.0.1',
    },
    'url-parser-node': {
        package: '@aws/url-parser-node',
        version: '^0.0.1',
    },
    'url-parser-universal': {
        package: '@aws/url-parser-universal',
        version: '^0.0.1',
    },
    'util-base64-browser': {
        package: '@aws/util-base64-browser',
        version: '^0.0.1',
    },
    'util-base64-node': {
        package: '@aws/util-base64-node',
        version: '^0.0.1',
    },
    'util-base64-universal': {
        package: '@aws/util-base64-universal',
        version: '^0.0.1',
    },
    'util-body-length-browser': {
        package: '@aws/util-body-length-browser',
        version: '^0.0.1',
    },
    'util-body-length-node': {
        package: '@aws/util-body-length-node',
        version: '^0.0.1',
    },
    'util-buffer-from': {
        package: '@aws/util-buffer-from',
        version: '^0.0.1',
    },
    'util-error-constructor': {
        package: '@aws/util-error-constructor',
        version: '^0.0.1',
    },
    'util-format-url': {
        package: '@aws/util-format-url',
        version: '^0.0.1',
    },
    'util-hex-encoding': {
        package: '@aws/util-hex-encoding',
        version: '^0.0.1',
    },
    'util-locate-window': {
        package: '@aws/util-locate-window',
        version: '^0.0.1',
    },
    'util-uri-escape': {
        package: '@aws/util-uri-escape',
        version: '^0.0.1',
    },
    'util-utf8-browser': {
        package: '@aws/util-utf8-browser',
        version: '^0.0.1',
    },
    'util-utf8-node': {
        package: '@aws/util-utf8-node',
        version: '^0.0.1',
    },
    'util-utf8-universal': {
        package: '@aws/util-utf8-universal',
        version: '^0.0.1',
    },
    'xml-body-builder': {
        package: '@aws/xml-body-builder',
        version: '^0.0.1',
    },
    'xml-body-parser': {
        package: '@aws/xml-body-parser',
        version: '^0.0.1',
    },
    'xml-builder': {
        package: '@aws/xml-builder',
        version: '^0.0.1',
    },
};
