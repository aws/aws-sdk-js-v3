...
export function serializerMiddleware(
  ...
  return {
    ...
    )}.serializerMiddleware(this.config.serializer)`,
...
function serializerProperty(
  ...
  switch (metadata.protocol) {
    ...
    const promisified = configuration.endpoint()
        .then(endpoint => new ${packageNameToVariable(
          "@aws-sdk/protocol-json-rpc"
        )}.JsonRpcSerializer<${streamType}>(
            endpoint,
...
export function streamCollectorProperty(
  ...
  return {
    ...
    universal: {
      ...
      default: {
        type: "value",
        expression: `${packageNameToVariable("@aws-sdk/is-node")}.isNode
