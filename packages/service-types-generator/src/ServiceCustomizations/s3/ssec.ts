...
function ssecProperties(props: SsecPropertiesConfiguration<any>): string {
    const propExpressions = Object.keys(props).map(propName =>
`${propName}: {
...
const keyType = `${packageNameToVariable('@aws/types')}.SourceData`;
...
const ssecKeyProperty: SyntheticParameterCustomizationDefinition = {
    ...
    documentation: "Specifies the customer-provided encryption key for Amazon S3 to use in encrypting data. This value is used to store the object and then it is discarded; Amazon does not store the encryption key. The key must be appropriate for use with the algorithm specified in the `SSECustomerAlgorithm` parameter",
};
