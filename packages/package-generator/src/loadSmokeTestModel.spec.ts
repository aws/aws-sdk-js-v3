...
describe('loadSmokeTestModel', () => {
    ...
    it('should load a smoke.json file from the given directory if present', () => {
        ...
        (readFileSync as any).mockImplementation(() => Buffer.from(JSON.stringify(
            mockModel, null, 4
        )));
    ...
    it(
        `should load a smoke.json file from the given directory's parent if not found at given location`,
        () => {
    ...
    it(
        `should return undefined if no smoke.json found`,
        () => {
