...
describe("importModule", () => {
  ...
  it("should create a temporary directory to house the module during generation", () => {
  ...
  it("should create subdirectories in the temporary directory as necessary", () => {
    ...
    const generator: ModuleGenerator = new Map([
      ...
      [join("deeply", "nested", "sub", "dir", "otherFile.ext"), "content"],
  ...
  it("should write all files yielded by the generator to a temporary directory", () => {
    importModule(generator);

    expect((writeFileSync as any).mock.calls).toEqual(
      [...generator].map(([filename, contents]) => [
  ...
  it("should move the temporary directory into the repository as a package with the provided name", () => {
    importModule(generator);
