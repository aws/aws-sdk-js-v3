...
import { parseUrl } from '@aws/url-parser-universal';

describe('modeledEndpointMiddleware', () => {
    it(
        'should use an element of the operation input as the request endpoint',
        async () => {
            ...
            await modeledEndpointMiddleware(parseUrl, 'endpoint')
                (next, { model: { http: { requestUri: '/path/to/operation' } } } as any)
                (args);
    ...
    it('should preserve interpolated path parameters', async () => {
        ...
        await modeledEndpointMiddleware(parseUrl, 'endpoint')
            (next, { model: { http: { requestUri: '/path/to/operation/{interpolateMe}/static' } } } as any)
            (args);
    ...
    it('should handle greedy path parameters', async () => {
        const args: BuildHandlerArguments<{endpoint: string}, any> = {
            ...
            request: {
                ...
                protocol: 'https:',
                hostname: 'service.region.amazonaws.com',
                path: '/endpoint/preamble/static/urlParam/static/many/additional/path/elements',
                port: 1234,
            },
        };
        const next = jest.fn(() => {});
