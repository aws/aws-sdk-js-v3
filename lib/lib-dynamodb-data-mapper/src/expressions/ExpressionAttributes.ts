...
import { marshall as marshallSDK } from "@aws-sdk/util-dynamodb";

import { AttributePath } from "./AttributePath";
import { AttributeValue } from "./AttributeValue";

export type ExpressionAttributeNameMap = { [key: string]: string };
export type ExpressionAttributeValueMap = { [key: string]: AttributeValueModel };
...
export class ExpressionAttributes {
  ...
  addValue(value: any): string {
    const attrValue = AttributeValue.isAttributeValue(value)
      ? value
      : new AttributeValue(marshallSDK(value) as AttributeValueModel);
