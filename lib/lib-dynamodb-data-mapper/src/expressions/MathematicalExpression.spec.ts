...
import { ExpressionAttributeNameMap, ExpressionAttributeValueMap } from "./ExpressionAttributes";
...
describe("MathematicalExpression", () => {
  ...
  > = [
    ...
    [
      new MathematicalExpression(new FunctionExpression("if_not_exists", new AttributePath("current_id"), 0), "+", 1),
  ...
  describe("::isMathematicalExpression", () => {
    ...
    it("should reject non-matching values", () => {
      for (const notMathematicalExpression of [
        ...
        expect(MathematicalExpression.isMathematicalExpression(notMathematicalExpression)).toBe(false);
      }
    });
  });
