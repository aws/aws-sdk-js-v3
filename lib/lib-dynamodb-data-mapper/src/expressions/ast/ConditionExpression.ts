...
export function lessThanOrEqualTo(operand: ComparisonOperand): LessThanOrEqualToExpressionPredicate {
  return {
    type: "LessThanOrEqualTo",
    object: operand,
  };
...
export function greaterThanOrEqualTo(operand: ComparisonOperand): GreaterThanOrEqualToExpressionPredicate {
  return {
    type: "GreaterThanOrEqualTo",
    object: operand,
  };
...
export function between(lowerBound: ComparisonOperand, upperBound: ComparisonOperand): BetweenExpressionPredicate {
  return {
    type: "Between",
    lowerBound,
    upperBound,
...
export function inList(...operands: Array<ComparisonOperand>): MembershipExpressionPredicate {
  return {
    type: "Membership",
    values: operands,
  };
 ...
 */
export function serializeConditionExpression(condition: ConditionExpression, attributes: ExpressionAttributes): string {
  ...
  switch (condition.type) {
    ...
    case "Function":
      ...
      switch (condition.name) {
        ...
        case "begins_with":
        case "contains":
          return new FunctionExpression(condition.name, subject, condition.expected).serialize(attributes);
      }
    case "Not":
...
function serializeOperand(operand: ComparisonOperand, attributes: ExpressionAttributes): string {
  ...
  return AttributePath.isAttributePath(operand) ? attributes.addName(operand) : attributes.addValue(operand);
}
