...
The `destroy()` call on document client is will throw an error as document client
does not create a new DynamoDB client. You need to call `destroy()` on DynamoDB client
to clean resources used by it as shown below.
...
ddbDocClient.destroy(); // throws error.
