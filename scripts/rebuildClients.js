...
console.info(
  ...
  existingServiceClients.map(item => path.parse(item).base)
);
...
function isClientPackage(directory) {
  ...
  try {
    ...
    for (const clientFilePattern of clientFiles) {
      ...
      if (
        !matchedFile &&
        fs.statSync(path.join(directory, matchedFile)).isFile() !==
          clientFilePattern.isFile
      ) {
        return false;
      }
    }
...
function generateClient(models, runtime) {
  const command = `node packages/package-generator/build/cli.js client --m ${
    models.service
  } ${models.smoke ? `--s ${models.smoke}` : ""} --r ${runtime}`;
