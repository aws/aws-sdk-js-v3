...
for (const [sdkId, examplesFile] of Object.entries(sdkIdToExamples)) {
  ...
  const serviceParam = process.argv.indexOf("--service");
  if (serviceParam !== -1 && id !== process.argv[serviceParam + 1]) {
    continue;
  }
  console.log("processing examples for", id);
  const commandsFolder = join(__dirname, "..", "..", "clients", `client-${id}`, "src", "commands");
...
`.split("\n")
        ...
        const final = `${buffer.map((line) => ` * ${line}`).join("\n")}
*/`;
