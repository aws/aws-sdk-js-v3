...
const clientModuleIdentifier = require("../../packages/package-generator/build/clientModuleIdentifier")
...
class ServiceModelFinder {
  ...
  fetchLatestModels(modelsDir) {
    ...
    for (const modelName of fs.readdirSync(modelsDir)) {
      ...
      const versions = fs
        ...
        .filter(version =>
          fs.statSync(path.join(modelDir, version)).isDirectory()
        );
  ...
  findModelsForService(packageName) {
    ...
    for (const latestModel of this.latestModels.slice(
      this.loadedModelCache.size
    )) {
