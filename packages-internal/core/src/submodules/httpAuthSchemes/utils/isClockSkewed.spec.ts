...
describe(isClockSkewed.name, () => {
  ...
  describe("returns true for time difference >=240000", () => {
    it.each([240000, 500000])("difference: %d", (difference) => {
      expect(isClockSkewed(mockSkewCorrectedDate.getTime() + difference, mockSystemClockOffset)).toBe(true);
      expect(isClockSkewed(mockSkewCorrectedDate.getTime() - difference, mockSystemClockOffset)).toBe(true);
    });
  });

  describe("returns false for time difference <240000", () => {
    it.each([239999, 100000, 0])("difference: %d", (difference) => {
